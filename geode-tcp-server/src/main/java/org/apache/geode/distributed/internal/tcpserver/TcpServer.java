  ...
  private void initializeServerSocket() throws IOException {
    if (srv_sock == null || srv_sock.isClosed()) {
      if (bind_address == null) {
        srv_sock = socketCreator.forServer().createServerSocket(port, backlogLimit);
        bind_address = srv_sock.getInetAddress();
      } else {
        srv_sock = socketCreator.forServer().createServerSocket(port, backlogLimit, bind_address);
      }
  ...
  private void processRequest(final Socket socket) {
    executor.execute(() -> {
      ...
      try {
        ...
        socketCreator.forServer().handshakeIfSocketIsSSL(socket, readTimeout);
