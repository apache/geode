...
public class ModuleSpecUtils {
      ...
      if (!entry.isEmpty()) {
        try {
          ...
          if (resourceJar.exists()) {
            // the JarFile is not closed because JBoss requires that it not be.
  ...
  public static ModuleSpec addModuleDependencyToSpec(ModuleSpec moduleSpec,
    ...
    return addModuleDependencyToSpec(moduleSpec, true, dependencyNames);
  }

  public static ModuleSpec addModuleDependencyToSpec(ModuleSpec moduleSpec, boolean export,
      String... dependencyNames) {
    validate(moduleSpec);
    ...
    ModuleSpec.Builder builder = createBuilder(moduleSpec);
    return ModuleSpecBuilderUtils.addModuleDependencies(builder, export, dependencyNames).create();
  }
