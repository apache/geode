...
public class DeployJarAcceptanceTest extends AbstractDockerizedAcceptanceTest {
  ...
  public void testDeployModuleWithInvalidDependencies() throws IOException {
    String outputText = GfshScript
        ...
        .execute(gfshRule).getOutputText();
    if (isModular()) {
      assertThat(outputText).contains("Cannot find module: invalid.module");
    } else {
  ...
  public void testUpdatePojoVersion() throws IOException {
    ...
    assertThat(GfshScript.of(getLocatorGFSHConnectionString(),
        "list deployed").execute(gfshRule).getOutputText()).contains(pojoVersion2Jar.getName())
            .doesNotContain(pojoVersion1Jar.getName());
