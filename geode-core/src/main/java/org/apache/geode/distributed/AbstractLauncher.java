...
public abstract class AbstractLauncher<T extends Comparable<T>> implements Runnable {
  ...
  public static final String SIGNAL_HANDLER_REGISTRATION_SYSTEM_PROPERTY = GEMFIRE_PREFIX + "launcher.registerSignalHandlers";
  ...
  protected static void assertPortAvailable(final InetAddress bindAddress, final int port) throws BindException {
    if (!AvailablePort.isPortAvailable(port, AvailablePort.SOCKET, bindAddress)) {
      throw new BindException(String.format("Network is unreachable; port (%1$d) is not available on %2$s.", port, (bindAddress != null ? bindAddress
    ...
    if (url != null) {
      try {
        ...
        try {
          ...
          String filename = DistributedSystem.getPropertiesFile();
          try {
            properties.load(AbstractLauncher.class.getResourceAsStream(filename));
          } catch (Exception ignore) {
            filename = GEMFIRE_PREFIX + ".properties";
          }
          properties.load(AbstractLauncher.class.getResourceAsStream(filename));
        } catch (Exception ignore) {
  ...
  public static enum Status {
    NOT_RESPONDING(LocalizedStrings.Launcher_Status_NOT_RESPONDING.toLocalizedString()), ONLINE(LocalizedStrings.Launcher_Status_ONLINE
      .toLocalizedString()), STARTING(LocalizedStrings.Launcher_Status_STARTING.toLocalizedString()), STOPPED(LocalizedStrings.Launcher_Status_STOPPED
      .toLocalizedString());
  ...
  public static abstract class ServiceState<T extends Comparable<T>> {
    ...
    public String toString() {
      switch (getStatus()) {
        case STARTING:
          return LocalizedStrings.Launcher_ServiceStatus_STARTING_MESSAGE.toLocalizedString(getServiceName(), getWorkingDirectory(), getServiceLocation(), getMemberName(), toString(getTimestamp()), toString(getPid()), toString(getGemFireVersion()), toString(getJavaVersion()), getLogFile(), toString(getJvmArguments()
            .toArray()), toString(getClasspath()));
        case ONLINE:
          return LocalizedStrings.Launcher_ServiceStatus_RUNNING_MESSAGE.toLocalizedString(getServiceName(), getWorkingDirectory(), getServiceLocation(), getMemberName(), getStatus(), toString(getPid()), toDaysHoursMinutesSeconds(getUptime()), toString(getGemFireVersion()), toString(getJavaVersion()), getLogFile(), toString(getJvmArguments()
            .toArray()), toString(getClasspath()));
        case STOPPED:
