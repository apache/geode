...
public class OpExecutorImpl implements ExecutablePool {
  ...
  public OpExecutorImpl(final ConnectionManager connectionManager,
      final QueueManager queueManager,
      final EndpointManager endpointManager,
      final RegisterInterestTracker riTracker, final int retryAttempts,
      final long serverTimeout, final long singleServerTimeout,
      final CancelCriterion cancelCriterion,
      final PoolImpl pool) {
  ...
  protected Object executeOnServer(ServerLocation server, Op op, boolean accessed,
    ...
    if (op instanceof PingOp.PingOpImpl) {
      ...
      if (queueManager != null) {
        ...
        // the ping on.
        Endpoint ep = endpointManager.getEndpointMap().get(server);
        if (ep != null) {
          QueueConnections qcs = queueManager.getAllConnectionsNoWait();
          connection = qcs.getConnection(ep);
          if (connection != null) {
