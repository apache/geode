...
class RegionPerfStats extends CachePerfStats {
  ...
  RegionPerfStats(StatisticsFactory statisticsFactory, CachePerfStats cachePerfStats,
    ...
    this(statisticsFactory, cachePerfStats, regionName, dataPolicy, meterRegistry,
        enableClockStats ? NanoTimer::getTime : () -> 0);
  }

  @VisibleForTesting
  RegionPerfStats(StatisticsFactory statisticsFactory, CachePerfStats cachePerfStats,
      String regionName, DataPolicy dataPolicy, MeterRegistry meterRegistry, Clock clock) {
    super(statisticsFactory, regionName, clock);
    this.cachePerfStats = cachePerfStats;

    putTimer = Timer
        .builder("cache.region.operations")
  ...
  public void endGet(long start, boolean miss) {
    if (enableClockStats) {
      ...
      getTimer.record(totalNanos, TimeUnit.NANOSECONDS);
    }
  ...
  public long endPut(long start, boolean isUpdate) {
    ...
    if (isUpdate) {
      ...
      if (enableClockStats) {
        ...
        putTimer.record(totalNanos, TimeUnit.NANOSECONDS);
      }
    } else {
