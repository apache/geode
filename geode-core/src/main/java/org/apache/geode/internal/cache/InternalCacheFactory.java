...
/**
 * Extracted from {@link CacheFactory} which now delegates to {@code InternalCacheFactory}.
 */
public class InternalCacheFactory {
   ...
   * Creates a cache factory with default configuration properties.
   */
  public InternalCacheFactory(CacheConfig cacheConfig) {
    this(new Properties(), cacheConfig);
  }

  private InternalCacheFactory(Properties configProperties, CacheConfig cacheConfig) {
  ...
  @VisibleForTesting
  InternalCacheFactory(Properties configProperties,
   ...
   *
   *      If a problem occurs while parsing the declarative caching XML file.
   *      If a {@link Region#put(Object, Object)} times out while initializing the cache.
   *      If a {@code CacheWriterException} is thrown while initializing the cache.
   *      If a {@code GatewayException} is thrown while initializing the cache.
   *      If the declarative caching XML file describes a region that already exists (including
   *      the root region).
   *      if cache already exists and is not compatible with the new configuration.
   *      if authentication fails.
   *      if the distributed system is in secure mode and this new member is not configured with
   ...
   *
   *      If {@code system} is not {@link DistributedSystem#isConnected connected}.
   ...
   *      the root region).
   *
   * @deprecated Please use {@link #InternalCacheFactory(Properties)} instead.
   */
  @Deprecated
  public InternalCache create(InternalDistributedSystem internalDistributedSystem)
    ...
    try {
      synchronized (InternalCacheFactory.class) {
        synchronized (GemFireCacheImpl.class) {
          ...
          if (cache == null) {
            ...
            // TODO: new CompositeMeterRegistry code goes here
            /*
             ...
             * CacheMetricsSession.create(meterRegistry, systemId, memberName);
             */
      ...
      // TODO: log the full stack trace or not?
   ...
   */
  public InternalCacheFactory set(String name, String value) {
   ...
   */
  public InternalCacheFactory setPdxReadSerialized(boolean readSerialized) {
   ...
   */
  public InternalCacheFactory setSecurityManager(SecurityManager securityManager) {
   ...
   */
  public InternalCacheFactory setPostProcessor(PostProcessor postProcessor) {
   ...
   */
  public InternalCacheFactory setPdxSerializer(PdxSerializer serializer) {
   ...
   */
  public InternalCacheFactory setPdxDiskStore(String diskStoreName) {
   ...
   */
  public InternalCacheFactory setPdxPersistent(boolean isPersistent) {
   ...
   */
  public InternalCacheFactory setPdxIgnoreUnreadFields(boolean ignore) {
    cacheConfig.setPdxIgnoreUnreadFields(ignore);
    return this;
  }

  public InternalCacheFactory setCacheXMLDescription(String cacheXML) {
    if (cacheXML != null) {
   ...
   */
  public InternalCacheFactory setIsExistingOk(boolean isExistingOk) {
   ...
   */
  public InternalCacheFactory setIsClient(boolean isClient) {
   ...
   */
  public InternalCacheFactory setUseAsyncEventListeners(boolean useAsyncEventListeners) {
   ...
   */
  public InternalCacheFactory setPoolFactory(PoolFactory poolFactory) {
   ...
   */
  public InternalCacheFactory setTypeRegistry(TypeRegistry typeRegistry) {
