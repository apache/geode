...
public class ClassPathLoader {
  ...
  public static void initialize(File workingDir) {
    if (latest == null) {
      ...
      latest.jarDeploymentService.reinitializeWithWorkingDirectory(workingDir);
    }
  }

  public static void close() {
    if (latest != null) {
      latest = new ClassPathLoader(Boolean.getBoolean(EXCLUDE_TCCL_PROPERTY), null);
    }
  }
  ...
  public Class<?> forName(final Class<?> contextClass, String name) throws ClassNotFoundException {
    return classPathService.forName(contextClass, name);
  }
