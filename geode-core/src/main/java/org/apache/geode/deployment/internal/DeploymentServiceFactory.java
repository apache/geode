...
public class DeploymentServiceFactory {
  ...
  private static synchronized JarDeploymentService createJarDeploymentService() {
  ...
  private static ClasspathService createClassPathService() {
    ServiceLoader<ClasspathService> classpathServices =
        ServiceLoader.load(ClasspathService.class);
    if (classpathServices.iterator().hasNext()) {
      return classpathServices.iterator().next();
    } else {
      throw new ServiceLoadingFailureException(
