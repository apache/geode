...
public class MsgStreamer extends AbstractDataOutputStream implements
        BaseMsgStreamer, ByteBufferWriter {
    ...
    public static BaseMsgStreamer create(List<?> cons,
        ...
        if (numCons > 1) {
            ...
            if (versionToConnMap == null) {
                ...
                for (ObjectIterator itr = versionToConnMap.object2ObjectEntrySet().fastIterator(); itr.hasNext(); ) {
...
//    if (logger.isTraceEnabled()) logger.trace(" byte={}", b);
    ...
    private void realFlush(boolean lastFlushForMessage) {
        if (isOverflowMode()) {
            if (this.overflowBuf == null) {
    ...
    public final void close() throws IOException {
        try {
            if (this.startedSerializingMsg && !this.doneWritingMsg) {
                ...
                if (this.flushedBytes > 0) {
                    for (Object con1 : this.cons) {
                        Connection con = (Connection) con1;
