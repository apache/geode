...
public class Message {
     ...
     */
    public static int MAX_MESSAGE_SIZE = Integer.getInteger(DistributionConfig.GEMFIRE_PREFIX
            + "client.max-message-size", DEFAULT_MAX_MESSAGE_SIZE);
    ...
    private static final ThreadLocal<ByteBuffer> TL_COMM_BUFFER = new ThreadLocal<>();
    ...
    public static ByteBuffer setTLCommBuffer(ByteBuffer bb) {
        ByteBuffer result = TL_COMM_BUFFER.get();
        TL_COMM_BUFFER.set(bb);
        return result;
    }
     ...
     */
    final void fetchHeader() throws IOException {
        final ByteBuffer cb = getCommBuffer();
        cb.clear();
    ...
    void readPayloadFields(final int numParts, final int len)
            throws IOException {
        if (len > 0 && numParts <= 0 ||
                len <= 0 && numParts > 0) {
