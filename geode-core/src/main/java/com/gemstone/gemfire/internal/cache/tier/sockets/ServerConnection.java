...
public class ServerConnection implements Runnable {
    ...
    public static void emptyCommBufferPool() {
        commBufferMap.values().forEach(LinkedBlockingQueue::clear);
    }
    ...
    private static short testVersionAfterHandshake = 4;
    ...
    public ServerConnection(
        ...
        buffer.append(communicationModeStr)
                .append(" host address=")
    ...
    static private final ThreadLocal<Byte> executeFunctionOnLocalNodeOnly =
...
//        + "   diff=" + (oldIdArray.length - memberIdByteArray.length));
    ...
    long setUserAuthorizeAndPostAuthorizeRequest(
    ...
    void refuseHandshake(String msg, byte exception) {
        try {
...
//  }
    ...
    private void doNormalMsg() {
        ...
        try {
            if (msg != null) {
                ...
                Command command = getCommand(msg.getMessageType());
                if (command == null) {
    ...
    private Command getCommand(Integer messageType) {
        ...
        Command cc = this.commands.get(messageType);
        return cc;
    }
    ...
    public byte[] setCredentials(Message msg)
        ...
        } catch (AuthenticationFailedException | AuthenticationRequiredException afe) {
            throw afe;
        } catch (Exception e) {
...
//  }
    ...
    public String getSocketString() {
        try {
            return valueOf(theSocket.getInetAddress()) +
                    ':' +
                    theSocket.getPort() +
                    " timeout: " +
    ...
    private static class AuthIds {
        ...
        AuthIds(byte[] bytes) throws Exception {
    ...
    private UserAuthAttributes getUserAuthAttributes(final long uniqueId) throws IOException {
    ...
    public AuthorizeRequest getAuthzRequest()
        ...
        UserAuthAttributes uaa = getUserAuthAttributes(uniqueId);
        if (uaa == null) {
            throw new AuthenticationRequiredException(
