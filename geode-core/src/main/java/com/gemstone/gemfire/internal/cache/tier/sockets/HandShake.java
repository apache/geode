...
//This assumes that authentication is the last piece of info in handshake
  public Properties readCredential(DataInputStream dis, DataOutputStream dos, DistributedSystem system)
    ...
    boolean requireAuthentication = GeodeSecurityUtil.isClientSecurityRequired(system.getSecurityProperties());
    try {
  ...
  public static void initDHKeys(DistributionConfig config) throws Exception {
    ...
    if ((dhSKAlgo != null && dhSKAlgo.length() > 0)
        || GeodeSecurityUtil.isClientSecurityRequired(config.getSecurityProps())) {
