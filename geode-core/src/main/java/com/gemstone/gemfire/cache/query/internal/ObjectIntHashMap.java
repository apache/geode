 ...
 * <tt>Map</tt> interface. This implementation provides all of the optional map
 * operations, and supports ONLY int primitive type values, stored in primitive
 * field of int type instead of promoting them to Integer and the <tt>null</tt>
 * key. The default for a value is 0. (The <tt>ObjectIntHashMap</tt> class is
 * roughly equivalent to <tt>HashMap</tt>, except that it is unsynchronized and
 * permits nulls.) This class makes no guarantees as to the order of the map; in
...
public class ObjectIntHashMap implements Cloneable, Serializable {
  ...
  private static final long serialVersionUID = 7718697444988416372L;
     ...
     * because IntHashMap uses power-of-two length hash tables, that
