...
public class PutOp {
            ...
            if (server != null) {
                try {
                    ...
                    boolean onlyUseExistingCnx = ((poolImpl.getMaxConnections() != -1 && poolImpl
                            .getConnectionCount() >= poolImpl.getMaxConnections()));
    ...
    private static class PutOpImpl extends AbstractOp {
        ...
        PutOpImpl(String regionName, Object key, Object value, byte[] deltaBytes,
                  EntryEventImpl event,
...
//          }
         ...
         * @throws Exception if response could not be processed or we received a
         *                   response with a server exception.
         * @since GemFire 6.1
         */
        private void processAck(Message msg, String opName, Connection con)
                throws Exception {
        ...
        protected boolean needsUserId() {
            return !this.isMetaRegionPutOp && super.needsUserId();
        }
