...
public class MetricsIntegrationTest {
  ...
  @Test
  public void newMetersAreForwardedToDownstreamRegistries() {
    MetricsCollector metricsCollector =
        ((InternalDistributedSystem) cache.getDistributedSystem()).getMetricsCollector();
    MeterRegistry downstreamRegistry = new SimpleMeterRegistry();
    metricsCollector.addDownstreamRegistry(downstreamRegistry);

    String gaugeName = "test.region.size";

    Gauge.builder(gaugeName, region, Region::size)
        .tag("region.name", region.getName())
        .register(metricsCollector.primaryRegistry());
