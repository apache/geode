...
public class DeprecatedAgentLauncherIntegrationTest {
  ...
  private static void runAgent(final String processOutputPattern, final String... args)
    ...
    assertThat(agentProcess.waitFor(TIMEOUT)).as("Expecting process started with:" + LINE_SEPARATOR
        + " " + Arrays.asList(args) + LINE_SEPARATOR + "with output:" + LINE_SEPARATOR + " "
        + agentProcess.getOutput(true) + LINE_SEPARATOR + "to terminate with exit code: 0"
        + LINE_SEPARATOR + "but waitFor is still waiting after timeout: " + TIMEOUT + " seconds.")
        .isEqualTo(0);
  }
