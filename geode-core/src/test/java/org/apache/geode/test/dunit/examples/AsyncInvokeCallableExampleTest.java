...
public class AsyncInvokeCallableExampleTest {
  ...
  public void invokeAsyncAsFutureWithTimeout() throws Exception {
    VM workerVM = Host.getHost(0).getVM(0);
    boolean success = workerVM.invokeAsync(() -> longRunningWorkWithResult()).get(1, TimeUnit.MINUTES);
    assertThat(success).isTrue();
  }
  ...
  public void invokeAsyncWithExceptionOccurred() throws Exception {
    ...
    AsyncInvocation<Boolean> asyncInvocation = workerVM.invokeAsync(() -> longRunningWorkThatThrowsException());
  ...
  public void invokeAsyncWithAwaitWithTimeout() throws Exception {
    Host.getHost(0).getVM(0).invokeAsync(() -> longRunningWorkThatThrowsException()).await(1, TimeUnit.MINUTES);
  }
