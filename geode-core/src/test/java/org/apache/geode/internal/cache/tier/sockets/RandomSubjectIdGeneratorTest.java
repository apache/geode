...
class RandomSubjectIdGeneratorTest {
  ...
  void returnsNextRandomLong() {
    ...
    assertThat(generatedIds)
        .isEqualTo(randomLongs);
  }

  @Test
  void skipsRandomLongIfNotAValidUserId() {
    long validSubjectId = 9034789L;
    when(random.nextLong())
        .thenReturn(-1L) // -1 is not a valid user ID
        .thenReturn(-1L)
        .thenReturn(-1L)
        .thenReturn(-1L)
        .thenReturn(-1L)
        .thenReturn(-1L)
        .thenReturn(validSubjectId);
  ...
  void reportsIdExhaustionIfRandomRepeatsFirstLong() {
    ...
    long generatedId = generator.generateId(); // Random repeats first random long

    assertThat(generatedId)
        .isEqualTo(-1); // Generator reports ID exhaustion by returning -1
  }
  ...
  void continuesWithReinitializedRandomAfterIdExhaustion() {
    ...
    List<Long> idsGeneratedAfterIdExhaustion = new ArrayList<>();
    generator.generateId(); // Remembers and returns first random long
    ...
    assertThat(idsGeneratedAfterIdExhaustion)
        .isEqualTo(randomLongsAfterIdExhaustion);
  }
}
