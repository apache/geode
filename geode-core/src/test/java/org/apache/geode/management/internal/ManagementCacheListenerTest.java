...
/**
 * Unit tests for {@code ManagementCacheListener} (ie the SUT). These are characterization tests
...
public class ManagementCacheListenerTest {
  ...
  public void setUp() throws Exception {
    ...
    monitoringEvent = uncheckedCast(mock(EntryEvent.class));
    ...
    proxy = mock(ProxyInterface.class);
    region = uncheckedCast(mock(Region.class));
  ...
  public void afterCreate_doesNothing_ifNotReady() {
    ...
    verify(mBeanProxyFactory, never())
        .createProxy(
            same(distributedMember),
            eq(objectName),
            same(region),
  ...
  /**
   * This behavior seems unintentional. It may be harmless.
   */
  @Test
  public void afterDestroy_removesProxy_ifNotReady() {
    when(monitoringEvent.getDistributedMember())
        .thenReturn(distributedMember);
    when(monitoringEvent.getKey())
        .thenReturn(OBJECT_NAME_KEY);
  ...
  @Test
  public void afterUpdate_updatesProxy() {
  ...
  @Test
  public void afterUpdate_updatesLastRefreshedTime() {
    when(mBeanProxyFactory.findProxyInfo(eq(objectName)))
        .thenReturn(proxyInfo);
    when(monitoringEvent.getKey())
        .thenReturn(OBJECT_NAME_KEY);
