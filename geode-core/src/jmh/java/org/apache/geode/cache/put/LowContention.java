...
public class LowContention {
  ...
  public static class CacheState {
    ...
    public void setup() {
      ...
      replicateRegion =
          cache.createRegionFactory(RegionShortcut.REPLICATE).create("ReplicateRegion");
      localRegion = cache.createRegionFactory(RegionShortcut.LOCAL).create("LocalRegion");
      partitionedRegion =
          cache.createRegionFactory(RegionShortcut.PARTITION).create("PartitionedRegion");
    }
  }
  ...
  public Object replicate(CacheState state) {
    return state.replicateRegion.put(getKey(), "value");
  }
  ...
  public Object local(CacheState state) {
    return state.localRegion.put(getKey(), "value");
  }
  ...
  public Object partition(CacheState state) {
    return state.partitionedRegion.put(getKey(), "value");
  }
  ...
  @Warmup(iterations = 1)
  @Measurement(iterations = 5)
  public void noop(CacheState state) {
    return;
  }
