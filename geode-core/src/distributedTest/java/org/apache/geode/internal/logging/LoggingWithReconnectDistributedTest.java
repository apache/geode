...
public class LoggingWithReconnectDistributedTest implements Serializable {
  ...
  public void logFileContainsBannerOnlyOnce() {
    ...
    server2VM.invoke(() -> {
      ...
      String[] startupConfiguration = StringUtils
          .split(STARTUP_CONFIGURATION + system.getConfig().toLoggerString(), LINE_SEPARATOR);
  ...
  private String expectedOneLogFile(File[] files) {
    return "Expecting directory:" + LINE_SEPARATOR + " " + server2Dir.getAbsolutePath()
        + LINE_SEPARATOR + "to contain only one log file:" + LINE_SEPARATOR + " " + server2Name
        + ".log" + LINE_SEPARATOR + "but found multiple log files:" + LINE_SEPARATOR + " "
        + Arrays.asList(files);
  }
