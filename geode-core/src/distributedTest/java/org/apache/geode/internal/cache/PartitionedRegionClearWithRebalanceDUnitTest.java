...
public class PartitionedRegionClearWithRebalanceDUnitTest implements Serializable {
  ...
  public void clearRegionStartedAfterRebalanceClearsRegion(TestVM clearCoordinatorVM,
    ...
    parametrizedSetup(regionType, singleton(REGION_NAME), true);
  ...
  public void clearParentColocatedRegionStartedAfterRebalanceOfColocatedRegionsClearsRegionAndDoesNotInterfereWithRebalance(
      TestVM clearCoordinatorVM, RegionShortcut regionType) throws InterruptedException {
  ...
  public void clearStartedBeforeRebalanceClearsRegionWhenNonCoordinatorMemberIsKilled(
      TestVM clearCoordinatorVM, RegionShortcut regionType) throws InterruptedException {
    parametrizedSetup(regionType, singleton(REGION_NAME), false);
  ...
  public void clearStartedAfterRebalanceClearsRegionWhenNewMemberJoins(TestVM clearCoordinatorVM,
    ...
    // system
    server1.invoke(() -> {
      initDataStore(regionType, singleton(REGION_NAME));
      Region<String, String> region = cacheRule.getCache().getRegion(REGION_NAME);
      range(0, ENTRIES).forEach(i -> region.put("key" + i, "value" + i));
    });

    server2.invoke(() -> initDataStore(regionType, singleton(REGION_NAME)));
    ...
    AsyncInvocation<?> createRegion = server3.invokeAsync(() -> {
      ...
      RegionFactory<String, String> factory = cacheRule.getCache()
          ...
          .setCacheWriter(new BlackboardSignaller(blackboard));
      ...
      blackboard.waitForGate(REBALANCE_HAS_BEGUN, getTimeout().toMillis(), MILLISECONDS);
  ...
  private void initAccessor(RegionShortcut regionShortcut, Iterable<String> regionNames) {
    ...
    regionNames.forEach(regionName -> {
      ...
      if (regionName.equals(COLOCATED_REGION)) {
        partitionAttributesFactory.setColocatedWith(REGION_NAME);
      }
  ...
  private void initDataStore(RegionShortcut regionShortcut, Iterable<String> regionNames) {
    // StartupRecoveryDelay is set to infinite to prevent automatic rebalancing when creating the
    // region on other members
    regionNames.forEach(regionName -> {
      PartitionAttributesFactory<String, String> attributesFactory =
  ...
  private void parametrizedSetup(RegionShortcut regionShortcut, Iterable<String> regionNames,
      boolean useAccessor) {
  ...
  private void setBlackboardSignallerCacheWriter(String regionName) {
    cacheRule.getCache().<String, String>getRegion(regionName)
  ...
  private void assertRebalanceDoesNoWork() {
    server1.invoke(() -> {
      ...
      assertThat(results.getTotalBucketTransfersCompleted())
          .as("Expected bucket transfers to be zero")
          .isEqualTo(0);
      assertThat(results.getTotalBucketCreatesCompleted())
          .as("Expected bucket creates to be zero")
          .isEqualTo(0);
      assertThat(results.getTotalPrimaryTransfersCompleted())
          .as("Expected primary transfers to be zero")
          .isEqualTo(0);
    });
  }
  ...
  private static class BlackboardSignaller extends CacheWriterAdapter<String, String> {

    private final Blackboard blackboard;

    private BlackboardSignaller(Blackboard blackboard) {
    ...
    public synchronized void beforeRegionClear(RegionEvent<String, String> event)
      ...
      blackboard.signalGate(CLEAR_HAS_BEGUN);
    }
  }
}
