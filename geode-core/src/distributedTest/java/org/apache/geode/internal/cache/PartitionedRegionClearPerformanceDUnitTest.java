...
public class PartitionedRegionClearPerformanceDUnitTest implements Serializable {
  ...
  public void testPerformance() {
    createRegionOnServers(RegionShortcut.PARTITION, 2, 113);
    populationRegion();
  ...
  private void createRegion(RegionShortcut shortcut, int redundancy, int numBuckets)
    ...
    cacheServer.start();
    cacheRule.getCache().createRegionFactory(shortcut).setPartitionAttributes(new PartitionAttributesFactory<>().setRedundantCopies(redundancy).setTotalNumBuckets(numBuckets).create()).create(REGION_NAME);
  }
  ...
  private void populationRegion() {
    client.invoke(() -> {
      ...
      Region region = clientCacheRule.getClientCache().createClientRegionFactory(ClientRegionShortcut.CACHING_PROXY).create(REGION_NAME);
