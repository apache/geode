...
public class ResumeAsyncEventQueueDUnitTest {
  ...
  public static final String CREATE_COMMAND = "create async-event-queue --listener=" + MyAsyncEventListener.class.getName();
  ...
  public void create_sync_event_queue() throws Exception {
    ...
    gfsh.executeAndAssertThat(CREATE_COMMAND + " --id=queue --start-paused").statusIsSuccess()
        .tableHasRowCount(1)
    ...
    gfsh.executeAndAssertThat(LIST_COMMAND).statusIsSuccess()
        .tableHasRowCount(1).tableHasRowWithValues("Member", "ID", "Start Paused", "Paused", "server-1", "queue", "true", "true");
    ...
    gfsh.executeAndAssertThat(LIST_COMMAND).statusIsSuccess()
        .tableHasRowCount(1).tableHasRowWithValues("Member", "ID", "Start Paused", "Paused", "server-1", "queue", "true", "false");
  }
}
