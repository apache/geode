...
public class DeveloperRestSecurityConfigurationDUnitTest {
  ...
  public void testWithSecurityManager() {
    ...
    assertResponse(client.doGet("/v3", "data", "data")).hasStatusCode(200);
    assertResponse(client.doGet("/v3/ping", "cluster", "cluster")).hasStatusCode(200);

    // secured no/incorrect credentials
    assertResponse(client.doGet("/v3", null, null)).hasStatusCode(401);
    assertResponse(client.doGet("/v3", "data", "invalid")).hasStatusCode(401);
  ...
  public void testWithoutSecurityManager() {
    ...
    // secured no/incorrect credentials
    assertResponse(client.doGet("/v3", null, null)).hasStatusCode(200);
    assertResponse(client.doGet("/v3", "data", "invalid")).hasStatusCode(200);
