...
public class PauseGatewaySenderCommandTest {
  ...
  private PauseGatewaySenderCommand command;
  ...
  @Before
  public void setUp() {
    command = spy(PauseGatewaySenderCommand.class);
    service =
  ...
  public void mandatoryOption() {
    gfsh.executeAndAssertThat(command, "pause gateway-sender").statusIsError()
        .containsOutput("Invalid command");
  }

  @Test
  public void pauseGatewaySenderSuccessfully() {
    ...
    doReturn(false).when(bean1).isPaused();

    gfsh.executeAndAssertThat(command, "pause gateway-sender --id=gatewaySender1").statusIsSuccess()
        .containsOutput("Cluster configuration for group 'cluster' is updated")
        .containsOutput("GatewaySender " + GATEWAY_SENDER_1 + " is paused on member " + MEMBER_1)
        .hasTableSection().hasColumn("Result")
        .containsExactlyInAnyOrder("OK");
  }

  @Test
  public void pauseGatewaySenderOnTwoMembersSuccessfully() {
    doReturn(locatorSystem).when(cache).getDistributedSystem();
    doReturn(locatorDistributedMember).when(locatorSystem).getDistributedMember();
    doReturn("locator").when(locatorDistributedMember).getId();
    ...
    doReturn(false).when(bean1).isPaused();
    doReturn(false).when(bean2).isPaused();


    gfsh.executeAndAssertThat(command, "pause gateway-sender --id=gatewaySender1").statusIsSuccess()
        .containsOutput("Cluster configuration for group 'cluster' is updated")
        .containsOutput("GatewaySender " + GATEWAY_SENDER_1 + " is paused on member " + MEMBER_1)
        .containsOutput("GatewaySender " + GATEWAY_SENDER_1 + " is paused on member " + MEMBER_2)
        .hasTableSection().hasColumn("Result")
        .containsExactlyInAnyOrder("OK", "OK");
  }

  @Test
  public void pauseGatewaySenderOnOneMembersSuccessfullyAndOneFail() {
    doReturn(locatorSystem).when(cache).getDistributedSystem();
    doReturn(locatorDistributedMember).when(locatorSystem).getDistributedMember();
    doReturn("locator").when(locatorDistributedMember).getId();
  ...
  @Test
  public void memberAlreadyPaused() {
    doReturn(locatorSystem).when(cache).getDistributedSystem();
    doReturn(locatorDistributedMember).when(locatorSystem).getDistributedMember();
    doReturn("locator").when(locatorDistributedMember).getId();
    ...
    doReturn(true).when(bean1).isRunning();
    doReturn(true).when(bean1).isPaused();

    gfsh.executeAndAssertThat(command, "pause gateway-sender --id=gatewaySender1").statusIsSuccess()
        .containsOutput("Cluster configuration for group 'cluster' is not updated")
        .containsOutput(
            "GatewaySender " + GATEWAY_SENDER_1 + " is already paused on member " + MEMBER_1)
        .hasTableSection().hasColumn("Result")
        .containsExactlyInAnyOrder("Error");
  }

  @Test
  public void memberIsNotRunning() {
    doReturn(locatorSystem).when(cache).getDistributedSystem();
    doReturn(locatorDistributedMember).when(locatorSystem).getDistributedMember();
    doReturn("locator").when(locatorDistributedMember).getId();
    ...
    gfsh.executeAndAssertThat(command, "pause gateway-sender --id=gatewaySender1").statusIsSuccess()
        .containsOutput(
            "GatewaySender " + GATEWAY_SENDER_1 + " is not running on member " + MEMBER_1)
        .containsOutput("Cluster configuration for group 'cluster' is not updated")
        .hasTableSection().hasColumn("Result")
        .containsExactlyInAnyOrder("Error");
  }
