...
public class ResumeGatewaySenderCommandTest {
  @ClassRule
  public static GfshParserRule gfsh = new GfshParserRule();

  private ResumeGatewaySenderCommand command;
  ...
  @Before
  public void setUp() {
    command = spy(ResumeGatewaySenderCommand.class);
    service =
  ...
  public void mandatoryOption() {
    gfsh.executeAndAssertThat(command, "resume gateway-sender").statusIsError()
        .containsOutput("Invalid command");
  }

  @Test
  public void resumeGatewaySenderSuccessfully() {
    ...
    gfsh.executeAndAssertThat(command, "resume gateway-sender --id=gatewaySender1")
        .statusIsSuccess()
        .containsOutput("Cluster configuration for group 'cluster' is updated")
        .containsOutput("GatewaySender " + GATEWAY_SENDER_1 + " is resumed on member " + MEMBER_1)
        .hasTableSection().hasColumn("Result")
        .containsExactlyInAnyOrder("OK");
  }

  @Test
  public void resumeGatewaySenderOnTwoMembersSuccessfully() {
    doReturn(locatorSystem).when(cache).getDistributedSystem();
    doReturn(locatorDistributedMember).when(locatorSystem).getDistributedMember();
    doReturn("locator").when(locatorDistributedMember).getId();
    ...
    doReturn(true).when(bean1).isPaused();
    doReturn(true).when(bean2).isPaused();

    gfsh.executeAndAssertThat(command, "resume gateway-sender --id=gatewaySender1")
        .statusIsSuccess()
        .containsOutput("Cluster configuration for group 'cluster' is updated")
        .containsOutput("GatewaySender " + GATEWAY_SENDER_1 + " is resumed on member " + MEMBER_1)
        .containsOutput("GatewaySender " + GATEWAY_SENDER_1 + " is resumed on member " + MEMBER_2)
        .hasTableSection().hasColumn("Result")
        .containsExactlyInAnyOrder("OK", "OK");
  }

  @Test
  public void resumeGatewaySenderOnOneMembersSuccessfullyAndOneFail() {
    doReturn(locatorSystem).when(cache).getDistributedSystem();
    doReturn(locatorDistributedMember).when(locatorSystem).getDistributedMember();
    doReturn("locator").when(locatorDistributedMember).getId();
  ...
  public void memberAlreadyStarted() {
    ...
    gfsh.executeAndAssertThat(command, "resume gateway-sender --id=gatewaySender1")
        ...
        .containsOutput(
            "GatewaySender " + GATEWAY_SENDER_1 + " is not paused on member " + MEMBER_1)
        .hasTableSection().hasColumn("Result")
