...
public class StopGatewaySenderCommandTest {
  ...
  public void mandatoryOption() {
    gfsh.executeAndAssertThat(command, "stop gateway-sender").statusIsError()
        .containsOutput("Invalid command");
  }

  @Test
  public void stopGatewaySenderSuccessfully() {
    ...
    gfsh.executeAndAssertThat(command, "stop gateway-sender --id=gatewaySender1").statusIsSuccess()
        .containsOutput("Cluster configuration for group 'cluster' is updated")
        .containsOutput("GatewaySender " + GATEWAY_SENDER_1 + " is stopped on member " + MEMBER_1)
        .hasTableSection().hasColumn("Result")
        .containsExactlyInAnyOrder("OK");
  }

  @Test
  public void stopGatewaySenderOnTwoMembersSuccessfully() {
    doReturn(locatorSystem).when(cache).getDistributedSystem();
    doReturn(locatorDistributedMember).when(locatorSystem).getDistributedMember();
    doReturn("locator").when(locatorDistributedMember).getId();
    ...
    gfsh.executeAndAssertThat(command, "stop gateway-sender --id=gatewaySender1").statusIsSuccess()
        .containsOutput("Cluster configuration for group 'cluster' is updated")
        .containsOutput("GatewaySender " + GATEWAY_SENDER_1 + " is stopped on member " + MEMBER_1)
        .containsOutput("GatewaySender " + GATEWAY_SENDER_1 + " is stopped on member " + MEMBER_2)
        .hasTableSection().hasColumn("Result")
        .containsExactlyInAnyOrder("OK", "OK");
  }

  @Test
  public void stopGatewaySenderOnOneMembersSuccessfullyAndOneFail() {
    doReturn(locatorSystem).when(cache).getDistributedSystem();
    doReturn(locatorDistributedMember).when(locatorSystem).getDistributedMember();
    doReturn("locator").when(locatorDistributedMember).getId();
  ...
  @Test
  public void memberAlreadyStopped() {
    doReturn(locatorSystem).when(cache).getDistributedSystem();
    doReturn(locatorDistributedMember).when(locatorSystem).getDistributedMember();
    doReturn("locator").when(locatorDistributedMember).getId();
