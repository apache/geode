...
public class UndeployCommand extends GfshCommand {
  ...
  public ResultModel undeploy(
      ...
      @CliOption(key = {CliStrings.DEPLOYMENT__NAME, CliStrings.DEPLOYMENT__NAMES},
          help = CliStrings.DEPLOYMENT__NAME__HELP) String[] deploymentNames) {
    ...
    if (jars != null && deploymentNames != null) {
      return ResultModel.createError(CliStrings.CANNOT_SPECIFY_JARS_AND_DEPLOYMENT_NAMES);
    }

    List<CliFunctionResult> results =
        executeAndGetFunctionResult(this.undeployFunction, new Object[] {jars, deploymentNames},
            targetMembers);

    ResultModel result = new ResultModel();
    TabularResultModel tabularData = result.addTable("jars");

    List<DeploymentInfo> deploymentInfoList =
        DeploymentInfoTableUtil.getDeploymentInfoFromFunctionResults(results);
    DeploymentInfoTableUtil.writeDeploymentInfoToTable(
        new String[] {"Member", "Deployment Name", "JAR", "Un-Deployed From JAR Location"},
        tabularData,
        deploymentInfoList);
    ...
    if (getConfigurationPersistenceService() == null) {
      ...
      Map<String, String> deploymentNamesToFileNames = new HashMap<>();
