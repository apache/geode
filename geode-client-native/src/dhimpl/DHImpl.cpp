...
void gf_clearDhKeys(void) {
  ...
  for (iter = m_serverCerts.begin();
         iter != m_serverCerts.end(); ++iter) {
    X509_free(*iter);
  }
...
unsigned char* gf_encryptDH(const unsigned char* cleartext, int len, int* retLen) {
  ...
  LOGDH(" DH: gf_encryptDH using sk algo: %s, Keysize: %d", m_skAlgo.c_str(),m_keySize);
  ...
  if (m_skAlgo == "AES") {
    ...
    ret = EVP_EncryptInit_ex(&ctx, cipherFunc, NULL, (unsigned char *) m_key, (unsigned char *) m_key+keySize);
  }
  else if (m_skAlgo == "Blowfish") {
    int keySize = m_keySize > 128 ? m_keySize/8 : 16;
    ret = EVP_EncryptInit_ex(&ctx, cipherFunc, NULL, NULL, (unsigned char *) m_key+keySize);
    ...
    ret = EVP_EncryptInit_ex(&ctx, cipherFunc, NULL, (unsigned char *) m_key, (unsigned char *) m_key+24);
  }
...
bool gf_verifyDH(const char* subject, const unsigned char* challenge,
  ...
  for (int item = 0; item < count; item++) {
    certsubject = X509_NAME_oneline(X509_get_subject_name(m_serverCerts[item]), NULL, 0);
