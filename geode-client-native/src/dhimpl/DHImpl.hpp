...
extern "C" {  
  ...
  CPPCACHE_EXPORT void gf_setPublicKeyOther(const unsigned char* pubkey, int length);
  CPPCACHE_EXPORT void gf_computeSharedSecret(void);
  CPPCACHE_EXPORT unsigned char* gf_encryptDH(const unsigned char* cleartext, int len, int* retLen);
  CPPCACHE_EXPORT bool gf_verifyDH(const char* subject, const unsigned char* challenge,
    int challengeLen, const unsigned char* response, int responseLen, int* reason);
}
