...
void SqLiteImpl::init(const RegionPtr& region, PropertiesPtr& diskProperties)
{
  ...
  if (diskProperties != NULLPTR) 
  {
    ...
    if (pageSizePtr != NULLPTR) 
      pageSize = atoi(pageSizePtr->asChar());
    

    if (persDir != NULLPTR) 
      m_persistanceDir = persDir->asChar();
  }
  ...
  LOGFINE("SqLiteImpl::init creating persistence directory: %s",m_persistanceDir.c_str());
  ...
  LOGFINE("SqLiteImpl::init creating persistence region file: %s",m_regionDBFile.c_str());
  ...
  LOGINFO("SqLiteImpl::init absolute path for persistence directory: %s",m_persistanceDir.c_str());
  ...
  if(m_sqliteHelper->initDB(region->getName(), maxPageCount, pageSize, m_regionDBFile.c_str()) !=0)
    throw IllegalStateException("Failed to initialize database in SQLITE.");
}

void SqLiteImpl::write(const CacheableKeyPtr&  key, const CacheablePtr&  value, void *& dbHandle)
{
  ...
  void* valueData = const_cast<uint8_t*>(valueDataBuffer.getBuffer(&valueBufferSize));
  
  if(m_sqliteHelper->insertKeyValue(keyData, keyBufferSize, valueData, valueBufferSize) != 0)
    throw IllegalStateException("Failed to write key value in SQLITE.");
}

bool SqLiteImpl::writeAll()
{
  return true;
}
...
CacheablePtr SqLiteImpl::read(const CacheableKeyPtr& key, void *& dbHandle)
{
  ...
  if( m_sqliteHelper->getValue(keyData, keyBufferSize, valueData, valueBufferSize) !=0)
    throw IllegalStateException("Failed to read the value from SQLITE.");

   // Deserialize object and return value.
  DataInput valueDataBuffer((uint8_t *)valueData, valueBufferSize);
...
extern "C" {
  ...
  LIBEXP PersistenceManager* createSqLiteInstance() {
