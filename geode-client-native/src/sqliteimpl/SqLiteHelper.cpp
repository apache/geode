...
int SqLiteHelper::initDB(const char * regionName, int maxPageCount, int pageSize, const char* regionDBfile)
{
  LOGDEBUG("SqLiteHelper::initDB Initializing SqLite with region name:%s, max page count : %d, page size:%d and region db file :%s", regionName, maxPageCount, pageSize,regionDBfile);
  //open the database
...
int SqLiteHelper::createTable()
{
  ...
  SNPRINTF(query,QUERY_SIZE, "CREATE TABLE IF NOT EXISTS %s(key BLOB PRIMARY KEY,value BLOB);", m_tableName);
  sqlite3_stmt *stmt;
...
int SqLiteHelper::insertKeyValue(void * keyData, uint32_t keyDataSize, void *valueData, uint32_t valueDataSize)
{
  //construct query
  char query[QUERY_SIZE];
  SNPRINTF(query,QUERY_SIZE, "REPLACE INTO %s VALUES(?,?);", m_tableName);

  LOGDEBUG("SqLiteHelper::insertKeyValue Inserting key value with query:%s", query);
...
int SqLiteHelper::getValue(void *keyData, uint32_t keyDataSize, void*& valueData, uint32_t& valueDataSize)
{
  
  //construct query
   char query[QUERY_SIZE];
  SNPRINTF(query,QUERY_SIZE, "SELECT value, length(value) AS valLength FROM %s WHERE key=?;", m_tableName);
  ...
  if(retCode == SQLITE_OK)
  {
    ...
    {
      void *tempBuff = (void*)sqlite3_column_blob(stmt, 0);
      valueDataSize = sqlite3_column_int(stmt,1);
      valueData = (uint8_t *)malloc(sizeof(uint8_t)*valueDataSize);
...
{
  LOGDEBUG("SqLiteHelper::closeDB closing the database for region %s", m_tableName);
  int retCode = dropTable();
  if(retCode == SQLITE_OK)
    retCode = sqlite3_close(m_dbHandle);
  
  return retCode;
}

int SqLiteHelper::executePragma(char *pragmaName, int pragmaValue)
{
  ...
  //execute PRAGMA
  if(retCode == SQLITE_OK && sqlite3_step(stmt) == SQLITE_ROW) //PRAGMA command return one row
      retCode = sqlite3_step(stmt);
