...
#include "ManagedCacheableKeyN.hpp"
...
namespace gemfire
{
  ...
  bool ManagedCacheableKeyGeneric::operator ==(const gemfire::CacheableKey& other) const
  {
    try {
      ...
      if (p_other != NULL) {
        return static_cast<ICacheableKey^>(
          (static_cast<IGFSerializable^>((IGFSerializable^)m_managedptr)))->Equals(
          static_cast<ICacheableKey^>(p_other->ptr()));
      }
      return false;
    }
    catch (GemStone::GemFire::Cache::Generic::GemFireException^ ex) {
  ...
  bool ManagedCacheableKeyGeneric::operator ==(const ManagedCacheableKeyGeneric& other) const
  {
    try {
      return static_cast<ICacheableKey^>(
        (IGFSerializable^)(IGFSerializable^)m_managedptr)->Equals(
        static_cast<ICacheableKey^>(other.ptr()));
    }
    catch (GemStone::GemFire::Cache::Generic::GemFireException^ ex) {
      ex->ThrowNative();
    }
