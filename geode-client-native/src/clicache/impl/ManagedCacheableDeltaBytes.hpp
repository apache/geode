...
namespace gemfire
{
  ...
  /// </summary>
  class ManagedCacheableDeltaBytes
  ...
  public:
        ...
        m_hashCode(0)
    {
      ...
      if(managedptr != nullptr)
      {
        ...
        GemStone::GemFire::Cache::Log::Fine("ManagedCacheableDeltaBytes::Constructor: class ID " + managedptr->ClassId + " : " + managedptr->ToString() + " storeBytes:" + storeBytes);
        if(storeBytes)
        {
...
/*
    ...
    inline ~ManagedCacheableDeltaBytes()
    {
      GemStone::GemFire::Cache::Log::Fine("ManagedCacheableDeltaBytes::Destructor current AppDomain ID: " + System::Threading::Thread::GetDomainID() + " for object: " + System::Convert::ToString((int)this) + " with its AppDomain ID: " + m_domainId);
      GF_SAFE_DELETE(m_bytes);
    }

  private:
    GemStone::GemFire::Cache::IGFDelta^ getManagedObject() const;
    /// <summary>
    ...
    ManagedCacheableDeltaBytes( const ManagedCacheableDeltaBytes& );
    ManagedCacheableDeltaBytes& operator = ( const ManagedCacheableDeltaBytes& );
  };
