...
template<> class ACE_Equal_To< gemfire::CacheableKeyPtr > {
  public:
    int operator() (const gemfire::CacheableKeyPtr &lhs, const gemfire::CacheableKeyPtr &rhs) const
    {
...
typedef ACE_Hash_Map_Manager_Ex< CacheableKeyPtr,
                                 CacheablePtr,
                                 ACE_Hash< CacheableKeyPtr >,
                                 ACE_Equal_To< CacheableKeyPtr >,
...
PropertiesPtr Properties::create( )
{
  return PropertiesPtr( new Properties() );
}
...
void Properties::insert( const CacheableKeyPtr &key,
...
void Properties::addAll( const PropertiesPtr& other )
{
  ...
  class Copier : public Visitor
  {
    Properties& m_lhs;
    public:
    Copier( Properties& lhs ) : m_lhs( lhs ) { }
...
PropertiesFile::PropertiesFile( Properties& props )
: m_props( props )
{
}
...
char* PropertiesFile::nextBufferLine( char** cursor, char* bufbegin, size_t totalLen )
{
  if ( (size_t)( (*cursor) - bufbegin ) >= totalLen ) {
    return 0;
  }

  char* lineBegin = *cursor;
  while( ((**cursor) != 10) && ((**cursor) != 13)
  && ( (size_t)((*cursor) - bufbegin) < totalLen ) ) {
    (*cursor)++;
  }
  **cursor = '\0';
...
Serializable* Properties::createDeserializable( )
{
  return new Properties( );
}

int32_t Properties::classId( ) const
{
  return 0;
}

int8_t Properties::typeId( ) const
{
  return GemfireTypeIds::Properties;
}
...
void Properties::toData( DataOutput& output ) const
{
  ...
  while( iter != MAP->end() ) {
    //HItesh changed
    CacheableString* csPtr = dynamic_cast<CacheableString*>(((*iter).ext_id_).ptr());
    if(csPtr == NULL) {
      output.writeObject((*iter).ext_id_); //HItesh changed
    }else {
    ...
    if(csPtr == NULL) {
      output.writeObject((*iter).int_id_); //HItesh changed
    }
    else {
      output.writeNativeString(csPtr->asChar());
    }
...
Serializable* Properties::fromData (DataInput& input )
{
  ...
  for(int i = 0; i < mapSize; i++)
  {
    ...
    //TODO:HItesh need to look just look typeid if string then convert otherwise continue with readobject
