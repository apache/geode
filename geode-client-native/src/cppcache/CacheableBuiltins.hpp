...
namespace gemfire
{
  ...
  protected:
    ...
    inline CacheableKeyType(const TObj value) :
      m_value(value)
    {
    }
  ...
  template<typename TObj, int8_t TYPEID> class SharedArrayPtr;
  ...
  protected:
    ...
    inline CacheableArrayType() :
      m_value(NULL), m_length(0)
    {
    }

    inline CacheableArrayType(int32_t length) :
      m_length(length)
    {
      if (length > 0) {
  ...
  protected:
    ...
    inline CacheableContainerType() :
      TBase()
    {
    }

    inline CacheableContainerType(const int32_t n) :
      TBase(n)
    {
    }
