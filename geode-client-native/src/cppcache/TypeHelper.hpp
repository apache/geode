...
namespace gemfire
{
  ...
  template<typename Target> class SharedPtr;

  // Forward declaration of SharedArrayPtr<T, ID>
  template<typename Target, int8_t TYPEID> class SharedArrayPtr;

  // Forward declaration of CacheableArrayType<T, ID>
  template<typename Target, int8_t TYPEID> class CacheableArrayType;
  ...
  namespace TypeHelper
  {
    ...
    template <typename TBase, typename TDerived> struct BDHelper
    {
      template <typename T>
     ...
     *        {@link http://groups.google.com/group/comp.lang.c++.moderated/msg/645b2486ae80e5fb}
     */
    template <typename TBase, typename TDerived> struct SuperSubclass
    {
      private:
     ...
     */
    template <typename TBase> struct SuperSubclass<TBase, TBase>
    {
     ...
     */
    template <bool getType=true> struct YesNoType
    {
     ...
     */
    template <> struct YesNoType<false>
    {
      static const no_type value = 0;
    };

    /** @brief This struct unwraps the type <code>T</code> inside SharedPtr. */
    template <class T> struct UnwrapSharedPtr
    {
    };

    /** @brief This struct unwraps the type <code>T</code> inside SharedPtr. */
    template <class T> struct UnwrapSharedPtr< SharedPtr<T> >
    {
      typedef T type;
    };

    /** @brief This struct unwraps the type <code>T</code> inside SharedArrayPtr. */
    template <class T, int8_t ID> struct UnwrapSharedPtr< SharedArrayPtr<T, ID> >
    {
...
#define GF_SRC_IS_TARGET_TYPE( TARGET, SRC ) \
  gemfire::TypeHelper::YesNoType< gemfire::TypeHelper::SuperSubclass< TARGET, SRC >::result >::value
