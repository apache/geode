...
#define _GF_EXCEPTION_DEF(x) \
  ...
  class CPPCACHE_EXPORT x \
  : public gemfire::Exception \
  { \
    public: \
      x(const char* msg1, const char* msg2 = NULL, bool forceStack = false, \
          const ExceptionPtr& cause = NULLPTR) : Exception(msg1, msg2, forceStack, cause) { } \
      x(const x& other) : Exception(other) { } \
      virtual Exception* clone() const { return new x(m_message, m_stack, m_cause); } \
      virtual ~x() { } \
      virtual const char* getName() const { return _exception_name_##x; } \
      virtual void raise() { throw *this; } \
    protected: \
      x(const CacheableStringPtr& message, const StackTracePtr& stack, const ExceptionPtr& cause) : Exception(message, stack, cause) { } \
  private: \
...
/**
* @brief Thrown when the transactional data host has shutdown or no longer has the data
...
/**
* @brief Thrown when a {@link RebalanceOperation} occurs concurrently with a transaction.
* This can be thrown while doing transactional operations or during commit.
**/
...
/**
 *@brief Thrown when a version on which delta is based is different than the current version
 **/

extern void CPPCACHE_EXPORT GfErrTypeThrowException( const char* str, GfErrType err );
