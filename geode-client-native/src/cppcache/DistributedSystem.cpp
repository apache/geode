...
#include "impl/ExpiryTaskManager.hpp"
#include "impl/CacheImpl.hpp"
#include <ace/Guard_T.h>
#include <ace/Recursive_Thread_Mutex.h>
#include "DataOutput.hpp"
#include "impl/TcrMessage.hpp"
#include "impl/DistributedSystemImpl.hpp"
#include "impl/RegionStats.hpp"
#include "impl/PoolStatistics.hpp"

#include "impl/DiffieHellman.hpp"
  ...
  void setLFH( )
  {
    ...
    if (kernelMod == NULL)
    {
      ...
      if (kernelMod != NULL)
      {
        ...
        PHSI pHSI = NULL;
        PGPH pGPH = NULL;
        if ((pHSI = (PHSI)GetProcAddress( kernelMod, "HeapSetInformation" )) != NULL)
        {
          // The LFH API is available
          /* Only set LFH for process heap; causes problems in C++ framework if set for all heaps
          ...
          */
          ...
          ULONG heapFragValue = 2;
          if ((pGPH = (PGPH)GetProcAddress( kernelMod, "GetProcessHeap" )) != NULL)
          {
            hProcessHeapHandle = pGPH( );
            LOGCONFIG( "Setting Microsoft Windows' low-fragmentation heap for use as the main process heap." );
            pHSI( hProcessHeapHandle,
              HeapCompatibilityInformation, &heapFragValue, sizeof(heapFragValue) );
          }
        }
      }
...
DistributedSystem::DistributedSystem(const char* name)
: m_name(NULL)
{
...
DistributedSystemPtr DistributedSystem::connect(const char* name,
        const PropertiesPtr& configPtr)
{
   ACE_Guard< ACE_Recursive_Thread_Mutex > disconnectGuard( *g_disconnectLock );
   setLFH();
   if(m_connected == true)
     throw AlreadyConnectedException("DistributedSystem::connect: already connected, call getInstance to get it");
   ...
   if (name == NULL) {
     delete g_sysProps;
     g_sysProps = NULL;
     throw IllegalArgumentException("DistributedSystem::connect: "
         "name cannot be NULL");
   }
   if (name[0] == '\0') {
     name = "NativeDS";
   ...
   if (logFilename != NULL && logFilename[0] != '\0') {
     const char* dirEnd;
     if ((dirEnd = strrchr(logFilename, '/')) != NULL ||
         (dirEnd = strrchr(logFilename, '\\')) != NULL) {
       int32_t sz =static_cast<int32_t> (dirEnd - logFilename);
       char* crashDumpLocation = new char[sz + 1];
       ACE_OS::strncpy(crashDumpLocation, logFilename, sz);
       crashDumpLocation[sz] = '\0';
       ++dirEnd;
       SignalHandler::init(g_sysProps->crashDumpEnabled(),
           crashDumpLocation, dirEnd);
       delete [] crashDumpLocation;
     }
     else {
       SignalHandler::init(g_sysProps->crashDumpEnabled(), "", logFilename);
     }
   }
   else {
     SignalHandler::init(g_sysProps->crashDumpEnabled(), "", "gemfire_nativeclient");
   }
   ...
   if(!TcrMessage::init()) {
     TcrMessage::cleanup();
     throw IllegalArgumentException("DistributedSystem::connect: preallocate message buffer failed!");
   }
   if (logFilename != NULL) {
      try {
       Log::close();
       Log::init(g_sysProps->logLevel(),
         logFilename,
         g_sysProps->logFileSizeLimit(),g_sysProps->logDiskSpaceLimit());
     } catch(GemfireIOException&) {
       Log::close(); 
       TcrMessage::cleanup();
   ...
   try {
     ...
     LOGCONFIG( "Using GemFire Native Client Product Directory: %s", gfcpp.c_str() );
   } catch ( Exception & ) {
   ...
   ACE_utsname u;
   ACE_OS::uname(&u);
   LOGCONFIG("Running on: SystemName=%s Machine=%s Host=%s Release=%s Version=%s", u.sysname, u.machine, u.nodename, u.release, u.version);
   ...
   LOGCONFIG("Current library path: %s", ld_libpath==NULL ? "NULL" : ld_libpath);
#else
  ...
  /* if (strlen(g_sysProps->securityClientDhAlgo())>0) {
   ...
   try {
     g_statMngr = StatisticsManager::initInstance
       ...
       g_sysProps->statisticsSampleInterval(),
       g_sysProps->statisticsEnabled(),
       g_sysProps->statsFileSizeLimit(),
       g_sysProps->statsDiskSpaceLimit());
   ...
   try {
     m_impl->connect();
    } catch ( gemfire::Exception & e ) {
      LOGERROR( "Exception caught during client initialization: %s", e.getMessage() );
     ...
     throw NotConnectedException( "DistributedSystem::connect: caught unknown exception" );
   }
...
void DistributedSystem::disconnect()
{
  ...
  if ( !m_connected ) {
    throw NotConnectedException( "DistributedSystem::disconnect: connect "
        "not called" );
  }
...
const char *  DistributedSystem::getName() const
{
   return m_name;
}
