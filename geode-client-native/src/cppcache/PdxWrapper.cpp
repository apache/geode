...
namespace gemfire {
  ...
  PdxWrapper::PdxWrapper(void * userObject, const char * className)
  {
    ...
    if (className != NULL) {
      ...
      throw IllegalArgumentException("Class name not provided to PdxWrapper constructor");
    }
    ...
    if (m_serializer == NULLPTR) {
      ...
      throw IllegalArgumentException("No registered PDX serializer found for PdxWrapper");
    }

    m_deallocator = m_serializer->getDeallocator(className);

    if (m_deallocator == NULL) {
      LOGERROR("No deallocator function found from PDX serializer for PdxWrapper for %s", className);
      throw IllegalArgumentException("No deallocator function found from PDX serializer for PdxWrapper");
    }
  ...
  PdxWrapper::PdxWrapper(const char * className)
  {
    if (className != NULL) {
      ...
      throw IllegalArgumentException("Class name not provided to PdxWrapper for deserialization");
    }

    m_serializer = SerializationRegistry::getPdxSerializer();

    if (m_serializer == NULLPTR) {
      LOGERROR("No registered PDX serializer found for PdxWrapper deserialization");
      throw IllegalArgumentException("No registered PDX serializer found for PdxWrapper deserialization");
    }

    m_deallocator = m_serializer->getDeallocator(className);

    if (m_deallocator == NULL) {
      LOGERROR("No deallocator function found from PDX serializer for PdxWrapper deserialization for %s", className);
      throw IllegalArgumentException("No deallocator function found from PDX serializer for PdxWrapper deserialization");
    }
  ...
  {
    PdxWrapper * wrapper = dynamic_cast<PdxWrapper*>((CacheableKey*)&other);
    if (wrapper == NULL) {
      return false;
    }
  ...
  {
    uint64_t hash = (uint64_t) (intptr_t) m_userObject;
  ...
  void PdxWrapper::toData( PdxWriterPtr output )
  {
    if (m_userObject != NULL) {
      ...
      throw IllegalStateException("User object is NULL or detached in PdxWrapper toData");
    }
  }
  ...
  Serializable* PdxWrapper::fromData( DataInput& input )
  {
    ...
    throw IllegalStateException("PdxWrapper fromData should not have been called");
  }
