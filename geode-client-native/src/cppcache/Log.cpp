...
namespace gemfire_log_globals {
  ...
  int32_t g_bytesWritten = 0;
  bool g_isLogFileOpened = false;

  int64 g_fileSizeLimit = GEMFIRE_MAX_LOG_FILE_LIMIT;
  int64 g_diskSpaceLimit = GEMFIRE_MAX_LOG_DISK_LIMIT;
...
static int selector (const dirent *d)
{
  ...
  std::string filebasename = ACE::basename(gemfire_log_globals::g_logFileWithExt->c_str());  
...
const char* Log::logFileName()
{
  ...
  (("Log file name is too long: " + *g_logFile).c_str());
    ACE_OS::strncpy(g_logFileNameBuffer, g_logFile->c_str(), sizeof(g_logFileNameBuffer));
  }
...
void Log::init
(LogLevel level, const char* logFileName, int32 logFileLimit, int64 logDiskSpaceLimit)
{
  if (g_log != NULL) {
    throw IllegalStateException("The Log has already been initialized. "
  ...
  if (logDiskSpaceLimit < 0 /*|| logDiskSpaceLimit > GEMFIRE_MAX_LOG_DISK_LIMIT*/)
  ...
  (("Log file name is too long: " + filename).c_str());
    ...
    *g_logFile = slashtmp;
    ...
    // Appending a ".log" at the end if it does not exist or file has some other extension.
  ...
  g_fileSizeLimit = (int32)logFileLimit * 1024 * 1024;
  ...
  if ( existingFile != NULL && logFileLimit > 0) {
   /* adongre
    * Coverity - II
    * CID 29205: Calling risky function (SECURE_CODING)[VERY RISKY]. Using "sprintf" can cause a 
    ...
    */
    ...
    int32 len = (int32)g_logFileWithExt->length();
    int32 lastPosOfSep = (int32)g_logFileWithExt->find_last_of(ACE_DIRECTORY_SEPARATOR_CHAR, len);
    if ( lastPosOfSep == -1 ) {
      logsdirname = ".";
    } else {
    ...
    int32 baselen = (int32)logsbasename.length();
    int32 posOfExt = (int32)logsbasename.find_last_of(logFileExtAfter, baselen);
    if ( posOfExt == -1 ) {
    ...
    ACE_OS::snprintf(rollFile, 1024 , "%s%c%s-%d.%s", logsdirname.c_str(),
      ACE_DIRECTORY_SEPARATOR_CHAR, fnameBeforeExt.c_str(), g_rollIndex++, extName.c_str());
...
void Log::writeBanner()
{
  ...
  if(GF_FILEEXISTS(dirname) !=0 && ACE_OS::mkdir(dirname) != 0 ) {
    std::string msg = "Error in creating directories for: "
                      + std::string(dirname);
    throw GemfireIOException(msg.c_str());
  }
  // retry some number of times before giving up when file is busy etc.
  ...
  if (!g_log) {
    //g_log = stdout;
   // fprintf(stdout,"File %s unable to open, logging into console.", g_logFile->c_str());
   // fflush(stdout);
    //g_logFile = NULL;
    //g_logFileWithExt = NULL;
  ...
  g_bytesWritten += (int32_t)(bannertext.length()+ numchars);
  fflush(g_log);
}
...
char* Log::formatLogLine(char* buf, Log::LogLevel level)
{
  ...
  ACE_OS::snprintf(pbuf , 300  , "%s:%d %lu] ", g_uname.nodename, g_pid, ACE_OS::thr_self());
...
void Log::put(LogLevel level, const char* msg)
{
  ...
  if (!g_logFile) {
    ...
    int numChars = (int)(ACE_OS::strlen(buf) + ACE_OS::strlen(msg));
    g_bytesWritten += numChars + 2;//bcoz we have to count trailing new line (\n)
  ...
  if ( (g_diskSpaceLimit > 0) && (g_spaceUsed >= g_diskSpaceLimit)) {	
	...
	/*for(int i = 1; i < entries_count; i++) {
	  ACE_OS::snprintf(fullpath , 512 , "%s%c%s",dirname.c_str(),ACE_DIRECTORY_SEPARATOR_CHAR,resultArray[i]->d_name);
     ...
     if (status != -1) {
	  for (int index = 1; index < sds.length (); ++index) {		
		  ACE_OS::snprintf(fullpath , 512 , "%s%c%s",dirname.c_str(),ACE_DIRECTORY_SEPARATOR_CHAR,sds[index]->d_name);
		  ACE_OS::stat(fullpath,&statBuf);
		  g_fileInfoPair = std::make_pair(fullpath,statBuf.st_size);
		  fileInfo.push_back(g_fileInfoPair);
  ...
  if ((numChars = fprintf(g_log, "%s%s\n", buf, msg))
    ...
    //lets continue wothout throwing the exception; it should not cause process to terminate
    fclose(g_log);
    g_log = NULL;
   // g_isLogFileOpened = false;
