...
int Pool::getSocketBufferSize() const {
  return m_attrs->getSocketBufferSize();
}
int Pool::getReadTimeout() const {
  return m_attrs->getReadTimeout();
}
int Pool::getMinConnections() const {
  return m_attrs->getMinConnections();
}
int Pool::getMaxConnections() const {
  return m_attrs->getMaxConnections();
}
long Pool::getIdleTimeout() const {
  return m_attrs->getIdleTimeout();
}
long Pool::getPingInterval() const {
...
int Pool::getRetryAttempts() const {
  return m_attrs->getRetryAttempts();
}
...
const char* Pool::getServerGroup() const {
...
RegionServicePtr Pool::createSecureUserCache(PropertiesPtr credentials)
{
  if (this->getMultiuserAuthentication())
  {
    ...
    //TODO:hitesh this will return cache with userattribtes
    ProxyCachePtr userCache(new ProxyCache(credentials, tmpPool));
    return userCache;
  }

  throw IllegalStateException("This operation is only allowed when attached pool is in multiuserSecureMode");
 // return NULLPTR;
}
...
int Pool::getPendingEventCount() const {
  ...
  if (!tccm.isDurable())
  {
    ...
    throw IllegalStateException("This operation should only be called by durable client");
  }
  ...
  if (poolHADM->isReadyForEvent())
  {
    ...
    throw IllegalStateException("This operation should only be called before readyForEvents");
  }
