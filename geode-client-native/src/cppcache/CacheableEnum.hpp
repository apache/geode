...
namespace gemfire {
  ...
  /**
  * Since C++ enums cannot be directly passed as a parameter to PdxWriter's writeObject and PdxReader's readObject api
  * wrap C++ enum in to a immutable wrapper CacheableEnum class type by specifying enum class name, enum value name and its ordinal.
  * C++ enum allows explicit setting of ordinal number, but it is up to the user to map java enumName with that of C++ enumName.
  * Currently this wrapper only works as part of PdxSerializable member object and cannot be directly used in Region operations.
  *
  ...
  */
  ...
  public:
    ...
    * @param enumName the name of the enum constant that maps to the java enum type.
    * @param ordinal the ordinal value of the enum constant that maps to the java enum type.
    ...
    */
    ...
    const char* getEnumClassName() const {         
      return m_enumClassName->asChar();      
    }

    /**@return enum name. */
    const char* getEnumName() const {         
      return m_enumName->asChar();      
    }
  ...
  protected:
    CacheableEnum();
    CacheableEnum(const char* enumClassName, const char* enumName, int32_t ordinal);
  private:
    // never implemented.
