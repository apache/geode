...
#include "impl/CacheXmlParser.hpp"
#include <ace/DLL.h>
#include <ace/OS.h>
...
:  Serializable( ),
   ...
   m_scope( ScopeType::DISTRIBUTED_NO_ACK),
...
:
   m_regionTimeToLiveExpirationAction( rhs.m_regionTimeToLiveExpirationAction ),
   m_regionIdleTimeoutExpirationAction( rhs.m_regionIdleTimeoutExpirationAction ),
   m_entryTimeToLiveExpirationAction( rhs.m_entryTimeToLiveExpirationAction ),
   m_entryIdleTimeoutExpirationAction( rhs.m_entryIdleTimeoutExpirationAction ),
   ...
   m_isConcurrencyChecksEnabled(rhs.m_isConcurrencyChecksEnabled)
{
  if ( rhs.m_cacheLoaderLibrary != NULL ) {
    int len = strlen( rhs.m_cacheLoaderLibrary ) + 1;
  ...
  if ( rhs.m_cacheWriterLibrary != NULL ) {
    int len = strlen( rhs.m_cacheWriterLibrary ) + 1;
  ...
  if ( rhs.m_cacheListenerLibrary != NULL ) {
    int len = strlen( rhs.m_cacheListenerLibrary ) + 1;
  ...
  if ( rhs.m_partitionResolverLibrary != NULL ) {
    int len = strlen( rhs.m_partitionResolverLibrary ) + 1;
    m_partitionResolverLibrary = new char[len];
    ACE_OS::strncpy( m_partitionResolverLibrary, rhs.m_partitionResolverLibrary, len);
  } else {
    m_partitionResolverLibrary = NULL;
  }
  if ( rhs.m_cacheLoaderFactory != NULL ) {
    int len = strlen( rhs.m_cacheLoaderFactory ) + 1;
  ...
  if ( rhs.m_cacheWriterFactory != NULL ) {
    int len = strlen( rhs.m_cacheWriterFactory ) + 1;
  ...
  if ( rhs.m_cacheListenerFactory != NULL ) {
    int len = strlen( rhs.m_cacheListenerFactory ) + 1;
  ...
  if ( rhs.m_partitionResolverFactory != NULL ) {
    int len = strlen( rhs.m_partitionResolverFactory ) + 1;
    m_partitionResolverFactory = new char[len];
    ACE_OS::strncpy( m_partitionResolverFactory, rhs.m_partitionResolverFactory, len);
  } else {
    m_partitionResolverFactory = NULL;
  }
  if ( rhs.m_endpoints != NULL ) {
    int len = strlen( rhs.m_endpoints ) + 1;
  ...
  if ( rhs.m_poolName != NULL ) {
    int len = strlen( rhs.m_poolName ) + 1;
  ...
  if ( rhs.m_persistenceLibrary != NULL ) {
    int len = strlen( rhs.m_persistenceLibrary ) + 1;
  ...
  if ( rhs.m_persistenceFactory != NULL ) {
    int len = strlen( rhs.m_persistenceFactory ) + 1;
...
#define RA_DELSTRING(x) if ( x != NULL ) { delete [] x; } x = NULL
...
/**
 * lib should be in the form required by ACE_DLL, typically just like specifying a
...
void* getFactoryFunc( const char* lib, const char* funcName )
{
  ...
  if ( func == NULL ) {
    char msg[1000];
    ACE_OS::snprintf( msg, 1000, "cannot find factory function %s in library %s", funcName, lib );
    throw IllegalArgumentException( msg );
  }
  return func;
}
...
CacheLoaderPtr RegionAttributes::getCacheLoader( )
{
  if ((m_cacheLoader == NULLPTR) && (m_cacheLoaderLibrary != NULL)) {
    if ( CacheXmlParser::managedCacheLoaderFn != NULL
      ...
      m_cacheLoader = ( *CacheXmlParser::managedCacheLoaderFn )( m_cacheLoaderLibrary,
                                                                 m_cacheLoaderFactory );
    } else {
      CacheLoader* (*funcptr)();
      funcptr = (CacheLoader* (*)()) gemfire_impl::getFactoryFunc( m_cacheLoaderLibrary, m_cacheLoaderFactory );
...
CacheWriterPtr RegionAttributes::getCacheWriter( )
{
  if ((m_cacheWriter == NULLPTR) && (m_cacheWriterLibrary != NULL)) {
    if ( CacheXmlParser::managedCacheWriterFn != NULL
      ...
      // this is a managed library
      m_cacheWriter = ( *CacheXmlParser::managedCacheWriterFn )( m_cacheWriterLibrary,
                                                                 m_cacheWriterFactory );
    } else {
      CacheWriter* (*funcptr)();
      funcptr = (CacheWriter* (*)()) gemfire_impl::getFactoryFunc( m_cacheWriterLibrary, m_cacheWriterFactory );
...
CacheListenerPtr RegionAttributes::getCacheListener( )
{
  if ((m_cacheListener == NULLPTR) && (m_cacheListenerLibrary != NULL)) {
    if ( CacheXmlParser::managedCacheListenerFn != NULL
      ...
      //LOGDEBUG( "RegionAttributes::getCacheListener: Trying to create instance from managed library." );
      // this is a managed library
      m_cacheListener = ( *CacheXmlParser::managedCacheListenerFn )( m_cacheListenerLibrary,
                                                                     m_cacheListenerFactory );
    } else {
      CacheListener* (*funcptr)();
      funcptr = (CacheListener* (*)()) gemfire_impl::getFactoryFunc( m_cacheListenerLibrary, m_cacheListenerFactory );
...
{
  if ((m_partitionResolver == NULLPTR) && (m_partitionResolverLibrary != NULL)) {
    if ( CacheXmlParser::managedPartitionResolverFn != NULL
         && strchr( m_partitionResolverFactory, '.' ) != NULL ) {
      //LOGDEBUG( "RegionAttributes::getCacheListener: Trying to create instance from managed library." );
      // this is a managed library
      m_partitionResolver = ( *CacheXmlParser::managedPartitionResolverFn )( m_partitionResolverLibrary,
                                                                     m_partitionResolverFactory );
    } else {
      PartitionResolver* (*funcptr)();
      funcptr = (PartitionResolver* (*)()) gemfire_impl::getFactoryFunc( m_partitionResolverLibrary, m_partitionResolverFactory );
...
PersistenceManagerPtr RegionAttributes::getPersistenceManager( )
{
  if ((m_persistenceManager == NULLPTR) && (m_persistenceLibrary != NULL)) {
    if ( CacheXmlParser::managedPartitionResolverFn != NULL
      ...
      LOGDEBUG( "RegionAttributes::getPersistenceManager: Trying to create instance from managed library." );
      // this is a managed library
      m_persistenceManager = ( *CacheXmlParser::managedPersistenceManagerFn )( m_persistenceLibrary,
                                                                     m_persistenceFactory );
    } else {
      PersistenceManager* (*funcptr)();
      funcptr = (PersistenceManager* (*)()) gemfire_impl::getFactoryFunc( m_persistenceLibrary, m_persistenceFactory );
...
const char* RegionAttributes::getEndpoints()
{
  return m_endpoints;
}
...
int RegionAttributes::getRegionTimeToLive()
{
...
int RegionAttributes::getRegionIdleTimeout()
{
...
int RegionAttributes::getEntryTimeToLive()
{
...
int RegionAttributes::getEntryIdleTimeout()
{
...
ScopeType::Scope RegionAttributes::getScope() const
{
   return m_scope;
}

int RegionAttributes::getInitialCapacity() const
{
   return m_initialCapacity;
}

float RegionAttributes::getLoadFactor() const
{
   return m_loadFactor;
}
...
const char* RegionAttributes::getPoolName() const
{
  return m_poolName;
}
...
int32_t RegionAttributes::classId( ) const
{
  return 0;
}
...
namespace gemfire_impl {
  ...
  void writeCharStar( DataOutput& out, const char* field ) {
    if ( field == NULL ) {
      out.writeBytes( (int8_t*) "", (uint32_t)0 );
    } else {
      out.writeBytes( (int8_t*) field, (uint32_t)strlen(field) + 1);
    }
  }
  ...
  void readCharStar( DataInput& in, char** field ) {
    ...
    if ( memlen != 0 ) {
      *field = new char[memlen];
      in.readBytesOnly( (int8_t*)*field, memlen );
    }
  }
}
...
{
  out.writeInt( (int32_t) m_regionTimeToLive );
  out.writeInt( (int32_t) m_regionTimeToLiveExpirationAction );
  out.writeInt( (int32_t) m_regionIdleTimeout );
  out.writeInt( (int32_t) m_regionIdleTimeoutExpirationAction );
  out.writeInt( (int32_t) m_entryTimeToLive );
  out.writeInt( (int32_t) m_entryTimeToLiveExpirationAction );
  out.writeInt( (int32_t) m_entryIdleTimeout );
  out.writeInt( (int32_t) m_entryIdleTimeoutExpirationAction );
  out.writeInt( (int32_t) m_scope );
  out.writeInt( (int32_t) m_initialCapacity );
  out.writeFloat( m_loadFactor );
  out.writeInt( (int32_t) m_maxValueDistLimit );
  out.writeInt( (int32_t) m_concurrencyLevel );
  out.writeInt( (int32_t) m_lruEntriesLimit );
  out.writeInt( (int32_t) m_lruEvictionAction );
  ...
  out.writeInt( (int32_t) m_diskPolicy );
...
{
  in.readInt( (int32_t*) &m_regionTimeToLive );
  in.readInt( (int32_t*) &m_regionTimeToLiveExpirationAction );
  in.readInt( (int32_t*) &m_regionIdleTimeout );
  in.readInt( (int32_t*) &m_regionIdleTimeoutExpirationAction );
  in.readInt( (int32_t*) &m_entryTimeToLive );
  in.readInt( (int32_t*) &m_entryTimeToLiveExpirationAction );
  in.readInt( (int32_t*) &m_entryIdleTimeout );
  in.readInt( (int32_t*) &m_entryIdleTimeoutExpirationAction );
  in.readInt( (int32_t*) &m_scope );
  in.readInt( (int32_t*) &m_initialCapacity );
  in.readFloat( &m_loadFactor );
  in.readInt( (int32_t*) &m_maxValueDistLimit );
  in.readInt( (int32_t*) &m_concurrencyLevel );
  in.readInt( (int32_t*) &m_lruEntriesLimit );
  in.readInt( (int32_t*) &m_lruEvictionAction );
  ...
  in.readInt( (int32_t*) &m_diskPolicy );
...
bool RegionAttributes::operator == ( const RegionAttributes& other ) const
{
  ...
  if ( m_regionTimeToLiveExpirationAction != other.m_regionTimeToLiveExpirationAction ) return false;
  if ( m_regionIdleTimeout != other.m_regionIdleTimeout ) return false;
  if ( m_regionIdleTimeoutExpirationAction != other.m_regionIdleTimeoutExpirationAction ) return false;
  if ( m_entryTimeToLive != other.m_entryTimeToLive ) return false;
  if ( m_entryTimeToLiveExpirationAction != other.m_entryTimeToLiveExpirationAction ) return false;
  if ( m_entryIdleTimeout != other.m_entryIdleTimeout ) return false;
  if ( m_entryIdleTimeoutExpirationAction != other.m_entryIdleTimeoutExpirationAction ) return false;
  if ( m_scope != other.m_scope ) return false;
  ...
  if ( m_clientNotificationEnabled != other.m_clientNotificationEnabled ) return false;

  if ( 0 != compareStringAttribute( m_cacheLoaderLibrary, other.m_cacheLoaderLibrary ) ) return false;
  if ( 0 != compareStringAttribute( m_cacheLoaderFactory, other.m_cacheLoaderFactory ) ) return false;
  if ( 0 != compareStringAttribute( m_cacheWriterLibrary, other.m_cacheWriterLibrary ) ) return false;
  if ( 0 != compareStringAttribute( m_cacheWriterFactory, other.m_cacheWriterFactory ) ) return false;
  if ( 0 != compareStringAttribute( m_cacheListenerLibrary, other.m_cacheListenerLibrary ) ) return false;
  if ( 0 != compareStringAttribute( m_cacheListenerFactory, other.m_cacheListenerFactory ) ) return false;
  if ( 0 != compareStringAttribute( m_partitionResolverLibrary, other.m_partitionResolverLibrary ) ) return false;
  if ( 0 != compareStringAttribute( m_partitionResolverFactory, other.m_partitionResolverFactory ) ) return false;
  if ( m_diskPolicy != other.m_diskPolicy ) return false;
  if ( 0 != compareStringAttribute( m_endpoints, other.m_endpoints ) ) return false;
  if ( 0 != compareStringAttribute( m_persistenceLibrary, other.m_persistenceLibrary ) ) return false;
  if ( 0 != compareStringAttribute( m_persistenceFactory, other.m_persistenceFactory ) ) return false;
  if ( m_isConcurrencyChecksEnabled != other.m_isConcurrencyChecksEnabled ) return false;

  return true;
}

int32_t RegionAttributes::compareStringAttribute(char* attributeA, char* attributeB) {
...
/* Throws IllegalStateException when attributes targetted for use on a server do not meet requirements. */
void RegionAttributes::validateSerializableAttributes( ) {
  if (m_cacheLoader != NULLPTR) {
    throw IllegalStateException( "CacheLoader must be set with setCacheLoader(library, factory) in members of type SERVER" );
  }
  if (m_cacheWriter != NULLPTR) {
    throw IllegalStateException( "CacheWriter must be set with setCacheWriter(library, factory) in members of type SERVER" );
  }
  if (m_cacheListener != NULLPTR) {
    throw IllegalStateException( "CacheListener must be set with setCacheListener(library, factory) in members of type SERVER" );
  }
  if (m_partitionResolver != NULLPTR) {
    throw IllegalStateException( "PartitionResolver must be set with setPartitionResolver(library, factory) in members of type SERVER" );
  }
  if (m_persistenceManager != NULLPTR) {
    throw IllegalStateException( "persistenceManager must be set with setPersistenceManager(library, factory,config) in members of type SERVER" );
  }
}
...
void RegionAttributes::setPersistenceManager(const char* lib, const char* func, const PropertiesPtr& config)
{
  GF_R_ASSERT( lib != NULL );
  GF_R_ASSERT( func != NULL );
...
void RegionAttributes::setCachingEnabled( bool enable)
{
  m_caching = enable;
}
...
void RegionAttributes::setScope(ScopeType::Scope scope)
{
  m_scope = scope;
}
