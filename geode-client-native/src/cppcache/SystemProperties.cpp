...
namespace {
  ...
  const char SslKeystorePassword[] = "ssl-keystore-password";// adongre: Added for Ticket #758
  ...
  const char OnClientDisconnectClearPdxTypeIds[] = "on-client-disconnect-clear-pdxType-Ids";
  ...
  const char DefaultSecurityClientDhAlgo[] = "";
  const char DefaultSecurityClientKsPath[] = "";
...
LibraryAuthInitializeFn SystemProperties::managedAuthInitializeFnGeneric = NULL;

SystemProperties::SystemProperties
(const PropertiesPtr& propertiesPtr, const char* configFile) :
    ...
    m_onClientDisconnectClearPdxTypeIds(DefaultOnClientDisconnectClearPdxTypeIds)
{
  ...
  class ProcessPropsVisitor : public Properties::Visitor
  {
    ...
    public:
    ProcessPropsVisitor( SystemProperties* sysProps ) : m_sysProps( sysProps ) { }
  ...
  try {
    std::string defsysprops = CppCacheLibrary::getProductDir() +
      "/defaultSystem/gfcpp.properties";
    givenConfigPtr->load( defsysprops.c_str() );
  } catch ( Exception & ) {
    LOGERROR("Unable to determine Product Directory. Please set the "
  ...
  m_AuthIniLoaderLibrary = m_securityPropertiesPtr->find(DefaultAuthIniLoaderLibrary);
  m_AuthIniLoaderFactory = m_securityPropertiesPtr->find(DefaultAuthIniLoaderFactory);
...
void SystemProperties::processProperty(const char* property, const char* value)
{
  ...
  if(prop == ThreadPoolSize) {
    ...
    if ( ! *end )
    {
      ...
      throwError
        (("SystemProperties: non-integer " + prop + "=" + value)
         .c_str());
  ...
  (("SystemProperties: non-integer " + prop + "=" + value)
   .c_str());
    ...
    if (val == "false")
      ...
      throwError
        (("SystemProperties: non-boolean " + prop + "=" + val)
         .c_str());
  ...
  (("SystemProperties: non-boolean " + prop + "=" + val)
   .c_str());
    ...
    try {
      ...
      throwError
  (("SystemProperties: unknown log level " + prop + "=" + value).c_str());
    ...
    if (!*end){
      ...
      throwError(("SystemProperties: non-integer " + prop + "=" + value).c_str());
    }
  ...
  (("SystemProperties: non-integer " + prop + "=" + value)
   .c_str());
  ...
  }else if(prop == TombstoneTimeoutInMSec){//Added system properties for TombStone-Timeout.
    char* end;
    uint32_t si = strtoul(value, &end, 10);
    if(!*end){
    ...
    ACE_OS::snprintf(msg, 1000,
        "SystemProperties: unknown property: %s = %s", property, value);
    throwError(msg);
  }
}
...
void SystemProperties::logSettings( )
{
   ...
   */
  ACE_OS::snprintf( buf , 2048, "%" PRIu32, (int)heapLRULimit() );
  settings += "\n  heap-lru-limit = ";
  settings += buf;
...
AuthInitializePtr SystemProperties::getAuthLoader() {
  if ((m_authInitializer == NULLPTR) && (m_AuthIniLoaderLibrary != NULLPTR &&
    ...
    if ( managedAuthInitializeFn != NULL
      ...
      m_authInitializer = ( *managedAuthInitializeFn )( m_AuthIniLoaderLibrary->asChar(),
                                                                 m_AuthIniLoaderFactory->asChar() );
    } else {
      AuthInitialize* (*funcptr)();
      funcptr = (AuthInitialize* (*)()) getFactoryFunc( m_AuthIniLoaderLibrary->asChar(),
                                                          m_AuthIniLoaderFactory->asChar() );
      if( funcptr == NULL ) {
