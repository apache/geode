 ...
 * Top level class for configuring and using GemFire on a client.This should be called once to create {@link Cache}.
 *<p>
...
class CPPCACHE_EXPORT CacheFactory: public SharedBase{
  ...
  */
  static CacheFactoryPtr createCacheFactory(const PropertiesPtr& dsProps = NULLPTR);
  ...
  */
   ...
   */
   static CachePtr create(const char *name, DistributedSystemPtr& system,
       const char* cacheXml = 0, const CacheAttributesPtr& attrs = NULLPTR, bool ignorePdxUnreadFields = false, bool readPdxSerialized = false);
  ...
  /**
   * The frequency with which client updates the locator list. To disable this set its
   * value to 0.
   * @param updateLocatorListInterval is the amount of time in milliseconds between
   ...
   * The setPRSingleHopEnabled avoids extra hops only for following cache operations:<br>
   ...
   * to -1.
   * @param name is boolean whether PR Single Hop optimization is enabled or not.
   * @return a reference to <code>this</code>
   */
  ...
  static void create_(const char* name, DistributedSystemPtr& system, const char* id_data, CachePtr& cptr, bool ignorePdxUnreadFields, bool readPdxSerialized);
  ...
  static void init();
  static void cleanup();
  static void handleXML( CachePtr& cachePtr, const char* cachexml, DistributedSystemPtr& system );
