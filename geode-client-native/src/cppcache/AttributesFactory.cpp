...
: m_regionAttributes()
    /* adongre
     * CID 28922: Uninitialized scalar field (UNINIT_CTOR)
     */
     ,m_useDefaultClientNotification(false)
{
}
...
void AttributesFactory::setPartitionResolver(const PartitionResolverPtr& aResolver)
{
...
void AttributesFactory::setPartitionResolver(const char* lib, const char* func)
{
  m_regionAttributes.setPartitionResolver(lib, func);
}

void AttributesFactory::setEntryIdleTimeout(ExpirationAction::Action action, int idleTimeout)
{
   m_regionAttributes.m_entryIdleTimeout = idleTimeout;
   m_regionAttributes.m_entryIdleTimeoutExpirationAction = action;
}

void AttributesFactory::setEntryTimeToLive(ExpirationAction::Action action, int timeToLive)
{
   m_regionAttributes.m_entryTimeToLive = timeToLive;
   m_regionAttributes.m_entryTimeToLiveExpirationAction = action;
}

void AttributesFactory::setRegionIdleTimeout(ExpirationAction::Action action, int idleTimeout)
{
   m_regionAttributes.m_regionIdleTimeout = idleTimeout;
   m_regionAttributes.m_regionIdleTimeoutExpirationAction = action;
}
void AttributesFactory::setRegionTimeToLive(ExpirationAction::Action action, int timeToLive)
{
   m_regionAttributes.m_regionTimeToLive = timeToLive;
   m_regionAttributes.m_regionTimeToLiveExpirationAction = action;
}

void AttributesFactory::setScope(const ScopeType::Scope scopeType)
{
   m_regionAttributes.m_scope = scopeType;
}

void AttributesFactory::setClientNotificationEnabled(bool clientNotificationEnabled)
{
...
RegionAttributesPtr AttributesFactory::createRegionAttributes()
{
  ...
  if( m_regionAttributes.m_poolName != NULL )
  {
    ...
    if (pool == NULLPTR) {
      throw IllegalStateException("Pool not found while creating region attributes");
    }
	  setClientNotificationEnabled(pool->getSubscriptionEnabled());
	  if( pool->getSubscriptionRedundancy() >0 ) setClientNotificationEnabled(true);
  }
  */
...
void AttributesFactory::validateAttributes(RegionAttributes& attrs)
{
  if (!attrs.m_caching) {
	...
	("Local scope is incompatible with disabled caching");
    ...
    if (attrs.m_diskPolicy != DiskPolicyType::NONE)
    {
      if(attrs.m_lruEntriesLimit == 0)
      throw IllegalStateException
        ("When DiskPolicy is OVERFLOWS, LRU entries limit must be non-zero with disabled caching");
    }
  }

  if(attrs.m_diskPolicy != DiskPolicyType::NONE){
    if(attrs.m_persistenceManager == NULLPTR &&(attrs.m_persistenceLibrary==NULL || attrs.m_persistenceFactory == NULL))
       throw IllegalStateException
...
void AttributesFactory::setDiskPolicy( const DiskPolicyType::PolicyType diskPolicy )
{
...
void AttributesFactory::setEndpoints( const char * endpoints )
{
  m_regionAttributes.setEndpoints( endpoints );
}

void AttributesFactory::setPersistenceManager(const PersistenceManagerPtr& persistenceManager, const PropertiesPtr& props)
{
  m_regionAttributes.m_persistenceManager = persistenceManager;
  m_regionAttributes.m_persistenceProperties = props;
}

void AttributesFactory::setPersistenceManager(const char* lib,
