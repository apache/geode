...
{
  /* adongre
   * CID 28991: Structurally dead code (UNREACHABLE)
   * added one return value, do not return from any 
   * code path.
   */
  ...
  if (regAttr->getPoolName() != NULL && strlen(regAttr->getPoolName()) > 0)
  {
    ...
    //return cache->createRegion(name, regAttr);
    retRegionPtr =  cache->createRegion(name, regAttr);
  }
  else
  {
    ...
    if( ! regAttr->getScope() == ScopeType::LOCAL )
    {
...
RegionFactoryPtr RegionFactory::setCacheLoader(const CacheLoaderPtr& cacheLoader)
{
...
RegionFactoryPtr RegionFactory::setCacheWriter(const CacheWriterPtr& cacheWriter)
{
  m_attributeFactory->setCacheWriter(cacheWriter);
  RegionFactoryPtr rfPtr(this);
  return rfPtr;
}
RegionFactoryPtr RegionFactory::setCacheListener(const CacheListenerPtr& aListener)
{
  m_attributeFactory->setCacheListener(aListener);
  RegionFactoryPtr rfPtr(this);
  return rfPtr;
}
RegionFactoryPtr RegionFactory::setPartitionResolver(const PartitionResolverPtr& aResolver)
{
...
RegionFactoryPtr RegionFactory::setCacheLoader(const char* lib, const char* func)
{
...
RegionFactoryPtr RegionFactory::setCacheWriter(const char* lib, const char* func)
{
...
RegionFactoryPtr RegionFactory::setCacheListener(const char* lib, const char* func)
{
...
RegionFactoryPtr RegionFactory::setPartitionResolver(const char* lib, const char* func)
{
...
RegionFactoryPtr RegionFactory::setEntryIdleTimeout(ExpirationAction::Action action, int idleTimeout)
{
...
RegionFactoryPtr RegionFactory::setEntryTimeToLive(ExpirationAction::Action action, int timeToLive)
{
...
RegionFactoryPtr RegionFactory::setRegionIdleTimeout(ExpirationAction::Action action, int idleTimeout)
{
  m_attributeFactory->setRegionIdleTimeout(action, idleTimeout);
  RegionFactoryPtr rfPtr(this);
  return rfPtr;
}
RegionFactoryPtr RegionFactory::setRegionTimeToLive(ExpirationAction::Action action, int timeToLive)
{
...
RegionFactoryPtr RegionFactory::setLruEntriesLimit(const uint32_t entriesLimit )
{
...
RegionFactoryPtr RegionFactory::setDiskPolicy( const DiskPolicyType::PolicyType diskPolicy )
{
...
RegionFactoryPtr RegionFactory::setPersistenceManager(const PersistenceManagerPtr& persistenceManager, 
...
RegionFactoryPtr RegionFactory::setPersistenceManager(const char* lib,
    const char* func, const PropertiesPtr& config)
{
