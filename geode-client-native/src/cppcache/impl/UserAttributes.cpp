...
UserAttributes::UserAttributes(PropertiesPtr credentials, PoolPtr pool, ProxyCache * proxyCache):m_isUserAuthenticated(false), m_pool(pool)
{
...
bool UserAttributes::isCacheClosed()
{
  return m_proxyCache->isClosed();
}
...
void UserAttributes::unAuthenticateEP(TcrEndpoint * endpoint)
{
  ...
  if (m_connectionAttr.size() == 0)
    return;
  //TODO:Hitesh it is always returning first one
  //need to take care when FE for onServers();
  //TODO:Hitesh chk before returing whether endpoint is up or not
  //std::map<std::string, UserConnectionAttributes>::iterator it;
...
PoolPtr UserAttributes::getPool()
{
  return m_pool;
}


UserConnectionAttributes* UserAttributes::getConnectionAttribute(TcrEndpoint* ep)
{
...
{
  LOGDEBUG("UserAttributes::isEndpointAuthenticated: (TcrEndpoint* ep) with EP.");
  if (m_connectionAttr.size() == 0 )
    return false;
...
ProxyCachePtr UserAttributes::getProxyCache()
{
  return m_proxyCache;
}

ACE_TSS<TSSUserAttributesWrapper> TSSUserAttributesWrapper::s_gemfireTSSUserAttributes;
...
void GuardUserAttribures::setProxyCache(ProxyCachePtr proxyCache)
{
  ...
  if(m_proxyCache != NULLPTR && !proxyCache->isClosed())
    TSSUserAttributesWrapper::s_gemfireTSSUserAttributes->setUserAttributes(proxyCache->m_userAttributes);
  else
...
GuardUserAttribures::~GuardUserAttribures()
{
  if(m_proxyCache != NULLPTR)
    TSSUserAttributesWrapper::s_gemfireTSSUserAttributes->setUserAttributes(NULLPTR);
}
