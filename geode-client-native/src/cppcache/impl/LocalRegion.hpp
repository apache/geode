  ...
  */
  ...
  public:
    ...
    */
    ...
    RegionAttributesPtr getAttributes() const
    {
    ...
    GfErrType localClearNoThrow(const UserDataPtr& aCallbackArgument = NULLPTR,
    ...
    bool remove(const CacheableKeyPtr& key, 
      const CacheablePtr& value, const UserDataPtr& aCallbackArgument = NULLPTR);
    bool removeEx(const CacheableKeyPtr& key, const UserDataPtr& aCallbackArgument = NULLPTR);
    bool localRemove(const CacheableKeyPtr& key, 
      const CacheablePtr& value, const UserDataPtr& aCallbackArgument = NULLPTR);
    bool localRemoveEx(const CacheableKeyPtr& key, const UserDataPtr& aCallbackArgument = NULLPTR);
    ...
    void getAll(const VectorOfCacheableKey& keys, HashMapOfCacheablePtr values,
        HashMapOfExceptionPtr exceptions, bool addToLocalCache, const UserDataPtr& aCallbackArgument = NULLPTR);
    void putAll(const HashMapOfCacheable& map, uint32_t timeout = DEFAULT_RESPONSE_TIMEOUT, const UserDataPtr& aCallbackArgument = NULLPTR);
    void removeAll(const VectorOfCacheableKey& keys, const UserDataPtr& aCallbackArgument = NULLPTR);
    uint32_t size();
    ...
    virtual GfErrType putNoThrow(const CacheableKeyPtr& key,
        ...
        DataInput* delta = NULL, EventIdPtr eventId = NULLPTR);
    ...
    virtual GfErrType removeNoThrowEx(const CacheableKeyPtr& key,
        const UserDataPtr& aCallbackArgument, int updateCount, 
        const CacheEventFlags eventFlags, VersionTagPtr versionTag);
    virtual GfErrType putAllNoThrow(const HashMapOfCacheable& map,
        uint32_t timeout = DEFAULT_RESPONSE_TIMEOUT, 
        const UserDataPtr& aCallbackArgument = NULLPTR);
    virtual GfErrType removeAllNoThrow(const VectorOfCacheableKey& keys,
        const UserDataPtr& aCallbackArgument = NULLPTR);
    ...
    // [sumedh] moved putLocal to public since this is used by a few other
    ...
    GfErrType putLocal(const char* name, bool isCreate,
        ...
        int destroyTracker, VersionTagPtr versionTag, DataInput* delta = NULL,
    ...
    ExpirationAction::Action adjustRegionExpiryAction( ExpirationAction::Action action );
    ExpirationAction::Action adjustEntryExpiryAction( ExpirationAction::Action action );
    ...
    virtual void adjustCacheListener(const char* libpath, const char* factoryFuncName);
    virtual void adjustCacheLoader(const CacheLoaderPtr& aLoader);
    virtual void adjustCacheLoader(const char* libpath, const char* factoryFuncName);
    virtual void adjustCacheWriter(const CacheWriterPtr& aWriter);
    virtual void adjustCacheWriter(const char* libpath, const char* factoryFuncName);
  ...
  protected:
    ...
    virtual GfErrType putAllNoThrow_remote(const HashMapOfCacheable& map, VersionedCacheableObjectPartListPtr& versionedObjPartList, uint32_t timeout, const UserDataPtr& aCallbackArgument);
    virtual GfErrType removeAllNoThrow_remote(const VectorOfCacheableKey& keys, VersionedCacheableObjectPartListPtr& versionedObjPartList, const UserDataPtr& aCallbackArgument);
    ...
    virtual GfErrType removeNoThrowEX_remote(const CacheableKeyPtr& keyPtr, 
      const UserDataPtr& aCallbackArgument, VersionTagPtr& versionTag);
    virtual GfErrType invalidateNoThrow_remote(const CacheableKeyPtr& keyPtr,
        const UserDataPtr& aCallbackArgument, VersionTagPtr& versionTag);
    virtual GfErrType getAllNoThrow_remote(const VectorOfCacheableKey* keys,
        const HashMapOfCacheablePtr& values,
    ...
    virtual const PoolPtr& getPool() {
      return m_attachedPool;
    }

    void setPool(const PoolPtr& p ) {
      m_attachedPool = p;
    }
    ...
    {
    	return typeid(*this) == typeid(LocalRegion) || (eventFlags && eventFlags->isLocal());
    }

    // template method for put and create
    template<typename TAction>
    GfErrType updateNoThrow(const CacheableKeyPtr& key,
        const CacheablePtr& value, const UserDataPtr& aCallbackArgument,
        CacheablePtr& oldValue, int updateCount,
        const CacheEventFlags eventFlags, VersionTagPtr versionTag, 
        DataInput* delta = NULL, EventIdPtr eventId = NULLPTR);
    ...
    bool invokeCacheWriterForEntryEvent(const CacheableKeyPtr& key,
        CacheablePtr& oldValue, const CacheablePtr& newValue,
    ...
    GfErrType invokeCacheListenerForEntryEvent(const CacheableKeyPtr& key,
        CacheablePtr& oldValue, const CacheablePtr& newValue,
        const UserDataPtr& aCallbackArgument, CacheEventFlags eventFlags,
        EntryEventType type, bool isLocal = false);
    ...
    virtual GfErrType getNoThrow_FullObject( EventIdPtr eventId, CacheablePtr& fullObject, VersionTagPtr& versionTag);
