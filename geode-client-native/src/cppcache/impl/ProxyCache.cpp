...
using namespace gemfire ;
   ...
   */
bool ProxyCache::isClosed() const
{
  return m_isProxyCacheClosed;
}
...
void ProxyCache::close()
{
  ...
  if (!m_isProxyCacheClosed ) 
  {
    ...
    {
      ProxyRemoteQueryService* prqs = static_cast<ProxyRemoteQueryService*>(m_remoteQueryService.ptr());
      prqs->closeCqs(false);
    }
...
{  
  ...
  if(!m_isProxyCacheClosed)
  {
    ...
    if (result != NULLPTR) {
      ...
      throw IllegalArgumentException("The Region argument is not attached with the pool, which used to create this user cache.");
    }
    ...
    if ( realCache != NULLPTR && !realCache->isClosed())
    {
     ...
     {  
       for(int32_t i = 0; i< tmp.size(); i++)
       {
         ...
         if (strcmp(m_userAttributes->getPool()->getName(), reg->getAttributes()->getPoolName()) == 0 )
         {
...
PdxInstanceFactoryPtr ProxyCache::createPdxInstanceFactory(const char* className)
{
