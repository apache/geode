...
public:
  ...
  CachePerfStats() {
    ...
    if ( statsType == NULL ) {
      ...
      statDescArr[0] = factory->createIntCounter
        ("creates", "The total number of cache creates", "entries", largerIsBetter);
      ...
      statDescArr[4] = factory->createIntCounter
        ("misses", "The total number of cache misses", "entries", ! largerIsBetter);
      statDescArr[5] = factory->createIntGauge
        ("entries", "The current number of cache entries", "entries", largerIsBetter);
      statDescArr[6] = factory->createIntCounter
        ("destroys", "The total number of cache destroys", "entries", largerIsBetter);
      statDescArr[7] = factory->createIntCounter
        ("overflows", "The total number of cache overflows to persistence backup", "entries", largerIsBetter);
      statDescArr[8] = factory->createIntCounter
        ("retrieves", "The total number of cache entries fetched from persistence backup into the cache", "entries", largerIsBetter);
      statDescArr[9] = factory->createIntCounter
        ("cacheListenerCallsCompleted", "Total number of times a cache listener call has completed", "operations", largerIsBetter);
      statDescArr[10] = factory->createIntCounter
        ("deltaPuts", "Total number of puts containing delta that have been sent from client to server", "entries", largerIsBetter);
      statDescArr[11] = factory->createIntCounter
        ("processedDeltaMessages","Total number of messages containing delta received from server and processed after reception", "entries", largerIsBetter);
      statDescArr[12] = factory->createIntCounter
        ("deltaMessageFailures", "Total number of messages containing delta (received from server) but could not be processed after reception.", "entries", largerIsBetter);
      statDescArr[13] = factory->createLongCounter
          ("processedDeltaMessagesTime", "Total time spent applying delta (received from server) on existing values at client", "operations", largerIsBetter);
      statDescArr[14] = factory->createIntCounter
          ("tombstoneCount", "The total number of current tombstones", "entries", false);
      statDescArr[15] = factory->createLongCounter
          ("nonReplicatedTombstonesSize", "The total number of bytes consumed by tombstones for all regions in client processes", "bytes", false);
      statDescArr[16] = factory->createIntCounter
          ("conflatedEvents", "The number of conflicting events that have been elided and not passed on to event listeners", "operations", largerIsBetter);
      statDescArr[17] = factory->createIntCounter
              ("pdxInstanceDeserializations", "Total number of times getObject has been called on a PdxInstance.", "entries", largerIsBetter);
      statDescArr[18] = factory->createLongCounter
          ("pdxInstanceDeserializationTime", "Total amount of time, in nanoseconds, spent deserializing PdxInstances by"
            "calling getObject.", "operations", ! largerIsBetter);
      statDescArr[19] = factory->createIntCounter
              ("pdxInstanceCreations", "Total number of times a deserialization created a PdxInstance.", "entries", largerIsBetter);
      statDescArr[20] = factory->createIntCounter
              ("pdxSerializations", "Total number of pdx serializations.", "entries", largerIsBetter);
      statDescArr[21] = factory->createLongCounter
              ("pdxSerializedBytes", "Total number of bytes produced by pdx serialization.", "entries", ! largerIsBetter);
      statDescArr[22] = factory->createIntCounter
              ("pdxDeserializations", "Total number of pdx deserializations.", "entries",  largerIsBetter);
      statDescArr[23] = factory->createLongCounter
              ("pdxDeserializedBytes", "Total number of bytes read by pdx deserialization.", "entries", ! largerIsBetter);

      statsType = factory->createType
        ("CachePerfStats", "Statistics about native client cache", statDescArr, 24);
    }
    GF_D_ASSERT(statsType != NULL);
    //Create Statistics object
    m_cachePerfStats = factory->createAtomicStatistics(statsType, "CachePerfStats" );
    ...
    m_processedDeltaMessagesTime = statsType->nameToId("processedDeltaMessagesTime");
    ...
    m_pdxInstanceDeserializationsId = statsType->nameToId("pdxInstanceDeserializations");
    m_pdxInstanceDeserializationTimeId = statsType->nameToId("pdxInstanceDeserializationTime");
  ...
  virtual ~CachePerfStats() {
    m_cachePerfStats = NULL;
  }

  void close()
  {
	/*StatisticDescriptor** statDescArr = m_cachePerfStats->getType()->getStatistics();
	for ( int i = 0; i < m_cachePerfStats->getType()->getDescriptorsCount(); i++) {
	    delete statDescArr[i];
	}*/
    m_cachePerfStats->close();
  }

  inline void incDestroys()
  {
    m_cachePerfStats->incInt(m_destroysId, 1);
  }

  inline void incCreates()
  {
    m_cachePerfStats->incInt(m_createsId, 1);
  }

  inline void incPuts()
  {
    m_cachePerfStats->incInt(m_putsId, 1);
  }

  inline void incGets()
  {
    m_cachePerfStats->incInt(m_getsId, 1);
  }

  inline void incHits()
  {
    m_cachePerfStats->incInt(m_hitsId, 1);
  }

  inline void incMisses()
  {
    m_cachePerfStats->incInt(m_missesId, 1);
  }

  inline void incOverflows()
  {
    m_cachePerfStats->incInt(m_overflowsId, 1);
  }

  inline void incRetrieves()
  {
  ...
  inline void incDeltaPut()
  {
    m_cachePerfStats->incInt(m_deltaPut, 1);
  }
  ...
  inline gemfire_statistics::Statistics* getStat()
  {
