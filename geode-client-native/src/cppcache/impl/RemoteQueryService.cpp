...
RemoteQueryService::RemoteQueryService( CacheImpl * cptr, ThinClientPoolDM* poolDM ) : m_invalid( true ),
  m_cqService(NULLPTR)
{
  if ( poolDM ) {
    m_tccdm = poolDM;
    ...
    m_tccdm = new ThinClientCacheDistributionManager(cptr->tcrConnectionManager());
  }
...
{
  LOGDEBUG("RemoteQueryService::newQuery: multiuserMode = %d ", m_tccdm->isMultiUserMode());
  if(!m_tccdm->isMultiUserMode()) 
  {
    TryReadGuard guard( m_rwLock, m_invalid );

    if ( m_invalid ) {
      throw CacheClosedException( "QueryService::newQuery: Cache has been closed." );
    }
    LOGDEBUG( "RemoteQueryService: creating a new query: %s", querystring );
    return QueryPtr(new RemoteQuery(querystring,
        RemoteQueryServicePtr(this), m_tccdm));
  }
  else
  {
    UserAttributesPtr ua = TSSUserAttributesWrapper::s_gemfireTSSUserAttributes->getUserAttributes();
    TryReadGuard guard( m_rwLock, m_invalid );
    ...
    LOGDEBUG( "RemoteQueryService: creating a new query: %s", querystring );
    return QueryPtr(new RemoteQuery(querystring,
      RemoteQueryServicePtr(this), m_tccdm, ua->getProxyCache()));
  }
}
...
GfErrType RemoteQueryService::executeAllCqs(TcrEndpoint * endpoint)
{
  ...
  if (m_cqService == NULLPTR) {
    LOGFINE("RemoteQueryService: no cq to execute after failover to endpoint[%s]",
      endpoint->name().c_str());
    return GF_NOERR;
  }
  else {
    LOGFINE("RemoteQueryService: execute all cqs after failover to endpoint[%s]",
      endpoint->name().c_str());
...
void RemoteQueryService::executeCqs()
{
  ...
  if ( m_invalid ) {
    throw CacheClosedException( "QueryService::executeCqs: Cache has been closed." );
  }
  //If cqService has not started, then no cq exists
  if (m_cqService != NULLPTR) {
...
CqServiceStatisticsPtr RemoteQueryService::getCqServiceStatistics()
{
  ...
  if ( m_invalid ) {
    throw CacheClosedException( "QueryService::getCqServiceStatistics: Cache has been closed." );
  }
  //If cqService has not started, then no cq exists
  if (m_cqService != NULLPTR) {
...
CacheableArrayListPtr RemoteQueryService::getAllDurableCqsFromServer()
{
  TryReadGuard guard( m_rwLock, m_invalid );
  if ( m_invalid ) {
    throw CacheClosedException( "QueryService::getAllDurableCqsFromServer: Cache has been closed." );
  }
  //If cqService has not started, then no cq exists
  if (m_cqService != NULLPTR) 
...
void RemoteQueryService::invokeCqConnectedListeners(ThinClientPoolDM* pool, bool connected) {    
  if ( m_cqService == NULLPTR ) {    
    return;
  }  
  std::string poolName;
