...
*
* The implementation of the function behaviors specified in the corresponding .hpp file.
*
*========================================================================
*/
...
int DistributedSystemImpl::currentInstances()
{
  ...
  if(DistributedSystem::getInstance() != NULLPTR &&
    ...
    !DistributedSystem::getInstance()->getSystemProperties()->isAppDomainEnabled()) {
    return 0;
  }
...
void DistributedSystemImpl::CallCliCallBack()
{
  ...
  if(m_isCliCallbackSet == true) {
    for (std::map<int , CliCallbackMethod>::iterator iter = m_cliCallbackMap.begin();
...
void DistributedSystemImpl::registerCliCallback( int appdomainId, CliCallbackMethod clicallback) 
{
  ACE_Guard<ACE_Recursive_Thread_Mutex> disconnectGuard(m_cliCallbackLock);
...
{
  ACE_Guard<ACE_Recursive_Thread_Mutex> disconnectGuard(m_cliCallbackLock);
  std::map<int , CliCallbackMethod>::iterator iter = m_cliCallbackMap.find(appdomainId);
