...
namespace gemfire {
  ...
  public:
    ...
    PdxReaderWithTypeCollector(DataInput& dataInput, PdxTypePtr pdxType, int pdxlen);

    virtual ~PdxReaderWithTypeCollector();

    PdxTypePtr getLocalType() const {
      return m_newPdxType;
    }
...
/*
    ...
    *
    ...
    virtual void readASCIIHuge(const char* fieldName, char** value,
      uint32_t* len = NULL);
    ...
    *
    ...
    virtual void readUTF(const char* fieldName, char** value,
      uint16_t* len = NULL);

    *
     * Read a UTF Huge string from the <code>PdxReader</code>.
     * @param fieldName name of the field which needs to serialize
     * @param value value of the field which needs to serialize
