...
namespace gemfire {
  ...
  /* adongre  - Coverity II
   * Non-static class member ""m_currentOffsetIndex"" is not initialized in this constructor nor in any functions that it calls.
   * Non-static class member ""m_startPositionOffset"" is not initialized in this constructor nor in any functions that it calls. 
   */
  PdxLocalWriter::PdxLocalWriter() :
      m_dataOutput(NULL),
      m_pdxType(NULLPTR),
      m_currentOffsetIndex(0),
      m_startPositionOffset(0), m_startPosition(NULL), m_domainClassName(NULL), m_pdxClassName(NULL) { // COVERUTY --> 29282 Uninitialized pointer field
    //m_dataOutput = NULL;
    //m_pdxType =NULLPTR;
  }
  ...
  PdxLocalWriter::PdxLocalWriter(DataOutput& output, PdxTypePtr pdxType) {
       ...
       * Non-static class member ""m_domainClassName"" is not initialized in this constructor nor in any functions that it calls. 
       * Fix : 
       */
      m_domainClassName = NULL;
  }

  PdxLocalWriter::PdxLocalWriter(DataOutput& dataOutput, PdxTypePtr pdxType, const char* pdxClassName) {
...
void PdxLocalWriter::initialize() {
  ...
  //start position, this should start of dataoutput buffer and then use bufferlen
  ...
  void PdxLocalWriter::writeOffsets(int32 len) {
    if (len <= 0xff) {
      for (int i = (int)m_offsets.size() -1; i>0; i--){
        m_dataOutput->write((uint8_t) m_offsets[i]);
      }
    } else if (len <= 0xffff) {
      for (int i = (int)m_offsets.size() -1; i>0; i--){
        m_dataOutput->writeInt((uint16_t) m_offsets[i]);
      }
    } else {
      for (int i = (int)m_offsets.size() -1; i>0; i--){
        m_dataOutput->writeInt((uint32_t) m_offsets[i]);
      }
    }
  }
  ...
  PdxWriterPtr PdxLocalWriter::writeUnreadFields(PdxUnreadFieldsPtr unread)
  {
    ...
    {
      throw new IllegalStateException ("WriteUnreadFields must be called before any other fields are written.");
    }
    ...
    if(unread != NULLPTR)
    {
      ...
      if (m_preserveData != NULLPTR){
        m_pdxType = PdxTypeRegistry::getPdxType(m_preserveData->getMergedTypeId());
        if(m_pdxType == NULLPTR)
        {
          //its local type
        ...
        throw new IllegalStateException("PdxLocalWriter::writeUnreadFields: m_preserveData should not be NULL");
      }
    }
  ...
  int32 PdxLocalWriter::calculateLenWithOffsets() {
		...
		if ( m_pdxType->getNumberOfVarLenFields() > 0)
			totalOffsets = m_pdxType->getNumberOfVarLenFields() - 1; //for first var len no need to append offset
  ...
  PdxWriterPtr PdxLocalWriter::writeChar(const char* fieldName, char value) {
    m_dataOutput->writeChar((uint16_t)value);
    return PdxWriterPtr(this);
    /* adongre  - Coverity II
     * CID 29296: Structurally dead code (UNREACHABLE) 
     * This code cannot be reached: "<temporary>.SharedPtr(gemfi...". 
     * Fix : Remove this line 
     */
    //return NULLPTR;
  }

  PdxWriterPtr PdxLocalWriter::writeWideChar(const char* fieldName, wchar_t value) {
    m_dataOutput->writeChar((uint16_t)value);
    return PdxWriterPtr(this);
  }
  ...
  PdxWriterPtr PdxLocalWriter::writeDate(const char* fieldName, CacheableDatePtr date) {
    //m_dataOutput->writeObject(date.ptr());
    if(date != NULLPTR){
      date->toData(*m_dataOutput);
    }else{
      m_dataOutput->writeInt((uint64_t)-1L);
    }
    return PdxWriterPtr(this);
  }

  PdxWriterPtr PdxLocalWriter::writeString(const char* fieldName, const char* value){
    addOffset();
    if ( value == NULL ) {
      m_dataOutput->write((int8_t) GemfireTypeIds::CacheableNullString);
    }else{
      int32_t len = DataOutput::getEncodedLength( value);
      if (len > 0xffff){
        //write HugUTF
        m_dataOutput->write((int8_t) GemfireTypeIds::CacheableStringHuge);
        m_dataOutput->writeUTFHuge(value);
      }else{
        //Write normal UTF
        m_dataOutput->write((int8_t) GemfireTypeIds::CacheableString);
  ...
  PdxWriterPtr PdxLocalWriter::writeWideString(const char* fieldName, const wchar_t* value){
    addOffset();
    if ( value == NULL ) {
      m_dataOutput->write((int8_t) GemfireTypeIds::CacheableNullString);
    }else{
  ...
  PdxWriterPtr PdxLocalWriter::writeWideStringwithoutOffset(const wchar_t* value){
    if ( value == NULL ) {
      m_dataOutput->write((int8_t) GemfireTypeIds::CacheableNullString);
    }else{
      int32_t len = DataOutput::getEncodedLength( value);
  ...
  PdxWriterPtr PdxLocalWriter::writeStringArray(const char* fieldName, char** array, int length){
    addOffset();
    if(array == NULL)
    {
      m_dataOutput->write((int8_t)-1);
      //WriteByte(-1);
    }
    else{
  ...
  PdxWriterPtr PdxLocalWriter::writeWideStringArray(const char* fieldName, wchar_t** array, int length){
    addOffset();
    if(array == NULL)
    {
      m_dataOutput->write((int8_t)-1);
  ...
  PdxWriterPtr PdxLocalWriter::writeObject(const char* fieldName, SerializablePtr value) {
    addOffset();
    ...
    if(value != NULLPTR && value->typeId() == (int8_t) GemfireTypeIds::CacheableEnum) {
      enumValPtr = dynCast<CacheableEnumPtr>(value);
    }
    ...
    if (enumValPtr != NULLPTR) {
      ...
      if(value != NULLPTR && value->typeId() == GemfireTypeIds::CacheableObjectArray)
      {
        objArrPtr = dynCast<CacheableObjectArrayPtr>(value);
        m_dataOutput->write((int8_t)GemfireTypeIds::CacheableObjectArray);
        m_dataOutput->writeArrayLen(objArrPtr->length());
        m_dataOutput->write((int8_t)GemfireTypeIdsImpl::Class);

        _VectorOfCacheable::Iterator iter = objArrPtr->begin( );
        PdxSerializablePtr actualObjPtr = dynCast<PdxSerializablePtr>(*iter);

        m_dataOutput->write((int8_t)GemfireTypeIds::CacheableASCIIString);
  ...
  PdxWriterPtr PdxLocalWriter::writeBooleanArray(const char* fieldName, bool* array, int length) {
    addOffset();
    writeObject(array, length);
    return PdxWriterPtr(this);
  }

  PdxWriterPtr PdxLocalWriter::writeCharArray(const char* fieldName, char* array, int length) {
    addOffset();
    writePdxCharArray(array, length);
    return PdxWriterPtr(this);
  }

  PdxWriterPtr PdxLocalWriter::writeWideCharArray(const char* fieldName, wchar_t* array, int length) {
    addOffset();
    writeObject(array, length);
    return PdxWriterPtr(this);
  }

  PdxWriterPtr PdxLocalWriter::writeByteArray(const char* fieldName, int8_t* array, int length) {
    addOffset();
    writeObject(array, length);
    return PdxWriterPtr(this);
  }

  PdxWriterPtr PdxLocalWriter::writeShortArray(const char* fieldName, int16_t* array, int length) {
    addOffset();
    writeObject(array, length);
    return PdxWriterPtr(this);
  }

  PdxWriterPtr PdxLocalWriter::writeIntArray(const char* fieldName, int32_t* array, int length) {
    addOffset();
    writeObject(array, length);
    return PdxWriterPtr(this);
  }

  PdxWriterPtr PdxLocalWriter::writeLongArray(const char* fieldName, int64_t* array, int length) {
    addOffset();
    writeObject(array, length);
    return PdxWriterPtr(this);
  }

  PdxWriterPtr PdxLocalWriter::writeFloatArray(const char* fieldName, float* array, int length) {
    addOffset();
    writeObject(array, length);
    return PdxWriterPtr(this);
  }

  PdxWriterPtr PdxLocalWriter::writeDoubleArray(const char* fieldName, double* array, int length) {
    addOffset();
    writeObject(array, length);
    return PdxWriterPtr(this);
  }

  PdxWriterPtr PdxLocalWriter::writeObjectArray(const char* fieldName, CacheableObjectArrayPtr array){
	addOffset();
    if(array != NULLPTR){
  ...
  PdxWriterPtr PdxLocalWriter::writeArrayOfByteArrays(const char* fieldName, int8_t** byteArrays, int arrayLength, int* elementLength ){
    addOffset();
    if(byteArrays != NULL)
    {
  ...
  {    
    uint8_t* stPos = const_cast<uint8_t*> (m_dataOutput->getBuffer()) + m_startPositionOffset;
  ...
  void PdxLocalWriter::writeByte(int8_t byte)
  {
    m_dataOutput->write(byte);
  }
