...
namespace gemfire {
    ...
    //std::map<PdxSerializablePtr, PdxRemotePreservedDataPtr> *PdxTypeRegistry::preserveData = NULL;
  ...
  bool PdxTypeRegistry::pdxIgnoreUnreadFields;
	...
	PdxTypeRegistry::PdxTypeRegistry() /*:pdxIgnoreUnreadFields (false), pdxReadSerialized(false)*/{
	}
    ...
    void PdxTypeRegistry::init()
    {
      ...
      throw gemfire::OutOfMemoryException( "Out of Memory while executing new in PdxTypeRegistry::init()");
      }*/
    }
	...
	int PdxTypeRegistry::testGetNumberOfPdxIds() {
		return (int)typeIdToPdxType->size();
	}
  ...
  int32 PdxTypeRegistry::getPDXIdForType(const char* type, const char* poolname, PdxTypePtr nType, bool checkIfThere){
  ...
  void PdxTypeRegistry::clear()
  {
    {
      WriteGuard guard(g_readerWriterLock);
      if(typeIdToPdxType != NULL)
        typeIdToPdxType->clear();

      if(remoteTypeIdToMergedPdxType != NULL)
        remoteTypeIdToMergedPdxType->clear();

      if(localTypeToPdxType != NULL)
        localTypeToPdxType->clear();

      if(intToEnum != NULLPTR)
        intToEnum->clear();

      if(enumToInt != NULLPTR)
        enumToInt->clear();

      if (pdxTypeToTypeIdMap != NULL)
  ...
  PdxTypePtr PdxTypeRegistry::getPdxType(int32 typeId) {
	...
	void PdxTypeRegistry::addLocalPdxType(const char* localType, PdxTypePtr pdxType) {
		WriteGuard guard(g_readerWriterLock);
		localTypeToPdxType->insert(std::pair<std::string, PdxTypePtr>(localType, pdxType));
	}
  ...
  PdxTypePtr PdxTypeRegistry::getLocalPdxType(const char* localType) {
    ...
    void PdxTypeRegistry::setPreserveData(PdxSerializablePtr obj, PdxRemotePreservedDataPtr pData){
      WriteGuard guard(getPreservedDataLock());
      pData->setOwner(obj);
      if(preserveData.contains(obj)){
        ...
        PreservedDataExpiryHandler* handler = new PreservedDataExpiryHandler(obj, 20);
        long id = CacheImpl::expiryTaskManager->scheduleExpiryTask(handler, 20, 0, false);
        pData->setPreservedDataExpiryTaskId(id);
        LOGDEBUG("PdxTypeRegistry::setPreserveData Schedule new expirt task with id=%ld", id);
        preserveData.insert(obj, pData);
      }

      LOGDEBUG("PdxTypeRegistry::setPreserveData Successfully inserted new entry in preservedData");
    }

    PdxRemotePreservedDataPtr PdxTypeRegistry::getPreserveData(PdxSerializablePtr pdxobj){
