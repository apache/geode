...
namespace gemfire {
  ...
  private:
    ...
    // private constructor
    inline CacheEventFlags(const uint8_t flags) :
      m_flags(flags)
    {
    }
  ...
  public:
    ...
    {
      return (m_flags & GF_NORMAL);
    }

    inline bool isLocal() const
    {
      return (m_flags & GF_LOCAL);
    }

    inline bool isNotification() const
    {
      return (m_flags & GF_NOTIFICATION);
    }
    ...
    {
      return (m_flags & GF_EVICTION);
    }

    inline bool isExpiration() const
    {
      return (m_flags & GF_EXPIRATION);
    }

    inline bool isCacheClose() const
    {
      return (m_flags & GF_CACHE_CLOSE);
    }

    inline bool isNoCacheWriter() const
    {
      return (m_flags & GF_NOCACHEWRITER);
    }
  ...
  */
  ...
  public:
    ...
    */
    ...
    virtual GfErrType getNoThrow(const CacheableKeyPtr& key,
        CacheablePtr& value, const UserDataPtr& aCallbackArgument) = 0;
    ...
    virtual GfErrType putNoThrow(const CacheableKeyPtr& key,
        ...
        const CacheEventFlags eventFlags, VersionTagPtr versionTag, 
        DataInput* delta = NULL, EventIdPtr eventId = NULLPTR) = 0;
    virtual GfErrType createNoThrow(const CacheableKeyPtr& key,
        const CacheablePtr& value, const UserDataPtr& aCallbackArgument,
        int updateCount, const CacheEventFlags eventFlags, VersionTagPtr versionTag) = 0;
    virtual GfErrType destroyNoThrow(const CacheableKeyPtr& key,
        const UserDataPtr& aCallbackArgument, int updateCount, 
        const CacheEventFlags eventFlags, VersionTagPtr versionTag) = 0;
    ...
    virtual ExpirationAction::Action adjustRegionExpiryAction( ExpirationAction::Action action ) = 0;
    virtual ExpirationAction::Action adjustEntryExpiryAction( ExpirationAction::Action action ) = 0;
    ...
    virtual void adjustCacheListener(const char* libpath, const char* factoryFuncName) = 0;
    virtual void adjustCacheLoader(const CacheLoaderPtr& aLoader) = 0;
    virtual void adjustCacheLoader(const char* libpath, const char* factoryFuncName) = 0;
    virtual void adjustCacheWriter(const CacheWriterPtr& aWriter) = 0;
    virtual void adjustCacheWriter(const char* libpath, const char* factoryFuncName) = 0;
