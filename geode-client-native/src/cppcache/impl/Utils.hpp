...
namespace gemfire
{
  ...
  public:
...
#ifdef __GNUC__
    inline static char* _gnuDemangledName(const char* typeIdName,
        size_t& len)
    {
      int status;
      char* demangledName =
          abi::__cxa_demangle(typeIdName, NULL, &len, &status);
    ...
    inline static CacheableStringPtr demangleTypeName(
    ...
    inline static int64 startStatOpTime()
	{
    	if(DistributedSystem::getSystemProperties() != NULL)
    		return  (DistributedSystem::getSystemProperties()->getEnableTimeStatistics()) ? NanoTimer::now() : 0;
    	else
    		return 0;
	}


    // Check objectSize() implementation return value and log a warning at most once.
    inline static uint32_t checkAndGetObjectSize(const CacheablePtr& theObject)
    {
      ...
      if ((objectSize == 0 || objectSize == ((uint32_t)-1)) && !youHaveBeenWarned) {
        LOGWARN("Object size for Heap LRU returned by class ID %d is 0 (zero) or -1 (UINT32_MAX). " 
          "Even for empty objects the size returned should be at least one (1 byte) and "
          "should not be -1 or UINT32_MAX.", theObject->classId());
        youHaveBeenWarned = true;
        LOGFINE("Type ID is %d for the object returning zero HeapLRU size", theObject->typeId());
      }
      GF_DEV_ASSERT(objectSize != 0 && objectSize != ((uint32_t)-1));
      return objectSize;
    }

    inline static void updateStatOpTime(gemfire_statistics::Statistics* m_regionStats, int32 statId, int64 start)
    {
    	if(DistributedSystem::getSystemProperties() != NULL){
    ...
    static void parseEndpointNamesString(const char* endpoints, std::unordered_set<
     ...
     */
    static CacheableStringPtr convertBytesToString(const uint8_t* bytes,
        int32_t length, size_t maxLength = _GF_MSG_LIMIT);

    /**
     * Convert the byte array to a string as "%d %d ...".
     * <code>maxLength</code> as zero implies no limit.
     */
    inline static CacheableStringPtr convertBytesToString(const char* bytes,
        int32_t length, size_t maxLength = _GF_MSG_LIMIT)
    {
