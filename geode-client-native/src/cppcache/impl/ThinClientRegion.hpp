...
namespace gemfire {
  ...
  */
    ...
    */
     ...
     */
     ...
     virtual void registerAllKeys(bool isDurable = false,
         ...
         bool getInitialValues = false, bool receiveValues = true);
     ...
     virtual SelectResultsPtr query(const char * predicate, uint32_t timeout = DEFAULT_QUERY_RESPONSE_TIMEOUT);
     virtual bool existsValue(const char * predicate, uint32_t timeout = DEFAULT_QUERY_RESPONSE_TIMEOUT);
     virtual SerializablePtr selectValue(const char * predicate, uint32_t timeout = DEFAULT_QUERY_RESPONSE_TIMEOUT);

    /** @brief Public Methods from RegionInternal
     *  These are all virtual methods
     */
    GfErrType putAllNoThrow_remote(const HashMapOfCacheable& map, VersionedCacheableObjectPartListPtr& versionedObjPartList, uint32_t timeout = DEFAULT_RESPONSE_TIMEOUT,
      const UserDataPtr& aCallbackArgument = NULLPTR);
    GfErrType removeAllNoThrow_remote(const VectorOfCacheableKey& keys, VersionedCacheableObjectPartListPtr& versionedObjPartList, const UserDataPtr& aCallbackArgument = NULLPTR);
    ...
    void addKeys( const VectorOfCacheableKey& keys, bool isDurable, bool receiveValues, InterestResultPolicy interestpolicy );
    void addRegex( const std::string& regex, bool isDurable, bool receiveValues, InterestResultPolicy interestpolicy );
    ...
    inline ThinClientBaseDM * getDistMgr() {
      return m_tcrdm;
    }

    CacheableVectorPtr reExecuteFunction(const char* func, const CacheablePtr& args, CacheableVectorPtr routingObj, uint8_t getResult, ResultCollectorPtr rc, int32_t retryAttempts, CacheableHashSetPtr& failedNodes, uint32_t timeout = DEFAULT_QUERY_RESPONSE_TIMEOUT);
    bool executeFunctionSH(const char* func, const CacheablePtr& args, uint8_t getResult, ResultCollectorPtr rc, 
      HashMapT<BucketServerLocationPtr, CacheableHashSetPtr>* locationMap, CacheableHashSetPtr& failedNodes, uint32_t timeout = DEFAULT_QUERY_RESPONSE_TIMEOUT, bool allBuckets = false);
    void executeFunction(const char* func, const CacheablePtr& args, CacheableVectorPtr routingObj, uint8_t getResult, ResultCollectorPtr rc, int32_t retryAttempts, uint32_t timeout = DEFAULT_QUERY_RESPONSE_TIMEOUT);
  ...
  protected:
    ...
    GfErrType getNoThrow_remote(const CacheableKeyPtr& keyPtr,
        CacheablePtr& valPtr, const UserDataPtr& aCallbackArgument, VersionTagPtr& versionTag);
    GfErrType putNoThrow_remote(const CacheableKeyPtr& keyPtr,
        const CacheablePtr& cvalue, const UserDataPtr& aCallbackArgument, VersionTagPtr& versionTag, bool checkDelta = true);
    GfErrType createNoThrow_remote(const CacheableKeyPtr& keyPtr,
        const CacheablePtr& cvalue, const UserDataPtr& aCallbackArgument, VersionTagPtr& versionTag);
    ...
    GfErrType removeNoThrow_remote(const CacheableKeyPtr& keyPtr,
        const CacheablePtr& cvalue, const UserDataPtr& aCallbackArgument, VersionTagPtr& versionTag);
    GfErrType removeNoThrowEX_remote(const CacheableKeyPtr& keyPtr, const UserDataPtr& aCallbackArgument, 
    ...
    GfErrType registerKeysNoThrow(const VectorOfCacheableKey& keys,
        bool attemptFailover = true, TcrEndpoint* endpoint = NULL,
        bool isDurable = false, InterestResultPolicy interestPolicy = InterestResultPolicy::NONE,
        bool receiveValues = true, TcrMessage* reply = NULL);
    ...
    GfErrType unregisterKeysNoThrowLocalDestroy(const VectorOfCacheableKey& keys,
            bool attemptFailover = true);
    GfErrType registerRegexNoThrow(const std::string& regex,
        bool attemptFailover = true, TcrEndpoint* endpoint = NULL,
        bool isDurable = false, VectorOfCacheableKeyPtr resultKeys = NULLPTR,
        InterestResultPolicy interestPolicy = InterestResultPolicy::NONE, bool receiveValues = true,
        TcrMessage* reply = NULL);
    ...
    InterestResultPolicy copyInterestList(VectorOfCacheableKey& keysVector, 
      std::unordered_map<CacheableKeyPtr, InterestResultPolicy>& interestList) const;
    ...
    std::unordered_map<CacheableKeyPtr, InterestResultPolicy> m_durableInterestList;
    std::unordered_map<std::string, InterestResultPolicy> m_durableInterestListRegex;
    std::unordered_map<CacheableKeyPtr, InterestResultPolicy> m_interestListForUpdatesAsInvalidates;
    std::unordered_map<std::string, InterestResultPolicy> m_interestListRegexForUpdatesAsInvalidates;
    std::unordered_map<CacheableKeyPtr, InterestResultPolicy> m_durableInterestListForUpdatesAsInvalidates;
    std::unordered_map<std::string, InterestResultPolicy> m_durableInterestListRegexForUpdatesAsInvalidates;
  ...
  private:
    ...
    bool isRegexRegistered( std::unordered_map<std::string, InterestResultPolicy> &interestListRegex, const std::string& regex, bool allKeys );
    GfErrType registerStoredRegex( TcrEndpoint*, std::unordered_map<std::string, InterestResultPolicy> &interestListRegex,
      bool isDurable = false, bool receiveValues = true );
    GfErrType unregisterStoredRegex( std::unordered_map<std::string, InterestResultPolicy>& interestListRegex );
    GfErrType unregisterStoredRegexLocalDestroy( std::unordered_map<std::string, InterestResultPolicy>& interestListRegex );
    void invalidateInterestList( std::unordered_map<CacheableKeyPtr, InterestResultPolicy>& interestList );
    GfErrType createOnServer(const CacheableKeyPtr& keyPtr, const CacheablePtr& cvalue, const UserDataPtr& aCallbackArgument);
    ...
    GfErrType getNoThrow_FullObject( EventIdPtr eventId, CacheablePtr& fullObject, VersionTagPtr& versionTag );

    // Disallow copy constructor and assignment operator.
    ThinClientRegion(const ThinClientRegion &);
    ThinClientRegion & operator = (const ThinClientRegion &);
    GfErrType singleHopPutAllNoThrow_remote(ThinClientPoolDM* tcrdm, const HashMapOfCacheable& map, VersionedCacheableObjectPartListPtr& versionedObjPartList, uint32_t timeout = DEFAULT_RESPONSE_TIMEOUT, const UserDataPtr& aCallbackArgument = NULLPTR);
    GfErrType multiHopPutAllNoThrow_remote(const HashMapOfCacheable& map, VersionedCacheableObjectPartListPtr& versionedObjPartList, uint32_t timeout = DEFAULT_RESPONSE_TIMEOUT, const UserDataPtr& aCallbackArgument = NULLPTR);

    GfErrType singleHopRemoveAllNoThrow_remote(ThinClientPoolDM* tcrdm, const VectorOfCacheableKey& keys, VersionedCacheableObjectPartListPtr& versionedObjPartList, const UserDataPtr& aCallbackArgument = NULLPTR);
    GfErrType multiHopRemoveAllNoThrow_remote(const VectorOfCacheableKey& keys, VersionedCacheableObjectPartListPtr& versionedObjPartList, const UserDataPtr& aCallbackArgument = NULLPTR);
  ...
  public:
    inline ChunkedInterestResponse(TcrMessage& msg,
        const VectorOfCacheableKeyPtr& resultKeys, TcrMessage& replyMsg) :
      TcrChunkedResult(), m_msg(msg), m_resultKeys(resultKeys), m_replyMsg(replyMsg)
    {
    }
    ...
    virtual void handleChunk(const uint8_t* chunk, int32_t chunkLen, uint8_t isLastChunkWithSecurity);
    virtual void reset();
  };
  ...
  public:
    inline ChunkedQueryResponse(TcrMessage& msg) :
      TcrChunkedResult(), m_msg(msg), m_queryResults(CacheableVector::create())
    {
    }
  ...
  private:
    ...
    ChunkedFunctionExecutionResponse& operator=(const ChunkedFunctionExecutionResponse&);

  public:
    inline ChunkedFunctionExecutionResponse(TcrMessage& msg, bool getResult, ResultCollectorPtr rc, ACE_Recursive_Thread_Mutex* resultCollectorLock = NULL) :
      TcrChunkedResult(), m_msg(msg),  m_getResult(getResult), m_rc(rc), m_resultCollectorLock(resultCollectorLock)
    {
    }
  ...
  typedef SharedPtr<ChunkedFunctionExecutionResponse> ChunkedFunctionExecutionResponsePtr;
  ...
  public:
    inline ChunkedGetAllResponse(TcrMessage& msg, ThinClientRegion* region,
        ...
        const HashMapOfExceptionPtr& exceptions,
        const VectorOfCacheableKeyPtr& resultKeys, MapOfUpdateCounters& trackerMap,
        int32_t destroyTracker, bool addToLocalCache, ACE_Recursive_Thread_Mutex& responseLock) :
          TcrChunkedResult(), m_msg(msg), m_region(region), m_keys(keys),
          m_values(values), m_exceptions(exceptions), m_resultKeys(resultKeys),
          m_trackerMap(trackerMap), m_destroyTracker(destroyTracker),
          m_addToLocalCache(addToLocalCache), m_keysOffset(0), m_responseLock(responseLock)
    {
    }
  ...
  */
  ...
  public:
    inline ChunkedPutAllResponse(const RegionPtr& region, TcrMessage& msg, ACE_Recursive_Thread_Mutex& responseLock, VersionedCacheableObjectPartListPtr& list) :
    TcrChunkedResult(), m_msg(msg), m_responseLock(responseLock), m_list(list), m_region(region)
    {
    }
  ...
  */
  ...
  public:
    inline ChunkedRemoveAllResponse(const RegionPtr& region, TcrMessage& msg, ACE_Recursive_Thread_Mutex& responseLock, VersionedCacheableObjectPartListPtr& list) :
    TcrChunkedResult(), m_msg(msg), m_responseLock(responseLock), m_list(list), m_region(region)
    {
    }
  ...
  public:
    inline ChunkedKeySetResponse(TcrMessage& msg,
        VectorOfCacheableKey& resultKeys, TcrMessage& replyMsg) :
      TcrChunkedResult(), m_msg(msg), m_resultKeys(resultKeys), m_replyMsg(replyMsg)
    {
    }
  ...
  public:
     ...
     TcrChunkedResult(), m_msg(msg), m_resultList(CacheableArrayList::create())
     {
     
     } 
     inline CacheableArrayListPtr getResults()
     {
       return m_resultList;
     }
