...
PreservedDataExpiryHandler::PreservedDataExpiryHandler(
                            PdxSerializablePtr pdxObjectPtr,
                            uint32_t duration):
...
int PreservedDataExpiryHandler::handle_timeout(const ACE_Time_Value& current_time,
    const void* arg)
{
  WriteGuard guard(PdxTypeRegistry::getPreservedDataLock());
  LOGDEBUG("Entered PreservedDataExpiryHandler PdxTypeRegistry::getPreserveDataMap().size() = %d", PdxTypeRegistry::getPreserveDataMap().size());
  ...
  try {
    // remove the entry from the map
	  if(PdxTypeRegistry::getPreserveDataMap().contains(m_pdxObjectPtr)){
		  ...
		  LOGDEBUG("PreservedDataExpiry:: preserveData erased entry from map updated size = %d", PdxTypeRegistry::getPreserveDataMap().size());
	  }else{
    ...
    LOGDEBUG("PreservedDataExpiry:: Error while Clearing PdxObject and its preserved data. Ignoring the error");
  }
  return 0;
}
