...
class PutAllWork: public PooledWork<GfErrType>, private NonCopyable, private NonAssignable {
...
public:
  PutAllWork(ThinClientPoolDM* poolDM, const BucketServerLocationPtr& serverLocation,
		  const RegionPtr& region, bool attemptFailover,
		  bool isBGThread,const HashMapOfCacheablePtr map,  const VectorOfCacheableKeyPtr keys,
          uint32_t timeout, const UserDataPtr& aCallbackArgument) :
    m_poolDM(poolDM), m_serverLocation(serverLocation),
    m_attemptFailover(attemptFailover), m_isBGThread(isBGThread),
    m_userAttribute(NULLPTR),
    m_region(region), m_map(map), m_keys(keys),
    m_timeout(timeout), m_papException(NULLPTR), m_isPapeReceived(false), m_aCallbackArgument(aCallbackArgument)
  {

    m_request = new TcrMessage(TcrMessage::PUTALL, m_region.ptr(), *m_map.ptr(), (int) (m_timeout* 1000), m_poolDM, aCallbackArgument);
    m_reply = new TcrMessage(true, m_poolDM);

    //create new instanceof VCOPL
    ACE_Recursive_Thread_Mutex responseLock;
    m_verObjPartListPtr = new VersionedCacheableObjectPartList(keys.ptr(), responseLock);
	...
	m_resultCollector = new ChunkedPutAllResponse(m_region, *m_reply, responseLock, m_verObjPartListPtr);
  ...
  HashMapOfCacheablePtr getPutAllMap() {
    return m_map;
  }
  ...
  ChunkedPutAllResponse* getResultCollector() {
    return m_resultCollector;
  }

  BucketServerLocationPtr getServerLocation(){
  ...
  GfErrType execute(void) {
    ...
    err = m_poolDM->sendSyncRequest(*m_request, *m_reply, m_attemptFailover, m_isBGThread, m_serverLocation);

    //Set Version Tags
	LOGDEBUG(" m_verObjPartListPtr size = %d err = %d ", m_resultCollector->getList()->size(), err);
	m_verObjPartListPtr->setVersionedTagptr(m_resultCollector->getList()->getVersionedTagptr());

    if (err != GF_NOERR)
      return err; /*This can be GF_NOTCON, counterpart to java ServerConnectivityException*/
    ...
    switch (m_reply->getMessageType()) {
      ...
      case TcrMessage::EXCEPTION:
        //TODO::Check for the PAPException and READ PutAllPartialResultServerException and set its member for later use.
        ...
        } else if (m_poolDM->isPutAllPartialResultException(m_reply->getException())) {
      ...
      case TcrMessage::PUT_DATA_ERROR:
        ...
        break;
      default:
        LOGERROR("Unknown message type %d during region put-all", m_reply->getMessageType());
        err = GF_NOTOBJ;
        break;
    }
    return err;
...
class RemoveAllWork: public PooledWork<GfErrType>, private NonCopyable, private NonAssignable {
...
public:
  RemoveAllWork(ThinClientPoolDM* poolDM, const BucketServerLocationPtr& serverLocation,
                  const RegionPtr& region, bool attemptFailover,
                  bool isBGThread, const VectorOfCacheableKeyPtr keys, const UserDataPtr& aCallbackArgument) :
    m_poolDM(poolDM), m_serverLocation(serverLocation),
    m_attemptFailover(attemptFailover), m_isBGThread(isBGThread),
    m_userAttribute(NULLPTR),
    m_region(region),  m_keys(keys),
    m_papException(NULLPTR), m_isPapeReceived(false), m_aCallbackArgument(aCallbackArgument)
  {
    m_request = new TcrMessage(TcrMessage::REMOVE_ALL, m_region.ptr(), *keys, m_aCallbackArgument, m_poolDM );
    m_reply = new TcrMessage(true, m_poolDM);
    //create new instanceof VCOPL
    ACE_Recursive_Thread_Mutex responseLock;
    m_verObjPartListPtr = new VersionedCacheableObjectPartList(keys.ptr(), responseLock);
    ...
    m_resultCollector = new ChunkedRemoveAllResponse(m_region, *m_reply, responseLock, m_verObjPartListPtr);
    m_reply->setChunkedResultHandler(m_resultCollector);
  }
  ...
  ChunkedRemoveAllResponse* getResultCollector() {
    return m_resultCollector;
  }
  ...
  GfErrType execute(void) {
    ...
    switch (m_reply->getMessageType()) {
      ...
      case TcrMessage::PUT_DATA_ERROR:
        ...
        break;
      default:
        LOGERROR("Unknown message type %d during region remove-all", m_reply->getMessageType());
        err = GF_NOTOBJ;
        break;
    }
    return err;
...
ThinClientRegion::ThinClientRegion(const std::string& name, CacheImpl* cache,
	...
	LocalRegion(name, cache, rPtr, attributes, stats, shared), m_notifyRelease(
			false), m_notificationSema(1), m_isMetaDataRefreshed(false),
      /* adongre
       * CID 28979: Uninitialized pointer field (UNINIT_CTOR)
       */
      m_tcrdm((ThinClientBaseDM*)0) {
	m_transactionEnabled = true;
	m_isDurableClnt = strlen(
			DistributedSystem::getSystemProperties()->durableClientId()) > 0;
}
...
void ThinClientRegion::initTCR() {
	...
	bool notificationEnabled = getAttributes()->getClientNotificationEnabled()
			|| subscription;
		...
		if (DistributedSystem::getSystemProperties()->isGridClient()) {
			LOGWARN(
						"Region %s: client subscription channel enabled for a grid "
							...
							"failover threads", m_fullPath.c_str());
			m_cacheImpl->tcrConnectionManager().startFailoverAndCleanupThreads();
		}
	}
	try {
		m_tcrdm = new TcrDistributionManager(this,
				m_cacheImpl->tcrConnectionManager());
		m_tcrdm->init();
...
void ThinClientRegion::registerKeys(const VectorOfCacheableKey& keys,
	...
	if (pool != NULLPTR) {
		...
		if (!getAttributes()->getClientNotificationEnabled()) {
			LOGERROR("Registering keys is supported "
  ...
  LOGDEBUG("ThinClientRegion::registerKeys : interestpolicy is %d", interestPolicy.ordinal);
...
void ThinClientRegion::unregisterKeys(const VectorOfCacheableKey& keys) {
		...
		if (!pool->getSubscriptionEnabled()) {
			LOGERROR(
						"Unregister keys is supported "
							"only if subscription-enabled attribute is true for pool %s",
						pool->getName());
			throw UnsupportedOperationException("Unregister keys is supported "
				"only if pool subscription-enabled attribute is true.");
		}
	} else {
		if (!getAttributes()->getClientNotificationEnabled()) {
			LOGERROR("Unregister keys is supported "
					"only if region client-notification attribute is true.");
			throw UnsupportedOperationException("Unregister keys is supported "
				"only if region client-notification attribute is true.");
		}
	...
	if (keys.empty()) {
		...
		throw IllegalArgumentException("Unregister keys "
			"keys vector is empty");
	}
...
void ThinClientRegion::registerAllKeys(bool isDurable,
		VectorOfCacheableKeyPtr resultKeys, bool getInitialValues,
		bool receiveValues) {
	PoolPtr pool = PoolManager::find(getAttributes()->getPoolName());
	if (pool != NULLPTR) {
		if (!pool->getSubscriptionEnabled()) {
			LOGERROR("Register all keys is supported only "
		...
		if (!getAttributes()->getClientNotificationEnabled()) {
			LOGERROR("Register all keys is supported only "
					"if region client-notification attribute is true.");
			throw UnsupportedOperationException(
					"Register all keys is supported only "
						"if region client-notification attribute is true.");
		}
	...
	if (isDurable && !isDurableClient()) {
		LOGERROR(
					"Register all keys durable flag is only applicable for durable clients");
  ...
  LOGDEBUG("ThinClientRegion::registerAllKeys : interestpolicy is %d", interestPolicy.ordinal);

	// [sumedh] if we need to fetch initial data, then we get the keys in
	// that call itself using the special GET_ALL message and do not need
	// to get the keys in the initial register interest call
	GfErrType err = registerRegexNoThrow(".*", true, NULL, isDurable,
      resultKeys, interestPolicy,
			receiveValues);
...
void ThinClientRegion::registerRegex(const char * regex, bool isDurable,
	...
	if (pool != NULLPTR) {
		if (!pool->getSubscriptionEnabled()) {
			LOGERROR("Register regex is supported only if "
		...
		if (!getAttributes()->getClientNotificationEnabled()) {
			LOGERROR("Register regex is supported only if "
					"region client-notification attribute is true.");
			throw UnsupportedOperationException(
					"Register regex is supported only if "
						"region client-notification attribute is true.");
		}
  ...
  LOGDEBUG("ThinClientRegion::registerRegex : interestpolicy is %d", interestPolicy.ordinal);
  
	// [sumedh] if we need to fetch initial data for "allKeys" case, then we
	...
	GfErrType err = registerRegexNoThrow(sregex, true, NULL, isDurable,
       resultKeys,
			interestPolicy, receiveValues);
...
void ThinClientRegion::unregisterRegex(const char * regex) {
	...
	if (pool != NULLPTR) {
		if (!pool->getSubscriptionEnabled()) {
			LOGERROR("Unregister regex is supported only if "
					"subscription-enabled attribute is true for pool ",
						pool->getName());
			throw UnsupportedOperationException(
...
void ThinClientRegion::unregisterAllKeys() {
	...
	if (pool != NULLPTR) {
		if (!pool->getSubscriptionEnabled()) {
			LOGERROR("Unregister all keys is supported only if "
					"subscription-enabled attribute is true for pool ",
						pool->getName());
			throw UnsupportedOperationException(
...
SelectResultsPtr ThinClientRegion::query(const char * predicate,
	...
	if (predlen > 6) // perhaps it has 'select' or 'import' if its > 6
	{
		...
		if (predlen - skipspace > 6) // check remaining length again to avoid reading past predicate char array
		{
			char firstWord[7] = { 0 };

			for (int charpos = 0; charpos < 6; charpos++) {
				firstWord[charpos] = ACE_OS::ace_tolower(predicate[charpos
						+ skipspace]);
			}

			if (!ACE_OS::strcmp(firstWord, "select") || !ACE_OS::strcmp(
					firstWord, "import")) {
				isFullQuery = true;
			}
		}
	...
	if (poolDM) {
		queryPtr
				= dynCast<RemoteQueryPtr> (
						poolDM-> getQueryServiceWithoutCheck()->newQuery(
								squery.c_str()));
	} else {
...
GfErrType ThinClientRegion::unregisterKeysBeforeDestroyRegion(){
  ...
  if (pool != NULLPTR) {
    if (!pool->getSubscriptionEnabled()) {
      LOGDEBUG("pool subscription-enabled attribute is false, No need to Unregister keys");
      return GF_NOERR;
    }
  } else {
    if (!getAttributes()->getClientNotificationEnabled()) {
      LOGDEBUG("client-notification attribute is false, no need to Unregister keys");
      return GF_NOERR;
    }
  }
  GfErrType err = GF_NOERR;
  ...
  err = opErr != GF_NOERR ? opErr : err;
  opErr = unregisterStoredRegexLocalDestroy(m_interestListRegexForUpdatesAsInvalidates);
  err = opErr != GF_NOERR ? opErr : err;
  opErr = unregisterStoredRegexLocalDestroy(m_durableInterestListRegexForUpdatesAsInvalidates);
  err = opErr != GF_NOERR ? opErr : err;
  ...
  copyInterestList(keysVecForUpdatesAsInvalidates, m_interestListForUpdatesAsInvalidates);
  opErr = unregisterKeysNoThrowLocalDestroy(keysVecForUpdatesAsInvalidates, false);
  err = opErr != GF_NOERR ? opErr : err;

  VectorOfCacheableKey keysVecDurableForUpdatesAsInvalidates;
  copyInterestList(keysVecDurableForUpdatesAsInvalidates, m_durableInterestListForUpdatesAsInvalidates);
  opErr = unregisterKeysNoThrowLocalDestroy(keysVecDurableForUpdatesAsInvalidates, false);
  err = opErr != GF_NOERR ? opErr : err;
  return err;
}
...
void ThinClientRegion::serverKeys(VectorOfCacheableKey& v) {
  ...
  TcrMessage request(TcrMessage::KEY_SET, m_fullPath, m_tcrdm);
  reply.setMessageTypeRequest(TcrMessage::KEY_SET);
  //hitesh need to check
  ChunkedKeySetResponse* resultCollector(new ChunkedKeySetResponse(request, v, reply));
...
bool ThinClientRegion::containsKeyOnServer(const CacheableKeyPtr& keyPtr) const {
	GfErrType err = GF_NOERR;
	bool ret = false;
...
//			txState->recordTXOperation(GF_CONTAINS_KEY, getFullPath(), keyPtr,
//					args);
//		}
	}
	...
	TcrMessage request(TcrMessage::CONTAINS_KEY, this, keyPtr,
			static_cast<UserDataPtr> (NULLPTR), true, m_tcrdm);
	TcrMessage reply(true, m_tcrdm);
...
bool ThinClientRegion::containsValueForKey_remote(const CacheableKeyPtr& keyPtr) const {
...
//		}
	...
	TcrMessage request(TcrMessage::CONTAINS_KEY, this, keyPtr,
			static_cast<UserDataPtr> (NULLPTR), false, m_tcrdm);
	TcrMessage reply(true, m_tcrdm);
	reply.setMessageTypeRequest(TcrMessage::CONTAINS_KEY);
	err = m_tcrdm->sendSyncRequest(request, reply);
	//if ( err != GF_NOERR ) return ret;
...
void ThinClientRegion::clear(const UserDataPtr& aCallbackArgument) {
	...
	TcrMessage request(TcrMessage::CLEAR_REGION, this, aCallbackArgument,
			-1, m_tcrdm);
	TcrMessage reply(true, m_tcrdm);
	err = m_tcrdm->sendSyncRequest(request, reply);
	if (err != GF_NOERR)
	...
	if (err == GF_NOERR)
		err = invokeCacheListenerForRegionEvent(aCallbackArgument,
				CacheEventFlags::NORMAL, AFTER_REGION_CLEAR);
	GfErrTypeToException("Region::clear", err);
}

GfErrType ThinClientRegion::getNoThrow_remote(const CacheableKeyPtr& keyPtr,
		CacheablePtr& valPtr, const UserDataPtr& aCallbackArgument, VersionTagPtr& versionTag) {
	GfErrType err = GF_NOERR;

	/** @brief Create message and send to bridge server */

	TcrMessage request(TcrMessage::REQUEST, this, keyPtr,
...
GfErrType ThinClientRegion::invalidateNoThrow_remote(
		const CacheableKeyPtr& keyPtr, const UserDataPtr& aCallbackArgument, VersionTagPtr& versionTag) {
	GfErrType err = GF_NOERR;

	/** @brief Create message and send to bridge server */

	TcrMessage request(TcrMessage::INVALIDATE, this, keyPtr,
			aCallbackArgument, m_tcrdm);
	TcrMessage reply(true, m_tcrdm);
	err = m_tcrdm->sendSyncRequest(request, reply);
	if (err != GF_NOERR)
...
GfErrType ThinClientRegion::putNoThrow_remote(const CacheableKeyPtr& keyPtr,
		const CacheablePtr& valuePtr, const UserDataPtr& aCallbackArgument, 
    VersionTagPtr& versionTag, bool checkDelta) {
	GfErrType err = GF_NOERR;
	// do TCR put
	...
	if (checkDelta && valuePtr != NULLPTR && conFlationValue != NULL && strcmp(
			conFlationValue, "true") != 0
			&& ThinClientBaseDM::isDeltaEnabledOnServer()) {
		Delta* temp = dynamic_cast<Delta*> (valuePtr.ptr());
		delta = (temp && temp->hasDelta());
	}
	TcrMessage request(TcrMessage::PUT, this, keyPtr, valuePtr,
			aCallbackArgument, delta, m_tcrdm);
	TcrMessage* reply = new TcrMessage(true, m_tcrdm);
	err = m_tcrdm->sendSyncRequest(request, *reply);
	if (delta) {
		m_cacheImpl->m_cacheStats->incDeltaPut(); // Does not chcek whether success of failure..
		if (reply->getMessageType() == TcrMessage::PUT_DELTA_ERROR) { // Try without delta
			TcrMessage request(TcrMessage::PUT, this, keyPtr, valuePtr,
					aCallbackArgument, false, m_tcrdm, false, true);
			delete reply;
			reply = new TcrMessage(true, m_tcrdm);
			err = m_tcrdm->sendSyncRequest(request, *reply);
		}
	}
	if (err != GF_NOERR)
...
GfErrType ThinClientRegion::createNoThrow_remote(const CacheableKeyPtr& keyPtr,
		const CacheablePtr& valuePtr, const UserDataPtr& aCallbackArgument, VersionTagPtr& versionTag) {
	return putNoThrow_remote(keyPtr, valuePtr, aCallbackArgument, versionTag, false);
}
...
GfErrType ThinClientRegion::destroyNoThrow_remote(
	...
	// do TCR destroy
	TcrMessage request(TcrMessage::DESTROY, this, keyPtr,
			aCallbackArgument, m_tcrdm);
	TcrMessage reply(true, m_tcrdm);
	err = m_tcrdm->sendSyncRequest(request, reply);
	if (err != GF_NOERR)
...
GfErrType ThinClientRegion::removeNoThrow_remote(const CacheableKeyPtr& keyPtr,
		const CacheablePtr& cvalue, const UserDataPtr& aCallbackArgument, VersionTagPtr& versionTag) {
	GfErrType err = GF_NOERR;

	// do TCR remove
	TcrMessage request(TcrMessage::DESTROY, this, keyPtr, cvalue,
			aCallbackArgument, m_tcrdm);
	TcrMessage reply(true, m_tcrdm);
	err = m_tcrdm->sendSyncRequest(request, reply);
	if (err != GF_NOERR) {
	...
	case TcrMessage::EXCEPTION: {
		err = handleServerException("Region::remove",
				reply.getException());
		break;
	}
	case TcrMessage::DESTROY_DATA_ERROR: {
...
GfErrType ThinClientRegion::removeNoThrowEX_remote(const CacheableKeyPtr& keyPtr, const UserDataPtr& aCallbackArgument, 
  VersionTagPtr& versionTag)
{
	...
	case TcrMessage::EXCEPTION: {
		err = handleServerException("Region::removeEx",
				reply.getException());
		break;
	}
	case TcrMessage::DESTROY_DATA_ERROR: {
...
GfErrType ThinClientRegion::getAllNoThrow_remote(
	...
	addToLocalCache = addToLocalCache
			&& m_regionAttributes->getCachingEnabled();
	if (addToLocalCache && !m_regionAttributes->getConcurrencyChecksEnabled()) {
		...
		if (keys == NULL) {
			...
			destroyTracker = m_entries->addTrackerForAllEntries(updateCountMap,
					true);
		} else {
			for (int32_t index = 0; index < keys->size(); ++index) {
				CacheablePtr oldValue;
				const CacheableKeyPtr& key = keys->operator[](index);
				int updateCount = m_entries->addTrackerForEntry(key, oldValue,
						true, false, false);
	...
	TcrMessage request(this, keys, m_tcrdm, aCallbackArgument);//now we need to initialize later
	
	TcrMessage reply(true, m_tcrdm);
    ACE_Recursive_Thread_Mutex responseLock;
    //hitesh need to check
	TcrChunkedResult* resultCollector(new ChunkedGetAllResponse(reply, this,
			keys, values, exceptions, resultKeys, updateCountMap,
	...
	if (addToLocalCache && !m_regionAttributes->getConcurrencyChecksEnabled()) {
		...
		for (MapOfUpdateCounters::const_iterator iter = updateCountMap.begin(); iter
				!= updateCountMap.end(); ++iter) {
			if (iter->second >= 0) {
...
GfErrType ThinClientRegion::singleHopPutAllNoThrow_remote(ThinClientPoolDM* tcrdm, const HashMapOfCacheable& map, VersionedCacheableObjectPartListPtr& versionedObjPartList,
		uint32_t timeout, const UserDataPtr& aCallbackArgument) {
  LOGDEBUG(" ThinClientRegion::singleHopPutAllNoThrow_remote map size = %d", map.size());
  RegionPtr region(this);

  GfErrType error = GF_NOERR;
  /*Step-1::
   * populate the keys vector from the user Map and pass it to the  getServerToFilterMap to generate locationMap
   * If locationMap is NULL try the old, existing putAll impl that may take multiple n/w hops
   */
  ...
  //LOGDEBUG("ThinClientRegion::singleHopPutAllNoThrow_remote keys.size() = %d ", userKeys->size());
  HashMapT<BucketServerLocationPtr, VectorOfCacheableKeyPtr>* locationMap = tcrdm->getClientMetaDataService()->getServerToFilterMap(userKeys, region, true);
  if(locationMap == NULL)
  {
    //putAll with multiple hop implementation
    LOGDEBUG("locationMap is Null or Empty");
    /*
    ==24194== 7,825,919 (24 direct, 7,825,895 indirect) bytes in 2 blocks are definitely lost in loss record 598 of 598
    ==24194==    at 0x4007D75: operator new(unsigned int) (vg_replace_malloc.c:313)
    ==24194==    by 0x43B5C5F: gemfire::ThinClientRegion::singleHopPutAllNoThrow_remote(gemfire::ThinClientPoolDM*, gemfire::HashMapOfCacheable const&, gemfire::SharedPtr<gemfire::VersionedCacheableObjectPartList>&, unsigned int) (ThinClientRegion.cpp:1180)
    ==24194==    by 0x43B8B65: gemfire::ThinClientRegion::putAllNoThrow_remote(gemfire::HashMapOfCacheable const&, gemfire::SharedPtr<gemfire::VersionedCacheableObjectPartList>&, unsigned int) (ThinClientRegion.cpp:1500)
    ==24194==    by 0x42E55F5: gemfire::LocalRegion::putAllNoThrow(gemfire::HashMapOfCacheable const&, unsigned int) (LocalRegion.cpp:1956)
    ==24194==    by 0x42DC797: gemfire::LocalRegion::putAll(gemfire::HashMapOfCacheable const&, unsigned int) (LocalRegion.cpp:366)
    ==24194==    by 0x806FF8D: Task_StepEight::doTask() (in /export/pnq-gst-dev01a/users/adongre/cedar_dev_Nov12/build-artifacts/linux/tests/cppcache/testThinClientPutAll)
    ==24194==    by 0x807CE2A: dunit::TestSlave::begin() (in /export/pnq-gst-dev01a/users/adongre/cedar_dev_Nov12/build-artifacts/linux/tests/cppcache/testThinClientPutAll)
    ==24194==    by 0x8078F57: dunit::dmain(int, char**) (in /export/pnq-gst-dev01a/users/adongre/cedar_dev_Nov12/build-artifacts/linux/tests/cppcache/testThinClientPutAll)
    ==24194==    by 0x805D7EA: main (in /export/pnq-gst-dev01a/users/adongre/cedar_dev_Nov12/build-artifacts/linux/tests/cppcache/testThinClientPutAll)
    ==24194==
    */
    delete userKeys;
    userKeys = NULL;

    return multiHopPutAllNoThrow_remote(map, versionedObjPartList, timeout, aCallbackArgument);
  }
  /*
  ==24194== 7,825,919 (24 direct, 7,825,895 indirect) bytes in 2 blocks are definitely lost in loss record 598 of 598
  ==24194==    at 0x4007D75: operator new(unsigned int) (vg_replace_malloc.c:313)
  ...
  */
  ...
  //set this flag that indicates putAll on PR is invoked with singlehop enabled.
   ...
   *     Create server specific filteredMap/subMap by populating all keys (locationIter.second()) and its corr. values from the user Map.
   *  c. create new instance of PutAllWork, i.e worker with required params.
   *     //TODO:: Add details of each parameter later
   *  d. enqueue the worker for thread from threadPool to perform/run execute method.
      ...
      locationIter != locationMap->end(); locationIter++)
  {
    ...
    for (HashMapOfCacheable::Iterator filteredMapIter = filteredMap->begin(); filteredMapIter != filteredMap->end(); ++filteredMapIter) {
      CacheableInt32Ptr kPtr = dynCast<CacheableInt32Ptr>(filteredMapIter.first()) ;
      CacheableInt32Ptr vPtr = dynCast<CacheableInt32Ptr>(filteredMapIter.second());
      LOGDEBUG("Key = %d  Value = %d ", kPtr->value(), vPtr->value() );
    }
    */

    PutAllWork* worker = new PutAllWork(tcrdm, serverLocation,region, true /*attemptFailover*/, false /*isBGThread*/, filteredMap, keys, timeout, aCallbackArgument);
  ...
  /**
   * Step::3
   * a. Iterate over all vector of putAllWorkers and populate worker specific information into the HashMap
   *    resultMap<BucketServerLocationPtr, SerializablePtr>, 2nd part, Value can be a VersionedCacheableObjectPartListPtr or PutAllPartialResultServerExceptionPtr.
   *    failedServers<BucketServerLocationPtr, CacheableInt32Ptr>, 2nd part, Value is a ErrorCode.
   * b. delete the worker
   */
  HashMapT<BucketServerLocationPtr, SerializablePtr>* resultMap = new HashMapT<BucketServerLocationPtr, SerializablePtr>();
  HashMapT<BucketServerLocationPtr, CacheableInt32Ptr>* failedServers = new HashMapT<BucketServerLocationPtr, CacheableInt32Ptr>();

  for(std::vector<PutAllWork*>::iterator iter = putAllWorkers.begin(); iter != putAllWorkers.end(); iter++)
  {
    PutAllWork* worker = *iter;

    GfErrType err = worker->getResult(); //wait() or blocking call for worker thread.
    ...
    if(err != GF_NOERR)
    {
      ...
      if(error == GF_PUTALL_PARTIAL_RESULT_EXCEPTION ){
        resultMap->insert(worker->getServerLocation(), worker->getPaPResultException());
      } else if (error == GF_NOTCON){
    	//Refresh the metadata in case of GF_NOTCON.
        tcrdm->getClientMetaDataService()->enqueueForMetadataRefresh(region->getFullPath(), 0);
      }
      failedServers->insert(worker->getServerLocation(), CacheableInt32::create(error));
    } else {
      //No Exception from server
	  resultMap->insert(worker->getServerLocation(), worker->getResultCollector()->getList());
    }

    LOGDEBUG("worker->getPutAllMap()->size() = %d ",  worker->getPutAllMap()->size());
	LOGDEBUG("worker->getResultCollector()->getList()->getVersionedTagsize() = %d ", worker->getResultCollector()->getList()->getVersionedTagsize());
   ...
   * b. Iterate over the resultMap and value for the particular serverlocation is of type VersionedCacheableObjectPartList add keys and versions.
   * C. ToDO:: what if the value in the resultMap is of type PutAllPartialResultServerException
   */
  ACE_Recursive_Thread_Mutex responseLock;
  PutAllPartialResultPtr result (new PutAllPartialResult(map.size(), responseLock));
  LOGDEBUG(" TCRegion:: %s:%d  result->getSucceededKeysAndVersions()->getVersionedTagsize() = %d ", __FILE__, __LINE__, result->getSucceededKeysAndVersions()->getVersionedTagsize());
  LOGDEBUG(" TCRegion:: %s:%d resultMap->size() ", __FILE__, __LINE__, resultMap->size());
  for(HashMapT<BucketServerLocationPtr, SerializablePtr>::Iterator resultMapIter = resultMap->begin();
		  resultMapIter != resultMap->end(); resultMapIter++)
  {
    ...
    papException = dynamic_cast<PutAllPartialResultServerException*>(value.ptr());
    //LOGDEBUG(" TCRegion:: %s:%d  result->getSucceededKeysAndVersions()->getVersionedTagsize() = %d ", __FILE__, __LINE__, result->getSucceededKeysAndVersions()->getVersionedTagsize());
    if(papException != NULL){
      //PutAllPartialResultServerException CASE:: value in resultMap is of type PutAllPartialResultServerException.
      //TODO:: Add failedservers.keySet= all fialed servers, i.e list out all keys in map failedServers,
      //       that is set view of the keys contained in failedservers map.
      //LOGDEBUG("TCRegion:: PutAll SingleHop encountered PutAllPartialResultServerException exception: %s , failedServers are: ", papException->getMessage()->asChar());
      //TODO:: need to read  papException and populate PutAllPartialResult.
      result->consolidate(papException->getResult());
    }else if (value != NULLPTR && (list = dynCast<VersionedCacheableObjectPartListPtr>(value)) != NULLPTR) {
      //value in resultMap is of type VCOPL.
      //LOGDEBUG("TCRegion::  %s:%d :: list->getSucceededKeys()->size()=%d  list->getVersionedTagsize() = %d", __FILE__, __LINE__, list->getSucceededKeys()->size(), list->getVersionedTagsize());
      result->addKeysAndVersions(list);
    }else{
      //ERROR CASE
      if (value != NULLPTR) {
        LOGERROR("ERROR:: ThinClientRegion::singleHopPutAllNoThrow_remote value could not Cast to either VCOPL or PutAllPartialResultServerException:%s",value->toString()->asChar());
      } else {
    	LOGERROR("ERROR:: ThinClientRegion::singleHopPutAllNoThrow_remote value is NULL");
      }
    }
  }

  /**
   * a. if PutAllPartialResult result does not contains any entry,  Iterate over locationMap.
   * b. Create VectorOfCacheableKey succeedKeySet, and keep adding set of keys (locationIter.second()) in locationMap for which failedServers->contains(locationIter.first()is false.
   */

  LOGDEBUG("ThinClientRegion:: %s:%d failedServers->size() = %d", __FILE__, __LINE__, failedServers->size());
    ...
    if(result->getSucceededKeysAndVersions()->size() == 0){
        ...
        locationIter != locationMap->end(); locationIter++)
      {
        if(!failedServers->contains(locationIter.first())) {
          for(int32_t i=0; (int32_t)i < locationIter.second()->size(); i++ ){
   ...
   * c. if failedServer map contains "GF_PUTALL_PARTIAL_RESULT_EXCEPTION" then continue, Do not retry putAll for corr. keys.
   * b. Retry for all the failed server.
   *    Generate a newSubMap by finding Keys specific to failedServers from locationMap and finding their respective values from the usermap.
   */
  error = GF_NOERR;
  bool oneSubMapRetryFailed = false;
  for(HashMapT<BucketServerLocationPtr, CacheableInt32Ptr>::Iterator failedServerIter = failedServers->begin();
      failedServerIter != failedServers->end(); failedServerIter++)
  {
    if(failedServerIter.second()->value() == GF_PUTALL_PARTIAL_RESULT_EXCEPTION) {//serverLocation
    ...
    HashMapT<BucketServerLocationPtr, VectorOfCacheableKeyPtr>::Iterator failedSerInLocMapIter = locationMap->find(failedServerIter.first());
    ...
    if(failedKeys == NULLPTR){
      LOGERROR("TCRegion::singleHopPutAllNoThrow_remote :: failedKeys are NULL that is not valid");
    }
    ...
    if(failedKeys != NULLPTR && failedKeys->size() > 0){
      for (int32_t index = 0; index < failedKeys->size(); index++){
        ...
        if(iter != map.end()){
          ...
          LOGERROR("DEBUG:: TCRegion.cpp singleHopPutAllNoThrow_remote KEY not found in user failedSubMap");
        }
      }
    }
    ...
    GfErrType errCode =  multiHopPutAllNoThrow_remote(*newSubMap.ptr(), vcopListPtr, timeout, aCallbackArgument);
    if(errCode == GF_NOERR){
  ...
  LOGDEBUG("singlehop versionedObjPartList = %d error=%d", versionedObjPartList->size(), error);
  delete locationMap;
...
GfErrType ThinClientRegion::multiHopPutAllNoThrow_remote(const HashMapOfCacheable& map, VersionedCacheableObjectPartListPtr& versionedObjPartList,
		uint32_t timeout, const UserDataPtr& aCallbackArgument) {
  ...
  TcrMessage request(TcrMessage::PUTALL, this, map, (int) (timeout
      * 1000), m_tcrdm, aCallbackArgument);
  TcrMessage reply(true, m_tcrdm);
  ...
  //HItesh need to check
  ChunkedPutAllResponse* resultCollector(new ChunkedPutAllResponse(RegionPtr(this), reply, responseLock, versionedObjPartList));
  reply.setChunkedResultHandler(resultCollector);

  err = m_tcrdm->sendSyncRequest(request, reply);

  versionedObjPartList = resultCollector->getList();
  LOGDEBUG("multiple hop versionedObjPartList size = %d , err = %d  ", versionedObjPartList->size(), err);
  delete resultCollector;
  if (err != GF_NOERR)
    return err;
    LOGDEBUG("ThinClientRegion::multiHopPutAllNoThrow_remote reply.getMessageType() = %d ", reply.getMessageType());
  switch (reply.getMessageType()) {
    case TcrMessage::REPLY:
      //LOGDEBUG("Map is written into remote server at region %s", m_fullPath.c_str());
      break;
	case TcrMessage::RESPONSE:
      LOGDEBUG("multiHopPutAllNoThrow_remote TcrMessage::RESPONSE %s, err = %d ", m_fullPath.c_str(), err);
      break;
    case TcrMessage::EXCEPTION:
      err = handleServerException("ThinClientRegion::putAllNoThrow",
        reply.getException());
      //TODO:: Do we need to read PutAllPartialServerException for multiple hop.
      break;
    case TcrMessage::PUT_DATA_ERROR:
      //LOGERROR( "A write error occurred on the endpoint %s",
      //m_tcrdm->getActiveEndpoint( )->name( ).c_str( ) );
      ...
      break;
    default:
      LOGERROR("Unknown message type %d during region put-all", reply.getMessageType());
      err = GF_NOTOBJ;
      break;
  }
  return err;
...
GfErrType ThinClientRegion::putAllNoThrow_remote(const HashMapOfCacheable& map, VersionedCacheableObjectPartListPtr& versionedObjPartList,
		uint32_t timeout, const UserDataPtr& aCallbackArgument) {
  ...
  if(poolDM != NULL){
    if(poolDM->getPRSingleHopEnabled() && poolDM->getClientMetaDataService() != NULL && txState == NULL /*For Tx use multi-hop*/ ){
      return singleHopPutAllNoThrow_remote(poolDM, map, versionedObjPartList, timeout, aCallbackArgument);
    }else{
      return multiHopPutAllNoThrow_remote(map, versionedObjPartList, timeout, aCallbackArgument);
    }
  }else{
...
GfErrType ThinClientRegion::singleHopRemoveAllNoThrow_remote(ThinClientPoolDM* tcrdm, const VectorOfCacheableKey& keys,
    VersionedCacheableObjectPartListPtr& versionedObjPartList, const UserDataPtr& aCallbackArgument)
{
  LOGDEBUG(" ThinClientRegion::singleHopRemoveAllNoThrow_remote keys size = %d", keys.size());
  RegionPtr region(this);
  GfErrType error = GF_NOERR;

  HashMapT<BucketServerLocationPtr, VectorOfCacheableKeyPtr>* locationMap = tcrdm->getClientMetaDataService()->getServerToFilterMap(&keys, region, true);
  if(locationMap == NULL)
  {
    //removeAll with multiple hop implementation
    LOGDEBUG("locationMap is Null or Empty");
    return multiHopRemoveAllNoThrow_remote(keys, versionedObjPartList, aCallbackArgument);
  }
      ...
      locationIter != locationMap->end(); locationIter++)
  {
    ...
    RemoveAllWork* worker = new RemoveAllWork(tcrdm, serverLocation,region, true /*attemptFailover*/, false /*isBGThread*/, mappedkeys, aCallbackArgument);
    threadPool->perform(worker);
  ...
  HashMapT<BucketServerLocationPtr, SerializablePtr>* resultMap = new HashMapT<BucketServerLocationPtr, SerializablePtr>();
  HashMapT<BucketServerLocationPtr, CacheableInt32Ptr>* failedServers = new HashMapT<BucketServerLocationPtr, CacheableInt32Ptr>();
  for(std::vector<RemoveAllWork*>::iterator iter = removeAllWorkers.begin(); iter != removeAllWorkers.end(); iter++)
  {
   ...
   */
  ACE_Recursive_Thread_Mutex responseLock;
  PutAllPartialResultPtr result (new PutAllPartialResult(keys.size(), responseLock));
  LOGDEBUG(" TCRegion:: %s:%d  result->getSucceededKeysAndVersions()->getVersionedTagsize() = %d ", __FILE__, __LINE__, result->getSucceededKeysAndVersions()->getVersionedTagsize());
  LOGDEBUG(" TCRegion:: %s:%d resultMap->size() ", __FILE__, __LINE__, resultMap->size());
  for(HashMapT<BucketServerLocationPtr, SerializablePtr>::Iterator resultMapIter = resultMap->begin();
                  resultMapIter != resultMap->end(); resultMapIter++)
  {
    ...
    if(papException != NULL){
      ...
      //ERROR CASE
      if (value != NULLPTR) {
        LOGERROR("ERROR:: ThinClientRegion::singleHopRemoveAllNoThrow_remote value could not Cast to either VCOPL or PutAllPartialResultServerException:%s",value->toString()->asChar());
      } else {
        LOGERROR("ERROR:: ThinClientRegion::singleHopRemoveAllNoThrow_remote value is NULL");
      }
    }
  }
      ...
      failedServerIter != failedServers->end(); failedServerIter++)
  {
    ...
    if(failedKeys == NULLPTR){
      LOGERROR("TCRegion::singleHopRemoveAllNoThrow_remote :: failedKeys are NULL that is not valid");
    }

    VersionedCacheableObjectPartListPtr vcopListPtr;
    PutAllPartialResultServerExceptionPtr papResultServerExc= NULLPTR;
    GfErrType errCode =  multiHopRemoveAllNoThrow_remote(*failedKeys, vcopListPtr, aCallbackArgument);
    if(errCode == GF_NOERR){
      result->addKeysAndVersions(vcopListPtr);
    }else if(errCode == GF_PUTALL_PARTIAL_RESULT_EXCEPTION){
      oneSubMapRetryFailed = true;
...
GfErrType ThinClientRegion::multiHopRemoveAllNoThrow_remote(const VectorOfCacheableKey& keys,
    VersionedCacheableObjectPartListPtr& versionedObjPartList, const UserDataPtr& aCallbackArgument)
{
  //Multiple hop implementation
   ...
   // Construct request/reply for putAll
   TcrMessage request(TcrMessage::REMOVE_ALL, this, keys, aCallbackArgument, m_tcrdm);
   TcrMessage reply(true, m_tcrdm);

   ACE_Recursive_Thread_Mutex responseLock;
   versionedObjPartList = new VersionedCacheableObjectPartList(responseLock);
   //HItesh need to check
   ChunkedRemoveAllResponse* resultCollector(new ChunkedRemoveAllResponse(RegionPtr(this), reply, responseLock, versionedObjPartList));
   reply.setChunkedResultHandler(resultCollector);
   ...
   if (err != GF_NOERR)
     return err;
     LOGDEBUG("ThinClientRegion::multiHopRemoveAllNoThrow_remote reply.getMessageType() = %d ", reply.getMessageType());
   switch (reply.getMessageType()) {
     case TcrMessage::REPLY:
       //LOGDEBUG("Map is written into remote server at region %s", m_fullPath.c_str());
       break;
         case TcrMessage::RESPONSE:
       LOGDEBUG("multiHopRemoveAllNoThrow_remote TcrMessage::RESPONSE %s, err = %d ", m_fullPath.c_str(), err);
       break;
     case TcrMessage::EXCEPTION:
       err = handleServerException("ThinClientRegion::putAllNoThrow",
         reply.getException());
       break;
     default:
       LOGERROR("Unknown message type %d during region remove-all", reply.getMessageType());
       err = GF_NOTOBJ;
       break;
   }
   return err;
...
GfErrType ThinClientRegion::removeAllNoThrow_remote(const VectorOfCacheableKey& keys,
    VersionedCacheableObjectPartListPtr& versionedObjPartList, const UserDataPtr& aCallbackArgument)
{
  ...
  if(poolDM != NULL){
    if(poolDM->getPRSingleHopEnabled() && poolDM->getClientMetaDataService() != NULL && txState == NULL /*For Tx use multi-hop*/ ){
      return singleHopRemoveAllNoThrow_remote(poolDM, keys, versionedObjPartList, aCallbackArgument);
    }else{
      return multiHopRemoveAllNoThrow_remote(keys, versionedObjPartList, aCallbackArgument);
    }
  }else{
    LOGERROR("ThinClientRegion::removeAllNoThrow_remote :: Pool Not Specified ");
    return GF_NOTSUP;
  }
}
...
uint32_t ThinClientRegion::size_remote() {
	...
	// do TCR size
	TcrMessage request(TcrMessage::SIZE, m_fullPath.c_str());
	TcrMessage reply(true, m_tcrdm);
	err = m_tcrdm->sendSyncRequest(request, reply);
	...
	switch (reply.getMessageType()) {
	case TcrMessage::RESPONSE: {
		CacheableInt32Ptr size = staticCast<CacheableInt32Ptr> (
				reply.getValue());
		return size->value();
		//LOGINFO("Map is written into remote server at region %s", m_fullPath.c_str());
	}
		break;
	case TcrMessage::EXCEPTION:
		err = handleServerException("ThinClientRegion::size",
				reply.getException());
		break;
...
GfErrType ThinClientRegion::registerStoredRegex(TcrEndpoint* endpoint,
		std::unordered_map<std::string, InterestResultPolicy> &interestListRegex, bool isDurable,
		bool receiveValues) {
	GfErrType opErr = GF_NOERR;
	GfErrType retVal = GF_NOERR;

	for (std::unordered_map<std::string, InterestResultPolicy>::iterator it = interestListRegex.begin(); it
			!= interestListRegex.end(); ++it) {
...
GfErrType ThinClientRegion::registerKeys(TcrEndpoint* endpoint,
	...
	err = opErr != GF_NOERR ? opErr : err;
	opErr = registerStoredRegex(endpoint,
			m_interestListRegexForUpdatesAsInvalidates, false, false);
	err = opErr != GF_NOERR ? opErr : err;
	opErr = registerStoredRegex(endpoint, m_durableInterestListRegex, true);
	err = opErr != GF_NOERR ? opErr : err;
	opErr = registerStoredRegex(endpoint,
			m_durableInterestListRegexForUpdatesAsInvalidates, true, false);
	err = opErr != GF_NOERR ? opErr : err;

	VectorOfCacheableKey keysVec;
	InterestResultPolicy interestPolicy = copyInterestList(keysVec, m_interestList);
  ...
  opErr = registerKeysNoThrow(keysVecForUpdatesAsInvalidates, false,
    endpoint, false, interestPolicy, false);
	err = opErr != GF_NOERR ? opErr : err;

	VectorOfCacheableKey keysVecDurable;
	interestPolicy = copyInterestList(keysVecDurable, m_durableInterestList);
  opErr = registerKeysNoThrow(keysVecDurable, false, endpoint, true, interestPolicy);
	err = opErr != GF_NOERR ? opErr : err;
	...
	if (request != NULL && request->getRegionName() == m_fullPath
			&& (request->getMessageType() == TcrMessage::REGISTER_INTEREST
					|| request->getMessageType()
							== TcrMessage::REGISTER_INTEREST_LIST)) {
		...
		if (newKeysVec == NULL || newKeysVec->empty()) {
			...
			if (!newRegex.empty()) {
				...
				opErr = registerRegexNoThrow(newRegex, false, endpoint,
          isDurable, NULLPTR, request->getInterestResultPolicy(), receiveValues, reply);
				err = opErr != GF_NOERR ? opErr : err;
			}
		} else {
			opErr = registerKeysNoThrow(*newKeysVec, false, endpoint,
					isDurable, request->getInterestResultPolicy(), receiveValues, reply);
			err = opErr != GF_NOERR ? opErr : err;
		}
	}
 return err;
...
GfErrType ThinClientRegion::unregisterStoredRegexLocalDestroy(
  ...
  for (std::unordered_map<std::string,InterestResultPolicy>::iterator it = interestListRegex.begin(); it != interestListRegex.end(); ++it) {
...
GfErrType ThinClientRegion::destroyRegionNoThrow_remote(
	...
	TcrMessage request(TcrMessage::DESTROY_REGION, this,
	...
	switch (reply.getMessageType()) {
	case TcrMessage::REPLY: {
		//LOGINFO("Region %s at remote is destroyed successfully", m_fullPath.c_str());
		break;
	}
	case TcrMessage::EXCEPTION: {
		err = handleServerException("Region::destroyRegion",
				reply.getException());
		break;
	}
...
GfErrType ThinClientRegion::registerKeysNoThrow(
		...
		bool receiveValues, TcrMessage* reply) {
	...
	TcrMessage replyLocal(true, m_tcrdm);
  ...
  LOGDEBUG("ThinClientRegion::registerKeysNoThrow : interestpolicy is %d", interestPolicy.ordinal);

  TcrMessage request(TcrMessage::REGISTER_INTEREST_LIST, this, keys,
			isDurable, getAttributes()->getCachingEnabled(), receiveValues, interestPolicy, 
			m_tcrdm);
  ACE_Recursive_Thread_Mutex responseLock;
  ...
  if(interestPolicy.ordinal == InterestResultPolicy::KEYS_VALUES.ordinal)
  {
    ...
    if(needToCreateRC) {
      resultCollector = (new ChunkedGetAllResponse(request, this, 
        &keys, values, exceptions, NULLPTR, trackers, destroyTracker, true, responseLock));
      reply->setChunkedResultHandler(resultCollector);
    }
  }
  else
  {
    if(needToCreateRC) {
      resultCollector = (new ChunkedInterestResponse(request,
        NULLPTR, *reply));
      reply->setChunkedResultHandler(resultCollector);
    }
  }
	...
	err = m_tcrdm->sendSyncRequestRegisterInterest(request, *reply,
			attemptFailover, this, endpoint);

	if (err == GF_NOERR /*|| err == GF_CACHE_REDUNDANCY_FAILURE*/) {
		if (reply->getMessageType() == TcrMessage::RESPONSE_FROM_SECONDARY) {
			LOGFINER("registerKeysNoThrow - got response from secondary for "
					"endpoint %s, ignoring.", endpoint->name().c_str());
...
GfErrType ThinClientRegion::unregisterKeysNoThrow(
	...
	TcrMessage request(TcrMessage::UNREGISTER_INTEREST_LIST, this, keys,
			false, false, true, InterestResultPolicy::NONE, m_tcrdm);
	err = m_tcrdm->sendSyncRequestRegisterInterest(request, reply);
	if (err == GF_NOERR /*|| err == GF_CACHE_REDUNDANCY_FAILURE*/) {
		if (attemptFailover) {
...
GfErrType ThinClientRegion::unregisterKeysNoThrowLocalDestroy(
  ...
  TcrMessage request(TcrMessage::UNREGISTER_INTEREST_LIST, this, keys, false, false, true, 
...
bool ThinClientRegion::isRegexRegistered(
	...
	if (interestListRegex.find(".*") != interestListRegex.end() || (!allKeys
			&& interestListRegex.find(regex) != interestListRegex.end())) {
		return true;
	}
	return false;
}

GfErrType ThinClientRegion::registerRegexNoThrow(const std::string& regex,
		bool attemptFailover, TcrEndpoint* endpoint, bool isDurable,
		VectorOfCacheableKeyPtr resultKeys, InterestResultPolicy interestPolicy,
		bool receiveValues, TcrMessage * reply) {
	RegionGlobalLocks acquireLocksRedundancy(this, false);
	RegionGlobalLocks acquireLocksFailover(this);
	CHECK_DESTROY_PENDING_NOTHROW(TryReadGuard);
	...
	if (attemptFailover) {
		if ((isDurable && (isRegexRegistered(m_durableInterestListRegex, regex,
				allKeys) || isRegexRegistered(
				m_durableInterestListRegexForUpdatesAsInvalidates, regex,
				allKeys))) || (!isDurable && (isRegexRegistered(
				m_interestListRegex, regex, allKeys) || isRegexRegistered(
				m_interestListRegexForUpdatesAsInvalidates, regex, allKeys)))) {
			return err;
		}
	}
  ...
  LOGDEBUG("ThinClientRegion::registerRegexNoThrow : interestpolicy is %d", interestPolicy.ordinal);

	//TODO:HItesh
	TcrMessage request(TcrMessage::REGISTER_INTEREST, 
                     m_fullPath,
			               regex.c_str(), 
                     interestPolicy,
                     isDurable,
			               getAttributes()->getCachingEnabled(), 
                     receiveValues, 
                     m_tcrdm);
  ACE_Recursive_Thread_Mutex responseLock;
	if (reply == NULL) {
    ...
    if(interestPolicy.ordinal == InterestResultPolicy::KEYS_VALUES.ordinal)
    {
      ...
      if(resultKeys == NULLPTR)
        resultKeys = VectorOfCacheableKeyPtr(new VectorOfCacheableKey());
      //hitesh need to check
      getAllResultCollector = (new ChunkedGetAllResponse(request, this, 
        NULL, values, exceptions, resultKeys, trackers, destroyTracker, true, responseLock));
    ...
    }else
    {
      isRCCreatedLocally= true;
      //hitesh need to check
      resultCollector = new ChunkedInterestResponse(request, resultKeys, replyLocal);
      reply->setChunkedResultHandler(resultCollector);
    }
  }
  err = m_tcrdm->sendSyncRequestRegisterInterest(request, *reply,
  ...
  if (err == GF_NOERR /*|| err == GF_CACHE_REDUNDANCY_FAILURE*/) {
  	if (reply->getMessageType() == TcrMessage::RESPONSE_FROM_SECONDARY) {
			LOGFINER("registerRegexNoThrow - got response from secondary for "
					"endpoint %s, ignoring.", endpoint->name().c_str());
		} else if (attemptFailover) {
      ...
      if (interestPolicy.ordinal != InterestResultPolicy::KEYS_VALUES.ordinal) {
				if (allKeys) {
  				localInvalidateRegion_internal();
					...
					const VectorOfCacheableKeyPtr& keys = resultCollector != NULL ? 
            resultCollector->getResultKeys() : getAllResultCollector->getResultKeys();
					if (keys != NULLPTR) {
  					localInvalidateForRegisterInterest(*keys);
					}
				}
  ...
  if(isRCCreatedLocally == true) {
	  if(resultCollector != NULL)
	    delete resultCollector;
    if(getAllResultCollector != NULL)
	    delete getAllResultCollector;
  }
	return err;
}
...
GfErrType ThinClientRegion::unregisterRegexNoThrow(const std::string& regex,
  ...
  if(err == GF_NOERR){
    TcrMessage reply(false, m_tcrdm);
    TcrMessage request(TcrMessage::UNREGISTER_INTEREST, m_fullPath, regex,
...
GfErrType ThinClientRegion::findRegex(const std::string& regex){
  ...
  if (m_interestListRegex.find(regex) == m_interestListRegex.end()
    && m_durableInterestListRegex.find(regex) == m_durableInterestListRegex.end()
    && m_interestListRegexForUpdatesAsInvalidates.find(regex) == m_interestListRegexForUpdatesAsInvalidates.end()
    && m_durableInterestListRegexForUpdatesAsInvalidates.find(regex) == m_durableInterestListRegexForUpdatesAsInvalidates.end()) {
      return GF_CACHE_ILLEGAL_STATE_EXCEPTION;
  }else {
    return err;
  }
...
GfErrType ThinClientRegion::unregisterRegexNoThrowLocalDestroy(const std::string& regex, bool attemptFailover) {
  GfErrType err = GF_NOERR;
  ...
  if(err == GF_NOERR){
    TcrMessage reply(false, m_tcrdm);
    TcrMessage request(TcrMessage::UNREGISTER_INTEREST, m_fullPath, regex, InterestResultPolicy::NONE, false, false, true, m_tcrdm);
    err = m_tcrdm->sendSyncRequestRegisterInterest(request, reply);
    if (err == GF_NOERR ) {
      if (attemptFailover) {
        clearRegex(regex);
...
void ThinClientRegion::addKeys(const VectorOfCacheableKey& keys,
		bool isDurable, bool receiveValues, InterestResultPolicy interestpolicy) {
	...
	for (VectorOfCacheableKey::Iterator iter = keys.begin(); iter != keys.end(); ++iter) {
    interestList.insert(std::pair<CacheableKeyPtr, InterestResultPolicy>(*iter, interestpolicy));
	}
}
...
void ThinClientRegion::addRegex(const std::string& regex, bool isDurable,
	...
	std::unordered_map<std::string, InterestResultPolicy>& interestListRegex =
			isDurable ? (receiveValues ? m_durableInterestListRegex
					: m_durableInterestListRegexForUpdatesAsInvalidates)
  ...
  interestListRegex.insert(std::pair<std::string, InterestResultPolicy>(regex, interestpolicy));
}
...
void ThinClientRegion::getInterestList(VectorOfCacheableKey & vlist) const {
	...
	for (std::unordered_map<CacheableKeyPtr, InterestResultPolicy>::iterator itr =
			nthis->m_durableInterestList.begin(); itr
			!= nthis->m_durableInterestList.end(); ++itr) {
        vlist.push_back(itr->first);
	}
	for (std::unordered_map<CacheableKeyPtr, InterestResultPolicy>::iterator itr =
			nthis->m_interestList.begin(); itr != nthis->m_interestList.end(); ++itr) {
        vlist.push_back(itr->first);
	}
}
void ThinClientRegion::getInterestListRegex(VectorOfCacheableString & vregex) const {
	ThinClientRegion* nthis = const_cast<ThinClientRegion*> (this);
	RegionGlobalLocks acquireLocksRedundancy(nthis, false);
	RegionGlobalLocks acquireLocksFailover(nthis);
	...
	for (std::unordered_map<std::string, InterestResultPolicy>::iterator itr =
			nthis->m_durableInterestListRegex.begin(); itr
			!= nthis->m_durableInterestListRegex.end(); ++itr) {
		vregex.push_back(CacheableString::create((*itr).first.c_str()));
	}
	for (std::unordered_map<std::string, InterestResultPolicy>::iterator itr =
			nthis->m_interestListRegex.begin(); itr
			!= nthis->m_interestListRegex.end(); ++itr) {
		vregex.push_back(CacheableString::create((*itr).first.c_str()));
	}
}
...
GfErrType ThinClientRegion::clientNotificationHandler(TcrMessage& msg) {
	...
	case TcrMessage::LOCAL_INVALIDATE: {
		LocalRegion::invalidateNoThrow(msg.getKey(), msg.getCallbackArgument(),
      -1, CacheEventFlags::NOTIFICATION | CacheEventFlags::LOCAL, msg.getVersionTag());
		break;
	}
	case TcrMessage::LOCAL_DESTROY: {
		err = LocalRegion::destroyNoThrow(msg.getKey(),
				msg.getCallbackArgument(), -1, CacheEventFlags::NOTIFICATION
        | CacheEventFlags::LOCAL, msg.getVersionTag());
		break;
	}
	case TcrMessage::CLEAR_REGION: {
		LOGDEBUG("remote clear region event for reigon[%s]",
					msg.getRegionName().c_str());
		err = localClearNoThrow(NULLPTR, CacheEventFlags::NOTIFICATION
	...
	case TcrMessage::LOCAL_DESTROY_REGION: {
		...
		preserveSB();
		err = LocalRegion::destroyRegionNoThrow(msg.getCallbackArgument(),
				true, CacheEventFlags::NOTIFICATION | CacheEventFlags::LOCAL);
		break;
	}
	case TcrMessage::LOCAL_CREATE:
		err = LocalRegion::putNoThrow(msg.getKey(), msg.getValue(),
				msg.getCallbackArgument(), oldValue, -1,
        CacheEventFlags::NOTIFICATION | CacheEventFlags::LOCAL, msg.getVersionTag());
		break;
	case TcrMessage::LOCAL_UPDATE: {
		// [sumedh] for update set the NOTIFICATION_UPDATE to trigger the
		// afterUpdate event even if the key is not present in local cache
		err = LocalRegion::putNoThrow(msg.getKey(), msg.getValue(),
				msg.getCallbackArgument(), oldValue, -1,
				CacheEventFlags::NOTIFICATION
						| CacheEventFlags::NOTIFICATION_UPDATE
						| CacheEventFlags::LOCAL, msg.getVersionTag()
            , msg.getDelta(),	msg.getEventId());
		break;
	}
  case TcrMessage::TOMBSTONE_OPERATION:
	...
	default: {
		if (TcrMessage::getAllEPDisMess() == &msg) {
			...
			LOGERROR(
						"Unknown message type %d in subscription event handler; possible serialization mismatch",
						msg.getMessageType());
			err = GF_MSG;
		}
		break;
...
GfErrType ThinClientRegion::handleServerException(const char* func,
	...
	if (strstr(exceptionMsg,
        ...
        }else if (strstr(exceptionMsg, "com.gemstone.gemfire.cache.CacheWriterException")
			!= NULL) {
	  ...
	  error = GF_FUNCTION_EXCEPTION;
			...
			"com.gemstone.gemfire.cache.CommitConflictException") != NULL) {
	...
	if(error != GF_AUTHENTICATION_REQUIRED_EXCEPTION){
	  LOGERROR("%s: An exception (%s) happened at remote server.", func, exceptionMsg);
	}else{
	  LOGFINER("%s: An exception (%s) happened at remote server.", func, exceptionMsg);
	}
	return error;
...
void ThinClientRegion::localInvalidateRegion_internal() {
	...
	for (VectorOfCacheableKey::Iterator iter = keysVec.begin(); iter
			!= keysVec.end(); ++iter) {
		VersionTagPtr versionTag;
...
void ThinClientRegion::invalidateInterestList(
	...
	for (std::unordered_map<CacheableKeyPtr,InterestResultPolicy>::iterator iter = interestList.begin(); iter
			!= interestList.end(); ++iter) {
		VersionTagPtr versionTag;
...
void ThinClientRegion::localInvalidateFailover() {
	...
	// VJR: No need to invalidate from the "m_xxxForUpdatesAsInvalidates" lists?
...
InterestResultPolicy ThinClientRegion::copyInterestList(VectorOfCacheableKey& keysVector,
		std::unordered_map<CacheableKeyPtr, InterestResultPolicy>& interestList) const {
  InterestResultPolicy interestPolicy = InterestResultPolicy::NONE;
	for (std::unordered_map<CacheableKeyPtr, InterestResultPolicy>::const_iterator iter =
			interestList.begin(); iter != interestList.end(); ++iter) {
...
void ThinClientRegion::registerInterestGetValues(const char* method,
		const VectorOfCacheableKey* keys,
		const VectorOfCacheableKeyPtr& resultKeys) {
	try {
		HashMapOfExceptionPtr exceptions(new HashMapOfException());
    GfErrType err = getAllNoThrow_remote(keys, NULLPTR, exceptions,
				resultKeys, true, NULLPTR);
		GfErrTypeToException(method, err);
		// log any exceptions here
		for (HashMapOfException::Iterator iter = exceptions->begin(); iter
				!= exceptions->end(); ++iter) {
		...
		LOGWARN("%s Exception while getting values: %s: %s", method,
					ex.getName(), ex.getMessage());
		std::string msg(method);
		msg += " failed in getting values";
		throw EntryNotFoundException(msg.c_str(), NULL, false, ExceptionPtr(
				ex.clone()));
	}
}
...
//	//		TcrEndpoint *ep = NULL;
// //     /*
//	//		std::string endpointStr = Utils::convertHostToCanonicalForm(
//	//				cs->asChar());
// //         */
...
//				m_fullPath, args, routingObj, getResult, failedNodes, m_tcrdm);
//		TcrMessage reply(true, m_tcrdm);
//		ChunkedFunctionExecutionResponsePtr
//				resultCollector(new ChunkedFunctionExecutionResponse(reply,
//						(getResult & 2), rc));
//		reply.setChunkedResultHandler(resultCollector);
//		reply.setTimeout(timeout);
//
...
void ThinClientRegion::executeFunction(const char* func,
  ...
  uint32_t timeout) {
    ...
    do {
      ...
      TcrMessage* msg;
      if (reExecuteForServ) {
        msg = new TcrMessage(TcrMessage::EXECUTE_REGION_FUNCTION, funcName,
          this, args, routingObj, getResult, failedNodes, timeout, m_tcrdm, (int8_t)1);
      }
      else {
        msg = new TcrMessage(TcrMessage::EXECUTE_REGION_FUNCTION, funcName,
          this, args, routingObj, getResult, failedNodes, timeout, m_tcrdm, (int8_t) 0);
      }
      TcrMessage reply(true, m_tcrdm);
      //hitesh need to check
      ...
      if (ThinClientBaseDM::isFatalClientError(err)) {
        ...
        if (err == GF_FUNCTION_EXCEPTION) {
          ...
          if (failedNodesIds != NULLPTR && failedNodesIds->size() > 0) {
            LOGDEBUG("ThinClientRegion::executeFunction with GF_FUNCTION_EXCEPTION failedNodesIds size = %d ", failedNodesIds->size());
            for(CacheableHashSet::Iterator itr = failedNodesIds->begin() ; itr != failedNodesIds->end(); ++itr) {
          ...
          attempt++;
          LOGDEBUG("ThinClientRegion::executeFunction with GF_NOTCON retry attempt = %d ", attempt);
          ...
          LOGINFO("function timeout. Name: %s, timeout: %d, params: %d, retryAttempts: %d ", funcName.c_str(), timeout, getResult, retryAttempts);
          GfErrTypeToException("ExecuteOnRegion", GF_TIMOUT);
        } 
		else if(err == GF_CLIENT_WAIT_TIMEOUT || err == GF_CLIENT_WAIT_TIMEOUT_REFRESH_PRMETADATA) {
          LOGINFO("function timeout, possibly bucket is not available or bucket blacklisted. Name: %s, timeout: %d, params: %d, retryAttempts: %d ", funcName.c_str(), timeout, getResult, retryAttempts);
    ...
    if (reExecute && (getResult & 1)) {
      reExecuteFunction(func, args, routingObj, getResult, rc, retryAttempts, failedNodes, timeout);
    }
    
}

CacheableVectorPtr ThinClientRegion::reExecuteFunction(const char* func,
  const CacheablePtr& args, CacheableVectorPtr routingObj,
  uint8_t getResult, ResultCollectorPtr rc, int32_t retryAttempts, CacheableHashSetPtr& failedNodes,
  uint32_t timeout) {
    ...
    do {
      ...
      std::string funcName(func);
      TcrMessage msg(TcrMessage::EXECUTE_REGION_FUNCTION, funcName,
        this, args, routingObj, getResult, failedNodes, timeout, m_tcrdm,  /*reExecute*/(int8_t) 1);
      TcrMessage reply(true, m_tcrdm);
      //hitesh need to check
      ChunkedFunctionExecutionResponse*
        resultCollector(new ChunkedFunctionExecutionResponse(reply,
      ...
      if (err == GF_NOERR && (reply.getMessageType() == TcrMessage::EXCEPTION
        || reply.getMessageType()
        == TcrMessage::EXECUTE_REGION_FUNCTION_ERROR)) {
          err = ThinClientRegion::handleServerException("Execute", reply.getException());
      }
      ...
      if (ThinClientBaseDM::isFatalClientError(err)) {
        ...
        if (err == GF_FUNCTION_EXCEPTION) {
          ...
          failedNodes->clear();
          if (failedNodesIds != NULLPTR && failedNodesIds->size() > 0) {
            LOGDEBUG("ThinClientRegion::reExecuteFunction with GF_FUNCTION_EXCEPTION failedNodesIds size = %d ", failedNodesIds->size());
            for(CacheableHashSet::Iterator itr = failedNodesIds->begin() ; itr != failedNodesIds->end(); ++itr) {
              failedNodes->insert(*itr);
            }
          }
          ...
          attempt++;
          LOGDEBUG("ThinClientRegion::reExecuteFunction with GF_NOTCON retry attempt = %d ", attempt);
          if (attempt > retryAttempts) {
            GfErrTypeToException("ExecuteOnRegion:", err);
          }
          reExecute = true;
...
bool ThinClientRegion::executeFunctionSH
  (const char* func, const CacheablePtr& args, uint8_t getResult, ResultCollectorPtr rc, 
  ...
  CacheableHashSetPtr& failedNodes, uint32_t timeout, bool allBuckets) {    
    ...
    for(HashMapT<BucketServerLocationPtr, CacheableHashSetPtr>::Iterator locationIter = locationMap->begin();
      locationIter != locationMap->end(); ++locationIter) {
        BucketServerLocationPtr serverLocation = locationIter.first();
        CacheableHashSetPtr buckets = locationIter.second();
        OnRegionFunctionExecution* worker = new OnRegionFunctionExecution(func, this, args, buckets, getResult,
          timeout, dynamic_cast< ThinClientPoolDM* >(m_tcrdm), &resultCollectorLock, rc, userAttr, false, serverLocation, allBuckets);
        threadPool->perform(worker);
        feWorkers.push_back(worker);
    }

    for(std::vector<OnRegionFunctionExecution*>::iterator iter = feWorkers.begin(); iter != feWorkers.end(); ++iter) {
      OnRegionFunctionExecution* worker = *iter;
      GfErrType err = worker->getResult();
      TcrMessage* currentReply = worker->getReply();

      if (err == GF_NOERR && ( currentReply->getMessageType() == TcrMessage::EXCEPTION
        || currentReply->getMessageType() == TcrMessage::EXECUTE_REGION_FUNCTION_ERROR)) {
          err = ThinClientRegion::handleServerException("Execute", currentReply->getException());
      }
      ...
      if (ThinClientBaseDM::isFatalClientError(err)) {
        ...
        if (err == GF_FUNCTION_EXCEPTION) {
          ...
          if ((poolDM != NULL) && (poolDM->getClientMetaDataService() != NULL)) {
            poolDM->getClientMetaDataService()->enqueueForMetadataRefresh( this->getFullPath(), 0);
          }
          ...
          if (failedNodeIds != NULLPTR && failedNodeIds->size() > 0) {
            LOGDEBUG("ThinClientRegion::executeFunctionSH with GF_FUNCTION_EXCEPTION failedNodeIds size = %d ", failedNodeIds->size());
            for(CacheableHashSet::Iterator itr = failedNodeIds->begin(); itr != failedNodeIds->end(); ++itr) {
              failedNodes->insert(*itr);
            }
          }
        } else if (err == GF_NOTCON) {
...
GfErrType ThinClientRegion::getFuncAttributes(const char* func, std::vector<int8>* *attr)
{
	GfErrType err = GF_NOERR;
	...
	std::string funcName(func);
	TcrMessage request(TcrMessage::GET_FUNCTION_ATTRIBUTES, funcName, m_tcrdm);
	TcrMessage reply(true, m_tcrdm);
	err = m_tcrdm->sendSyncRequest(request, reply);
	if (err != GF_NOERR) {
		return err;
...
GfErrType ThinClientRegion::getNoThrow_FullObject(EventIdPtr eventId,
		CacheablePtr& fullObject, VersionTagPtr& versionTag) {
...
void ThinClientRegion::txPut(const CacheableKeyPtr& key,
		const CacheablePtr& value, const UserDataPtr& aCallbackArgument, VersionTagPtr versionTag) {
	CacheablePtr oldValue;
	...
	Utils::updateStatOpTime(m_regionStats->getStat(),
			RegionStatType::getInstance()->getPutTimeId(), sampleStartNanos);
...
void ChunkedInterestResponse::handleChunk(const uint8_t* chunk,
		int32_t chunkLen, uint8_t isLastChunkWithSecurity) {
	DataInput input(chunk, chunkLen);

	input.setPoolName(m_replyMsg.getPoolName());

	uint32_t partLen;
	if (TcrMessageHelper::readChunkPartHeader(m_msg, input, 0,
			GemfireTypeIds::CacheableArrayList, "ChunkedInterestResponse",
...
void ChunkedKeySetResponse::handleChunk(const uint8_t* chunk,
		int32_t chunkLen, uint8_t isLastChunkWithSecurity) {
	DataInput input(chunk, chunkLen);
	...
	uint32_t partLen;
	if (TcrMessageHelper::readChunkPartHeader(m_msg, input, 0,
			GemfireTypeIds::CacheableArrayList, "ChunkedKeySetResponse",
			partLen, isLastChunkWithSecurity) != TcrMessageHelper::OBJECT) {
		// encountered an exception part, so return without reading more
		m_replyMsg.readSecureObjectPart(input, false, true,
				isLastChunkWithSecurity);
...
void ChunkedQueryResponse::readObjectPartList(DataInput& input, bool isResultSet)
{
  bool hasKeys;
      ...
      if (isResultSet) {
        ...
        if (arrayType == GemfireTypeIdsImpl::FixedIDByte) {
          ...
          if (arrayType != GemfireTypeIdsImpl::CacheableObjectPartList) {
            LOGERROR( "Query response got unhandled message format %d while expecting struct set object part list; possible serialization mismatch", arrayType);
            throw MessageException("Query response got unhandled message format while expecting struct set object part list; possible serialization mismatch");
          }
...
void ChunkedQueryResponse::handleChunk(const uint8_t* chunk, int32_t chunkLen,
	...
	if ((objType = TcrMessageHelper::readChunkPartHeader(m_msg, input,
			GemfireTypeIdsImpl::FixedIDByte,
			(uint8_t)GemfireTypeIdsImpl::CollectionTypeImpl, "ChunkedQueryResponse",
			partLen, isLastChunkWithSecurity)) == TcrMessageHelper::EXCEPTION) {
		// encountered an exception part, so return without reading more
	...
	if (strcmp(isStructTypeImpl, "com.gemstone.gemfire.cache.query.Struct")
			== 0) {
  ...
  if (arrayType == GemfireTypeIds::CacheableObjectArray) {
    ...
    input.read(&arrayType);
    if (arrayType != GemfireTypeIdsImpl::CacheableObjectPartList) {
      LOGERROR( "Query response got unhandled message format %d while expecting object part list; possible serialization mismatch", arrayType);
      throw MessageException("Query response got unhandled message format while expecting object part list; possible serialization mismatch");
    }
    readObjectPartList(input, isResultSet);
  } else {
    LOGERROR( "Query response got unhandled message format %d; possible serialization mismatch", arrayType);
    throw MessageException("Query response got unhandled message format; possible serialization mismatch");
  }
...
void ChunkedQueryResponse::skipClass(DataInput& input) {
	...
	if (classByte == GemfireTypeIdsImpl::Class) {
		...
		throw IllegalStateException("ChunkedQueryResponse::skipClass: "
...
void ChunkedFunctionExecutionResponse::handleChunk(const uint8_t* chunk,
		int32_t chunkLen, uint8_t isLastChunkWithSecurity) {
	...
	if ((TcrMessageHelper::ChunkObjectType) arrayType
	...
	int32_t len;
  int startLen = input.getBytesRead() - 1;//from here need to look value part + memberid AND -1 for array type
	input.readArrayLen(&len);  
  
  //read a byte to determine whether to read exception part for sendException or read objects.
  uint8_t partType;
  input.read(&partType);  
  ...
  bool readPart = true;
  LOGDEBUG("ChunkedFunctionExecutionResponse::handleChunk chunkLen = %d & partLen = %d ", chunkLen, partLen);
  if (partType == GemfireTypeIdsImpl::JavaSerializable) {
    isExceptionPart = true;
    //reset the input.
    input.reset();
    
    if (((isLastChunkWithSecurity & 0x02) && (chunkLen - (int32_t)partLen <= CHUNK_HDR_LEN + SECURE_PART_LEN)) 
      || (((isLastChunkWithSecurity & 0x02) == 0)  && (chunkLen - (int32_t)partLen <= CHUNK_HDR_LEN))) {
      readPart = false;
      input.readInt(&partLen);
      ...
      startLen = input.getBytesRead();//reset from here need to look value part + memberid AND -1 for array type
      
      //Since it is contained as a part of other results, read arrayType which is arrayList = 65.
      input.read(&arrayType);     
    ...
    //rewind cursor by 1 to what we had read a byte to determine whether to read exception part or read objects.
 ...
 // CacheablePtr memberId;  
  if (readPart) {        
    ...
    LOGDEBUG("function input.getBytesRemaining() = %d ",  input.getBytesRemaining());
    //is there any way to assert it, as after that we need to read security header
    /*if(input.getBytesRemaining() !=  0) {
      LOGERROR("Function response not read all bytes");
      throw new IllegalStateException("Function Execution didn't read all bytes");
    }*/
  }
  else {
    ...
    if (isExceptionPart) {      
      UserFunctionExecutionExceptionPtr uFEPtr(new UserFunctionExecutionException(value->toString()));      
...
void ChunkedGetAllResponse::handleChunk(const uint8_t* chunk, int32_t chunkLen,
	...
	input.setPoolName(m_msg.getPoolName());
	uint32_t partLen;
	if (TcrMessageHelper::readChunkPartHeader(m_msg, input,
			GemfireTypeIdsImpl::FixedIDByte,
	...
	VersionedCacheableObjectPartList objectList(m_keys, &m_keysOffset, m_values,
			m_exceptions, m_resultKeys, m_region, &m_trackerMap,
			m_destroyTracker, m_addToLocalCache, m_dsmemId, m_responseLock);
...
void ChunkedGetAllResponse::add(const ChunkedGetAllResponse* other) {
  if (m_values != NULLPTR) {
		for (HashMapOfCacheable::Iterator iter = other->m_values->begin(); iter
				!= other->m_values->end(); iter++) {
			m_values->insert(iter.first(), iter.second());
		}
	}
  
	if (m_exceptions != NULLPTR) {
		for (HashMapOfException::Iterator iter = other->m_exceptions->begin(); iter
				!= other->m_exceptions->end(); iter++) {
			m_exceptions->insert(iter.first(), iter.second());
		}
	}

	for (MapOfUpdateCounters::iterator iter = other->m_trackerMap.begin(); iter
			!= other->m_trackerMap.end(); iter++) {
		m_trackerMap[iter->first] = iter->second;
	}

	if (m_resultKeys != NULLPTR) {
		for (VectorOfCacheableKey::Iterator iter = other->m_resultKeys->begin(); iter
				!= other->m_resultKeys->end(); iter++) {
...
void ChunkedPutAllResponse::handleChunk(const uint8_t* chunk, int32_t chunkLen,
  uint8_t isLastChunkWithSecurity) {
    ...
    int8_t chunkType;
    if ((chunkType
      = (TcrMessageHelper::ChunkObjectType) TcrMessageHelper::readChunkPartHeader(m_msg, input,
      GemfireTypeIdsImpl::FixedIDByte,
      GemfireTypeIdsImpl::VersionedObjectPartList,
      "ChunkedPutAllResponse", partLen, isLastChunkWithSecurity)) == TcrMessageHelper::NULL_OBJECT) {
    ...
    if ((TcrMessageHelper::ChunkObjectType) chunkType
      == TcrMessageHelper::OBJECT) {
        LOGDEBUG("ChunkedPutAllResponse::handleChunk object");
        ACE_Recursive_Thread_Mutex responseLock;
        VersionedCacheableObjectPartListPtr vcObjPart(new VersionedCacheableObjectPartList(m_msg.getChunkedResultHandler()->getEndpointMemId(), responseLock));
      ...
      int8_t byte0;
      input.read(&byte0);
      LOGDEBUG("ChunkedPutAllResponse::handleChunk single-hop bytes byte0 = %d ", byte0);
      int8_t byte1;
      input.read(&byte1);
      m_msg.readSecureObjectPart(input, false, true, isLastChunkWithSecurity);
      ...
      if (pool != NULLPTR && !pool->isDestroyed() && pool->getPRSingleHopEnabled())
      {
        ...
        if ((poolDM != NULL) && (poolDM->getClientMetaDataService() != NULL) && (byte0 != 0)) {
          LOGFINE("enqueued region %s for metadata refresh for singlehop for PUTALL operation.", m_region->getFullPath());
          poolDM->getClientMetaDataService()->enqueueForMetadataRefresh( m_region->getFullPath(), byte1);
        }
      }
    }
}
...
void ChunkedRemoveAllResponse::handleChunk(const uint8_t* chunk, int32_t chunkLen,
  uint8_t isLastChunkWithSecurity) {
    DataInput input(chunk, chunkLen);
    input.setPoolName(m_msg.getPoolName());
    uint32_t partLen;
    ...
    if ((chunkType
      ...
      GemfireTypeIdsImpl::FixedIDByte,
      GemfireTypeIdsImpl::VersionedObjectPartList,
      "ChunkedRemoveAllResponse", partLen, isLastChunkWithSecurity)) == TcrMessageHelper::NULL_OBJECT) {
    ...
    if ((TcrMessageHelper::ChunkObjectType) chunkType
      ...
      int8_t byte0;
      input.read(&byte0);
      LOGDEBUG("ChunkedRemoveAllResponse::handleChunk single-hop bytes byte0 = %d ", byte0);
      int8_t byte1;
      input.read(&byte1);
      m_msg.readSecureObjectPart(input, false, true, isLastChunkWithSecurity);
      ...
      if (pool != NULLPTR && !pool->isDestroyed() && pool->getPRSingleHopEnabled())
      {
        ThinClientPoolDM* poolDM = dynamic_cast< ThinClientPoolDM* >(pool.ptr());
        if ((poolDM != NULL) && (poolDM->getClientMetaDataService() != NULL) && (byte0 != 0)) {
          LOGFINE("enqueued region %s for metadata refresh for singlehop for REMOVEALL operation.", m_region->getFullPath());
          poolDM->getClientMetaDataService()->enqueueForMetadataRefresh( m_region->getFullPath(), byte1);
        }
      }
    }
...
void ChunkedDurableCQListResponse::handleChunk(const uint8_t* chunk,
  ...
  if (!isObj) {
    ...
    char exMsg[256];
    ACE_OS::snprintf(exMsg, 255, "ChunkedDurableCQListResponse::handleChunk: part is not object");
  ...
  input.read(&stringParts);  //read the number of strings in the message this is one byte
