...
namespace gemfire {
  ...
  PdxFieldType::PdxFieldType():Serializable() {
    ...
    PdxFieldType::PdxFieldType(const char* fieldName, const char* className, uint8_t typeId,
			int32 sequenceId, bool isVariableLengthType, int32 fixedSize,
			int32 varLenFieldIdx):Serializable() {
  ...
  bool PdxFieldType::equals(PdxFieldTypePtr otherObj)
  {
    ...
    if(otherFieldType->m_fieldName.compare(m_fieldName)==0 && otherFieldType->m_typeId == m_typeId)
      return true;
	...
	CacheableStringPtr PdxFieldType::toString() const {
		char stringBuf[1024];
    /* adongre  - Coverity II
     * CID 29208: Calling risky function (SECURE_CODING)[VERY RISKY]. Using "sprintf" can cause a 
     ...
     */
		ACE_OS::snprintf(stringBuf, 1024,
