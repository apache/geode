...
uint16_t MemberListForVersionStamp::add(DSMemberForVersionStampPtr member)
{
  ...
  std::unordered_map<std::string, DistributedMemberWithIntIdentifier>::iterator it = m_members2.find(member->getHashKey());
  if (it != m_members2.end ()) 
    return (*it).second.m_identifier;
  ...
  LOGDEBUG("Adding a new member to the member list maintained for version stamps member Ids. HashKey: %s MemberCounter: %d", member->getHashKey().c_str(), m_memberCounter);
  return m_memberCounter;
}

// This function is protected  using readers/writer lock
DSMemberForVersionStampPtr MemberListForVersionStamp::getDSMember(uint16_t memberId)
{
  ReadGuard guard (m_mapLock);
  std::unordered_map<uint32_t, DSMemberForVersionStampPtr>::iterator it = m_members1.find(memberId);
  if (it != m_members1.end ()) 
    return (*it).second;
  return NULLPTR;
}
