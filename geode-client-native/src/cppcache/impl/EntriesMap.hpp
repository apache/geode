...
#define SYNCHRONIZE_SEGMENT_FOR_KEY(keyPtr) SegmentMutexGuard _segment_guard( \
    ((EntriesMap*)m_entries)->segmentFor(keyPtr))
  ...
  public:
    ...
    EntriesMap(EntryFactory* entryFactory) :
    ...
    virtual GfErrType invalidate(const CacheableKeyPtr& key,
        MapEntryImplPtr& me, CacheablePtr& oldValue, VersionTagPtr versionTag) = 0;
    virtual GfErrType create(const CacheableKeyPtr& key,
        const CacheablePtr& newValue, MapEntryImplPtr& me,
        CacheablePtr& oldValue, int updateCount, int destroyTracker, VersionTagPtr versionTag) = 0;
    ...
    virtual GfErrType remove(const CacheableKeyPtr& key, CacheablePtr& result,
        MapEntryImplPtr& me, int updateCount, VersionTagPtr versionTag, bool afterRemote) = 0;
    ...
    virtual CacheablePtr getFromDisk( const CacheableKeyPtr& key, MapEntryImpl* me) const
    {
      return NULLPTR;
    }
     ...
     */
    virtual GfErrType isTombstone(CacheableKeyPtr& key, MapEntryImplPtr& me, bool& result) = 0;
