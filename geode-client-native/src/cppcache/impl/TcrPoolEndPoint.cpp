...
TcrPoolEndPoint::TcrPoolEndPoint( const std::string& name, CacheImpl* cache,
        ...
        ACE_Semaphore& redundancySema, ThinClientPoolDM* dm ): TcrEndpoint(name, cache, failoverSema, cleanupSema, redundancySema, dm), m_dm(dm)
{
}
...
void TcrPoolEndPoint::processMarker( )
{
  m_dm->processMarker( );
}
...
void TcrPoolEndPoint::sendRequestForChunkedResponse(const TcrMessage& request, TcrMessage& reply, TcrConnection* conn)
{
  conn->sendRequestForChunkedResponse(request, request.getMsgLength(), reply, request.getTimeout(), reply.getTimeout());
}
ThinClientPoolDM* TcrPoolEndPoint::getPoolHADM( ) 
{ 
  return m_dm; 
}
...
{
  if( !m_dm->getThreadLocalConnections( ) )
    closeConnection( conn );
}

bool TcrPoolEndPoint::isMultiUserMode()
{
  return m_dm->isMultiUserMode();
}
...
GfErrType TcrPoolEndPoint::registerDM( bool clientNotification, bool isSecondary, bool isActiveEndpoint, ThinClientBaseDM * distMgr)
{
  ...
  if ( !connected() ) {
    ...
    if ( ( err = createNewConnection( newConn, false, false,
  	   DistributedSystem::getSystemProperties()->connectTimeout(), 0, connected() ) ) != GF_NOERR ) {
	  setConnected( false );
      return err;
    }
  ...
  LOGFINEST("TcrEndpoint::registerPoolDM( ): registering DM and notification "
      "channel for endpoint %s", name().c_str());

  if (m_numRegionListener == 0) {
    if ( ( err = createNewConnection( m_notifyConnection, true,
      isSecondary, DistributedSystem::getSystemProperties()->connectTimeout()*3, 0 ) ) != GF_NOERR ) {
	  setConnected( false );
      LOGWARN("Failed to start subscription channel for endpoint %s", name().c_str());
      return err;
    }
    m_notifyReceiver = new GF_TASK_T<TcrEndpoint>( this, &TcrEndpoint::receiveNotification , NC_Notification);
...
void TcrPoolEndPoint::unregisterDM( bool clientNotification, ThinClientBaseDM * distMgr, bool checkQueueHosted )
{
	ACE_Guard< ACE_Recursive_Thread_Mutex > guard(getQueueHostedMutex( ));

  if ( checkQueueHosted && !m_isQueueHosted ) {
    LOGFINEST("TcrEndpoint: unregistering pool DM, notification channel not present for %s", name().c_str());
    return;
  }

  LOGFINEST("TcrEndpoint: unregistering pool DM and closing notification "
    "channel for endpoint %s", name().c_str());
  ...
  LOGFINEST("Decremented notification count for endpoint %s to %d",
    name().c_str(), m_numRegionListener);
  LOGFINEST("TcrEndpoint: unregisterPoolDM done for endpoint %s", name().c_str());
}
