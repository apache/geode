...
LRUEntriesMap::LRUEntriesMap(EntryFactory* entryFactory,
  ...
  ConcurrentEntriesMap(entryFactory, concurrencyChecksEnabled, region, concurrency), m_lruList(),
      m_limit(limit), m_validEntries(0), m_pmPtr(NULLPTR), m_heapLRUEnabled(
          heapLRUEnabled)
{
  m_currentMapSize = 0;
  m_action = NULL;
    ...
    int updateCount, int destroyTracker, VersionTagPtr versionTag)
{
  ...
  { //SYNCHRONIZE_SEGMENT(segmentRPtr);
    ...
    becomesInvalid = (newValue == NULLPTR);
    if ((err = segmentRPtr->create(key, newValue, me, oldValue,
        updateCount, destroyTracker, versionTag)) != GF_NOERR) {
      return err;
    }
    // TODO: [sumedh] can newValue ever be a token ??
    if (!CacheableToken::isToken(newValue)) {
      m_validEntries++;
    }
    // [sumedh] oldValue can be an invalid token when "createIfInvalid" is true
  ...
  if (m_evictionControllerPtr != NULL) {
    int64_t newSize = (int64_t) Utils::checkAndGetObjectSize(newValue);
    newSize += (int64_t) Utils::checkAndGetObjectSize(key);
    if (oldValue != NULLPTR) {
      newSize -= (int64_t) oldValue->objectSize();
    }
    else {
      newSize -= (int64_t) sizeof(void*);
    }
...
void LRUEntriesMap::processLRU(int32_t numEntriesToEvict)
{
  ...
  for (int32_t i = 0; i < numEntriesToEvict; i++) {
    if (m_validEntries.value() > 0 && (int32_t) size() > 0) {
    ...
    MapEntryImplPtr& me, CacheablePtr& oldValue, VersionTagPtr versionTag)
{
  ...
  GfErrType err = segmentRPtr->invalidate(key, me, oldValue, versionTag, isTokenAdded);
  if (isTokenAdded) {
  ++m_size;
  }
  if (err != GF_NOERR) {
  ...
  // [sumedh] get the old value first which is required for heapLRU
...
GfErrType LRUEntriesMap::put(const CacheableKeyPtr& key,
    const CacheablePtr& newValue, MapEntryImplPtr& me, CacheablePtr& oldValue,
    int updateCount, int destroyTracker, VersionTagPtr versionTag, bool& isUpdate, 
    DataInput* delta)
{
  MapSegment* segmentRPtr = segmentFor(key);
  ...
  bool becomesInvalid = false;
  {
    ...
    if ((err = segmentRPtr->put(key, newValue, me, oldValue, updateCount,
        destroyTracker, isUpdate, versionTag, delta )) != GF_NOERR) {
      if (segmentLocked == true)
        segmentRPtr->release();
      return err;
    }

    bool isOldValueToken = CacheableToken::isToken(oldValue);
    // TODO: [sumedh] need tests for checking that oldValue is returned
    // correctly to listeners for overflow -- this is for all operations
    // put, invalidate, destroy
    // TODO: [sumedh] need tests for checking if the overflowed entry is
    // destroyed *from disk* in put
    // [sumedh] existing overflowed entry should be returned correctly in
    ...
    // TODO: [sumedh] when can newValue be a token ??
  ...
  if (m_evictionControllerPtr != NULL) {
    ...
    /*
    if (newSize == 0) {
      LOGWARN("Object size for class ID %d should not be zero when HeapLRU is enabled", newValue->classId());
      LOGDEBUG("Type ID is %d for the object returning zero HeapLRU size", newValue->typeId());
    }
    */
    if (isUpdate == false) {
      newSize += (int64_t) Utils::checkAndGetObjectSize(key);
    ...
    MapEntryImplPtr& me)
{
  ...
  }
  {
    ...
    if (returnPtr != NULLPTR && CacheableToken::isOverflowed(returnPtr)) {
      ...
      bool isUpdate;
      VersionTagPtr versionTag; 
      if (GF_NOERR == segmentRPtr->put(key, tmpObj, mePtr, oldValue, 0, 0, isUpdate, versionTag, NULL)) {
      //m_entriesRetrieved++;
        ++m_validEntries;
      ...
      if (m_evictionControllerPtr != NULL) {
        int64_t newSize = 0;
        if (tmpObj != NULLPTR) {
          newSize += (int64_t)(tmpObj->objectSize()-CacheableToken::invalid(
              )->objectSize());
        }
        else {
          newSize += sizeof(void*);
    ...
    VersionTagPtr versionTag, bool afterRemote)
{
  ...
  GfErrType err;
  if ((err = segmentRPtr->remove(key, result, me, updateCount, versionTag, afterRemote, isEntryFound)) == GF_NOERR) {
    //ACE_Guard<MapSegment> _guard(*segmentRPtr);
    if (result != NULLPTR && me != NULLPTR) {
      ...
      if (m_evictionControllerPtr != NULL) {
        int64_t sizeToRemove = (int64_t) key->objectSize();
        sizeToRemove += (int64_t) result->objectSize();
...
CacheablePtr LRUEntriesMap::getFromDisk( const CacheableKeyPtr& key, MapEntryImpl* me) const
{
  void* persistenceInfo = me->getLRUProperties().getPersistenceInfo();
  CacheablePtr tmpObj;
  try {
    LOGDEBUG( "Reading value from persistence layer for key: %s", key->toString( )->asChar( ) );
    tmpObj = m_pmPtr->read(key, persistenceInfo);
  }
  catch (Exception& ex) {
    LOGERROR("read on the persistence layer failed - %s", ex.getMessage());
