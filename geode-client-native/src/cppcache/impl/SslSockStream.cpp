...
void* SslSockStream::getACESSLFuncPtr(const char* function_name) {
  ...
  if ( func == NULL ) {
    char msg[1000];
    ACE_OS::snprintf( msg, 1000, "cannot find function %s in library %s", function_name, "cryptoImpl" );
    LOGERROR(msg);
...
void SslSockStream::initACESSLFuncPtrs() {
  ...
  if ( m_dll.open( libName, ACE_DEFAULT_SHLIB_MODE, 0 ) == -1 ) {
      ...
      LOGERROR( "Failed to open cryptoImpl . Errno: %d : %s",
...
SslSockStream::SslSockStream(ACE_SOCKET sock, const char* pubkey, const char* privkey)
:m_ctx(NULL), m_sock(sock), m_pubkey(pubkey), m_privkey(privkey),
 gf_initSslImpl_Ptr(NULL), gf_clearSslImpl_Ptr(NULL), gf_set_option_Ptr(NULL),
 gf_listen_Ptr(NULL), gf_connect_Ptr(NULL), gf_recv_n_Ptr(NULL),
 gf_send_n_Ptr(NULL), gf_get_local_addr_Ptr(NULL)
{
}
...
int
SslSockStream::set_option (int level, int option, void *optval, int optlen) const
{
  return gf_set_option_Ptr(m_ctx, level, option, optval, optlen);
}

int
SslSockStream::listen(ACE_INET_Addr addr, unsigned waitSeconds)
{
  return gf_listen_Ptr(m_ctx, addr, waitSeconds);
}

int
SslSockStream::connect(ACE_INET_Addr ipaddr, unsigned waitSeconds)
{
  return gf_connect_Ptr(m_ctx, ipaddr, waitSeconds);
}

ssize_t
SslSockStream::recv_n (void *buf, size_t len, const ACE_Time_Value *timeout,
...
ssize_t
SslSockStream::send_n (const void *buf, size_t len, const ACE_Time_Value *timeout,
                        size_t *bytes_transferred) const
{
...
int
SslSockStream::get_local_addr(ACE_Addr & addr) const
{
