...
ThinClientPoolRegion::ThinClientPoolRegion( const std::string& name,
...
{
  try {
    ThinClientPoolDM* poolDM = dynamic_cast< ThinClientPoolDM* >( PoolManager::find( m_regionAttributes->getPoolName( ) ).ptr( ) );
    m_tcrdm = dynamic_cast< ThinClientBaseDM* >( poolDM );
    if ( !m_tcrdm ) {
      // ARB: TODO: create a PoolNotFound exception.
      throw IllegalStateException( "pool not found" );
    }
    poolDM->incRegionCount( );
  }
  catch (const Exception& ex) {
    LOGERROR("Failed to initialize region due to %s: %s", ex.getName(), ex.getMessage());
    throw;
  }
}
