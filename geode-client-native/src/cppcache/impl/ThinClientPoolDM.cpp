 ...
 */
class GetAllWork: public PooledWork<GfErrType>, private NonCopyable, private NonAssignable {
...
public:
  GetAllWork(ThinClientPoolDM* poolDM, const RegionPtr& region,
      ...
      bool isBGThread, bool addToLocalCache, ChunkedGetAllResponse* responseHandler, const UserDataPtr& aCallbackArgument) :
    m_poolDM(poolDM), m_serverLocation(serverLocation), m_attemptFailover(
        attemptFailover), m_isBGThread(isBGThread), m_addToLocalCache(
        addToLocalCache), m_userAttribute(NULLPTR), m_responseHandler(responseHandler), m_regionName(region->getFullPath()),
       m_keys(keys), m_region(region), m_aCallbackArgument(aCallbackArgument)
    {
    
      m_request = new TcrMessage(region.ptr(), m_keys.ptr(), m_poolDM, m_aCallbackArgument);
    m_reply = new TcrMessage(true, m_poolDM);
    if(m_poolDM->isMultiUserMode())
      m_userAttribute = TSSUserAttributesWrapper::s_gemfireTSSUserAttributes->getUserAttributes();
   ...
   // VectorOfCacheableKeyPtr resultKeys(new VectorOfCacheableKey());
    ...
    m_resultCollector = (new ChunkedGetAllResponse(*m_reply,
      dynamic_cast<ThinClientRegion*> (m_region.ptr()), m_keys.ptr(), m_responseHandler->getValues(),
      m_responseHandler->getExceptions(), m_responseHandler->getResultKeys(), m_responseHandler->getUpdateCounters(), 0, m_addToLocalCache, m_responseHandler->getResponseLock()));
  ...
  GfErrType execute(void) {
   ...
   // init();
    ...
    m_request->InitializeGetallMsg(m_request->getCallbackArgument());//now init getall msg
...
ThinClientPoolDM::ThinClientPoolDM( const char* name, PoolAttributesPtr poolAttrs,
  TcrConnectionManager& connManager ) : m_poolName( name ), m_isDestroyed( false ),
  m_locHelper( NULL ), ThinClientBaseDM( connManager , NULL ), Pool( poolAttrs ),
  m_connSema(0), m_pingTask(NULL), m_cliCallbackTask(NULL), m_connManageTask(NULL), m_pingSema( 0 ), m_cliCallbackSema(0), m_poolSize(0),
  m_stats(NULL), m_PoolStatsSampler( NULL ), m_pingTaskId(-1), m_updateLocatorListTaskId(-1), m_updateLocatorListSema(0), m_updateLocatorListTask(NULL),
  m_connManageTaskId(-1), m_destroyPending( false ), m_numRegions( 0 ),
  m_server(0), m_isMultiUserMode( false ), 
  m_clientMetadataService(NULL),
  m_destroyPendingHADM( false ), m_sticky (false), m_primaryServerQueueSize(PRIMARY_QUEUE_NOT_AVAILABLE)
{
  ...
  //hitesh to set security flag at pool level
  ...
  std::string clientDurableId = (durableId == NULL || strlen(durableId) == 0)? durableId :
    durableId + (m_poolName.c_str()!= NULL ?( poolSeparator + m_poolName) : "") ;
  
  const uint32_t durableTimeOut = (sysProp != NULL)? sysProp->durableTimeout() :0;
  m_memId = new ClientProxyMembershipID( hostName, hostAddr, hostPort, clientDurableId.c_str(), durableTimeOut );

  if (m_attrs->m_initLocList.size()==0 && m_attrs->m_initServList.size()==0) {
...
void ThinClientPoolDM::init( )
{
  ...
  SystemProperties* sysProp = DistributedSystem::getSystemProperties();
   ...
   if (m_isMultiUserMode) {
     LOGINFO("Multiuser authentication is enabled for pool %s", m_poolName.c_str());
   }
  //hitesh to set security flag at pool level
  this->m_isSecurityOn = sysProp->isSecurityOn();

  LOGDEBUG("ThinClientPoolDM::init: security in on/off = %d ",this->m_isSecurityOn);
...
//  }
  ...
  LOGDEBUG("ThinClientPoolDM::init: is grid client = %d ", props->isGridClient());
...
PropertiesPtr ThinClientPoolDM::getCredentials(TcrEndpoint* ep)
{
  ...
  if (authInitialize != NULLPTR) {
    LOGFINER("ThinClientPoolDM::getCredentials: acquired handle to authLoader, "
        "invoking getCredentials %s", ep->name().c_str());
    /* adongre
     * CID 28901: Copy into fixed size buffer (STRING_OVERFLOW)
     * You might overrun the 100 byte fixed-size string "tmpEndpoint" by copying the 
     * return value of "stlp_std::basic_string<char, stlp_std::char_traits<char>, 
...
void ThinClientPoolDM::startBackgroundThreads()
{
  ...
  m_pingTask = new  GF_TASK_T< ThinClientPoolDM >( this, &ThinClientPoolDM::pingServer, NC_Ping_Thread );
  m_pingTask->start( );

  SystemProperties* props = DistributedSystem::getSystemProperties();

  if(props->onClientDisconnectClearPdxTypeIds() == true) {
    m_cliCallbackTask = new  GF_TASK_T< ThinClientPoolDM >( this, &ThinClientPoolDM::cliCallback );
    m_cliCallbackTask->start( );
  }

  LOGDEBUG("ThinClientPoolDM::startBackgroundThreads: Creating ping task");
  ACE_Event_Handler* pingHandler = new ExpiryHandler_T<
    ThinClientPoolDM >( this, &ThinClientPoolDM::doPing );

  long pingInterval = getPingInterval( ) /(1000 * 2);
  if(pingInterval > 0)
  {
    LOGDEBUG("ThinClientPoolDM::startBackgroundThreads: Scheduling ping task at %ld",pingInterval);
    m_pingTaskId = getCacheImplExpiryTaskManager( )->scheduleExpiryTask(
        pingHandler, 1, pingInterval, false );
  }
  else
  {
    LOGDEBUG("ThinClientPoolDM::startBackgroundThreads: Not Scheduling ping task as ping interval %ld",getPingInterval( ));
  }

  long updateLocatorListInterval = getUpdateLocatorListInterval( ) ;

  if (updateLocatorListInterval > 0) {
    m_updateLocatorListTask = new  GF_TASK_T< ThinClientPoolDM >( this, &ThinClientPoolDM::updateLocatorList );
    m_updateLocatorListTask->start( );

    updateLocatorListInterval = updateLocatorListInterval/1000;//seconds
    LOGDEBUG("ThinClientPoolDM::startBackgroundThreads: Creating updateLocatorList task");
    ACE_Event_Handler* updateLocatorListHandler = new ExpiryHandler_T<
      ThinClientPoolDM >( this, &ThinClientPoolDM::doUpdateLocatorList );

    LOGDEBUG("ThinClientPoolDM::startBackgroundThreads: Scheduling updater Locator task at %ld",updateLocatorListInterval);
    m_updateLocatorListTaskId = getCacheImplExpiryTaskManager( )->scheduleExpiryTask(
        updateLocatorListHandler, 1, updateLocatorListInterval, false );
  }

  LOGDEBUG("ThinClientPoolDM::startBackgroundThreads: Starting manageConnections thread");
  //Manage Connection Thread
  m_connManageTask = new GF_TASK_T<ThinClientPoolDM>(this,
	  &ThinClientPoolDM::manageConnections, NC_MC_Thread);
  ...
  if (idle != -1) {
    ...
    LOGDEBUG("ThinClientPoolDM::startBackgroundThreads: Starting manageConnections task");
    ACE_Event_Handler* connHandler = new ExpiryHandler_T<
      ThinClientPoolDM >( this, &ThinClientPoolDM::doManageConnections );

    LOGDEBUG("ThinClientPoolDM::startBackgroundThreads: Scheduling manageConnections task");
    m_connManageTaskId = getCacheImplExpiryTaskManager( )->scheduleExpiryTask(
        connHandler, 1, idle/1000+1, false );
  }

  LOGDEBUG("ThinClientPoolDM::startBackgroundThreads: Starting remote query service");
  //Init Query Service
  m_remoteQueryServicePtr = new RemoteQueryService( m_connManager.getCacheImpl(), this );
  m_remoteQueryServicePtr->init();

  LOGDEBUG("ThinClientPoolDM::startBackgroundThreads: Starting pool stat sampler");
  if( m_PoolStatsSampler == NULL && getStatisticInterval() > -1 &&
      DistributedSystem::getSystemProperties()->statisticsEnabled()){
    m_PoolStatsSampler = new PoolStatsSampler(getStatisticInterval()/1000+1, m_connManager.getCacheImpl(), this);
...
void ThinClientPoolDM::cleanStaleConnections(volatile bool& isRunning)
{
  ...
  ACE_Time_Value _nextIdle = _idle;
  {
    ...
    while ( (conn = getNoWait()) != NULL && isRunning ) {
        if ( canItBeDeleted(conn) ) {
            ...
            ACE_Time_Value nextIdle = _idle - (ACE_OS::gettimeofday() - conn->getLastAccessed());
    ...
    int replaceCount = m_attrs->getMinConnections() - (int)savelist.size();
    ...
    for (std::vector<TcrConnection*>::const_iterator iter = replacelist.begin();
        ...
        if(replaceCount <= 0) {
            ...
            error = createPoolConnection( newConn, excludeServers, maxConnLimit,/*hasExpired(conn) ? NULL :*/ conn );
            if ( newConn ) {
                ACE_Time_Value nextIdle = _idle - (ACE_OS::gettimeofday() - newConn->getLastAccessed());
                if ( ( ACE_Time_Value( 0, 0 ) < nextIdle ) && ( nextIdle < _nextIdle ) ) {
                    _nextIdle = nextIdle;
                }
  ...
  if (m_connManageTaskId>= 0 && isRunning && getCacheImplExpiryTaskManager()->resetTask(m_connManageTaskId, static_cast<uint32_t> (_nextIdle.sec()+1))) {
    LOGERROR("Failed to reschedule connection manager");
  } else {
    LOGFINEST("Rescheduled next connection manager run after %d seconds", _nextIdle.sec()+1);
  }
...
{
  try {
    LOGFINE( "ThinClientPoolDM::manageConnections(): checking connections in pool queue %d", size( ) );
...
std::string ThinClientPoolDM::selectEndpoint( std::set< ServerLocation >& excludeServers, const TcrConnection* currentServer )
{
  if (m_attrs->m_initLocList.size()) { // query locators
    ServerLocation outEndpoint;
    std::string additionalLoc;
    LOGFINE("Asking locator for server from group [%s]", m_attrs->m_serverGrp.c_str());

    //Update Locator Request Stats
    getStats().incLoctorRequests();

    if (GF_NOERR != ((ThinClientLocatorHelper*)m_locHelper)->getEndpointForNewFwdConn( outEndpoint, additionalLoc,
        excludeServers, m_attrs->m_serverGrp, currentServer))
    {
      throw IllegalStateException("Locator query failed");
    }
    //Update Locator stats
    getStats().setLocators(((ThinClientLocatorHelper*)m_locHelper)->getCurLocatorsNum());
    getStats().incLoctorResposes();

    char epNameStr[128] = {0};
    ACE_OS::snprintf( epNameStr, 128, "%s:%d", outEndpoint.getServerName( ).c_str( ), outEndpoint.getPort( ) );
    LOGFINE("ThinClientPoolDM: Locator returned endpoint [%s]", epNameStr);
    return epNameStr;
  }
  else if (m_attrs->m_initServList.size()) { // use specified server endpoints
...
GfErrType ThinClientPoolDM::sendRequestToAllServers( const char* func, uint8_t getResult, uint32_t timeout, CacheablePtr args, ResultCollectorPtr & rs, CacheableStringPtr& exceptionPtr )
{
   GfErrType err = GF_NOERR;
  ...
  for (int i = 0; i < csArray->length(); i++)
  {
    ...
    */
    if ( m_endpoints.find(endpointStr,ep))
    {
      ...
      LOGFINE("ThinClientPoolDM::sendRequestToAllServers server not connected %s ", cs->asChar());
      //continue;
    }
    FunctionExecution* funcExe = &fePtrList[feIndex++];
    funcExe->setParameters(func, getResult, timeout, args, ep, this, &resultCollectorLock, &rs, userAttr);
  ...
  for (int i = 0; i < feIndex; i++ ) {
      ...
      err == GF_AUTHENTICATION_REQUIRED_EXCEPTION)
    {
      ...
      finalErrorReturn == GF_AUTHENTICATION_REQUIRED_EXCEPTION))//returning auth errors to client..preference over other errors..
    {
      finalErrorReturn = err;
    }
  }

  if ((uint8_t)(getResult & 2) == (uint8_t)2) {
    rs->endResults();
  }
  ...
  //Utils::updateStatOpTime(getStats().getStats(), m_poolStatType->getInstance()->getClientOpsSucceededTimeId(), sampleStartNanos);
...
const CacheableStringArrayPtr ThinClientPoolDM::getLocators()const
{
  ...
  for (int i =0; i < size; ++i) {
    ptrArr[i] = CacheableString::create(m_attrs->m_initLocList[i].c_str(),static_cast<int32_t> (m_attrs->m_initLocList[i].length()));
  }
...
const CacheableStringArrayPtr ThinClientPoolDM::getServers()
{
  ...
  if (size > 0) {
    ...
    for (int32_t i =0; i < size; ++i) {
      ptrArr[i] = CacheableString::create(m_attrs->m_initServList[i].c_str(),static_cast<int32_t> (m_attrs->m_initServList[i].length()));
    }
  }
  ...
  if (size > 0) {
    ...
    ((ThinClientLocatorHelper*)m_locHelper)->getAllServers(vec, m_attrs->m_serverGrp);
    ...
    for ( it = vec.begin() ; it < vec.end(); it++ )
    {
      ...
      ACE_OS::snprintf(buffer , 256 , "%s:%d", serLoc.getServerName().c_str( ), serLoc.getPort());
...
void ThinClientPoolDM::destroy( bool keepAlive )
{
  ...
  if ( !m_isDestroyed && ( !m_destroyPending || m_destroyPendingHADM ) ) {
    ...
    LOGDEBUG( "ThinClientPoolDM::destroy( ): closing FairQueue, pool size = %d", m_poolSize );
    close( );
    LOGDEBUG( "ThinClientPoolDM::destroy( ): after close ");


    /**********************************************************************
      ==31849==    at 0x4007D75: operator new(unsigned int) (vg_replace_malloc.c:313)
      ==31849==    by 0x423063E: gemfire::ThinClientPoolHADM::createEP(char const*) (ThinClientPoolHADM.hpp:113)
      ==31849==    by 0x4375CB7: gemfire::ThinClientPoolDM::addEP(char const*) (ThinClientPoolDM.cpp:1893)
      ==31849==    by 0x437DC29: gemfire::ThinClientPoolDM::addEP(gemfire::ServerLocation&) (ThinClientPoolDM.cpp:1880)
      ==31849==    by 0x43861D9: gemfire::ThinClientRedundancyManager::getAllEndpoints(stlp_std::vector<gemfire::TcrEndpoint*, stlp_std::allocator<gemfire::TcrEndpoint*> >&) (ThinClientRedundancyManager.cpp:912)
      ==31849==    by 0x4386D0E: gemfire::ThinClientRedundancyManager::initialize(int) (ThinClientRedundancyManager.cpp:613)
      ==31849==    by 0x4382301: gemfire::ThinClientPoolHADM::startBackgroundThreads() (ThinClientPoolHADM.cpp:37)
      ==31849==    by 0x422F11F: gemfire::PoolFactory::create(char const*) (PoolFactory.cpp:163)
      ==31849==    by 0x8066755: CacheHelper::createPooledRegion(char const*, bool, char const*, char const*, char const*, bool, bool, int, int, int, int, int, gemfire::SharedPtr<gemfire::CacheListener> const&, gemfire::ExpirationAction::Action) (in /export/pnq-gst-dev01a/users/adongre/valgrind_702/nc/ThinClient702X_maint/build-artifacts/linux/tests/cppcache/testThinClientPutAll)
      ==31849==    by 0x805AD4D: createPooledRegion(char const*, bool, char const*, char const*, char const*, bool, bool) [clone .clone.2] (in /export/pnq-gst-dev01a/users/adongre/valgrind_702/nc/ThinClient702X_maint/build-artifacts/linux/tests/cppcache/testThinClientPutAll)
      ==31849==    by 0x8066F6E: Task_StepOne_Pooled_Locator::doTask() (in /export/pnq-gst-dev01a/users/adongre/valgrind_702/nc/ThinClient702X_maint/build-artifacts/linux/tests/cppcache/testThinClientPutAll)
      ==31849==    by 0x806DB7A: dunit::TestSlave::begin() (in /export/pnq-gst-dev01a/users/adongre/valgrind_702/nc/ThinClient702X_maint/build-artifacts/linux/tests/cppcache/testThinClientPutAll)
      ==31849==
     ******************************************************************************/
    for (ACE_Map_Manager< std::string, TcrEndpoint*,ACE_Recursive_Thread_Mutex>::iterator iter = m_endpoints.begin();
   ...
   // delete m_manager; m_manager = NULL;
    m_isDestroyed = true;
    LOGDEBUG( "ThinClientPoolDM::destroy( ): after close m_isDestroyed = %d ",m_isDestroyed);
  }
  if(m_poolSize != 0) {
...
QueryServicePtr ThinClientPoolDM::getQueryService()
{
  //TODO: hitesh
  if (m_isMultiUserMode) {
    LOGERROR("Pool is in multiuser authentication mode. Get query service using RegionService.getQueryService()");
    throw UnsupportedOperationException("Pool is in multiuser authentication mode. Get QueryService() using RegionService.getQueryService()");
  }
...
void ThinClientPoolDM::sendUserCacheCloseMessage(bool keepAlive)
{
  ...
  std::map<std::string, UserConnectionAttributes*>& uca = userAttribute->getUserConnectionServers();
  ...
  for( it = uca.begin(); it != uca.end(); it++ )
  {
    ...
    {
      TcrMessage request(TcrMessage::REMOVE_USER_AUTH,keepAlive, this);
...
TcrConnection* ThinClientPoolDM::getConnectionInMultiuserMode(UserAttributesPtr userAttribute)
{
  ...
  if (uca != NULL)
  {
    ...
    LOGDEBUG("ThinClientPoolDM::getConnectionInMultiuserMode endpoint got = %s ", ep->name().c_str());
    return getFromEP(ep);
  }
  else
  {
...
int32_t ThinClientPoolDM::GetPDXIdForType(SerializablePtr pdxType)
{
  ...
  TcrMessage request(TcrMessage::GET_PDX_ID_FOR_TYPE, pdxType, this);
  ...
  {
    LOGDEBUG("ThinClientPoolDM::GetPDXTypeById: Exception = %s ", reply.getException()  );
    throw new IllegalStateException("Failed to register PdxSerializable Type");
  }

	int32_t pdxTypeId = static_cast<CacheableInt32*>( reply.getValue().ptr())->value();

	//need to broadcast this id to all other pool
	{
		const HashMapOfPools& pools = PoolManager::getAll();

		 for ( HashMapOfPools::Iterator iter = pools.begin( );
        iter != pools.end( );
        ++iter ) {
			ThinClientPoolDM* currPool = static_cast<ThinClientPoolDM*>( iter.second( ).ptr() );
...
void ThinClientPoolDM::AddPdxType(SerializablePtr pdxType, int32_t pdxTypeId)
{
  ...
  TcrMessage request(TcrMessage::ADD_PDX_TYPE, pdxType, this, pdxTypeId);
...
SerializablePtr ThinClientPoolDM::GetPDXTypeById(int32_t typeId)
{
  ...
  TcrMessage request(TcrMessage::GET_PDX_TYPE_BY_ID, typeId, this);
  ...
  {
    LOGDEBUG("ThinClientPoolDM::GetPDXTypeById: Exception = %s ", reply.getException()  );
    throw new IllegalStateException("Failed to understand PdxSerializable Type");
  }
...
int32_t ThinClientPoolDM::GetEnumValue(SerializablePtr  enumInfo)
{
  ...
  TcrMessage request(TcrMessage::GET_PDX_ID_FOR_ENUM, enumInfo, this);
  ...
  {
    LOGDEBUG("ThinClientPoolDM::GetEnumValue: Exception = %s ", reply.getException()  );
    throw new IllegalStateException("Failed to register Pdx enum Type");
  }

	int32_t enumVal = static_cast<CacheableInt32*>( reply.getValue().ptr())->value();
...
SerializablePtr ThinClientPoolDM::GetEnum(int32_t val)
{
  ...
  TcrMessage request(TcrMessage::GET_PDX_ENUM_BY_ID, val, this);
  ...
  {
    LOGDEBUG("ThinClientPoolDM::GetEnum: Exception = %s ", reply.getException()  );
    throw new IllegalStateException("Failed to understand enum Type");
  }
...
void ThinClientPoolDM::AddEnum(SerializablePtr  enumInfo, int enumVal)
{
  ...
  TcrMessage request(TcrMessage::ADD_PDX_ENUM, enumInfo, this, enumVal);
  ...
  {
    LOGDEBUG("ThinClientPoolDM::AddEnum: Exception = %s ", reply.getException()  );
    throw new IllegalStateException("Failed to register enum Type");
  }
}

GfErrType ThinClientPoolDM::sendUserCredentials(PropertiesPtr credentials, TcrConnection*& conn, bool isBGThread, bool & isServerException)
{
  ...
  err = conn->getEndpointObject()->sendRequestConnWithRetry( request, reply, conn );
  
  if(conn)
    err = handleEPError( conn->getEndpointObject(), reply, err );

  LOGDEBUG("ThinClientPoolDM::sendUserCredentials: Error after sending cred request = %d ", err);
  ...
  if (err == GF_NOERR) 
  {
    switch ( reply.getMessageType( ) )
    {
      ...
      case TcrMessage::EXCEPTION: {
        if ( err == GF_NOERR ) {
         putInQueue( conn, isBGThread ); //connFound is only relevant for Sticky conn.
        }
        //this will set error type if there is some server exception
        err = ThinClientRegion::handleServerException("ThinClientPoolDM::sendUserCredentials AuthException", reply.getException());        
...
TcrEndpoint * ThinClientPoolDM::getSingleHopServer(TcrMessage& request, int8_t& version, BucketServerLocationPtr& serverlocation
  , std::set< ServerLocation >& excludeServers) {
 ...
 if(region == NULLPTR)
  ...
  if (region != NULLPTR) {
    ...
    region, key, request.getValueRef(), request.getCallbackArgumentRef(), request.forPrimary(),serverlocation, version);
      
    if(serverlocation != NULLPTR && serverlocation->isValid())
    {
      LOGFINE("Server host and port are %s:%d", serverlocation->getServerName().c_str(), serverlocation->getPort());
...
TcrEndpoint * ThinClientPoolDM::getEndPoint(const BucketServerLocationPtr& serverLocation,
  ...
  if (serverLocation->isValid()) {
    ...
    if (excludeServer(serverLocation->getEpString(), excludeServers) ) {
      LOGFINE("ThinClientPoolDM::getEndPoint Exclude Server true for %s ", serverLocation->getEpString().c_str());
      return ep;
    }

    //ACE_TCHAR serverLocn[256];
    //ACE_OS::snprintf(serverLocn, 256, "%s:%d", serverLocation->getServerName().c_str(),
    ...
    //do for pool with endpoints. Add endpoint into m_endpoints only when we did not find it above and it is in the pool's m_initServList. 
    for(std::vector<std::string>::iterator itr = m_attrs->m_initServList.begin(); itr!=m_attrs->m_initServList.end(); ++itr)
    {
      if ( (ACE_OS::strcmp(serverLocation->getEpString().c_str(), (*itr).c_str()) == 0) )
      {
    ...
    //do only for locator
    //if servergroup is there, then verify otherwise you may reach to another group
...
GfErrType ThinClientPoolDM::sendSyncRequest(TcrMessage& request,
    TcrMessage& reply, bool attemptFailover,
    bool isBGThread) {
  int32_t type = request.getMessageType();
  
      
  if(!request.forTransaction() && m_attrs->getPRSingleHopEnabled() && (type == TcrMessage::GET_ALL_70 || type == TcrMessage::GET_ALL_WITH_CALLBACK) && m_clientMetadataService != NULL)
  {
    GfErrType error = GF_NOERR;
    RegionPtr region;
      m_connManager.getCacheImpl( )->getRegion(request.getRegionName().c_str(), region);
    HashMapT<BucketServerLocationPtr, VectorOfCacheableKeyPtr>* locationMap =
      m_clientMetadataService->getServerToFilterMap(request.getKeys(), region, request.forPrimary());

    if(locationMap == NULL)
    {
      request.InitializeGetallMsg(request.getCallbackArgument());//now initialize getall msg
      return sendSyncRequest(request, reply, attemptFailover, isBGThread, NULLPTR);
    }
    std::vector<GetAllWork*> getAllWorkers;
    ThreadPool* threadPool = TPSingleton::instance();
    ChunkedGetAllResponse* responseHandler = static_cast<ChunkedGetAllResponse*>(reply.getChunkedResultHandler());    

    for(HashMapT<BucketServerLocationPtr, VectorOfCacheableKeyPtr>::Iterator locationIter = locationMap->begin();
        locationIter != locationMap->end(); locationIter++)
    {
      ...
      GetAllWork* worker = new GetAllWork(this, region, serverLocation, keys, attemptFailover, isBGThread, responseHandler->getAddToLocalCache(), responseHandler, request.getCallbackArgument());
      threadPool->perform(worker);
      getAllWorkers.push_back(worker);
    }
    reply.setMessageType(TcrMessage::RESPONSE);

    for(std::vector<GetAllWork*>::iterator iter = getAllWorkers.begin(); iter != getAllWorkers.end(); iter++)
    {
      ...
      //ChunkedGetAllResponsePtr currentResponseHandler(currentReply->getChunkedResultHandler());
    ...
    if(type == TcrMessage::GET_ALL_70 || type == TcrMessage::GET_ALL_WITH_CALLBACK)
      request.InitializeGetallMsg(request.getCallbackArgument());//now initialize getall msg
    return sendSyncRequest(request, reply, attemptFailover, isBGThread, NULLPTR);
  }
}
...
GfErrType ThinClientPoolDM::sendSyncRequest(
  TcrMessage& request, TcrMessage& reply, bool attemptFailover, bool isBGThread, const BucketServerLocationPtr& serverLocation)
{
  LOGDEBUG("ThinClientPoolDM::sendSyncRequest: ....%d %s", request.getMessageType(), m_poolName.c_str());
  //Increment clientOps
  HostAsm::atomicAdd( m_clientOps, 1 );
  getStats().setCurClientOps(m_clientOps);
  ...
  while( retryAllEPsOnce || retry-- || (isAuthRequireExcep && isAuthRequireExcepMaxTry >=0 ) ) {
    isAuthRequireExcep = false;
    if (!firstTry)
      request.updateHeaderForRetry();
    ...
    if (!this->m_isMultiUserMode || (!TcrMessage::isUserInitiativeOps(request)) ) {
      conn = getConnectionFromQueueW( &queueErr, excludeServers, isBGThread, request, version, singleHopConnFound,connFound, serverLocation);
    }
    else {
      userAttr = TSSUserAttributesWrapper::s_gemfireTSSUserAttributes->getUserAttributes();
      if(userAttr == NULLPTR)
      {
        LOGWARN("Attempted operation type %d without credentials", request.getMessageType());
        return GF_NOT_AUTHORIZED_EXCEPTION;
      }
      //Can i assume here that we will always get connection here
      conn = getConnectionFromQueueW( &queueErr, excludeServers, isBGThread, request, version, singleHopConnFound,connFound, serverLocation);
      
      LOGDEBUG("ThinClientPoolDM::sendSyncRequest: after getConnectionInMultiuserMode %d",isUserNeedToReAuthenticate);
      if (conn != NULL)
      {   //need to chk whether user is already authenticated to this endpoint or not.
          isUserNeedToReAuthenticate = !(userAttr->isEndpointAuthenticated(conn->getEndpointObject()));
      }
    }
    ...
    else if (queueErr == GF_CLIENT_WAIT_TIMEOUT_REFRESH_PRMETADATA)
    {
      ...
      if (region != NULLPTR)
      { 
        LOGFINE("Need to refresh pr-meta-data timeout in client only  with refresh metadata");
        ThinClientRegion* tcrRegion = dynamic_cast<ThinClientRegion*> (region.ptr());        
        tcrRegion->setMetaDataRefreshed(false);
        m_clientMetadataService->enqueueForMetadataRefresh(region->getFullPath(), reply.getserverGroupVersion());
      }
      return GF_CLIENT_WAIT_TIMEOUT_REFRESH_PRMETADATA;
    }

    LOGDEBUG("ThinClientPoolDM::sendSyncRequest: isUserNeedToReAuthenticate = %d ", isUserNeedToReAuthenticate);
    LOGDEBUG("ThinClientPoolDM::sendSyncRequest: m_isMultiUserMode = %d  conn = %d  type = %d", m_isMultiUserMode, conn, type);
    ...
    if( conn ) {
      ...
      LOGDEBUG("ThinClientPoolDM::sendSyncRequest: sendSyncReq ep->isAuthenticated() = %d ", ep->isAuthenticated());
      GfErrType userCredMsgErr = GF_NOERR;
      bool isServerException = false;
      if (TcrMessage::isUserInitiativeOps(request) && (this->m_isSecurityOn||this->m_isMultiUserMode)) {
        if ( !this->m_isMultiUserMode && !ep->isAuthenticated()) {
          //first authenticate him on this endpoint
          userCredMsgErr = this->sendUserCredentials(this->getCredentials(ep), conn, isBGThread, isServerException);
        } else if (isUserNeedToReAuthenticate) {
          userCredMsgErr = this->sendUserCredentials(userAttr->getCredentials(), conn, isBGThread, isServerException);
        }
      }
      ...
      if(!isServerException) {
        ...
        //  afterSendingRequest(request.getMessageType( ),reply, conn);
          ...
          putInQueue( conn, isBGThread || request.getMessageType() == TcrMessage::GET_ALL_70 || request.getMessageType() == TcrMessage::GET_ALL_WITH_CALLBACK || request.getMessageType() == TcrMessage::EXECUTE_REGION_FUNCTION_SINGLE_HOP, request.forTransaction() ); //connFound is only relevant for Sticky conn.
          LOGDEBUG("putting connection back in queue DONE");
          //LOGFINE("putting connection back in queue");
        }else {
          if( error != GF_TIMOUT)
            removeEPConnections( ep );
          // Update stats for the connection that failed.
          removeEPConnections( 1, false );          
          setStickyNull( isBGThread || request.getMessageType() == TcrMessage::GET_ALL_70 || request.getMessageType() == TcrMessage::GET_ALL_WITH_CALLBACK || request.getMessageType() == TcrMessage::EXECUTE_REGION_FUNCTION_SINGLE_HOP);
          if(conn) {
        	 GF_SAFE_DELETE_CON(conn)
          }
        ...
        return error;//server exception while sending credentail message to server...
    }
    ...
    if ( error == GF_NOERR ) {
      if((this->m_isSecurityOn||this->m_isMultiUserMode))
      {
        ...
        {          
          if (isAuthRequireException(reply.getException()))
          {
            ...
            LOGFINEST("After getting AuthenticationRequiredException trying again.");
      ...
      LOGFINER("reply Metadata version is %d & bsl version is %d reply.isFEAnotherHop()=%d", reply.getMetaDataVersion(), version, reply.isFEAnotherHop());
     // LOGINFO("reply Metadata version is %d & bsl version is %d reply.isFEAnotherHop()=%d connFound= %d", reply.getMetaDataVersion(), version, reply.isFEAnotherHop(), connFound);
      /*if(reply.getMetaDataVersion() > 0 ) {
        LOGCONFIG("reply Metadata version is %d & bsl version is %d", reply.getMetaDataVersion(), version);
      }*/
      if (m_clientMetadataService != NULL && request.forSingleHop() && 
            (reply.getMetaDataVersion() != 0 || 
            (request.getMessageType() == TcrMessage::EXECUTE_REGION_FUNCTION && request.getKeyRef() != NULLPTR && reply.isFEAnotherHop())
            ))             
      {
        ...
        if (region != NULLPTR)
        { 
          if(!connFound)//max limit case then don't refresh otherwise always refresh
          {
    ...
    if ( !attemptFailover || error == GF_NOERR ) {
      ...
      /*Update the time stat for clientOpsTime */
      //Utils::updateStatOpTime(m_stats->getStats(), m_poolStatType->getInstance()->getClientOpsSucceededTimeId(), sampleStartNanos);
      return error;
    }
...
void ThinClientPoolDM::removeEPConnections( int numConn, bool triggerManageConn )
{
  //TODO: Delete EP
...
// to create one. If it fails to create one,  it gets connection to any other server
// and fails over the transaction to that server. 
// This function is used when the transaction is to be resumed to a specified 
// server. 
GfErrType ThinClientPoolDM::getConnectionToAnEndPoint(std::string epNameStr, TcrConnection*& conn)
{
  conn = NULL;
  ...
  LOGFINE("ThinClientPoolDM::getConnectionToAnEndPoint( ): Getting endpoint object for %s", epNameStr.c_str());
  if (theEP != NULL && theEP->connected()) {
    LOGFINE("ThinClientPoolDM::getConnectionToAnEndPoint( ): Getting connection for endpoint %s", epNameStr.c_str());
    ...
    bool maxConnLimit =false;
    if (conn == NULL) {
      LOGFINE("ThinClientPoolDM::getConnectionToAnEndPoint( ): Create connection for endpoint %s", epNameStr.c_str());
  ...
  if (conn == NULL) {
      std::set< ServerLocation > excludeServers;
      bool maxConnLimit = false;
      LOGFINE("ThinClientPoolDM::getConnectionToAnEndPoint( ): No connection available for endpoint %s. Create connection to any endpoint.", epNameStr.c_str());
      conn = getConnectionFromQueue( true, &error, excludeServers,maxConnLimit );
      /* adongre
       * CID 28680: Dereference after null check (FORWARD_NULL)Comparing "conn" to null implies that "conn" might be null. 
       * Passing null variable "conn" to function "gemfire::TcrConnection::getEndpointObject() const", which dereferences it. 
       */
      //if(conn != NULL || error == GF_NOERR)
      if(conn != NULL && error == GF_NOERR)
      {
        if (conn->getEndpointObject()->name() != epNameStr) {
          LOGFINE("ThinClientPoolDM::getConnectionToAnEndPoint( ): Endpoint %s different than the endpoint %s. New connection created and failing over.", epNameStr.c_str(),conn->getEndpointObject()->name().c_str());
          GfErrType failoverErr = doFailover(conn);
          if (failoverErr != GF_NOERR){
            LOGFINE("ThinClientPoolDM::getConnectionToAnEndPoint( ):Failed to failover transaction to another server. From endpoint %s to %s", epNameStr.c_str(), conn->getEndpointObject()->name().c_str());
  ...
  if(conn == NULL || error != GF_NOERR) 
  {
    LOGFINE("ThinClientPoolDM::getConnectionToAEndPoint( ):Failed to connect to %s", theEP->name().c_str());
    if(conn != NULL) 
      GF_SAFE_DELETE( conn );
  }
...
GfErrType ThinClientPoolDM::createPoolConnectionToAEndPoint(TcrConnection*& conn, TcrEndpoint* theEP, bool &maxConnLimit, bool appThreadrequest)
{
  ...
  int min = 0;
  {
    ...
    if(m_poolSize >= max) {
      maxConnLimit = true;
      LOGFINER( "ThinClientPoolDM::createPoolConnectionToAEndPoint( ): current pool size has reached limit %d, %d", m_poolSize, max );
      return error;
    }
  }
  
  LOGFINE("ThinClientPoolDM::createPoolConnectionToAEndPoint( ): creating a new connection to the endpoint %s", theEP->name().c_str());
 // if the pool size is within limits, create a new connection. 
  error = theEP->createNewConnection( conn, false, false,
    DistributedSystem::getSystemProperties()->connectTimeout(), false, true, appThreadrequest );
  if(conn == NULL || error != GF_NOERR) 
  {
...
void ThinClientPoolDM::reducePoolSize(int num) 
{
  ...
  if(m_poolSize == 0) {
    if(m_cliCallbackTask != NULL)
      m_cliCallbackSema.release();
  }
}
GfErrType ThinClientPoolDM::createPoolConnection( TcrConnection*& conn, std::set< ServerLocation >& excludeServers, bool& maxConnLimit, const TcrConnection* currentserver)
//GfErrType ThinClientPoolDM::createPoolConnection( TcrConnection*& conn, std::set< ServerLocation >& excludeServers, const TcrConnection* currentserver)
{
  ACE_Guard< ACE_Recursive_Thread_Mutex > _guard( m_queueLock );
  GfErrType error = GF_NOERR;
  int max = m_attrs->getMaxConnections();
  ...
  int min = m_attrs->getMinConnections();
  max = max > min ? max : min;
  LOGDEBUG( "ThinClientPoolDM::createPoolConnection( ): current pool size has reached limit %d, %d, %d", m_poolSize, max, min );
  ...
  conn = NULL;
  {
    //ACE_Guard< ACE_Recursive_Thread_Mutex > _guard( m_queueLock );    
    if(m_poolSize >= max) {
      LOGDEBUG( "ThinClientPoolDM::createPoolConnection( ): current pool size has reached limit %d, %d", m_poolSize, max );
      maxConnLimit = true;
      return error;
    }    
    //m_poolSize++;
  ...
  //LOGINFO("serverlist: %d, locList: %d", m_attrs->m_initServList.size(), m_attrs->m_initLocList.size());
  ...
  while( true )
  {
    ...
    if(currentserver != NULL && epNameStr == currentserver->getEndpointObject()->name()) 
    {
    ...
    if(conn == NULL || error != GF_NOERR) {
      ...
      if ( ThinClientBaseDM::isFatalClientError( error ) ) {
        // ARB: log the error string instead of error number.
...
TcrConnection* ThinClientPoolDM::getConnectionFromQueue( bool timeout, GfErrType* error, std::set< ServerLocation >& excludeServers, bool& maxConnLimit )
{
  int64_t timeoutTime = m_attrs->getFreeConnectionTimeout() / 1000; // in millisec so divide by 1000
  ...
  int64 sampleStartNanos =Utils::startStatOpTime();
  TcrConnection* mp = getUntil( timeoutTime, error, excludeServers, maxConnLimit );
  /*Update the time stat for clientOpsTime */
  Utils::updateStatOpTime(getStats().getStats(), PoolStatType::getInstance()->getTotalWaitingConnTimeId(), sampleStartNanos);
  return mp;
}

bool ThinClientPoolDM::isEndpointAttached(TcrEndpoint * ep)
{
  return true;
}

GfErrType ThinClientPoolDM::sendRequestToEP( const TcrMessage &request, TcrMessage & reply, TcrEndpoint* currentEndpoint )
{
  ...
  { 
    ...
    if ( conn == NULL ) {
        LOGDEBUG( "ThinClientPoolDM::sendRequestToEP(): got NULL connection from pool, creating new connection in the pool." );
        bool maxConnLimit = false;
        error = createPoolConnectionToAEndPoint(conn, currentEndpoint, maxConnLimit);
      if (conn == NULL || error != GF_NOERR) {
        LOGDEBUG( "ThinClientPoolDM::sendRequestToEP(): couldnt create a pool connection, creating a temporary connection." );
        error = currentEndpoint->createNewConnection( conn, false, false,
          DistributedSystem::getSystemProperties()->connectTimeout(), false );
        putConnInPool = false;
   ...
   // reply.setTimeout( this->getReadTimeout()/1000 );
    ...
    LOGDEBUG("ThinClientPoolDM::sendRequestToEP: this->m_isMultiUserMode = %d", this->m_isMultiUserMode);
    bool isServerException = false;
    //if( !(TcrMessage::USER_CREDENTIAL_MESSAGE == request.getMessageType())){
      if (TcrMessage::isUserInitiativeOps( (request)) && (this->m_isSecurityOn||this->m_isMultiUserMode)) {
        if ( !this->m_isMultiUserMode && !currentEndpoint->isAuthenticated()) {
          //first authenticate him on this endpoint
          error = this->sendUserCredentials(this->getCredentials(currentEndpoint), conn, false, isServerException);
          ...
          if(ua == NULLPTR)
          {
           ...
           // GfErrTypeToException("Found operation without credential", GF_NOT_AUTHORIZED_EXCEPTION);
            return GF_NOT_AUTHORIZED_EXCEPTION;
          }
          else
          {
            UserConnectionAttributes* uca = ua->getConnectionAttribute(currentEndpoint);

            if (uca == NULL)
              error = this->sendUserCredentials(ua->getCredentials(), conn, false, isServerException);
          }
        }
      }
    //}
    ...
    if(error == GF_NOERR && conn != NULL) {
      error = currentEndpoint->sendRequestConnWithRetry( request, reply, conn, true );
    }
    ...
    if ( error == GF_NOERR ) {
      ...
      if (replyMsgType == TcrMessage::EXCEPTION || 
          ...
          replyMsgType == TcrMessage::CQDATAERROR_MSG_TYPE) { 
            error = ThinClientRegion::handleServerException("ThinClientPoolDM::sendRequestToEP", 
            reply.getException()); 
      } 
    ...
    if ( error == GF_NOERR || error == GF_CACHESERVER_EXCEPTION || error == GF_AUTHENTICATION_REQUIRED_EXCEPTION) {
      if((this->m_isSecurityOn||this->m_isMultiUserMode))
      {
        if (reply.getMessageType( ) == TcrMessage::EXCEPTION)
        {          
...
TcrEndpoint* ThinClientPoolDM::addEP( const char* endpointName )
{
  ...
  if ( m_endpoints.find( fullName, ep ) ) {
    LOGFINE( "Created new endpoint %s for pool %s", fullName.c_str(), m_poolName.c_str());
    ep = createEP( fullName.c_str() );
    if ( m_endpoints.bind( fullName, ep ) ) {
      LOGERROR( "Failed to add endpoint %s to pool %s", fullName.c_str(), m_poolName.c_str() );
      GF_DEV_ASSERT( "ThinClientPoolDM::addEP( ): failed to add endpoint" ? false : false );
    }    
  }  
  //Update Server Stats
...
void ThinClientPoolDM::pingServerLocal()
{
 ACE_Guard<ACE_Recursive_Thread_Mutex> _guard( getPoolLock( ) );
 ACE_Guard< ACE_Recursive_Thread_Mutex > guard( m_endpointsLock );
  for( ACE_Map_Manager< std::string, TcrEndpoint *, ACE_Recursive_Thread_Mutex >::iterator it = m_endpoints.begin(); it != m_endpoints.end(); it++){
...
bool ThinClientPoolDM::canItBeDeleted(TcrConnection* conn)
{
  ...
  if ( conn && candidateForDeletion ) {
    ...
    bool queue = false;
    {
      ...
      ACE_Guard< ACE_Recursive_Thread_Mutex > guardQueue( endPt->getQueueHostedMutex( ) );
...
bool ThinClientPoolDM::excludeServer( std::string endpoint, std::set< ServerLocation >& excludeServers )
{
  if ( excludeServers.size() == 0 || excludeServers.find( ServerLocation( endpoint ) ) == excludeServers.end( ) ) {
    return false;
  }
  else {
    return true;
...
bool ThinClientPoolDM::excludeConnection( TcrConnection* conn, std::set< ServerLocation >& excludeServers )
{
...
{
  ACE_Guard< ACE_Recursive_Thread_Mutex > _guard( m_queueLock );
  for ( std::deque< TcrConnection* >::iterator itr = m_queue.begin( ); itr != m_queue.end( ); itr++ ) {
...
TcrConnection* ThinClientPoolDM::getNoGetLock( bool& isClosed, GfErrType* error, std::set< ServerLocation >& excludeServers, bool& maxConnLimit )
{
...
bool ThinClientPoolDM::exclude( TcrConnection* conn, std::set< ServerLocation >& excludeServers )
{
...
void ThinClientPoolDM::checkRegions( )
{
  ...
  if ( m_numRegions > 0 ) {
    throw IllegalStateException( "Failed to destroy pool because regions are connected with it." );
  }

  m_destroyPending = true;
}
void ThinClientPoolDM::updateNotificationStats(bool isDeltaSuccess, long timeInNanoSecond)
{
  if( isDeltaSuccess ) {
...
GfErrType ThinClientPoolDM::doFailover(TcrConnection* conn)
{
	...
	TcrMessage request(TcrMessage::TX_FAILOVER);
	TcrMessage reply;
	...
	if(err == GF_NOERR) {
		...
		case TcrMessage::EXCEPTION: {
			...
			err = ThinClientRegion::handleServerException("CacheTransactionManager::failover",
					exceptionMsg);
			break;
		}
		...
		default: {
			LOGERROR("Unknown message type in failover reply %d", reply.getMessageType());
			err = GF_MSG;
			break;
		}
		}
