...
class TheTypeMap : private NonCopyable, private NonAssignable
{
  ...
  inline void setup()
  {
	...
	rebind2(GemfireTypeIdsImpl::DiskVersionTag, DiskVersionTag::createDeserializable);
  ...
  inline void bind( TypeFactoryMethod func )
  {
    ...
    SpinLockGuard guard( m_mapLock );
    int64_t compId = (int64_t)obj->typeId( );
    if ( compId == GemfireTypeIdsImpl::CacheableUserData ||
      ...
      compId |= ( ( (int64_t)obj->classId( ) ) << 32 );
    }
    delete obj;
    int bindRes = m_map->bind( compId, func );
    if ( bindRes == 1 ) {
      LOGERROR( "A class with "
          "ID %d is already registered.", compId );
      throw IllegalStateException( "A class with "
          "given ID is already registered." );
    } else if ( bindRes == -1 ) {
      LOGERROR( "Unknown error "
          "while adding class ID %d to map.", compId );
  ...
  inline void bind2( TypeFactoryMethod func )
  {
    ...
    if(dsfid == GemfireTypeIdsImpl::FixedIDShort)
      compId = compId = (int64_t)obj->classId();
    else
      compId = (int64_t)obj->typeId( );
    delete obj;
    int bindRes = m_map2->bind( compId, func );
    if ( bindRes == 1 ) {
      LOGERROR( "A fixed class with "
          "ID %d is already registered.", compId );
      throw IllegalStateException( "A fixed class with "
          "given ID is already registered." );
    } else if ( bindRes == -1 ) {
      LOGERROR( "Unknown error "
          "while adding class ID %d to map2.", compId );
      throw IllegalStateException( "Unknown error "
  ...
  inline void bindPdxType( TypeFactoryMethodPdx func ) {
    ...
    delete obj;
		...
		if (bindRes == 1) {
			...
			throw IllegalStateException("A Object with "
					"given FullName is already registered.");
		} else if (bindRes == -1) {
			LOGERROR("Unknown error "
						"while adding Pdx Object named %s to map.", objFullName);
			throw IllegalStateException("Unknown error "
					"while adding type to map.");
		}
	}
	...
	inline void rebindPdxType(char *objFullName, TypeFactoryMethodPdx func) {
		...
		if (bindRes == -1) {
			LOGERROR("Unknown error "
						"while adding Pdx Object FullName %s to map.", objFullName);
			throw IllegalStateException("Unknown error "
					"while adding type to map.");
		}
	}
...
SerializablePtr SerializationRegistry::deserialize( DataInput& input, int8_t typeId  )
{
  bool findinternal = false;
  int8_t currentTypeId = typeId;
  
  if( typeId == -1)
    input.read( &currentTypeId );
  int64_t compId = currentTypeId;

  LOGDEBUG("SerializationRegistry::deserialize typeid = %d currentTypeId= %d ", typeId, currentTypeId);
  ...
  if ( createType == NULL ) {
    if(findinternal)
    {
      LOGERROR("Unregistered class ID %d during deserialization: Did the application register "
        "serialization types?", compId);
    }
    else
    {
      LOGERROR("Unregistered class ID %d during deserialization: Did the application register "
        "serialization types?", (compId >> 32));
    }
...
PdxSerializablePtr SerializationRegistry::getPdxType(char* className){
  ...
  if (objectType == NULL) {
    try {
      ...
      LOGERROR("Unregistered class %s during PDX deserialization: Did the "
        "application register the PDX type or serializer?", className);
      throw IllegalStateException( "Unregistered class or serializer in PDX deserialization" );
    }
  } else {
	  pdxObj = objectType();
  }
...
int32_t SerializationRegistry::GetPDXIdForType(const char* poolName, SerializablePtr pdxType)
{
...
SerializablePtr SerializationRegistry::GetPDXTypeById(const char* poolName, int32_t typeId)
{
  PoolPtr pool = NULLPTR;
	...
	if(poolName == NULL)
	{
		 ...
		 {
       /* adongre
        * CID 28994: Structurally dead code (UNREACHABLE)
        */
			  /*for ( HashMapOfPools::Iterator iter = pools.begin( );
					iter != pools.end( );
					++iter ) 
