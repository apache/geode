...
namespace gemfire {
  ...
  const char* PdxType::m_javaPdxClass =  "com.gemstone.gemfire.pdx.internal.PdxType";
  ...
  PdxType::PdxType(const char *pdxDomainClassName,
  ...
  void PdxType::toData(DataOutput& output) const {			
    output.write((int8_t)GemfireTypeIdsImpl::DataSerializable); //45
    output.write((int8_t)GemfireTypeIdsImpl::Class); //43
    output.write((int8_t)GemfireTypeIds::CacheableASCIIString);
    ...
    output.writeArrayLen((int32_t)m_pdxFieldTypes->size());        

    for(std::vector<PdxFieldTypePtr>::iterator it = m_pdxFieldTypes->begin(); it != m_pdxFieldTypes->end(); ++it) {
  ...
  void PdxType::addFixedLengthTypeField(const char* fieldName, const char*  className, int8_t typeId, int32 size) {
    if (fieldName == NULL /*|| *fieldName == '\0'*/ || m_fieldNameVsPdxType.find(fieldName) != m_fieldNameVsPdxType.end()) { // COVERITY ---> 30289 Same on both sides
      char excpStr[256] = {0};
      /* adongre
       * Coverity - II
      * CID 29269: Calling risky function (SECURE_CODING)[VERY RISKY]. Using "sprintf" can cause a 
      ...
      */
      ACE_OS::snprintf(excpStr, 256, "Field: %s is either already added into PdxWriter or it is null ", fieldName);
      throw IllegalStateException(excpStr);
    }
    PdxFieldTypePtr pfxPtr (new PdxFieldType (fieldName, className, typeId, (int32)m_pdxFieldTypes->size() , false , size, 0));
    m_pdxFieldTypes->push_back(pfxPtr);
    m_fieldNameVsPdxType[fieldName] = pfxPtr;
  }

  void PdxType::addVariableLengthTypeField(const char* fieldName, const char* className, int8_t typeId) {
    if (fieldName == NULL /*|| *fieldName == '\0'*/ || m_fieldNameVsPdxType.find(fieldName) != m_fieldNameVsPdxType.end()) { // COVERITY ---> 30289 Same on both sides
      char excpStr[256] = {0};
      ACE_OS::snprintf(excpStr, 256, "Field: %s is either already added into PdxWriter or it is null ", fieldName);
      throw IllegalStateException(excpStr);
    ...
    PdxFieldTypePtr pfxPtr (new PdxFieldType (fieldName, className, typeId, (int32)m_pdxFieldTypes->size() , true , -1, m_varLenFieldIdx));
    m_pdxFieldTypes->push_back(pfxPtr);
    m_fieldNameVsPdxType[fieldName] = pfxPtr;
  }

  void PdxType::initRemoteToLocal() {
    //This method is to check if we have some extra fields in remote PdxType that are absent in local PdxType.
    ...
    //5. If found update in m_remoteToLocalFieldMap that field there in remote type. else update m_numberOfFieldsExtra
    //6. else update in m_remoteToLocalFieldMap that local type don't have this fields
    ...
    {
      std::vector<PdxFieldTypePtr>* localPdxFields = localPdxType->getPdxFieldTypes();
      int32 fieldIdx = 0;
      
      m_remoteToLocalFieldMap = new int32_t[m_pdxFieldTypes->size()];
      LOGDEBUG("PdxType::initRemoteToLocal m_pdxFieldTypes->size() =%d AND localPdxFields->size()=%d", m_pdxFieldTypes->size(), localPdxFields->size());
      for(std::vector<PdxFieldTypePtr>::iterator remotePdxField = m_pdxFieldTypes->begin(); remotePdxField != m_pdxFieldTypes->end(); ++remotePdxField)
      {
        bool found  = false;
        
        for(std::vector<PdxFieldTypePtr>::iterator localPdxfield = localPdxFields->begin(); localPdxfield != localPdxFields->end(); ++localPdxfield)
        {
  ...
  void PdxType::initLocalToRemote() {
    ...
    //1. Get local PdxType from type registry
    //2. Get list of PdxFieldTypes from it
    //3. Iterate over local PdxFields to compare it with remote PdxFields and if they r equal that means there sequence is same mark it with -2
    //4. Iterate over local PdxFields and remote PdxFields and if they are equal then mark m_localToRemoteFieldMap with remotePdxField sequenceId
    ...
    {          
      ...
      int32_t localToRemoteFieldMapSize = (int32)localPdxType->m_pdxFieldTypes->size();
      m_localToRemoteFieldMap = new int32_t[localToRemoteFieldMapSize];

      for(int32_t i = 0; i < (int32_t)localToRemoteFieldMapSize && i < (int32_t)m_pdxFieldTypes->size(); i++)
      {
      ...
      {
        PdxFieldTypePtr localPdxField  = localPdxType->m_pdxFieldTypes->at(fieldIdx);
        bool found  = false;
  ...
  int32_t PdxType::getFieldPosition(const char* fieldName, uint8_t* offsetPosition, int32_t offsetSize, int32_t pdxStreamlen) {
  ...
  int32_t PdxType::getFieldPosition(int32_t fieldIdx, uint8_t* offsetPosition, int32_t offsetSize, int32_t pdxStreamlen) {
  ...
  int32_t PdxType::fixedLengthFieldPosition(PdxFieldTypePtr fixLenField, uint8_t* offsetPosition, int32_t offsetSize, int32_t pdxStreamlen) {
    ...
    else
    {
      ...
      //TODO      
      return PdxHelper::readInt(offsetPosition + (m_numberOfVarLenFields - offset -1)*offsetSize, offsetSize) + fixLenField->getRelativeOffset() ;      
    }
  }

  int32_t PdxType::variableLengthFieldPosition(PdxFieldTypePtr varLenField, uint8_t* offsetPosition, int32_t offsetSize, int32_t pdxStreamlen) {
  ...
  PdxTypePtr PdxType::isContains(PdxTypePtr first, PdxTypePtr second) {
	int j = 0;
    for(int i =0; i< (int)second->m_pdxFieldTypes->size(); i++)
    {   
      PdxFieldTypePtr secondPdt = second->m_pdxFieldTypes->at(i);
      bool matched = false;
      for(; j< (int)first->m_pdxFieldTypes->size(); j++)
      {
  ...
  PdxTypePtr PdxType::mergeVersion(PdxTypePtr otherVersion)
  {
    ...
    for(std::vector<PdxFieldTypePtr>::iterator it = otherVersion->m_pdxFieldTypes->begin(); it != otherVersion->m_pdxFieldTypes->end(); ++it)
    {
      bool found = false;
      //for each(PdxFieldType^ tmpNew in newone->m_pdxFieldTypes)
      for(std::vector<PdxFieldTypePtr>::iterator it2 = newone->m_pdxFieldTypes->begin(); it2 != newone->m_pdxFieldTypes->end(); ++it2)
      {
        if((*it2)->equals(*it))
        {
          found = true;
      ...
      if(!found)
      {
        PdxFieldTypePtr newFt (new PdxFieldType((*it)->getFieldName(),
  	    		                                (*it)->getClassName(),
  	    		                                (*it)->getTypeId(),
  	    		                                (int32)newone->m_pdxFieldTypes->size(),//sequence id
  	                                            (*it)->IsVariableLengthType(),
  	                                            (*it)->getFixedSize(),
  	                                            ((*it)->IsVariableLengthType()? varLenFields++/*it increase after that*/: 0)));
        newone->m_pdxFieldTypes->push_back(newFt);//fieldnameVsPFT will happen after that
      }
    }

     newone->setNumberOfVarLenFields(varLenFields);
     if(varLenFields >0)
       newone->setVarLenFieldIdx(varLenFields);
  ...
  void PdxType::generatePositionMap() {
    ...
    for (int i = (int)m_pdxFieldTypes->size() - 1; i >= 0; i--) {
      ...
      if (tmpft->IsVariableLengthType()) {
        ...
        if (foundVarLen) {
          ...
          tmpft->setRelativeOffset(-tmpft->getFixedSize() + previousField->getRelativeOffset());
        } else {
  ...
  bool PdxType::Equals(PdxTypePtr otherObj)
  {
    ...
    if(ot->m_pdxFieldTypes->size() != m_pdxFieldTypes->size())
      return false;
