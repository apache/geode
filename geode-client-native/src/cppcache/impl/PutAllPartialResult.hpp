...
namespace gemfire
{
  ...
  class PutAllPartialResult : public Serializable
  {
    ...
    public:
      PutAllPartialResult(int totalMapSize, ACE_Recursive_Thread_Mutex& responseLock);
      ...
      bool hasFailure() {
        return m_firstFailedKey != NULLPTR;
      }
      ...
      virtual CacheableStringPtr toString() const {
        ...
        if (m_firstFailedKey!=NULLPTR) {
          ACE_OS::snprintf(msgStr1,1024,"[ Key =%s ]", m_firstFailedKey->toString()->asChar());
        }
        ...
        if (m_totalMapSize > 0) {
          ...
          if (failedKeyNum>0) {
            ACE_OS::snprintf(msgStr2, 1024,"The putAll operation failed to put %d out of %d entries ", failedKeyNum, m_totalMapSize);
          } else {
       	    ACE_OS::snprintf(msgStr2, 1024,"The putAll operation successfully put %d out of %d entries ", m_succeededKeys->size(), m_totalMapSize);
          }
        }

        char stringBuf[7000];
        ACE_OS::snprintf(stringBuf, 7000, "PutAllPartialResult: %s%s",msgStr1, msgStr2);
        return CacheableString::create( stringBuf );
      }

      void toData( DataOutput& output ) const {
        throw IllegalStateException("PutAllPartialResult::toData is not intended for use.");
      }

      Serializable* fromData( DataInput& input ) {
        throw IllegalStateException("PutAllPartialResult::fromData is not intended for use.");
        return NULL;
      }

      int32_t classId( ) const {
        throw IllegalStateException("PutAllPartialResult::classId is not intended for use.");
        return 0;
      }

      uint32_t objectSize( ) const {
        throw IllegalStateException("PutAllPartialResult::objectSize is not intended for use.");
        return 0;
      }
