...
public:
	...
	PdxLocalWriter(DataOutput& output, PdxTypePtr pdxType, const char* pdxDomainType);
  ...
  inline void writeObject(bool value){
  ...
  inline void writeObject(int8_t value){
    m_dataOutput->write(value);
  }

  inline void writeObject(int16_t value){
    m_dataOutput->writeInt(value);
  }

  inline void writeObject(int32_t value){
    m_dataOutput->writeInt(value);
  }

  inline void writeObject(int64_t value){
    m_dataOutput->writeInt(value);
  }

  inline void writeObject(float value){
    m_dataOutput->writeFloat(value);
  }

  inline void writeObject(double value){
  ...
  virtual PdxWriterPtr writeWideString(const char* fieldName, const wchar_t* value);
  /**
   *Write a object to the PdxWriter.
   *@param fieldName The name of the field associated with the value.
   *@param value The object to write
   */
  virtual PdxWriterPtr writeObject(const char* fieldName, SerializablePtr value);
   ...
   */
  virtual PdxWriterPtr writeBooleanArray(const char* fieldName, bool* array, int length);
   ...
   */
  virtual PdxWriterPtr writeCharArray(const char* fieldName, char* array, int length);

  virtual PdxWriterPtr writeWideCharArray(const char* fieldName, wchar_t* array, int length);
  /**
   *Write a Byte array to the PdxWriter.
   *@param fieldName The name of the field associated with the value.
   *@param value The byte array value to write
   */
  virtual PdxWriterPtr writeByteArray(const char* fieldName, int8_t* array, int length);
   ...
   */
  virtual PdxWriterPtr writeShortArray(const char* fieldName, int16_t* array, int length);
   ...
   */
  virtual PdxWriterPtr writeIntArray(const char* fieldName, int32_t* array, int length);
   ...
   */
  virtual PdxWriterPtr writeLongArray(const char* fieldName, int64_t* array, int length);
   ...
   */
  virtual PdxWriterPtr writeFloatArray(const char* fieldName, float* array, int length);
   ...
   */
  virtual PdxWriterPtr writeDoubleArray(const char* fieldName, double* array, int length);
   ...
   */
  virtual PdxWriterPtr writeStringArray(const char* fieldName, char** array, int length);

  virtual PdxWriterPtr writeWideStringArray(const char* fieldName, wchar_t** array, int length);
  /**
   *Write a object array to the PdxWriter.
   *@param fieldName The name of the field associated with the value.
   *@param value The object array value to write
   */
  virtual PdxWriterPtr writeObjectArray(const char* fieldName, CacheableObjectArrayPtr array);
   ...
   */
  virtual PdxWriterPtr writeArrayOfByteArrays(const char* fieldName, int8_t** array, int arrayLength, int* elementLength );

  /**
   *Indicate that the given field name should be included in hashCode and equals checks
   *of this object on a server that is using {@link CacheFactory#setPdxReadSerialized(boolean)} or when a client executes a query on a server.
   ...
   *
   *If no fields are set as identity fields, then all fields will be used in hashCode and equal checks.
   *
   *The identity fields should make marked after they are written using a write* method.
   *
   *@param fieldName The name of the field that should be used in the as part of the identity.
  ...
  inline int32_t getStartPositionOffset()
  {
