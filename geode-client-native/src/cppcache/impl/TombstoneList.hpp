...
{
public:
  TombstoneList(MapSegment* mapSegment) 
  {
    m_mapSegment = mapSegment;
  }
  virtual ~TombstoneList() { cleanUp(); }
  void add(RegionInternal* rptr, MapEntryImpl* entry, TombstoneExpiryHandler* handler, long taskID);
  ...
  void eraseEntryFromTombstoneList(CacheableKeyPtr key, RegionInternal* region, bool cancelTask = true);
  long eraseEntryFromTombstoneListWithoutCancelTask(CacheableKeyPtr key, RegionInternal* region, TombstoneExpiryHandler*& handler);
