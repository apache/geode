...
void TcpConn::clearNagle( ACE_SOCKET sock ) {
  ...
  if ( 0 != setsockopt( sock, IPPROTO_TCP, 1, param, plen ) ) {
    ...
    LOGERROR( "Failed to set TCP_NODELAY on socket. Errno: %d: %s", lastError, ACE_OS::strerror(lastError) );
  }
}
...
int32_t TcpConn::maxSize( ACE_SOCKET sock, int32_t flag, int32_t size ) {
  ...
  while ( lastRed != red ) {
    ...
    if ( 0 != setsockopt( sock, SOL_SOCKET, flag, cparam, clen ) ) {
      int32_t lastError = ACE_OS::last_error( );
      LOGERROR( "Failed to set socket options. Errno: %d : %s ", lastError, ACE_OS::strerror(lastError));
    }
    if ( 0 != getsockopt( sock, SOL_SOCKET, flag, param, &plen ) ) {
      int32_t lastError = ACE_OS::last_error( );
      LOGERROR( "Failed to get buffer size for flag %d on socket. Errno: %d : %s",
...
void TcpConn::init() {
   ...
   * This less-than-zero comparison of an unsigned value is never true. "sock < 0U".
   */
  ACE_SOCKET sock = socket( AF_INET, SOCK_STREAM, 0 );
  //if ( sock < 0 ) {
  if (sock == -1 )  {
    int32_t lastError = ACE_OS::last_error( );
    LOGERROR( "Failed to create socket. Errno: %d: %s", lastError, ACE_OS::strerror(lastError) );
    char msg[256];
...
TcpConn::TcpConn():m_io(NULL),m_maxBuffSizePool(0),m_waitSeconds(0)
{
}

TcpConn::TcpConn( const char * ipaddr, uint32_t waitSeconds, int32_t maxBuffSizePool )
:m_io(NULL),m_maxBuffSizePool( maxBuffSizePool ), m_addr(ipaddr), m_waitSeconds(waitSeconds)
{
}

TcpConn::TcpConn( const char * hostname, int32_t port, uint32_t waitSeconds, int32_t maxBuffSizePool)
:m_io(NULL), m_maxBuffSizePool( maxBuffSizePool ), m_addr( port, hostname ), m_waitSeconds(waitSeconds)
{
}
...
void  TcpConn::listen( ACE_INET_Addr addr, uint32_t waitSeconds )
{
  ...
  if ( retVal == -1 ) {
    ...
    if ( lastError == ETIME || lastError == ETIMEDOUT ) {
      /* adongre
       * Coverity - II
      * CID 29270: Calling risky function (SECURE_CODING)[VERY RISKY]. Using "sprintf" can cause a 
      ...
      */
      ACE_OS::snprintf( msg, 256,"TcpConn::listen Attempt to listen timed out after %d seconds.", waitSeconds );
      throw TimeoutException( msg );
    }
    ACE_OS::snprintf( msg, 256, "TcpConn::listen failed with errno: %d: %s", lastError, ACE_OS::strerror(lastError) );
    throw GemfireIOException( msg );
  }
}

void TcpConn::connect( const char * hostname, int32_t port, uint32_t waitSeconds)
{
  ACE_INET_Addr addr( port, hostname );
  m_addr = addr;
...
void TcpConn::connect()
{
  ...
  if ( retVal == -1 ) {
    ...
    int32_t lastError = ACE_OS::last_error( );
    if ( lastError == ETIME || lastError == ETIMEDOUT ) {
      ACE_OS::snprintf(msg, 256, "TcpConn::connect Attempt to connect timed out after %d seconds.", waitSeconds);
      // [sumedh] this is only called by constructor, so we must delete m_io
      GF_SAFE_DELETE(m_io);
      throw TimeoutException( msg );
    }
    ACE_OS::snprintf(msg, 256, "TcpConn::connect failed with errno: %d: %s",
  ...
  if ( -1 == rc )
  {
    char msg[250];
    int32_t lastError = ACE_OS::last_error( );
    ACE_OS::snprintf(msg, 256, "TcpConn::NONBLOCK: %d: %s",
      lastError, ACE_OS::strerror(lastError));
...
int32_t TcpConn::receive( char* buff, int32_t len, uint32_t waitSeconds, uint32_t waitMicroSeconds  )
{
  return socketOp( SOCK_READ, buff, len, waitSeconds );
}

int32_t TcpConn::send( const char* buff, int32_t len, uint32_t waitSeconds, uint32_t waitMicroSeconds  )
{
  return socketOp( SOCK_WRITE, (char*)buff, len, waitSeconds );
}
  ...
  
{
  ...
  if ( len <= 0 ) {
    LOGERROR( "TcpConn::socketOp called with a length of %d specified. "
...
// ARB: Return the local port for this TCP connection.
