...
TcrConnection* ThinClientPoolStickyDM::getConnectionFromQueueW( GfErrType* error,
  std::set< ServerLocation >& excludeServers, bool isBGThread, TcrMessage & request, int8_t& version, bool & match, bool &connFound, const BucketServerLocationPtr& serverLocation)
{
  TcrConnection* conn = NULL;
  TcrEndpoint* ep = NULL;
  bool maxConnLimit = false;
  if( isBGThread || request.getMessageType() == TcrMessage::GET_ALL_70 || request.getMessageType() == TcrMessage::GET_ALL_WITH_CALLBACK){
    conn = ThinClientPoolDM::getConnectionFromQueueW( error, excludeServers, isBGThread, request, version, match, connFound, serverLocation);
    return conn;
  }  
  ...
  if (m_attrs->getPRSingleHopEnabled() && !request.forTransaction()) {                          
    ...
    if (ep != NULL /*&& ep->connected()*/) {        
      //LOGINFO("Hitesh getSingleHopServer returns ep");
      m_manager->getSingleHopStickyConnection(ep,conn);
      if (!conn) {
        conn = getFromEP(ep);
        if(!conn) {            
          *error = createPoolConnectionToAEndPoint(conn, ep, maxConnLimit, true);
    ...
    LOGDEBUG("ThinClientPoolStickyDM::getConnectionFromQueueW return conn = %p match = %d connFound=%d", conn, match,connFound );    
    return conn;
  }

  	bool cf = m_manager->getStickyConnection(conn , error, excludeServers, request.forTransaction());
...
void ThinClientPoolStickyDM::putInQueue(TcrConnection* conn, bool isBGThread, bool isTransaction )
{
  if( !isBGThread ) {
