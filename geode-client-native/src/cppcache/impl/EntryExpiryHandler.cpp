    ...
    const void* arg)
{
  ...
  try {
    uint32_t curr_time = (uint32_t)current_time.sec();
    ...
    LOGDEBUG("Entered entry expiry task handler for key [%s] of region [%s]: "
        "%d,%d,%d,%d", Utils::getCacheableKeyString(key)->asChar(),
        m_regionPtr->getFullPath(), curr_time, lastTimeForExp, m_duration, sec);
    if (sec >= 0) {
      ...
      LOGDEBUG("Resetting expiry task %d secs later for key [%s] of region "
          "[%s]", -sec, Utils::getCacheableKeyString(key)->asChar(),
  ...
  // [sumedh] we now delete the handler in GF_Timer_Heap_ImmediateReset_T
    ...
    const CacheableKeyPtr& key)
{
  ...
  switch (m_action) {
    case ExpirationAction::INVALIDATE: {
      LOGDEBUG("EntryExpiryHandler::DoTheExpirationAction INVALIDATE "
          "for region %s entry with key %s", m_regionPtr->getFullPath(),
    ...
    case ExpirationAction::LOCAL_INVALIDATE: {
      LOGDEBUG("EntryExpiryHandler::DoTheExpirationAction LOCAL_INVALIDATE "
          "for region %s entry with key %s", m_regionPtr->getFullPath(),
          Utils::getCacheableKeyString(key)->asChar());
      m_regionPtr->invalidateNoThrow(key, NULLPTR, -1,
    ...
    case ExpirationAction::DESTROY: {
      LOGDEBUG("EntryExpiryHandler::DoTheExpirationAction DESTROY "
          "for region %s entry with key %s", m_regionPtr->getFullPath(),
          Utils::getCacheableKeyString(key)->asChar());
      m_regionPtr->destroyNoThrow(key, NULLPTR, -1,
          CacheEventFlags::EXPIRATION, versionTag);
      break;
    }
    case ExpirationAction::LOCAL_DESTROY: {
      LOGDEBUG("EntryExpiryHandler::DoTheExpirationAction LOCAL_DESTROY "
          "for region %s entry with key %s", m_regionPtr->getFullPath(),
          Utils::getCacheableKeyString(key)->asChar());
      m_regionPtr->destroyNoThrow(key, NULLPTR, -1,
          CacheEventFlags::EXPIRATION | CacheEventFlags::LOCAL, versionTag);
    ...
    default: {
      LOGERROR("Unknown expiration action "
          "%d for region %s for key %s", m_action, m_regionPtr->getFullPath(),
          Utils::getCacheableKeyString(key)->asChar());
      break;
    }
  }
