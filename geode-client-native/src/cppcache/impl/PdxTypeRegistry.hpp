...
namespace gemfire {
  ...
  typedef HashMapT<PdxSerializablePtr, PdxRemotePreservedDataPtr> PreservedHashMap;
...
private:
    ...
    //static std::map<PdxSerializablePtr , PdxRemotePreservedDataPtr> *preserveData;
...
public:
	...
	static void setPreserveData(PdxSerializablePtr obj, PdxRemotePreservedDataPtr preserveDataPtr);

	static PdxRemotePreservedDataPtr getPreserveData(PdxSerializablePtr obj);

	static void clear();

	static int32 getPDXIdForType(const char* type, const char* poolname, PdxTypePtr nType, bool checkIfThere);

	static bool getPdxIgnoreUnreadFields(){
	...
	static void setPdxReadSerialized(bool value){
		pdxReadSerialized = value;
	}

	static bool getPdxReadSerialized(){
