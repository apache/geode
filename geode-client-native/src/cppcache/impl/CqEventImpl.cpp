...
CqEventImpl::CqEventImpl(CqQueryPtr& cQuery, CqOperation::CqOperationType baseOp, CqOperation::CqOperationType cqOp,
    CacheableKeyPtr& key, CacheablePtr& value, ThinClientBaseDM* tcrdm, CacheableBytesPtr deltaBytes, EventIdPtr eventId):
 m_error(false)
{
    m_cQuery = cQuery;
    m_queryOp = cqOp;
    ...
    m_key = key;
    m_newValue = value;
    if(m_queryOp==CqOperation::OP_TYPE_INVALID)
      m_error = true;
    m_tcrdm = tcrdm;
...
CqQueryPtr CqEventImpl::getCq() const{
    return m_cQuery;
}
...
CqOperation::CqOperationType CqEventImpl::getQueryOperation() const {
   ...
   * @return Object key.
   */
CacheableKeyPtr CqEventImpl::getKey() const {
    return m_key;
}
  /**
...
CacheablePtr CqEventImpl::getNewValue() const {
    if (m_deltaValue == NULLPTR) {
      ...
      TcrMessage fullObjectMsg( m_eventId );
      TcrMessage reply;
      ...
      if ( poolHADM ) {
        ...
        err = static_cast< ThinClientCacheDistributionManager* >( m_tcrdm )->sendRequestToPrimary( fullObjectMsg, reply );
      }
...
bool CqEventImpl::getError() {
    return m_error;
}

std::string CqEventImpl::toString() {
    char buffer[1024];
    ACE_OS::snprintf(buffer, 1024, "CqEvent CqName=%s; base operation=%d; cq operation= %d;key=%s;value=%s",
     m_cQuery->getName(),
	m_baseOp,
	m_queryOp,
	m_key->toString()->asChar(),
	m_newValue->toString()->asChar());
    return buffer;
}

CacheableBytesPtr CqEventImpl::getDeltaValue() const
{
  return m_deltaValue;
}
