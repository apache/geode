...
StructSetImpl::StructSetImpl(const CacheableVectorPtr& response,
    const std::vector<CacheableStringPtr>& fieldNames)
{
  ...
  for (size_t i = 0; i < numOfFields; i++) {
    ...
    m_fieldNameIndexMap.insert(std::make_pair(fieldNames[i]->asChar(), (int32_t)i));
  }
...
bool StructSetImpl::isModifiable() const
{
  return false;
}

int32_t StructSetImpl::size() const
{
  return m_structVector->size();
}
...
{
  std::map<std::string, int32_t>::iterator iter = m_fieldNameIndexMap.find(
      fieldname);
...
const char * StructSetImpl::getFieldName(int32_t index)
{
  for (std::map<std::string, int32_t>::iterator iter =
      m_fieldNameIndexMap.begin(); iter != m_fieldNameIndexMap.end(); ++iter) {
