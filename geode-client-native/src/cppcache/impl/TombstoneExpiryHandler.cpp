...
TombstoneExpiryHandler::TombstoneExpiryHandler(TombstoneEntryPtr entryPtr, TombstoneList* tombstoneList, 
                    uint32_t duration):
                    m_entryPtr(entryPtr),
                    m_duration(duration), 
    ...
    const void* arg)
{
  ...
  int64_t curr_time = (int64_t)current_time.get_msec();
  int64_t expiryTaskId = m_entryPtr->getExpiryTaskId ();
  int64_t sec = curr_time - creationTime - m_duration * 1000;
  try {
    LOGDEBUG("Entered entry expiry task handler for tombstone of key [%s]: "
        "%lld,%lld,%d,%lld",
        Utils::getCacheableKeyString(key)->asChar(), curr_time, creationTime, m_duration, sec);
    if (sec >= 0) {
      ...
      LOGDEBUG("Resetting expiry task %d secs later for key "
          "[%s]", -sec/1000+1,   Utils::getCacheableKeyString(key)->asChar());
      CacheImpl::expiryTaskManager->resetTask((long)(m_entryPtr->getExpiryTaskId ()),
          uint32_t(-sec/1000+1));
      return 0;
    }
  }
  ...
  CacheImpl::expiryTaskManager->resetTask((long)expiryTaskId, 0);
  return 0;
}
