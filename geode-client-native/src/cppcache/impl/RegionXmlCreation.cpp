...
RegionAttributesPtr RegionXmlCreation::getAttributes()
{
  return regAttrs;
}
...
void RegionXmlCreation::createRoot(Cache* cache)
{
  ...
  //  if(cache->m_cacheImpl->getDefaultPool() == NULLPTR)
    {
      ...
      if(regAttrs->getScope() != ScopeType::LOCAL && regAttrs->getEndpoints() == NULL)
      {
  ...
  rootRegPtr = cache->createRegion(regionName.c_str(), regAttrs);
  fillIn(rootRegPtr);
}
...
std::string RegionXmlCreation::getAttrId() const {
  return attrId;
}
