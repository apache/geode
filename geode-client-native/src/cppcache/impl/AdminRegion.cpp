...
AdminRegion::AdminRegion(CacheImpl *cache, ThinClientBaseDM* distMan) :
  m_destroyPending(false)
     ...
     */
     ,m_connectionMgr((TcrConnectionManager*)0),m_distMngr((ThinClientBaseDM*)0)
{
    ...
    const CacheablePtr& valuePtr)
{
  ...
  TcrMessage request(TcrMessage::PUT, NULL, keyPtr, valuePtr, NULLPTR, false,m_distMngr, true, false, m_fullPath.c_str());
  request.setMetaRegion(true);
  TcrMessage reply(true,m_distMngr);
  ...
  switch (reply.getMessageType()) {
    case TcrMessage::REPLY: {
      LOGDEBUG("AdminRegion::put: entry is written into remote server "
          "at region %s", m_fullPath.c_str());
      break;
    }
...
const bool& AdminRegion::isDestroyed()
{
  return m_destroyPending;
}
ACE_RW_Thread_Mutex& AdminRegion::getRWLock()
{
  return m_rwLock;
}
