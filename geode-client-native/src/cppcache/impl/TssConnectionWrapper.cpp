...
void TssConnectionWrapper::setSHConnection(TcrEndpoint* ep, TcrConnection* conn)
{
...
TcrConnection* TssConnectionWrapper::getSHConnection(TcrEndpoint* ep, const char * poolname)
{
...
{                    
   m_EpnameVsConnection.insert(std::pair<std::string, TcrConnection*>(ep->name(), conn));      
}
...
{
  for(EpNameVsConnection::iterator iter = m_EpnameVsConnection.begin(); iter != m_EpnameVsConnection.end(); iter++) {    
    TcrConnection* tmp = iter->second;    
