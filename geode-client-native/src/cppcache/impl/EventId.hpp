...
namespace gemfire {
  ...
  class CPPCACHE_EXPORT EventId : public Cacheable
  {
    ...
    public:
      ...
      static EventIdPtr create( char * memId, uint32_t memIdLen, int64_t thr, int64_t seq )
      {
      ...
      EventId( bool doInit = true, uint32_t reserveSize=0 , bool fullValueAfterDeltaFail = false); //set init=false if we dont want to inc sequence (for de-serialization)

    protected:

      void initFromTSS(); // init from TSS and increment per-thread sequence number
      void initFromTSS_SameThreadIdAndSameSequenceId();//for after delta message fail, and server is looking for full value. So eventiD should be same
