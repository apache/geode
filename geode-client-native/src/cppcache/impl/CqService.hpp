 ...
 */
class CPPCACHE_EXPORT CqService : public SharedBase , private NonCopyable, private NonAssignable{
  private:
  ...
  CqServiceVsdStats & getCqServiceVsdStats(){return *dynamic_cast<CqServiceVsdStats*>(m_stats.ptr());}
   ...
   *
   */
CqQueryPtr newCq(std::string& cqName, std::string& queryString, CqAttributesPtr& cqAttributes, bool isDurable=false);
...
bool isCqExists(std::string& cqName);
   ...
   * @param value
   */
void invokeCqListeners(const std::map<std::string, int>* cqs, uint32_t messageType, CacheableKeyPtr key,
