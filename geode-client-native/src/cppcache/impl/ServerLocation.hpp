...
namespace gemfire
{
  ...
  {
  public:
     ServerLocation(std::string serverName, int port):Serializable(),
        m_port( port )
    {
      if (serverName.length() > 0) {
        m_serverName = CacheableString::create( serverName.c_str() );
        LOGDEBUG( "ServerLocation::ServerLocation(): creating ServerLocation for %s:%d", serverName.c_str( ), port );
      }      
      makeEpString();
    }
    ServerLocation():Serializable(),m_serverName(NULLPTR),m_port(-1)//Default constructor for deserialiozation.
    {
    }
    ...
    void setServername(CacheableStringPtr sn)
    {
      m_serverName = sn;
    }
    int getPort() const
    ...
    void toData( DataOutput& output ) const
    {
      if (m_serverName != NULLPTR) {
       // output.writeObject( m_serverName );
         output.writeNativeString(m_serverName->asChar( ) );//Hitesh changed
      }
    ...
    {
      if (m_serverName != NULLPTR) {
        return (uint32_t) sizeof(int)+ (uint32_t) (m_serverName->length()) * (uint32_t) sizeof( char);
      }
      return 0;
    }
    int8_t typeId( ) const
    ...
    bool operator==(const ServerLocation& rhs ) const{
      ...
      return( !strcmp(m_serverName->asChar(), rhs.getServerName( ).c_str()) && (m_port == rhs.getPort( ) ));
    }
