...
ThinClientPoolHADM::ThinClientPoolHADM(const char* name, PoolAttributesPtr poolAttr, TcrConnectionManager& connManager):
	ThinClientPoolDM(name, poolAttr, connManager), m_theTcrConnManager( connManager ),
  m_redundancySema( 0 ), m_redundancyTask( NULL ), m_servermonitorTaskId( -1 )
{
  m_redundancyManager = new ThinClientRedundancyManager(&connManager, poolAttr->getSubscriptionRedundancy( ), this);
}
...
void ThinClientPoolHADM::startBackgroundThreads()
{
  ...
  ThinClientPoolHADM>(this, &ThinClientPoolHADM::checkRedundancy);
  ...
  LOGFINE( "ThinClientPoolHADM::ThinClientPoolHADM Registered server "
  ...
  if ( ThinClientBaseDM::isFatalClientError( err ) ) {
    if ( err == GF_CACHE_LOCATOR_EXCEPTION ) {
      LOGWARN( "No locators were available during pool initialization with subscription redundancy." );
    }
    else {
  ...
  m_redundancyTask = new GF_TASK_T< ThinClientPoolHADM >( this, &ThinClientPoolHADM::redundancy, NC_Redundancy);
  m_redundancyTask->start( );
}

GfErrType ThinClientPoolHADM::sendSyncRequest(
    TcrMessage& request, TcrMessage& reply, bool attemptFailover, bool isBGThread)
{
  ...
  {
    err = ThinClientPoolDM::sendSyncRequest(request, reply, attemptFailover, isBGThread);
  }
  return err;
}

bool ThinClientPoolHADM::registerInterestForHARegion( TcrEndpoint *ep,
...
{
  return ThinClientBaseDM::sendSyncRequestRegisterInterest( request, reply, attemptFailover, NULL, endpoint );
}

 GfErrType ThinClientPoolHADM::sendSyncRequestRegisterInterest(TcrMessage& request, TcrMessage& reply, bool attemptFailover, ThinClientRegion* region, TcrEndpoint* endpoint )
 {
   return m_redundancyManager->sendSyncRequestRegisterInterest( request, reply, attemptFailover, endpoint, this, region );
 }

 GfErrType ThinClientPoolHADM::sendSyncRequestCq( TcrMessage& request, TcrMessage& reply)
 {
	 return m_redundancyManager->sendSyncRequestCq( request, reply, this );
 }

 bool ThinClientPoolHADM::preFailoverAction( )
 {
	 return true;
 }
...
*/
 ...
 GfErrType ThinClientPoolHADM::registerInterestAllRegions( TcrEndpoint *ep,
...
void ThinClientPoolHADM::addDisMessToQueue( ThinClientRegion* theTCR )
{
 ...
 {
   ACE_Guard< ACE_Recursive_Thread_Mutex > guard( m_regionsLock );
   for ( std::list< ThinClientRegion* >::iterator itr = m_regions.begin( ); itr != m_regions.end( ); itr++) {
     if ( *itr == theTCR ) {
       m_regions.erase( itr );
       return;
     }
 ...
 void ThinClientPoolHADM::readyForEvents()
 {
    ...
    const char * durable = DistributedSystem::getSystemProperties()->durableClientId();
 ...
 {
   ACE_Guard<ACE_Recursive_Thread_Mutex> guard(m_redundancyManager->getRedundancyLock());
...
{
  ACE_Guard< ACE_Recursive_Thread_Mutex > guard( m_regionsLock );
  for( std::list< ThinClientRegion* >::iterator it = m_regions.begin(); it != m_regions.end(); it++ )
  {
