...
EvictionController::EvictionController(size_t maxHeapSize, int32_t heapSizeDelta, CacheImpl* cache):
  m_run(false),
...
//  m_currentHeapSize = DistributedSystem::getSystemProperties()->gfHighWaterMark(), DistributedSystem::getSystemProperties()->gfMessageSize();

}

EvictionController::~EvictionController() {
  GF_SAFE_DELETE(evictionThreadPtr);
}
...
void EvictionController::processHeapInfo(int64_t& readInfo, int64_t& pendingEvictions) {
  ...
  if (sizeToCompare  > m_maxHeapSize) {
    ...
    int64_t sizeOverflow = (int64_t) sizeToCompare - m_maxHeapSize;

    //Calculate the percentage that we are over the limit.
    int32_t fractionalOverflow = (int32_t) (((sizeOverflow*100)%m_maxHeapSize) > 0)? 1:0;
    int32_t percentage = (int32_t) ((sizeOverflow * 100)/m_maxHeapSize) + fractionalOverflow;
      //need to evict
      int32_t evictionPercentage = (int32_t) (percentage+m_heapSizeDelta);
      int32_t bytesToEvict = (int32_t)((sizeToCompare*evictionPercentage)/100);
...
void EvictionController::registerRegion(std::string& name) {
  ...
  LOGFINE("Registered region with Heap LRU eviction controller: name is %s", name.c_str());
}
...
void EvictionController::deregisterRegion(std::string& name) {
  ...
  for(size_t i = 0;i< m_regions.size();i++) {
    ...
    if (str == name) {
      ...
      LOGFINE("Deregistered region with Heap LRU eviction controller: name is %s", name.c_str());
      break;
    }
  }
}
...
void EvictionController::evict(int32_t percentage) {
  ...
  // TODO: [sumedh] Shouldn't we take the CacheImpl::m_regions
