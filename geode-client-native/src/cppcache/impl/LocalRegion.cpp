  ...
  m_transactionEnabled(false)
{
  ...
  LOGFINE("Region %s :scope=%s", m_name.c_str(),
  ScopeType::fromOrdinal(m_regionAttributes->getScope()));
...
const char* LocalRegion::getName() const
{
  return m_name.c_str();
}

const char* LocalRegion::getFullPath() const
{
...
void LocalRegion::updateAccessAndModifiedTime(bool modified)
{
  ...
  if (regionExpiryEnabled()) {
    ...
    // TODO: [sumedh] should we really touch the parent region??
...
CacheStatisticsPtr LocalRegion::getStatistics() const
{
  ...
  SystemProperties* props = getCache()->getDistributedSystem()->getSystemProperties();
  if (props)
  {
    m_statisticsEnabled = props->statisticsEnabled();
  }
  if( !m_statisticsEnabled )
    throw StatisticsDisabledException("LocalRegion::getStatistics statistics disabled for this region");
...
{
  GfErrType err = invalidateRegionNoThrow(aCallbackArgument,
      CacheEventFlags::NORMAL);
...
{
  GfErrType err = destroyRegionNoThrow(aCallbackArgument, true,
      CacheEventFlags::NORMAL);
...
void LocalRegion::tombstoneOperationNoThrow(const CacheableHashMapPtr& tombstoneVersions, 
      const CacheableHashSetPtr& tombstoneKeys)
{
  ...
  if (tombstoneVersions.ptr() != NULL)
  {
    ...
    for (HashMapT<CacheableKeyPtr, CacheablePtr>::Iterator itr = tombstoneVersions->begin();
            itr != tombstoneVersions->end();
            ++itr)
    {
      try 
      {
        DSMemberForVersionStampPtr member = dynCast< DSMemberForVersionStampPtr >( itr.first() );
        uint16_t memberId = getCacheImpl()->getMemberListForVersionStamp()->add(member);
        int64_t version = (dynCast< CacheableInt64Ptr >( itr.second()))->value();
        gcVersions[memberId] = version;
      }
      catch ( const ClassCastException& ) {
        LOGERROR("tombstone_operation contains incorrect gc versions in the message. Region %s", getFullPath());
        continue;
      }
    }
...
RegionPtr LocalRegion::getSubregion(const char* path )
{
  ...
  if((pathstr == slash) || (pathstr.length() < 1)) {
     LOGERROR( "Get subregion path [%s] is not valid.",
         pathstr.c_str( ) );
...
RegionPtr LocalRegion::createSubregion(const char* subregionName,
    const RegionAttributesPtr& aRegionAttributes)
{
  CHECK_DESTROY_PENDING(TryWriteGuard, LocalRegion::createSubregion);
    ...
    if(0 == m_subRegions.find( subregionName, region_ptr)){
      throw RegionExistsException("LocalRegion::createSubregion: named region exists in the region");
    }
...
RegionEntryPtr LocalRegion::getEntry(const CacheableKeyPtr& key)
{
	if(getTXState() != NULL)
	{
		GfErrTypeThrowException("GetEntry is not supported in transaction", GF_NOTSUP);
	}
  RegionEntryPtr rptr;
...
void LocalRegion::getEntry(const CacheableKeyPtr& key,
    CacheablePtr& valuePtr)
{
  if (key == NULLPTR) {
    ...
    const UserDataPtr& aCallbackArgument)
{
  ...
  Utils::updateStatOpTime(m_regionStats->getStat(),  RegionStatType::getInstance()->getGetTimeId(), sampleStartNanos);
...
void LocalRegion::put(const CacheableKeyPtr& key,
    const CacheablePtr& value, const UserDataPtr& aCallbackArgument)
{
  CacheablePtr oldValue;
  int64 sampleStartNanos =Utils::startStatOpTime();
  VersionTagPtr versionTag;
  GfErrType err = putNoThrow(key, value, aCallbackArgument, oldValue, -1, CacheEventFlags::NORMAL, versionTag);
  Utils::updateStatOpTime(m_regionStats->getStat(), RegionStatType::getInstance()->getPutTimeId(), sampleStartNanos);
  ...
  const UserDataPtr& aCallbackArgument)
{
  if ((timeout * 1000) >= 0x7fffffff) {
    throw IllegalArgumentException("Region::putAll: timeout parameter "
        "greater than maximum allowed (2^31/1000 i.e 2147483).");
  }
  int64 sampleStartNanos =Utils::startStatOpTime();
  GfErrType err = putAllNoThrow(map, timeout, aCallbackArgument);
  Utils::updateStatOpTime(m_regionStats->getStat(), RegionStatType::getInstance()->getPutAllTimeId(), sampleStartNanos);
 // handleReplay(err, NULLPTR);
  GfErrTypeToException("Region::putAll", err);

}

void LocalRegion::removeAll(const VectorOfCacheableKey& keys, const UserDataPtr& aCallbackArgument)
{
  if (keys.size() == 0) {
  ...
  Utils::updateStatOpTime(m_regionStats->getStat(), RegionStatType::getInstance()->getRemoveAllTimeId(), sampleStartNanos);
  GfErrTypeToException("Region::removeAll", err);
}

void LocalRegion::create(const CacheableKeyPtr& key,
    const CacheablePtr&  value, const UserDataPtr& aCallbackArgument)
{
  VersionTagPtr versionTag;
...
bool LocalRegion::remove(const CacheableKeyPtr& key,
    const CacheablePtr&  value, const UserDataPtr& aCallbackArgument)
{  
  VersionTagPtr versionTag;
...
bool LocalRegion::removeEx(const CacheableKeyPtr& key, const UserDataPtr& aCallbackArgument)
{
  VersionTagPtr versionTag;
...
bool LocalRegion::localRemoveEx(const CacheableKeyPtr& key, const UserDataPtr& aCallbackArgument)
{
  VersionTagPtr versionTag;
  GfErrType err = removeNoThrowEx(key,  aCallbackArgument, -1,
      CacheEventFlags::LOCAL, versionTag);
...
{
  throw UnsupportedOperationException( "serverKeys is not supported for local regions." );
}

void LocalRegion::values(VectorOfCacheable& vc)
{
  CHECK_DESTROY_PENDING(TryReadGuard, LocalRegion::values);
  if ( ! m_regionAttributes->getCachingEnabled() ) { return; }
...
void LocalRegion::getAll(const VectorOfCacheableKey& keys,
    ...
    bool addToLocalCache, const UserDataPtr& aCallbackArgument)
{
  if (keys.size() == 0) {
    throw IllegalArgumentException("Region::getAll: zero keys provided");
  }
  // check for the combination which will result in no action
  if (values == NULLPTR && !(addToLocalCache
      && m_regionAttributes->getCachingEnabled())) {
    throw IllegalArgumentException("Region::getAll: either output \"values\""
        " parameter should be non-null, or \"addToLocalCache\" should be true "
        "and caching should be enabled for the region [%s]", getFullPath());
  }

  int64 sampleStartNanos =Utils::startStatOpTime();
  GfErrType err = getAllNoThrow(keys, values, exceptions, addToLocalCache, aCallbackArgument);
  Utils::updateStatOpTime(m_regionStats->getStat(), RegionStatType::getInstance()->getGetAllTimeId(), sampleStartNanos);
 // handleReplay(err, NULLPTR);
...
CachePtr LocalRegion::getCache() const
{
  CHECK_DESTROY_PENDING(TryReadGuard, LocalRegion::getCache);
  return CachePtr(m_cacheImpl->getCache());
}
...
bool LocalRegion::containsValueForKey_remote(const CacheableKeyPtr& keyPtr) const
{
	  CHECK_DESTROY_PENDING(TryReadGuard, LocalRegion::containsValueForKey);
	  if ( !m_regionAttributes->getCachingEnabled() ) { return false; }
	  CacheablePtr valuePtr;
	  MapEntryImplPtr mePtr;
...
{
  if (keyPtr == NULLPTR) {
    throw IllegalArgumentException("LocalRegion::containsValueForKey: "
        "key is null");
  }
...
{
  throw UnsupportedOperationException("LocalRegion::containsKeyOnServer: is not supported.");
}
void LocalRegion::getInterestList(VectorOfCacheableKey & vlist) const
{
  throw UnsupportedOperationException("LocalRegion::getInterestList: is not supported.");
}
void LocalRegion::getInterestListRegex(VectorOfCacheableString & vregex) const
{
  throw UnsupportedOperationException("LocalRegion::getInterestListRegex: is not supported.");
}

bool LocalRegion::containsKey(const CacheableKeyPtr&  keyPtr) const
{
  if (keyPtr == NULLPTR) {
    throw IllegalArgumentException("LocalRegion::containsKey: "
        "key is null");
  }
...
void LocalRegion::setRegionExpiryTask()
{
  if (regionExpiryEnabled()) {
    ...
    RegionExpiryHandler* handler = new RegionExpiryHandler(rptr,
      getRegionExpiryAction(), duration);
    long expiryTaskId = CacheImpl::expiryTaskManager->scheduleExpiryTask(
        handler, duration, 0);
    handler->setExpiryTaskId( expiryTaskId );
    LOGFINE("expiry for region [%s], expiry task id = %d, duration = %d, "
        "action = %d", m_fullPath.c_str(), expiryTaskId, duration,
...
void LocalRegion::registerEntryExpiryTask(MapEntryImplPtr& entry)
{
  ...
  EntryExpiryHandler* handler = new EntryExpiryHandler(rptr, entry,
      getEntryExpirationAction(), duration);
  long id = CacheImpl::expiryTaskManager->scheduleExpiryTask(
      handler, duration, 0);
  if (Log::finestEnabled()) {
    CacheableKeyPtr key;
    entry->getKeyI(key);
    LOGFINEST("entry expiry in region [%s], key [%s], task id = %d, "
        "duration = %d, action = %d", m_fullPath.c_str(),
...
void LocalRegion::release(bool invokeCallbacks)
{
  ...
  if (invokeCallbacks) {
    try {
      ...
      // TODO: [sumedh] shouldn't listener also be here instead of
      // during CacheImpl.close()
    }
    catch (...) {
      LOGWARN("Region close caught unknown exception in loader/writer "
          "close; continuing");
    }
  }
...
void LocalRegion::subregions_internal(const bool recursive, VectorOfRegion& sr)
{
  ...
  for( MapOfRegionWithLock::iterator p = m_subRegions.begin(); p!=m_subRegions.end() ; ++p )
  {
  ...
  if(recursive==true)
  {
    ...
    for( int32_t i = 0; i < subRegions.size(); i++ ) {
      dynamic_cast<LocalRegion *>(subRegions.at(i).ptr())->subregions_internal( true, sr );
    }
  }
}

GfErrType LocalRegion::getNoThrow(const CacheableKeyPtr& keyPtr,
    CacheablePtr& value, const UserDataPtr& aCallbackArgument)
{
  ...
  // TODO: [sumedh] CacheableToken::isInvalid should be completely hidden
  ...
  if (cachingEnabled) {
    ...
    if (isLocal &&
        (value != NULLPTR && !CacheableToken::isInvalid(value))) {
    ...
    if (!m_regionAttributes->getConcurrencyChecksEnabled()){
      updateCount = m_entries->addTrackerForEntry(keyPtr, value, true, false,
          false);
    LOGDEBUG("Region::get: added tracking with update counter [%d] for key "
        "[%s] with value [%s]", updateCount, Utils::getCacheableKeyString(
            keyPtr)->asChar(), Utils::getCacheableString(value)->asChar());
    }
  }
  ...
  public:
    ...
    ~RemoveTracking()
    {
      if (m_updateCount >= 0 && !m_region.getAttributes()->getConcurrencyChecksEnabled()) {
  ...
  // Its a cache missor it is invalid token then Check if we have a local loader.
  if ((value == NULLPTR || CacheableToken::isInvalid(value) || CacheableToken::isTombstone(value)) 
       && m_loader != NULLPTR) {
    try {
        ...
        Utils::updateStatOpTime(m_regionStats->getStat(), RegionStatType::getInstance()->getLoaderCallTimeId(), sampleStartNanos);
  ...
  if ( value != NULLPTR /*&& value != CacheableToken::invalid( )*/ && cachingEnabled && 
      !(CacheableToken::isTombstone(value) && (localValue == NULLPTR || 
          CacheableToken::isInvalid(localValue)))) {
    // [sumedh] try to create the entry and if that returns an existing value
    // (e.g. from another thread or notification) then return that
    LOGDEBUG("Region::get: creating entry with tracking update counter [%d] for key [%s]",
        updateCount, Utils::getCacheableKeyString(keyPtr)->asChar());
    if ((err = putLocal("Region::get", false, keyPtr, value, oldValue,
        cachingEnabled, updateCount, 0, versionTag)) != GF_NOERR) {
      if (err == GF_CACHE_CONCURRENT_MODIFICATION_EXCEPTION)
      {
        LOGDEBUG("Region::get: putLocal for key [%s] failed because the cache already contains \
  ...
  // should be AFTER_UPDATE or AFTER_CREATE depending on oldValue, so don't check here.
  if(isLoaderInvoked == false && err == GF_NOERR && value != NULLPTR)
  {
    err = invokeCacheListenerForEntryEvent(keyPtr, oldValue, value,
      aCallbackArgument, CacheEventFlags::NORMAL, AFTER_UPDATE, isLocal);
  }
...
//			args->push_back(VectorOfCacheableKeyPtr(new VectorOfCacheableKey(keys)));
//			args->push_back(values);
//			args->push_back(exceptions);
//			args->push_back(CacheableBoolean::create(addToLocalCache));
//			txState->recordTXOperation(GF_GET_ALL, getFullPath(), NULLPTR, args);
//		}
    err =  getAllNoThrow_remote(&keys, values, exceptions, NULLPTR, false, aCallbackArgument);
		if(err == GF_NOERR)
		{
			txState->setDirty();
		}
  ...
  if (serverKeys.size() > 0) {
	  err = getAllNoThrow_remote(&serverKeys, values, exceptions,
        NULLPTR, addToLocalCache, aCallbackArgument);
  }
...
namespace gemfire
{
  ...
  public:
    ...
    inline PutActions(LocalRegion& region) :
      m_region(region)
    {
    ...
    inline static GfErrType checkArgs(const CacheableKeyPtr& key,
        const CacheablePtr& value, DataInput* delta = NULL)
    {
    ...
    inline void getCallbackOldValue(bool cachingEnabled,
        const CacheableKeyPtr& key, MapEntryImplPtr& entry,
        ...
        const CacheablePtr& oldValue)
    {
      ...
      LOGFINER("Cache writer vetoed %s for key %s", (isUpdate ? "update" :
          "create"), Utils::getCacheableKeyString(key)->asChar());
    }
...
//    		m_txState->recordTXOperation(GF_PUT, m_region.getFullPath(), key, args);
//    	}
      // propagate the put to remote server, if any
      return m_region.putNoThrow_remote(key, value, aCallbackArgument, versionTag);
    }

    inline GfErrType localUpdate(const CacheableKeyPtr& key,
        const CacheablePtr& value, CacheablePtr& oldValue,
        bool cachingEnabled, const CacheEventFlags eventFlags,
        int updateCount, VersionTagPtr versionTag, 
        DataInput* delta = NULL, EventIdPtr eventId = NULLPTR, bool afterRemote = false)
    {
      return m_region.putLocal(name(), false, key, value, oldValue,
          cachingEnabled, updateCount, 0, versionTag, delta, eventId);
    }
  ...
  // encapsulates actions that need to be taken for a put() operation. This implementation allows
  // null values in Put during transaction. See defect #743
  class PutActionsTx : public PutActions
  {
  public:     
    
    inline PutActionsTx(LocalRegion& region) :
      PutActions(region)
    {
    }
    inline static GfErrType checkArgs(const CacheableKeyPtr& key,
  ...
  public:
    ...
    inline CreateActions(LocalRegion& region) :
      m_region(region)
    {
    	m_txState = TSSTXStateWrapper::s_gemfireTSSTXState->getTXState();
    }
    ...
    inline static GfErrType checkArgs(const CacheableKeyPtr& key,
        const CacheablePtr& value, DataInput* delta)
    {
      if (key == NULLPTR) {
        return GF_CACHE_ILLEGAL_ARGUMENT_EXCEPTION;
      }
...
//  		  args->push_back(value);
//  		  args->push_back(aCallbackArgument);
//  		  m_txState->recordTXOperation(GF_CREATE, m_region.getFullPath(), key, args);
//  	  }
      return m_region.createNoThrow_remote(key, value, aCallbackArgument, versionTag);
    }

    inline GfErrType localUpdate(const CacheableKeyPtr& key,
        const CacheablePtr& value, CacheablePtr& oldValue,
        bool cachingEnabled, const CacheEventFlags eventFlags,
        int updateCount, VersionTagPtr versionTag, DataInput* delta = NULL, EventIdPtr eventId = NULLPTR, bool afterRemote = false)
    {
  ...
  public:
    ...
    inline DestroyActions(LocalRegion& region) :
      m_region(region)
    {
    	m_txState = TSSTXStateWrapper::s_gemfireTSSTXState->getTXState();
    }
...
//    		VectorOfCacheablePtr args(new VectorOfCacheable());
//    		args->push_back(aCallbackArgument);
//    		m_txState->recordTXOperation(GF_DESTROY, m_region.getFullPath(), key, args);
//    	}
        ...
        DataInput* delta = NULL, EventIdPtr eventId = NULLPTR, bool afterRemote = false)
    {
      if (cachingEnabled) {
        ...
        // [sumedh] for notification invoke the listener even if the key does
        // not exist locally
        GfErrType err;
        LOGDEBUG("Region::destroy: region [%s] destroying key [%s]",
            m_region.getFullPath(),
            Utils::getCacheableKeyString(key)->asChar());
        if ((err = m_region.m_entries->remove(key, oldValue, entry,
            updateCount, versionTag, afterRemote)) != GF_NOERR) {
          if (eventFlags.isNotification()) {
            LOGDEBUG("Region::destroy: region [%s] destroy key [%s] for "
        ...
        if (oldValue != NULLPTR) {
          LOGDEBUG("Region::destroy: region [%s] destroyed key [%s] having "
              "value [%s]", m_region.getFullPath(),
              Utils::getCacheableKeyString(key)->asChar(),
              Utils::getCacheableString(oldValue)->asChar());
        ...
        const CacheablePtr& value, const UserDataPtr& aCallbackArgument, VersionTagPtr& versionTag)
    {
      ...
      if (!allowNULLValue && m_region.getAttributes()->getCachingEnabled()) {
        ...
        if (valuePtr != NULLPTR && value != NULLPTR) {
          if (valuePtr->classId() != value->classId() || valuePtr->typeId() != value->typeId()) {
            err = GF_ENOENT;
            return err;
          }
          ...
          if (memcmp(out1.getBuffer(), out2.getBuffer(), out1.getBufferLength()) != 0) {            
            err = GF_ENOENT;
            return err;
          }
        }
...
//        		args->push_back(value);
//        		args->push_back(aCallbackArgument);
//        		m_txState->recordTXOperation(GF_REMOVE, m_region.getFullPath(), key, args);
//        	}

        	  m_ServerResponse = m_region.removeNoThrow_remote(key, value, aCallbackArgument, versionTag);
...
//  	}
    if(allowNULLValue)
    {
  	  m_ServerResponse = m_region.removeNoThrowEX_remote(key, aCallbackArgument, versionTag);
    } else {
      m_ServerResponse = m_region.removeNoThrow_remote(key, value, aCallbackArgument, versionTag);
    }
        ...
        DataInput* delta = NULL, EventIdPtr eventId = NULLPTR, bool afterRemote = false)
    {
      ...
      if (!allowNULLValue && cachingEnabled) {
        ...
        else if (value == NULLPTR && (! CacheableToken::isInvalid(valuePtr) || valuePtr == NULLPTR)) {          
          err = (m_ServerResponse == 0 && valuePtr == NULLPTR)? GF_NOERR:GF_ENOENT;
          if (updateCount >= 0 && !m_region.getAttributes()->getConcurrencyChecksEnabled()) { // This means server has deleted an entry & same entry has been destroyed locally
            // So call removeTrackerForEntry to remove key that was added in the map during addTrackerForEntry call.
            m_region.m_entries->removeTrackerForEntry(key);
          }
          return err;
        }
        else if (valuePtr == NULLPTR && value != NULLPTR && m_ServerResponse != 0) {                    
          err = GF_ENOENT;
          return err;
        }
      }
      ...
      if (cachingEnabled) {
        ...
        if ((err = m_region.m_entries->remove(key, oldValue, entry,
            updateCount, versionTag, afterRemote)) != GF_NOERR) {
          if (eventFlags.isNotification()) {
        	  LOGDEBUG("Region::remove: region [%s] remove key [%s] for "
                "notification having value [%s] failed with %d",
                m_region.getFullPath(),
                Utils::getCacheableKeyString(key)->asChar(),
                Utils::getCacheableString(oldValue)->asChar(), err);
        ...
        if (oldValue != NULLPTR) {
          LOGDEBUG("Region::remove: region [%s] removed key [%s] having "
              "value [%s]", m_region.getFullPath(),
              Utils::getCacheableKeyString(key)->asChar(),
              Utils::getCacheableString(oldValue)->asChar());
          // any cleanup required for the entry (e.g. removing from LRU list)
  ...
  public:
    ...
    inline InvalidateActions(LocalRegion& region) :
      m_region(region)
    {
    	m_txState = TSSTXStateWrapper::s_gemfireTSSTXState->getTXState();
    }
        ...
        const CacheablePtr& oldValue)
    {
      bool isUpdate = (oldValue != NULLPTR);
      LOGFINER("Cache writer vetoed %s for key %s", (isUpdate ? "update" :
          "invalidate"), Utils::getCacheableKeyString(key)->asChar());
    }
...
//    		VectorOfCacheablePtr args(new VectorOfCacheable());
//    		args->push_back(aCallbackArgument);
//    		m_txState->recordTXOperation(GF_INVALIDATE, m_region.getFullPath(), key, args);
//    	}
      // propagate the invalidate to remote server, if any
      return m_region.invalidateNoThrow_remote(key, aCallbackArgument, versionTag);
    }
...
GfErrType LocalRegion::updateNoThrow(const CacheableKeyPtr& key,
    const CacheablePtr& value, const UserDataPtr& aCallbackArgument,
    CacheablePtr& oldValue, int updateCount, const CacheEventFlags eventFlags,
    VersionTagPtr versionTag, DataInput* delta, EventIdPtr eventId)
{
  GfErrType err = GF_NOERR;
  ...
  // [sumedh] do not invoke the writer in case of notification/eviction
  // or expiration
  if (m_writer != NULLPTR && eventFlags.invokeCacheWriter()) {
    ...
    if (!invokeCacheWriterForEntryEvent(key, oldValue, value,
        aCallbackArgument, eventFlags, TAction::s_beforeEventType)) {
      TAction::logCacheWriterFailure(key, oldValue);
      return GF_CACHEWRITER_ERROR;
    }
  }
  bool remoteOpDone = false;
  // [sumedh] try the remote update; but if this fails (e.g. due to security
  ...
  if (!eventFlags.isLocal() && !eventFlags.isNotification()) {
    if (cachingEnabled && updateCount < 0 && !m_regionAttributes->getConcurrencyChecksEnabled()) {
      // add a tracking for the entry
      if ((updateCount = m_entries->addTrackerForEntry(key, oldValue,
          TAction::s_addIfAbsent, TAction::s_failIfPresent, true)) < 0) {
        if (oldValue != NULLPTR) {
  ...
  if (!eventFlags.isNotification() || getProcessedMarker()) {
    if ((err = action.localUpdate(key, value, oldValue, cachingEnabled,
        eventFlags, updateCount, versionTag, delta, eventId, remoteOpDone)) == GF_CACHE_ENTRY_UPDATED) {
      LOGFINEST("%s: did not change local value for key [%s] since it has "
    ...
    {
      LOGDEBUG("Region::localUpdate: updateNoThrow<%s> for key [%s] failed because the cache already contains \
        an entry with higher version. The cache listener will not be invoked.",TAction::name(),
        Utils::getCacheableKeyString(key)->asChar());
    ...
    {
      LOGDEBUG("Region::localUpdate: updateNoThrow<%s> for key [%s] failed because of invalid delta.", TAction::name(),
        Utils::getCacheableKeyString(key)->asChar());
      ...
      if ( err == GF_NOERR && newValue1 != NULLPTR ) {
        err = m_entries->put( key, newValue1, entry, oldValue, updateCount,
          0, versionTag1 != NULLPTR ? versionTag1 : versionTag);
        if (err == GF_CACHE_CONCURRENT_MODIFICATION_EXCEPTION)
        {
          LOGDEBUG("Region::localUpdate: updateNoThrow<%s> for key [%s] failed because the cache already contains \
            an entry with higher version. The cache listener will not be invoked.",TAction::name(),
  ...
  err = invokeCacheListenerForEntryEvent(key, oldValue, value,
    aCallbackArgument, eventFlags, TAction::s_afterEventType);
  return err;
}
    ...
    VersionTagPtr versionTag, DataInput* delta, EventIdPtr eventId)
{
  ...
  if (!eventFlags.isNotification() || getProcessedMarker()) {
    if ((err = action.localUpdate(key, value, oldValue, cachingEnabled,
        eventFlags, updateCount, versionTag, delta, eventId)) == GF_CACHE_ENTRY_UPDATED) {
      LOGFINEST("%s: did not change local value for key [%s] since it has "
          "been updated by another thread while operation was in progress",
          TAction::name(), Utils::getCacheableKeyString(key)->asChar());
      err = GF_NOERR;
      ...
      // Entry not found. Possibly because the entry was added and removed in the 
      // same transaction. Ignoring this error #739
      LOGFINE("%s: No entry found. Possibly because the entry was added and removed in the same transaction. "
        "Ignoring this error. ",
          TAction::name(), Utils::getCacheableKeyString(key)->asChar());
      err = GF_NOERR;
    }
...
GfErrType LocalRegion::putNoThrow(const CacheableKeyPtr& key,
    ...
    VersionTagPtr versionTag, DataInput* delta, EventIdPtr eventId)
{
  return updateNoThrow<PutActions> (key, value, aCallbackArgument,
      oldValue, updateCount, eventFlags, versionTag, delta, eventId);
}
...
GfErrType LocalRegion::putNoThrowTX(const CacheableKeyPtr& key,
    ...
    VersionTagPtr versionTag, DataInput* delta, EventIdPtr eventId)
{
  return updateNoThrowTX<PutActionsTx> (key, value, aCallbackArgument,
      oldValue, updateCount, eventFlags, versionTag, delta, eventId);
}

GfErrType LocalRegion::createNoThrow(const CacheableKeyPtr& key,
    const CacheablePtr& value, const UserDataPtr& aCallbackArgument,
    int updateCount, const CacheEventFlags eventFlags, VersionTagPtr versionTag)
{
  CacheablePtr oldValue;
  return updateNoThrow<CreateActions> (key, value, aCallbackArgument,
...
GfErrType LocalRegion::removeNoThrow(const CacheableKeyPtr& key,
    const CacheablePtr& value, const UserDataPtr& aCallbackArgument, int updateCount,
    const CacheEventFlags eventFlags, VersionTagPtr versionTag)
{
  CacheablePtr oldValue;  
  return updateNoThrow<RemoveActions> (key, value, aCallbackArgument,
      oldValue, updateCount, eventFlags, versionTag);
}
...
GfErrType LocalRegion::invalidateNoThrowTX(const CacheableKeyPtr& key,
    const UserDataPtr& aCallbackArgument, int updateCount, const CacheEventFlags eventFlags, VersionTagPtr versionTag)
{
	  CacheablePtr oldValue;
    ...
    uint32_t timeout, const UserDataPtr& aCallbackArgument)
{
  ...
  GfErrType err = GF_NOERR;
  //VersionTagPtr versionTag;
  VersionedCacheableObjectPartListPtr versionedObjPartListPtr; //= new VersionedCacheableObjectPartList();
  TXState* txState = getTXState();
  if(txState != NULL)
  {
	  if(isLocalOp())
	  {
		...
		//}
    err = putAllNoThrow_remote(map, /*versionTag*/versionedObjPartListPtr, timeout, aCallbackArgument);
		if(err == GF_NOERR)
		{
			txState->setDirty();
		}
  ...
  public:
    ...
    ~RemoveTracking()
    {
      if (!m_region.getAttributes()->getConcurrencyChecksEnabled()) {
        ...
        for (MapOfOldValue::const_iterator iter = m_oldValueMap.begin(); iter
            != m_oldValueMap.end(); ++iter) {
    ...
    for (HashMapOfCacheable::Iterator iter = map.begin(); iter != map.end();
      ...
      if (cachingEnabled && !m_regionAttributes->getConcurrencyChecksEnabled() ) {
        int updateCount = m_entries->addTrackerForEntry(key, oldValue,
            true, false, true);
        oldValueMap.insert(std::make_pair(key, std::make_pair(oldValue,
            updateCount)));
      }
      if (m_writer != NULLPTR) {
        // invokeCacheWriterForEntryEvent method has the check that if oldValue
        // is a CacheableToken then it sets it to NULL; also determines if it
        // should be BEFORE_UPDATE or BEFORE_CREATE depending on oldValue
        if (!invokeCacheWriterForEntryEvent(key, oldValue, iter.second(),
            aCallbackArgument, CacheEventFlags::LOCAL, BEFORE_UPDATE)) {
  ...
  if ((err = putAllNoThrow_remote(map, versionedObjPartListPtr, timeout, aCallbackArgument)) != GF_NOERR) {
	  return err;
  }
  // next the local puts
  GfErrType localErr;
  ...
  if (cachingEnabled) {
    if(m_isPRSingleHopEnabled){ /*New PRSingleHop Case:: PR Singlehop condition*/
      for(int keyIndex = 0; keyIndex < versionedObjPartListPtr->getSucceededKeys()->size(); keyIndex++){
    	const CacheablePtr valPtr = versionedObjPartListPtr->getSucceededKeys()->at(keyIndex);
        ...
        if(mapIter != map.end()){
          ...
          //ThrowERROR
          LOGERROR("ERROR :: LocalRegion::putAllNoThrow() Key must be found in the usermap");
        }

        if (versionedObjPartListPtr != NULLPTR && versionedObjPartListPtr.ptr() != NULL) {
		  LOGDEBUG("versionedObjPartListPtr->getVersionedTagptr().size() = %d ", versionedObjPartListPtr->getVersionedTagptr().size());
        ...
        if ((localErr = LocalRegion::putNoThrow(key, value, aCallbackArgument, p.first,
            p.second, CacheEventFlags::LOCAL | CacheEventFlags::NOCACHEWRITER, versionTag))
               == GF_CACHE_ENTRY_UPDATED) {
            LOGFINEST("Region::putAll: did not change local value for key [%s] "
                      "since it has been updated by another thread while operation was "
                      "in progress", Utils::getCacheableKeyString(key)->asChar());
        }
      ...
      LOGDEBUG("NILKANTH LocalRegion::putAllNoThrow m_isPRSingleHopEnabled = %d expected false", m_isPRSingleHopEnabled);
      int index=0;
        ...
        const UserDataPtr& aCallbackArgument)
{
  ...
  if(txState != NULL)
  {
    if(isLocalOp())
	    return GF_NOTSUP;
	  err = removeAllNoThrow_remote(keys, versionedObjPartListPtr, aCallbackArgument);
		if(err == GF_NOERR)
      txState->setDirty();
		return err;
  }
  ...
  if (cachingEnabled) 
  {
    ...
    if(m_isPRSingleHopEnabled)
      ...
      keysPtr = (VectorOfCacheableKey *)&keys;
    ...
    for(int keyIndex = 0; keyIndex < keysPtr->size(); keyIndex++)
    {
      ...
      if (versionedObjPartListPtr != NULLPTR && versionedObjPartListPtr.ptr() != NULL) 
      {
        ...
        {
          LOGDEBUG("RemoveAll hits EntryNotFoundException at server side for key [%s], not to destroy it from local cache.", Utils::getCacheableKeyString(key)->asChar());
          continue;
        }
      }

      if ((localErr = LocalRegion::destroyNoThrow(key, aCallbackArgument, -1, 
        CacheEventFlags::LOCAL | CacheEventFlags::NOCACHEWRITER, versionTag)) == GF_CACHE_ENTRY_UPDATED) 
      {
        LOGFINEST("Region::removeAll: did not remove local value for key [%s] "
          "since it has been updated by another thread while operation was "
          "in progress", Utils::getCacheableKeyString(key)->asChar());
      }
      else if (localErr == GF_CACHE_LISTENER_EXCEPTION) 
      ...
      {
        LOGFINER("Region::removeAll: error [%d] for key [%s]",
               localErr, Utils::getCacheableKeyString(key)->asChar());
      }
      else if (localErr != GF_NOERR) {
        return localErr;
...
void LocalRegion::clear(const UserDataPtr& aCallbackArgument)
{
	...
	Utils::updateStatOpTime(m_regionStats->getStat(), RegionStatType::getInstance()->getClearsId(), sampleStartNanos);
}
void LocalRegion::localClear(const UserDataPtr& aCallbackArgument)
{
  ...
  if ( err != GF_NOERR )
     GfErrTypeToException("LocalRegion::localClear", err);
}
...
GfErrType LocalRegion::invalidateLocal(const char* name, const CacheableKeyPtr& keyPtr,
		const CacheablePtr& value, const CacheEventFlags eventFlags, VersionTagPtr versionTag)
{
  if (keyPtr == NULLPTR) {
    return GF_CACHE_ILLEGAL_ARGUMENT_EXCEPTION;
  }
  ...
  if (!eventFlags.isNotification() || getProcessedMarker()) {
    if (cachingEnabled) {
       ...
       */
      //VersionTagPtr versionTag;
      if ((err = m_entries->invalidate(keyPtr, me, oldValue, versionTag)) != GF_NOERR) {
        if (eventFlags.isNotification()) {
          LOGDEBUG("Region::invalidate: region [%s] invalidate key [%s] "
              "failed with error %d", getFullPath(),
              Utils::getCacheableKeyString(keyPtr)->asChar(), err);
        }
        if (err == GF_CACHE_CONCURRENT_MODIFICATION_EXCEPTION) {
          LOGDEBUG("Region::invalidateLocal: invalidate for key [%s] failed because the cache already contains \
    ...
    const CacheEventFlags eventFlags)
{
  ...
  if (m_subRegions.current_size() > 0) {
    ...
    for (MapOfRegionWithLock::iterator p = m_subRegions.begin(); p
        != m_subRegions.end(); ++p) {
    ...
    const CacheEventFlags eventFlags)
{
  ...
  // TODO: [sumedh] This should go into RegionGlobalLocks
  ...
  //Fix for BUG:849, i.e Remove subscription on region before destroying the region
  if( eventFlags == CacheEventFlags::LOCAL ){
    if(unregisterKeysBeforeDestroyRegion() != GF_NOERR ){
      LOGDEBUG("DEBUG :: LocalRegion::destroyRegionNoThrow UnregisteredKeys Failed");
    }
  }
  ...
  // [sumedh] do not invoke the writer for expiry or notification
  if (!eventFlags.isNotification() && !eventFlags.isEvictOrExpire()) {
    if (!invokeCacheWriterForRegionEvent(aCallbackArgument, eventFlags,
        BEFORE_REGION_DESTROY)) {
      // [sumedh] do not let CacheWriter veto when this is Cache::close()
      if (!eventFlags.isCacheClose()) {
    ...
    // [sumedh] for the expiry case try the local destroy first and remote
  ...
  LOGFINE("Region %s is being destroyed", m_fullPath.c_str());
  {
    ...
    for (MapOfRegionWithLock::iterator p = m_subRegions.begin();
      ...
      RegionInternal* subRegion =
        dynamic_cast<RegionInternal *>((*p).int_id_.ptr());
      if (subRegion != NULL) {
        // [sumedh] for subregions never remove from parent since that will cause
        // the region to be destroyed and SEGV; unbind_all takes care of that
        // Also don't send remote destroy message for sub-regions
        err = subRegion->destroyRegionNoThrow(aCallbackArgument,
            false, eventFlags | CacheEventFlags::LOCAL);
        // [sumedh] for Cache::close() keep going as far as possible
  ...
  // [sumedh] if we are not removing from parent then this is a proper
...
GfErrType LocalRegion::putLocal(const char* name, bool isCreate,
    ...
    CacheablePtr& oldValue, bool cachingEnabled, int updateCount,
    int destroyTracker, VersionTagPtr versionTag, DataInput* delta, EventIdPtr eventId)
{
  GfErrType err = GF_NOERR;
  ...
  if (cachingEnabled) {
    ...
    LOGDEBUG("%s: region [%s] putting key [%s], value [%s]", name,
        ...
        Utils::getCacheableString(value)->asChar());
    if (isCreate) {
      err = m_entries->create(key, value, entry, oldValue,
        updateCount, destroyTracker, versionTag);
    }
    else {
      ...
      if ( err == GF_INVALID_DELTA ) {
        ...
        if ( err == GF_NOERR && newValue1 != NULLPTR ) {
          err = m_entries->put( key, newValue1, entry, oldValue, updateCount,
            destroyTracker, versionTag1 != NULLPTR ? versionTag1 : versionTag, isUpdate );
        }
      }
      if( delta != NULL && err == GF_NOERR){ //Means that delta is on and there is no failure.
    ...
    LOGDEBUG("%s: region [%s] %s key [%s], value [%s]", name, getFullPath(),
        isUpdate ? "updated" : "created", Utils::getCacheableKeyString(
            key)->asChar(), Utils::getCacheableString(value)->asChar());
    // entry/region expiration
    ...
    const bool recursive)
{
  ...
  if (recursive == true) {
    ...
    for (MapOfRegionWithLock::iterator p = m_subRegions.begin(); p
        != m_subRegions.end(); ++p) {
      dynamic_cast<LocalRegion *> ((*p).int_id_.ptr())->entries_internal(me,
          true);
    }
  }
}
    ...
    CacheEventFlags eventFlags, EntryEventType type)
{
  ...
  if (m_writer != NULLPTR) {
    ...
    try {
      ...
      if (updateStats) {
    	  Utils::updateStatOpTime(m_regionStats->getStat(), RegionStatType::getInstance()->getWriterCallTimeId(), sampleStartNanos);
    	  m_regionStats->incWriterCallsCompleted();
      }

    }
    catch (const Exception& ex) {
      LOGERROR("Exception in CacheWriter::%s: %s: %s", eventStr,
    ...
    RegionEventType type)
{
  ...
  if (m_writer != NULLPTR) {
    ...
    try {
      ...
      LOGERROR("Exception in CacheWriter::%s: %s", eventStr,
          ex.getName(), ex.getMessage());
      bCacheWriterReturn = false;
    }
    catch (...) {
    ...
    CacheEventFlags eventFlags, EntryEventType type, bool isLocal)
{
  ...
  if (m_listener != NULLPTR) {
    ...
    try {
      ...
      switch (type) {
        case AFTER_UPDATE: {
          // [sumedh] when CREATE is received from server for notification
          // then force an afterUpdate even if key is not present in cache.
          if (oldValue != NULLPTR || eventFlags.isNotificationUpdate() || isLocal) {
...
// TODO: [sumedh] pass current time instead of evaluating it twice, here
// and in region
...
uint32_t LocalRegion::adjustLruEntriesLimit( uint32_t limit )
{
  ...
  if ( hadlru != needslru ) {
    throw IllegalStateException( "Cannot disable or enable LRU, can only adjust limit." );
  }
...
ExpirationAction::Action LocalRegion::adjustRegionExpiryAction( ExpirationAction::Action action )
{
  ...
  if ( ! hadExpiry ) {
    throw IllegalStateException( "Cannot change region ExpirationAction for region created without region expiry." );
  }
...
ExpirationAction::Action LocalRegion::adjustEntryExpiryAction( ExpirationAction::Action action )
{
  ...
  if ( ! hadExpiry ) {
    throw IllegalStateException( "Cannot change entry ExpirationAction for region created without entry expiry." );
  }
...
int32_t LocalRegion::adjustRegionExpiryDuration( int32_t duration )
{
  ...
  bool hadExpiry = ( getEntryExpiryDuration() != 0 );
  if ( ! hadExpiry ) {
    throw IllegalStateException( "Cannot change region  expiration duration for region created without region expiry." );
  }
...
int32_t LocalRegion::adjustEntryExpiryDuration( int32_t duration )
{
  ...
  bool hadExpiry = ( getEntryExpiryDuration() != 0 );
  if ( ! hadExpiry ) {
    throw IllegalStateException( "Cannot change entry expiration duration for region created without entry expiry." );
  }
...
bool LocalRegion::isStatisticsEnabled()
{
  ...
  {
    SystemProperties* props = m_cacheImpl->getCache()->getDistributedSystem()->getSystemProperties();
    if (props)
    {
...
{
  if ( m_regionAttributes->getCachingEnabled() && 0 != m_regionAttributes->getEntryIdleTimeout() )
  {
    return true;
  }
  else
...
GfErrType LocalRegion::unregisterKeysBeforeDestroyRegion(){
  return GF_NOERR;
}
...
GfErrType LocalRegion::putNoThrow_remote(const CacheableKeyPtr& keyPtr,
    const CacheablePtr& cvalue, const UserDataPtr& aCallbackArgument, VersionTagPtr& versionTag, bool checkDelta)
{
  return GF_NOERR;
}

GfErrType LocalRegion::putAllNoThrow_remote(const HashMapOfCacheable& map, VersionedCacheableObjectPartListPtr& putAllResponse,
    uint32_t timeout, const UserDataPtr& aCallbackArgument)
{
  return GF_NOERR;
}

GfErrType LocalRegion::removeAllNoThrow_remote(const VectorOfCacheableKey& keys,
    VersionedCacheableObjectPartListPtr& versionedObjPartList, const UserDataPtr& aCallbackArgument)
{
  return GF_NOERR;
}

GfErrType LocalRegion::createNoThrow_remote(const CacheableKeyPtr& keyPtr,
    const CacheablePtr& cvalue, const UserDataPtr& aCallbackArgument, VersionTagPtr& versionTag)
{
  return GF_NOERR;
}

GfErrType LocalRegion::destroyNoThrow_remote(const CacheableKeyPtr& keyPtr,
    const UserDataPtr& aCallbackArgument, VersionTagPtr& versionTag)
{
  return GF_NOERR;
}

GfErrType LocalRegion::removeNoThrow_remote(const CacheableKeyPtr& keyPtr,
    const CacheablePtr& cvalue, const UserDataPtr& aCallbackArgument, VersionTagPtr& versionTag)
{
  return GF_NOERR;
}

GfErrType LocalRegion::removeNoThrowEX_remote(const CacheableKeyPtr& keyPtr, const UserDataPtr& aCallbackArgument, VersionTagPtr& versionTag)
{
	return GF_NOERR;
}

GfErrType LocalRegion::invalidateNoThrow_remote(const CacheableKeyPtr& keyPtr,
        const UserDataPtr& aCallbackArgument, VersionTagPtr& versionTag)
{
  return GF_NOERR;
}

GfErrType LocalRegion::getAllNoThrow_remote(const VectorOfCacheableKey* keys,
    const HashMapOfCacheablePtr& values,
    const HashMapOfExceptionPtr& exceptions,
    const VectorOfCacheableKeyPtr& resultKeys, bool addToLocalCache, const UserDataPtr& aCallbackArgument)
{
  return GF_NOERR;
}
...
void LocalRegion::evict(int32_t percentage) {
  ...
  if (m_entries != NULL) {
    ...
    LRUEntriesMap* lruMap = static_cast<LRUEntriesMap*>(m_entries);
    LOGINFO("Evicting %d entries. Current entry count is %d", entriesToEvict, size);
...
void LocalRegion::invokeAfterAllEndPointDisconnected()
{
  if( m_listener != NULLPTR ){
    ...
    try {
      ...
      LOGERROR("Exception in CacheListener::afterRegionDisconnected: %s: %s", ex.getName(), ex.getMessage());
    }
    catch (...) {
...
GfErrType LocalRegion::getNoThrow_FullObject( EventIdPtr eventId, CacheablePtr& fullObject, VersionTagPtr& versionTag )
{
  return GF_NOERR;
}

CacheablePtr LocalRegion::handleReplay(GfErrType& err, CacheablePtr value) const
{
	if(err == GF_TRANSACTION_DATA_REBALANCED_EXCEPTION || err == GF_TRANSACTION_DATA_NODE_HAS_DEPARTED_EXCEPTION)
	{
		bool isRollBack = (err == GF_TRANSACTION_DATA_REBALANCED_EXCEPTION);
		TXState* txState = getTXState();
		if(txState == NULL)
		{
			GfErrTypeThrowException("TXState is NULL", GF_CACHE_ILLEGAL_STATE_EXCEPTION);
		}
...
TombstoneListPtr LocalRegion::getTombstoneList()
{
