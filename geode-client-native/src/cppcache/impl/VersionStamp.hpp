...
namespace gemfire
{
  ...
  {
  public:
    VersionStamp():m_entryVersionHighByte(0), m_entryVersionLowBytes(0), m_regionVersionHighBytes(0), 
      m_regionVersionLowBytes(0), m_memberID(0)
    {
    }
    ...
    GfErrType processVersionTag(RegionInternal* region, CacheableKeyPtr keyPtr, VersionTagPtr tag, bool deltaCheck);
  ...
  private:
    ...
    GfErrType checkForConflict(RegionInternal* region, std::string keystr, VersionTagPtr tag, bool deltaCheck);
    GfErrType checkForDeltaConflict(RegionInternal* region, std::string keystr, int64_t stampVersion, int64_t tagVersion, 
                               VersionTagPtr tag);
  };
