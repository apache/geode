...
public:
  ...
  static ClientHealthStatsPtr create(int gets, int puts, int misses,
    int listCalls,int numThreads, int64_t cpuTime = 0, int cpus = 0)
  {
    return ClientHealthStatsPtr(new ClientHealthStats( gets, puts, misses, listCalls, numThreads, cpuTime, cpus));
  }
  ~ClientHealthStats( );

private:  
  ClientHealthStats(int gets, int puts, int misses,
