...
ClientMetadata::ClientMetadata(int totalNumBuckets, CacheableStringPtr colocatedWith, ThinClientPoolDM* tcrdm, 
                               std::vector<FixedPartitionAttributesImplPtr>* fpaSet):
m_totalNumBuckets(totalNumBuckets), m_colocatedWith(colocatedWith), m_tcrdm(tcrdm), m_previousOne(NULLPTR), m_partitionNames(NULLPTR)
{
  ...
  if(fpaSet != NULL) {
  LOGDEBUG("ClientMetadata Creating metadata with %d buckets & fpaset size is %d ", totalNumBuckets, fpaSet->size());
    if ( !fpaSet->empty() ) {
      int totalFPABuckets = 0;
      for (int i = 0; i < (int)fpaSet->size(); i++ ) {
...
ClientMetadata::ClientMetadata():m_totalNumBuckets(0),m_colocatedWith(NULLPTR), m_tcrdm(NULL), m_partitionNames(NULLPTR)
{
}
...
void ClientMetadata::setPartitionNames()
{
  ...
  for ( FixedMapType::iterator it=m_fpaMap.begin() ; it != m_fpaMap.end(); it++ ) {        
...
{  
  ...
  if(m_bucketServerLocationsList[bucketId].empty()) {
    ...
    LOGFINER("returning primary & m_bucketServerLocationsList size is %d",m_bucketServerLocationsList.size());
...
void ClientMetadata::updateBucketServerLocations(int bucketId,
  BucketServerLocationsType bucketServerLocations)
{
  ...
  // This is for pruning according to server groups, only applicable when client is configured with
  // server-group.
  if ( serverGroup.length() != 0 ) {
    BucketServerLocationsType primaries;
    BucketServerLocationsType secondaries;
    for (BucketServerLocationsType::iterator iter = bucketServerLocations.begin();
    ...
    for (BucketServerLocationsType::iterator iter = primaries.begin();
      iter != primaries.end(); ++iter) {
        LOGFINER("updating primaries with bucketId %d and Server = %s ",bucketId, (*iter)->getEpString().c_str());
        m_bucketServerLocationsList[bucketId].push_back(*iter);
    }
...
void ClientMetadata::removeBucketServerLocation(BucketServerLocation serverLocation)
{
  /*WriteGuard guard( m_readWriteLock );
  int i=0;
  ...
  BucketServerLocationsListType::iterator iter = m_bucketServerLocationsList.begin();
  for(iter = m_bucketServerLocationsList.begin(); iter != m_bucketServerLocationsList.end();
    iter++, i++) {
      locations = *(iter);
      for(locnsIter = locations.begin(); locnsIter != locations.end(); locnsIter++) {
        LOGFINE("Erasing from inner list its size is %d",locations.size());
        if (locations.erase(std::find(locations.begin(), locations.end(), serverLocation)) != locations.end()) {
          locnfound = true;
  ...
  LOGFINE("updating outer list size is & i is %d\t %d", m_bucketServerLocationsList.size(),i);
  if (locnfound) {
...
void ClientMetadata::populateDummyServers(int bucketId, BucketServerLocationsType locations)
{
 // WriteGuard guard( m_readWriteLock );
...
int ClientMetadata::assignFixedBucketId(const char* partitionName, CacheableKeyPtr resolvekey)
{
  LOGDEBUG("FPR assignFixedBucketId partititonname = %s , m_fpaMap.size() = %d ", partitionName, m_fpaMap.size());
  FixedMapType::iterator iter = m_fpaMap.find(partitionName);
  if (iter != m_fpaMap.end()) {
    std::vector<int> attList = iter->second;
    int hc = (int)resolvekey->hashcode();
    int bucketId = std::abs((int) hc % (attList.at(0)));
...
std::vector<BucketServerLocationPtr> ClientMetadata::adviseServerLocations(int bucketId) {
  checkBucketId(bucketId);
  return m_bucketServerLocationsList[bucketId];
}

BucketServerLocationPtr ClientMetadata::advisePrimaryServerLocation(int bucketId) {
  std::vector<BucketServerLocationPtr> locations = adviseServerLocations(bucketId);
...
BucketServerLocationPtr ClientMetadata::adviseRandomServerLocation() {
  if (m_bucketServerLocationsList.size() > 0) {
    RandGen randGen;
    int random = randGen(m_bucketServerLocationsList.size());
    checkBucketId(random);
    std::vector<BucketServerLocationPtr> locations = m_bucketServerLocationsList[random];
    if (locations.size() == 0)
      return NULLPTR;
