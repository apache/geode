...
/*TcrConnection* ThinClientPoolStickyHADM::getConnectionFromQueueW( GfErrType* error,
  std::set< ServerLocation >& excludeServers, bool isBGThread, TcrMessage & request, int8_t& version, bool & dummy, const BucketServerLocationPtr& serverLocation )
{
  TcrConnection* conn = NULL;
  if( isBGThread ){ 
    conn = ThinClientPoolDM::getConnectionFromQueueW( error, excludeServers, isBGThread, request, version, dummy, serverLocation);
    return conn;
  }
  
  m_manager->getStickyConnection(conn , error, excludeServers, request.forTransaction());
  return conn;
}
void ThinClientPoolStickyHADM::putInQueue(TcrConnection* conn, bool isBGThread, bool isTransaction )
{
 if( !isBGThread ) 
