...
namespace gemfire {
  ...
  void StackTrace::print() const {
    for ( std::list<std::string>::const_iterator itr = m_frames.begin(); itr != m_frames.end(); ++itr) {
      printf("%s\n", (*itr).c_str());
    }
  }
...
namespace gemfire {
  ...
  /** Records stack frame lines into an array of std::string. */
  class TraceToFrames : public LineTracer
  {
  public:

    std::string* m_frameLines;
    int32_t m_index;
    StackTrace* m_stack;

    TraceToFrames( StackTrace* stack )
      : m_index( 0 ),
      m_stack( stack )
    {
    }
    ...
    void TraceLine( PSTR line )
    {
      if ( m_index < GF_TRACE_LEN ) {
        if ( 0 == strncmp( "Function: ", line, 10 ) ) {
          for( uint32_t i = 10; i < strlen( line ); i++ ) {
            if ( line[i] == ' ' || line[i] == '\r' ) {
              line[i] = '\0';
            }
          }
          m_stack->addFrame( line + 10, m_index );
          m_index++;
        }
      }
    }
  ...
  StackTrace::StackTrace( )
  {
    ...
    TraceToFrames lineTracer( this );
    TraceCurrentLocation( &lineTracer, GF_TRACE_LEN );
    m_size = lineTracer.size();
  }
  StackTrace::~StackTrace( )
  {
  }
  void StackTrace::addFrame( const char* line, int i ) {
    m_frames[i].init( line );
  }
