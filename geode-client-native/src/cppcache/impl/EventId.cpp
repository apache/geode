...
namespace gemfire
{
  ...
  public:
    ...
    inline int64_t getEidThr()
    {
      return m_eidThrTSS;
    }

    inline int64_t getAndIncEidSeq()
    {
      return m_eidSeqTSS++;
    }

    inline int64_t getSeqNum()
    {
      return m_eidSeqTSS - 1;
    }
  ...
  {
    // ARB: This method is always expected to write out nonstatic distributed memberid.
    output.writeBytes((int8_t*)m_eidMem, m_eidMemLen);
  ...
  Serializable* EventId::fromData( DataInput& input )
  {
    ...
    input.readBytesOnly((int8_t*)m_eidMem, m_eidMemLen);
    int32_t arrayLen;
    input.readArrayLen(&arrayLen);
    char numberCode = 0;
    input.read((uint8_t *)&numberCode);
  ...
  const char* EventId::getMemId() const
  {
    return m_eidMem;
  }

  int32_t EventId::getMemIdLen() const
  {
    return m_eidMemLen;
  }

  int64_t EventId::getThrId() const
  {
    return m_eidThr;
  }

  int64_t EventId::getSeqNum() const
  {
    return m_eidSeq;
  }
  ...
  int64_t EventId::getEventIdData( DataInput& input, char numberCode )
  {
    ...
    // ARB: Read number based on numeric code written by java server.
    if (numberCode == 0) {
      int8_t byteVal;
      input.read(&byteVal);
      retVal = byteVal;
  ...
  {
    return new EventId( false ); //use false since we dont want to inc sequence (for de-serialization)
  }

  int32_t EventId::classId( ) const
  {
    return 0;
  }

  int8_t EventId::typeId( ) const
  {
    return GemfireTypeIdsImpl::EventId;
  }
  ...
  EventId::EventId( bool doInit, uint32_t reserveSize, bool fullValueAfterDeltaFail) :
          /* adongre
           * CID 28934: Uninitialized scalar field (UNINIT_CTOR)
           */
  m_eidMemLen(0),m_eidSeq(0),m_eidThr(0),m_bucketId(-1), m_breadcrumbCounter(0)
  {
    if(!doInit) return;
  ...
  {
    return ACE_OS::snprintf(buffer, maxLength, "EventId( memID=[binary], thr=%"
        PRIi64 ", seq=%" PRIi64 " )", m_eidThr, m_eidSeq);
  }
}
