...
namespace gemfire
{
    ...
    if (len > 0) {
      ...
      for (int32_t index = keysOffset; index < keysOffset + len; ++index) {
        if (hasKeys) {
          ...
          throw FatalInternalException("CacheableObjectPartList: "
        ...
        if (byte == 2/* for exception*/) {
         ...
         // input.readObject(exMsgPtr, true);//Hitesh Changed
          ...
          if (m_addToLocalCache) {
            ...
            {
              LOGDEBUG("CacheableObjectPartList::fromData putLocal for key [%s] failed because the cache \
  ...
  int32_t CacheableObjectPartList::classId() const
  {
    return 0;
  }
  ...
  uint32_t CacheableObjectPartList::objectSize() const
  {
    return 0;
  }
