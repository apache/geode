...
namespace gemfire
{
   ...
   * `constructor "gemfire::Set<unsigned short>::Iterator::Iterator(gemfire::Set<unsigned short>::Iterator const &)" i
   * but no corresponding user-written assignment operator. 
   * 
   * FIX : Make the class non copyable
   */
  template < typename T > class CPPCACHE_EXPORT Set : private NonAssignable
  {
    public:
      ...
      class Iterator
      {
        ...
        public:
          ...
          {
            return ( m_iter != m_set.m_set.end( ) );
          }

          ~Iterator( )
          {
            m_set.m_mutex.release( );
          }
