...
bool LRULocalDestroyAction::evict(const MapEntryImplPtr& mePtr)
{
  ...
  // [sumedh] we should invoke the destroyNoThrow with appropriate
  ...
  GfErrType err = m_regionPtr->destroyNoThrow(keyPtr, NULLPTR, -1,
      CacheEventFlags::EVICTION | CacheEventFlags::LOCAL, versionTag);
  return (err == GF_NOERR);
}
