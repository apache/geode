...
public:
  ...
  void setLocators(int32 curVal) {
    getStats()->setInt(m_locatorsId, curVal);
  }

  void setServers(int32 curVal) {
  ...
  void incLoctorRequests() {
    getStats()->incLong(m_locReqsId, 1);
  }

  void incLoctorResposes() {
    getStats()->incLong(m_locRespsId, 1);
  }
  ...
  void incPoolConnects() {
    getStats()->incInt(m_connectsId, 1);
  }

  void incPoolDisconnects() {
    getStats()->incInt(m_disconnectsId, 1);
  }
  ...
  inline gemfire_statistics::Statistics* getStats() {
	  return m_poolStats;
  }
...
public:
  int32_t getLocatorsId() {
    return m_locatorsId;
  }

  int32_t getServersId() {
    return m_serversId;
  }

  int32_t getSubscriptionServersId() {
    return m_subsServsId;
  }

  int32_t getLocatorRequestsId() {
    return m_locReqsId;
  }

  int32_t getLocatorResposesId() {
    return m_locRespsId;
  }

  int32_t getPoolConnectionsId() {
    return m_poolConnsId;
  }

  int32_t getConnectsId() {
    return m_connectsId;
  }

  int32_t getDisconnectsId() {
    return m_disconnectsId;
  }

  int32_t getMinPoolSizeConnectsId() {
    return m_minPoolConnectsId;
  }

  int32_t getLoadCondConnectsId() {
    return m_loadCondConnectsId;
  }
  int32_t getIdleDisconnectsId() {
    return m_idleDisconnectsId;
  }
  int32_t getLoadCondDisconnectsId() {
    return m_loadCondDisconnectsId;
  }
  int32_t getWaitingConnectionsId() {
    return m_waitingConnectionsId;
  }
  int32_t getTotalWaitingConnsId() {
    return m_totalWaitingConnsId;
  }
  int32_t getTotalWaitingConnTimeId() {
      return m_totalWaitingConnTimeId;
  }
  int32_t getCurClientOpsId() {
    return m_curClientOpsId;
  }
  int32_t getClientOpsSucceededId() {
    return m_clientOpsSuccessId;
  }
  int32_t getClientOpsSucceededTimeId() {
      return m_clientOpsSuccessTimeId;
  }
  int32_t getClientOpsFailedId() {
    return m_clientOpsFailedId;
  }
  int32_t getClientOpsTimeoutId() {
    return m_clientOpsTimeoutId;
  }
  int32_t getReceivedBytesId() {
    return m_receivedBytesId;
  }
  int32_t getMessagesBeingReceivedId() {
    return m_messagesBeingReceivedId;
  }
  int32_t getProcessedDeltaMessagesId() {
    return m_processedDeltaMessagesId;
  }
  int32_t getDeltaMessageFailuresId() {
    return m_deltaMessageFailuresId;
  }
  int32_t getProcessedDeltaMessagesTimeId() {
    return m_processedDeltaMessagesTimeId;
  }
  int32_t getQueryExecutionId() {
      return m_queryExecutionsId;
  }
  int32_t getQueryExecutionTimeId() {
      return m_queryExecutionTimeId;
  }
};
