...
namespace gemfire
{
  ...
  void VersionedCacheableObjectPartList::readObjectPart(int32_t index, DataInput& input, CacheableKeyPtr keyPtr){
    uint8_t objType = 0;
    ...
    if(isException){ //Exception case
      ...
      if (m_exceptions != NULLPTR) {
        ...
        if (strstr(exMsg, "com.gemstone.gemfire.security."
              "NotAuthorizedException") != NULL) {
          ex = new NotAuthorizedException("Authorization exception at server:", exMsg);
        }
        else {
          ex = new CacheServerException(
      ...
      //set NULLPTR to indicate that there is no exception for the key on this index
      //readObject
  ...
  Serializable* VersionedCacheableObjectPartList::fromData(DataInput& input)
  {
    ...
    if (!m_hasKeys && !hasObjects && !m_hasTags) {
      LOGDEBUG("VersionedCacheableObjectPartList::fromData: Looks like message has no data. Returning,");
      return NULL;
    }
    ...
    if (m_hasKeys) {
      ...
      if( m_keys == NULL && m_resultKeys == NULLPTR) {
        LOGERROR("VersionedCacheableObjectPartList::fromData: Exception: hasObjects is true and m_keys and m_resultKeys are also NULL");
        throw FatalInternalException("VersionedCacheableObjectPartList: "
            "hasObjects is true and m_keys is also NULL");
      }else {
        LOGDEBUG("VersionedCacheableObjectPartList::fromData m_keys or m_resultKeys not null");
      }
    }else{
      LOGDEBUG("VersionedCacheableObjectPartList::fromData m_hasKeys, m_keys, hasObjects all are NULL");
    ...
    if(hasObjects){
      ...
      for (int32_t index = 0; index < len; ++index) {
        if (m_keys != NULL && !m_hasKeys) {
          ...
          LOGERROR("VersionedCacheableObjectPartList::fromData: hasObjects = true but m_keys is NULL and m_resultKeys== NULL or m_resultKeys->size=0" );
        }*/
      }
    ...
    }else{ //if consistancyEnabled=false, we need to pass empty or NULLPtr m_versionTags
      for (int32_t index = 0; index < len; ++index) {
        VersionTagPtr versionTag;
        m_versionTags[index] = versionTag;
      }
    ...
    if(hasObjects){
      ...
      for (int32_t index = 0; index < len; ++index) {
        if (m_keys != NULL && !m_hasKeys) {
          ...
          LOGERROR("VersionedCacheableObjectPartList::fromData: hasObjects = true but m_keys is NULL AND m_resultKeys=NULLPTR or m_resultKeys->size=0" );
        }*/
        ...
        if(m_byteArray[index] != 3){ //3 - key not found on server
          ...
          if (m_addToLocalCache) {
            ...
            GfErrType err = m_region->putLocal("getAll", false, key , value, oldValue,
                true, updateCount, m_destroyTracker, versionTag);
            if (err == GF_CACHE_CONCURRENT_MODIFICATION_EXCEPTION)
            {
              LOGDEBUG("VersionedCacheableObjectPartList::fromData putLocal for key [%s] failed because the cache \
  ...
  int32_t VersionedCacheableObjectPartList::classId() const
  {
    return 0;
  }
  ...
  uint32_t VersionedCacheableObjectPartList::objectSize() const
  {
    return 0;
  }
