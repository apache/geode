...
long TcrConnectionManager::getPingTaskId()
{
  return m_pingTaskId;
}
void TcrConnectionManager::init( bool isPool )
{
  ...
  if (!props->isGridClient() && !isPool ) {
    ACE_Event_Handler* connectionChecker = new ExpiryHandler_T<
      TcrConnectionManager>(this, &TcrConnectionManager::checkConnection);
    m_pingTaskId = CacheImpl::expiryTaskManager->scheduleExpiryTask(
        connectionChecker, 10, pingInterval, false);
    LOGFINE( "TcrConnectionManager::TcrConnectionManager Registered ping "
        "task with id = %ld, interval = %ld", m_pingTaskId, pingInterval );
  }
  ...
  if (cacheAttributes != NULLPTR && (cacheAttributes->getRedundancyLevel() > 0
        || m_isDurable) && (endpoints = cacheAttributes->getEndpoints())
      != NULL && strcmp(endpoints, "none") != 0) {    
    ...
    // ARB: Call maintain redundancy level, so primary is available for notification operations.
    GfErrType err = m_redundancyManager->maintainRedundancyLevel( true );
    m_redundancyManager->m_HAenabled = m_redundancyManager->m_HAenabled || ThinClientBaseDM::isDeltaEnabledOnServer( );

    ACE_Event_Handler* redundancyChecker = new ExpiryHandler_T<
      TcrConnectionManager>(this, &TcrConnectionManager::checkRedundancy);
    ...
    LOGFINE( "TcrConnectionManager::TcrConnectionManager Registered server "
        "monitor task with id = %ld, interval = %ld", m_servermonitorTaskId,
    ...
    m_redundancyTask = new GF_TASK_T<TcrConnectionManager>(this,
        &TcrConnectionManager::redundancy, NC_Redundancy);
...
void TcrConnectionManager::startFailoverAndCleanupThreads(bool isPool)
{
  if (m_failoverTask == NULL ||  m_cleanupTask == NULL) {
    ...
    if (m_failoverTask == NULL && !isPool) {
      m_failoverTask = new GF_TASK_T<TcrConnectionManager>(this,
          &TcrConnectionManager::failover, NC_Failover);
    ...
    if (m_cleanupTask == NULL && !isPool) {
      ...
      m_cleanupTask = new GF_TASK_T<TcrConnectionManager>(this,
          &TcrConnectionManager::cleanup, NC_CleanUp);
      m_cleanupTask->start();
    }
  }
}
...
TcrConnectionManager::~TcrConnectionManager()
{
  if ( m_cleanupTask != NULL ) {
    ...
    // ARB: cleanup of endpoints, when regions are destroyed via notification
    {
      ...
      for (ACE_Map_Manager< std::string, TcrEndpoint*,
          ACE_Recursive_Thread_Mutex>::iterator iter = m_endpoints.begin();
...
void TcrConnectionManager::connect(ThinClientBaseDM* distMng,
    ...
    const std::unordered_set<std::string>& endpointStrs)
{
  ACE_Guard<ACE_Recursive_Thread_Mutex> guardDistMngrs(m_distMngrsLock);
  {
    ...
    if (numEndPoints == 0) {
      LOGFINE("TcrConnectionManager::connect(): Empty endpointstr vector "
          "passed to TCCM, will initialize endpoints list with all available "
          "endpoints (%d).", m_endpoints.current_size());
      for (ACE_Map_Manager<std::string, TcrEndpoint*,
          ACE_Recursive_Thread_Mutex>::iterator currItr = m_endpoints.begin();
          currItr != m_endpoints.end(); ++currItr) {
        TcrEndpoint *ep = (*currItr).int_id_;       
        ep->setNumRegions(ep->numRegions() + 1);
        LOGFINER("TCCM 2: incremented region reference count for endpoint %s "
            "to %d", ep->name().c_str(), ep->numRegions());
        endpoints.push_back(ep);
      }
    }
    else {
      for (std::unordered_set<std::string>::const_iterator iter = endpointStrs.begin();
  ...
  if (m_redundancyManager->m_globalProcessedMarker) {
    ...
    if (tcrHADM != NULL) {
      ThinClientHARegion* tcrHARegion = dynamic_cast< ThinClientHARegion*>(
          tcrHADM->m_region);
...
TcrEndpoint* TcrConnectionManager::addRefToTcrEndpoint(std::string endpointName, ThinClientBaseDM* dm)
{
  TcrEndpoint *ep = NULL;  
  /*
  ...
  */
  if (0 != m_endpoints.find(endpointName, ep)) {
    ...
    ep = new TcrEndpoint(endpointName, m_cache, m_failoverSema,
      m_cleanupSema, m_redundancySema, dm, false);
...
void TcrConnectionManager::disconnect(ThinClientBaseDM * distMng, std::vector<TcrEndpoint *> &endpoints, bool keepEndpoints)
{
  ACE_Guard<ACE_Recursive_Thread_Mutex> guardDistMngrs(m_distMngrsLock);
  {
  ACE_Guard<ACE_Recursive_Thread_Mutex> guard(m_endpoints.mutex());
...
bool TcrConnectionManager::removeRefToEndpoint(TcrEndpoint *ep,bool keepEndpoint)
{
...
int TcrConnectionManager::processEventIdMap(const ACE_Time_Value& currTime, const void *)
{
  return m_redundancyManager->processEventIdMap( currTime, NULL );
}

int TcrConnectionManager::checkConnection(const ACE_Time_Value &, const void *)
{
  ACE_Guard<ACE_Recursive_Thread_Mutex> guard(m_endpoints.mutex());
...
int TcrConnectionManager::checkRedundancy(const ACE_Time_Value &, const void *)
{
  m_redundancySema.release( );
  return 0;
}
  ...
  while (isRunning) {
    ...
    if (isRunning && !isNetDown) {
      try {
        ...
        for (std::list<ThinClientBaseDM *>::iterator it =
            m_distMngrs.begin(); it != m_distMngrs.end(); ++it) {
          (*it)->failover();
        }
        ...
        LOGERROR("Unexpected exception while failing over to a "
            "different endpoint");
      }
    }
  }
...
void TcrConnectionManager::getAllEndpoints( std::vector<TcrEndpoint *> &endpoints )
{
  ACE_Guard< ACE_Recursive_Thread_Mutex > guard( m_endpoints.mutex( ) );

  for ( ACE_Map_Manager< std::string, TcrEndpoint *, ACE_Recursive_Thread_Mutex>::iterator currItr = m_endpoints.begin( );
            currItr != m_endpoints.end( ); currItr++)  {
...
GfErrType TcrConnectionManager::registerInterestAllRegions( TcrEndpoint *ep,
    const TcrMessage* request, TcrMessage* reply )
{
  // Preconditions:
  // 1. m_distMngrs.size() > 1 (query distribution manager + 1 or more TcrHADistributionManagers).
  ...
  for (std::list<ThinClientBaseDM *>::iterator it = begin; it != end; ++it) {
    TcrHADistributionManager *tcrHADM = dynamic_cast<TcrHADistributionManager *>(*it);
    if (tcrHADM != NULL) {
      if ( (opErr = tcrHADM->registerInterestForRegion( ep, request, reply )) != GF_NOERR ) {
        if (err == GF_NOERR) {
          err = opErr;
        }
      }
...
GfErrType TcrConnectionManager::sendSyncRequestCq( TcrMessage& request, TcrMessage& reply )
{
...
void TcrConnectionManager::netDown()
{
  ...
  // ARB: sleep for 15 seconds to allow ping and redundancy threads to pause.
...
/* Need to do a get on unknown key after calling this Fn to restablish all connection */
void TcrConnectionManager::revive()
{
  isNetDown = false;

  // ARB: sleep for 15 seconds to allow redundancy thread to reestablish connections.
  gemfire::millisleep(15000);
}
...
void TcrConnectionManager::addNotificationForDeletion( GF_TASK_T<TcrEndpoint> *notifyReceiver, TcrConnection *notifyConnection,  ACE_Semaphore& notifyCleanupSema )
{
...
int TcrConnectionManager::cleanup(volatile bool& isRunning)
{
  ...
  do {
    ...
    // ARB: If we block on acquire, the queue must be empty (precondition).
    if ( m_receiverReleaseList.size( ) == 0 ) {
      LOGDEBUG( "TcrConnectionManager::cleanup(): waiting to acquire cleanup semaphore." );
  ...
  // ARB: Postcondition - all notification channels should be cleaned up by the end of this function.
...
// ARB: TESTING: Durable clients - return queue status of endpoing. Not thread safe.
...
GfErrType TcrConnectionManager::sendSyncRequestCq( TcrMessage& request, TcrMessage& reply, TcrHADistributionManager* theHADM )
{
...
GfErrType TcrConnectionManager::sendSyncRequestRegisterInterest(
    TcrMessage& request,
    TcrMessage& reply,
    bool attemptFailover, TcrEndpoint* endpoint, TcrHADistributionManager* theHADM, ThinClientRegion* region )
{
  return m_redundancyManager->sendSyncRequestRegisterInterest( request, reply, attemptFailover, endpoint, theHADM, region );
}
