...
namespace gemfire
{
  ...
  public:
    FixedPartitionAttributesImpl():Serializable(), m_partitionName(NULLPTR), m_isPrimary(false),
      m_numBuckets(1), m_startingBucketId(-1)
    {
    }   
    ...
    {
      if (m_partitionName != NULLPTR) {
        return (uint32_t) sizeof(int)+ (uint32_t) sizeof(int) + (uint32_t) sizeof(bool) + 
    ...
    FixedPartitionAttributesImpl& operator=(const FixedPartitionAttributesImpl& rhs)
    {
      if( this == &rhs)
        return *this;
