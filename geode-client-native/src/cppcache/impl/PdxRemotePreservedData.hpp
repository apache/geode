...
public:
  ...
  PdxRemotePreservedData(int32 typeId, int32 mergedTypeId, int32 numberOfFields, /*Object^*/
  ...
  void initialize(int32 typeId, int32 mergedTypeId, int32 numberOfFields, /*Object^*/
	SerializablePtr owner) {
    m_typeId = typeId;
    m_mergedTypeId = mergedTypeId;
    m_currentIndex = 0;
	...
	inline int32 getMergedTypeId() {
		return m_mergedTypeId;
	}
    ...
    inline long getPreservedDataExpiryTaskId() {
      return m_expiryTakId;
    }

	SerializablePtr getOwner() {
		return m_owner;
	}

	void setOwner(SerializablePtr val) {
		m_owner = val;
	}
