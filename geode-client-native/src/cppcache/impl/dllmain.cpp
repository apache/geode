...
#include <windows.h>
  ...
  LIBEXP void DllMainGetPath(char *result, int maxLen)
  {
    ...
    if ( libgfcppcache_module == 0 ) {
      ...
      if (libgfcppcache_module == 0) {
        libgfcppcache_module = GetModuleHandle("gfclicache.dll");
      }
    }
    if ( libgfcppcache_module != 0 ) {
...
namespace gemfire
{
  ...
  void setDefaultNewAndDelete_disabled()
  {
    ...
    {
      throw IllegalStateException("Could not set default new and delete operators.");
    }
  }
  */
