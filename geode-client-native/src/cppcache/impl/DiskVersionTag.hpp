  ...
  protected:
    virtual void readMembers(uint16_t flags, DataInput& input )
    {
      ...
      if ((flags & HAS_MEMBER_ID) != 0) {
        ...
        m_internalMemId = memberList->add((DSMemberForVersionStampPtr)internalMemId);
      }
      ...
      if ((flags & HAS_PREVIOUS_MEMBER_ID) != 0) {
        if ((flags & DUPLICATE_MEMBER_IDS) != 0) {
          ...
          m_previousMemId = memberList->add((DSMemberForVersionStampPtr)previousMemId);
        }
      }
    }
  public:
     ...
     */
    DiskVersionTag(int32_t entryVersion, 
      int16_t regionVersionHighBytes, int32_t regionVersionLowBytes, 
      uint16_t internalMemId, uint16_t previousMemId): VersionTag(entryVersion, 
      regionVersionHighBytes, regionVersionLowBytes, 
