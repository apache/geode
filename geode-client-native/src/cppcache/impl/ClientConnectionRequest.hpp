...
namespace gemfire
{
  ...
  public:
#ifdef _TEST_
    ClientConnectionRequest(const std::set<ServerLocation>& excludeServergroup, std::string servergroup = ""):
    ServerLocationRequest(),
    m_excludeServergroup_serverLocation( excludeServergroup ),
    m_servergroup( servergroup ){}
#else
    ClientConnectionRequest(const std::set<TcrEndpoint*>& excludeServergroup, std::string servergroup = ""):
    m_excludeServergroup( excludeServergroup ),
    m_servergroup( servergroup ){}
#endif
