...
#define _VERSION_TAG_NULL_CHK (versionTag != NULLPTR && versionTag.ptr() != NULL)
    ...
    uint32_t size, volatile int* destroyTrackers, bool concurrencyChecksEnabled)
{
  ...
  LOGFINER("Initializing MapSegment with size %d (given size %d).",
      mapSize, size);
  m_map->open(mapSize);
...
int MapSegment::acquire()
{
  return m_segmentMutex.acquire();
}

int MapSegment::release()
{
  return m_segmentMutex.release();
}
    ...
    int updateCount, int destroyTracker, VersionTagPtr versionTag)
{
	...
	GfErrType err = GF_NOERR;
    {
      ...
      if ((status = m_map->find(key, entry)) == -1) {
        if ((err = putNoEntry(key, newValue, me, updateCount,
        ...
        if (oldValue == NULLPTR ||  CacheableToken::isTombstone(oldValue)) {
          ...
          if (oldValue == NULLPTR) {
            err = putForTrackedEntry(key, newValue, entry, entryImpl, updateCount, versionStamp);
          }
          else {
            unguardedRemoveActualEntryWithoutCancelTask(key, handler, taskid);
            err = putNoEntry(key, newValue, me, updateCount, destroyTracker, versionTag, &versionStamp);
          }
...
GfErrType MapSegment::put(const CacheableKeyPtr& key,
    const CacheablePtr& newValue, MapEntryImplPtr& me, CacheablePtr& oldValue,
    int updateCount, int destroyTracker, bool& isUpdate, 
    VersionTagPtr versionTag, DataInput* delta)
{
	...
	GfErrType err = GF_NOERR;
    {
      ...
      int status;
      if ((status = m_map->find(key, entry)) == -1) {
        if( delta != NULL ) return GF_INVALID_DELTA; // You can not apply delta when there is no entry hence ask for full object
        isUpdate = false;
        err = putNoEntry(key, newValue, me, updateCount, destroyTracker, versionTag);
      }
      else {
        MapEntryImpl* entryImpl = entry->getImplPtr();
        ...
        if (CacheableToken::isTombstone(meOldValue)){
          ...
          updateCount, versionStamp, delta)) == GF_NOERR) {
          me = entryImpl;
          oldValue = meOldValue;
...
GfErrType MapSegment::invalidate(const CacheableKeyPtr& key,
    MapEntryImplPtr& me, CacheablePtr& oldValue, VersionTagPtr versionTag, bool& isTokenAdded)
{
  _GF_GUARD_SEGMENT;
  int status;
  isTokenAdded = false;
  ...
  if ((status = m_map->find(key, entry)) != -1) {
    ...
    if(m_concurrencyChecksEnabled){
      if ((err = putNoEntry(key, CacheableToken::invalid(), me, -1,
              -1, versionTag)) != GF_NOERR) {
        return err;
      }
      isTokenAdded = true;
    }
...
GfErrType MapSegment::removeWhenConcurrencyEnabled(const CacheableKeyPtr& key,
    ...
    long expiryTaskID, TombstoneExpiryHandler* handler, bool& expTaskSet)
{
  ...
  if ((status = m_map->find(key, entry)) != -1) 
  {
    ...
    if (_VERSION_TAG_NULL_CHK) {
      ...
      if ((err = entry->getVersionStamp().processVersionTag(m_region, keyPtr, versionTag, false)) != 
        GF_NOERR)
        return err;
      versionStamp.setVersions(versionTag);
    }
    ...
    if ((err = putForTrackedEntry(key, CacheableToken::tombstone(), entry, entryImpl,
        updateCount, versionStamp)) == GF_NOERR) {
    ...
    if (CacheableToken::isTombstone(oldValue)) {
      oldValue = NULLPTR;
      if (afterRemote) {
        return GF_NOERR; // We are here because a remote op succeeded, no need to throw an error
      }
      else {
        return GF_CACHE_ENTRY_NOT_FOUND;
      }
  ...
  else
  {
    ...
    if (_VERSION_TAG_NULL_CHK)
    {
      ...
      m_tombstoneList->add(m_region, mapEntry->getImplPtr(), handler, expiryTaskID);
      expTaskSet = true;
    }
    oldValue = NULLPTR;
    isEntryFound = false;
    if (afterRemote)
      err =  GF_NOERR; // We are here because a remote op succeeded, no need to throw an error
    else
      err =  GF_CACHE_ENTRY_NOT_FOUND;
  }
  return err;
 ...
 */
GfErrType MapSegment::remove(const CacheableKeyPtr& key,
    CacheablePtr& oldValue, MapEntryImplPtr& me, int updateCount, 
    VersionTagPtr versionTag, bool afterRemote, bool& isEntryFound)
{
//  _GF_GUARD_SEGMENT;
  ...
  if(m_concurrencyChecksEnabled){
    ...
    GfErrType err;
    {
      _GF_GUARD_SEGMENT;
      //if (m_concurrencyChecksEnabled)
      err = removeWhenConcurrencyEnabled(key, oldValue, me, updateCount, versionTag, afterRemote, isEntryFound, id, handler, expTaskSet);
    }
...
bool MapSegment::unguardedRemoveActualEntry(const CacheableKeyPtr& key, bool cancelTask)
{
  MapEntryPtr entry;
...
bool MapSegment::unguardedRemoveActualEntryWithoutCancelTask(const CacheableKeyPtr& key, TombstoneExpiryHandler*& handler, long& taskid)
{
  MapEntryPtr entry;
  taskid = m_tombstoneList->eraseEntryFromTombstoneListWithoutCancelTask(key, m_region, handler);
  if (m_map->unbind(key, entry) == -1){
      return false;
  }
  return true;
...
bool MapSegment::removeActualEntry(const CacheableKeyPtr& key, bool cancelTask)
{
  _GF_GUARD_SEGMENT;
...
{
  _GF_GUARD_SEGMENT;
  for (CacheableKeyHashMap::iterator iter = m_map->begin(); iter
      != m_map->end(); iter++) {
  ...
  for (CacheableKeyHashMap::iterator iter = m_map->begin(); iter
    ...
    if(status != -1)
    {
      ...
      if (valuePtr != NULLPTR && !CacheableToken::isInvalid(valuePtr) && 
        !CacheableToken::isDestroyed(valuePtr) && !CacheableToken::isTombstone(valuePtr)) {
        if( CacheableToken::isOverflowed(valuePtr) )
        { //get Value from disc. 
...
// This function will not get called if concurrency checks are enabled. The versioning
...
// This function will not get called if concurrency checks are enabled. The versioning
// changes takes care of the version and no need for tracking the entry 
void MapSegment::addTrackerForAllEntries(MapOfUpdateCounters& updateCounterMap)
{
  if (m_concurrencyChecksEnabled)
    return;
  _GF_GUARD_SEGMENT;
...
CacheablePtr MapSegment::getFromDisc(CacheableKeyPtr key, MapEntryImpl* entryImpl)
{
  LocalRegion* lregion = static_cast<LocalRegion*> (m_region);
  EntriesMap* em = lregion->getEntryMap();
  return em->getFromDisk( key, entryImpl ) ;
}
GfErrType MapSegment::putForTrackedEntry(const CacheableKeyPtr& key,
  ...
  if (updateCount < 0 || m_concurrencyChecksEnabled) {
    ...
    if ( delta != NULL ) {
      ...
      if ( oldValue == NULLPTR || CacheableToken::isDestroyed( oldValue ) || 
        CacheableToken::isInvalid(oldValue) || CacheableToken::isTombstone(oldValue)) {
        if (m_poolDM) m_poolDM->updateNotificationStats(false, 0);
        return GF_INVALID_DELTA;
      }
      else if( CacheableToken::isOverflowed(oldValue) ){ //get Value from disc.
      ...
      try {
        if( m_region->getAttributes()->getCloningEnabled( ) ){
          ...
          if (m_poolDM) m_poolDM->updateNotificationStats(true, ((ACE_OS::gettimeofday()-currTimeBefore).msec())*1000000);
          newValue1 = tempVal;
...
GfErrType MapSegment::isTombstone(CacheableKeyPtr key, MapEntryImplPtr& me, bool& result)
{
  CacheablePtr value;
  MapEntryPtr entry;
  MapEntryImpl* mePtr;
