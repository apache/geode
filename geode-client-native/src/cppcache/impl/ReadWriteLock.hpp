...
class TimedTryWriteGuard {    
  ...
  ~TimedTryWriteGuard() { if (isAcquired_) lock_.release(); }    
...
class WriteGuard {    
  ...
  : lock_(lock) { lock_.acquire_write(); }
...
class TryReadGuard {    
  ...
  ~TryReadGuard() { if (isAcquired_) lock_.release(); }    
  bool isAcquired() const { return isAcquired_; }
...
class TryWriteGuard {    
  ...
  ~TryWriteGuard() { if (isAcquired_) lock_.release(); }    
  bool isAcquired() const { return isAcquired_; }
