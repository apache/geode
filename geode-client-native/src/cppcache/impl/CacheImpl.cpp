...
CacheImpl::CacheImpl(Cache* c, const char* name, DistributedSystemPtr sys, const char* id_data, bool iPUF, bool readPdxSerialized ):
  m_implementee(c),
  m_closed(false),
  m_cond( m_mutex ),
  ...
  m_initDone(false)
{
  m_cacheTXManager = InternalCacheTransactionManager2PCPtr(new InternalCacheTransactionManager2PCImpl(c));
  ...
  if (prop && prop->heapLRULimitEnabled()) {
    m_evictionControllerPtr = new EvictionController(prop->heapLRULimit(),prop->heapLRUDelta(), this);
...
CacheImpl::CacheImpl(Cache* c, const char* name, DistributedSystemPtr sys , bool iPUF, bool readPdxSerialized):
  m_implementee(c),
  m_closed(false),
  m_cond( m_mutex ),
  m_distributedSystem(sys),
...
void CacheImpl::initServices( )
{
  ...
  CacheImpl::s_versionStampMemIdList = MemberListForVersionStampPtr(new MemberListForVersionStamp());
  if( !m_initDone && m_attributes != NULLPTR && m_attributes->getEndpoints() ){
	if(PoolManager::getAll().size() > 0 && getCacheMode()){
	  LOGWARN("At least one pool has been created so ignoring cache level redundancy setting");
	}
...
int CacheImpl::blackListBucketTimeouts()
{
  return s_blacklistBucketTimeout;
}

void CacheImpl::setBlackListBucketTimeouts()
{
  s_blacklistBucketTimeout += 1;
}
...
void CacheImpl::netDown()
{
  ...
  for ( HashMapOfPools::Iterator itr = pools.begin( ); itr != pools.end( ); itr++ ) {
    currPool = itr.second( );
    try {
...
void CacheImpl::revive()
{
  m_tcrConnectionManager->revive();
}
    ...
    const RegionAttributesPtr& rattrs ) const
{
  ...
  if (m_attributes != NULLPTR && (endpoints = m_attributes->getEndpoints())
      != NULL && (m_attributes->getRedundancyLevel() > 0
  ...
  if ( ( endpoints = rattrs->getEndpoints( ) ) != NULL ) {
    ...
    if((pPtr!=NULLPTR && (pPtr->getSubscriptionRedundancy()>0 || pPtr->getSubscriptionEnabled()) ) || m_tcrConnectionManager->isDurable()) {
	    regionKind = THINCLIENT_HA_REGION; //As of now ThinClinetHARegion deals with Pool as well.
    } else {
...
QueryServicePtr CacheImpl::getQueryService(const char* poolName)
{
  ...
  /* adongre
   * CID 28992: Structurally dead code (UNREACHABLE)
   */
  //return NULLPTR;
}
...
void CacheImpl::sendNotificationCloseMsgs()
{
  ...
  for (HashMapOfPools::Iterator iter = pools.begin(); iter != pools.end(); ++iter) {
    ThinClientPoolHADM * pool = dynamic_cast<ThinClientPoolHADM*>(iter.second().ptr());
    if (pool != NULL) {
...
void CacheImpl::close(bool keepalive)
{
  ...
  if (m_tcrConnectionManager != NULL) {
   ...
   for(MapOfRegionWithLock::iterator q = regions.begin();
       q != regions.end(); ++q) {
     // TODO: remove dynamic_cast here by having RegionInternal in the regions map
     RegionInternal* rImpl = dynamic_cast<RegionInternal *>( (*q).int_id_.ptr());
     if (rImpl != NULL) {
       rImpl->destroyRegionNoThrow(NULLPTR, false, CacheEventFlags::LOCAL |
           CacheEventFlags::CACHE_CLOSE);
     }
   }
   ...
   LOGFINE("Closed pool manager with keepalive %s", keepalive?"true":"false");
...
bool CacheImpl::isCacheDestroyPending() const{
  return m_destroyPending;
}

void CacheImpl::setDefaultPool(PoolPtr pool) {
  m_defaultPool = pool;
}

PoolPtr CacheImpl::getDefaultPool(){
  return m_defaultPool;
}

void CacheImpl::validateRegionAttributes( const char* name,
    const RegionAttributesPtr& attrs ) const
{
  ...
  if ( kind != CPP_REGION ) {
    if ( attrs->m_scope == ScopeType::LOCAL ) {
      buffer += "A native client region cannot have LOCAL scope";
      throw UnsupportedOperationException( buffer.c_str( ) );
    }
  } else{
    if ( attrs->m_scope == ScopeType::DISTRIBUTED_ACK || attrs->m_scope == ScopeType::DISTRIBUTED_NO_ACK ) {
      buffer += "Missing endpoints for non-LOCAL scope";
      throw UnsupportedOperationException( buffer.c_str( ) );
    }
  }
  if ( attrs->m_clientNotificationEnabled && kind == CPP_REGION ) {
    buffer += "Client notification can be enabled only for native client region";
    throw UnsupportedOperationException( buffer.c_str( ) );
  }
}
    ...
    const RegionAttributesPtr& aRegionAttributes, RegionPtr& regionPtr)
{
  ...
  if(aRegionAttributes == NULLPTR)
    throw IllegalArgumentException("Cache::createRegion: RegionAttributes is null");
  std::string namestr( name );

  if ( namestr.find( '/' ) != std::string::npos ) {
    throw IllegalArgumentException("Malformed name string, contains region path seperator '/'" );
  }
  ...
  RegionInternal* rpImpl = NULL;
  {
    ...
    //hard to be atomic since a regionImpl needs to be valid before it can be bound
    MapOfRegionGuard guard1( m_regions->mutex() );
    RegionPtr tmp;
    if(0 == m_regions->find(namestr, tmp))
    {
      char buffer[256];
      ACE_OS::snprintf( buffer, 256, "Cache::createRegion: \"%s\" region exists in local cache", namestr.c_str());
...
//      LOGERROR( "Cache::createRegion: region creation failed, caught exception: %s", ex.getMessage() );
//      ex.printStackTrace();
      throw;
} catch ( std::exception & ex ) {
      char buffer[512];
      ACE_OS::snprintf( buffer, 512, "Cache::createRegion: Failed to create Region \"%s\" due to unknown exception: %s", namestr.c_str(), ex.what());
      throw UnknownException( buffer );
} catch (...) {
      char buffer[256];
      ACE_OS::snprintf( buffer, 256, "Cache::createRegion: Failed to create Region \"%s\" due to unknown exception.", namestr.c_str());
      throw UnknownException( buffer );
}
    if(rpImpl==NULL)
    {
      char buffer[256];
      ACE_OS::snprintf( buffer, 256, "Cache::createRegion: Failed to create Region \"%s\"", namestr.c_str());
    ...
    if ( aRegionAttributes->getDiskPolicy() == DiskPolicyType::OVERFLOWS ) {
      ...
      if (pmPtr == NULLPTR) {
        throw NullPointerException("PersistenceManager could not be instantiated");
      }
    ...
    //When region is created, added that region name in client meta data service to fetch its 
    ...
    if ( !props->isGridClient() ) {
      ...
      if (poolName != NULL) {
        PoolPtr pool = PoolManager::find(poolName);
        if (pool != NULLPTR && !pool->isDestroyed() && pool->getPRSingleHopEnabled())
        {
          ThinClientPoolDM* poolDM = dynamic_cast< ThinClientPoolDM* >(pool.ptr());
          if ((poolDM != NULL) && (poolDM->getClientMetaDataService() != NULL)) {
            LOGFINE("enqueued region %s for initial metadata refresh for singlehop ", name);
            poolDM->getClientMetaDataService()->enqueueForMetadataRefresh( regionPtr->getFullPath(), 0);
          }
        }
      }
    }
  ...
  //   LOGFINE( "Returning from CacheImpl::createRegion call for Region %s", regionPtr->getFullPath() );
...
RegionInternal* CacheImpl::createRegion_internal(const std::string& name,
    RegionInternal* rootRegion, const RegionAttributesPtr& attrs,
    const CacheStatisticsPtr& csptr, bool shared)
{
  if (attrs == NULLPTR) {
    throw IllegalArgumentException("createRegion: "
  ...
  const char * cacheEndpoints = m_attributes==NULLPTR ? NULL : m_attributes->getEndpoints();
  ...
  if (poolName != NULL) {
     ...
     if (pool != NULLPTR && !pool->isDestroyed())
     {
       ...
       if (isMultiUserSecureMode && 
         (attrs->getCachingEnabled() || 
           //attrs->getClientNotificationEnabled() ||
           attrs->getScope() == ScopeType::LOCAL)) {
             LOGERROR("Pool [%s] is in multiuser authentication mode so region local caching or local scope are not supported.", poolName);
             throw IllegalStateException("Pool is in multiuser authentication so region local caching or local scope are not supported.");
       }
    }
  }
  ...
  if ( poolName && strlen(poolName) &&
    ...
    LOGERROR("Cache or region endpoints cannot be specified when pool name is specified for region %s", name.c_str());
    throw IllegalArgumentException( "Cache or region endpoints cannot be specified when pool name is specified" );
  }

  DeleteObject<RegionInternal> delrptr(rptr);
  if (regionKind == THINCLIENT_REGION) {
    LOGINFO("Creating region %s with region endpoints %s",
        name.c_str(), attrs->getEndpoints());
    GF_NEW(rptr, ThinClientRegion(name, this, rootRegion, attrs,
            csptr, shared));
    ...
    GF_NEW(rptr, ThinClientHARegion(name, this, rootRegion, attrs,
            csptr, shared));
    ((ThinClientHARegion*)rptr)->initTCR();
    delrptr.noDelete();
  }
  else if (attrs->getScope() == ScopeType::LOCAL) {
    LOGINFO("Creating local region %s", name.c_str());
    GF_NEW(rptr, LocalRegion(name, this, rootRegion, attrs,
            csptr, shared));
    delrptr.noDelete();
  }
  else if ( regionKind == THINCLIENT_POOL_REGION ) {
    LOGINFO("Creating region %s attached to pool %s", name.c_str(), attrs->getPoolName());
    GF_NEW(rptr, ThinClientPoolRegion(name, this, rootRegion, attrs,
            csptr, shared));
    ((ThinClientPoolRegion*)rptr)->initTCR();
    delrptr.noDelete();
  }
  else {
    LOGERROR("Failed to create region because endpoints were not specified and "
      "the region is not defined to have local scope");
    throw IllegalArgumentException(" Failed to create region because "
        "endpoints were not specified and the region is not defined "
        "to have local scope");
  }
  return rptr;
}
...
void CacheImpl::rootRegions(VectorOfRegion& regions)
{
   ...
   for(MapOfRegionWithLock::iterator q=m_regions->begin(); q!=m_regions->end(); ++q)
   {
...
{
  bool autoReadyForEvents = DistributedSystem::getSystemProperties()->autoReadyForEvents();
 	bool isDurable = m_tcrConnectionManager->isDurable();

  if( !isDurable && autoReadyForEvents ) {
    LOGERROR("Only durable clients or clients with the "
 	    "auto-ready-for-events property set to false should call readyForEvents()");
 	  throw IllegalStateException("Only durable clients or clients with the "
 	    "auto-ready-for-events property set to false should call readyForEvents()");
  }
  ...
  const HashMapOfPools& pools = PoolManager::getAll( );
  if ( pools.empty( ) )
    throw IllegalStateException( "No pools found." );
  PoolPtr currPool = NULLPTR;
  for ( HashMapOfPools::Iterator itr = pools.begin( ); itr != pools.end( ); itr++ ) {
    currPool = itr.second( );
    ...
    try {
      ...
      LOGWARN( "readyForEvents( ) failed for pool %s with exception: %s", currPool->getName( ), ex.getMessage( ) );
    }
  }
}
...
bool CacheImpl::getEndpointStatus(const std::string& endpoint)
{
  ...
  /*
  fullName = endpoint.find(':') == std::string::npos ? endpoint : Utils::convertHostToCanonicalForm(endpoint.c_str() );
  */
  fullName = endpoint;
  ...
  fullName = endpoint.find(':') == std::string::npos ? endpoint : Utils::convertHostToCanonicalForm(endpoint.c_str() );  
  ThinClientPoolDMPtr currPool = staticCast< ThinClientPoolDMPtr >( pools.begin( ).second( ) );
  ACE_Guard< ACE_Recursive_Thread_Mutex > guard( currPool->m_endpointsLock );
  for ( ACE_Map_Manager< std::string, TcrEndpoint *, ACE_Recursive_Thread_Mutex >::iterator itr = currPool->m_endpoints.begin( );
...
void CacheImpl::processMarker( )
{
  ...
  for( MapOfRegionWithLock::iterator q = m_regions->begin( ); q != m_regions->end( ); ++q )
  {
    ...
    {
      ThinClientHARegion* tcrHARegion = dynamic_cast< ThinClientHARegion* >( ( *q ).int_id_.ptr( ) );
      if (tcrHARegion != NULL) {
        ...
        VectorOfRegion subregions;
 	      ...
 	      for (VectorOfRegion::Iterator iter = subregions.begin(); iter != subregions.end(); ++iter) {
 	        if (!(*iter)->isDestroyed()) {
 	          ThinClientHARegion* subregion = dynamic_cast< ThinClientHARegion* >((*iter).ptr());
 	          if (subregion != NULL) {
 	            regionMsg = new TcrMessage( );
 	            regionMsg->setMessageType( TcrMessage::CLIENT_MARKER );
...
// Version ordinal accessor for unit tests
int8_t CacheImpl::getVersionOrdinalForTest()
{
  return Version::m_ordinal;
}
...
RegionFactoryPtr CacheImpl::createRegionFactory(RegionShortcut preDefinedRegion)
{
  RegionFactoryPtr rfPtr(new RegionFactory(preDefinedRegion));
  return rfPtr;
}

void CacheImpl::setRegionShortcut(AttributesFactoryPtr attrFact, RegionShortcut preDefinedRegionAttr)
{
  ...
  case LOCAL:
    {
      attrFact->setScope(ScopeType::LOCAL);
    }
    break;
...
std::map<std::string, RegionAttributesPtr> CacheImpl::getRegionShortcut()
{
  ...
  regAttr_LOCAL->setScope(ScopeType::LOCAL);
  ...
  regAttr_LOCAL_LRU->setScope(ScopeType::LOCAL);
