...
PoolAttributes::PoolAttributes():
  ...
  m_msgTrackTimeout(PoolFactory::DEFAULT_SUBSCRIPTION_MESSAGE_TRACKING_TIMEOUT),
  ...
  m_updateLocatorListInterval(PoolFactory::DEFAULT_UPDATE_LOCATOR_LIST_INTERVAL),
...
bool PoolAttributes::operator == ( const PoolAttributes& other ) const
{
  ...
  if ( m_updateLocatorListInterval != other.m_updateLocatorListInterval ) return false;
  ...
  if ( 0 != compareStringAttribute(const_cast<char *> (m_serverGrp.c_str()), const_cast<char *> (other.m_serverGrp.c_str()) ) ) return false;
  ...
  if(!compareVectorOfStrings(m_initServList, other.m_initServList)) return false;
  
  return true;
}

bool PoolAttributes::compareVectorOfStrings(const std::vector<std::string>& thisVector1,const std::vector<std::string>& otherVector1)
{
  std::vector<std::string>& thisVector = *(const_cast<std::vector<std::string> *>(&thisVector1));
  std::vector<std::string>& otherVector = *(const_cast<std::vector<std::string> *>(&otherVector1));
  ...
  for ( it = thisVector.begin() ; it < thisVector.end(); it++ )
  {
    ...
    for ( itOther = otherVector.begin() ; itOther < otherVector.end(); itOther++ )
    {
...
int32_t PoolAttributes::compareStringAttribute(const char* attributeA,const  char* attributeB) {
...
void PoolAttributes::addLocator(const char* host, int port) {
    if (m_initServList.size()) {
      throw IllegalArgumentException("Cannot add both locators and servers to a pool");
    }
    char buff[128] = {'\0'};
