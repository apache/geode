...
namespace gemfire
{
  ...
  template < class T, class MUTEX = ACE_Thread_Mutex > class FairQueue
  {
    public:
    ...
    protected:
      ...
      template< typename U > T* getUntilWithToken( int64_t& sec, bool& isClosed, U* excludeList = NULL )
      {
        T* mp = NULL;
        ...
        if ( !m_closed ) {
          ...
          } while ( mp == NULL && ( currTime = ACE_OS::gettimeofday( ) ) <
              stopAt && !isClosed );
