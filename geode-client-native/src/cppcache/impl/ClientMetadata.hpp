...
namespace gemfire {
  ...
  //typedef std::map<int,BucketServerLocationsType > BucketServerLocationsListType;
  ...
  private:
   ...
   // ACE_RW_Thread_Mutex m_readWriteLock;
    ...
    inline void checkBucketId(int bucketId) {
      if (bucketId >= (int) m_bucketServerLocationsList.size()) {
        LOGERROR("ClientMetadata::getServerLocation(): BucketId out of range.");
        throw IllegalStateException("ClientMetadata::getServerLocation(): BucketId out of range.");
      }
    }
    
  public:
    void setPreviousone(ClientMetadataPtr cptr) {
      m_previousOne = cptr;
    }
    ~ClientMetadata();
    ClientMetadata();
    ClientMetadata(int totalNumBuckets, CacheableStringPtr colocatedWith, ThinClientPoolDM* tcrdm, std::vector<FixedPartitionAttributesImplPtr>* fpaSet);
    void getServerLocation(int bucketId, bool tryPrimary, BucketServerLocationPtr& serverLocation,  int8_t& version);
    //ServerLocation getPrimaryServerLocation(int bucketId);
    void updateBucketServerLocations(int bucketId, BucketServerLocationsType bucketServerLocations);
    ...
    int assignFixedBucketId(const char* partitionName, CacheableKeyPtr resolvekey);
