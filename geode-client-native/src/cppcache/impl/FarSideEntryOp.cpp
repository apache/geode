...
FarSideEntryOp::FarSideEntryOp(RegionCommit* region):
       ...
       * 
       */
       m_didDestroy(false), m_eventOffset(0), m_modSerialNum(0),m_op(0)
...
{
	return op == DESTROY || op == LOCAL_DESTROY || op == EVICT_DESTROY || op == EXPIRE_DESTROY || op == EXPIRE_LOCAL_DESTROY || op == REMOVE;
}

bool FarSideEntryOp::isInvalidate(int8_t op)
{
	return op == INVALIDATE || op == LOCAL_INVALIDATE || op == EXPIRE_INVALIDATE || op == EXPIRE_LOCAL_INVALIDATE;
}

void FarSideEntryOp::fromData(DataInput& input, bool largeModCount, uint16_t memId)
{
	...
	} else if(structType == GemfireTypeIdsImpl::DataSerializable)
	{
      ...
      */
      ...
      if (hasCQs) {
		...
		LOGERROR("FarSideEntryOp::skipFilterRoutingInfo Unexpected type id: %d while desirializing commit response", structType);
		GfErrTypeThrowException("FarSideEntryOp::skipFilterRoutingInfo Unable to handle commit response", GF_CACHE_ILLEGAL_STATE_EXCEPTION);
	}
}
/*
