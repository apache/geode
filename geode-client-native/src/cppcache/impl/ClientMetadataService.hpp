...
namespace gemfire {
  ...
  class ClientMetadataService : public ACE_Task_Base, private NonCopyable, private NonAssignable
  {
  ...
  public:
    ...
    void getBucketServerLocation(const RegionPtr& region, const CacheableKeyPtr& key, 
    ...
    void populateDummyServers(const char * regionName, ClientMetadataPtr clientmetadata);

    void enqueueForMetadataRefresh(const char* regionFullPath, int8 serverGroupFlag);

    HashMapT<BucketServerLocationPtr, VectorOfCacheableKeyPtr>* getServerToFilterMap(const VectorOfCacheableKey* keys,
    		const RegionPtr& region, bool isPrimary);

    void markPrimaryBucketForTimeout(const RegionPtr& region, const CacheableKeyPtr& key,
    const CacheablePtr& value,const UserDataPtr& aCallbackArgument, bool isPrimary, BucketServerLocationPtr& serverLocation,  int8_t& version);
    ...
    BucketServerLocationPtr findNextServer(HashMapT<BucketServerLocationPtr, CacheableHashSetPtr>* serverToBucketsMap, CacheableHashSetPtr& currentBucketSet);

    HashMapT<CacheableInt32Ptr, CacheableHashSetPtr>* groupByBucketOnClientSide(const RegionPtr& region, CacheableVectorPtr* keySet, ClientMetadataPtr& metadata);

    HashMapT<BucketServerLocationPtr, CacheableHashSetPtr>* getServerToFilterMapFESHOP(CacheableVectorPtr* keySet,
      const RegionPtr& region, bool isPrimary);

    HashMapT<BucketServerLocationPtr, CacheableHashSetPtr>* groupByServerToAllBuckets(const RegionPtr& region, bool optimizeForWrite);

    HashMapT<BucketServerLocationPtr, CacheableHashSetPtr>* groupByServerToBuckets(ClientMetadataPtr& metadata, CacheableHashSetPtr& bucketSet, bool optimizeForWrite);

    HashMapT<BucketServerLocationPtr, CacheableHashSetPtr>* pruneNodes(ClientMetadataPtr& metadata, CacheableHashSetPtr& totalBuckets);    
   
  private:   

    //const PartitionResolverPtr& getResolver(const RegionPtr& region, const CacheableKeyPtr& key,
      //const UserDataPtr& aCallbackArgument);

    //BucketServerLocation getServerLocation(ClientMetadataPtr cptr, int bucketId, bool isPrimary);

    ClientMetadataPtr SendClientPRMetadata(const char* regionPath, ClientMetadataPtr cptr);
