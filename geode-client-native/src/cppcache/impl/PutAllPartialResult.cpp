...
namespace gemfire {
  ...
  PutAllPartialResult::PutAllPartialResult(int totalMapSize, ACE_Recursive_Thread_Mutex& responseLock)
  {
    m_succeededKeys = new VersionedCacheableObjectPartList(new VectorOfCacheableKey(), responseLock);
  ...
  void PutAllPartialResult::addKeysAndVersions(VersionedCacheableObjectPartListPtr keysAndVersion)
  {
  ...
  {
    {
      ...
      if(m_succeededKeys->getVersionedTagsize() > 0){
        throw IllegalStateException("attempt to store versionless keys when there are already versioned results");
      }
      this->m_succeededKeys->addAllKeys(m_keys);
    }
  }

  VersionedCacheableObjectPartListPtr PutAllPartialResult::getSucceededKeysAndVersions() {
