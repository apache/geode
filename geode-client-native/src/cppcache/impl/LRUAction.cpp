...
LRUAction* LRUAction::newLRUAction( const LRUAction::Action& actionType, RegionInternal* regionPtr, LRUEntriesMap* entriesMapPtr )
{
...
bool LRUOverFlowToDiskAction::evict(const MapEntryImplPtr& mePtr)
{
  if(m_regionPtr->isDestroyed()) {
    LOGERROR("[internal error] :: OverflowAction: region is being destroyed, so not evicting entries");
    return false;
  }
  ...
  if (valuePtr == NULLPTR) {
    LOGERROR("[internal error]:: OverflowAction: destroyed entry added to "
        "LRU list");
    throw FatalInternalException("OverflowAction: destroyed entry added to "
        "LRU list");
  }
  ...
  if (m_entriesMapPtr != NULL) {
    int64_t newSize = CacheableToken::overflowed()->objectSize() -
        valuePtr->objectSize();
...
bool LRULocalInvalidateAction::evict(const MapEntryImplPtr& mePtr)
{
  ...
  CacheableKeyPtr keyPtr;
  mePtr->getKeyI(keyPtr);
  // [sumedh] we should invoke the invalidateNoThrow with appropriate
  ...
  if (!m_regionPtr->isDestroyed()) {
    err = m_regionPtr->invalidateNoThrow(keyPtr, NULLPTR, -1,
