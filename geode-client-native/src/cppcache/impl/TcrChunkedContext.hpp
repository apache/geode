...
namespace gemfire
{
  ...
  public:
    inline TcrChunkedResult() :
      m_finalizeSema(NULL), m_ex(NULLPTR), m_inSameThread(false),
      /* Non-static class member ""m_dsmemId"" is not initialized in this 
    ...
    virtual void handleChunk(const uint8_t* bytes, int32_t len, uint8_t isLastChunkWithSecurity) = 0;
    ...
    inline bool exceptionOccurred() const
    {
      return (m_ex != NULLPTR);
    }
    ...
    inline ExceptionPtr& getException() 
    {
      return m_ex;
    }
  };
  ...
  public:
    ...
    m_bytes(bytes), m_len(len), m_result(result), m_isLastChunkWithSecurity(isLastChunkWithSecurity)
    {
    }

    inline ~TcrChunkedContext()
    {
      GF_SAFE_DELETE_ARRAY(m_bytes);
    }

    inline const uint8_t* getBytes() const
    {
      return m_bytes;
    }
    ...
    void handleChunk(bool inSameThread)
    {
      if (m_bytes == NULL) {
        ...
        try {
          m_result->handleChunk(m_bytes, m_len, m_isLastChunkWithSecurity);
        }
        catch (Exception& ex) {
          LOGERROR("HandleChunk error message %s, name = %s", ex.getMessage(), ex.getName());
          ...
          exMsg += Utils::demangleTypeName(typeid(
              *m_result).name())->asChar();
          exMsg += "::handleChunk while processing response, possible serialization mismatch";
