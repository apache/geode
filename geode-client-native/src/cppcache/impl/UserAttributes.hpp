...
namespace gemfire {
  ...
  public:
    ...
    bool isAuthenticated()
    {
      ...
      return m_isAuthenticated && (m_connectedEndpoint->numberOfTimesFailed() == m_numberOfTimesEndpointFailed);
    }
  ...
  {
    //TODO:hitesh need to add lock here so that user should not be authenticated at two servers
    public:
      
      ~UserAttributes();
      UserAttributes(PropertiesPtr credentials, PoolPtr pool, ProxyCache * proxyCache);
  ...
  public:
    ...
    void setUserAttributes(UserAttributesPtr userAttr) { m_userAttribute = userAttr; }
