    ...
    const void* arg)
{
  ...
  try {
    ...
    int32_t sec = (int32_t)curr_time - lastTimeForExp - m_duration;
    LOGDEBUG("Entered region expiry task handler for region [%s]: %d,%d,%d,%d",
        m_regionPtr->getFullPath(), curr_time, lastTimeForExp,
        m_duration, sec);
    if (sec >= 0) {
...
int RegionExpiryHandler::handle_close(ACE_HANDLE handle, ACE_Reactor_Mask close_mask)
{
  // [sumedh] we now delete the handler in GF_Timer_Heap_ImmediateReset_T
  //delete this;
  return 0;
...
void RegionExpiryHandler::DoTheExpirationAction()
{
  switch (m_action) {
    case ExpirationAction::INVALIDATE: {
      LOGDEBUG("RegionExpiryHandler::DoTheExpirationAction INVALIDATE "
      "region [%s]", m_regionPtr->getFullPath());
      m_regionPtr->invalidateRegionNoThrow(NULLPTR,
          CacheEventFlags::EXPIRATION);
      break;
    }
    case ExpirationAction::LOCAL_INVALIDATE: {
      LOGDEBUG("RegionExpiryHandler::DoTheExpirationAction LOCAL_INVALIDATE "
          "region [%s]", m_regionPtr->getFullPath());
      m_regionPtr->invalidateRegionNoThrow(NULLPTR,
          CacheEventFlags::EXPIRATION | CacheEventFlags::LOCAL);
      break;
    }
    case ExpirationAction::DESTROY: {
      LOGDEBUG("RegionExpiryHandler::DoTheExpirationAction DESTROY "
          "region [%s]", m_regionPtr->getFullPath());
    ...
    case ExpirationAction::LOCAL_DESTROY: {
      LOGDEBUG("RegionExpiryHandler::DoTheExpirationAction LOCAL_DESTROY "
          "region [%s]", m_regionPtr->getFullPath());
      m_regionPtr->destroyRegionNoThrow(NULLPTR, true,
          CacheEventFlags::EXPIRATION | CacheEventFlags::LOCAL);
      break;
    ...
    default: {
      LOGERROR("Unknown expiration action "
          "%d for region [%s]", m_action, m_regionPtr->getFullPath());
      break;
    }
  }
}
