...
namespace gemfire
{
  ...
  public:
    ...
    virtual void cleanup(const CacheEventFlags eventFlags)
    {
      if (!eventFlags.isEviction()) {
        // TODO: [sumedh] this needs an implementation of doubly-linked list
  ...
  protected:
    ...
    inline LRUMapEntry(const CacheableKeyPtr& key) :
      MapEntryImpl(key)
    {
    }
  ...
  protected:
    inline explicit VersionedLRUMapEntry(bool noInit) :
      LRUMapEntry(true)
    {
    }

    inline VersionedLRUMapEntry(const CacheableKeyPtr& key) :
      LRUMapEntry(key)
    {
    }
