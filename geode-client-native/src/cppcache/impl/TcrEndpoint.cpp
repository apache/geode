...
TcrEndpoint::TcrEndpoint( const std::string& name, CacheImpl* cache,
    ...
    ACE_Semaphore& redundancySema, ThinClientBaseDM* DM, bool isMultiUserMode)
:m_name(name),
m_connected(false),
m_numRegions(0),
m_maxConnections(DistributedSystem::getSystemProperties()->javaConnectionPoolSize()),
...
TcrEndpoint::~TcrEndpoint()
{
  ...
  closeConnections();
  {
    ...
    if (m_numRegionListener > 0) {
      LOGFINE("Connection to %s still has references "
          "to subscription channel while closing", m_name.c_str());
  ...
  while (m_notifyCount > 0) {
    LOGDEBUG( "TcrEndpoint::~TcrEndpoint(): reducing notify count at %d", m_notifyCount );
...
inline bool TcrEndpoint::needtoTakeConnectLock()
{
    ...
    if(DistributedSystem::getSystemProperties()->connectWaitTimeout() > 0) {
      return m_needToConnectInLock;// once pipe or other socket error will take lock to connect.
    }
...
GfErrType TcrEndpoint::createNewConnectionWL(TcrConnection*& newConn, bool isClientNotification, bool isSecondary, uint32_t connectTimeout)
{
  LOGFINE("TcrEndpoint::createNewConnectionWL");
  uint32_t connectWaitTimeout = DistributedSystem::getSystemProperties()->connectWaitTimeout() * 1000;//HItesh need to change
  ...
  {        
    ...
    LOGFINE("TcrEndpoint::createNewConnectionWL ret = %d interval = %ld error =%s" , ret, interval.get_msec(), ACE_OS::strerror(ACE_OS::last_error( )));
    ...
    { //got lock
      try
      {
        ...
        newConn->InitTcrConnection( this, m_name.c_str(), m_ports,
            isClientNotification, isSecondary, connectTimeout);
      ...
      {
        m_connectLock.release();
        LOGWARN( "Failed1 in handshake with endpoint[%s]: %s",
...
GfErrType TcrEndpoint::createNewConnection( TcrConnection*& newConn,
    bool isClientNotification, bool isSecondary, uint32_t connectTimeout,
    int32_t timeoutRetries, bool sendUpdateNotification, bool appThreadRequest )
{
  LOGFINE("TcrEndpoint::createNewConnection: connectTimeout =%d m_needToConnectInLock=%d appThreadRequest =%d", connectTimeout, m_needToConnectInLock, appThreadRequest);
  ...
  while ( timeoutRetries-- >= 0 ) {
    try {
      if ( newConn == NULL ) {
        ...
        newConn = new TcrConnection(m_connected);
        bool authenticate = newConn->InitTcrConnection( this, m_name.c_str(), m_ports,
            isClientNotification, isSecondary, connectTimeout);
        if(authenticate) {
          authenticateEndpoint(newConn);
        }
        } else {
          err = createNewConnectionWL(newConn,isClientNotification, isSecondary, connectTimeout);
      ...
      if ( !isClientNotification && sendUpdateNotification ) {
        ...
        if ( notificationStarted ) {
          LOGFINE( "Sending update notification message to endpoint %s",
              m_name.c_str( ) );
          TcrMessage updateNotificationMsg( (int32_t)newConn->getPort( ) );
      ...
      LOGWARN( "Exception in handshake on server[%s]: %s",
          m_name.c_str( ), ex.getMessage( ) );
      err = GF_CACHESERVER_EXCEPTION;
      break;
    } catch ( const Exception& ex ) {
      LOGWARN( "Failed in handshake with endpoint[%s]: %s",
          m_name.c_str( ), ex.getMessage( ) );
      err = GF_MSG;
      break;
    }catch(std::exception& ex)
...
{
  LOGDEBUG("TcrEndpoint::authenticateEndpoint m_isAuthenticated  = %d this->m_baseDM = %d",m_isAuthenticated, this->m_baseDM);
  if(!m_isAuthenticated && this->m_baseDM)
  {
    ...
    ACE_Guard< ACE_Recursive_Thread_Mutex > guard( m_endpointAuthenticationLock );
      ...
      if (creds != NULLPTR) {
        LOGDEBUG("TcrEndpoint::authenticateEndpoint got creds from app = %d", creds->getSize());
      }
      else {
        LOGDEBUG("TcrEndpoint::authenticateEndpoint no creds from app ");
      }
      
      TcrMessage request(TcrMessage::USER_CREDENTIAL_MESSAGE, creds, this->m_baseDM);
      
      LOGDEBUG("request is created");
      TcrMessage reply(true, this->m_baseDM);
      ...
      if (err == GF_NOERR) 
      {
        ...
        switch ( reply.getMessageType( ) )
        {
          ...
          case TcrMessage::EXCEPTION: {
            err = ThinClientRegion::handleServerException("AuthException", reply.getException());
            break;
          }
          default: {
...
PropertiesPtr TcrEndpoint::getCredentials()
{
  ...
  if (authInitialize != NULLPTR) {
    LOGFINER("Acquired handle to AuthInitialize plugin, "
        "getting credentials for %s", m_name.c_str());
    /* adongre
     * CID 28899: Copy into fixed size buffer (STRING_OVERFLOW) 
     * You might overrun the 100 byte fixed-size string "tmpEndpoint" by copying the return value of 
     * "stlp_std::basic_string<char, stlp_std::char_traits<char>, stlp_std::allocator<char> >::c_str() const" without checking the length.  
     */
    //char tmpEndpoint[100] = { '\0' } ;
    //strcpy(tmpEndpoint, m_name.c_str());
    PropertiesPtr tmpAuthIniSecurityProperties =
      authInitialize->getCredentials( tmpSecurityProperties, /*tmpEndpoint*/ m_name.c_str() ); 
...
ServerQueueStatus TcrEndpoint::getFreshServerQueueStatus( int32_t& queueSize, bool addToQueue, TcrConnection*& statusConn )
{
  GfErrType err = GF_NOERR;
  TcrConnection* newConn;
  ServerQueueStatus status = NON_REDUNDANT_SERVER;

  err = createNewConnection( newConn, false, false, DistributedSystem::getSystemProperties()->connectTimeout() );
  if ( err == GF_NOERR ) {
    ...
    if ( status == REDUNDANT_SERVER || status == PRIMARY_SERVER ) {
      ...
      // ARB: remove port from ports list (which is sent to server in notification handshake).
...
GfErrType TcrEndpoint::registerDM( bool clientNotification,
    bool isSecondary, bool isActiveEndpoint, ThinClientBaseDM * distMgr )
{
  // Pre-conditions:
  ...
  if ( m_maxConnections <= 0 ) {
    ...
    if ( maxConnections > 0 ) {
      LOGINFO( "Starting Handshake with %s%s", ( isSecondary ?
            "secondary server " : ( isActiveEndpoint ? "" :
              "primary server " ) ), m_name.c_str( ) );
      for ( int connNum = 0; connNum < maxConnections; ++connNum ) {
        TcrConnection* newConn;
        if ( ( err = createNewConnection( newConn, false, false,
                DistributedSystem::getSystemProperties()->connectTimeout(), 0, m_connected ) ) != GF_NOERR ) {
          m_connected = false;
          m_isActiveEndpoint = false;
          closeConnections( );
          return err;
      ...
      LOGINFO( "Handshake with %s%s success", ( isSecondary ?
            "secondary server " : ( isActiveEndpoint ? "" :
              "primary server " ) ), m_name.c_str( ) );
      m_connected = true;
  ...
  if ( m_connected || connected ) {
    if ( clientNotification ) {
      ...
      LOGFINEST("Registering subscription "
          "channel for endpoint %s", m_name.c_str());
      ...
      if ( m_numRegionListener == 0 ) {
        if ( ( err = createNewConnection( m_notifyConnection, true,
          ...
          m_isActiveEndpoint = false;
          closeConnections( );
          LOGWARN("Failed to start subscription channel for endpoint %s", m_name.c_str());
          return err;
        }
        m_notifyReceiver = new GF_TASK_T<TcrEndpoint>( this,
            &TcrEndpoint::receiveNotification, NC_Notification );
  ...
  if( !m_isActiveEndpoint && !isActiveEndpoint && m_connected &&
    ...
    LOGWARN( "Inactive connected endpoint does not have exactly one "
        "connection. Number of connections: %d, number of region listeners: "
        "%d", numConnections, m_numRegionListener );
  }
#endif
...
void TcrEndpoint::unregisterDM( bool clientNotification, ThinClientBaseDM * distMgr, bool checkQueueHosted )
{
  if ( clientNotification ) {
    LOGFINEST("Closing subscription "
      "channel for endpoint %s", m_name.c_str());
...
void TcrEndpoint::pingServer(ThinClientPoolDM* poolDM)
{
  ...
  if ( !m_msgSent && !m_pingSent ) {
    TcrMessage * pingMsg = TcrMessage::getPingMessage();
    TcrMessage reply;
    ...
    if (error == GF_TIMOUT && m_pingTimeouts < 2) {
      ...
      m_pingTimeouts = 0;
    // VJR: Only call setConnectionStatus if the status has changed (non thread safe check)
    // This is to avoid blocking the ping thread if notification channel takes a long time to
    // complete causing the server to drop the client in the midst of connection establishment.
      bool connected = (error == GF_NOERR) ? (reply.getMessageType() == TcrMessage::REPLY) : false;
      if (m_connected != connected) {
  ...
  while (isRunning) {
    ...
    try {
      ...
      if(opErr == CONN_IOERR) {
        ...
        LOGFINER("IO exception while receiving subscription event for endpoint %d",
          opErr);
        if (isRunning) {
      ...
      if (data) {
        msg = new TcrMessage();
        msg->initCqMap();
        msg->setData(data, static_cast<int32_t> (dataLen), this->getDistributedMemberID());
        ...
        if (!msg->hasCqPart()) {
          if ( msg->getMessageType() != TcrMessage::CLIENT_MARKER ) {
            ...
            if (region1 != NULLPTR && !static_cast<ThinClientRegion *>(
                region1.ptr())->getDistMgr()->isEndpointAttached(this)) {
              // drop event before even processing the eventid for duplicate checking
          ...
          if(!msg->hasCqPart())// || msg->isInterestListPassed())
	  {
             ...
             if (region != NULLPTR) {
               static_cast<ThinClientRegion *>(region.ptr())->receiveNotification(msg);
             }
             else {
            LOGWARN("Notification for region %s that does not exist in "
                "client cache.", regionFullPath.c_str());
             }
	  }
	  else {
            LOGDEBUG("receive cq notification %d", msg->getMessageType());
              ...
              if (queryService != NULLPTR) {
	              static_cast<RemoteQueryService*>(queryService.ptr())->receiveNotification( msg );
	            }
	  }
        }
      }
      ...
      LOGDEBUG("receiveNotification timed out: no data received from "
          "endpoint %s", m_name.c_str());
    }
    catch (const GemfireIOException & e) {
      // Endpoint is disconnected, this exception is expected
      LOGFINER("IO exception while receiving subscription event for endpoint %s: %s",
      ...
      GF_SAFE_DELETE(msg);
      LOGERROR("Exception while receiving subscription event for endpoint %s:: %s: %s",
          m_name.c_str(), ex.getName(), ex.getMessage());
    }
    catch (...) {
      GF_SAFE_DELETE(msg);
      LOGERROR("Unexpected exception while "
          "receiving subscription event from endpoint %s", m_name.c_str());
    }
  }
...
inline bool TcrEndpoint::compareTransactionIds(int32_t reqTransId,
    int32_t replyTransId, std::string& failReason, TcrConnection* conn)
{
  LOGDEBUG("TcrEndpoint::compareTransactionIds requested id = %d ,replied = %d",reqTransId, replyTransId );
  if ( replyTransId != reqTransId ) {
    LOGERROR( "Transaction ids do not match on endpoint %s for "
        "send operation: %d, %d. Possible serialization mismatch", m_name.c_str( ), reqTransId, replyTransId );
    ...
    TcrConnection*& conn, bool isBgThread )
{
  ...
  LOGFINE("IO error during send for endpoint %s "
      "[errno: %d: %s]: %s", m_name.c_str(), lastError,
      ACE_OS::strerror(lastError), message.c_str());
  //EAGAIN =11, EWOULDBLOCK = 10035L, EPIPE = 32, ECONNRESET =10054L(An existing connection was forcibly closed by the remote host.)
...
GfErrType TcrEndpoint::sendRequestConn(const TcrMessage &request, TcrMessage & reply, TcrConnection* conn, std::string& failReason )
{
  int32_t type = request.getMessageType();
  GfErrType error = GF_NOERR;

  LOGFINER("Sending request type %d to endpoint [%s] via connection [%p]", type, m_name.c_str(), conn);
  //TcrMessage * req = const_cast<TcrMessage *>(&request);
  LOGDEBUG("TcrEndpoint::sendRequestConn  = %d",m_baseDM);
  if (m_baseDM != NULL)
    m_baseDM->beforeSendingRequest(request, conn);
  if(((type == TcrMessage::EXECUTE_FUNCTION ||
      type == TcrMessage::EXECUTE_REGION_FUNCTION) &&  (request.hasResult() & 2 )))
  {
     sendRequestForChunkedResponse(request, reply, conn);
      ...
      type == TcrMessage::EXECUTE_REGION_FUNCTION) &&  (request.hasResult() & 2 ))||
      type == TcrMessage::EXECUTE_REGION_FUNCTION_SINGLE_HOP ||//This is kept aside as server always sends chunked response.
    ...
    //Chk request type to request if so request.getCallBackArg flag & setCall back arg flag to true, and in response chk for this flag.
    ...
    char* data = conn->sendRequest(request.getMsgData(),
      request.getMsgLength(), &dataLen, request.getTimeout(), reply.getTimeout(), request.getMessageType());
    reply.setMessageTypeRequest(type);
    reply.setData(data, static_cast<int32_t>(dataLen), this->getDistributedMemberID()); // memory is released by TcrMessage setData().
  }
  ...
  if ( reply.getMessageType( ) == TcrMessage::INVALID ) {
    ...
    {
          ChunkedFunctionExecutionResponse* resultCollector = dynamic_cast<ChunkedFunctionExecutionResponse*>(reply.getChunkedResultHandler());
 ...
 //            m_opConnections.put( conn, false );
  ...
  else if ( !compareTransactionIds( request.getTransId( ),
		  reply.getTransId( ), failReason, conn ) ) {
    error = GF_NOTCON;
  }
  if (error == GF_NOERR) {
    if(m_baseDM != NULL)
...
GfErrType TcrEndpoint::sendRequestWithRetry( const TcrMessage &request, TcrMessage & reply, TcrConnection*& conn,
    bool& epFailure, std::string& failReason, int maxSendRetries, bool useEPPool, int64_t requestedTimeout, bool isBgThread )
{
  ...
  // ARB: Retry on the following send errors:
  // Timeout: 1 retry
  // EAGAIN, ECONNRESET, EWOULDBLOCK: 1 retry
  // Connection pool is empty (too many threads or no connections available): 1 retry
  ...
  do {
    ...
    if ( useEPPool ) {
      if ( m_maxConnections == 0 ) {
        ACE_Guard< ACE_Recursive_Thread_Mutex > guard( m_connectionLock );
        if ( m_maxConnections == 0 ) {
          LOGFINE( "Creating a new connection when connection-pool-size system property set to 0" );
          if ( ( error = createNewConnection( conn, false, false,
            DistributedSystem::getSystemProperties()->connectTimeout()) ) != GF_NOERR ) {
            epFailure = true;
            continue;
          }
          m_maxConnections = 1;
    ...
    if ( createNewConn ) {
      ...
      else if ( ( error = createNewConnection( conn, false, false,
              DistributedSystem::getSystemProperties()->connectTimeout(), 0, true ) ) != GF_NOERR ) {
        epFailure = true;
        continue;
      }
    }
    else if ( conn == NULL && useEPPool ) {
      LOGFINER("sendRequestWithRetry:: looking for connection in queue timeout = %d ",timeout);
      conn = m_opConnections.getUntil( timeout ); // max wait time to get a connection
    }
    if ( !m_connected ) {
      return GF_NOTCON;
    }
    if ( conn != NULL ) {
      ...
      try {
        ...
        error = GF_TIMOUT;
        LOGFINE("Send timed out for endpoint %s. "
            "Message txid = %d", m_name.c_str( ), reqTransId);
        closeFailedConnection( conn );
        /*
        if ( !(m_poolHADM && m_poolHADM->getThreadLocalConnections()) ){ //close connection only when not a sticky connection.
          closeConnection( conn );
        }*/
        gemfire::millisleep( 10 );
        int32_t type =request.getMessageType();
        epFailure = (type != TcrMessage::QUERY &&
                     type != TcrMessage::PUTALL &&
                     type != TcrMessage::PUT_ALL_WITH_CALLBACK &&
                     type != TcrMessage::EXECUTE_FUNCTION &&
                     type != TcrMessage::EXECUTE_REGION_FUNCTION &&
        ...
        if ( !handleIOException( ex.getMessage( ), conn, isBgThread ) ) { //change here
          break;
        }
        createNewConn = true;
      }
        ...
        LOGWARN("Error during send for endpoint %s due to %s",
        ...
        if ( compareTransactionIds( reqTransId, reply.getTransId( ),
          ...
          //we are here its better to close the connection as "compareTransactionIds" 
        ...
        LOGERROR( "Unexpected exception while sending request to "
            "endpoint %s", m_name.c_str( ) );
        if ( compareTransactionIds( reqTransId, reply.getTransId( ),
              failReason, conn ) ) {
          error = GF_MSG;
      ...
      if ( useEPPool ) {
        ...
        if ( timeout <= 0 ) {
          error = GF_TIMOUT;
          LOGWARN( "No connection available for %ld seconds "
              "for endpoint %s.", requestedTimeout, m_name.c_str( ) );
        }
        else {
          error = GF_NOTCON;
          LOGFINE( "Returning without connection with %d seconds remaining "
              "for endpoint %s.", timeout, m_name.c_str( ) );
        }
      }
      else {
        LOGERROR( "Unexpected failure while sending request to server.");
        GF_DEV_ASSERT( "Bug in TcrEndpoint::sendRequestWithRetry()?" ? false : true );
      }
    }
...
void TcrEndpoint::setRetryAndTimeout( const TcrMessage& request, int& maxSendRetries, uint32_t& requestedTimeout )
{
  int32_t type = request.getMessageType();
...
GfErrType TcrEndpoint::send( const TcrMessage &request, TcrMessage & reply )
{
  GfErrType error = GF_NOTCON;
  ...
  // ARB: TODO: remove sendRetryCount as parameter.
  error = sendRequestWithRetry( request, reply, conn, epFailure, failReason, maxSendRetries, true, requestedTimeout );
  ...
  if ( error != GF_NOERR && epFailure ) {
    LOGFINE("Send Giving up for endpoint %s; reason: %s.",
        m_name.c_str(), failReason.c_str());
    setConnectionStatus( false );
  }
  ...
  if ( !m_isActiveEndpoint && ( opConnectionsSize > 1 ) ) {
    LOGWARN( "Connections size = %d, expected maximum %d",
        opConnectionsSize, 1 );
  }
  else if ( opConnectionsSize > m_maxConnections ) {
    LOGWARN( "Connections size = %d, expected maximum %d",
        opConnectionsSize, m_maxConnections );
  }
#endif
...
GfErrType TcrEndpoint::sendRequestConnWithRetry( const TcrMessage &request, TcrMessage & reply, TcrConnection*& conn, bool isBgThread )
{
  GfErrType error = GF_NOTCON;
  int maxSendRetries = 1;
  ...
  error = sendRequestWithRetry( request, reply, conn, epFailure, failReason, maxSendRetries, false, requestedTimeout, isBgThread );
  if ( error == GF_NOERR ) {
    m_msgSent = true;
  }
...
{
  // [sumedh]: Store the original value of m_isActiveEndpoint.
...
{
  LOGFINEST("Closing subscription channel for endpoint %s",
    m_name.c_str());
  m_notifyConnection->close();
  m_notifyReceiver->stopNoblock( );
  TcrConnectionManager &tccm = m_cache->tcrConnectionManager();
  tccm.addNotificationForDeletion(m_notifyReceiver, m_notifyConnection, m_notificationCleanupSema);
  m_notifyCount++;
  m_cleanupSema.release();
  m_isQueueHosted = false;
  LOGFINEST("Added susbcription channel for deletion and "
    "released cleanup semaphore for endpoint %s", m_name.c_str());
}
...
void TcrEndpoint::stopNotifyReceiverAndCleanup()
{
  ...
  if(m_notifyReceiver != NULL )
  {
    ...
    bool found = false;
    for( std::list< GF_TASK_T<TcrEndpoint> *>::iterator it = m_notifyReceiverList.begin(); it != m_notifyReceiverList.end(); it++)
    {
  ...
  {
    LOGFINER("TcrEndpoint::stopNotifyReceiverAndCleanup: notifylist size = %d", m_notifyReceiverList.size());
    for( std::list< GF_TASK_T<TcrEndpoint> *>::iterator it = m_notifyReceiverList.begin(); it != m_notifyReceiverList.end(); it++)
    {
      LOGFINER("TcrEndpoint::stopNotifyReceiverAndCleanup: deleting old notify recievers.");
      GF_SAFE_DELETE(*it);
    }
  }
  ...
  {
    LOGFINER("TcrEndpoint::stopNotifyReceiverAndCleanup: notifylist size = %d", m_notifyConnectionList.size());
    for( std::list< TcrConnection *>::iterator it = m_notifyConnectionList.begin(); it != m_notifyConnectionList.end(); it++)
    {
      LOGFINER("TcrEndpoint::stopNotifyReceiverAndCleanup: deleting old notify connections.");
      GF_SAFE_DELETE(*it);
    }
  }
}

void TcrEndpoint::setServerQueueStatus(ServerQueueStatus queueStatus ,int32_t queueSize )
{
...
bool TcrEndpoint::isQueueHosted()
{
  return m_isQueueHosted;
}
 ...
 void TcrEndpoint::sendRequestForChunkedResponse(const TcrMessage& request, TcrMessage& reply, TcrConnection* conn)
{
