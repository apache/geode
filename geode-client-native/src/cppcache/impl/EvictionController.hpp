 ...
 *  3> Determines the nmber of entries per region (based on size per entry) that
       needs to be evicted. This is a slice of the total eviction that is needed.
...
class CPPCACHE_EXPORT EvictionController: public ACE_Task_Base, public SharedBase {
  public:

    EvictionController(size_t maxHeapSize, int32_t heapSizeDelta, CacheImpl* cache);
