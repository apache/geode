...
StatisticsType * CqQueryStatType::getStatType() {
  ...
  m_stats[0] = factory->createIntCounter
    ("inserts", "The total number of inserts this cq qurey", "entries", largerIsBetter);
  m_stats[1] = factory->createIntCounter
    ("updates", "The total number of updates for this cq query", "entries", largerIsBetter);
  m_stats[2] = factory->createIntCounter
    ("deletes", "The total number of deletes for this cq query", "entries", largerIsBetter);
  m_stats[3] = factory->createIntCounter
    ("events", "The total number of events for this cq query", "entries", largerIsBetter);

  statsType = factory->createType
    (cqStatsName, cqStatsDesc, m_stats, 4);
...
CqQueryStatType::CqQueryStatType() :
         ...
         */
          m_numDeletesId(0), m_numEventsId(0), m_numInsertsId(0), m_numUpdatesId(0)
{
...
//typedef ACE_Singleton<CqQueryVsdStatsInit, ACE_Thread_Mutex> TheCqQueryVsdStatsInit;
...
CqQueryVsdStats::CqQueryVsdStats(const char* cqqueryName)
{
  ...
  m_cqQueryVsdStats = factory->createAtomicStatistics(statsType, const_cast<char*>( cqqueryName ) );
