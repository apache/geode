...
template<> class ACE_Hash<gemfire::CacheableKeyPtr>
  {
  public:
  ...
  template<> class ACE_Equal_To<gemfire::CacheableKeyPtr>
  {
  public:
...
namespace gemfire
{
  ...
  typedef ::ACE_Hash_Map_Manager_Ex<CacheableKeyPtr, MapEntryPtr, ::ACE_Hash<
      CacheableKeyPtr>, ::ACE_Equal_To<CacheableKeyPtr>, ::ACE_Null_Mutex>
      CacheableKeyHashMap;
  ...
  private:
    ...
    inline void removeTrackerForEntry(const CacheableKeyPtr& key,
        MapEntryPtr& entry, MapEntryImpl* entryImpl)
    {
      // This function is disabled if concurrency checks are enabled. The versioning
      // changes takes care of the version and no need for tracking the entry 
      if (m_concurrencyChecksEnabled)
    ...
    inline GfErrType putNoEntry(const CacheableKeyPtr& key,
        const CacheablePtr& newValue, MapEntryImplPtr& newEntry,
        int updateCount, int destroyTracker, VersionTagPtr versionTag, VersionStamp* versionStamp = NULL)
    {
      if (!m_concurrencyChecksEnabled)
      {
        if (updateCount >= 0) {
    ...
    GfErrType putForTrackedEntry(const CacheableKeyPtr& key,
        ...
        VersionStamp& versionStamp, DataInput* delta = NULL);

    CacheablePtr getFromDisc(CacheableKeyPtr key, MapEntryImpl* entryImpl);
    
    GfErrType removeWhenConcurrencyEnabled(const CacheableKeyPtr& key,
    CacheablePtr& oldValue, MapEntryImplPtr& me, int updateCount, 
    VersionTagPtr versionTag, bool afterRemote, bool& isEntryFound, 
    long expiryTaskID, TombstoneExpiryHandler* handler, bool& expTaskSet);

  public:

    MapSegment() :
      m_map(NULL), m_entryFactory(NULL), m_region(NULL),
      m_primeIndex(0), m_spinlock(), m_segmentMutex(), m_rehashCount(0),
      m_numDestroyTrackers(NULL), m_concurrencyChecksEnabled(false) // COVERITY  --> 30303 Uninitialized scalar field
    {
    ...
    void open(RegionInternal* region, const EntryFactory* entryFactory,
        uint32_t size, volatile int* destroyTrackers, bool concurrencyChecksEnabled);
    ...
    GfErrType invalidate(const CacheableKeyPtr& key, MapEntryImplPtr& me,
        CacheablePtr& oldValue, VersionTagPtr versionTag, bool& isTokenAdded);

    /**
     * @brief remove an entry from the map, setting oldValue.
     */
    GfErrType remove(const CacheableKeyPtr& key, CacheablePtr& oldValue,
        MapEntryImplPtr& me, int updateCount, VersionTagPtr versionTag, bool afterRemote, bool& isEntryFound);
    ...
    inline uint32_t rehashCount()
    {
      return m_rehashCount;
    }

    int addTrackerForEntry(const CacheableKeyPtr& key, CacheablePtr& oldValue,
        bool addIfAbsent, bool failIfPresent, bool incUpdateCount);
    ...
    bool unguardedRemoveActualEntryWithoutCancelTask(const CacheableKeyPtr& key, TombstoneExpiryHandler*& handler, long& taskid);

    bool unguardedRemoveActualEntry(const CacheableKeyPtr& key,
        bool cancelTask = true);

    GfErrType isTombstone(CacheableKeyPtr key, MapEntryImplPtr& me,
        bool& result);
