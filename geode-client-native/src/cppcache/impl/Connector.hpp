...
namespace gemfire {
  class Connector
  {
     ...
     * less than or equal to <code>0</code> an <code>IllegalArgumentException</code>
     * is thrown.
     *
     ...
     * @param      waitSeconds   the number of seconds to allow the read to complete.
     * @return     the total number of bytes read into the buffer, or
     *             <code>-1</code> if an error was encountered.
     * @exception  GemfireIOException, TimeoutException, IllegalArgumentException, OutOfMemoryException.
     */
    virtual int32_t receive( char *b, int32_t len, uint32_t waitSeconds, uint32_t waitMicroSeconds ) = 0;
     ...
     */
    virtual int32_t send( const char * b, int32_t len, uint32_t waitSeconds , uint32_t waitMicroSeconds ) = 0;
