...
namespace gemfire
{
  ...
  class ClientProxyMembershipID
    : public DSMemberForVersionStamp
  {
  public:
    const char* getDSMemberId(uint32_t& mesgLength) const;
    const char* getDSMemberIdForCS43(uint32_t& mesgLength) const;
    ClientProxyMembershipID( const char* hostname, uint32_t hostAddr, uint32_t hostPort,const char *durableClientId = NULL,
    ...
    ClientProxyMembershipID( uint8_t* hostAddr, uint32_t hostAddrLen, uint32_t hostPort,
                              const char* dsname, const char* uniqueTag, uint32_t vmViewId);
    //ClientProxyMembershipID(const char *durableClientId = NULL, const uint32_t durableClntTimeOut = 0);
    ...
    static Serializable * createDeserializable() { return new ClientProxyMembershipID(); }
    ...
    int8_t typeId( ) const { return GemfireTypeIdsImpl::InternalDistributedMember; }
    uint32_t objectSize() const { return 0; }
    int8_t DSFID( ) const { return (int8_t) GemfireTypeIdsImpl::FixedIDByte; }
    void initObjectVars( const char* hostname, uint8_t* hostAddr, uint32_t hostAddrLen, 
                          ...
                          int8_t vmkind, int8_t splitBrainFlag, const char* dsname, const char* uniqueTag, uint32_t vmViewId);
