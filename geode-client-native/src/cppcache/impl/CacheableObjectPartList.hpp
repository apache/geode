...
namespace gemfire
{
  ...
  protected:
    ...
    inline CacheableObjectPartList() :
      m_keys(NULL), m_keysOffset(NULL), m_values(NULLPTR),
      m_exceptions(NULLPTR), m_resultKeys(NULLPTR), m_region(NULL),
      m_updateCountMap(NULL), m_destroyTracker(0), m_addToLocalCache(false)
    {
    }
  ...
  public:
    ...
    CacheableObjectPartList(const VectorOfCacheableKey* keys,
        ...
        bool addToLocalCache) :
          m_keys(keys), m_keysOffset(keysOffset), m_values(values),
          m_exceptions(exceptions), m_resultKeys(resultKeys), m_region(region),
          m_updateCountMap(trackerMap), m_destroyTracker(destroyTracker),
