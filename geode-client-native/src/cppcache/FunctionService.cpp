...
#include "impl/ProxyRegion.hpp"
#include "impl/UserAttributes.hpp"
#include "impl/ProxyCache.hpp"
#include "PoolManager.hpp"
#include "impl/CacheRegionHelper.hpp"
    ...
    if (pool->getMultiuserAuthentication()) {
      ...
      if (pr != NULL) {
        ...
        if (!CacheFactory::getAnyInstance()->isClosed()) {
          CacheRegionHelper::getCacheImpl(
              CacheFactory::getAnyInstance() .ptr())->getRegion(
              region->getName(), tmpRegion);
        }
        else {
...
ExecutionPtr FunctionService::onServersWithCache(const RegionServicePtr& cache)
{
  if (cache->isClosed()) {
    throw IllegalStateException("Cache has been closed");
  }
