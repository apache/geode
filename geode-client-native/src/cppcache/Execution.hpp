...
class CPPCACHE_EXPORT Execution : public SharedBase
{
   ...
   * @param routingObj Set defining the data filter to be used for executing the function
   * @param args user data passed to the function execution
   * @param rs * Specifies the {@link ResultCollector} that will receive the results after
   * the function has been executed.
   * @param func the name of the function to be executed
   * @param hasResult indicating if results are expected, @deprecated since 3.6, use of this parameter is ignored
   * @param timeout value to wait for the operation to finish before timing out.
  ...
  virtual  ResultCollectorPtr execute(CacheableVectorPtr& routingObj, CacheablePtr& args, ResultCollectorPtr& rs,
                           const char* func, bool hasResult, uint32_t timeout, bool isHA , bool optimizeForWrite) = 0;
};
} //namespace gemfire
