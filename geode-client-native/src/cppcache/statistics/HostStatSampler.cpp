...
#include "../impl/ClientHealthStats.hpp"
#include "../impl/ClientProxyMembershipID.hpp"
...
namespace gemfire_stats_globals {
  ...
  std::pair<std::string, int> g_fileInfoPair;
...
int selector (const dirent *d)
{
  ...
  std::string filebasename = ACE::basename(gemfire_stats_globals::g_statFileWithExt.c_str());
  ...
  if ( fileExtPos != std::string::npos ) {
    ...
    if ( fileHyphenPos != std::string::npos ) {
      ...
      std::string buff = tempname.substr(fileHyphenPos+1,tempname.length()-fileHyphenPos-1);
...
HostStatSampler::HostStatSampler(const char* filePath, int64 sampleIntervalMs, StatisticsManager *statMngr,
								 int64 statFileLimit, int64 statDiskSpaceLimit)
{
  ...
  if (m_archiveFileSizeLimit < 0 || m_archiveFileSizeLimit > GEMFIRE_MAX_STATS_FILE_LIMIT) {
  ...
  if (m_isStatDiskSpaceEnabled) {    
    ...
    if (m_archiveDiskSpaceLimit < 0 /*|| m_archiveDiskSpaceLimit > GEMFIRE_MAX_STAT_DISK_LIMIT*/)
      m_archiveDiskSpaceLimit = GEMFIRE_MAX_STAT_DISK_LIMIT;

    //If FileSizelimit is greater than DiskSpaceLimit & diskspaceLimit is set, then set DiskSpaceLimit to FileSizelimit
    if ( m_archiveFileSizeLimit > m_archiveDiskSpaceLimit ) {
      m_archiveFileSizeLimit = m_archiveDiskSpaceLimit;
    }
    
    //If only DiskSpaceLimit is specified and no FileSizeLimit specified, then set DiskSpaceLimit to FileSizelimit.
...
std::string
HostStatSampler::getArchiveFileName() {
  return m_archiveFileName;
}

int64
HostStatSampler::getArchiveFileSizeLimit() {
  return m_archiveFileSizeLimit;
}

int64
HostStatSampler::getArchiveDiskSpaceLimit() {
  return m_archiveDiskSpaceLimit;
}

int64
HostStatSampler::getSampleRate() {
  return m_sampleRate;
}

bool
HostStatSampler::isSamplingEnabled() {
  return true;
}

void
HostStatSampler::accountForTimeSpentWorking(int64 nanosSpentWorking) {
  m_samplerStats->tookSample(nanosSpentWorking);
}

bool
HostStatSampler::statisticsExists(const int64 id) {
...
Statistics *
HostStatSampler::findStatistics(const int64 id) {
  ACE_Guard<ACE_Recursive_Thread_Mutex> guard(m_statMngr->getListMutex());
  std::vector<Statistics *>& statsList = m_statMngr->getStatsList();
  std::vector<Statistics *>::iterator i;
  for(i=statsList.begin(); i != statsList.end(); ++i) {
...
HostStatSampler::getStatListMutex()
{
  return m_statMngr->getListMutex();
}

int32
HostStatSampler::getStatisticsModCount() {
  return m_statMngr->getStatListModCount();
}

std::vector<Statistics *>&
HostStatSampler::getStatistics() {
  return m_statMngr->getStatsList();
}

std::vector<Statistics *>&
HostStatSampler::getNewStatistics() {
  return m_statMngr->getNewlyAddedStatsList();
}

int64
HostStatSampler::getSystemId() {
  return m_pid;
}

int64
HostStatSampler::getSystemStartTime() {
  return m_startTime;
}

std::string
HostStatSampler::getSystemDirectoryPath() {
  ACE_utsname u;
  ACE_OS::uname(&u);
...
std::string
HostStatSampler::getProductDescription() {
  return "Pivotal Gemfire Native Client ";
}

void
HostStatSampler::changeArchive(std::string filename) {
...
std::string
HostStatSampler::chkForGFSExt(std::string filename) {
  if (!m_isStatDiskSpaceEnabled) {
  ...
  return filename;
    ...
    int32 len = (int32)filebasename.length();
...
int32
HostStatSampler::rollArchive(std::string filename) {  
  ...
  int32 len = (int32)filename.length();
  int32 lastPosOfSep = (int32)filename.find_last_of(ACE_DIRECTORY_SEPARATOR_CHAR, len);
  if ( lastPosOfSep == -1 ) {
    statsdirname = ".";
  } else {
  ...
  int32 posOfExt = static_cast<int32> (statsbasename.find_last_of(gfsFileExtAfter, static_cast<int32> (baselen)));
  if ( posOfExt == -1 ) {
  ...
  while(!gotNewFileName) {
    ...
    if ( i < 10 ) {       
      ACE_OS::snprintf(newfilename, 1000, "%s%c%s-%d.%s", statsdirname.c_str(),
        ACE_DIRECTORY_SEPARATOR_CHAR, fnameBeforeExt.c_str(), i, extName.c_str());
    } else {
      ACE_OS::snprintf(newfilename, 1000, "%s%c%s-%d.%s", statsdirname.c_str(),
        ACE_DIRECTORY_SEPARATOR_CHAR, fnameBeforeExt.c_str(), i, extName.c_str());
    }
...
void
HostStatSampler::initSpecialStats() {
  //After Special categories are decided initialize them here
  HostStatHelper::newProcessStats(m_pid, "ProcessStats");
}

void
HostStatSampler::sampleSpecialStats() {
  HostStatHelper::refresh();
}

void
HostStatSampler::closeSpecialStats() {
  ACE_Guard<ACE_Recursive_Thread_Mutex> guard(m_statMngr->getListMutex());
...
void
HostStatSampler::checkListeners() {
}

void
HostStatSampler::start() {
  if (! m_running) {
    m_running = true;
    this->activate();
  }
...
bool
HostStatSampler::isRunning() {
  return m_running;
}

void
HostStatSampler::putStatsInAdminRegion() {
  try
  {
    ...
    if( !adminRgn->isDestroyed() ) {
      ...
      if(m_conn_man->getNumEndPoints() > 0)
      {
       ...
       int puts = 0, gets = 0, misses = 0, numListeners = 0, numThreads = 0, creates = 0;
       int64 cpuTime = 0;
       GemfireStatisticsFactory* gf = GemfireStatisticsFactory::getExistingInstance();
       if(gf ) {
         StatisticsType* cacheStatType = gf->findType("CachePerfStats");
         if(cacheStatType ) {
           Statistics* cachePerfStats = gf->findFirstStatisticsByType(cacheStatType);
           if (cachePerfStats) {
             ...
             numListeners = cachePerfStats->getInt(
                 (char*)"cacheListenerCallsCompleted");
             puts += creates;
           }
         }
       ...
       ClientHealthStatsPtr obj = ClientHealthStats::create(gets, puts, misses, numListeners, numThreads,cpuTime,numCPU);
       if(clientId.empty())
       {
...
void
HostStatSampler::doSample(std::string & archivefilename)
{
  ACE_Guard<ACE_Recursive_Thread_Mutex> samplingGuard(m_samplingLock);
  ...
  if(isSamplingEnabled() && !m_adminError && !TcrConnectionManager::isNetDown)
  ...
  if ( (m_archiveDiskSpaceLimit != 0) && (g_spaceUsed >= (m_archiveDiskSpaceLimit - m_archiver->getSampleSize()))) {
    checkDiskLimit();
   }  
...
void HostStatSampler::checkDiskLimit() {   
  ...
  if (status != -1) {
	  for(int i = 1; i < sds.length(); i++) {
		  ACE_OS::snprintf(fullpath , 512 , "%s%c%s",dirname.c_str(),ACE_DIRECTORY_SEPARATOR_CHAR,sds[i]->d_name);
  ...
  int fileIndex = 0;  
  while( (g_spaceUsed  > m_archiveDiskSpaceLimit) /*&& (fileIndex < entries_count)*/ ) {
    int64 fileSize = fileInfo[fileIndex].second;
    if( ACE_OS::unlink(fileInfo[fileIndex].first.c_str()) == 0) {      
      g_spaceUsed -= fileSize;
    }
    else {						
      LOGWARN("%s\t%s\n", "Could not delete", fileInfo[fileIndex].first.c_str());
    }
    fileIndex++;			
  }
}
...
HostStatSampler::svc(void) {
  ...
  try {
    ...
    int32 msRate = ( int32 )getSampleRate();
    ...
    while(! m_stopRequested) {
		try
		{
		  ...
		  msSpentWorking = (int32) (nanosSpentWorking/1000000);
		  ...
		  while (msToWait > 0) {
			...
			LOGERROR("Exception in statistics sampler thread: %s: %s", e.getName(), e.getMessage());
