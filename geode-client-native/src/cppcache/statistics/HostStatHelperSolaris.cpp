...
void HostStatHelperSolaris::refreshProcess(ProcessStats* processStats)
{
  ...
  SolarisProcessStats* solProcessStat = dynamic_cast<SolarisProcessStats*>(processStats);
  if(solProcessStat == NULL){
    LOGFINE("HostStatHelperSolaris::refreshProcess failed due to null processStat");
    return;
  }
  ...
  if (fPtr != -1)
  {
    if (read(fPtr, &currentInfo, sizeof(currentInfo)) 
        != sizeof(currentInfo))
    {
      int32 errNum = errno; // for debugging
      if ( m_logStatErrorCountDown-- > 0 ) {
        LOGFINE("Error reading procFileName %s, errno %d pid %lu", procFileName, errNum, (uint32)thePid);
      }
    } else {
  ...
  if (fPtr != -1) 
  {
       ...
       != sizeof(currentUsage))
    {
      int32 errNum = errno; // for debugging
      if ( m_logStatErrorCountDown-- > 0 ) {
        LOGFINE("Error reading procFileName %s, errno %d pid %lu", 
                 procFileName, errNum, (uint32)thePid);
      }
    } else {
  ...
  try {
    ...
    if ( m_initialized ) { 
      ...
      if ( timeDelta > CPU_USAGE_STAT_THRESHOLD ) {
        //Math is equivalent to 100*(timeDelta-idleDelta)/timeDelta, rounding aside
        cpuUsage = 100-100*(idleDelta)/timeDelta;
        //Update the previous copy 
        memcpy(m_cpuUtilPrev, cpuUtil, sizeof( uint32_t [CPU_STATES] ));
      } else {
        if ( m_logStatErrorCountDown-- > 0 ) {
          LOGFINE("Kernel stat sampling failed to meet CPU_USAGE_THRESHOLD, reporting 100%% cpu load");
        }
        cpuUsage = 100;
      }
    } else {
...
void HostStatHelperSolaris::getKernelStats(uint32_t* cpuUtil) 
{
  if ( m_kstat == NULL ) {
    ...
    if ( m_kstat == NULL ) {
      char buf[128];
      ACE_OS::snprintf(buf, 128, "Unable to obtain kstat data, errno %d", errno);
  ...
  for (ksp = wkc.kc_chain; ksp != NULL; ksp = ksp->ks_next) {
    ...
    if (ksp->ks_type == KSTAT_TYPE_RAW)
    {
      ...
      {
        wksp = ksp;
        //Read the data corresponding to the pointer. cpu_stat modules deliver cpu_stat_t data.
        kcid = kstat_read(&wkc, wksp, &rcw_cpu_stat_t);
        if ( kcid == -1 )
        {
          LOGERROR("Error reading stats, errno \"%d\"", errno);
          throw new NullPointerException("Error reading cpu_stat struct from kernal stats");
        }
        /*
