...
StatisticsManager::StatisticsManager( const char* filePath, int64 sampleInterval,
									 bool enabled,int64 statFileLimit,int64 statDiskSpaceLimit):m_adminRegion(NULLPTR),m_sampler(NULL)
{
  m_sampleIntervalMs = ( int32_t )sampleInterval * 1000; /* convert to millis */
  ...
  try {
    if (m_sampler == NULL && enabled) {
      m_sampler = new HostStatSampler(filePath,m_sampleIntervalMs, this, statFileLimit, statDiskSpaceLimit); 
      m_sampler->start();
    }
  } catch (...) {
    delete m_sampler;
...
StatisticsManager* StatisticsManager::initInstance(const char* filePath,
                                                                 ...
                                                                 bool enabled,int64 statsFileLimit,int64 statsDiskSpaceLimit)
{
  if (!s_singleton) {
    s_singleton = new StatisticsManager(filePath, sampleIntervalMs, enabled, statsFileLimit, statsDiskSpaceLimit);
  }
...
StatisticsManager::~StatisticsManager()
{
  try {
    ...
    Log::warningCatch
      ("~StatisticsManager swallowing GemFire exception", ex);
...
void StatisticsManager::addStatisticsToList(Statistics * stat)
{
  if(stat) {
    ...
    /* Add to m_newlyAddedStatsList also so that a fresh traversal not needed before sampling.
...
StatisticsManager::findStatisticsByType(StatisticsType* type)
{
...
std::vector<Statistics *>
StatisticsManager::findStatisticsByTextId(char* textId)
{
  std::vector<Statistics *> hits;
...
std::vector<Statistics*>
StatisticsManager::findStatisticsByNumericId(int64 numericId)
{
  std::vector<Statistics*> hits;
