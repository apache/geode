  ...
  int32 statsLengthArg)
{
  ...
  if (statsLengthArg > MAX_DESCRIPTORS_PER_TYPE)
  {
    ...
    std::string s = "The requested descriptor count " + temp + " exceeds the maximum which is ";
  ...
  for (int32 i=0; i < this->statsLength; i++)
  {
    ...
    StatisticDescriptorImpl* sd = dynamic_cast<StatisticDescriptorImpl*>(stats[i]);
    /* adongre  
     * CID 28694: Dereference after null check (FORWARD_NULL)Comparing "sd" to null implies that "sd" might be null. 
     *
    ...
    if(sd !=NULL)
    {
      ...
      else
      {
        ...
        statsDescMap.insert(StatisticsDescMap::value_type(stats[i]->getName(), stats[i]));
      }
    }     
  }//for
...
const char* StatisticsTypeImpl::getName()
{
  return name.c_str();
}

const char* StatisticsTypeImpl::getDescription()
{
  return description.c_str();
}

StatisticDescriptor** StatisticsTypeImpl::getStatistics()
{
  return stats;
}
...
StatisticDescriptor* StatisticsTypeImpl::nameToDescriptor(const char* nameArg)
{
  ...
  if( iterFind ==  statsDescMap.end() )
  {
    ...
    std::string  s = "There is no statistic named " + statName + " in this statistics instance ";
...
*/
int32 StatisticsTypeImpl::getIntStatCount()
{
  return intStatCount;
}
...
*/
int32 StatisticsTypeImpl::getLongStatCount()
{
...
*/
int32 StatisticsTypeImpl::getDoubleStatCount()
{
...
*/
int32 StatisticsTypeImpl::getDescriptorsCount()
{
  return statsLength;
}
