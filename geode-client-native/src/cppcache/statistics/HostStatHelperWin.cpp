...
PERF_COUNTER_DEFINITION HostStatHelperWin::processCtrCache[MAX_PROCESS_CTRS_COLLECTED];

PERF_COUNTER_DEFINITION HostStatHelperWin::processorCtrCache[MAX_PROCESSOR_CTRS_COLLECTED];

PERF_COUNTER_DEFINITION HostStatHelperWin::systemCtrCache[MAX_SYSTEM_CTRS_COLLECTED];

PERF_COUNTER_DEFINITION HostStatHelperWin::memoryCtrCache[MAX_MEMORY_CTRS_COLLECTED];

PERF_COUNTER_DEFINITION HostStatHelperWin::objectsCtrCache[MAX_OBJECTS_CTRS_COLLECTED];
...
PPERF_INSTANCE_DEFINITION HostStatHelperWin::FirstInstance( PPERF_OBJECT_TYPE PerfObj )
{
  return( (PPERF_INSTANCE_DEFINITION)((PBYTE)PerfObj +
                                      PerfObj->DefinitionLength) );
}

PPERF_INSTANCE_DEFINITION HostStatHelperWin::NextInstance(PPERF_COUNTER_BLOCK PerfCntrBlk)
{
  return( (PPERF_INSTANCE_DEFINITION)((PBYTE)PerfCntrBlk +
                                      PerfCntrBlk->ByteLength) );
}

char* HostStatHelperWin::getInstIdStr(PPERF_INSTANCE_DEFINITION PerfInst, char* prefix)
{
...
PPERF_COUNTER_DEFINITION HostStatHelperWin::FirstCounter( PPERF_OBJECT_TYPE PerfObj )
{
...
void HostStatHelperWin::HostStatsFetchData()
{
  ...
  if (res != ERROR_SUCCESS) {
    LOGDEBUG("HostStatHeleperWin: Can't get Windows performance data. RegQueryValueEx returned %ld\n",res);
    return;
  }
  ...
  currentFetchData.perfTimeMs = (uint32)(((int64_t)PerfData->PerfTime100nSec.QuadPart) / 10000);
  if (ProcessorObj) {
    int32 i;
    ...
    if (ProcessorObj->NumInstances > 0) {
      ...
      for (i=0; i < (int32)ProcessorObj->NumInstances; i++) {
        ...
        if (PerfInst->UniqueID == PERF_NO_UNIQUE_ID && unicodePtr[0] == '_') {
          ...
          currentFetchData.usertime = getInt64Value(&processorCtrCache[TOTALUSERTIME_IDX], PerfCntrBlk);
          currentFetchData.systime = getInt64Value(&processorCtrCache[TOTALPRIVILEGEDTIME_IDX], PerfCntrBlk);
          currentFetchData.idletime = getInt64Value(&processorCtrCache[TOTALPROCESSORTIME_IDX], PerfCntrBlk);
          currentFetchData.inttime = getInt64Value(&processorCtrCache[INTERRUPTTIME_IDX], PerfCntrBlk);
          currentFetchData.interrupts = getInt32Value(&processorCtrCache[INTERRUPTS_IDX], PerfCntrBlk);
          foundIt = TRUE;
          break;
        }
...
int32 HostStatHelperWin::getPid(int32 pidCtrOffset, PPERF_COUNTER_BLOCK PerfCntrBlk)
{
...
int64 HostStatHelperWin::getInt64Value(PPERF_COUNTER_DEFINITION PerfCntr,
                           PPERF_COUNTER_BLOCK PerfCntrBlk, bool convertMS)
{
  if (PerfCntr->CounterOffset == 0) {
#ifdef FLG_DEBUG
    LOGDEBUG("HostStatHeleperWin: missing counter id=%d\n", PerfCntr->CounterNameTitleIndex);
...
#ifdef FLG_DEBUG
    LOGDEBUG("HostStatHeleperWin: unexpected CounterSize of %ld NameOffset=%d\n", PerfCntr->CounterSize, PerfCntr->CounterNameTitleIndex);// fflush(stderr);
#endif
    return 0;
  }
}
...
void HostStatHelperWin::refreshProcess(ProcessStats* processStats)
{
  ...
  WindowsProcessStats* winProcessStat = dynamic_cast<WindowsProcessStats* >(processStats);
  ...
  for (i=0; i < (int32)ProcessObj->NumInstances; i++) {
    ...
    PerfInst = NextInstance(PerfCntrBlk);
     ...
     for (i=0; i < (int32)ProcessObj->NumInstances; i++) {
       ...
       {
          stats->setInt(winProcessStat->handlesINT, getInt32Value(&processCtrCache[HANDLECOUNT_IDX], PerfCntrBlk));
          stats->setInt(winProcessStat->priorityBaseINT, getInt32Value(&processCtrCache[PRIORITYBASE_IDX], PerfCntrBlk));
          stats->setInt(winProcessStat->threadsINT, getInt32Value(&processCtrCache[THREADCOUNT_IDX], PerfCntrBlk));
          stats->setLong(winProcessStat->activeTimeLONG, getInt64Value(&processCtrCache[PROCESSORTIME_IDX], PerfCntrBlk, false));
          stats->setLong(winProcessStat->pageFaultsLONG, getInt32Value(&processCtrCache[PAGEFAULTS_IDX], PerfCntrBlk));
          stats->setLong(winProcessStat->pageFileSizeLONG, getInt64Value(&processCtrCache[PAGEFILEBYTES_IDX], PerfCntrBlk));
          stats->setLong(winProcessStat->pageFileSizePeakLONG, getInt64Value(&processCtrCache[PAGEFILEBYTESPEAK_IDX], PerfCntrBlk));
          stats->setLong(winProcessStat->privateSizeLONG, getInt64Value(&processCtrCache[PRIVATEBYTES_IDX], PerfCntrBlk));
          stats->setLong(winProcessStat->systemTimeLONG, getInt64Value(&processCtrCache[PRIVILEGEDTIME_IDX], PerfCntrBlk));
          stats->setLong(winProcessStat->userTimeLONG, getInt64Value(&processCtrCache[USERTIME_IDX], PerfCntrBlk));
          stats->setLong(winProcessStat->virtualSizeLONG, getInt64Value(&processCtrCache[VIRTUALBYTES_IDX], PerfCntrBlk));
          stats->setLong(winProcessStat->virtualSizePeakLONG, getInt64Value(&processCtrCache[VIRTUALBYTESPEAK_IDX], PerfCntrBlk));
          stats->setLong(winProcessStat->workingSetSizeLONG, getInt64Value(&processCtrCache[WORKINGSET_IDX], PerfCntrBlk));
          stats->setLong(winProcessStat->workingSetSizePeakLONG, getInt64Value(&processCtrCache[WORKINGSETPEAK_IDX], PerfCntrBlk));
...
int HostStatHelperWin::calculateCpuUsage(PPERF_COUNTER_BLOCK& ctrBlk)
{
  ...
  int64 newProcessorTime = getInt64Value(&processCtrCache[PROCESSORTIME_IDX], ctrBlk, false);
