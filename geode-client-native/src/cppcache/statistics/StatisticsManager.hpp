...
private:
  ...
  std::vector<Statistics *> m_statsList; //Vector containing all the Stats objects

  //Vector containing stats pointers which are not yet sampled.
  std::vector<Statistics *> m_newlyAddedStatsList; 

  ACE_Recursive_Thread_Mutex m_statsListLock; //Mutex to lock the list of Stats
  ...
  StatisticsManager(const char* filePath, int64 sampleIntervalMs, bool enabled, int64 statFileLimit = 0, int64 statDiskSpaceLimit = 0);
...
public:
  ...
  static StatisticsManager* initInstance
    (const char* filePath, int64 sampleIntervalMs, bool enabled, int64 statFileLimit = 0, int64 statDiskSpaceLimit = 0);

  static StatisticsManager* getExistingInstance ();

  void RegisterAdminRegion(AdminRegionPtr adminRegPtr){ m_adminRegion = adminRegPtr; }
