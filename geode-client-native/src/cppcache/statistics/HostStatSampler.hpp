 ...
 */
  ...
  * managed in its constructor and destructor but has no user-written copy constructor. 
  *
  * CID 28719: Other violation (MISSING_ASSIGN) Class "gemfire_statistics::HostStatSampler" 
  * owns resources that are managed in its constructor and destructor but has no user-written assignment operator. 
  *
  * FIX : Make the class NonCopyable
  */

class CPPCACHE_EXPORT HostStatSampler : public ACE_Task_Base , private NonCopyable , private NonAssignable{

  public:
    /*
     * Constructor:
     */
    HostStatSampler(const char* filePath, int64 sampleIntervalMs, StatisticsManager *statMngr, int64 statFileLimit = 0, int64 statDiskSpaceLimit = 0);
    ...
    /**
     * Returns a vector of ptrs to all the newly added statistics resource instances.
     */
    ...
    /**
     * If the total size of all the archive files exceeds the archive disk space limit then the older 
     * files are deleted.
     */
     void checkDiskLimit();
