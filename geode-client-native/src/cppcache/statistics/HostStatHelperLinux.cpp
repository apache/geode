...
void HostStatHelperLinux::refreshProcess(ProcessStats* processStats)
{
  ...
  LinuxProcessStats* linProcessStat = dynamic_cast<LinuxProcessStats*>(processStats);
  if(linProcessStat == NULL){
    LOGFINE("HostStatHelperLinux::refreshProcess failed due to null processStat");
    return;
  }
