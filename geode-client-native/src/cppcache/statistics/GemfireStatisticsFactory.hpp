...
private:
  ...
  int64 m_statsListUniqueId; //Creates a unique id for each stats object in the list

  ACE_Recursive_Thread_Mutex m_statsListUniqueIdLock;

  /* Maps a stat name to its StatisticDescriptor*/
  ACE_Map_Manager<std::string, StatisticsTypeImpl*, ACE_Recursive_Thread_Mutex> statsTypeMap;
...
public:
  ...
  static GemfireStatisticsFactory* initInstance
  ...
  Statistics* createAtomicStatistics(StatisticsType* type, const char* textId, int64 numericId);


  //------------ StatisticsFactory methods: Statistics Type ------------------------------
  StatisticsType* createType
  (const char* name, const char* description, StatisticDescriptor** stats, int32 statsLength);

  StatisticsType* findType(const char* name);


  //------------ StatisticsFactory methods: Statistics Descriptor ---------------------
  StatisticDescriptor* createIntCounter(const char* name, const char* description,
           const char* units, int8 largerBetter);

  StatisticDescriptor* createLongCounter(const char* name, const char* description,
           const char* units, int8 largerBetter);

  StatisticDescriptor* createDoubleCounter(const char* name, const char* description,
                                           const char* units, int8 largerBetter);

  StatisticDescriptor* createIntGauge(const char* name, const char* description,
              const char* units, int8 largerBetter);

  StatisticDescriptor* createLongGauge(const char* name, const char* description,
                                       const char* units, int8 largerBetter);

  StatisticDescriptor* createDoubleGauge(const char* name, const char* description,
           const char* units, int8 largerBetter);
