...
GemfireStatisticsFactory::GemfireStatisticsFactory( StatisticsManager * statMngr)
{
...
GemfireStatisticsFactory* GemfireStatisticsFactory::initInstance(StatisticsManager * statMngr)
{
  if (!s_singleton) {
...
GemfireStatisticsFactory::~GemfireStatisticsFactory()
{
  try {
    ...
    if (statsTypeMap.total_size() == 0)
      return;


    ACE_Map_Manager<std::string, StatisticsTypeImpl*, ACE_Recursive_Thread_Mutex>::iterator iterFind= statsTypeMap.begin();
    ...
    Log::warningCatch
      ("~GemfireStatisticsFactory swallowing GemFire exception", ex);
...
const char* GemfireStatisticsFactory::getName()
{
  return m_name;
}


int64 GemfireStatisticsFactory::getId()
{
  return m_id;
}
...
Statistics* GemfireStatisticsFactory::createStatistics(StatisticsType* type, const char* textId)
{
  return createAtomicStatistics(type, textId, 0);
}


Statistics* GemfireStatisticsFactory::createStatistics
...
Statistics* GemfireStatisticsFactory::createOsStatistics
(StatisticsType* type, const char* textId, int64 numericId)
{
  // Validate input 
  if(type == NULL ){
  ...
  Statistics* result = new OsStatisticsImpl
    (type, textId, numericId, myUniqueId, this);
  {
...
Statistics* GemfireStatisticsFactory::createAtomicStatistics(StatisticsType* type) {
  return createAtomicStatistics(type, NULL, 0);
}
...
(StatisticsType* type, const char* textId, int64 numericId)
{
  ...
  Statistics* result = new AtomicStatisticsImpl
    (type, textId, numericId, myUniqueId, this);
...
Statistics* GemfireStatisticsFactory::findFirstStatisticsByType( StatisticsType* type )
{
...
StatisticsTypeImpl* GemfireStatisticsFactory::addType(StatisticsTypeImpl* st)
{
  ...
  if ( status == 1 ) {
    ...
    throw IllegalArgumentException( "GemfireStatisticsFactory::addType: failed "
        "to add new type %s", temp.c_str( ) );
  }
  return st;
}
...
{
   StatisticsTypeImpl* st = new StatisticsTypeImpl(name,
                                 description, stats, statsLength);
...
(const char* name, const char* description, const char* units, int8 largerBetter)
{
