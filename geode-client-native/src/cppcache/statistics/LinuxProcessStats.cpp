...
void LinuxProcessStats::createType(StatisticsFactory* statFactory)
{
  try
  {
     ...
     statDescriptorArr[2] = statFactory->createIntCounter("userTime",
       "The os statistic for the process cpu usage in user time","jiffies");
     ...
     statDescriptorArr[4] = statFactory->createIntGauge("hostCpuUsage",
       "The os statistic for the host cpu usage","percent cpu");

     statDescriptorArr[5] = statFactory->createIntGauge("threads",
      "Number of threads currently active in this process.","threads");
      
     try {
       m_statsType = statFactory->createType("LinuxProcessStats",
                   "Statistics for a Linux process.",statDescriptorArr, 6);
     }
     catch (Exception& ) {
     ...
     if(m_statsType == NULL)
     {
        throw OutOfMemoryException
              ("LinuxProcessStats::createType: out memory");
     }
...
int64 LinuxProcessStats::getCPUTime()
{
  return stats->getInt(userTimeINT);
}
int32 LinuxProcessStats::getNumThreads()
{
