...
{
  int64_t msec = (int64_t) m_timevalue.tv_sec;
  msec *= 1000;
  msec += (m_timevalue.tv_usec / 1000);
  output.writeInt( (int64_t)msec );
}
...
Serializable* CacheableDate::fromData( DataInput& input )
{
  ...
  m_timevalue.tv_sec = (long) (msec/1000);
  m_timevalue.tv_usec = ( (suseconds_t) (msec%1000)*1000 );
...
int32_t CacheableDate::classId( ) const
{
  return 0;
}

int8_t CacheableDate::typeId( ) const
{
  return GemfireTypeIds::CacheableDate;
}
...
bool CacheableDate::operator == ( const CacheableKey& other ) const
{
  ...
  const CacheableDate& otherDt = 
    static_cast<const CacheableDate&>( other );
...
{
  int64_t tmp = 0;
  tmp = (int64_t) m_timevalue.tv_sec;
  tmp *= 1000;
...
uint32_t CacheableDate::hashcode( ) const
{
  if ( m_hash == 0 ) {
    ...
    self->m_hash =  (int) hash ^ (int) ((int64_t)hash >> 32);
  }
  return m_hash;
}
