...
const char*  Cache::getName() const
{
   ...
   */
bool Cache::isClosed() const
{
  return m_cacheImpl->isClosed();
}
...
void Cache::close(bool keepalive)
{
 ...
 if( DistributedSystemImpl::currentInstances() > 0)
    return;
   ...
   */
RegionPtr Cache::createRegion(const char* name,
    const RegionAttributesPtr& aRegionAttributes)
{
  LOGDEBUG("Cache::createRegion");
   RegionPtr result;
   m_cacheImpl->createRegion(name, aRegionAttributes, result);

   if (result != NULLPTR)
   {
     if(isPoolInMultiuserMode(result))
     {
       LOGWARN("Pool attached with this region is in multiuserSecure mode, \
         don't do any operation on this instance as this instance don't have any credentials. \
         Get region instance from RegionService"); 
      // ProxyCachePtr pc(new ProxyCache(NULLPTR, result->getAttributes()->getPoolName()));
       //ProxyRegionPtr prp(new ProxyRegion(pc.ptr(), result));
       //return prp;
     }
   }
   return result;
...
{  
  ...
  if (result != NULLPTR)
  {
    if(isPoolInMultiuserMode(result))
    {
      LOGWARN("Pool [%s] attached with region [%s] is in multiuser authentication mode. "
...
Cache::Cache(const char* name, DistributedSystemPtr sys, bool ignorePdxUnreadFields,bool readPdxSerialized )
{
   m_cacheImpl = new CacheImpl(this, name, sys, ignorePdxUnreadFields, readPdxSerialized);
}
Cache::Cache(const char* name, DistributedSystemPtr sys, const char* id_data, bool  ignorePdxUnreadFields, bool readPdxSerialized)
{
   m_cacheImpl = new CacheImpl(this, name, sys, id_data, ignorePdxUnreadFields, readPdxSerialized);
}
...
RegionServicePtr Cache::createAuthenticatedView(PropertiesPtr userSecurityProperties, const char* poolName)
{
  if(poolName == NULL)
  {
    if(!this->isClosed() && m_cacheImpl->getDefaultPool() != NULLPTR)
    {
      return m_cacheImpl->getDefaultPool()->createSecureUserCache(userSecurityProperties);
    }

    throw IllegalStateException("Either cache has been closed or there are more than two pool." 
  ...
  else
  {
    if(!this->isClosed())
    {
      if (poolName != NULL)
      {
        ...
        throw IllegalStateException("Either pool not found or it has been destroyed");
      }
