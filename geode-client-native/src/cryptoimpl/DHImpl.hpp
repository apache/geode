...
extern "C" {  
  CPPCACHE_EXPORT int gf_initDhKeys(void ** dhCtx, const char* dhAlgo, const char* ksPath);
  CPPCACHE_EXPORT void gf_clearDhKeys(void * dhCtx);
  CPPCACHE_EXPORT unsigned char* gf_getPublicKey(void * dhCtx, int* len);
  CPPCACHE_EXPORT void gf_setPublicKeyOther(void * dhCtx, const unsigned char* pubkey, int length);
  CPPCACHE_EXPORT void gf_computeSharedSecret(void * dhCtx);
  CPPCACHE_EXPORT unsigned char* gf_encryptDH(void * dhCtx, const unsigned char* cleartext,
    int len, int* retLen);
  CPPCACHE_EXPORT unsigned char* gf_decryptDH(void * dhCtx, const unsigned char* cleartext,
    int len, int* retLen);
  CPPCACHE_EXPORT bool gf_verifyDH(void * dhCtx, const char* subject, const unsigned char* challenge,
