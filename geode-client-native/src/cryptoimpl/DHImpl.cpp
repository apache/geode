...
int gf_initDhKeys(void ** dhCtx, const char* dhAlgo, const char* ksPath){
  ...
  *dhCtx = (void*) dhimpl;
  ...
  // note that OpenSSL 0.9.8g has a bug where it can read only the first one in the keystore
...
unsigned char* gf_getPublicKey(void * dhCtx, int* pLen) {
  ...
  //
  // VJR: Note, this temp pointer is needed because OpenSSL increments the pointer passed in
  // so that following encoding can be done at the current output location, this will cause a
  // problem if we try to free the pointer which has been moved by OpenSSL.
  //  
  i2d_DH_PUBKEY(dhpubkey, &temp);
  
  // VJR: TODO: uncomment this - causing problem in computeSecret?
...
void gf_setPublicKeyOther(void * dhCtx, const unsigned char*  pubkey, int length) {
...
void gf_computeSharedSecret(void * dhCtx) {
  ...
  LOGDH("COMPUTE: DH ptr %p, pubkeyOther ptr %p", dhimpl->m_dh, dhimpl->m_pubKeyOther);
  
  LOGDH("DHcomputeKey DHSize is %d", DH_size(dhimpl->m_dh));
  int ret = DH_compute_key(dhimpl->m_key, dhimpl->m_pubKeyOther, dhimpl->m_dh);
  LOGDH("DHcomputeKey ret %d : Compute err(%d): %s",ret, ERR_get_error(), ERR_error_string(ERR_get_error(), NULL));
}
...
int DHImpl::setSkAlgo(const char * skalgo) {
  ...
  std::string algoStr = (colIdx == std::string::npos) ? inAlgo : 
                    inAlgo.substr(0, colIdx);
  int keySize = 0;
  ...
  if( algoStr == "aes" ) {
    keySize = (colIdx == std::string::npos) ? 128 : 
...
unsigned char* gf_encryptDH(void * dhCtx, const unsigned char* cleartext, int len, int* retLen) {
  ...
  LOGDH(" DH: gf_encryptDH using sk algo: %s, Keysize: %d", dhimpl->m_skAlgo.c_str(), dhimpl->m_keySize);
  
  unsigned char * ciphertext = new unsigned char[len + 50]; // give enough room for padding
  int outlen, tmplen;
  EVP_CIPHER_CTX ctx;
  ...
  if (dhimpl->m_skAlgo == "AES") {
    ...
    ret = EVP_EncryptInit_ex(&ctx, cipherFunc, NULL, (unsigned char *) dhimpl->m_key,
      (unsigned char *) dhimpl->m_key+keySize);
  }
  else if (dhimpl->m_skAlgo == "Blowfish") {
    int keySize = dhimpl->m_keySize > 128 ? dhimpl->m_keySize/8 : 16;
    ret = EVP_EncryptInit_ex(&ctx, cipherFunc, NULL, NULL, (unsigned char *) dhimpl->m_key+keySize);
    LOGDH("DHencrypt: init BF ret %d", ret);
    EVP_CIPHER_CTX_set_key_length(&ctx, keySize); 
    LOGDH("DHencrypt: BF keysize is %d", keySize);
    ret = EVP_EncryptInit_ex(&ctx, NULL, NULL, (unsigned char *) dhimpl->m_key, NULL);
  }
...
unsigned char* gf_decryptDH(void * dhCtx, const unsigned char* cleartext, int len, int* retLen) {
  ...
  if (dhimpl->m_skAlgo == "AES") {
    int keySize = dhimpl->m_keySize > 128 ? dhimpl->m_keySize/8 : 16;
    ret = EVP_DecryptInit_ex(&ctx, cipherFunc, NULL, (unsigned char *) dhimpl->m_key,
      (unsigned char *) dhimpl->m_key+keySize);
  }
  else if (dhimpl->m_skAlgo == "Blowfish") {
    int keySize = dhimpl->m_keySize > 128 ? dhimpl->m_keySize/8 : 16;
    ret = EVP_DecryptInit_ex(&ctx, cipherFunc, NULL, NULL, (unsigned char *) dhimpl->m_key+keySize);
    LOGDH("DHencrypt: init BF ret %d", ret);
    EVP_CIPHER_CTX_set_key_length(&ctx, keySize); 
    LOGDH("DHencrypt: BF keysize is %d", keySize);
    ret = EVP_DecryptInit_ex(&ctx, NULL, NULL, (unsigned char *) dhimpl->m_key, NULL);
  }
  else if ( dhimpl->m_skAlgo == "DESede") {
    ret = EVP_DecryptInit_ex(&ctx, cipherFunc, NULL, (unsigned char *) dhimpl->m_key,
      (unsigned char *) dhimpl->m_key+24);
...
bool gf_verifyDH(void * dhCtx, const char* subject, const unsigned char* challenge,
  ...
  for (int item = 0; item < count; item++) {
    certsubject = X509_NAME_oneline(X509_get_subject_name(dhimpl->m_serverCerts[item]), NULL, 0);
  ...
  {
    LOGDH("algo %s -- %s \n", cert->sig_alg->algorithm->sn, cert->sig_alg->algorithm->ln);
  }
  else
    LOGDH("algo is null \n");
  ...
  LOGDH("after EVP_get_digestbyobj  :  err(%d): %s", ERR_get_error(), ERR_error_string(ERR_get_error(), NULL));
  EVP_MD_CTX signatureCtx;
  EVP_MD_CTX_init( &signatureCtx );

  int result1 = EVP_VerifyInit_ex( &signatureCtx, signatureDigest, NULL );
   LOGDH("after EVP_VerifyInit_ex ret %d : err(%d): %s",result1, ERR_get_error(), ERR_error_string(ERR_get_error(), NULL));
...
EVP_PKEY * DH_PUBKEY_get(DH_PUBKEY *key)
{
  ...
  if (a->parameter && (a->parameter->type == V_ASN1_SEQUENCE))
    ...
    if (!d2i_DHparams(&ret->pkey.dh, &cp, (long)j))
      goto err;
    }
