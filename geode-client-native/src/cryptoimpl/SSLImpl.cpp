...
void * gf_create_SslImpl(ACE_SOCKET sock, const char * pubkeyfile, const char * privkeyfile, const char *pemPassword)
{
...
SSLImpl::SSLImpl(ACE_SOCKET sock, const char * pubkeyfile, const char * privkeyfile, const char* password)
{
  ...
  if(SSLImpl::s_initialized == false)
  {
    ...
    if ( strlen(password) > 0 ) {
      ...
      SSL_CTX_set_default_passwd_cb_userdata(sslctx->context(), const_cast<char*>(password));
    }
...
ssize_t SSLImpl::send(const void *buf, size_t len, const ACE_Time_Value *timeout = 0,
               size_t *bytes_transferred = 0)
{
  return m_io->send_n(buf, len, 0, timeout, bytes_transferred);
}

int SSLImpl::getLocalAddr(ACE_Addr & addr)
{
  return m_io->get_local_addr(addr);
}
