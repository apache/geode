...
void Security::getClientSecurityParams(PropertiesPtr prop, std::string credentials) {
...
int32_t Security::doServerKeys()
{
  ...
  if ( regionPtr == NULL ) {
    FWKEXCEPTION( "Security::doServerKeys(): No region to perform operations on." );
  }
...
void Security::initStrKeys(int32_t low, int32_t high, const std::string & keyBase) {
...
int32_t Security::createRegion()
{
  ...
  try {
    ...
    FWKINFO( "Security::createRegion Created region " << region->getName() << std::endl);
    result = FWK_SUCCESS;

  } catch ( Exception e ) {
    FWKSEVERE( "Security::createRegion FAILED -- caught exception: " << e.getMessage() );
  } catch ( FwkException& e ) {
    FWKSEVERE( "Security::createRegion FAILED -- caught test exception: " << e.getMessage() );
  } catch ( ... ) {
...
int32_t Security::verifyInterestList()
{
  ...
  try {
    ...
    if( countUpdate == numOfRegisterKeys){
      FWKINFO( "Security::verifyInterestList update interest list count " << countUpdate <<
                 " is equal to number of register keys " << numOfRegisterKeys);
      result = FWK_SUCCESS;
    }
    else{
      FWKSEVERE( "Security::verifyInterestList update interest list count " << countUpdate << " is not equal to number of register keys " << numOfRegisterKeys);
      return result;
    }

  } catch ( Exception e ) {
    FWKSEVERE( "Security::verifyInterestList Caught Exception: " << e.getMessage() );
  } catch ( FwkException& e ) {
    FWKSEVERE( "Security::verifyInterestList Caught FwkException: " << e.getMessage() );
  } catch ( ... ) {
...
int32_t Security::populateRegion()
{
  ...
  try {
    ...
    FWKSEVERE( "Security::populateRegion() Caught FwkException: " << e.getMessage() );
  } catch ( ... ) {
...
int32_t Security::registerInterestList()
{
  ...
  FWKINFO("Security::registerInterestList region name is " << region->getName());
  region->registerKeys(registerKeyList);
  result = FWK_SUCCESS;

  } catch ( Exception& e ) {
    FWKEXCEPTION( "Security::registerInterestList() Caught Exception: " << e.getMessage() );
  } catch ( FwkException& e ) {
    FWKEXCEPTION( "Security::registerInterestList() Caught FwkException: " << e.getMessage() );
  } catch ( ... ) {
...
int32_t Security::registerRegexList()
{
  ...
  FWKINFO("Security::registerRegexList region name is " << region->getName() << "regex is: " << registerRegex.c_str());
  region->registerRegex(registerRegex.c_str());
  result = FWK_SUCCESS;

  } catch ( Exception& e ) {
    FWKSEVERE( "Security::registerRegexList() Caught Exception: " << e.getMessage() );
  } catch ( FwkException& e ) {
    FWKSEVERE( "Security::registerRegexList() Caught FwkException: " << e.getMessage() );
  } catch ( ... ) {
...
int32_t Security::unregisterRegexList()
{
  ...
  FWKINFO("Security::unregisterRegexList region name is " << region->getName() << "regex is: " << unregisterRegex.c_str());
  region->unregisterRegex(unregisterRegex.c_str());
  result = FWK_SUCCESS;

  } catch ( Exception& e ) {
    FWKSEVERE( "Security::unregisterRegexList() Caught Exception: " << e.getMessage() );
  } catch ( FwkException& e ) {
    FWKSEVERE( "Security::unregisterRegexList() Caught FwkException: " << e.getMessage() );
  } catch ( ... ) {
...
int32_t Security::registerAllKeys()
{
  ...
  result = FWK_SUCCESS;
    ...
    FWKSEVERE( "Security::registerAllKeys() Caught Exception: " << e.getMessage() );
  } catch ( FwkException& e ) {
    FWKSEVERE( "Security::registerAllKeys() Caught FwkException: " << e.getMessage() );
  } catch ( ... ) {
...
int32_t Security::checkValues()
{
  ...
  try {
    ...
    FWKINFO( "Security::checkValues Found " << creates << " values from creates, " << updates \
      << " values from updates, and " << unknowns << " unknown values." );
    result = FWK_SUCCESS;
  } catch ( Exception e ) {
    FWKSEVERE( "Security::checkValues FAILED -- caught exception: " << e.getMessage() );
  } catch ( FwkException& e ) {
    FWKSEVERE( "Security::checkValues FAILED -- caught test exception: " << e.getMessage() );
  } catch ( ... ) {
...
RegionPtr Security::getRegionPtr( const char * reg )
{
  ...
  try {
    ...
    FWKEXCEPTION( "In Security::getRegionPtr()  CacheFactory::getInstance encountered "
...
void Security::runQuery(int32_t & queryCnt)
{
  try {
    ...
    if(allowQuery(resultsetQueries[i].category,resultsetQueries[i].haveLargeResultset,islargeSetQuery,isUnsupportedPRQuery)){
      FWKINFO(" running resultsetQueries["<< i << "] query : " <<resultsetQueries[i].query());
      qry = qs->newQuery(resultsetQueries[i].query());
      startTime = ACE_OS::gettimeofday();
      results = qry->execute( 600 );
      endTime = ACE_OS::gettimeofday() - startTime;
      FWKINFO(" Time Taken to execute the reselt set query : "<< resultsetQueries[i].query() << ": is " <<  endTime.sec() << "." << endTime.usec() << " sec");
      queryCnt++;
    }
    i = GsRandom::random((uint32_t)0,( uint32_t)QueryStrings::SSsize());
    if(allowQuery(structsetQueries[i].category,structsetQueries[i].haveLargeResultset,islargeSetQuery,isUnsupportedPRQuery)){
      FWKINFO(" running structsetQueries["<< i << "] query : " << structsetQueries[i].query());
      qry = qs->newQuery(structsetQueries[i].query());
      startTime = ACE_OS::gettimeofday();
      results = qry->execute( 600 );
      endTime = ACE_OS::gettimeofday() - startTime;
      FWKINFO(" Time Taken to execute the struct set query : "<< structsetQueries[i].query() << ": is " <<  endTime.sec() << "." << endTime.usec() << " sec");
      queryCnt++;
    }
  } catch ( Exception e ) {
...
int32_t Security::doEntryOperations()
{
    ...
    try {
      ...
      if ( opcode == "add" ) {
        ...
        if(!objectType.empty()) {
          ...
          tmpValue = CacheableBytes::create( ( const unsigned char * )valBuf, strlen( valBuf ) );
  ...
  FWKINFO( "doEntryOperations did " << creates << " creates, " << puts << " puts, " << gets << " gets, " << invals << " invalidates, " << dests << " destroys, " << query << " query.");
  return fwkResult;
}
