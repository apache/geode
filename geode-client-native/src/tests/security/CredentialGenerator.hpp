...
class CredentialGenerator : public SharedBase
{
  ...
  protected:
    ...
    CredentialGenerator(const CredentialGenerator& other) : m_id(other.m_id), m_name(other.m_name) { }
  ...
  public:
     ...
     static registeredClassMap & getRegisteredSchemes() {
       return generators();
     }
     ...
     void getAuthInit(PropertiesPtr& prop) {
        ...
        if( !authinit.empty() ) {
          FWKINFO("Authentication initializer : " << authinit << " library " << this->getClientAuthInitLoaderLibrary());
          prop->insert("security-client-auth-factory", authinit.c_str());
          prop->insert("security-client-auth-library", this->getClientAuthInitLoaderLibrary().c_str());
        }
     }
     ...
     std::string getServerCmdParams(std::string securityParams,std::string workingDir = "", bool userMode = false) {
        std::string securityCmdStr;
        FWKINFO("User mode is " << userMode);
        if( securityParams.find("authenticator") != std::string::npos && !this->getClientAuthenticator().empty() ) {
          securityCmdStr  = this->getInitArgs(workingDir,userMode);
          securityCmdStr += std::string(" --J=-Dgemfire.security-client-authenticator=") + this->getClientAuthenticator();
        }
       if( (securityParams.find("authorizer") != std::string::npos) && (!this->getClientAuthorizer().empty()) ) {
          securityCmdStr += std::string(" --J=-Dgemfire.security-client-accessor=") + this->getClientAuthorizer();
        }
        if( (securityParams.find("authorizerPP") != std::string::npos) && (!this->getClientAuthorizer().empty()) ) {
          securityCmdStr += std::string(" --J=-Dgemfire.security-client-accessor-pp=") + this->getClientAuthorizer();
        }
        if (m_id == ID_PKI) {
          securityCmdStr += std::string(" --J=-Dgemfire.security-publickey-filepath=") + this->getPublickeyfile();
          securityCmdStr += std::string(" --J=-Dgemfire.security-publickey-pass=gemfire");
        }
        if( (securityParams.find("dummy") != std::string::npos) && (!this->getClientDummyAuthorizer().empty()) ) {
          securityCmdStr += std::string(" --J=-Dgemfire.security-client-accessor=") + this->getClientDummyAuthorizer();
        }
     ...
     virtual void getAllowedCredentialsForOps(opCodeList& opCodes, PropertiesPtr& p, stringList* regionNames = NULL) {
         ;; //no credentials by default
     }
     virtual void getDisallowedCredentialsForOps(opCodeList& opCodes, PropertiesPtr& p, stringList* regionNames = NULL) {
         ;; //no credentials by default
     }
   ...
   public:
     ...
     virtual std::string getInitArgs(std::string workingDir,bool userMode) { return ""; }
