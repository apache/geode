...
class LdapUserCredentialGenerator : public CredentialGenerator
{
  public:
     LdapUserCredentialGenerator()
      : CredentialGenerator(ID_LDAP,"LDAP") {
        ;;
     };
     ...
     std::string getInitArgs(std::string workingDir,bool userMode) {
       ...
       additionalArgs += std::string(" --J=\\\"-Dgemfire.security-ldap-basedn=")
            + ( ldapRoot!=NULL ? ldapRoot : "ou=ldapTesting,dc=pune,dc=gemstone,dc=com" )
            + "\\\"";
