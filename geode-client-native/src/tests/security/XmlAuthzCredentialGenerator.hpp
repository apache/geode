...
const opCodeList::value_type RArr[] = { OP_GET, OP_GETALL, OP_REGISTER_INTEREST, OP_UNREGISTER_INTEREST,
                                        OP_KEY_SET, OP_CONTAINS_KEY };

const opCodeList::value_type WArr[] = { OP_CREATE, OP_UPDATE, OP_PUTALL, OP_DESTROY, OP_INVALIDATE,
                                        OP_REGION_CLEAR, OP_EXECUTE_FUNCTION };
...
class XmlAuthzCredentialGenerator : public SharedBase {
   ...
   public:
    ...
    virtual void getAllowedCredentials(opCodeList& opCode, PropertiesPtr& prop, stringList* regionNames) {
    ...
    virtual void getDisallowedCredentials(opCodeList& opCode, PropertiesPtr& prop, stringList* regionNames) {
    ...
    private:
       void getAllowedDummyAuthz(ROLES role) {
          ...
          FWKINFO("inserted " << validity << " dummy security-username " << (*m_prop)->find("security-username")->asChar()
                 << " password "
                 << ((*m_prop)->find("security-password") != NULLPTR ? (*m_prop)->find("security-password")->asChar():"not set") );
       }
       ...
       std::string getAllowedUser(ROLES role) {
          ...
          switch(role) {
           case READER_ROLE:
             sprintf(userName, PRiUsnm, userPrefix.c_str(), readerIndices[ rand()%readerIndSz ]);
             break;
           case WRITER_ROLE:
             sprintf(userName, PRiUsnm, userPrefix.c_str(), writerIndices[ rand()%writerIndSz ]);
             break;
           case QUERY_ROLE:
             sprintf(userName, PRiUsnm, userPrefix.c_str(), queryIndices[ rand()%queryIndSz ]);
             break;
           case ADMIN_ROLE:
           default:
             sprintf(userName, PRiUsnm, userPrefix.c_str(), adminIndices[ rand()%adminIndSz ]);
             break;
          };
       ...
       void getAllowedLdapAuthz(ROLES role) {
         ...
         FWKINFO("inserted  ldap security-username " << (*m_prop)->find("security-username")->asChar()
             << " password "
             << ((*m_prop)->find("security-password") != NULLPTR ? (*m_prop)->find("security-password")->asChar():"not set") );
       }
       ...
       void getAllowedPkcsAuthz(ROLES role) {
         ...
         FWKINFO("inserted  PKCS security-alias" << (*m_prop)->find("security-alias")->asChar()
             << " password "
             << ((*m_prop)->find("security-keystorepass") != NULLPTR ? (*m_prop)->find("security-keystorepass")->asChar():"not set") );
       }
       ...
       ROLES getRequiredRole() {
         ...
         for( opCodeList::iterator it = m_opCode->begin(); it != m_opCode->end(); it++) {

           if( requireReaders && std::find(Readers.begin(), Readers.end(),(*it)) == Readers.end() ) {
             requireReaders = false;
           }

           if( requireWriters && std::find(Writers.begin(), Writers.end(),(*it)) == Writers.end() ) {
             requireWriters = false;
           }

           if( requireQuery && std::find(Query.begin(), Query.end(),(*it)) == Query.end() ) {
             requireQuery = false;
           }
         }
         ...
         if( requireReaders ) {
           ...
           /*
            if( m_regionNames != NULL && m_regionNames->size() > 0 ) {
              ...
              for( stringList::iterator rit = m_regionNames->begin(); rit != m_regionNames->end(); rit++) {
                 if( queryUsers && std::find(QueryRegions.begin(), QueryRegions.end(),(*rit)) == QueryRegions.end() ) {
                   queryUsers = false;
                 }
              }
              if( queryUsers ) {
