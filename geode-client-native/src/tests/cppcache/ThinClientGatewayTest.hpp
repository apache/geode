...
typedef gemfire::SharedPtr< MyListener >
   MyListenerPtr;
...
{
    CacheHelper::initLocator( 1,false,true, 1, CacheHelper::staticLocatorHostPort2 );
...
DUNIT_TASK_DEFINITION(SERVER2, SetupClient2)
{
  ...
  RegionPtr regPtr = createPooledRegion( "exampleRegion", false, NULL, locHostPort2, "poolName", true, reg1Listener1);
...
DUNIT_TASK_DEFINITION(SERVER1, SetupClient) 
{
  ...
  LOG("Hitesh region is created ");
   for (int i = 0; i < 100; i++) {
   LOG("Hitesh region is created put");
      regPtr->put( i,i);
    }
...
{
  LOGINFO("Hitesh nevents = %d got" , reg1Listener1->getNumEvents());
  ASSERT(reg1Listener1->getNumEvents() > 0 , "Events should be greater than zero");
}
END_TASK_DEFINITION
