...
DUNIT_TASK_DEFINITION(CLIENT1, StepFour)
{
  ...
  {
    QueryPtr qry = qs->newQuery(const_cast<char*>(resultsetQueriesOPL[i].query()));
    ...
    for( int32_t rows = 0; rows < rsptr->size(); rows++)
    {
      ...
      if(!m_isPdx){
        ...
        if (instanceOf<PortfolioPtr>(ser)) {
          ...
          printf("   query idx %d pulled portfolio object ID %d, pkid  :: %s\n",
          ...
          printf("   query idx %d pulled position object secId %s, shares  :: %d\n",
              i, position->getSecId()->asChar(), position->getSharesOutstanding());
        }else {
          if (ser != NULLPTR) {
            printf (" query idx %d pulled object %s \n",
                i, ser->toString()->asChar() );
          }
          else {
         ...
         if (instanceOf<PortfolioPdxPtr>(pdxser)) {
           ...
           printf("   query idx %d pulled portfolioPdx object ID %d, pkid %s  :: \n",
            ...
            PositionPdxPtr positionpdx = staticCast<PositionPdxPtr>(pdxser);
              printf("   query idx %d pulled positionPdx object secId %s, shares %d  :: \n",
                  i, positionpdx->getSecId(), positionpdx->getSharesOutstanding());
          }
          else {
            if (pdxser != NULLPTR) {
              if (pdxser->toString()->isWideString()) {
                printf (" query idx %d pulled object %S  :: \n",
                  i, pdxser->toString()->asWChar() );
              } else {
              printf (" query idx %d pulled object %s  :: \n",
                  i, pdxser->toString()->asChar() );
              }
            }
            else {
              printf("   query idx %d pulled bad object  :: \n", i);
...
DUNIT_TASK_DEFINITION(CLIENT1, StepFive)
{
  ...
  for ( int i = 0; i < QueryStrings::RSsize(); i++)
  {
    if (m_isPdx == true)
    {
      if (i == 2 || i == 3 || i == 4)
      {
        LOGINFO("Skipping query index %d for Pdx because it is function type.", i);
        continue;
      }
    }
    ...
    {
      QueryPtr qry = qs->newQuery(const_cast<char*>(resultsetQueries[i].query()));
      SelectResultsPtr results = qry->execute();
      if( ! qh->verifyRS(results, (qh->isExpectedRowsConstantRS(i) ? resultsetRowCounts[i] : resultsetRowCounts[i] * qh->getPortfolioNumSets()) )  )
      {
        char failmsg[100] = {0};
        ACE_OS::sprintf(failmsg, "Query verify failed for query index %d", i);
        doAnyErrorOccured = true;
...
DUNIT_TASK_DEFINITION(CLIENT1, StepSix)
{
  ...
  for ( int i = 0; i < QueryStrings::RSPsize(); i++)
  {
    ...
    {
      QueryPtr qry = qs->newQuery(const_cast<char*>(resultsetparamQueries[i].query()));
      //LOGINFO("NIL::229:Retrieved QueryString = %s", qry->getQueryString());

      CacheableVectorPtr paramList = CacheableVector::create();
      for (int j=0;j < noofQueryParam[i]; j++)
      {
        //LOGINFO("NIL::260: queryparamSet[%d][%d] = %s", i, j, queryparamSet[i][j]);
      ...
      if( ! qh->verifyRS(results, (qh->isExpectedRowsConstantPQRS(i) ? resultsetRowCountsPQ[i] : resultsetRowCountsPQ[i] * qh->getPortfolioNumSets())  )  )
      {
        char failmsg[100] = {0};
        ACE_OS::sprintf(failmsg, "Query verify failed for query index %d", i);
        doAnyErrorOccured = true;
...
void runRemoteQueryRSTest(bool poolConfig = false, bool withLocators = false)
{
  if(!poolConfig){
...
DUNIT_MAIN
{
  ...
  for (int i = 0; i < 2; i++)
  {
    ...
    runRemoteQueryRSTest(true,true);
    setPortfolioPdxType();
  }
}
END_MAIN
