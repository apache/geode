...
void validateEventCount(int line) {
  ...
  char buf[1024];
  sprintf(buf, "Got wrong number of creation events. expected[%d], real[%d]", numCreates, num );
  ASSERT( num == numCreates, buf );
  num = reg1Listener1->getUpdates();
  sprintf(buf, "Got wrong number of update events. expected[%d], real[%d]", numUpdates, num );
  ASSERT( num == numUpdates, buf);
  num = reg1Writer1->getCreates();
  sprintf(buf, "Got wrong number of writer events. expected[%d], real[%d]", numCreates, num );
  ASSERT( num == numCreates, buf);
  num = reg1Listener1->getInvalidates();
  sprintf(buf, "Got wrong number of invalidate events. expected[%d], real[%d]", numInvalidates, num );
  ASSERT( num == numInvalidates, buf);
  num = reg1Listener1->getDestroys();
  sprintf(buf, "Got wrong number of destroys events. expected[%d], real[%d]", numDestroys, num );
...
{
  if ( isLocalServer )
    CacheHelper::initServer( 1 , "cacheserver_notify_subscription_PutAllTimeout.xml");
  LOG("SERVER started");
}
END_TASK_DEFINITION
...
void putAllWithOneEntryTimeout(int timeout, int waitTimeOnServer)
{
  ...
  map0.insert(CacheableKey::create("timeout-this-entry"), CacheableString::create(val));
...
void putAllWithOneEntryTimeoutWithCallBackArg(int timeout, int waitTimeOnServer)
{
...
DUNIT_TASK_DEFINITION(CLIENT1, testWithoutTimeoutException)
{
  ...
  {
    printf("Exception while putAllWithOneEntryTimeoutWithCallBackArg :: %s : %s\n", ex.getName(), ex.getMessage());
...
void runThinClientPutAllTimeout( bool poolConfig, bool isLocator = true )
{
