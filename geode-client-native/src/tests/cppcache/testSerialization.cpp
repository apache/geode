...
class OtherType : public Serializable
{
  ...
  OtherType( int32_t classIdToReturn =  g_classIdToReturn ):m_classIdToReturn(classIdToReturn)
  {
    m_struct.a = 0;
    m_struct.b = 0;
    m_struct.c = 0;
  ...
  virtual Serializable* fromData( DataInput& input )
  {
    ...
    input.readBytesOnly((uint8_t*) &m_struct, size);
  ...
  static CacheablePtr uniqueCT( int32_t i, int32_t classIdToReturn = g_classIdToReturn )
  {
    OtherType* ot = new OtherType(classIdToReturn);
    ot->m_struct.a = (int) i;
    ot->m_struct.b = ( i % 2 == 0 ) ? true : false;
    ot->m_struct.c = (char) 65 + i;
    ot->m_struct.d = ( ((double)2.0) * (double) i );

    printf( "Created OtherType: %d, %s, %c, %e\n",
               ot->m_struct.a,
               ot->m_struct.b ? "true" : "false",
               ot->m_struct.c,
               ot->m_struct.d );
...
DUNIT_TASK(NoDist,SerializeInMemory)
{
  ...
  ASSERT( longcopy->value( ) == 0x1122334455667788LL, "expected value 0x1122334455667788." );
  ...
  CacheableBytesPtr bytes = CacheableBytes::create( (uint8_t*)&borig, sizeof( blob ) );
...
DUNIT_TASK(NoDist,OtherTypeInMemory)
{
  ...
  if (copy->classId() > 0xFFFF) {
    ASSERT( copy->typeId( ) == GemfireTypeIdsImpl::CacheableUserData4,
        "typeId should be equal to GemfireTypeIdsImpl::CacheableUserData4." );
  } else if (copy->classId() > 0xFF) {
    ASSERT( copy->typeId( ) == GemfireTypeIdsImpl::CacheableUserData2,
        "typeId should be equal to GemfireTypeIdsImpl::CacheableUserData2." );
  } else {
    ASSERT( copy->typeId( ) == GemfireTypeIdsImpl::CacheableUserData,
...
DUNIT_TASK(Sender,SetupAndPutInts)
{
  ...
  initClient(true);
  createRegion( "DistRegionAck", USE_ACK, endPoint, true, NULLPTR, false);
