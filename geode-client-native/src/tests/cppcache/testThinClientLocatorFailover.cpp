...
const char* locatorsG = CacheHelper::getLocatorHostPort( isLocator, 3);
#define CLIENT1 s1p1
#define CLIENT2 s1p2
...
DUNIT_TASK(CLIENT1, SetupClient1_All)
{
  //starting client 1
  initClient(true);
  createPooledRegion( regionNames[0], USE_ACK, NULL, locatorsG, "Pool1");
...
DUNIT_TASK(CLIENT2, SetupClient2_All)
{
  //starting client 2
  initClient(true);
  createPooledRegion( regionNames[0], USE_ACK, NULL, locatorsG, "Pool2");
...
DUNIT_TASK(CLIENT1, AgainAgainFailoverC1_All)
{
  try{
    ...
    doNetsearch( regionNames[0], keys[0], vals[0],false );
    updateEntry( regionNames[0], keys[1], vals[1] );
    FAIL("No locator exception should have been raised");
  }
  catch( const NotConnectedException& ex ){
    try {
      ExceptionPtr exCause = dynCast< SharedPtr< NoAvailableLocatorsException > >( ex.getCause( ) );
      LOG("Expected NoAvailableLocatorsException");
    }
    catch( ClassCastException& ) {
      FAIL("NotConnectedException with cause NoAvailableLocatorsException should have been raised");
    }
  }
}
