...
const char* getServerSecurityParams()
{
  ...
  if (serverSecurityParams.size() == 0) {
    serverSecurityParams = "security-client-authenticator="
      "templates.security.LdapUserAuthenticator.create "
      "security-client-accessor="
      "com.gemstone.gemfire.internal.security.FilterPreAuthorization.create "
      "security-client-accessor-pp="
      "com.gemstone.gemfire.internal.security.FilterPostAuthorization.create log-level=fine security-log-level=finest";

    char* ldapSrv = ACE_OS::getenv("LDAP_SERVER");
    serverSecurityParams += std::string(" security-ldap-server=") + (ldapSrv
        != NULL ? ldapSrv : "ldap");

    char* ldapRoot = ACE_OS::getenv("LDAP_BASEDN");
    serverSecurityParams += std::string(" security-ldap-basedn=") + (ldapRoot
        != NULL ? ldapRoot : "ou=ldapTesting,dc=pune,dc=gemstone,dc=com");

    char* ldapSSL = ACE_OS::getenv("LDAP_USESSL");
    serverSecurityParams += std::string(" security-ldap-usessl=") + (ldapSSL
        != NULL ? ldapSSL : "false");
  }
...
void checkValuesMap(const HashMapOfCacheablePtr& values,
    int clientNum, int numKeys)
{
  int expectedNum = 0;
  CacheableKeyPtr key;
  ...
  printf("Expected number of values: %d; got values: %d",
      expectedNum, values->size());
  ASSERT(values->size() == expectedNum, "unexpected number of values");
}

void checkExceptionsMap(const HashMapOfExceptionPtr& exceptions,
    int clientNum, int numKeys)
{
  int expectedNum = 0;
  CacheableKeyPtr key;
  ...
  printf("Expected number of exceptions: %d; got exceptions: %d",
      expectedNum, exceptions->size());
...
DUNIT_TASK_DEFINITION(READER2_CLIENT,SetReader2TestCase1)
{
  setGlobals(1);
}
END_TASK_DEFINITION
DUNIT_TASK_DEFINITION(READER2_CLIENT,SetReader2TestCase2)
{
  setGlobals(2);
}
END_TASK_DEFINITION

DUNIT_TASK_DEFINITION(READER2_CLIENT,SetReader2TestCase3)
{
  setGlobals(3);
}
END_TASK_DEFINITION
...
void callSetGlobals(int testCase)
{
  switch (testCase){
    ...
    case 3: {      // pool-with-locator case: pool == true, locators == true
      ...
      CALL_TASK(SetReader2TestCase3);
      break;
    }
  }
}

void doThinClientSecurityPostAuthorization( int testCase )
{
...
DUNIT_MAIN
{
  doThinClientSecurityPostAuthorization(1); // normal case: pool == false, locators == false

  doThinClientSecurityPostAuthorization(2); // pool-with-endpoints case: pool == true, locators == false

  doThinClientSecurityPostAuthorization(3); // pool-with-locator case: pool == true, locators == true
}
END_MAIN
