...
typedef gemfire::SharedPtr< SimpleCacheListener >
...
{
  LOGINFO("SimpleCacheListener: Got an afterCreate event for %s region .", event.getRegion()->getName());
  m_creates++;
}

void SimpleCacheListener::afterUpdate(const EntryEvent& event)
{
  LOGINFO("SimpleCacheListener: Got an afterUpdate event for %s region .", event.getRegion()->getName());
}

void SimpleCacheListener::afterInvalidate(const EntryEvent& event)
{
  LOGINFO("SimpleCacheListener: Got an afterInvalidate event for %s region .", event.getRegion()->getName());
}

void SimpleCacheListener::afterDestroy(const EntryEvent& event)
{
  LOGINFO("SimpleCacheListener: Got an afterDestroy event for %s region .", event.getRegion()->getName());
}

void SimpleCacheListener::afterRegionInvalidate(const RegionEvent& event)
{
  LOGINFO("SimpleCacheListener: Got an afterRegionInvalidate event for %s region .", event.getRegion()->getName());
}

void SimpleCacheListener::afterRegionDestroy(const RegionEvent& event)
{
  LOGINFO("SimpleCacheListener: Got an afterRegionDestroy event for %s region .", event.getRegion()->getName());
}

void SimpleCacheListener::close(const RegionPtr& region)
{
  LOGINFO("SimpleCacheListener: Got an close event for %s region .", region.ptr()->getName());
}

void SimpleCacheListener::afterRegionClear( const RegionEvent& event )
{
  LOGINFO("SimpleCacheListener: Got an afterRegionClear event for %s region .", event.getRegion()->getName());
  m_clears++;
}

/*
 * start server with NBS true/false
 * start one cache less client which will entry operations ( put/invalidate/destroy ).
 * start 2nd cacheless client with cache listener,  writer and client notification true.
...
const char * myRegNames[] = { "DistRegionAck", "DistRegionNoAck", "ExampleRegion", "SubRegion1", "SubRegion2"  };
...
DUNIT_TASK_DEFINITION(SERVER1, CreateServer1)
{
  ...
  if ( isLocalServer ) CacheHelper::initServer( 1, "cacheserver_notify_subscriptionBug849.xml" );
  LOG("SERVER1 started");
}
END_TASK_DEFINITION
...
DUNIT_TASK_DEFINITION(SERVER1, CreateServerWithNBSFalse)
{
  ...
  if ( isLocalServer )
    CacheHelper::initServer( 1, "cacheserver.xml" );
  LOG("SERVER1 started");
}
END_TASK_DEFINITION
...
DUNIT_TASK_DEFINITION( SERVER1, CreateServer1_With_Locator_NBSFalse )
{
  //starting server with notify_subscription false
  if ( isLocalServer )
    CacheHelper::initServer( 1, "cacheserver.xml", locatorsG );
  LOG("SERVER1 started");
}
END_TASK_DEFINITION
...
DUNIT_TASK_DEFINITION(CLIENT1,SetupClient1)
{
  ...
  createRegion( regionNames[0], false, endPoint, true, NULLPTR, false);
}
END_TASK_DEFINITION
...
DUNIT_TASK_DEFINITION(CLIENT1,SetupClient1_Pooled_Locator)
{
  initClient(true);
  LOG( "Creating region in CLIENT1, no-ack, no-cache, no-listener" );
  createPooledRegion( regionNames[0], false, NULL, locatorsG, poolName, true, NULLPTR, false);
}
END_TASK_DEFINITION
...
DUNIT_TASK_DEFINITION(CLIENT1,SetupClient1withCachingEnabled_Pooled_Locator)
{
  initClient(true);
  LOG( "Creating region in CLIENT1, no-ack, no-cache, no-listener" );
  createPooledRegion( myRegNames[0], false, NULL, locatorsG, poolName, true, NULLPTR, true);
  createPooledRegion( myRegNames[1], false, NULL, locatorsG, poolName, true, NULLPTR, true);
  createPooledRegion( myRegNames[2], false, NULL, locatorsG, poolName, true, NULLPTR, true);

  //create subregion
  RegionPtr exmpRegptr = getHelper()->getRegion(myRegNames[2]);
  RegionAttributesPtr lattribPtr = exmpRegptr->getAttributes();
  RegionPtr subregPtr1 = exmpRegptr->createSubregion( myRegNames[3], lattribPtr );
  RegionPtr subregPtr2 = exmpRegptr->createSubregion( myRegNames[4], lattribPtr );

  LOGINFO(" CLIENT1 SetupClient1withCachingEnabled_Pooled_Locator subRegions created successfully");

}
END_TASK_DEFINITION

DUNIT_TASK_DEFINITION(CLIENT1,SetupClient1_Pooled_EndPoint)
{
  initClient(true);
  LOG( "Creating region in CLIENT1, no-ack, no-cache, no-listener" );
  createPooledRegion( regionNames[0], false, endPoint, NULL, poolName, true, NULLPTR, false);
}
END_TASK_DEFINITION

DUNIT_TASK_DEFINITION(CLIENT1,SetupClient1withCachingEnabled_Pooled_EndPoint)
{
  initClient(true);
  LOG( "Creating region in CLIENT1, no-ack, no-cache, no-listener" );
  createPooledRegion( myRegNames[0], false, endPoint, NULL, poolName, true, NULLPTR, true);
  createPooledRegion( myRegNames[1], false, endPoint, NULL, poolName, true, NULLPTR, true);
  createPooledRegion( myRegNames[2], false, endPoint, NULL, poolName, true, NULLPTR, true);
  ...
  LOGINFO("CLIENT1 SetupClient1withCachingEnabled_Pooled_EndPoint:: subRegions created successfully");
...
{
  initClient(true);
  LOG( "Creating region in CLIENT2 , no-ack, no-cache, with-listener and writer" );
  regListener = new TallyListener();
  createRegion( regionNames[0], false, endPoint, true,regListener,false);
...
DUNIT_TASK_DEFINITION(CLIENT2, SetupClient2_Pooled_Locator)
{
  ...
  LOG( "Creating region in CLIENT2 , no-ack, no-cache, with-listener and writer" );
  regListener = new TallyListener();
  createPooledRegion( regionNames[0], false, NULL, locatorsG, poolName, true, regListener, false);
  regWriter   = new TallyWriter  ();
  setCacheWriter  (regionNames[0],regWriter);
  RegionPtr regPtr0 = getHelper()->getRegion( regionNames[0] );
 // regPtr0->registerAllKeys();
...
DUNIT_TASK_DEFINITION(CLIENT2, SetupClient2withCachingEnabled_Pooled_Locator)
{
  ...
  createPooledRegion( myRegNames[0], false, NULL, locatorsG, poolName, true, distRegCacheListener, true);
  createPooledRegion( myRegNames[1], false, NULL, locatorsG, poolName, true, NULL, true);
  createPooledRegion( myRegNames[2], false, NULL, locatorsG, poolName, true, parentRegCacheListener, true);
  ...
  LOGINFO("CLIENT2 SetupClient2withCachingEnabled_Pooled_Locator:: subRegions created successfully");

  //Attach Listener to subRegion
  //Attache Listener

  AttributesMutatorPtr subregAttrMutatorPtr = subregPtr1->getAttributesMutator();
...
DUNIT_TASK_DEFINITION(CLIENT2, SetupClient2_Pooled_EndPoint)
{
  initClient(true);
  LOG( "Creating region in CLIENT2 , no-ack, no-cache, with-listener and writer" );
  regListener = new TallyListener();
  createPooledRegion( regionNames[0], false, endPoint, NULL, poolName, true, regListener, false);
  regWriter   = new TallyWriter  ();
  setCacheWriter  (regionNames[0],regWriter);
  RegionPtr regPtr0 = getHelper()->getRegion( regionNames[0] );
//  regPtr0->registerAllKeys();
...
DUNIT_TASK_DEFINITION(CLIENT2, SetupClient2withCachingEnabled_Pooled_EndPoint)
{
  initClient(true);
  LOG( "Creating region in CLIENT2 , no-ack, no-cache, with-listener and writer" );

  parentRegCacheListener = SimpleCacheListenerPtr(new SimpleCacheListener()); //TallyListener();
  distRegCacheListener = SimpleCacheListenerPtr(new SimpleCacheListener());

  createPooledRegion( myRegNames[0], false, endPoint, NULL, poolName, true, distRegCacheListener, true);
  createPooledRegion( myRegNames[1], false, endPoint, NULL, poolName, true, NULL, true);
  createPooledRegion( myRegNames[2], false, endPoint, NULL, poolName, true, parentRegCacheListener, true);
  ...
  LOGINFO("CLIENT2 SetupClient2withCachingEnabled_Pooled_EndPoint:: subRegions created successfully");
...
DUNIT_TASK_DEFINITION(CLIENT3, SetupClient3)
{
...
DUNIT_TASK_DEFINITION(CLIENT3, SetupClient3_Pooled_EndPoint)
{
  //client with no registerAllKeys.....
  initClient(true);
  LOG( "Creating region in CLIENT2 , no-ack, no-cache, with-listener and writer" );
...
DUNIT_TASK_DEFINITION(CLIENT1, doEventOperations)
{
  ...
  LOGINFO("CLIENT-1 localCaching Enabled After Put ....ExampleRegion.size() = %d", exmpRegPtr->size());
  ASSERT(exmpRegPtr->size() == 5, "Total number of entries in the region should be 5");
  //SLEEP( 1000 ); // let the events reach at other end.

  LOGINFO("CLIENT-1 localCaching Enabled After Put ....DistRegionAck.size() = %d", regPtr0->size());

  //TEST COVERAGE FOR cacheListener.afterRegionClear() API
  exmpRegPtr->clear();
  LOGINFO("CLIENT-1 AFTER Clear() call ....reg.size() = %d", exmpRegPtr->size());
  ASSERT(exmpRegPtr->size() == 0, "Total number of entries in the region should be 0");

  LOGINFO("CLIENT-1 AFTER Clear() call ....SubRegion-1.size() = %d", subregPtr1->size());
  ASSERT(subregPtr1->size() == 5, "Total number of entries in the region should be 0");

  LOGINFO("CLIENT-1 AFTER Clear() call ....SubRegion-2.size() = %d", subregPtr2->size());
  ASSERT(subregPtr2->size() == 5, "Total number of entries in the region should be 0");
...
DUNIT_TASK_DEFINITION(CLIENT2, validateListenerWriterEventsWithNBSTrue)
{
  ...
  //regListener->showTallies();
  LOGINFO(" distRegCacheListener->getCreates() = %d", distRegCacheListener->getCreates());

  //LOGINFO(" parentRegCacheListener->getCreates() = %d", parentRegCacheListener->getCreates());
  ASSERT( parentRegCacheListener->getCreates() == 10, "Should be 10 creates" );
  ASSERT( regWriter->isWriterInvoked() == false, "Writer Should not be invoked");

  //Verify that the region.clear event is received and it has cleared all entries in region
  //LOGINFO("parentRegCacheListener::m_clears = %d ", parentRegCacheListener->getClears());
  ASSERT( parentRegCacheListener->getClears() == 1, "region.clear() should be called once" );

  RegionPtr exmpRegPtr = getHelper()->getRegion(myRegNames[2]);
  //LOGINFO(" Total Entries in ExampleRegion = %d ", exmpRegPtr->size());
  ASSERT( exmpRegPtr->size() == 0, "Client-2 ExampleRegion.clear() should have called and so Exampleregion size is expected to 0 " );
  ...
  ASSERT(  subRegCacheListener->getCreates() == 5, "should be 5 creates for SubRegion-1 " );
  ASSERT(  subRegCacheListener->getClears() == 0, "should be 0 clears for SubRegion-1 " );
  ASSERT( subregPtr1->size() == 5, "Client-2 SubRegion-1 should contains 5 entries " );
  ASSERT( subregPtr2->size() == 5, "Client-2 SubRegion-2 should contains 5 entries " );

  //LOGINFO(" SubRegion-1 CREATES:: subRegCacheListener::m_creates = %d ", subRegCacheListener->getCreates());
  //LOGINFO(" SubRegion-1 CLEARS:: subRegCacheListener::m_clears = %d ", subRegCacheListener->getClears());

  LOGINFO("validateListenerWriterEventsWithNBSTrue :: Event Validation Passed....!!");
}
END_TASK_DEFINITION
...
void runListenerWriter(bool poolConfig = true, bool isLocator = true)
{
 ...
 {
  CALL_TASK(SetupClient1_Pooled_EndPoint);
  CALL_TASK(SetupClient2_Pooled_EndPoint);
  CALL_TASK(Register2WithTrue);
 }
 ...
 if(poolConfig && isLocator)
  ...
  {
    CALL_TASK(CreateServerWithNBSFalse);
  }
 ...
 else
 {
  ...
  CALL_TASK(SetupClient2_Pooled_EndPoint);
  CALL_TASK(Register2WithFalse);
  CALL_TASK(SetupClient3_Pooled_EndPoint);
  CALL_TASK(Register3WithFalse);
 }
...
void runListenerWriterAndVerfiyEvents(bool poolConfig = true, bool isLocator = true)
{
 if( poolConfig && isLocator )
  {
    CALL_TASK(CreateLocator1);
 ...
 {
  CALL_TASK(SetupClient1withCachingEnabled_Pooled_EndPoint);
  CALL_TASK(SetupClient2withCachingEnabled_Pooled_EndPoint);
 }
