...
void createRegion( const char * name, bool ackMode, const char * endpoints ,bool clientNotificationEnabled = false)
{
  ...
  fflush( stdout );
  // ack, caching
  RegionPtr regPtr = getHelper()->createRegion( name, ackMode, true, NULLPTR, endpoints,clientNotificationEnabled );
...
DUNIT_TASK_DEFINITION(CLIENT1, CreateRegions1)
{
  ...
  regPtr->registerAllKeys(false, NULLPTR, false, false);
  LOG( "CreateRegions1 complete." );
}
END_TASK_DEFINITION

DUNIT_TASK_DEFINITION(CLIENT2, CreateRegions2)
{
  createRegion( regionNames[0], USE_ACK, endPoints,true);
  createRegion( regionNames[1], NO_ACK, endPoints,true);
  RegionPtr regPtr = getHelper()->getRegion( regionNames[0] );
...
DUNIT_TASK_DEFINITION(CLIENT1, CreateRegions1_PoolEndpoints)
{
  ...
  regPtr = getHelper()->getRegion( regionNames[1] );
  regPtr->registerAllKeys(false, NULLPTR, false, false);
  LOG( "CreateRegions1_PoolEndpoints complete." );
}
END_TASK_DEFINITION

DUNIT_TASK_DEFINITION(CLIENT2, CreateRegions2_PoolEndpoints)
{
  createPooledRegion( regionNames[0], USE_ACK, endPoints, NULL, "__TESTPOOL1_", true );
  createPooledRegion( regionNames[1], NO_ACK, endPoints, NULL, "__TESTPOOL1_", true );
  RegionPtr regPtr = getHelper()->getRegion( regionNames[0] );
  ...
  regPtr = getHelper()->getRegion( regionNames[1] );
  regPtr->registerAllKeys(false, NULLPTR, false, false);
  LOG( "CreateRegions2_PoolEndpoints complete." );
}
END_TASK_DEFINITION
...
void doThinClientNotification( bool poolConfig = true, bool poolLocators = true) {
  ...
  if ( poolConfig ) {
    if ( poolLocators ) {
      ...
      CALL_TASK( CreateRegions1_PoolEndpoints );
      CALL_TASK( CreateRegions2_PoolEndpoints );
    }
  }
  else {
    CALL_TASK( CreateRegions1 );
    CALL_TASK( CreateRegions2 );
  }
  CALL_TASK( StepThree );
  CALL_TASK( StepFour );
  CALL_TASK( StepFive );
