...
DUNIT_TASK_DEFINITION(CLIENT1, StepThree)
{
  ...
  doNetsearch( regionNames[1], keys[1], nvals[1] ); //this should verify due to registerRegex() call before
...
DUNIT_TASK_DEFINITION(CLIENT1, StepFive)
{
  ...
  verifyEntry( regionNames[1], keys[1], vals[1] ); //this should verify due to registerRegex() call before
  SLEEP( 1000 );
...
DUNIT_TASK_DEFINITION(CLIENT2, StepSix)
{
  ...
  verifyEntry( regionNames[1], keys[2], vals[2] ); //this key-regex was unregistered before failover
...
{
  LOG( "StepEight start." );
  verifyEntry( regionNames[0], keys[0], vals[0] ); //this should verify now (registered after failover)
  verifyEntry( regionNames[1], keys[2], vals[2] ); //this should verify now (not registered after failover)
...
void runThinClientFailOverRegex( bool poolConfig = true, bool isLocator = true)
{
