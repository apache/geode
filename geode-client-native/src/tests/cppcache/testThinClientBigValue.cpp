...
void verify( CacheableBytesPtr& valuePtr, int size )
{
  ...
  for( int i = 0; i < size; i++ ) {
    if ( valuePtr->value()[i] != base ) {
      sprintf( msg, "verifying buf[%d] == %d for size %d, found %d instead", i, base, size, valuePtr->value()[i] );
    }
...
DUNIT_TASK(CLIENT2, VerifyPuts)
{
  ...
  for( int i = 0; i <= MAX_PAYLOAD; grow( &i ) ) {
    ...
    CacheableBytesPtr valPtr = dynCast<CacheableBytesPtr>( regPtr->get( keyPtr ) );
    int ntry=20;
...
DUNIT_TASK(CLIENT2, VerifyManyPuts)
{
  ...
  for (int index = 0; index < entriesExpected; ++index) {
    sprintf(keybuf, "keys1%010d", index);
    CacheableStringPtr valPtr = dynCast<CacheableStringPtr>(regPtr->get(keybuf));
...
DUNIT_TASK(CLIENT2, VerifyUpdatedManyPutsGetAll)
{
  ...
  try {
    ...
    FAIL("Expected IllegalArgumentException when nothing is being fetched");
  }
  ...
  LOG("On client Found all entries with correct size via getAll.");
}
...
DUNIT_TASK(CLIENT2, VerifyManyPutsInt64)
{
  ...
  for (int64_t index = 0; index < entriesExpected; ++index) {
    int64_t key = index * index * index;
    //CacheableStringPtr valPtr = dynCast<CacheableStringPtr>(regPtr->get(key));
    CacheableStringPtr valPtr = dynCast<CacheableStringPtr>(regPtr->get(CacheableInt64::create(key)));
    ASSERT(valPtr != NULLPTR, "expected non-null value");
    ASSERT(valPtr->length() == 207, "unexpected size of value in verify");
  }
  LOG("On client Found all int64 entries with correct size via get.");
  for (int64_t index = 0; index < entriesExpected; ++index) {
    //CacheableKeyPtr key = CacheableKey::create(CacheableInt64::create(index * index * index));
    ...
    CacheableStringPtr valPtr = dynCast<CacheableStringPtr>(entry->getValue());
    ASSERT(valPtr != NULLPTR, "expected non-null value");
    ASSERT(valPtr->length() == 207, "unexpected size of value in verify");
  }
  LOG("On client Found all int64 entries with correct size via getEntry.");
}
...
DUNIT_TASK(CLIENT2, VerifyUpdatedManyPutsInt64GetAll)
{
  ...
  for (int64_t index = 0; index < entriesExpected; ++index) {
    CacheableKeyPtr key = vec[(size_t)index];
    RegionEntryPtr entry = regPtr->getEntry(key);
    ASSERT(entry != NULLPTR, "expected non-null entry");
    CacheableStringPtr valPtr = dynCast<CacheableStringPtr>(entry->getValue());
    ASSERT(valPtr != NULLPTR, "expected non-null value");
  ...
  LOG("On client Found all int64 entries with correct size via getAll.");
}
