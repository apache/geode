...
void setAttributes(RegionAttributesPtr& attrsPtr, std::string pDir = sqlite_dir)
{
...
void validateAttribute(RegionPtr& regionPtr)
{
  ...
  ASSERT(type == DiskPolicyType::OVERFLOWS ,"Expected Action should overflow to disk");
}
...
void checkOverflowTokenValues(RegionPtr& regionPtr,uint32_t num)
{
  ...
  ASSERT(nonoverflowCount == v.size() ,"Non overflowed entries should match key size");
}
...
void checkOverflowToken(RegionPtr& regionPtr, uint32_t lruLimit)
{
  ...
  ASSERT(normalCount <= (int)lruLimit ,"Normal entries count should not exceed LRU entries limit.");
}
...
uint32_t doNgetLargeData(RegionPtr& regionPtr, int num) {
  ...
  for(int i = 0; i < num; i++ ) {
    ...
    CacheableStringPtr valuePtr = dynCast<CacheableStringPtr>( regionPtr->get( i ) );
    if (valuePtr == NULLPTR) {
      countNotFound++;
    } else {
      countFound++;
...
uint32_t doNget(RegionPtr& regionPtr, uint32_t num, uint32_t start = 0) {
  ...
  for( uint32_t i = start; i < num; i++ ) {
    char keybuf[100];
    sprintf( keybuf, "key-%d", i );
    CacheableStringPtr valuePtr = dynCast<CacheableStringPtr>( regionPtr->get( keybuf ) );
  ...
  ASSERT(countFound == (num - start), "Number of entries found and put should match");
  LOGINFO("found:%d and Not found: %d",countFound,countNotFound);
  return countFound;
}
/**
...
void verifyGetAll(RegionPtr region,int startIndex)
{
  ...
  for(int i=0; i<=100; i++)
    keysVector.push_back(CacheableKey::create(i));
 ...
 // keysVector.push_back(CacheableKey::create(101)); //key not there
  ...
  if(valuesMap->size() == keysVector.size()){
    int i = startIndex;
    for(HashMapOfCacheable::Iterator iter = valuesMap->begin(); iter != valuesMap->end(); iter++, i++)
    {
...
void createRegion(RegionPtr& regionPtr, const char* regionName, PropertiesPtr& cacheProps, PropertiesPtr& sqLiteProps)
{
  CacheFactoryPtr cacheFactoryPtr = CacheFactory::createCacheFactory(cacheProps);
...
void setSqLiteProperties(PropertiesPtr& sqliteProperties, int maxPageCount = 1073741823, int pageSize = 65536, std::string pDir = sqlite_dir)
{
  ...
  ASSERT(sqliteProperties != NULLPTR, "Expected sqlite properties to be NON-NULL");
}
// creation of subregion.

void createSubRegion(RegionPtr& regionPtr, RegionPtr& subRegion, const char* regionName, std::string pDir = sqlite_dir)
{
  ...
  ACE_stat fileStat;
  ASSERT(ACE_OS::stat(fileName, &fileStat) == 0, "persistence file not present");
  doNput(subRegion,50);
  doNget(subRegion,50);
}
...
BEGIN_TEST(OverFlowTest)
{
  ...
  RegionPtr regionPtr;
  createRegion(regionPtr,"OverFlowRegion", cacheProperties, sqliteProperties);
  ASSERT(regionPtr != NULLPTR, "Expected regionPtr to be NON-NULL");
  ...
  for(int i = 0; i<10; i++)
  {
    ...
    ASSERT(subRegion != NULLPTR, "Expected region to be NON-NULL");
    checkOverflowToken(subRegion,10); //check the overflow count for each reion
    ...
    ACE_stat fileStat;
    ASSERT(ACE_OS::stat(sqliteDirSubRgn, &fileStat) == -1, "persistence file still present");
  }
  // cache close
  regionPtr->getCache()->close();
}
...
BEGIN_TEST(OverFlowTest_absPath)
{
  ...
  for(int i = 0; i<10; i++)
  {
    ...
    subRegion = NULLPTR;
    char fileName[512];
    sprintf( fileName, "%s/%s/%s.db", absPersistenceDir.c_str(),"SubRegion","SubRegion");
    ACE_stat fileStat;
    ASSERT(ACE_OS::stat(fileName, &fileStat) == -1, "persistence file still present");
  }
  // cache close
  regionPtr->getCache()->close();
}
...
BEGIN_TEST(OverFlowTest_SqLiteFull)
{
  ...
  regionFactoryPtr->setDiskPolicy(DiskPolicyType::OVERFLOWS);
  PropertiesPtr sqliteProperties = Properties::create();
  sqliteProperties->insert("MaxPageCount","10"); // 10 * 1024 is arround 10kB is the db file size
  sqliteProperties->insert("PageSize","1024");
  sqliteProperties->insert("PersistenceDirectory", sqlite_dir.c_str());
  regionFactoryPtr->setPersistenceManager("SqLiteImpl","createSqLiteInstance",sqliteProperties);
  ...
  catch(gemfire::Exception ex){ //expected sqlite full exception catching generic message as we dont have any specific sqlitefull exception
    char buffer[1024];
...
//#if ( defined(_WIN64) || defined(__sparcv9) || defined(__x86_64__) ) // run this test only for 64 bit mode
...
//  attrsFact.setDiskPolicy(DiskPolicyType::OVERFLOWS);
//  PropertiesPtr sqliteProperties = Properties::create();
//  sqliteProperties->insert("MaxPageCount","2147483646");  //maximum allowed for sqlite
...
BEGIN_TEST(OverFlowTest_HeapLRU)
{
  ...
  regionFactoryPtr->setDiskPolicy(DiskPolicyType::OVERFLOWS);
  PropertiesPtr sqliteProperties = Properties::create();
  sqliteProperties->insert("MaxPageCount","2147483646"); // 10 * 1024 is arround 10kB is the db file size
  sqliteProperties->insert("PageSize","65536");
  sqliteProperties->insert("PersistenceDirectory", sqlite_dir.c_str());
  regionFactoryPtr->setPersistenceManager("SqLiteImpl","createSqLiteInstance",sqliteProperties);
  RegionPtr regionPtr = regionFactoryPtr->create("OverFlowRegion");
  ...
  for(int i = 0; i<10; i++)
  {
    ...
    ASSERT(subRegion != NULLPTR, "Expected region to be NON-NULL");
    char fileName[512];
    sprintf( fileName, "%s/%s/%s.db", sqlite_dir.c_str(),"SubRegion","SubRegion");
    ACE_stat fileStat;
    ASSERT(ACE_OS::stat(fileName, &fileStat) == 0, "persistence file not present");
    doNput(subRegion,50);
    doNget(subRegion,50);
...
BEGIN_TEST(OverFlowTest_MultiThreaded)
{
  ...
  {
    threads[thdIdx] = new PutThread(regionPtr, thdIdx * 100 + 1, 100 * (thdIdx +1));
