...
DUNIT_TASK_DEFINITION(CLIENT1, StepOne_Pooled_Locator)
{
  ...
  getHelper()->createRegionAndAttachPool2(partitionRegionNames[0], USE_ACK, "__TEST_POOL1__",cptr1);  
  getHelper()->createRegionAndAttachPool2(partitionRegionNames[1], USE_ACK, "__TEST_POOL1__",cptr2);
  getHelper()->createRegionAndAttachPool2(partitionRegionNames[2], USE_ACK, "__TEST_POOL1__",cptr3);
...
DUNIT_TASK_DEFINITION(CLIENT1, CheckPrSingleHopForIntKeysTask_REGION)
{
  ...
  LOGDEBUG("CheckPrSingleHopForIntKeysTask_REGION create region  = %s ", partitionRegionName);
  ...
  for(int i = 0; i < 3000; i++)
  {
    ...
    try {      
      ...
      LOGDEBUG("CheckPrSingleHopForIntKeysTask_REGION: networkhop %d ", networkhop);  
      if (networkhop){
        failureCount++;
      }
      int8 serverGroupFlag = TestUtils::getCacheImpl(getHelper( )->cachePtr)->getAndResetServerGroupFlag();
      LOGDEBUG("CheckPrSingleHopForIntKeysTask_REGION: serverGroupFlag is %d failureCount = %d", serverGroupFlag, failureCount); 
  ...
  for(int i = 0; i < 1000; i++)
  {
    ...
    try {      
      ...
      int8 serverGroupFlag = TestUtils::getCacheImpl(getHelper( )->cachePtr)->getAndResetServerGroupFlag();
      LOGDEBUG("CheckPrSingleHopForIntKeysTask_REGION: serverGroupFlag is %d ", serverGroupFlag); 
      ASSERT( serverGroupFlag != 2, "serverGroupFlag should not be equal to 2" );
    }
   catch(CacheServerException&) {
  ...
  for(int i = 0; i < 1000; i++)
  {
    ...
    try {      
      LOGDEBUG("CPPTEST: destroying key %d with hashcode %d", i, (int32_t)keyPtr->hashcode());      
