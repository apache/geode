...
#define verifyGetResults() \
	 bool found = false; \
	     ...
	     { \
         LOGINFO("buf = %s dynCast<CacheableStringPtr>(resultList->operator[](i))->asChar() = %s ", buf, dynCast<CacheableStringPtr>(resultList->operator[](i))->asChar());\
	       found = true; \
	       break; \
	     } \
	 } \
...
DUNIT_TASK_DEFINITION(CLIENT1, StartC1)
{
  ...
  if(!isPoolWithEndpoint) {
    initClientWithPool(true, NULL, locHostPort,serverGroup, NULL, NULLPTR, 0, true);
    //createPool(poolName, locHostPort,serverGroup, NULL, 0, true );
  }
  else {
    initClientWithPool(true, NULL, NULL,serverGroup, endPoints, NULLPTR, 0, true, -1, -1, 60000);
  }
...
DUNIT_TASK_DEFINITION(CLIENT1, Client1OpTest )
{
  ...
  try
  {
    ...
    CacheableVectorPtr executeFunctionResult = 
      exe1->execute(rjFuncName, getResult, 15, true, true)->getResult();
    ...
    } else 
    {
      ...
      LOG(buf);
      try {
         CacheableStringPtr csp = dynCast<CacheableStringPtr>(executeFunctionResult->operator[](0));
         sprintf(buf, "echo String : cast successful, echoed string= %s", csp->asChar());
         LOG(buf);
    ...
    executeFunctionResult = 
         exc->withFilter(routingObj)->withArgs(args)->execute(exFuncName, getResult, 15, true, true)->getResult();

    executeFunctionResult = 
         exc->withFilter(routingObj)->withArgs(args)->execute(rjFuncName, getResult, 15, true, true)->getResult();
    if(executeFunctionResult==NULLPTR)
    {
      ASSERT(false, "echo String : executeFunctionResult is NULL");
    } else 
    ...
    {
      sprintf(buf, "echo Boolean: result count = %d", executeFunctionResult->size());
      LOG(buf);
      bool b = dynCast<CacheableBooleanPtr>(executeFunctionResult->operator[](0))->value();
    ...
    */
    ...
    } else 
    {
      ...
      LOG(buf);
      ASSERT(resultList->size() == 34, "region get: resultList count is not 34");
      for(int32_t i=0; i < resultList->size(); i++)
      {
         CacheableStringPtr csPtr = dynCast<CacheableStringPtr>(resultList->operator[](i));
      //  printf("hitesh in csPtr = %u \n",csPtr);
        if (csPtr != NULLPTR) {
          sprintf(buf, "Region get: result[%d]=%s", i, dynCast<CacheableStringPtr>(resultList->operator[](i))->asChar());
          LOG(buf);
          verifyGetResults()
          }
          else {
            ...
            if(tmp != NULLPTR)
              printf("hitesh in typeid = %d  \n",tmp->typeId());
            
            else
              printf("hitesh in typeid is null \n");
            }
      }
    }
    ...
    if(executeFunctionResult==NULLPTR)
    {
      ASSERT(false, "region get new collector: executeFunctionResult is NULL");
    } else 
    {
      resultList->clear();
      for (unsigned item=0; item < (uint32_t)executeFunctionResult->size(); item++)
      ...
      sprintf(buf, "Region get new collector: result list count = %d", resultList->size());
      LOG(buf);
      sprintf(buf, "Region get new collector: result count = %d", executeFunctionResult->size());
      LOG(buf);
      ASSERT(executeFunctionResult->size() == resultList->size(), "region get new collector: executeFunctionResult count is not 34");
      for(int32_t i=0; i < executeFunctionResult->size(); i++)
      {
         sprintf(buf, "Region get new collector: result[%d]=%s", i, dynCast<CacheableStringPtr>(resultList->operator[](i))->asChar());
         LOG(buf);
	 verifyGetResults()
      }
    }
    ...
    for(int i=0; i < 34; i++)
    {
      ...
      sprintf(buf, "KEY--%d", i);
      CacheableKeyPtr key = CacheableKey::create(buf);
      CacheableStringPtr value = dynCast<CacheableStringPtr>(regPtr0->get(key));
    ...
    getResult = true;    
    executeFunctionResult = 
      exc->withArgs(args)->withFilter(routingObj)->execute(getFuncName, getResult, 15, true, true)->getResult();
    ...
    } else 
    {
      ...
      LOGINFO(buf);
      ASSERT(resultList->size() == 34, "get executeFunctionResult count is not 34");
      for(int32_t i=0; i < resultList->size(); i++)
      {
	...
	LOGINFO("result->size() = %d & arrList->size()  = %d ", result->size(), arrList->size() + 1);
    ...
    collector = funcExec->withArgs(args)->withFilter(filter)->execute(exFuncNameSendException, getResult, 15, true, true);
    ASSERT(collector!=NULLPTR, "onRegion collector for string NULL");    
    result = collector->getResult(); 
		LOGINFO("result->size() for Multiple String = %d ", result->size());
    ASSERT(result->size() == 1, "region get: resultList count is not as string exception");
    ...
    try {        
      ...
      {        
        ...
        LOG(logmsg.c_str());
        ex.printStackTrace();        
        FAIL("exFuncNameSendException casting to string for string arguement exception.");
      }
      catch(...) {
        FAIL("exFuncNameSendException casting to string for string arguement Unknown exception.");
      }

    LOG("exFuncNameSendException for string arguement done.");

    LOGINFO("test exFuncNameSendException function with customer collector with bool as arguement using onRegion.");

    MyResultCollector *myRC1 = new MyResultCollector();
    result = funcExec->withArgs(args)->withFilter(filter)->withCollector(ResultCollectorPtr(myRC1) )
      ->execute(exFuncNameSendException, getResult)->getResult();
    ...
    {     
      ...
      ASSERT(1  == result->size(), "region get new collector: result count is not 1");     
    }

    LOGINFO("test exFuncNameSendException function with customer collector with bool as arguement done using onRegion.");

    //-----------------------Test with sendException onRegion done-------------------------------//
...
//    PoolPtr pptr = PoolManager::find(poolName);
  ...
  CacheableVectorPtr executeFunctionResultPdx = 
    pdxExc->withArgs(pdxRoutingObj)->execute(exFuncNamePdxType, getResult)->getResult();
  ...
  {
    LOGINFO("executeFunctionResultPdx size for PdxObject = %d ", executeFunctionResultPdx->size());
    ASSERT(2  == executeFunctionResultPdx->size(), "executeFunctionResultPdx->size() is not 2");     
    CacheableVectorPtr resultListPdx = CacheableVector::create();
    for (unsigned item=0; item < (uint32_t)executeFunctionResultPdx->size(); item++)
    {
      CacheableArrayListPtr arrayList = dynCast<CacheableArrayListPtr>(executeFunctionResultPdx->operator[](item));
      for (unsigned pos=0; pos < (uint32_t)arrayList->size(); pos++)
      {
    ...
    for(int32_t i=0; i < resultListPdx->size(); i++)
    {
      ...
      LOGINFO("get result[%d]=%s", i, dynCast<PdxTypes8Ptr>(resultListPdx->operator[](i))->toString()->asChar());
      PdxTypes8Ptr pdxObj2 = dynCast<PdxTypes8Ptr>(resultListPdx->operator[](i));
      ASSERT(pdxobj->equals(pdxObj2) == true , "Pdx Object not equals original object.");
    }
  }
  LOG("test pdx data function on all servers done");

  PdxInstanceFactoryPtr pifPtr = cacheHelper->getCache()->createPdxInstanceFactory("PdxTests.PdxTypes8");
  ...
  ExecutionPtr pdxInstanceExc = FunctionService::onServers(getHelper()->cachePtr);
  CacheableVectorPtr executeFunctionResultPdxInstance =
    pdxInstanceExc->withArgs(pdxInstanceRoutingObj)->execute(exFuncNamePdxType, getResult)->getResult();
  LOGINFO("FE on pdxObject done");
  ...
  {
    LOGINFO("executeFunctionResultPdxInstance size for PdxObject = %d ", executeFunctionResultPdxInstance->size());
    ASSERT(2  == executeFunctionResultPdx->size(), "executeFunctionResultPdxInstance->size() is not 2");
    CacheableVectorPtr resultListPdxInstance = CacheableVector::create();
    for (unsigned item=0; item < (uint32_t)executeFunctionResultPdxInstance->size(); item++)
    {
      CacheableArrayListPtr arrayList =
        dynCast<CacheableArrayListPtr>(executeFunctionResultPdxInstance->operator[](item));
      for (unsigned pos=0; pos < (uint32_t)arrayList->size(); pos++)
      {
        resultListPdxInstance->push_back(arrayList->operator[](pos));
      }
    }
    LOGINFO("resultlistPdxInstance size: %d", resultListPdxInstance->size());
    for(int32_t i=0; i < resultListPdxInstance->size(); i++)
    {
      ...
      LOGINFO("get result[%d]=%s", i,
          dynCast<PdxTypes8Ptr>(resultListPdxInstance->operator[](i))->toString()->asChar());
  ...
  //-----------------------Test with PdxObject onServers done.-------------------------------//	
    ...
    SLEEP(10000); //let the put finish
    //---------------------------------------------------------------------
    //adding one more server, this should be return by locator and then function should be executed on all the 3 servers
    ...
    } else 
    {
      ...
      ASSERT(resultList->size() == 51, "get executeFunctionResult on all servers count is not 51");
      for(int32_t i=0; i < resultList->size(); i++)
      {
	 sprintf(buf, "result[%d] is null\n", i);
         ASSERT(resultList->operator[](i)!=NULLPTR, buf);
    ...
    collector = funcExec->withArgs(args)->execute(exFuncNameSendException, getResult, 15, true, true);    
    ASSERT(collector!=NULLPTR, "onServers collector for bool NULL");        

    result = collector->getResult();    
    ASSERT(result->size() == 3, "Should have got 3 exception strings for sendException.");
    if(result==NULLPTR)
    {
      ASSERT(false, "echo String : result is NULL");
    } else 
    ...
    collector = funcExec->withArgs(arrList)->execute(exFuncNameSendException, getResult, 15, true, true);
    ASSERT(collector!=NULLPTR, "onServers collector for arrList NULL");

    result = collector->getResult();    
    ASSERT(result->size() == (arrList->size() + 1) * 3, "onServers get: resultList count is not as arrayList count + exception");
    
    LOG("Printing only string exception results for onServers with sendException");
    for (int i=0; i < result->size(); i++) {
      try {
        CacheableInt32Ptr intValue = dynCast<CacheableInt32Ptr>(result->operator[](i));         
        ASSERT(intValue!=NULLPTR, "int value is NULL");                    
        ...
        FAIL("exFuncNameSendException casting to string for arrList arguement for onServers Unknown exception.");
      }
    }
    ...
    for(int i=0; i<result->size(); i++) {      
      try {        
       ...
       LOGINFO("intValue is %d ", intValue->value());         
        ...
        LOG(logmsg.c_str());
        ex.printStackTrace();
        LOG("exFuncNameSendException now casting to string for arrayList arguement exception.");
        UserFunctionExecutionExceptionPtr uFEPtr = dynCast<UserFunctionExecutionExceptionPtr>(result->operator[](i));          
        ASSERT(uFEPtr!=NULLPTR, "uFEPtr exception is NULL");  
        LOGINFO("Done casting to uFEPtr");          
        LOGINFO("Read expected uFEPtr exception %s ", uFEPtr->getMessage()->asChar());        
    ...
    result = collector->getResult();    
    ASSERT(result->size() == 3, "region get: resultList count is not as string exception");
    ...
    LOG("exFuncNameSendException for string arguement done.");
      ...
      LOGINFO("test exFuncNameSendException function with customer collector with bool as arguement using onServers.");

      MyResultCollector *myRC2 = new MyResultCollector();
      result = funcExec->withArgs(args)->withCollector(ResultCollectorPtr(myRC2) )
        ->execute(exFuncNameSendException, getResult)->getResult();
      ...
      {     
        LOGINFO("Region get new collector: result count = %d", result->size());      
        ASSERT(3  == result->size(), "region get new collector: result count is not 1");     
      }

    LOGINFO("test exFuncNameSendException function with customer collector with bool as arguement done using onServers.");

    LOG("OnServers with sendException done");

    //-----------------------Test with sendException onServers done.-------------------------------//
  }
...
DUNIT_TASK_DEFINITION(CLIENT1, Client2OpTest )
{
  ...
  {    
    ...
    CacheableVectorPtr vec = serverExc->withArgs(CacheableInt32::create(5000))->execute(FETimeOut, true, 5000)->getResult();
    if(vec==NULLPTR)
    {
      ASSERT(false, "functionResult is NULL");
    } else 
    ...
    ExecutionPtr serversExc = FunctionService::onServers(getHelper()->cachePtr);
    CacheableVectorPtr vecs = serversExc->withArgs(CacheableInt32::create(5000))->execute(FETimeOut, true, 5000)->getResult();
    if(vecs==NULLPTR)
    {
      ASSERT(false, "functionResult is NULL");
    } else 
...
{
  LOG("hitesh in before starting StartTestClient" );
  ...
  if(!isPoolWithEndpoint) {
    initClientWithPool(true, NULL, locHostPort,serverGroup, NULL, config, 0, true, -1, -1, -1, true, false);
    //createPool(poolName, locHostPort,serverGroup, NULL, 0, true );
  }
  else {
    initClientWithPool(true, NULL, NULL,serverGroup, endPoints, config, 0, true, -1, 5, 60000, true, false);
  }
...
class putThread : public ACE_Task_Base
{
  ...
  putThread(RegionPtr rp, int min, int max, bool isWarmUpTask )
    : regPtr( rp), m_min(min), m_max(max),  m_failureCount(0),
	 m_timeoutCount(0), m_stop(false){}
	...
	while(!m_stop) {
		for ( int i = m_min ; i < m_max ; i++ )
		{
		  try {        
			  ...
			  routingObj->push_back(key);
			  ExecutionPtr exc = FunctionService::onRegion(regPtr0);
			  exc->execute(routingObj, args, rPtr, getFuncName2, true, 300/*in millis*/, true, true)->getResult();
		  }
 ...
 void executeFunction() {
	...
	ASSERT(failureCount <= 10 && failureCount > 0,
        "failureCount should be zero");
}
...
void waitForNoTimeout() {
  ...
  while(maxTry-- > 0) {
	  for(thdIdx = 0; thdIdx < nThreads; thdIdx++)
	  {
		...
		SLEEP(10);
		if(currentTimeout != threads[thdIdx]->getTimeoutCount())
			break;
	  }
      if(thdIdx == nThreads)
        break;		  
  ...
  if(thdIdx < nThreads ){
	  LOGINFO("waitForNoTimeout failed still getting timeouts nThreads: %d,  thdIdx: %d",nThreads, thdIdx);
	  ASSERT(thdIdx < nThreads ,
        "waitForNoTimeout failed still getting timeouts");
  }
  SLEEP(20000);
}
...
void verifyTimeoutFirst() {
  ...
  int blackListBucketTimeouts = TestUtils::getCacheImpl(getHelper( )->cachePtr)->blackListBucketTimeouts(); 
