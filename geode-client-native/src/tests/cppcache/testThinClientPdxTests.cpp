...
void initClient( const bool isthinClient, bool  isPdxIgnoreUnreadFields, const PropertiesPtr& configPtr = NULLPTR)
{
  LOGINFO("isPdxIgnoreUnreadFields = %d ", isPdxIgnoreUnreadFields);
  if ( cacheHelper == NULL ) {
    cacheHelper = new CacheHelper(isthinClient, isPdxIgnoreUnreadFields, false, configPtr, false );
  }
...
void initClientN( const bool isthinClient, bool  isPdxIgnoreUnreadFields, bool  isPdxReadSerialized = false, const PropertiesPtr& configPtr = NULLPTR)
{
  LOGINFO("isPdxIgnoreUnreadFields = %d ", isPdxIgnoreUnreadFields);
  if ( cacheHelper == NULL ) {
    cacheHelper = new CacheHelper(isthinClient, isPdxIgnoreUnreadFields, isPdxReadSerialized, configPtr, false );
  }
  ASSERT( cacheHelper, "Failed to create a CacheHelper client instance." );
}

void stepOneN(bool pool = false,
			  bool locator = false,
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOnePoolEP1)
{
  LOG("Starting Step One with Pool + Explicit server list");
  stepOneN(true, false, false, true,  NULLPTR);
}
END_TASK_DEFINITION

DUNIT_TASK_DEFINITION(CLIENT2, StepTwoPoolEP1)
{
  LOG("Starting Step One with Pool + Explicit server list");
  stepOneN(true, false, false, true, NULLPTR);
}
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOnePoolLoc1)
{
  ...
  stepOneN(true, true, false, true,  NULLPTR);
}
END_TASK_DEFINITION
...
void initClient1WithClientName( const bool isthinClient, const PropertiesPtr& configPtr = NULLPTR )
{
...
void stepOneForClient1(bool pool = false, bool locator = false, bool  isPdxIgnoreUnreadFields = false )
{
  //Create just one pool and attach all regions to that.
...
void stepOne(bool pool = false, bool locator = false, bool  isPdxIgnoreUnreadFields = false, PropertiesPtr config = NULLPTR )
{
  try {
    //Serializable::registerType(Position::createDeserializable);
    //Serializable::registerType(Portfolio::createDeserializable);
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOnePoolLocSysConfig)
{
  ...
  stepOne(true, true, false, config);
}
END_TASK_DEFINITION
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOnePoolEPSysConfig)
{
  LOG("Starting Step One with Pool + Explicit server list");
  PropertiesPtr config = Properties::create();
  config->insert("on-client-disconnect-clear-pdxType-Ids",  "true");
  stepOne(true, false, false, config);
}
END_TASK_DEFINITION

DUNIT_TASK_DEFINITION(CLIENT2, StepTwoPoolEPSysConfig)
{
  LOG("Starting Step One with Pool + Explicit server list");
  PropertiesPtr config = Properties::create();
  config->insert("on-client-disconnect-clear-pdxType-Ids",  "true");
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOnePoolEPBug866)
{
  LOG("Starting Step One with Pool + Explicit server list");
  stepOneForClient1(true);
}
END_TASK_DEFINITION
...
DUNIT_TASK_DEFINITION(CLIENT1, testPdxWriterAPIsWithInvalidArgs)
{
  ...
  try{
    Serializable::registerPdxType(AddressWithInvalidAPIUsage::createDeserializable);
  }catch (const IllegalStateException& ) {
    // ignore exception
  }
  ...
  expectedExceptionCount = (dynCast<PdxTests::InvalidPdxUsagePtr>(pdxobj))->gettoDataExceptionCount();
  //LOGINFO("TASK::testPdxWriterAPIsWithInvalidArgs:: toData ExceptionCount :: %d ", expectedExceptionCount);
  ASSERT(expectedExceptionCount == 41, "Task testPdxWriterAPIsWithInvalidArgs:Did not get expected toDataExceptionCount");

  //Get Operation and check fromDataExceptionCount, Expected is 41.
  PdxTests::InvalidPdxUsagePtr  obj2 = dynCast<PdxTests::InvalidPdxUsagePtr>(regPtr0->get(keyport));
  //LOGINFO("TASK::testPdxWriterAPIsWithInvalidArgs:: fromData ExceptionCOunt :: %d ", obj2->getfromDataExceptionCount());
  expectedExceptionCount = obj2->getfromDataExceptionCount();
  ASSERT(expectedExceptionCount == 41, "Task testPdxWriterAPIsWithInvalidArgs:Did not get expected fromDataExceptionCount");
...
DUNIT_TASK_DEFINITION(CLIENT2, testPdxReaderAPIsWithInvalidArgs)
{
  ...
  PdxTests::InvalidPdxUsagePtr  obj1 = dynCast<PdxTests::InvalidPdxUsagePtr>(regPtr0->get(keyport1));

  //Check the exception count:: expected is 41.
  //LOGINFO("TASK::testPdxReaderAPIsWithInvalidArgs:: fromDataExceptionCount :: %d ", obj1->getfromDataExceptionCount());
  expectedExceptionCount = obj1->getfromDataExceptionCount();
  ASSERT(expectedExceptionCount == 41, "Task testPdxReaderAPIsWithInvalidArgs:Did not get expected fromDataExceptionCount");
...
DUNIT_TASK_DEFINITION(CLIENT1, testPutWithMultilevelInheritance)
{
  ...
  //Get Operation and check fromDataExceptionCount, Expected is 41.
  PdxTests::ChildPtr  obj2 = dynCast<PdxTests::ChildPtr>(regPtr0->get(keyport));
...
DUNIT_TASK_DEFINITION(CLIENT2, testGetWithMultilevelInheritance)
{
  ...
  CacheableKeyPtr keyport1 = CacheableKey::create(1);
  PdxTests::ChildPtr  obj1 = dynCast<PdxTests::ChildPtr>(regPtr0->get(keyport1));
...
DUNIT_TASK_DEFINITION(CLIENT1, JavaPutGet1)
{
  ...
  CacheableInt32Ptr getVal = dynCast<CacheableInt32Ptr>(regPtr0->get(keyport));
  ...
  LOGINFO("Task:JavaPutGet PDX-ON read-serialized = %d", cacheHelper->getCache()->getPdxReadSerialized());
  ...
  */
  ...
  ASSERT(genericValCompare(m_linkedlist->size(), listPtr->size()) == true , "LinkedList size should be equal");
...
DUNIT_TASK_DEFINITION(CLIENT1, JavaPutGet)
{
  ...
  LOGINFO("NILKATH JavaPutGet PDX-ON read-serialized = %d", cacheHelper->getCache()->getPdxReadSerialized());
  PdxInstancePtr jsonDoc = dynCast<PdxInstancePtr>(regPtr0->get("jsondoc1"));
  int age = 0;
  jsonDoc->getField("age", age);
  ...
  LOGINFO("NILKATH JavaPutGet PDX-OFF read-serialized = %d", cacheHelper->getCache()->getPdxReadSerialized());
  */
}
END_TASK_DEFINITION
...
DUNIT_TASK_DEFINITION(CLIENT2, JavaGet)
{
  ...
  PdxTests::PdxTypePtr  obj1 = dynCast<PdxTests::PdxTypePtr>(regPtr0->get(keyport1));
  LOGDEBUG("JavaGet-3 Line_316");
  CacheableKeyPtr keyport2 = CacheableKey::create("putFromjava");
  LOGDEBUG("JavaGet-4 Line_316");
  PdxTests::PdxTypePtr  obj2 = dynCast<PdxTests::PdxTypePtr>(regPtr0->get(keyport2));
...
DUNIT_TASK_DEFINITION(CLIENT2, Get2)
{
  ...
  PdxTests::PdxTypes2Ptr  obj2 = dynCast<PdxTests::PdxTypes2Ptr>(regPtr0->get(keyport));
...
DUNIT_TASK_DEFINITION(CLIENT1, generateJavaPdxType)
{
  ...
  ResultCollectorPtr collector = funcExec->withArgs(args)->withFilter(routingObj)->execute("ComparePdxTypes", true);
...
DUNIT_TASK_DEFINITION(CLIENT1, putOperation)
{
  ...
  const char* clientName1 = (dynCast<CacheableStringPtr>(valuePtr1))->asChar();
  LOGINFO(" C1.putOperation Got ClientName1 = %s ", clientName1);
  ASSERT(strcmp(clientName1,"Client-1") == 0, "ClientName for Client-1 is not set");
}
END_TASK_DEFINITION
...
DUNIT_TASK_DEFINITION(CLIENT2, getOperation)
{
  ...
  const char* clientName2 = (dynCast<CacheableStringPtr>(valuePtr2))->asChar();
  LOGINFO(" C2.getOperation Got ClientName2 = %s ", clientName2);
  ASSERT(strcmp(clientName2,"Client-2") == 0, "ClientName for Client-2 is not set");
...
DUNIT_TASK_DEFINITION(CLIENT2, getCharTypes)
{
  ...
  PdxTests::CharTypesPtr  remotePdxptr = dynCast<PdxTests::CharTypesPtr>(regPtr0->get(keyport));
  LOG("Client-2 PdxTests::CharTypes GET OP Done....");


  PdxTests::CharTypes* localPdx  = localPdxptr.ptr();
  PdxTests::CharTypes* remotePdx = dynamic_cast<PdxTests::CharTypes*>(remotePdxptr.ptr());

  LOGINFO("testThinClientPdxTests:StepFour before equal() check");
  ASSERT(remotePdx->equals(*localPdx) == true, "PdxTests::PdxTypes should be equal.");
...
DUNIT_TASK_DEFINITION(CLIENT2, StepFour)
{
  ...
  m_localObjectArray->push_back(AddressPtr(new Address(1, "street0", "city0")));
  m_localObjectArray->push_back(AddressPtr(new Address(2, "street1", "city1")));
  m_localObjectArray->push_back(AddressPtr(new Address(3, "street2", "city2")));
  m_localObjectArray->push_back(AddressPtr(new Address(4, "street3", "city3")));
  m_localObjectArray->push_back(AddressPtr(new Address(5, "street4", "city4")));
  m_localObjectArray->push_back(AddressPtr(new Address(6, "street5", "city5")));
  m_localObjectArray->push_back(AddressPtr(new Address(7, "street6", "city6")));
  m_localObjectArray->push_back(AddressPtr(new Address(8, "street7", "city7")));
  m_localObjectArray->push_back(AddressPtr(new Address(9, "street8", "city8")));
  m_localObjectArray->push_back(AddressPtr(new Address(10, "street9", "city9")));
  ...
  LOGINFO("Client-2 PdxTests::PdxType GET OP Done.. Received CObjeArray Size = %d", remoteCObjArray->size());
  ASSERT(remoteCObjArray->size() == 10, "PdxTests StepFour: CacheableObjectArray Size should be equal to 10");
  ...
  for(int i = 0; i< remoteCObjArray->size(); i++) {
    ...
    LOGINFO("Remote Address:: %d th element  AptNum=%d  street=%s  city=%s ", i, rAddr1->getAptNum(), rAddr1->getStreet(), rAddr1->getCity());
    if (!rAddr1->equals(*lAddr1)){
      isEqual = false;
      break;
    }
  }
  ASSERT(isEqual == true, "PdxTests StepFour: CacheableObjectArray elements are not matched");
...
void runPdxLongRunningClientTest (bool poolConfig = false, bool withLocators = false)
{
  if(!poolConfig){
    ...
    CALL_TASK(CreateServer)
    CALL_TASK(StepOnePoolEPSysConfig)
	CALL_TASK(StepTwoPoolEPSysConfig)
  }
...
void testBug866(bool poolConfig = false, bool withLocators = false)
{
  if(withLocators){
    CALL_TASK(StartLocator)
    CALL_TASK(CreateServerWithLocator)
    ...
    CALL_TASK(CreateServer)
    CALL_TASK(StepOnePoolEPBug866)
    CALL_TASK(StepTwoPoolEPBug866)
  }
  //StepThree: Put some portfolio/Position objects
...
void runJavaInterOpsUsingLinkedList(bool poolConfig = false, bool withLocators = false)
{
  if(withLocators){
	CALL_TASK(StartLocator)
	CALL_TASK(CreateServerWithLocator2)
	...
	CALL_TASK(CreateServer2)
	CALL_TASK(StepOnePoolEP1)
	CALL_TASK(StepTwoPoolEP1)
  }
...
//test case that checks for Invalid Usage and corr. IllegalStatException for PDXReader And PDXWriter APIs.
void _disable_see_bug_999_testReaderWriterInvalidUsage(bool poolConfig = false, bool withLocators = false)
{
  if(withLocators){
    CALL_TASK(StartLocator)
    CALL_TASK(CreateServerWithLocator2)
...
void runPdxMetadataCheckTest(bool poolConfig = false, bool withLocators = false){
  if(withLocators){
    CALL_TASK(StartLocator)
...
DUNIT_MAIN
{
 ...
 //Test LinkedList
  {
    runJavaInterOpsUsingLinkedList(true);  //pool with server endpoints
    runJavaInterOpsUsingLinkedList(true, true);  // pool with locators
  }
  ...
  //PdxMetadataCheckTest
  {
    ...
    runPdxMetadataCheckTest(true, true);
  }

  //PdxBankTest
  {
    runPdxBankTest(true);
    runPdxBankTest(true, true);
  }

  //Polymorphic-multilevel inheritance
  {
    testPolymorphicUseCase(true);
    testPolymorphicUseCase(true, true);
  }
