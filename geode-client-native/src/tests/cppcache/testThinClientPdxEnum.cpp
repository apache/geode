...
{  
  ...
  initClient(true);
  if(locator){
    createPool("__TEST_POOL1__", locatorsG, NULL, NULL, 0, true );
    createRegionAndAttachPool( "DistRegionAck", USE_ACK, "__TEST_POOL1__");        
  }else {
    createPool("__TEST_POOL1__", NULL, NULL,endPoints, 0, true );
    createRegionAndAttachPool( "DistRegionAck", USE_ACK, "__TEST_POOL1__");    
  }  
...
{ 
  ...
  QueryPtr qry = qs->newQuery("select distinct * from /DistRegionAck this where m_enumid.name = 'id3'");
  results = qry->execute();
...
void runThinClientPdxInstance(bool isLocator)
{
  if( isLocator )
  {
...
DUNIT_MAIN
{
  runThinClientPdxInstance( true );   // Locator, caching = true
  runThinClientPdxInstance( false );  // EP, caching = true  
}
END_MAIN
