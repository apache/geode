...
class QueryHelper
{
  ...
  virtual void populatePortfolioData(RegionPtr & pregion, int setSize, int numSets , int32_t objSize=1, CacheableStringArrayPtr nm=NULLPTR);
  virtual void populatePositionData (RegionPtr & pregion, int setSize, int numSets );
  virtual void populatePortfolioPdxData(RegionPtr & pregion, int setSize, int numSets , int32_t objSize=1, char** nm=NULL);
  virtual void populatePositionPdxData (RegionPtr & pregion, int setSize, int numSets );
  ...
  virtual bool verifySS(SelectResultsPtr & structset, int rowCount, int fieldCount);
  ...
  bool isExpectedRowsConstantRS(int queryindex) {
    for( int i = (sizeof(constantExpectedRowsRS)/sizeof(int)) - 1; i > -1; i--)
  ...
  bool isExpectedRowsConstantPQRS(int queryindex) {
    for( int i = (sizeof(constantExpectedRowsPQRS)/sizeof(int)) - 1; i > -1; i--)
  ...
  bool isExpectedRowsConstantSS(int queryindex) {
    for( int i = (sizeof(constantExpectedRowsSS)/sizeof(int)) - 1; i > -1; i--) {
  ...
  bool isExpectedRowsConstantSSPQ(int queryindex) {
    for( int i = (sizeof(constantExpectedRowsSSPQ)/sizeof(int)) - 1; i > -1; i--) {
...
void QueryHelper::populatePortfolioData(RegionPtr & rptr, int setSize, int numSets, int32_t objSize, CacheableStringArrayPtr nm)
{
  ...
  for(int set=1; set<=numSets; set++)
  {
    for(int current=1; current<=setSize; current++)
    {
      ...
      //printf(" QueryHelper::populatePortfolioData creating key = %s and puting data \n",portname);
      rptr->put(keyport, port);
    }
  }
  //portfolioSetSize = setSize; portfolioNumSets = numSets; objectSize = objSize;
...
void QueryHelper::populatePositionData(RegionPtr & rptr, int setSize, int numSets)
{
  ...
  for(int set=1; set<=numSets; set++)
  {
    for(int current=1; current<=setSize; current++)
    {
      CacheablePtr pos(new Position(secIds[current % numSecIds], current*100));
...
void QueryHelper::populatePortfolioPdxData(RegionPtr & rptr, int setSize, int numSets, int32_t objSize, char** nm)
{
  ...
  for(int set=1; set<=numSets; set++)
  {
    for(int current=1; current<=setSize; current++)
    {
      ...
      rptr->put(keyport, port);
      LOGINFO("populatePortfolioPdxData:: Put for iteration current = %d done", current);
    }
  }
  //portfolioSetSize = setSize; portfolioNumSets = numSets; objectSize = objSize;
...
void QueryHelper::populatePositionPdxData(RegionPtr & rptr, int setSize, int numSets)
{
  int numSecIds = sizeof(secIds)/sizeof(char*);
  ...
  for(int set=1; set<=numSets; set++)
  {
    for(int current=1; current<=setSize; current++)
    {
      CacheablePtr pos(new PositionPdx(secIds[current % numSecIds], current*100));
...
void QueryHelper::populatePDXObject(RegionPtr & rptr) {
  ...
  LOGINFO("get... Result-1: Returned float=%f, String val = %s double=%lf", obj2->getFloat(), obj2->getString(), obj2->getDouble());
  //LOGINFO("get.. Result-2: Returned BOOL = %d and BYTE = %s SHORT=%d INT=%d", obj2->getBool(), obj2->getByte(), obj2->getShort(), obj2->getInt());
...
bool QueryHelper::verifySS(SelectResultsPtr & structSet, int expectedRows, int expectedFields)
{
  if (!instanceOf<StructSetPtr>(structSet)) {
    if( expectedRows == 0 && expectedFields == 0 ) return true; //quite possible we got a null set back.
  ...
  for(SelectResults::Iterator iter = ssptr->begin(); iter != ssptr->end(); iter++)
  {
