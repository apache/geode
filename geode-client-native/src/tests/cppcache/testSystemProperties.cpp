...
using namespace gemfire;
  ...
  const bool checkSecurityProperties( PropertiesPtr securityProperties , const char* key , const char* value ) {
    bool flag ;
...
BEGIN_TEST(NEW_CONFIG)
{
  ...
  ASSERT( sp->redundancyMonitorInterval() == 456, "expected 456 redundancyMonitorInterval");
  ...
  ASSERT( sp->notifyDupCheckLife() == 4321, "expected 4321 notifyDupCheckLife");
  ...
  ASSERT( checkSecurityProperties(securityProperties ,"security-username", "username") == true , "SecurityProperties Not Stored" );
  ASSERT( checkSecurityProperties(securityProperties,"security-password", "password") == true , "SecurityProperties Not Stored" );
