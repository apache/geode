...
{
  initClient(true);  
  getHelper()->createPoolWithLocators("__TEST_POOL1__", locatorsG, false, -1 , -1, -1, false, group1);
...
{
  initClient(true);
  getHelper()->createPoolWithLocators("__TEST_POOL2__", locatorsG, false, -1 , -1, -1, false, group2);
  getHelper()->createRegionAndAttachPool(regionNames[0], USE_ACK, "__TEST_POOL2__",false);                              
...
{
  initClient(true);
  getHelper()->createPoolWithLocators("__TEST_POOL3__", locatorsG, false, -1 , -1, -1, false, group3);
  getHelper()->createRegionAndAttachPool(regionNames[0], USE_ACK, "__TEST_POOL3__",false);                              
...
DUNIT_TASK_DEFINITION(CLIENT1, CheckPrSingleHopForIntKeysTask_CLIENT1)
{
  ...
  for(int i = 0; i < 1000; i++)
  {
    ...
    try {      
      ...
      LOGINFO("CheckPrSingleHopForIntKeysTask_CLIENT1: networkhop %d ", networkhop);  
      if (networkhop){
        failureCount++;
      }
      int8 serverGroupFlag = TestUtils::getCacheImpl(getHelper( )->cachePtr)->getAndResetServerGroupFlag();
      LOGINFO("CheckPrSingleHopForIntKeysTask_CLIENT1: serverGroupFlag is %d ", serverGroupFlag); 
      ASSERT( serverGroupFlag != 2, "serverGroupFlag should not be equal to 2" );     
      ...
      LOGINFO("CheckPrSingleHopForIntKeysTask_CLIENT1: nonSingleHopCount is %d & metadatarefreshCount is %d failureCount = %d", nonSingleHopCount, metadatarefreshCount, failureCount);     
    }
  ...
  ASSERT( metadatarefreshCount < 70, "metadatarefreshCount should be less than 70" );
  ...
  for(int i = 0; i < 1000; i++)
  {
    ...
    try {      
      LOGINFO("CPPTEST: destroying key %d with hashcode %d", i, (int32_t)keyPtr->hashcode());      
...
DUNIT_TASK_DEFINITION(CLIENT2, CheckPrSingleHopForIntKeysTask_CLIENT2)
{
  ...
  for(int i = 0; i < 1000; i++)
  {
    ...
    try {      
      ...
      dataReg->put(keyPtr,(int32_t)keyPtr->hashcode());
      bool networkhop = TestUtils::getCacheImpl(getHelper( )->cachePtr)->getAndResetNetworkHopFlag();
      LOGINFO("CheckPrSingleHopForIntKeysTask_CLIENT2: networkhop %d ", networkhop);  
      if (networkhop){
        failureCount++;
      }
      int8 serverGroupFlag = TestUtils::getCacheImpl(getHelper( )->cachePtr)->getAndResetServerGroupFlag();
      LOGINFO("CheckPrSingleHopForIntKeysTask_CLIENT2: serverGroupFlag is %d ", serverGroupFlag); 
      ASSERT( serverGroupFlag != 2, "serverGroupFlag should not be equal to 2" );     
      ...
      LOGINFO("CheckPrSingleHopForIntKeysTask_CLIENT2: nonSingleHopCount is %d & metadatarefreshCount is %d ", nonSingleHopCount, metadatarefreshCount);     
    }
   catch(CacheServerException&) {      
      LOGERROR("CPPTEST: Put caused extra hop.");
      FAIL( "Put caused extra hop." );
  ...
  ASSERT( metadatarefreshCount > 0, "metadatarefreshCount should be greater than 1" );
  ...
  for(int i = 1000; i < 2000; i++)
  {
    ...
    try {
      ...
      int8 serverGroupFlag = TestUtils::getCacheImpl(getHelper( )->cachePtr)->getAndResetServerGroupFlag();
      LOG("after gatall ");
      LOGINFO("CheckPrSingleHopForIntKeysTask_CLIENT2: serverGroupFlag is %d getall size = %d", serverGroupFlag, values->size()); 
...
DUNIT_TASK_DEFINITION(CLIENT3, CheckPrSingleHopForIntKeysTask_CLIENT3)
{
  ...
  for(int i = 0; i < 1000; i++)
  {
    ...
    try {      
      ...
      dataReg->put(keyPtr,(int32_t)keyPtr->hashcode());
      bool networkhop = TestUtils::getCacheImpl(getHelper( )->cachePtr)->getAndResetNetworkHopFlag();
      LOGINFO("CheckPrSingleHopForIntKeysTask_CLIENT3: networkhop %d ", networkhop);  
      if (networkhop){
        failureCount++;
      }
      int8 serverGroupFlag = TestUtils::getCacheImpl(getHelper( )->cachePtr)->getAndResetServerGroupFlag();
      LOGINFO("CheckPrSingleHopForIntKeysTask_CLIENT3: serverGroupFlag is %d ", serverGroupFlag); 
      ASSERT( serverGroupFlag != 2, "serverGroupFlag should not be equal to 2" );     
      ...
      LOGINFO("CheckPrSingleHopForIntKeysTask_CLIENT3: nonSingleHopCount is %d & metadatarefreshCount is %d ", nonSingleHopCount, metadatarefreshCount);     
    }
   catch(CacheServerException&) {      
      LOGERROR("CPPTEST: Put caused extra hop.");
      FAIL( "Put caused extra hop." );
...
DUNIT_TASK_DEFINITION(SERVER1, CreateServers_With_Locator_PR)
{
  ...
  //starting servers
  if ( isLocalServer )
    CacheHelper::initServer( 1, "cacheserver1_partitioned_servergroup.xml", locatorsG );

  //starting servers
  if ( isLocalServer )
    CacheHelper::initServer( 2, "cacheserver2_partitioned_servergroup.xml", locatorsG );

  //starting servers
  if ( isLocalServer )
    CacheHelper::initServer( 3, "cacheserver3_partitioned_servergroup.xml", locatorsG );
}
END_TASK_DEFINITION

DUNIT_MAIN
{
  // server-group applicable only with locators, so no test case with pool endpoints.
