...
class CacheableWrapper
{
  ...
  public:
    ...
    virtual CacheablePtr getCacheable( ) const
    {
...
class CacheableWrapperFactory
{
  public:
    ...
    static void registerType( int8_t typeId,
        const std::string wrapperType, const CacheableWrapperFunc wrapperFunc,
        const bool isKey );
...
std::map<int8_t,
  CacheableWrapperFunc> CacheableWrapperFactory::m_registeredKeyMap;
std::map<int8_t,
  CacheableWrapperFunc> CacheableWrapperFactory::m_registeredValueMap;
std::map<int8_t,
  std::string> CacheableWrapperFactory::m_typeIdNameMap;
...
void CacheableWrapperFactory::registerType( int8_t typeId,
    const std::string wrapperType, const CacheableWrapperFunc factoryFunc,
    const bool isKey )
{
  if ( isKey ) {
...
std::vector<int8_t> CacheableWrapperFactory::getRegisteredKeyTypes( )
{
  ...
  for ( keyMapIterator = m_registeredKeyMap.begin( ); keyMapIterator !=
      m_registeredKeyMap.end( ); ++keyMapIterator ) {
...
std::vector<int8_t> CacheableWrapperFactory::getRegisteredValueTypes( )
{
  ...
  for ( valueMapIterator = m_registeredValueMap.begin( ); valueMapIterator !=
      m_registeredValueMap.end( ); ++valueMapIterator ) {
