...
const char * vals[] = { "Value-1", "Value-2", "Value-3", "Value-4", "Non-Existent-Value" };
const char * nvals[] = { "New Value-1", "New Value-2", "New Value-3", "New Value-4" };

const char * regionNames[] = { "DistRegionAck", "DistRegionNoAck", "exampleRegion" };
...
{
  if ( isLocalServer )
    CacheHelper::initServer( 2, "cacheserver1_expiry.xml" );
  LOG("SERVER2 started");
}
END_TASK_DEFINITION
...
{
  initClient(true);
  createRegion( regionNames[2], USE_ACK, endPoints);  
...
{
  initClient(true);
  createPooledRegion( regionNames[2], USE_ACK, NULL, locatorsG, "__TESTPOOL1_" );  
...
DUNIT_TASK_DEFINITION(CLIENT1, StepSeven_Pooled_EndPoint)
{
  initClient(true);
  createPooledRegion( regionNames[2], USE_ACK, endPoints, NULL, "__TESTPOOL1_" );  
  LOG( "StepSeven_Pooled_EndPoint complete." );
}
END_TASK_DEFINITION
...
DUNIT_TASK_DEFINITION(CLIENT1, StepFive)
{
  ...
  ASSERT( reg0->remove(keys[4], vals[4]) == false, "Result of remove should be false, as this entry is not present in first region." );  
  ASSERT( reg1->remove(keys[4], vals[4]) == false, "Result of remove should be false, as this entry is not present in second region." );

  // Try removing non-existent key, but existing value from regions, result should be false.
  ASSERT( reg0->remove(keys[4], vals[0]) == false, "Result of remove should be false, as this key is not present in first region." );
  ASSERT( reg1->remove(keys[4], vals[0]) == false, "Result of remove should be false, as this key is not present in second region." );

  // Try removing existent key, but non-existing value from regions, result should be false.
  ASSERT( reg0->remove(keys[0], vals[4]) == false, "Result of remove should be false, as this value is not present in first region." );
  ASSERT( reg1->remove(keys[0], vals[4]) == false, "Result of remove should be false, as this value is not present in second region." );

  // Try removing existent key, and existing value from regions, result should be true.
  ASSERT( reg0->remove(keys[0], vals[0]) == true, "Result of remove should be true, as this entry is present in first region." );
  ASSERT( reg1->remove(keys[2], vals[2]) == true, "Result of remove should be true, as this entry is present in second region." );

  ASSERT(reg0->containsKey(keys[0]) == false, "containsKey should be false");
  ASSERT(reg0->containsKeyOnServer(keyPtr) == false, "containsKeyOnServer should be false");
  ASSERT(reg1->containsKey(keys[2]) == false, "containsKey should be false");  
  ASSERT(reg1->containsKeyOnServer(keyPtr1) == false, "containsKeyOnServer should be false");

  // Try removing already deleted entry from regions, result should be false, but no exception.
  ASSERT( reg0->remove(keys[0], vals[0]) == false, "Result of remove should be false, as this entry is not present in first region." );
  ASSERT( reg1->remove(keys[0], vals[0]) == false, "Result of remove should be false, as this entry is not present in second region." );

  // Try locally destroying already deleted entry from regions, It should result into exception.
  try
  {
    reg0->localDestroy(keys[0]);
    FAIL( "local destroy on already removed key should have thrown EntryNotFoundException" );
...
DUNIT_TASK_DEFINITION(CLIENT2, StepSix)
{
  ...
  ASSERT( reg0->remove(keys[1], nvals[1]) == true, "Result of remove should be true, as this value is present locally, & also present on server." );  
  ASSERT( reg1->remove(keys[3], nvals[3]) == true, "Result of remove should be true, as this value is present locally, & also present on server." );
  ...
  ASSERT( reg0->remove(keys[1], nvals[1]) == false, "Result of remove should be false, as this value is present locally, but not present on server." );  
  ASSERT( reg1->remove(keys[3], nvals[3]) == false, "Result of remove should be false, as this value is present locally, but not present on server." );
  ASSERT(reg0->containsKey(keys[1]) == true, "containsKey should be true");
  ASSERT(reg0->containsKeyOnServer(keyPtr) == true, "containsKeyOnServer should be true");
  ASSERT(reg1->containsKey(keys[3]) == true, "containsKey should be true");  
  ASSERT(reg1->containsKeyOnServer(keyPtr1) == true, "containsKeyOnServer should be true");
  ...
  localPutEntry(regionNames[0], keys[1], nvals[1]);
  localPutEntry(regionNames[1], keys[3], nvals[3]);
  ASSERT( reg0->remove(keys[1], vals[1]) == false, "Result of remove should be false, as this value is not present locally, but present only on server." );  
  ASSERT( reg1->remove(keys[3], vals[3]) == false, "Result of remove should be false, as this value is not present locally, but present only on server." );
  ASSERT(reg0->containsKey(keys[1]) == true, "containsKey should be true");
  ASSERT(reg0->containsKeyOnServer(keyPtr) == true, "containsKeyOnServer should be true");
  ASSERT(reg1->containsKey(keys[3]) == true, "containsKey should be true");  
  ASSERT(reg1->containsKeyOnServer(keyPtr1) == true, "containsKeyOnServer should be true");
  ...
  ASSERT( reg0->remove(keys[1], nvals[1]) == false, "Result of remove should be false, as this value is not present locally, but present only on server." );  
  ASSERT( reg1->remove(keys[3], nvals[3]) == false, "Result of remove should be false, as this value is not present locally, but present only on server." );
  ASSERT(reg0->containsKey(keys[1]) == true, "containsKey should be true");
  ASSERT(reg0->containsKeyOnServer(keyPtr) == true, "containsKeyOnServer should be true");
  ASSERT(reg1->containsKey(keys[3]) == true, "containsKey should be true");  
  ASSERT(reg1->containsKeyOnServer(keyPtr1) == true, "containsKeyOnServer should be true");
  ...
  ASSERT( reg0->remove(keys[1], (CacheablePtr)NULLPTR) == false, "Result of remove should be false, as this value is not present locally, but present only on server." );  
  ASSERT( reg1->remove(keys[3], (CacheablePtr)NULLPTR) == false, "Result of remove should be false, as this value is not present locally, but present only on server." );
  ASSERT(reg0->containsKey(keys[1]) == true, "containsKey should be true");
  ASSERT(reg0->containsKeyOnServer(keyPtr) == true, "containsKeyOnServer should be true");
  ASSERT(reg1->containsKey(keys[3]) == true, "containsKey should be true");  
  ASSERT(reg1->containsKeyOnServer(keyPtr1) == true, "containsKeyOnServer should be true");
  ...
  ASSERT( reg0->remove("NewKey1", "NewValue1") == false, "Result of remove should be false, as this value is not present locally, and not present on server." );  
  ASSERT( reg1->remove("NewKey3", "NewValue3") == false, "Result of remove should be false, as this value is not present locally, and not present on server." );
  CacheableKeyPtr keyPtr2 = CacheableString::create("NewKey1");
  CacheableKeyPtr keyPtr3 = CacheableString::create("NewKey3");
  ASSERT(reg0->containsKey("NewKey1") == false, "containsKey should be false");
  ASSERT(reg0->containsKeyOnServer(keyPtr2) == false, "containsKeyOnServer should be false");
  ASSERT(reg1->containsKey("NewKey3") == false, "containsKey should be false");  
  ASSERT(reg1->containsKeyOnServer(keyPtr3) == false, "containsKeyOnServer should be false");
  ...
  ASSERT( reg0->remove("NewKey1", (CacheablePtr)NULLPTR) == false, "Result of remove should be false, as this value is not present locally, and not present on server." );  
  ASSERT( reg1->remove("NewKey3", (CacheablePtr)NULLPTR) == false, "Result of remove should be false, as this value is not present locally, and not present on server." );
  ASSERT(reg0->containsKey("NewKey1") == false, "containsKey should be false");
  ASSERT(reg0->containsKeyOnServer(keyPtr2) == false, "containsKeyOnServer should be false");
  ASSERT(reg1->containsKey("NewKey3") == false, "containsKey should be false");  
  ASSERT(reg1->containsKeyOnServer(keyPtr3) == false, "containsKeyOnServer should be false");
  ...
  ASSERT( reg0->remove(keys[1], nvals[1]) == true, "Result of remove should be true, as this value does not exist locally, but exists on server." );  
  ASSERT( reg1->remove(keys[3], nvals[3]) == true, "Result of remove should be true, as this value does not exist locally, but exists on server." );  
  ASSERT(reg0->containsKey(keys[1]) == false, "containsKey should be false");
  ASSERT(reg0->containsKeyOnServer(keyPtr) == false, "containsKeyOnServer should be false");
  ASSERT(reg1->containsKey(keys[3]) == false, "containsKey should be false");  
  ASSERT(reg1->containsKeyOnServer(keyPtr1) == false, "containsKeyOnServer should be false");
  ...
  reg0->destroy(keys[1]);
  reg1->destroy(keys[3]);
  ASSERT( reg0->remove(keys[1], (CacheablePtr)NULLPTR) == false, "Result of remove should be false, as this value does not exist locally, but exists on server." );  
  ASSERT( reg1->remove(keys[3], (CacheablePtr)NULLPTR) == false, "Result of remove should be false, as this value does not exist locally, but exists on server." );  
  ASSERT(reg0->containsKey(keys[1]) == false, "containsKey should be false");
  ASSERT(reg0->containsKeyOnServer(keyPtr) == false, "containsKeyOnServer should be false");
  ASSERT(reg1->containsKey(keys[3]) == false, "containsKey should be false");  
  ASSERT(reg1->containsKeyOnServer(keyPtr1) == false, "containsKeyOnServer should be false");
  ...
  ASSERT( reg0->remove(keys[1], vals[1]) == true, "Result of remove should be true, as this value does not exists locally." );  
  ASSERT( reg0->remove(keys[1], vals[1]) == false, "Result of remove should be false, as this value does not exists locally." );  
  ASSERT( reg1->remove(keys[3], vals[3]) == true, "Result of remove should be false, as this value does not exists locally." );  
  ASSERT( reg1->remove(keys[3], vals[3]) == false, "Result of remove should be false, as this value does not exists locally." );
  ASSERT(reg0->containsKey(keys[1]) == false, "containsKey should be false");
  ASSERT(reg1->containsKey(keys[3]) == false, "containsKey should be false");
  ...
  {
    reg0->destroy(keys[1]);
    FAIL( "destroy on already removed key should have thrown EntryNotFoundException" );
  } catch(EntryNotFoundException/*& ex*/)
  {
    LOG( "Got expected EntryNotFoundException for "
  ...
  ASSERT( reg0->localRemove(keys[1], vals[1]) == true, "Result of remove should be true, as this value exists locally." );  
  ASSERT( reg1->localRemove(keys[3], vals[3]) == true, "Result of remove should be true, as this value exists locally." );  
  ASSERT(reg0->containsKey(keys[1]) == false, "containsKey should be false");
  ASSERT(reg1->containsKey(keys[3]) == false, "containsKey should be false");
  ...
  reg0->localPut(keys[1], vals[1]);
  reg1->localPut(keys[3], vals[3]);
  ASSERT( reg0->localRemove(keys[1], nvals[1]) == false, "Result of remove should be false, as this value does not exists locally." );  
  ASSERT( reg1->localRemove(keys[3], nvals[3]) == false, "Result of remove should be false, as this value does not exists locally." );  
  ASSERT(reg0->containsKey(keys[1]) == true, "containsKey should be true");
  ASSERT(reg1->containsKey(keys[3]) == true, "containsKey should be true");
  ...
  reg0->invalidate(keys[1]);
  reg1->invalidate(keys[3]);
  ASSERT( reg0->localRemove(keys[1], vals[1]) == false, "Result of remove should be false, as this value does not exists locally." );  
  ASSERT( reg1->localRemove(keys[3], vals[3]) == false, "Result of remove should be false, as this value does not exists locally." );  
  ASSERT(reg0->containsKey(keys[1]) == true, "containsKey should be true");
  ASSERT(reg1->containsKey(keys[3]) == true, "containsKey should be true");
  ...
  reg0->invalidate(keys[1]);
  reg1->invalidate(keys[3]);
  ASSERT( reg0->localRemove(keys[1], (CacheablePtr)NULLPTR) == true, "Result of remove should be true, as this value does not exists locally." );  
  ASSERT( reg1->localRemove(keys[3], (CacheablePtr)NULLPTR) == true, "Result of remove should be true, as this value does not exists locally." );  
  ASSERT(reg0->containsKey(keys[1]) == false, "containsKey should be false");
  ASSERT(reg1->containsKey(keys[3]) == false, "containsKey should be false");
  ...
  reg0->localPut(keys[1], vals[1]);
  reg1->localPut(keys[3], vals[3]);
  ASSERT( reg0->localRemove(keys[1], (CacheablePtr)NULLPTR) == false, "Result of remove should be false, as this value does not exists locally." );  
  ASSERT( reg1->localRemove(keys[3], (CacheablePtr)NULLPTR) == false, "Result of remove should be false, as this value does not exists locally." );  
  ASSERT(reg0->containsKey(keys[1]) == true, "containsKey should be true");
  ASSERT(reg1->containsKey(keys[3]) == true, "containsKey should be true");
  ...
  reg0->localPut(keys[1], vals[1]);
  reg1->localPut(keys[3], vals[3]);
  ASSERT( reg0->localRemove(keys[1], vals[1]) == true, "Result of remove should be true, as this value does not exists locally." );  
  ASSERT( reg0->localRemove(keys[1], vals[1]) == false, "Result of remove should be false, as this value does not exists locally." );  
  ASSERT( reg1->localRemove(keys[3], vals[3]) == true, "Result of remove should be false, as this value does not exists locally." );  
  ASSERT( reg1->localRemove(keys[3], vals[3]) == false, "Result of remove should be false, as this value does not exists locally." );  
  ASSERT(reg0->containsKey(keys[1]) == false, "containsKey should be false");
  ASSERT(reg1->containsKey(keys[3]) == false, "containsKey should be false");
...
DUNIT_TASK_DEFINITION(CLIENT1, StepEight)
{
  ...
  ASSERT( reg->remove(keys[0], vals[0]) == false, "Result of remove should be false, as this value is present locally, but not present on server." );  
  ASSERT( reg->remove(keys[1], vals[1]) == false, "Result of remove should be false, as this value is present locally, but not present on server." );
  ASSERT(reg->containsKey(keys[0]) == true, "containsKey should be true");
  ASSERT(reg->containsKeyOnServer(keyPtr) == true, "containsKeyOnServer should be true");
  ASSERT(reg->containsKey(keys[1]) == true, "containsKey should be true");  
  ASSERT(reg->containsKeyOnServer(keyPtr1) == true, "containsKeyOnServer should be true");
  ...
  SLEEP(10000); //This is for expiration on server to execute.
  ASSERT( reg->remove(keys[0], (CacheablePtr)NULLPTR) == true, "Result of remove should be true, as this value is not present locally, & not present on server." );  
  ASSERT( reg->remove(keys[1], (CacheablePtr)NULLPTR) == true, "Result of remove should be true, as this value is not present locally, & not present on server." );  
  ASSERT(reg->containsKeyOnServer(keyPtr) == false, "containsKeyOnServer should be false");
  ...
  ASSERT(reg->containsKeyOnServer(keyPtr1) == false, "containsKeyOnServer should be false");
  ...
  putEntry( regionNames[2], keys[1], nvals[1] );
  SLEEP(10000); //This is for expiration on server to execute.
  ASSERT( reg->remove(keys[0], (CacheablePtr)NULLPTR) == false, "Result of remove should be false, as this value is present locally, & not present on server." );  
  ASSERT( reg->remove(keys[1], (CacheablePtr)NULLPTR) == false, "Result of remove should be false, as this value is present locally, & not present on server." );
  ASSERT(reg->containsKey(keys[0]) == true, "containsKey should be true");
  ASSERT(reg->containsKeyOnServer(keyPtr) == true, "containsKeyOnServer should be true");
  ASSERT(reg->containsKey(keys[1]) == true, "containsKey should be true");  
  ASSERT(reg->containsKeyOnServer(keyPtr1) == true, "containsKeyOnServer should be true");
  ...
  try {
	reg->destroy("key300");
	FAIL( "destroy on non=existent key should have thrown EntryNotFoundException" );
  } catch(EntryNotFoundException& /*ex*/) {
  ...
  ASSERT( keys.size() == reg->size(), "region size should be equal to keys size");

  VectorOfCacheable values;
  reg->values(values);
  LOGINFO("Region values = %d ", values.size());
  ASSERT( values.size() == reg->size(), "region size should be equal to values size");
...
DUNIT_TASK_DEFINITION(CLIENT1, StepFiveA)
{
  ...
  // Try removing non-existent entry from regions, result should be false.
  ASSERT( reg0->removeEx(keys[4]) == false, "Result of remove should be false, as this entry is not present in first region." );
  ASSERT( reg1->removeEx(keys[4]) == false, "Result of remove should be false, as this entry is not present in second region." );

  // Try removing existent key, and existing value from regions, result should be true.

  ASSERT( reg0->removeEx(keys[0]) == true, "Result of remove should be true, as this entry is present in first region." );
  ASSERT( reg1->removeEx(keys[2]) == true, "Result of remove should be true, as this entry is present in second region." );
  ...
  // Try removing already deleted entry from regions, result should be false, but no exception.
  ASSERT( reg0->removeEx(keys[0]) == false, "Result of remove should be false, as this entry is not present in first region." );
  ASSERT( reg1->removeEx(keys[0]) == false, "Result of remove should be false, as this entry is not present in second region." );
...
DUNIT_TASK_DEFINITION(CLIENT1,StepTwelve)
{
    ...
    CacheableStringPtr vString = dynCast<CacheableStringPtr>(regPtr0->get(intKey));
    ASSERT( strcmp ( vString->asChar() , strVal ) == 0,"Int Key and String Value Mismatch"  );

    int key1 = 1;
    int val1 = 20;
    try {
      regPtr0->localCreate(CacheableInt32::create(key1), CacheableInt32::create(val1));
    ...
    int64_t longVal = 200;
    regPtr0->localCreate(CacheableInt64::create(longKey), CacheableInt64::create(longVal));
    CacheableInt64Ptr vLong = dynCast<CacheableInt64Ptr>(regPtr0->get(CacheableInt64::create(longKey)));
    ...
    CacheableInt32Ptr vIntPtr = dynCast<CacheableInt32Ptr>(regPtr0->get(strKey));
    ...
    CacheableStringPtr vStr = dynCast<CacheableStringPtr>(regPtr0->get(CacheableInt64::create(longKey)));
    ASSERT ( strcmp ( vStr->asChar() , strVal ) == 0,"Long Key and String Value Mismatch"  );
    ...
    ASSERT ( strcmp ( v->asChar() ,  strVal) == 0 ,"String Key and String Value Mismatch");

    int i = 1234;
    regPtr0->localCreate(CacheableInt32::create(i), CacheableInt32::create(i));
    CacheableInt32Ptr result = dynCast<CacheableInt32Ptr>(regPtr0->get(CacheableInt32::create(i)));
    ASSERT (result->value() == i,"localcreate new entry with cacheableInt key and cacheableInt value Fail") ;

    regPtr0->localCreate(CacheableInt32::create(12345), CacheableString::create("abced"));
    CacheableStringPtr resultString = dynCast<CacheableStringPtr>(regPtr0->get(CacheableInt32::create(12345)));
    ASSERT (strcmp(resultString->asChar() , "abced" ) == 0,"localcreate new with entry cacheableInt key and cacheablestring value Fail"  ) ;

    regPtr0->localCreate(CacheableString::create("X"), CacheableString::create("Y"));
    CacheableStringPtr resultStringY = dynCast<CacheableStringPtr>(regPtr0->get(CacheableString::create("X")));
    ASSERT (strcmp(resultStringY->asChar() , "Y" ) == 0 ,"localcreate new with entry cacheablestring key and cacheablestring value") ;

    try {
        regPtr0->localCreate(CacheableString::create("X"), CacheableString::create("Y"));
        FAIL( "Should have thrown EntryExistsException if entry already exist in region." );
    } catch ( EntryExistsException e ) {
        LOG ( "Expected : check EntryExistsException if entry already exist in region." );
    }

    regPtr0->localDestroy(CacheableString::create("X"));
    if ( dynCast<CacheableStringPtr>(regPtr0->get(CacheableString::create("X"))) != NULLPTR ) {
  ...
  ASSERT( regPtr0->localRemove(keyObject1, x) == true, "Result of remove should be true, as this value nullptr exists locally." );
  ASSERT(regPtr0->containsKey(keyObject1) == false, "containsKey should be false");
  regPtr0->localCreate(keyObject1, x);
  ASSERT( regPtr0->localRemoveEx(keyObject1) == true, "Result of remove should be true, as this value does not exists locally." );
  ASSERT(regPtr0->containsKey(keyObject1) == false, "containsKey should be false");
  regPtr0->localCreate(keyObject1, x);
  ...
  retVal = regPtr0->get(keyObject1);
  ASSERT(retVal == NULLPTR,"value should not be found");
  ASSERT( regPtr0->localRemove(keyObject1, 1) == false, "Result of remove should be false, as this value does not exists locally." );
  ASSERT(regPtr0->containsKey(keyObject1) == true, "containsKey should be true");
  regPtr0->localPut(keyObject1, 1);
  ASSERT( regPtr0->localRemoveEx(keyObject1) == true, "Result of remove should be true, as this value exists locally." );
  ASSERT(regPtr0->containsKey(keyObject1) == false, "containsKey should be false");
  ...
  if(regPtr0->containsKey(keyObject3)){
    ...
    ASSERT(strcmp(strVal1->toString()->asChar(),"testString") ==0,"strVal should be testString.");
  }
  try{
    if(regPtr0->containsKey(keyObject3)){
  ...
  PdxTests::PdxTypePtr objVal1 = dynCast<PdxTests::PdxTypePtr>(regPtr0->get(keyObject4));
  ...
  ASSERT( regPtr0->localRemove(keyObject4, valObject1) == false, "Result of remove should be false, as this value does not exists locally." );
  ASSERT(regPtr0->containsKey(keyObject4) == true, "containsKey should be true");
  regPtr0->localPut(keyObject4, valObject1);
  ASSERT( regPtr0->localRemove(keyObject4, valObject1) == true, "Result of remove should be true, as this value exists locally." );
  ASSERT(regPtr0->containsKey(keyObject4) == false, "containsKey should be false");
  regPtr0->localPut(keyObject4, valObject1);
  ASSERT( regPtr0->localRemoveEx(keyObject4) == true, "Result of remove should be true, as this value exists locally." );
  ASSERT(regPtr0->containsKey(keyObject4) == false, "containsKey should be false");
  regPtr0->localPut(keyObject4, valObject1);
  regPtr0->localDestroy(keyObject4);
  /*try {
  	objVal1 = dynCast<PdxTests::PdxTypePtr>(regPtr0->get(keyObject4));;// need to verify that if entry is deleted then some exception should be thrown
  ...
  }catch (Exception)
  {
   LOG (" Expected  exception thrown by get");
    ...
    createRegion( "ABC", USE_ACK, endPoints, true, true);
    RegionPtr regPtr2 = getHelper()->getRegion("ABC");
    regPtr2->localDestroyRegion();
    try {
        regPtr2->localCreate(CacheableString::create("XY"), CacheableString::create("Y"));
        FAIL ("localDestroyRegion should have thrown exception");
    } catch ( RegionDestroyedException e ) {
        LOG (" Expected exception thrown by localCreate, since Region does not exist");
    }
}
END_TASK_DEFINITION
...
DUNIT_TASK_DEFINITION(CLIENT2, StepSixA)
{
  ...
  // Try removing value that is present on client as well as server, result should be true.
  ASSERT( reg0->removeEx(keys[1]) == true, "Result of remove should be true, as this value is present locally, & also present on server." );
  ASSERT( reg1->removeEx(keys[3]) == true, "Result of remove should be true, as this value is present locally, & also present on server." );
  ASSERT(reg0->containsKey(keys[1]) == false, "containsKey should be false");
  ASSERT(reg0->containsKeyOnServer(keyPtr) == false, "containsKeyOnServer should be false");
  ASSERT(reg1->containsKey(keys[3]) == false, "containsKey should be false");
  ASSERT(reg1->containsKeyOnServer(keyPtr1) == false, "containsKeyOnServer should be false");
  ...
  reg0->invalidate(keys[1]);
  reg1->invalidate(keys[3]);
  ASSERT( reg0->removeEx(keys[1]) == true, "Result of remove should be true, as invalidated key can be deleted." );
  ASSERT( reg1->removeEx(keys[3]) == true, "Result of remove should be true, as invalidated key can be deleted." );
  LOGINFO("Step 6a.2 complete.");

  // Try removing a entry (value) which is not present on client as well as server, result should be false.
  ASSERT( reg0->removeEx("NewKey1") == false, "Result of remove should be false, as this value is not present." );
  ASSERT( reg1->removeEx("NewKey3") == false, "Result of remove should be false, as this value is not present." );
  CacheableKeyPtr keyPtr2 = CacheableString::create("NewKey1");
  CacheableKeyPtr keyPtr3 = CacheableString::create("NewKey3");
  ASSERT(reg0->containsKey("NewKey1") == false, "containsKey should be false");
  ASSERT(reg0->containsKeyOnServer(keyPtr2) == false, "containsKeyOnServer should be false");
  ...
  reg0->localDestroy(keys[1]);
  reg1->localDestroy(keys[3]);
  ASSERT( reg0->removeEx(keys[1]) == true, "Result of remove should be true, as this value does not exist locally, but exists on server." );
  ASSERT( reg1->removeEx(keys[3]) == true, "Result of remove should be true, as this value does not exist locally, but exists on server." );
  ASSERT(reg0->containsKey(keys[1]) == false, "containsKey should be false");
  ASSERT(reg0->containsKeyOnServer(keyPtr) == false, "containsKeyOnServer should be false");
  ASSERT(reg1->containsKey(keys[3]) == false, "containsKey should be false");
  ASSERT(reg1->containsKeyOnServer(keyPtr1) == false, "containsKeyOnServer should be false");
  ...
  reg0->localPut(keys[1], vals[1]);
  reg1->localPut(keys[3], vals[3]);
  ASSERT( reg0->localRemoveEx(keys[1]) == true, "Result of remove should be true, as this value exists locally." );
  ASSERT( reg1->localRemoveEx(keys[3]) == true, "Result of remove should be true, as this value exists locally." );
  ASSERT(reg0->containsKey(keys[1]) == false, "containsKey should be false");
  ASSERT(reg1->containsKey(keys[3]) == false, "containsKey should be false");
  ...
  reg0->invalidate(keys[1]);
  reg1->invalidate(keys[3]);
  ASSERT( reg0->localRemoveEx(keys[1]) == true, "Result of remove should be true." );
  ASSERT( reg1->localRemoveEx(keys[3]) == true, "Result of remove should be true." );
  ASSERT(reg0->containsKey(keys[1]) == false, "containsKey should be true");
  ASSERT(reg1->containsKey(keys[3]) == false, "containsKey should be true");
  ...
  reg0->localDestroy(keys[1]);
  reg1->localDestroy(keys[3]);
  ASSERT( reg0->localRemoveEx(keys[1]) == false, "Result of remove should be false, as this value does not exists locally." );
  ASSERT( reg1->localRemoveEx(keys[3]) == false, "Result of remove should be false, as this value does not exists locally." );
  ASSERT(reg0->containsKey(keys[1]) == false, "containsKey should be true");
  ASSERT(reg1->containsKey(keys[3]) == false, "containsKey should be true");
  ...
  reg0->localPut(keys[1], vals[1]);
  reg1->localPut(keys[3], vals[3]);
  ASSERT( reg0->localRemoveEx(keys[1]) == true, "Result of remove should be true, as this value does not exists locally." );
  ASSERT( reg0->localRemoveEx(keys[1]) == false, "Result of remove should be false, as this value does not exists locally." );
  ASSERT( reg1->localRemoveEx(keys[3]) == true, "Result of remove should be false, as this value does not exists locally." );
  ASSERT( reg1->localRemoveEx(keys[3]) == false, "Result of remove should be false, as this value does not exists locally." );
  ASSERT(reg0->containsKey(keys[1]) == false, "containsKey should be false");
  ASSERT(reg1->containsKey(keys[3]) == false, "containsKey should be false");
...
void runRemoveOps( bool poolConfig, bool isLocator ) {
...
void runRemoveOps1( bool poolConfig, bool isLocator ) {
  ...
  {    
    CALL_TASK( StepSeven_Pooled_EndPoint );    
  }
  CALL_TASK( StepEight );
  CALL_TASK( CloseCache1 );    
...
DUNIT_MAIN
{
  //runRemoveOps( false, false );
  runRemoveOps( true, false );
  runRemoveOps( true, true );

  runRemoveOps1( false, false );
  runRemoveOps1( true, false );
  runRemoveOps1( true, true );
