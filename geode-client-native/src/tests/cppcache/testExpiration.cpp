...
void startDSandCreateCache(DistributedSystemPtr& dsys, CachePtr& cache)
{
  PropertiesPtr pp = Properties::create();
  dsys = DistributedSystem::connect("TESTDSYS", pp);
  ASSERT(dsys != NULLPTR, "dsys not equal to null expected");
  cache = CacheFactory::create( "theCache", dsys );
  ASSERT(cache != NULLPTR, "cache not equal to null expected");
}

void closeCacheAndStopDS(DistributedSystemPtr& dsys, CachePtr& cache)
{
  cache->close();
  dsys->disconnect();
}
...
void setExpTimes(RegionAttributesPtr& attrs, int ettl = 0, int eit = 0, int rttl = 0, int rit = 0)
{
...
BEGIN_TEST(TEST_EXPIRATION)
{
  ...
  startDSandCreateCache(dsys, cache);

  ASSERT(dsys != NULLPTR, "Expected dsys to be NON-NULL");
  ...
  RegionPtr R1 = cache->createRegion("R1", attrs_1);
  ...
  RegionPtr R2 = cache->createRegion("R2", attrs_2);
  ...
  RegionPtr R3 = cache->createRegion("R3", attrs_3);
  ...
  RegionPtr R4 = cache->createRegion("R4", attrs_4);
  ...
  RegionPtr R5 = cache->createRegion("R5", attrs_5);
  ...
  RegionPtr R6 = cache->createRegion("R6", attrs_6);
  ...
  RegionPtr R7 = cache->createRegion("R7", attrs_7);
  ...
  RegionPtr R8 = cache->createRegion("R8", attrs_8);
  ...
  RegionPtr R9 = cache->createRegion("R9", attrs_9);
  ...
  RegionPtr R10 = cache->createRegion("R10", attrs_10);
  ...
  RegionPtr R11 = cache->createRegion("R11", attrs_11);
  ...
  ASSERT(R11->isDestroyed() == false,"Expected R11 to be alive as the get has changed the access time");
  ...
  RegionPtr R12 = cache->createRegion("R12", attrs_12);
...
/////////
  ...
  RegionPtr R14 = cache->createRegion("R14", attrs_14);
  ...
  RegionPtr R15 = cache->createRegion("R15", attrs_15);
...
//////////////
  ...
  RegionPtr R18 = cache->createRegion("R18", attrs_18);
  ...
  RegionPtr R19 = cache->createRegion("R19", attrs_19);
  ...
  ASSERT(R19->isDestroyed() == false, "Expected R19 to be alive as an entry was put");

  ACE_OS::sleep(4);

  ASSERT(R19->isDestroyed() == true, "Expected R19 to be dead");
  closeCacheAndStopDS(dsys, cache);
}
