...
DUNIT_TASK_DEFINITION(CLIENT1, Client1OpTest )
{
  ...
  try
  {
    ...
    */
    ...
    executeFunctionResult = 
         exc->withFilter(routingObj)->withArgs(args)->withCollector(myRC)->execute(getFuncName, getResult)->getResult();
    ...
    result = funcExec->withArgs(args)->withFilter(filter)->withCollector(myRC1)
...
//    PoolPtr pptr = PoolManager::find(poolName);
  ...
  //-----------------------Test with PdxObject onServers done.-------------------------------//	
    ...
    LOG("exFuncNameSendException for string arguement done.");
      ...
      result = funcExec->withArgs(args)->withCollector(myRC2)
        ->execute(exFuncNameSendException, getResult)->getResult();
    ...
    for(int i=0; i< 230; i++) {
      ...
      CacheableVectorPtr executeFunctionResult2 = exe->withFilter(fil)->execute(FEOnRegionPrSHOP, getResult, 15, true, true)->getResult();
      if(executeFunctionResult2==NULLPTR)
      {
        ASSERT(false, "executeFunctionResult2 is NULLPTR");
      } else 
      {
        sprintf(buf, "executeFunctionResult2 count = %d", executeFunctionResult2->size());
        LOGINFO(buf);
        ASSERT(1  == executeFunctionResult2->size(), "executeFunctionResult2->size() is not 1");
      ...
      CacheableVectorPtr functionResult = exe->withFilter(fil)->execute(FEOnRegionPrSHOP_OptimizeForWrite, getResult, 15, true, true)->getResult();
      ...
      } else
      {
        ...
        LOGINFO(buf);
        ASSERT(1  == functionResult->size(), "functionResult->size() is not 1");
    ...
    //-----------------------Test with single filter key onRegion done-------------------------------//
