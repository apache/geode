...
class UserThread : public ACE_Task_Base
{
  ...
  void getOp()
  {
    ...
    try
    {
      ...
      {
        char tmp[256] = {'\0'};
        sprintf(tmp, "Get ops should have passed for user id = %d for key = %s", m_userId, key);
        LOG(tmp);
        m_failed = true;
      }
    } 
      ...
      m_failed = true;
      char tmp[256] = {'\0'};
      sprintf(tmp, "Some other gemfire exception got for user id = %d", m_userId);
      LOG(tmp);
  ...
  void putOp()
  {
    ...
    try
    {
      ...
      {
        char tmp[256] = {'\0'};
        sprintf(tmp, "Put ops should have passed for user id = %d for key = %s", m_userId, key);
        LOG(tmp);
        m_failed = true;
      }
    } 
  ...
  public:
    ...
    void setParameters(int numberOfOps, int userId, PoolPtr pool, int numberOfUsers) {
    ...
    int getTotalOpsPassed()
    {
      return m_totalOpsPassed;
    }
...
DUNIT_TASK_DEFINITION(CLIENT_1, StartServer1)
{
  ...
  if ( isLocalServer ) {
    cmdServerAuthenticator = credentialGeneratorHandler->getServerCmdParams("authenticator:authorizer:authorizerPP", getXmlPath());
...
DUNIT_TASK_DEFINITION(CLIENT_1, StepOne)
{
  ...
  try {
    ...
    char tmp[256] = {'\0'};
    sprintf(tmp, "Total ops passed = %d , expected = %d", totalOpsPassed, (m_numberOfOps*m_numberOfUsers)/2);
    printf("%s\n",tmp);
...
DUNIT_TASK_DEFINITION(CLIENT_1,SetAdminTestCase1)
{
  setGlobals(1);
}
END_TASK_DEFINITION
DUNIT_TASK_DEFINITION(CLIENT_1,SetAdminTestCase2)
{
  setGlobals(2);
}
END_TASK_DEFINITION
DUNIT_TASK_DEFINITION(CLIENT_1,SetAdminTestCase3)
{
  setGlobals(3);
}
END_TASK_DEFINITION
