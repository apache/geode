...
void createCommRegions( int redundancy, const char* endpointsList = endPoints)
{
  ...
  if ( g_poolConfig ) {
    ...
    createRegion( regionNames[1], NO_ACK, true );
  }
}
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOne)
{
  ...
  // ARB: Sleep to allow update on no-ack region to propagate to all servers.
...
void runThinClientMixedRedundancy(bool poolConfig = true, bool poolLocator = true)
{
  initLocatorSettings(poolConfig, poolLocator);
...
DUNIT_MAIN
{
  runThinClientMixedRedundancy( false );
  runThinClientMixedRedundancy( true, false );
  runThinClientMixedRedundancy( true, true );
}
END_MAIN
