...
public:
   ...
   CacheablePtr load(const RegionPtr& rp,
    ...
    int32_t loadValue = dynCast<CacheableInt32Ptr>(TallyLoader::load(rp,
          key, aCallbackArgument))->value();
    char lstrvalue[32];
    sprintf(lstrvalue,"%i", loadValue);
    CacheableStringPtr lreturnValue = CacheableString::create(lstrvalue);
    if( key != NULLPTR && ( NULL !=  rp->getAttributes( )->getEndpoints( ) || rp->getAttributes( )->getPoolName( ) != NULL ) ) {
      LOGDEBUG("Putting the value (%s) for local region clients only ", lstrvalue);
...
void validateEventCount(int line) {
  ...
  ASSERT( reg1Listener1->getCreates() == numCreates, "Got wrong number of creation events." );
  ASSERT( reg1Listener1->getUpdates() == numUpdates, "Got wrong number of update events." );
  ASSERT( reg1Loader1  ->getLoads()   == numLoads, "Got wrong number of loader events." );
  ASSERT( reg1Writer1  ->getCreates() == numCreates, "Got wrong number of writer events." );
}
...
DUNIT_TASK_DEFINITION(CLIENT1, SetupClient_Pooled_Locator)
{
  initClient(true);
  reg1Listener1 = new TallyListener();
  createPooledRegion( regionNames[0], false, NULL, locatorsG, poolName, true, reg1Listener1);
  reg1Loader1   = new ThinClientTallyLoader();
  reg1Writer1   = new TallyWriter  ();
  setCacheLoader  (regionNames[0],reg1Loader1  );
  setCacheWriter  (regionNames[0],reg1Writer1  );
...
DUNIT_TASK_DEFINITION(CLIENT1, SetupClient_Pooled_EndPoint)
{
  initClient(true);
  reg1Listener1 = new TallyListener();
  createPooledRegion( regionNames[0], false, endPoint, NULL, poolName, true, reg1Listener1);
  reg1Loader1   = new ThinClientTallyLoader();
  reg1Writer1   = new TallyWriter  ();
  setCacheLoader  (regionNames[0],reg1Loader1  );
  setCacheWriter  (regionNames[0],reg1Writer1  );
...
DUNIT_TASK_DEFINITION(CLIENT1, testCreatesAndUpdates)
{
  ...
  /*NIL: Changed the asserion due to the change in invalidate.
    Now we create new entery for every invalidate event received or localInvalidate call
    so expect  containsKey to returns true insted of false earlier. 
    and used put call to update value for that key, instead of create call earlier or will get
...
void runListenerInit( bool poolConfig = true, bool isLocator = true)
{
  ...
  {
    CALL_TASK(SetupClient_Pooled_EndPoint);
  }
