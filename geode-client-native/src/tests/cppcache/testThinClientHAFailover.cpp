...
void initClientAndRegion( int redundancy, const char* endpointsList = endPoints)
{
  ...
  if ( g_poolConfig ) {
    ...
    if (g_poolLocators) {
      ...
      getHelper()->createPoolWithLocators("__TESTPOOL2_", locatorsG, true, redundancy);
    }
    else {
      getHelper()->createPoolWithEPs("__TESTPOOL1_", endpointsList, true, redundancy);
      getHelper()->createPoolWithEPs("__TESTPOOL2_", endpointsList, true, redundancy);
    }
    getHelper()->createRegionAndAttachPool(regionNames[0], USE_ACK, "__TESTPOOL1_", true);
    getHelper()->createRegionAndAttachPool(regionNames[1], USE_ACK, "__TESTPOOL2_", true);
  }
  else {
...
DUNIT_TASK_DEFINITION( SERVER1, SetLocator_S1 )
{
  g_poolConfig = true;
  g_poolLocators = true;
}
...
DUNIT_TASK_DEFINITION( SERVER2, SetLocator_S2 )
{
  g_poolConfig = true;
  g_poolLocators = true;
}
...
DUNIT_TASK_DEFINITION( SERVER1, ClearLocator_S1 )
{
  g_poolConfig = false;
...
DUNIT_TASK_DEFINITION( SERVER2, ClearLocator_S2 )
{
  g_poolConfig = false;
  g_poolLocators = false;
}
...
void initLocatorSettingsServers( bool useLocator )
{
  CALL_TASK( ClearLocator_S1 );
  CALL_TASK( ClearLocator_S2 );
  if ( useLocator ) {
    CALL_TASK( SetLocator_S1 );
    CALL_TASK( SetLocator_S2 );
  }
}

void doThinClientHAFailover( bool poolConfig = true, bool poolLocators = true )
{
  initLocatorSettings( poolConfig, poolLocators );
  initLocatorSettingsServers( poolConfig && poolLocators );
...
DUNIT_MAIN
{
  doThinClientHAFailover( false );
  doThinClientHAFailover( true, true );
  doThinClientHAFailover( true, false );
}
END_MAIN
