...
const char * endPoints = CacheHelper::getTcrEndpoints2(isLocalServer, 4);
...
std::vector<char *> storeEndPoints( const char * points )
{
 ...
 ASSERT( endpointNames.size() == 4, "There should be 4 end points" );
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOne_Pooled_Locator_Client1)
{
  ...
  getHelper()->createRegionAndAttachPool(regionNames[0], USE_ACK, "__TEST_POOL1__");
  getHelper()->createRegionAndAttachPool(regionNames[1], NO_ACK, "__TEST_POOL1__");  
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOne_Pooled_Endpoint_Client1)
{
  initClient(true);
  
  char endpoints[1024] = {0};
  sprintf( endpoints, "%s,%s,%s,%s", endpointNames.at(0),endpointNames.at(1),endpointNames.at(2),endpointNames.at(3));
  ...
  LOG( "StepOne_Pooled_Endpoint_Client1 complete." );
}
END_TASK_DEFINITION

DUNIT_TASK_DEFINITION(CLIENT2, StepOne_Pooled_Endpoint_Client2)
{
  initClient(true);
  ...
  getHelper()->createPoolWithEPs("__TEST_POOL1__", endpoints, true,2);
  getHelper()->createRegionAndAttachPool(regionNames[0], USE_ACK, "__TEST_POOL1__");
  getHelper()->createRegionAndAttachPool(regionNames[1], NO_ACK, "__TEST_POOL1__");
  ...
  LOG( "StepOne_Pooled_Endpoint_Client2 complete." );
}
END_TASK_DEFINITION
...
DUNIT_TASK_DEFINITION(CLIENT1, PutAllOneTask)
{
  ...
  for (int32_t item = 0; item < 1000; item++) {
   ...
   LOGDEBUG("CPPTEST:PutAllOneTask Doing PutAll on key using key: = %s: & value: = %s", key, value);
 }
  try {
...
DUNIT_TASK_DEFINITION(CLIENT1, PutAllTwoTask)
{
  ...
  for (int32_t item = 1000; item < 2000; item++) {
   sprintf(key, "key-%d", item);
   sprintf(value, "%d", item);
   LOGDEBUG("CPPTEST:PutAllTwoTask Doing PutAll on key using key: = %s: & value: = %s", key, value);
   entryMap.insert(CacheableKey::create(key), CacheableString::create(value));
 }
  try {
    dataReg->putAll(entryMap);
...
DUNIT_TASK_DEFINITION(CLIENT1, PutAllThreeTask)
{
  ...
  for (int32_t item = 2000; item < 3000; item++) {
   sprintf(key, "key-%d", item);
   sprintf(value, "%d", item);
   LOGDEBUG("CPPTEST:PutAllThreeTask Doing PutAll on key using key: = %s: & value: = %s", key, value);
   entryMap.insert(CacheableKey::create(key), CacheableString::create(value));
 }
  try {
    dataReg->putAll(entryMap);
...
DUNIT_TASK_DEFINITION(CLIENT1, PutAllFourTask)
{
  ...
  for (int32_t item = 3000; item < 4000; item++) {
   sprintf(key, "key-%d", item);
   sprintf(value, "%d", item);
   LOGDEBUG("CPPTEST:PutAllFourTask Doing PutAll on key using key: = %s: & value: = %s", key, value);
   entryMap.insert(CacheableKey::create(key), CacheableString::create(value));
 }
  try {
    dataReg->putAll(entryMap);
...
DUNIT_TASK_DEFINITION(CLIENT2, VerifyAllPutAllTask)
{
  ...
  ASSERT( reg1Listener1->getCreates() == 4000, "Got wrong number of creation events." );
  ...
  for (int32_t item = 0; item < 4000; item++) {
   sprintf(key, "key-%d", item);
   sprintf(value, "%d", item);
    LOGDEBUG("CPPTEST:VerifyAllPutAllTask Doing get on key using: = %s: ", key);
   CacheableStringPtr checkPtr = dynCast<CacheableStringPtr>( dataReg->get( CacheableKey::create(key) ) );
   ASSERT(checkPtr != NULLPTR, "Value Ptr should not be null.");
   LOGDEBUG("CPPTEST:VerifyAllPutAllTask value is: = %s: ", checkPtr->asChar());     
...
DUNIT_TASK_DEFINITION(CLIENT1, RemoveAllOneTask)
{
  ...
  for (int32_t item = 0; item < 1000; item++) {
   ...
   LOGDEBUG("CPPTEST:RemoveAllOneTask Doing PutAll on key using key: = %s: & value: = %s", key, value);
 }
  try {
    dataReg->putAll(entryMap);
...
DUNIT_TASK_DEFINITION(CLIENT1, RemoveAllTwoTask)
{
  ...
  for (int32_t item = 1000; item < 2000; item++) {
   ...
   entryMap.insert(CacheableKey::create(key), CacheableString::create(value));
   keys.push_back(CacheableKey::create(key));
   LOGDEBUG("CPPTEST:RemoveAllTwoTask Doing RemoveAll on key using key: = %s: & value: = %s", key, value);
 }
  try {
    dataReg->putAll(entryMap);
    dataReg->removeAll(keys);
...
DUNIT_TASK_DEFINITION(CLIENT1, RemoveAllThreeTask)
{
  ...
  for (int32_t item = 2000; item < 3000; item++) {
   ...
   entryMap.insert(CacheableKey::create(key), CacheableString::create(value));
   keys.push_back(CacheableKey::create(key));
   LOGDEBUG("CPPTEST:RemoveAllThreeTask Doing RemoveAll on key using key: = %s: & value: = %s", key, value);
 }
  try {
    dataReg->putAll(entryMap);
    dataReg->removeAll(keys);
   ...
   LOGERROR("CPPTEST: RemoveAll caused random exception in RemoveAllThreeTask");
   cleanProc();
   FAIL( "RemoveAll caused unexpected exception" );
   throw IllegalStateException("TEST FAIL");
  }
...
DUNIT_TASK_DEFINITION(CLIENT1, RemoveAllFourTask)
{
  ...
  for (int32_t item = 3000; item < 4000; item++) {
   ...
   entryMap.insert(CacheableKey::create(key), CacheableString::create(value));
   keys.push_back(CacheableKey::create(key));
   LOGDEBUG("CPPTEST:RemoveAllFourTask Doing RemoveAll on key using key: = %s: & value: = %s", key, value);
 }
  try {
    dataReg->putAll(entryMap);
    dataReg->removeAll(keys);
   ...
   LOGERROR("CPPTEST: RemoveAll caused random exception in RemoveAllFourTask");
   cleanProc();
   FAIL( "RemoveAll caused unexpected exception" );
   throw IllegalStateException("TEST FAIL");
  }
...
DUNIT_TASK_DEFINITION(CLIENT2, VerifyAllRemoveAllTask)
{
  ...
  ASSERT( reg1Listener1->getDestroys() == 4000, "Got wrong number of destroy events." );
...
DUNIT_MAIN
{
  ...
  CALL_TASK(StepOne_Pooled_Endpoint_Client1);
  CALL_TASK(StepOne_Pooled_Endpoint_Client2);
