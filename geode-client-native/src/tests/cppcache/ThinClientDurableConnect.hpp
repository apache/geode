...
/* Test case covered:
With Four Servers
1-  ( R = 1 ) All Server Up, Check if client reconnect to either of server having queue.
2-  (  R = 1 ) One Redundant Server down, Check if client reconnect to another server having queue.
3-  (  R = 1 ) Both Redundant Server down, Check if client reconnect to both non redundant server and all events are lost.
4-  (  R = 1 ) Both Non-Redundant Server down, Check if client reconnect to both redundant server.
5-  ( R = 1 ) All Server Up, Check if client reconnect to either of server after timeout period and all the events are lost.
*/
...
{
  if ( isLocalServer ) {
    CacheHelper::initServer( 1 , "cacheserver_notify_subscription.xml", g_Locators );
  }
...
{
  if ( isLocalServer )
    CacheHelper::initServer( 2 , "cacheserver_notify_subscription2.xml", g_Locators );
...
{
  if ( isLocalServer )
    CacheHelper::initServer( 3 , "cacheserver_notify_subscription3.xml", g_Locators );
...
{
  if ( isLocalServer )
    CacheHelper::initServer( 4 , "cacheserver_notify_subscription4.xml", g_Locators );
...
{
  initClientAndRegion( 1, 0, endPointsList);
...
{
  initClientAndRegion( 0, 0, endPointsList);
  getHelper()->cachePtr->readyForEvents();
...
void doThinClientDurableConnect( bool poolConfig = true, bool poolLocators = true )
{
