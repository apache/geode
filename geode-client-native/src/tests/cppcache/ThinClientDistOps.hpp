...
void createAndVerifyEntry( const char * name)
{
  ...
  CacheableInt64Ptr longRetValue = dynCast<CacheableInt64Ptr>(regPtr->get((int64_t)int64Key));
  ASSERT(in64Value == longRetValue->value(), "longRetValue and longvalue should match");
  ...
  ASSERT(strcmp(strRetValue->asChar(), "testvalue")==0, "strRetValue and 'testvalue' should match");

  /*3.create new with entry nullptr key and nullptr value. IllegalArgumentException thrown*/
  CacheableInt32Ptr x;
  try{
    regPtr->create(x, 1);
  ...
  /*5.create new with entry userobject cantain all cacheable type ( like cacheableInt,CacheableDouble, CacheableString,CacheableHashMap etc) key and null value*/
  ...
  /*6.create new with entry userobject cantain all cacheable type ( like cacheableInt,CacheableDouble, CacheableString,CacheableHashMap etc) key and int value*/
  ...
  /*7.create new with entry userobject cantain all cacheable type ( like cacheableInt,CacheableDouble, CacheableString,CacheableHashMap etc) key and string value*/
  ...
  ASSERT(strcmp(strVal->toString()->asChar(),"testString") ==0,"strVal should be testString.");

  /*8.create new with entry userobject cantain all cacheable type ( like cacheableInt,CacheableDouble, CacheableString,CacheableHashMap etc) key and userobject
   * cantain all cacheable type ( like cacheableInt,CacheableDouble,      CacheableString,CacheableHashMap etc)  value*/
  ...
  PdxTests::PdxTypePtr objVal = dynCast<PdxTests::PdxTypePtr>(regPtr->get(keyObject4));
  ASSERT(valObject == objVal, "valObject and objVal should match.");



  /*9.create new entry witn non serialize object. IllegalArgumentException thrown*/
...
const char * regionNames[] = { "DistRegionAck", "DistRegionNoAck", "testregion", "CreateVerifyTestRegion" };
...
DUNIT_TASK_DEFINITION(CLIENT1, CreatePoolForUpdateLocatorList)
{
	...
	PoolPtr  createPool(const char* poolName, const char* locators, const char* serverGroup,
                      ...
                      int connections = -1, int loadConditioningInterval = - 1, bool isMultiuserMode = false, int updateLocatorListInterval = 5000 )
	*/
  initClient( true ); 
  getHelper()->createPool( "__TESTPOOL1_", locatorsG, NULL, NULL, 0, false, -1, -1, -1, false);
...
DUNIT_TASK_DEFINITION(CLIENT1, VerifyUpdateLocatorListThread)
{
  ...
  int numLocatorListUpdates = CacheHelper::getNumLocatorListUpdates("Querying locator list at:");
  ...
  {
	  ASSERT(numLocatorListUpdates >= numExpectedLocatorListUpdates-1 && numLocatorListUpdates <= numExpectedLocatorListUpdates+1, "Got unexpected number of LocatorList updates");
  }
  else
  {
	  ASSERT(numLocatorListUpdates == 0, "Got unexpected LocatorList updates with LocatorUpdateInterval set to 0");
  }
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOne)
{
  ...
  createRegion( regionNames[3], NO_ACK, endPoints);
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOne_Pooled_Locator)
{
  ...
  createPooledRegion( regionNames[3], NO_ACK, NULL, locatorsG, "__TESTPOOL1_" );
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOne_Pooled_Locator_Sticky)
{
  ...
  createPooledRegionSticky( regionNames[3], NO_ACK, NULL, locatorsG, "__TESTPOOL1_" );
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOne_Pooled_EndPoint)
{
  ...
  createPooledRegion( regionNames[3], NO_ACK, endPoints, NULL, "__TESTPOOL1_" );
  LOG( "StepOne_Pooled complete." );
}
END_TASK_DEFINITION
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOne_Pooled_EndPoint_Sticky)
{
  ...
  createPooledRegionSticky( regionNames[3], NO_ACK, endPoints, NULL, "__TESTPOOL1_" );
  LOG( "StepOne_Pooled complete." );
}
END_TASK_DEFINITION
...
void runDistOps( bool poolConfig = true, bool isLocator = true, bool isSticky = false) {
...
void runDistOps2( bool isLocatorUpdateFrequencyNonZero = true) {
	CALL_TASK(CreateLocator1);
    CALL_TASK(CreateServer1_With_Locator)

	if (isLocatorUpdateFrequencyNonZero)
	{
