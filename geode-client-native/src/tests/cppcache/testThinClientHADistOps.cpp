...
void createPooledRegion( const char * name, bool ackMode, const char * endpoints, const char* locators,const char* poolname, int reduendency = 1, bool clientNotificationEnabled = false, bool cachingEnable = true)
{
  LOG( "createRegion_Pool() entered." );
  fprintf( stdout, "Creating region --  %s  ackMode is %d\n", name, ackMode );
  fflush( stdout );
  PoolPtr poolPtr = getHelper()->createPool(poolname, locators, NULL, endpoints, reduendency, clientNotificationEnabled);
  RegionPtr regPtr = getHelper()->createRegionAndAttachPool(name, ackMode, poolname, cachingEnable);
  ASSERT( regPtr != NULLPTR, "Failed to create region." );
...
DUNIT_TASK_DEFINITION(CLIENT1, InitClient1_Xml)
{
  initClient("cache_redundancy.xml");
}
END_TASK_DEFINITION

DUNIT_TASK_DEFINITION(CLIENT2, InitClient2_Xml)
{
  initClient("cache_redundancy.xml");
}
END_TASK_DEFINITION
...
{
  initClient("client_pool.xml");
}
END_TASK_DEFINITION

DUNIT_TASK_DEFINITION(CLIENT2, InitClient2_Pool_XML)
{
  initClient("client_pool.xml");
}
END_TASK_DEFINITION
...
{
  getHelper()->createPoolWithEPs("__TESTPOOL1_", endPoints, true, 1);

  getHelper()->createRegionAndAttachPool(regionNames[0], USE_ACK, "__TESTPOOL1_", true);
  getHelper()->createRegionAndAttachPool(regionNames[1], NO_ACK, "__TESTPOOL1_", true);
  /*createPooledRegion( regionNames[0], USE_ACK, endPoints, NULL, "__TESTPOOL3_" );
  createPooledRegion( regionNames[1], NO_ACK, endPoints, NULL, "__TESTPOOL4_" );*/
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOne_Pool_XML)
{
  createEntry( regionNames[0], keys[0], vals[0] );
  createEntry( regionNames[1], keys[2], vals[2] );
...
DUNIT_TASK_DEFINITION(CLIENT2, StepTwo_Pool_XML)
{
  createEntry( regionNames[0], keys[1], vals[1] );
  createEntry( regionNames[1], keys[3], vals[3] );

  LOG( "StepTwo_Pool_XML complete." );
}
END_TASK_DEFINITION
...
{
  // Test cases:
  // 3: Pools (programmatic), Locators
  for(int j=0; j<2; j++ ){//with and without gridclient
    // i == 3 means running with endPoint i == 4 means running with locator.
    for (int i=0; i<6; i++) {
  ...
  /* Starting Server*/
      ...
      else {// With Locator i ==5 locator and xml with i == 4 means endPoints and xml
  ...
  /* Client init*/
      ...
      if(i == 1) {
        CALL_TASK( InitClient1_Xml);
        CALL_TASK( InitClient2_Xml);
      }
  
      if(2 == i || 3 == i){ //with Pool
        CALL_TASK( InitClient1_Pool );
        CALL_TASK( InitClient2_Pool );
      }
      if( 4 == i || 5 == i) //with pool with xml
      {
        CALL_TASK(InitClient1_Pool_XML);
        CALL_TASK(InitClient2_Pool_XML);
      }
   /* Client init*/
      if( i == 0 || i == 1){
        CALL_TASK(StepOne);
      ...
      {
        CALL_TASK(StepOne_Pool_XML);
        CALL_TASK(StepTwo_Pool_XML)
      }
