...
DUNIT_TASK_DEFINITION(CLIENT1, StepOne_Pooled_Endpoint)
{
  ...
  getHelper()->createRegionAndAttachPool(regionNames[0], USE_ACK, "__TEST_POOL1__",true /*false:: caching disabled originally*/);
  getHelper()->createRegionAndAttachPool(regionNames[1], NO_ACK, "__TEST_POOL1__",true);
...
DUNIT_TASK_DEFINITION(CLIENT1, WarmUpTask)
{
  ...
  ASSERT( metadatarefreshCount < 100, "metadatarefreshCount should be less than 100" );
...

  {
	...
	try {
      ...
      for(int j = 1000; j < 25000; j++){
    	CacheableKeyPtr keyPtr = dynCast<CacheableKeyPtr> (CacheableInt32::create(j));
    	CacheablePtr valPtr = dynCast<CacheablePtr>(CacheableInt32::create(keyPtr->hashcode()));
    	LOGINFO("CPPTEST: putALL CASE:: getting key %d with hashcode %d", j, (int32_t)keyPtr->hashcode());
      ...
      LOGINFO("Time taken to execute putAll SH sec = %d and MSec = %d ", interval.sec(), interval.usec());
      bool networkhop = TestUtils::getCacheImpl(getHelper( )->cachePtr)->getAndResetNetworkHopFlag();
      LOGINFO("CheckPrSingleHopForIntKeysTask2: putALL OP :: networkhop %d ", networkhop);
      ...
      LOGERROR("CPPTEST: putAll caused unexpected %s: %s", ex.getName(),ex.getMessage());
      cleanProc();
...
DUNIT_TASK_DEFINITION(CLIENT1, CheckPrSingleHopRemoveAllForIntKeysTask)
{
...

  {
    LOGINFO("Iteration removeAll Start ");
      ...
      for(int j = 1000; j < 25000; j++){
        CacheableKeyPtr keyPtr = dynCast<CacheableKeyPtr> (CacheableInt32::create(j));
        CacheablePtr valPtr = dynCast<CacheablePtr>(CacheableInt32::create(keyPtr->hashcode()));
        LOGINFO("CPPTEST: removeall CASE:: getting key %d with hashcode %d", j, (int32_t)keyPtr->hashcode());
        valMap.insert(keyPtr, valPtr);
      ...
      LOGINFO("Time taken to execute removeAll SH sec = %d and MSec = %d ", interval.sec(), interval.usec());
      networkhop = TestUtils::getCacheImpl(getHelper( )->cachePtr)->getAndResetNetworkHopFlag();
      LOGINFO("CheckPrSingleHopForIntKeysTask2: removeall OP :: networkhop %d ", networkhop);
      ...
      LOGERROR("CPPTEST: removeall caused unexpected %s: %s", ex.getName(),ex.getMessage());
      cleanProc();
      FAIL( "putAll caused unexpected exception" );
  ...
  int poolconn = TestUtils::getCacheImpl(getHelper( )->cachePtr)->getPoolSize("__TEST_POOL1__");
  LOGINFO("CheckPrSingleHopRemoveAllForIntKeysTask: poolconn is %d ", poolconn);
