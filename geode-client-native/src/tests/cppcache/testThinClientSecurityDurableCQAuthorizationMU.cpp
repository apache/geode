...
class MyCqListener : public CqListener {
  ...
  inline void updateCount(const CqEvent& cqEvent)
  {
      ...
      switch (cqEvent.getQueryOperation())
      {
       ...
       printf("hitesh in create = %d, update = %d , delete = %d ", m_numInserts, m_numUpdates, m_numDeletes);
  }
...
DUNIT_TASK_DEFINITION(CLIENT1, CreateServer1)
{
  ...
  if ( isLocalServer ) {
    cmdServerAuthenticator = credentialGeneratorHandler->getServerCmdParams("authenticator:authorizerPP", getXmlPath());
    printf("string %s",cmdServerAuthenticator.c_str());
    CacheHelper::initServer( 1,"remotequery.xml",NULL , (char*)cmdServerAuthenticator.c_str());
    LOG("Server1 started");
  }
}
END_TASK_DEFINITION
...
DUNIT_TASK_DEFINITION(CLIENT1, CreateServer2)
{
  ...
  if ( isLocalServer ) {
    cmdServerAuthenticator = credentialGeneratorHandler->getServerCmdParams("authenticator:authorizerPP", getXmlPath());
    printf("string %s",cmdServerAuthenticator.c_str());
    CacheHelper::initServer( 2,"remotequery2.xml",NULL , (char*)cmdServerAuthenticator.c_str());
    LOG("Server2 started");
  }
  SLEEP(20000);
}
...
void stepOne(bool pool = false, bool locator = false)
{
  ...
  createRegionForCQMU( pool, locator, regionNamesCq[0], USE_ACK, endPoints, true, 0, NULLPTR, false, true);
 ...
 // userCache = getVirtualCache(userCreds, regionNamesCq[0]);
  ...
  try
  {
    for(i=0; i < MAX_LISTNER; i++)
    {
      ...
      CqQueryPtr qry = qs->newCq(cqNames[i], queryStrings[i], cqAttr, durableCq);
      qry->execute();
    }
...
DUNIT_TASK_DEFINITION(CLIENT1, StepFour)
{
  ...
  printf("hitesh cqListener_3 should have one create event = %d \n", cqListener_3->getNumInserts());
  ASSERT(cqListener_3->getNumInserts() == 1, "incorrect number of events got listener 3");
  ...
  printf("hitesh cqListener_4 should have one create event = %d \n", cqListener_4->getNumInserts());
  ASSERT(cqListener_4->getNumInserts() == 1, "incorrect number of events got listener 4");
...
SLEEP(15000); // sleep .25 min
  ...
  if ((durableCq && !logicalCacheKeepAlive && !closeLogicalCache) || (durableCq && logicalCacheKeepAlive && closeLogicalCache))
  {
    //if key port1-4 then only query 3 and 4 will satisfied
    CqQueryPtr cqy = qs->getCq(cqNames[3]);
    CqAttributesPtr cqAttr = cqy->getCqAttributes();
    ...
    MyCqListener* cqListener_3 = static_cast<MyCqListener*>(vl[0].ptr());
    printf("hitesh cqListener_3 should have one update event = %d \n", cqListener_3->getNumUpdates());
    ASSERT(cqListener_3->getNumUpdates() == 1, "incorrect number of events got listener 3");
    ...
    MyCqListener* cqListener_4 = static_cast<MyCqListener*>(vl[0].ptr());
    printf("hitesh cqListener_4 should have one update event = %d \n", cqListener_4->getNumUpdates());
    ASSERT(cqListener_4->getNumUpdates() == 1, "incorrect number of events got listener 4");
  }
  ...
  if((!durableCq && !logicalCacheKeepAlive && !closeLogicalCache) ||  (durableCq && !logicalCacheKeepAlive && closeLogicalCache))
  {
    ...
    MyCqListener* cqListener_3 = static_cast<MyCqListener*>(vl[0].ptr());
    printf("hitesh cqListener_3 should have zero update event = %d \n", cqListener_3->getNumUpdates());
    ASSERT(cqListener_3->getNumUpdates() == 0, "incorrect number of events got listener 3");
    ...
    MyCqListener* cqListener_4 = static_cast<MyCqListener*>(vl[0].ptr());
    printf("hitesh cqListener_4 should have zero update event = %d \n", cqListener_4->getNumUpdates());
    ASSERT(cqListener_4->getNumUpdates() == 0, "incorrect number of events got listener 4");
  }
