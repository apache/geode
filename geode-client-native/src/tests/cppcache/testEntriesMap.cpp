...
BEGIN_TEST(RemoveTest)
{
  ...
  ASSERT( resPtr->operator==( *createCacheable("200")), "CustomerType with m_foobar 200." );
  (void)entries->remove(keyPtr, myValuePtr, me, -1, versionTag, false);
...
BEGIN_TEST(GetEntryTest)
{
  ...
  ASSERT( valPtr->operator==( *cst ), "Entry should have a CustomerType Value of 200" );
...
BEGIN_TEST(ValuesTest)
{
  ...
  for( int k = 0; k < 10; k++ ) {
    expectedTotal += k;
    CacheableStringPtr valuePtr = dynCast<CacheableStringPtr>( valuesVec->back() );
...
{
  EntryFactory* entryFactory = EntryFactory::singleton;
  ConcurrentEntriesMap* entries = new ConcurrentEntriesMap(entryFactory, false,
                                                           NULL, 1);
  entries->open( 10 );
  ASSERT( entries->totalSegmentRehashes() == 0, "should not have rehashed yet." );
  char keyBuf[100];
  char valBuf[100];
  MapEntryImplPtr me;
  ...
  // check rehash count...
  ASSERT( entries->totalSegmentRehashes() > 0, "should have rehashed several times." );
...
BEGIN_TEST(LRURemoveTest)
{
  ...
  ASSERT( myValuePtr->operator==( *createCacheable("200")), "CustomerType with m_foobar 200." );
...
BEGIN_TEST(LRUGetEntryTest)
{
  ...
  ASSERT( ctPtr->operator==( *cst ), "Entry should have a CustomerType Value of 200" );
  CacheableKeyPtr keyPtr1;
  mePtr->getKey( keyPtr1 );
  ASSERT( keyPtr1->operator==( *keyPtr ), "should have same key." );
}
