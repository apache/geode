...
void initClient( const bool isthinClient, bool  isPdxReadSerailized)
{
  ...
  if ( cacheHelper == NULL ) {
    ...
    cacheHelper = new CacheHelper(isthinClient, false, isPdxReadSerailized, config, false );
  } 
...
{
  if ( isLocalServer ) CacheHelper::initServer(1,"cacheserver_pdxinstance_hashcode.xml");
  LOG("SERVER1 started");
}
END_TASK_DEFINITION
...
{
  if ( isLocalServer )
    CacheHelper::initServer( 1, "cacheserver_pdxinstance_hashcode.xml", locatorsG );
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOne_Pooled_Locator_PdxReadSerialized)
{
  ...
  createPooledRegion( regionNames[0], USE_ACK, NULL, locatorsG, "__TEST_POOL1__", false, false);  
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOne_Pooled_EndPoint_PdxReadSerialized)
{
  initClient(true, true);  
  createPooledRegion( regionNames[0], USE_ACK, endPoints, NULL, "__TEST_POOL1__", false, false);  
  LOG( "StepOne_Pooled_EndPoint complete." );
}
END_TASK_DEFINITION

DUNIT_TASK_DEFINITION(CLIENT2, StepTwo_Pooled_EndPoint_PdxReadSerialized)
{
  initClient(true, true);  
  createPooledRegion( regionNames[0], USE_ACK, endPoints, NULL, "__TEST_POOL1__", false, false);  
  LOG( "StepOne_Pooled_EndPoint complete." );
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOne_Pooled_EndPoint_PdxReadSerialized_XML)
{
  initClient("client_pdx.xml");   
  LOG( "StepOne_Pooled_EndPoint_PdxReadSerialized_XML complete." );
}
END_TASK_DEFINITION

DUNIT_TASK_DEFINITION(CLIENT2, StepTwo_Pooled_EndPoint_PdxReadSerialized_XML)
{
  initClient("client_pdx.xml");   
  LOG( "StepTwo_Pooled_EndPoint_PdxReadSerialized_XML complete." ); 
}
END_TASK_DEFINITION


DUNIT_TASK_DEFINITION(CLIENT2, StepTwo_Pooled_Locator_CachingEnabled_PdxReadSerialized)
{
  initClient(true, true);  
  createPooledRegion( regionNames[0], USE_ACK, NULL, locatorsG, "__TEST_POOL1__", false, true);  
  LOG( "StepTwo_Pooled_Locator complete." );
}
END_TASK_DEFINITION

DUNIT_TASK_DEFINITION(CLIENT2, StepTwo_Pooled_EndPoint_CachingEnabled_PdxReadSerialized)
{
  initClient(true, true);
  createPooledRegion( regionNames[0], USE_ACK, endPoints, NULL, "__TEST_POOL1__", false, true);  
...
{    
  ASSERT(cacheHelper->getCache()->getPdxReadSerialized() == true, "Pdx read serialized property should be true.");
...
{ 
  ...
  PdxInstancePtr pi = dynCast<PdxInstancePtr>(rptr->get(CacheableInt32::create(1)));
  ...
  LOGINFO("pdxInstanceDeserializations for (PdxTests.PdxType) PdxInstance  = %d ", lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceDeserializations());
  LOGINFO("pdxInstanceCreations for (PdxTests.PdxType) PdxInstance  = %d ", lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceCreations());
  LOGINFO("pdxInstanceDeserializationTime for(PdxTests.PdxType) PdxInstance  = %d ", lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceDeserializationTime());

  ASSERT(lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceDeserializations() == 0, "pdxInstanceDeserialization should be equal to 0.");
  ASSERT(lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceCreations() == 1, "pdxInstanceCreations should be equal to 1.");
  ASSERT(lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceDeserializationTime() > 0, "pdxInstanceDeserializationTime should be greater than equal to 0.");

  ASSERT(pi->getFieldNames()->length() == 4, "number of fields should be four in SerializePdx");

  ASSERT(pi->isIdentityField("i1") == true, "SerializePdx1.i1 should be identity field");

  ASSERT(pi->isIdentityField("i2") == false, "SerializePdx1.i2 should NOT be identity field");

  ASSERT(pi->hasField("i1") == true, "SerializePdx1.i1 should be in PdxInstance stream");

  ASSERT(pi->hasField("i3") == false, "There is no field i3 in SerializePdx1's PdxInstance stream");
  ...
  ASSERT(javaPdxHC == pdxInstHashcode, "Pdxhashcode for identity field object SerializePdx1 not matched with java pdx hash code.");

  PdxInstancePtr pi2 = dynCast<PdxInstancePtr>(rptr->get(CacheableInt32::create(1)));
  ...
  ASSERT(lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceDeserializations() == 0, "pdxInstanceDeserialization should be equal to 0.");
  ASSERT(lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceCreations() == 2, "pdxInstanceCreations should be equal to 2.");
  ASSERT(lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceDeserializationTime() > 0, "pdxInstanceDeserializationTime should be greater than 0.");
...
{ 
  ...
  CacheableObjectArrayPtr objectArrayPtr = rptr->get(CacheableInt32::create(100));
  ...
  for( int i=0; i<objectArrayPtr->size(); i++ ) {
    ...
    PdxInstanceFactoryPtr pifPtr = cacheHelper->getCache()->createPdxInstanceFactory("PdxTests.Address");
    ...
    ASSERT(addrPtr.ptr()->equals(*(newAddrPtr.ptr()))== true , "Both PdxInstances should be equal.");
...
{ 
  ...
  PdxInstancePtr pi = dynCast<PdxInstancePtr>(rptr->get(CacheableInt32::create(2)));
  LOG("PdxInstancePtr get complete");
  ...
  PdxInstancePtr pi2 = dynCast<PdxInstancePtr>(rptr->get(CacheableInt32::create(2)));
...
{ 
  ...
  ASSERT(javaPdxHC == pdxInstHashcode, "Pdxhashcode hashcode not matched with java pdx hash code.");
...
DUNIT_TASK_DEFINITION(CLIENT2, getObject)
{
  ...
  ASSERT(strcmp(pIPtr->getClassName(), "PdxTests.PdxType") == 0, "pdxInstance.getClassName should return PdxTests.PdxType.");
  ...
  ASSERT(lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceDeserializations() == 1, "pdxInstanceDeserialization should be equal to 1.");
  ASSERT(lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceCreations() > 1, "pdxInstanceCreations should be greater than 1.");
  ASSERT(lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceDeserializationTime() > 0, "pdxInstanceDeserializationTime should be greater than 0.");
  ...
  ASSERT(obj1->equals(*obj2, true) == true , "PdxInstance.getObject not equals original object.");
  ...
  ASSERT(lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceDeserializations() == 2, "pdxInstanceDeserialization should be equal to 2.");
  ASSERT(lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceCreations() > 1, "pdxInstanceCreations should be greater than 1.");
  ASSERT(lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceDeserializationTime() > 0, "pdxInstanceDeserializationTime should be greater than 0.");
  ...
  ASSERT(parentObj1->equals(*parentObj2, true) == true , "PdxInstance.getObject not equals original object.");
...
DUNIT_TASK_DEFINITION(CLIENT2, verifyPdxInstanceHashcode)
{
  ...
  ASSERT(javaPdxHC == pdxInstHashcode, "Pdxhashcode hashcode not matched with java padx hash code.");
...
DUNIT_TASK_DEFINITION(CLIENT2, accessPdxInstance)
{
  ...
  ASSERT(pIPtr->getFieldType("m_bool") == PdxFieldTypes::BOOLEAN, "Type Value BOOLEAN Mismatch");

  wchar_t charVal = ' ';
  pIPtr->getField("m_char", charVal);
  ASSERT(pdxobjPtr->getChar() == charVal , "char values should be equal");
  ASSERT(pIPtr->getFieldType("m_char") == PdxFieldTypes::CHAR, "Type Value CHAR Mismatch");

  signed char byteVal = 0;
  pIPtr->getField("m_byte", byteVal);
  ASSERT(pdxobjPtr->getByte() == byteVal , "byte values should be equal");
  ASSERT(pIPtr->getFieldType("m_byte") == PdxFieldTypes::BYTE, "Type Value BYTE Mismatch");

  pIPtr->getField("m_sbyte", byteVal);
  ASSERT(pdxobjPtr->getSByte() == byteVal , "Sbyte values should be equal");
  ASSERT(pIPtr->getFieldType("m_sbyte") == PdxFieldTypes::BYTE, "Type Value BYTE Mismatch");
  
  int16_t shortVal = 0;
  pIPtr->getField("m_int16", shortVal);
  ASSERT(pdxobjPtr->getShort() == shortVal , "shortVal should be equal");
  ASSERT(pIPtr->getFieldType("m_int16") == PdxFieldTypes::SHORT, "Type Value SHORT Mismatch");
  
  pIPtr->getField("m_uint16", shortVal);
  ASSERT(pdxobjPtr->getUint16() == shortVal , "m_uint16 should be equal");
  ASSERT(pIPtr->getFieldType("m_uint16") == PdxFieldTypes::SHORT, "Type Value SHORT Mismatch");

  int val = 0;
  pIPtr->getField("m_int32", val);
  ASSERT(pdxobjPtr->getInt() == val , "int32 values should be equal");
  ASSERT(pIPtr->getFieldType("m_int32") == PdxFieldTypes::INT, "Type Value INT Mismatch");

  pIPtr->getField("m_uint32", val);
  ASSERT(pdxobjPtr->getUInt() == val , "m_uint32 values should be equal");
  ASSERT(pIPtr->getFieldType("m_uint32") == PdxFieldTypes::INT, "Type Value INT Mismatch");

  int64_t longVal = 0;
  pIPtr->getField("m_long", longVal);
  ASSERT(pdxobjPtr->getLong() == longVal , "int64 values should be equal");
  ASSERT(pIPtr->getFieldType("m_long") == PdxFieldTypes::LONG, "Type Value LONG Mismatch");

  pIPtr->getField("m_ulong", longVal);
  ASSERT(pdxobjPtr->getULong() == longVal , "m_ulong values should be equal");
  ASSERT(pIPtr->getFieldType("m_ulong") == PdxFieldTypes::LONG, "Type Value LONG Mismatch");

  float floatVal = 0.0f;
  pIPtr->getField("m_float", floatVal);
  ASSERT(pdxobjPtr->getFloat() == floatVal , "floatVal should be equal");
  ASSERT(pIPtr->getFieldType("m_float") == PdxFieldTypes::FLOAT, "Type Value FLOAT Mismatch");

  double doubleVal = 0.0;
  pIPtr->getField("m_double", doubleVal);
  ASSERT(pdxobjPtr->getDouble() == doubleVal , "doubleVal should be equal");
  ASSERT(pIPtr->getFieldType("m_double") == PdxFieldTypes::DOUBLE, "Type Value DOUBLE Mismatch");

  char* stringVal = NULL;
  pIPtr->getField("m_string", &stringVal);  
  ASSERT(strcmp(stringVal, pdxobjPtr->getString()) == 0 , "stringVal should be equal");
  ASSERT(pIPtr->getFieldType("m_string") == PdxFieldTypes::STRING, "Type Value STRING Mismatch");

  char** stringArrayVal = NULL;
  int32_t stringArrayLen = 0;
  pIPtr->getField("m_stringArray", &stringArrayVal, stringArrayLen);
  ASSERT(genericValCompare(pdxobjPtr->getStringArrayLength(), stringArrayLen) == true , "stringArrayLen should be equal");
  ASSERT(pIPtr->getFieldType("m_stringArray") == PdxFieldTypes::STRING_ARRAY, "Type Value STRING_ARRAY Mismatch");
  char** strArray = pdxobjPtr->getStringArray();
  for(int i=0; i<stringArrayLen; i++) {
    ASSERT(strcmp(strArray[i], stringArrayVal[i]) == 0, "All stringVals should be equal");    
  }  
  
  signed char* byteArray = NULL;
  int32_t byteArrayLength = 0; 
  pIPtr->getField("m_byteArray", &byteArray, byteArrayLength);
  ASSERT(genericValCompare(pdxobjPtr->getByteArrayLength(), byteArrayLength) == true , "byteArrayLength should be equal");
  ASSERT(genericCompare(pdxobjPtr->getByteArray(), byteArray, byteArrayLength) == true , "byteArray should be equal");
  ASSERT(pIPtr->getFieldType("m_byteArray") == PdxFieldTypes::BYTE_ARRAY, "Type Value BYTE_ARRAY Mismatch");

  wchar_t* charArray = NULL;
  int32_t charArrayLength = 0; 
  pIPtr->getField("m_charArray", &charArray, charArrayLength);  
  ASSERT(genericValCompare(pdxobjPtr->getCharArrayLength(), charArrayLength) == true , "charArrayLength should be equal");   
  ASSERT(genericCompare(pdxobjPtr->getCharArray(), charArray, charArrayLength) == true , "charArray should be equal");
  ASSERT(pIPtr->getFieldType("m_charArray") == PdxFieldTypes::CHAR_ARRAY, "Type Value CHAR_ARRAY Mismatch");

  pIPtr->getField("m_sbyteArray", &byteArray, byteArrayLength);
  ASSERT(genericValCompare(pdxobjPtr->getByteArrayLength(), byteArrayLength) == true , "sbyteArrayLength should be equal");
  ASSERT(genericCompare(pdxobjPtr->getSByteArray(), byteArray, byteArrayLength) == true , "m_sbyteArray should be equal");
  ASSERT(pIPtr->getFieldType("m_sbyteArray") == PdxFieldTypes::BYTE_ARRAY, "Type Value BYTE_ARRAY Mismatch");

  bool* boolArray = NULL;
  int32_t boolArrayLength = 0; 
  pIPtr->getField("m_boolArray", &boolArray, boolArrayLength);
  ASSERT(genericValCompare(pdxobjPtr->getBoolArrayLength(), boolArrayLength) == true , "boolArrayLength should be equal");
  ASSERT(genericCompare(pdxobjPtr->getBoolArray(), boolArray, boolArrayLength) == true , "boolArray should be equal");
  ASSERT(pIPtr->getFieldType("m_boolArray") == PdxFieldTypes::BOOLEAN_ARRAY, "Type Value BOOLEAN_ARRAY Mismatch");

  int16_t* shortArray = NULL;
  int32_t shortArrayLength = 0; 
  pIPtr->getField("m_int16Array", &shortArray, shortArrayLength);
  ASSERT(genericValCompare(pdxobjPtr->getShortArrayLength(), shortArrayLength) == true , "shortArrayLength should be equal");
  ASSERT(genericCompare(pdxobjPtr->getShortArray(), shortArray, shortArrayLength) == true , "shortArray should be equal");
  ASSERT(pIPtr->getFieldType("m_int16Array") == PdxFieldTypes::SHORT_ARRAY, "Type Value SHORT_ARRAY Mismatch");

  pIPtr->getField("m_uint16Array", &shortArray, shortArrayLength);
  ASSERT(genericValCompare(pdxobjPtr->getShortArrayLength(), shortArrayLength) == true , "shortArrayLength should be equal");
  ASSERT(genericCompare(pdxobjPtr->getUInt16Array(), shortArray, shortArrayLength) == true , "m_uint16Array should be equal");
  ASSERT(pIPtr->getFieldType("m_uint16Array") == PdxFieldTypes::SHORT_ARRAY, "Type Value SHORT_ARRAY Mismatch");

  int32_t* intArray = NULL;
  int32_t intArrayLength = 0; 
  pIPtr->getField("m_int32Array", &intArray, intArrayLength);
  ASSERT(genericValCompare(pdxobjPtr->getIntArrayLength(), intArrayLength) == true , "intArrayLength should be equal");
  ASSERT(genericCompare(pdxobjPtr->getIntArray(), intArray, intArrayLength) == true , "intArray should be equal");
  ASSERT(pIPtr->getFieldType("m_int32Array") == PdxFieldTypes::INT_ARRAY, "Type Value INT_ARRAY Mismatch");

  pIPtr->getField("m_uint32Array", &intArray, intArrayLength);
  ASSERT(genericValCompare(pdxobjPtr->getIntArrayLength(), intArrayLength) == true , "intArrayLength should be equal");
  ASSERT(genericCompare(pdxobjPtr->getUIntArray(), intArray, intArrayLength) == true , "m_uint32Array should be equal");
  ASSERT(pIPtr->getFieldType("m_uint32Array") == PdxFieldTypes::INT_ARRAY, "Type Value INT_ARRAY Mismatch");

  int64_t* longArray = NULL;
  int32_t longArrayLength = 0; 
  pIPtr->getField("m_longArray", &longArray, longArrayLength);
  ASSERT(genericValCompare(pdxobjPtr->getLongArrayLength(), longArrayLength) == true , "longArrayLength should be equal");
  ASSERT(genericCompare(pdxobjPtr->getLongArray(), longArray, longArrayLength) == true , "longArray should be equal");
  ASSERT(pIPtr->getFieldType("m_longArray") == PdxFieldTypes::LONG_ARRAY, "Type Value LONG_ARRAY Mismatch");

  pIPtr->getField("m_ulongArray", &longArray, longArrayLength);
  ASSERT(genericValCompare(pdxobjPtr->getLongArrayLength(), longArrayLength) == true , "longArrayLength should be equal");
  ASSERT(genericCompare(pdxobjPtr->getULongArray(), longArray, longArrayLength) == true , "m_ulongArray should be equal");
  ASSERT(pIPtr->getFieldType("m_ulongArray") == PdxFieldTypes::LONG_ARRAY, "Type Value LONG_ARRAY Mismatch");

  double* doubleArray = NULL;
  int32_t doubleArrayLength = 0; 
  pIPtr->getField("m_doubleArray", &doubleArray, doubleArrayLength);
  ASSERT(genericValCompare(pdxobjPtr->getDoubleArrayLength(), doubleArrayLength) == true , "doubleArrayLength should be equal");
  ASSERT(genericCompare(pdxobjPtr->getDoubleArray(), doubleArray, doubleArrayLength) == true , "doubleArray should be equal");
  ASSERT(pIPtr->getFieldType("m_doubleArray") == PdxFieldTypes::DOUBLE_ARRAY, "Type Value DOUBLE_ARRAY Mismatch");

  float* floatArray = NULL;
  int32_t floatArrayLength = 0; 
  pIPtr->getField("m_floatArray", &floatArray, floatArrayLength);
  ASSERT(genericValCompare(pdxobjPtr->getFloatArrayLength(), floatArrayLength) == true , "floatArrayLength should be equal");
  ASSERT(genericCompare(pdxobjPtr->getFloatArray(), floatArray, floatArrayLength) == true , "floatArray should be equal");
  ASSERT(pIPtr->getFieldType("m_floatArray") == PdxFieldTypes::FLOAT_ARRAY, "Type Value FLOAT_ARRAY Mismatch");
  ...
  ASSERT(enumObject->getEnumOrdinal() == pdxobjPtr->getEnum()->getEnumOrdinal(), "enumObject ordinal should be equal");  
  ASSERT(strcmp(enumObject->getEnumClassName() , pdxobjPtr->getEnum()->getEnumClassName()) == 0, "enumObject classname should be equal");
  ASSERT(strcmp(enumObject->getEnumName() , pdxobjPtr->getEnum()->getEnumName()) == 0, "enumObject enumname should be equal");  
  ASSERT(pIPtr->getFieldType("m_pdxEnum") == PdxFieldTypes::OBJECT, "Type Value OBJECT Mismatch");

  CacheableDatePtr dateObject = NULLPTR;
  pIPtr->getField("m_dateTime", dateObject);
  ASSERT(dateObject != NULLPTR , "date should not be NULL");  
  ASSERT((*(dateObject.ptr()) == *(pdxobjPtr->getDate().ptr())) == true, "dateObject should be equal"); 
  ASSERT(pIPtr->getFieldType("m_dateTime") == PdxFieldTypes::DATE, "Type Value DATE Mismatch");
  ...
  ASSERT(genericValCompare(pdxobjPtr->getHashMap()->size(), mapObject->size()) == true , "mapobject size should be equal");
  ASSERT(pIPtr->getFieldType("m_map") == PdxFieldTypes::OBJECT, "Type Value OBJECT Mismatch");
     
  pIPtr->getField("m_vector", object2);
  ASSERT(object2 != NULLPTR , "object2 should not be NULL");
  CacheableVectorPtr vec = dynCast<CacheableVectorPtr>(object2);
  ASSERT(genericValCompare(pdxobjPtr->getVector()->size(), vec->size()) == true , "vec size should be equal");
  ASSERT(pIPtr->getFieldType("m_vector") == PdxFieldTypes::OBJECT, "Type Value OBJECT Mismatch");
  
  pIPtr->getField("m_arraylist", object2);
  ASSERT(object2 != NULLPTR , "object2 should not be NULL");
  CacheableArrayListPtr arrList = dynCast<CacheableArrayListPtr>(object2);
  ASSERT(genericValCompare(pdxobjPtr->getArrayList()->size(), arrList->size()) == true , "arrList size should be equal");
  ASSERT(pIPtr->getFieldType("m_arraylist") == PdxFieldTypes::OBJECT, "Type Value OBJECT Mismatch");
     
  pIPtr->getField("m_chs", object2);
  ASSERT(object2 != NULLPTR , "object2 should not be NULL");
  CacheableHashSetPtr hashSet = dynCast<CacheableHashSetPtr>(object2);
  ASSERT(genericValCompare(pdxobjPtr->getHashSet()->size(), hashSet->size()) == true , "hashSet size should be equal");  
  ASSERT(pIPtr->getFieldType("m_chs") == PdxFieldTypes::OBJECT, "Type Value OBJECT Mismatch");
  
  pIPtr->getField("m_clhs", object2);
  ASSERT(object2 != NULLPTR , "object2 should not be NULL");
  CacheableLinkedHashSetPtr linkedHashSet = dynCast<CacheableLinkedHashSetPtr>(object2);
  ASSERT(genericValCompare(pdxobjPtr->getLinkedHashSet()->size(), linkedHashSet->size()) == true , "linkedHashSet size should be equal");  
  ASSERT(pIPtr->getFieldType("m_clhs") == PdxFieldTypes::OBJECT, "Type Value OBJECT Mismatch");

  int8_t** byteByteArrayVal = NULL;
  int32_t byteArrayLen = 0;
  int32_t* elementLength = NULL; 
  pIPtr->getField("m_byteByteArray", &byteByteArrayVal, byteArrayLen, elementLength);
  ASSERT(genericValCompare(pdxobjPtr->getbyteByteArrayLength(), byteArrayLen) == true , "byteByteArrayLength should be equal");
  int8_t** bytArray = pdxobjPtr->getArrayOfByteArrays();
  ASSERT(generic2DCompare(byteByteArrayVal, bytArray, byteArrayLen, elementLength) == true , "byteByteArray should be equal");  
  ASSERT(pIPtr->getFieldType("m_byteByteArray") == PdxFieldTypes::ARRAY_OF_BYTE_ARRAYS, "Type Value ARRAY_OF_BYTE_ARRAYS Mismatch");
  ...
  ASSERT(genericValCompare(pdxobjPtr->getCacheableObjectArray()->size(), objectArray->size()) == true , "objectArray size should be equal");  
  ASSERT(pIPtr->getFieldType("m_objectArray") == PdxFieldTypes::OBJECT_ARRAY, "Type Value OBJECT_ARRAY Mismatch");

  CacheableObjectArrayPtr objectArrayEmptyFieldName = NULLPTR; 
  pIPtr->getField("", objectArrayEmptyFieldName);
  ASSERT(objectArrayEmptyFieldName != NULLPTR , "objectArrayEmptyFieldName should not be NULL");  
  ASSERT(genericValCompare(pdxobjPtr->getCacheableObjectArrayEmptyPdxFieldName()->size(), objectArrayEmptyFieldName->size()) == true , 
  ...
  ASSERT((cpo.ptr()->equals(*childpdxobjPtr.ptr())) == true, "child pdx should be equal");
...
DUNIT_TASK_DEFINITION(CLIENT2, modifyPdxInstance)
{
  ...
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");

  wpiPtr = pIPtr->createWriter();
  try {
  wpiPtr->setField("m_int32", false);
  FAIL("setField on m_int32 with bool value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_int32 with bool value caught expected IllegalStateException");
  }
  ...
  FAIL("setField on m_bool with int value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_bool with int value caught expected IllegalStateException");
  }
  ...
  FAIL("setField on m_char with float value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_char with float value caught expected IllegalStateException");
  }
  ...
  wpiPtr = pIPtr->createWriter();
  try {
  wpiPtr->setField("m_byte", (int64_t)0x56787878);
  FAIL("setField on m_byte with int64_t value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_byte with int64_t value caught expected IllegalStateException");
  }
 
  int16_t shortVal = 0;
  wpiPtr = pIPtr->createWriter();
  wpiPtr->setField("m_int16", (int16_t)0x5678);
  rptr->put(keyport, wpiPtr);
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport));
  ...
  wpiPtr = pIPtr->createWriter();
  try {
  wpiPtr->setField("m_int16", (int64_t)0x56787878);
  FAIL("setField on m_int16 with int64_t value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_int16 with int64_t value caught expected IllegalStateException");
  }

  int64_t longVal = 0;
  wpiPtr = pIPtr->createWriter();
  wpiPtr->setField("m_long", (int64_t)0x56787878);
  rptr->put(keyport, wpiPtr);
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport));
  ...
  FAIL("setField on m_long with float value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_long with float value caught expected IllegalStateException");
  }
  ...
  FAIL("setField on m_float with char value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_float with char value caught expected IllegalStateException");
  }
  ...
  FAIL("setField on m_double with char value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_double with char value caught expected IllegalStateException");
  }
  ...
  rptr->put(keyport, wpiPtr);
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport));
  ASSERT(newPiPtr->hasField("m_boolArray") == true, "m_boolArray = true expected");
  newPiPtr->getField("m_boolArray", &getBoolArray, arrayLen);
  ASSERT(arrayLen == 8, "Arraylength == 8 expected");
  ASSERT(genericCompare(setBoolArray, getBoolArray, arrayLen) == true , "boolArray should be equal");
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");

  wpiPtr = pIPtr->createWriter();
  try {
  wpiPtr->setField("m_boolArray", 'D');
  FAIL("setField on m_boolArray with char value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_boolArray with char value caught expected IllegalStateException");
  }
  ...
  rptr->put(keyport, wpiPtr);
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport));
  ASSERT(newPiPtr->hasField("m_byteArray") == true, "m_byteArray = true expected");
  newPiPtr->getField("m_byteArray", &getByteArray, arrayLen);
  ASSERT(arrayLen == 4, "Arraylength == 4 expected");
  ASSERT(genericCompare(setByteArray, getByteArray, arrayLen) == true , "byteArray should be equal");
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");

  wpiPtr = pIPtr->createWriter();
  try {
  wpiPtr->setField("m_byteArray", "string");
  FAIL("setField on m_byteArray with string value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_byteArray with string value caught expected IllegalStateException");
  }
  ...
  rptr->put(keyport, wpiPtr);
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport));
  ASSERT(newPiPtr->hasField("m_charArray") == true, "m_charArray = true expected");
  newPiPtr->getField("m_charArray", &getCharArray, arrayLen);
  ASSERT(arrayLen == 4, "Arraylength == 4 expected");
  ASSERT(genericCompare(setCharArray, getCharArray, arrayLen) == true , "charArray should be equal");
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");

  wpiPtr = pIPtr->createWriter();
  try {
  wpiPtr->setField("m_charArray", "string");
  FAIL("setField on m_charArray with string value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_charArray with string value caught expected IllegalStateException");
  }
  ...
  rptr->put(keyport, wpiPtr);
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport));
  ASSERT(newPiPtr->hasField("m_int16Array") == true, "m_int16Array = true expected");
  newPiPtr->getField("m_int16Array", &getShortArray, arrayLen);
  ASSERT(arrayLen == 4, "Arraylength == 4 expected");
  ASSERT(genericCompare(setShortArray, getShortArray, arrayLen) == true , "shortArray should be equal");
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");

  wpiPtr = pIPtr->createWriter();
  try {
  wpiPtr->setField("m_int16Array", setCharArray);
  FAIL("setField on m_int16Array with setCharArray value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_int16Array with setCharArray value caught expected IllegalStateException");
  }
  ...
  rptr->put(keyport, wpiPtr);
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport));
  ASSERT(newPiPtr->hasField("m_int32Array") == true, "m_int32Array = true expected");
  newPiPtr->getField("m_int32Array", &newValArray, arrayLen);
  ASSERT(arrayLen == 3, "Arraylength == 3 expected");
  ASSERT(genericCompare(setIntArray, newValArray, arrayLen) == true , "intArray should be equal");
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");

  wpiPtr = pIPtr->createWriter();
  try {
  wpiPtr->setField("m_int32Array", setShortArray);
  FAIL("setField on m_int32Array with setShortArray value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_int32Array with setShortArray value caught expected IllegalStateException");
  }
  ...
  rptr->put(keyport, wpiPtr);
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport));
  ASSERT(newPiPtr->hasField("m_longArray") == true, "m_longArray = true expected");
  newPiPtr->getField("m_longArray", &getLongArray, arrayLen);
  ASSERT(arrayLen == 2, "Arraylength == 2 expected");
  ASSERT(genericCompare(setLongArray, getLongArray, arrayLen) == true , "longArray should be equal");
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");

  wpiPtr = pIPtr->createWriter();
  try {
  wpiPtr->setField("m_longArray", setIntArray);
  FAIL("setField on m_longArray with setIntArray value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_longArray with setIntArray value caught expected IllegalStateException");
  }
  ...
  rptr->put(keyport, wpiPtr);
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport));
  ASSERT(newPiPtr->hasField("m_floatArray") == true, "m_floatArray = true expected");
  newPiPtr->getField("m_floatArray", &getFloatArray, arrayLen);
  ASSERT(arrayLen == 2, "Arraylength == 2 expected");
  ASSERT(genericCompare(setFloatArray, getFloatArray, arrayLen) == true , "floatArray should be equal");
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");

  wpiPtr = pIPtr->createWriter();
  try {
  wpiPtr->setField("m_floatArray", setLongArray);
  FAIL("setField on m_floatArray with setLongArray value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_floatArray with setLongArray value caught expected IllegalStateException");
  }
  ...
  rptr->put(keyport, wpiPtr);
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport));
  ASSERT(newPiPtr->hasField("m_doubleArray") == true, "m_doubleArray = true expected");
  newPiPtr->getField("m_doubleArray", &getDoubleArray, arrayLen);
  ASSERT(arrayLen == 2, "Arraylength == 2 expected");
  ASSERT(genericCompare(setDoubleArray, getDoubleArray, arrayLen) == true , "doubleArray should be equal");
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");

  wpiPtr = pIPtr->createWriter();
  try {
  wpiPtr->setField("m_doubleArray", setFloatArray);
  FAIL("setField on m_doubleArray with setFloatArray value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_doubleArray with setFloatArray value caught expected IllegalStateException");
  }
  ...
  ASSERT(wcscmp(wideStringVal, setWideString) == 0 , "wide stringVal should be equal");
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");

  wpiPtr = pIPtr->createWriter();
  try {
  wpiPtr->setField("m_string", setFloatArray);
  FAIL("setField on m_string with setFloatArray value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_string with setFloatArray value caught expected IllegalStateException");
  }
  ...
  rptr->put(keyport, wpiPtr);  
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport));
  ASSERT(newPiPtr->hasField("m_dateTime") == true, "m_dateTime = true expected");
  ASSERT(pIPtr->hasField("m_dateTime") == true, "m_date = true expected");
  newPiPtr->getField("m_dateTime", dateVal);
  ASSERT((*(dateVal.ptr()) == *(datePtr.ptr())) == true, "dateObject should be equal");
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");

  wpiPtr = pIPtr->createWriter();
  try {
  wpiPtr->setField("m_dateTime", setString);
  FAIL("setField on m_dateTime with setString value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_dateTime with setString value caught expected IllegalStateException");
  }
  ...
  ASSERT(genericValCompare(setVec->size(), vecVal->size()) == true , "vec size should be equal");
  ...
  FAIL("setField on m_vector with setString value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_vector with setString value caught expected IllegalStateException");
  }
  ...
  rptr->put(keyport, wpiPtr);  
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport));
  ASSERT(newPiPtr->hasField("m_arraylist") == true, "m_arraylist = true expected");
  ASSERT(pIPtr->hasField("m_arraylist") == true, "m_arraylist = true expected");
  newPiPtr->getField("m_arraylist", object);
  CacheableArrayListPtr arrVal = dynCast<CacheableArrayListPtr>(object);
  ASSERT(genericValCompare(setarr->size(), arrVal->size()) == true , "arrList size should be equal");
  ...
  FAIL("setField on m_arraylist with setFloatArray value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_arraylist with setFloatArray value caught expected IllegalStateException");
  }
  ...
  ASSERT(genericValCompare(hashset->size(), hashsetVal->size()) == true , "m_chs size should be equal");

  wpiPtr = pIPtr->createWriter();
  try {
  wpiPtr->setField("m_chs", 100);
  FAIL("setField on m_chs with int value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_chs with int value caught expected IllegalStateException");
  }
  ...
  ASSERT(genericValCompare(hashmap->size(), hashmapVal->size()) == true , "m_map size should be equal");

  wpiPtr = pIPtr->createWriter();
  try {
  wpiPtr->setField("m_map", 0.0f);
  FAIL("setField on m_map with float value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_map with float value caught expected IllegalStateException");
  }
  ...
  CacheableLinkedHashSetPtr linkedhashsetVal = dynCast<CacheableLinkedHashSetPtr>(object);
  ASSERT(genericValCompare(linkedhashsetVal->size(), linkedhashset->size()) == true , "m_clhs size should be equal");

  wpiPtr = pIPtr->createWriter();
  try {
  wpiPtr->setField("m_clhs", 'c');
  FAIL("setField on m_clhs with char value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_clhs with char value caught expected IllegalStateException");
  }
  ...
  ASSERT(newPiPtr->hasField("m_byteByteArray") == true, "m_byteByteArray = true expected");
  LOGINFO("Testing byteByteArray hasField");
  ASSERT(pIPtr->hasField("m_byteByteArray") == true, "m_byteByteArray = true expected");
  newPiPtr->getField("m_byteByteArray", &getbyteByteArray, byteByteArrayLength, elementLength);
  LOGINFO("Testing byteByteArray getField");
  ASSERT(genericValCompare(4, byteByteArrayLength) == true , "byteByteArrayLength should be equal");
  ASSERT(generic2DCompare(getbyteByteArray, setbyteByteArray, byteByteArrayLength, elementLength) == true , "byteByteArray should be equal");

  wpiPtr = pIPtr->createWriter();
  try {
  wpiPtr->setField("m_byteByteArray", (CacheablePtr)linkedhashset);
  FAIL("setField on m_byteByteArray with linkedhashset value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_byteByteArray with linkedhashset value caught expected IllegalStateException");
  }
  ...
  int size = wcslen(wstr1);
  for(int i = 0; i < 3; i++){
    setWideStringArray[i] = new wchar_t[size];
  }
  setWideStringArray[0] = (wchar_t*)wstr1;
  setWideStringArray[1] = (wchar_t*)wstr2;
  setWideStringArray[2] = (wchar_t*)wstr3;
  wchar_t** getWideStringArray = NULL;    
  wpiPtr->setField("m_stringArray", setWideStringArray, 3);
  rptr->put(keyport, wpiPtr);
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport));
  ASSERT(pIPtr->hasField("m_stringArray") == true, "m_stringArray = true expected");
  ...
  for(int i=0; i<arrayLen; i++) {
    ...
    ASSERT(wcscmp(setWideStringArray[i], getWideStringArray[i]) == 0, "All stringVals should be equal");    
  } 
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");
  ...
  FAIL("setField on m_stringArray with setbyteByteArray value should throw expected IllegalStateException");
  } catch (IllegalStateException& ) {
    LOG("setField on m_stringArray with setbyteByteArray value caught expected IllegalStateException");
  }
  ...
  setStringArray[0] = (char*)str1;
  setStringArray[1] = (char*)str2;
  setStringArray[2] = (char*)str3;
  ...
  for(int i=0; i<arrayLen; i++) {
    ...
    ASSERT(strcmp(setStringArray[i], getStringArray[i]) == 0, "All stringVals should be equal");    
  } 
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");
  ...
  ASSERT((*cpi.ptr() == *cpi1.ptr()) == false, "PdxInstance should not be equal");
  ...
  rptr->put(keyport1, wpiPtr);  
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport1));
  ASSERT(newPiPtr->hasField("m_wideChar") == true, "m_wideChar = true expected");
  newPiPtr->getField("m_wideChar", parentWideCharVal);  
  ASSERT(parentWideCharVal == parentWideCharSetVal, "m_wideChar is not equal");
  ...
  rptr->put(keyport1, wpiPtr);
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport1));
  ASSERT(newPiPtr->hasField("m_wideCharArray") == true, "m_wideCharArray = true expected");
  newPiPtr->getField("m_wideCharArray", &getParentWideCharArray, arrayLen);
  ASSERT(arrayLen == 4, "Arraylength == 4 expected");
  ASSERT(genericCompare(setParentWideCharArray, getParentWideCharArray, arrayLen) == true , "m_wideCharArray should be equal");
  ...
  ASSERT(arrayLen == 4, "Arraylength == 4 expected");
  ASSERT(genericCompare(setParentCharArray, getParentCharArray, arrayLen) == true , "charArray should be equal");
...
DUNIT_TASK_DEFINITION(CLIENT2, modifyPdxInstanceAndCheckLocally)
{
  ...
  rptr->put(keyport, wpiPtr);
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport));
  ASSERT(pIPtr->hasField("m_boolArray") == true, "m_boolArray = true expected");
  newPiPtr->getField("m_boolArray", &getBoolArray, arrayLen);
  ASSERT(arrayLen == 8, "Arraylength == 8 expected");
  ASSERT(genericCompare(setBoolArray, getBoolArray, arrayLen) == true , "boolArray should be equal");
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");
  ...
  rptr->put(keyport, wpiPtr);
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport));
  ASSERT(pIPtr->hasField("m_byteArray") == true, "m_byteArray = true expected");
  newPiPtr->getField("m_byteArray", &getByteArray, arrayLen);
  ASSERT(arrayLen == 4, "Arraylength == 4 expected");
  ASSERT(genericCompare(setByteArray, getByteArray, arrayLen) == true , "byteArray should be equal");
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");
  ...
  rptr->put(keyport, wpiPtr);
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport));
  ASSERT(pIPtr->hasField("m_int16Array") == true, "m_int16Array = true expected");
  newPiPtr->getField("m_int16Array", &getShortArray, arrayLen);
  ASSERT(arrayLen == 4, "Arraylength == 4 expected");
  ASSERT(genericCompare(setShortArray, getShortArray, arrayLen) == true , "shortArray should be equal");
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");
  ...
  rptr->put(keyport, wpiPtr);
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport));
  ASSERT(pIPtr->hasField("m_int32Array") == true, "m_int32Array = true expected");
  newPiPtr->getField("m_int32Array", &newValArray, arrayLen);
  ASSERT(arrayLen == 3, "Arraylength == 3 expected");
  ASSERT(genericCompare(setIntArray, newValArray, arrayLen) == true , "intArray should be equal");
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");
  ...
  rptr->put(keyport, wpiPtr);
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport));
  ASSERT(pIPtr->hasField("m_longArray") == true, "m_longArray = true expected");
  newPiPtr->getField("m_longArray", &getLongArray, arrayLen);
  ASSERT(arrayLen == 2, "Arraylength == 2 expected");
  ASSERT(genericCompare(setLongArray, getLongArray, arrayLen) == true , "longArray should be equal");
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");
  ...
  rptr->put(keyport, wpiPtr);
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport));
  ASSERT(pIPtr->hasField("m_floatArray") == true, "m_floatArray = true expected");
  newPiPtr->getField("m_floatArray", &getFloatArray, arrayLen);
  ASSERT(arrayLen == 2, "Arraylength == 2 expected");
  ASSERT(genericCompare(setFloatArray, getFloatArray, arrayLen) == true , "floatArray should be equal");
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");
  ...
  rptr->put(keyport, wpiPtr);
  newPiPtr = dynCast<PdxInstancePtr>(rptr->get(keyport));
  ASSERT(pIPtr->hasField("m_doubleArray") == true, "m_doubleArray = true expected");
  newPiPtr->getField("m_doubleArray", &getDoubleArray, arrayLen);
  ASSERT(arrayLen == 2, "Arraylength == 2 expected");
  ASSERT(genericCompare(setDoubleArray, getDoubleArray, arrayLen) == true , "doubleArray should be equal");
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");
  ...
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == true, "PdxInstance should be equal");
  ...
  LOG( "modifyPdxInstanceAndCheckLocally get again complete." );
  newPiPtr->getField("m_float", fVal);
  LOGINFO( "modifyPdxInstanceAndCheckLocally getField complete. fval = %f", fVal );
  ASSERT(fVal == 18389.34f, "fval is not equal");
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");
  ...
  LOG( "modifyPdxInstanceAndCheckLocally get again complete." );
  newPiPtr->getField("m_double", dVal);
  LOGINFO( "modifyPdxInstanceAndCheckLocally getField complete. fval = %lf", dVal );
  ...
  LOG( "modifyPdxInstanceAndCheckLocally get again complete." );
  newPiPtr->getField("m_byte", byteVal);
  LOGINFO( "modifyPdxInstanceAndCheckLocally getField complete byteVal = %d ", byteVal);
  ...
  LOG( "modifyPdxInstanceAndCheckLocally get again complete." );
  newPiPtr->getField("m_int16", shortVal);
  LOGINFO( "modifyPdxInstanceAndCheckLocally getField complete shortVal = %d ", shortVal );
  ASSERT(shortVal == 0x5678, "short is not equal");
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");
  ...
  LOG( "modifyPdxInstanceAndCheckLocally get again complete." );
  newPiPtr->getField("m_long", longVal);
  LOGINFO( "modifyPdxInstanceAndCheckLocally getField complete longVal = %ld ", longVal );
  ASSERT(longVal == 0x56787878, "long is not equal");  
  ASSERT((*pIPtr.ptr() == *newPiPtr.ptr()) == false, "PdxInstance should not be equal");
  ...
  newPiPtr->getField("m_string", &stringVal);
  LOGINFO( "modifyPdxInstanceAndCheckLocally stringVal = %ls , str = %ls ", stringVal, str );
  ...
  LOGINFO( "modifyPdxInstanceAndCheckLocally getstringVal = %s , str1 = %s ", getstringVal, str1 );
...
DUNIT_TASK_DEFINITION(CLIENT1, pdxIFPutGetTest)
{
  ...
  PdxInstanceFactoryPtr pifPtr = cacheHelper->getCache()->createPdxInstanceFactory("PdxTests.PdxType");
  ...
  try {
    pifPtr->writeBoolean("m_bool", pdxobj->getBool());
    FAIL( "calling writeBoolean on same fieldname should have thrown IllegalStateException" );
  } catch(IllegalStateException&) {
  ...
  pifPtr->writeObjectArray("m_objectArray", pdxobj->getCacheableObjectArray());
  ...
  pifPtr->writeArrayOfByteArrays("m_byteByteArray", pdxobj->getArrayOfByteArrays(), 2, lengthArr);
  ...
  pifPtr->writeObjectArray("", pdxobj->getCacheableObjectArrayEmptyPdxFieldName());

  PdxInstancePtr ret = pifPtr->create();
  LOG("PdxInstancePtr created....");

  LOGINFO("PdxInstance getClassName = %s ", ret->getClassName()); 
  ASSERT(strcmp(ret->getClassName(), "PdxTests.PdxType") == 0, "pdxInstance.getClassName should return PdxTests.PdxType.");
  ...
  ASSERT(lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceDeserializations() == 1, "pdxInstanceDeserialization should be equal to 1.");
  ASSERT(lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceCreations() == 0, "pdxInstanceCreations should be equal to 0.");
  ASSERT(lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceDeserializationTime() == 0, "pdxInstanceDeserializationTime should be equal to 0.");
  ...
  PdxInstanceFactoryPtr if2 = cacheHelper->getCache()->createPdxInstanceFactory("testobject::ParentPdx");
  ...
  if2->writeWideStringArray("m_wideparentArrayName", pp->getWideParentArrayName(), 3);  
  ...
  ASSERT(strcmp(ip2->getClassName(), "testobject::ParentPdx") == 0, "pdxInstance.getClassName should return testobject::ParentPdx.");
  ...
  LOGINFO("pdxInstanceDeserializations for (testobject::ParentPdx) PdxInstance  = %d ", lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceDeserializations());
  LOGINFO("pdxInstanceCreations for (testobject::ParentPdx) PdxInstance  = %d ", lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceCreations());
  LOGINFO("pdxInstanceDeserializationTime for(testobject::ParentPdx) PdxInstance  = %d ", lregPtr->getCacheImpl()->m_cacheStats->getPdxInstanceDeserializationTime());
  ...
  ASSERT(rawPP1->equals(*rawPP2, false) == true, "ParentPdx objects should be equal.");
...
DUNIT_TASK_DEFINITION(CLIENT1, pdxInstanceWithEmptyKeys)
{
  ...
  RegionPtr rptr = getHelper()->getRegion( regionNames[0] );
  PdxInstanceFactoryPtr pifPtr = cacheHelper->getCache()->createPdxInstanceFactory("EMPTY_KEY_NAME");
  LOG("PdxInstanceFactoryPtr created....");
  ...
  ASSERT (*putValue.ptr() == *getValue.ptr(), "Boolean Value Did not match in case of Empty PdxField Key" );
  bool fieldValue;
  getValue->getField("", fieldValue);
  ASSERT (fieldValue == falseValue, "Mismatch in the PdxInstance with Empty key");
...
void runThinClientPdxInstance(bool isLocator, int index)
{
  if( isLocator )
  {
  ...
  {    
    ...
    { 
      CALL_TASK( StepOne_Pooled_EndPoint_PdxReadSerialized);
      CALL_TASK( StepTwo_Pooled_EndPoint_CachingEnabled_PdxReadSerialized);      
    }
  ...
  {  
    ...
    { 
      CALL_TASK( StepOne_Pooled_EndPoint_PdxReadSerialized);
      CALL_TASK( StepTwo_Pooled_EndPoint_PdxReadSerialized);      
    }
    CALL_TASK( pdxPut );
    CALL_TASK( getObject ); 
    ...
    CALL_TASK( StepOne_Pooled_EndPoint_PdxReadSerialized_XML);
    CALL_TASK( StepTwo_Pooled_EndPoint_PdxReadSerialized_XML);

    CALL_TASK(testReadSerializedXMLProperty_Client1);
    CALL_TASK(testReadSerializedXMLProperty_Client2);
    CALL_TASK(putPdxWithIdentityField);
    CALL_TASK(verifyPdxIdentityField);
    CALL_TASK(putPdxWithNullIdentityFields);
    CALL_TASK(verifyPdxNullIdentityFieldHC);    

    CALL_TASK(putCacheableObjectArrayWithPdxFields);
    CALL_TASK(verifyCacheableObjectArrayWithPdxField);
...
DUNIT_MAIN
{
  ...
  runThinClientPdxInstance( true, 0 );   // Locator, caching = false, PdxReadSerialized = false
  runThinClientPdxInstance( false, 0 );  // EP, caching = false, PdxReadSerialized = false  

  runThinClientPdxInstance( true, 1 );   // Locator, caching = true, PdxReadSerialized = true
  runThinClientPdxInstance( false, 1 );  // EP, caching = true, PdxReadSerialized = true 

  runThinClientPdxInstance( true, 2 );   // Locator, caching = false, PdxReadSerialized = true
  runThinClientPdxInstance( false, 2 );  // EP, caching = false, PdxReadSerialized = true 
  
  runThinClientPdxInstance( false, 3 );  // EP, caching = false, PdxReadSerialized = true with client xml
