...
void doAttrTestingAndCreatePool(const char* poolName)
{
 ...
 // poolFacPtr->setMultiuserSecurityMode(true);
  ...
  ASSERT(pptr->getFreeConnectionTimeout()==10000,"FreeConnectionTimeout Should have been 10000");
  ASSERT(pptr->getLoadConditioningInterval()==60000,"LoadConditioningInterval Should have been 60000");
  ASSERT(pptr->getSocketBufferSize()==1024,"SocketBufferSize Should have been 1024");
  ...
  ASSERT(pptr->getPingInterval()==120000,"PingInterval Should have been 120000");
  ASSERT(pptr->getUpdateLocatorListInterval()==122000,"UpdateLocatorListInterval Should have been 122000");
  ASSERT(pptr->getStatisticInterval()==120000,"StatisticInterval Should have been 120000");
  ASSERT(strcmp(pptr->getServerGroup(),"ServerGroup1")==0,"ServerGroup Should have been ServerGroup1");
  ASSERT(pptr->getSubscriptionEnabled()==true,"SubscriptionEnabled Should have been true");
  ASSERT(pptr->getSubscriptionRedundancy()==1,"SubscriptionRedundancy Should have been 1");
  ASSERT(pptr->getSubscriptionMessageTrackingTimeout()==500000,"SubscriptionMessageTrackingTimeout Should have been 500000");
  ASSERT(pptr->getSubscriptionAckInterval()==120000,"SubscriptionAckInterval Should have been 120000");
 // ASSERT(pptr->getMultiuserSecurityMode()==true,"SetMultiuserSecurityMode Should have been true");
  ASSERT(pptr->getPRSingleHopEnabled()== false, "PRSingleHopEnabled should have been false");
}
...
void doAttrTesting(const char* poolName1)
{
    ...
    ASSERT(strcmp(pptr->getName(),"clientPool")==0, "Pool name should have been clientPool" );
    ASSERT(pptr->getFreeConnectionTimeout()==10000,"FreeConnectionTimeout Should have been 10000");
    ASSERT(pptr->getLoadConditioningInterval()==1,"LoadConditioningInterval Should have been 1");
    ASSERT(pptr->getSocketBufferSize()==1024,"SocketBufferSize Should have been 1024");
	...
	ASSERT(pptr->getUpdateLocatorListInterval()==25000,"UpdateLocatorListInterval Should have been 25000");
    ASSERT(pptr->getStatisticInterval()==1,"StatisticInterval Should have been 1");
    ASSERT(strcmp(pptr->getServerGroup(),"ServerGroup1")==0,"ServerGroup Should have been ServerGroup1");
    ASSERT(pptr->getSubscriptionEnabled()==true,"SubscriptionEnabled Should have been true");
    ASSERT(pptr->getSubscriptionRedundancy()==1,"SubscriptionRedundancy Should have been 1");
    ASSERT(pptr->getSubscriptionMessageTrackingTimeout()==5,"SubscriptionMessageTrackingTimeout Should have been 5");
    ASSERT(pptr->getSubscriptionAckInterval()==1,"SubscriptionAckInterval Should have been 1");
    ASSERT(pptr->getPRSingleHopEnabled()== false, "PRSingleHopEnabled should have been false");
}
...
DUNIT_TASK(CLIENT2, StartC2)
{
  ...
  try {
    LOG("hitesh starts client");
    initClient(true, props);
    LOG("hitesh started client");
    ASSERT( PoolManager::find("clientPoolMultiUser")->getMultiuserAuthentication() == true, "MultiUser secure mode should be true for Pool"); 
...
{
  // Wait for load conditioning thread to reduce pool connections to min.
  SLEEP( 5000 );
  //Check current # connections they should be == min
  char number[20] = {0};
  std::string poolName = getHelper()->getRegion(poolRegNames[0])->getAttributes()->getPoolName();
  int level = TestUtils::getCacheImpl(getHelper()->cachePtr)->getPoolSize(poolName.c_str());
  int min = PoolManager::find(poolName.c_str())->getMinConnections();
  char logmsg[100] = {0};
  sprintf(logmsg, "Pool level not equal to min level. Expected %d, actual %d", min, level);
  ...
  level = TestUtils::getCacheImpl(getHelper()->cachePtr)->getPoolSize(poolName.c_str());
  int max = PoolManager::find(poolName.c_str())->getMaxConnections();
  sprintf(logmsg, "Pool level not equal to max level. Expected %d, actual %d", max, level);
  ...
  //Milli second sleep: IdleTimeout is 5 sec, load conditioning interval is 1 min
  LOG("Waiting 25 sec for idle timeout to kick in");
  SLEEP(25000);

  level = TestUtils::getCacheImpl(getHelper()->cachePtr)->getPoolSize(poolName.c_str());
  min = PoolManager::find(poolName.c_str())->getMinConnections();
  sprintf(logmsg, "Pool level not equal to min level after idle timeout. Expected %d, actual %d", min, level);
  ASSERT(level==min, logmsg);
  ...
  level = TestUtils::getCacheImpl(getHelper()->cachePtr)->getPoolSize(poolName.c_str());
  sprintf(logmsg, "Pool level not equal to min level after load conditioning. Expected %d, actual %d", min, level);
  ASSERT(level==min, logmsg);
}
END_TASK(ClientOp)
