...
const char * serverEndPoints = CacheHelper::getTcrEndpoints(isLocalServer, 2);
...
void createServer(bool locator = false)
{
  ...
  if ( isLocalServer ) CacheHelper::initServer( 1, "remotequery.xml", locator?locHostPort:NULL);
  LOG("SERVER1 started");
}
...
void createServer2(bool locator = false)
{
  ...
  if ( isLocalServer ) CacheHelper::initServer( 2, "remotequery2.xml", locator?locHostPort:NULL);  
  LOG("SERVER2 started");
}
...
{
  LOG("Starting SERVER1...");
  if ( isLocalServer ) CacheHelper::initServer( 1, XML, locator?locHostPort:NULL);
  LOG("SERVER1 started");
}
...
{
  LOG("Starting SERVER2...");
  if ( isLocalServer ) CacheHelper::initServer( 2, XML, locator?locHostPort:NULL);
  LOG("SERVER2 started");
}
...
void initCqStatusClient(bool pool = false, bool locator = false)
{
  if ( cacheHelper == NULL ) {
  ...
  createRegionForCQ( pool, locator, regionName, USE_ACK, serverEndPoints, true);
  createRegionForCQ( pool, locator, regionName1, USE_ACK, serverEndPoints, true);
  LOG( "initCqStatusClient complete." );
}
 
DUNIT_TASK_DEFINITION(CLIENT1, initCqStatusClientEP)
{
  initCqStatusClient(true, false);
}
END_TASK_DEFINITION

DUNIT_TASK_DEFINITION(CLIENT1, initCqStatusClientLoc)
{
  initCqStatusClient(true, true);
}
END_TASK_DEFINITION
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOne_Pooled_Locator)
{
  ...
  getHelper()->createPoolWithLocators("__TEST_POOL1__", locHostPort, true, -1 , -1, -1, false, "group1");
  getHelper()->createRegionAndAttachPool(regionName, USE_ACK, "__TEST_POOL1__", true);

  getHelper()->createPoolWithLocators("__TEST_POOL2__", locHostPort, true, -1 , -1, -1, false, "group2");
  getHelper()->createRegionAndAttachPool(regionName1, USE_ACK, "__TEST_POOL2__", true);
...
DUNIT_TASK_DEFINITION(CLIENT1, StepThree)
{
  ...
  /* Test for #1026 */
  {
	try
	{
      ...
      regionPtr->put("1026_Key2","Schüler");//string with extended charater set
      regionPtr->put("1026_Key3","Gebäude");//string with extended charater set
      regionPtr->put("1026_Key4","Königin");//string with extended charater set
...
DUNIT_TASK_DEFINITION(CLIENT1, StepFour)
{
  ...
  for(i=0; i < MAX_LISTNER; i++)
  {
    ...
    sprintf(buf, "Cq[%s]From CqStatistics: numInserts[%d], numDeletes[%d], numUpdates[%d], numEvents[%d]", cqNames[i], cqStats->numInserts(), cqStats->numDeletes(), cqStats->numUpdates(), cqStats->numEvents());
    LOG(buf);
    for(j=0; j <= i; j++)
    {
    ...
    sprintf(buf, "number of listeners for cq[%s] is %d", cqNames[i], vl.size());
    LOG(buf);
    ...
    if(i==(MAX_LISTNER-1))
    {
       ...
       for(j=0; j< MAX_LISTNER; j++)
       {
         ...
         sprintf(buf, "MyCount for Listener[%d]: numInserts[%d], numDeletes[%d], numUpdates[%d], numEvents[%d]", j, ml->getNumInserts(), ml->getNumDeletes(), ml->getNumUpdates(), ml->getNumEvents());
         LOG(buf);
         sprintf(buf, "sum of stats for Listener[%d]: numInserts[%d], numDeletes[%d], numUpdates[%d], numEvents[%d]",  j, inserts[j], deletes[j], updates[j], events[j]);
         LOG(buf);
	 ASSERT(ml->getNumInserts()==inserts[j],"accumulative insert count incorrect");
	 ASSERT(ml->getNumUpdates()==updates[j],"accumulative updates count incorrect");
	 ASSERT(ml->getNumDeletes()==deletes[j],"accumulative deletes count incorrect");
	 ASSERT(ml->getNumEvents()==events[j],"accumulative events count incorrect");
       }
...
DUNIT_TASK_DEFINITION(CLIENT1, createCQ)
{
  ...
  cqFac.addCqListener(cqLstner);
  CqAttributesPtr cqAttr = cqFac.create();
  CqQueryPtr cq = qs->newCq((char*)cqName, cqQueryStatusString, cqAttr);
  ...
  VectorOfCqListener vl;
  cqAttr->getCqListeners(vl);  
  MyCqStatusListener* myStatusCq = dynamic_cast<MyCqStatusListener*>(vl[0].ptr());
  LOGINFO("checkCQStatusOnConnect = %d ", myStatusCq->getCqsConnectedCount());  
  ASSERT(myStatusCq->getCqsConnectedCount() == 1, "incorrect number of CqStatus Connected count.");
...
DUNIT_TASK_DEFINITION(CLIENT1, createCQ_Pool)
{
  ...
  CqQueryPtr cq2 = qs2->newCq((char*)cqName1, cqQueryStatusString1, cqAttr1);
  cq2->execute();  
  SLEEP(20000); 
  ...
  MyCqStatusListener* myStatusCq2 = dynamic_cast<MyCqStatusListener*>(vl2[0].ptr());
  LOGINFO("checkCQStatusOnConnect = %d ", myStatusCq2->getCqsConnectedCount());  
  ASSERT(myStatusCq2->getCqsConnectedCount() == 1, "incorrect number of CqStatus Connected count.");
  ...
  ASSERT(myStatusCq->getNumInserts() == 5, "incorrect number of CqStatus Updates count.");

  LOGINFO("checkCQStatusOnPutEvent = %d ", myStatusCq2->getNumInserts());  
  ASSERT(myStatusCq2->getNumInserts() == 5, "incorrect number of CqStatus Updates count.");
...
void executeCq(const char* poolName, const char* name) {    
  ...
  CqQueryPtr cq = qs->getCq((char*)name);  
  cq->execute();  
  SLEEP(20000);
...
void checkCQStatusOnConnect(const char* poolName, const char* name, int connect) {
  PoolPtr pool = PoolManager::find(poolName);
  QueryServicePtr qs;
  if(pool != NULLPTR) {    
    qs = pool->getQueryService();
  ...
  MyCqStatusListener* myStatusCq = dynamic_cast<MyCqStatusListener*>(vl[0].ptr());
  LOGINFO("checkCQStatusOnConnect = %d ", myStatusCq->getCqsConnectedCount());  
  ASSERT(myStatusCq->getCqsConnectedCount() == connect, "incorrect number of CqStatus Connected count.");
}

DUNIT_TASK_DEFINITION(CLIENT1, checkCQStatusOnConnect)
{ 
  SLEEP(20000);
  checkCQStatusOnConnect(regionName, (char*)cqName, 1);
...
{ 
  SLEEP(20000);
  checkCQStatusOnConnect(regionName, (char*)cqName, 2);
...
{ 
  SLEEP(20000);
  checkCQStatusOnConnect("__TEST_POOL1__", (char*)cqName, 1);  
  checkCQStatusOnConnect("__TEST_POOL2__", (char*)cqName1, 1);

  LOG( "checkCQStatusOnConnect_Pool complete." );
}
END_TASK_DEFINITION

void checkCQStatusOnDisConnect(const char* poolName, const char* cqName, int disconnect) {
  PoolPtr pool = PoolManager::find(poolName);
  QueryServicePtr qs;
  if(pool != NULLPTR) {      
    qs = pool->getQueryService();
  } 
  CqQueryPtr cq = qs->getCq((char*)cqName);    
  CqAttributesPtr cqAttr = cq->getCqAttributes();
  VectorOfCqListener vl;
  cqAttr->getCqListeners(vl);  
  MyCqStatusListener* myStatusCq = dynamic_cast<MyCqStatusListener*>(vl[0].ptr());
  LOGINFO("checkCQStatusOnDisConnect = %d ", myStatusCq->getCqsDisConnectedCount());    
  ASSERT(myStatusCq->getCqsDisConnectedCount() == disconnect, "incorrect number of CqStatus Disconnected count.");
}

DUNIT_TASK_DEFINITION(CLIENT1, checkCQStatusOnDisConnect0)
{  
  SLEEP(20000);
  checkCQStatusOnDisConnect(regionName, (char*)cqName, 0);
...
{  
  SLEEP(20000);
  checkCQStatusOnDisConnect("__TEST_POOL1__", (char*)cqName, 1);
  checkCQStatusOnDisConnect("__TEST_POOL2__", (char*)cqName1, 1);
...
{  
  SLEEP(20000);
  checkCQStatusOnDisConnect(regionName, (char*)cqName, 1);
...
{  
  SLEEP(20000);
  checkCQStatusOnDisConnect(regionName, (char*)cqName, 2);
...
void checkCQStatusOnPutEvent(const char* poolName, const char* cqName, int count) {
  PoolPtr pool = PoolManager::find(poolName);
  QueryServicePtr qs;
  if(pool != NULLPTR) {      
    qs = pool->getQueryService();
  ...
  MyCqStatusListener* myStatusCq = dynamic_cast<MyCqStatusListener*>(vl[0].ptr());
  LOGINFO("checkCQStatusOnPutEvent = %d ", myStatusCq->getNumInserts());  
  ASSERT(myStatusCq->getNumInserts() == count, "incorrect number of CqStatus Updates count.");
}
...
{  
  ...
  MyCqStatusListener* myStatusCq = dynamic_cast<MyCqStatusListener*>(vl[1].ptr());
  LOGINFO("No of insert events = %d ", myStatusCq->getNumInserts());  
  LOGINFO("No of OnCqConnected events = %d ", myStatusCq->getCqsConnectedCount());  
  ASSERT(myStatusCq->getNumInserts() == 5, "incorrect number of CqStatus Updates count.");
  ASSERT(myStatusCq->getCqsConnectedCount() == 1, "incorrect number of CqStatus Connected count.");

  MyCqListener* myCq = dynamic_cast<MyCqListener*>(vl[0].ptr());
  LOGINFO("No of insert events = %d ", myCq->getNumInserts());      
  ASSERT(myCq->getNumInserts() == 5, "incorrect number of CqStatus Updates count.");
  ...
  MyCqStatusListener* myStatusCq2 = dynamic_cast<MyCqStatusListener*>(vl3[0].ptr());
  myStatusCq2->clear();
  ...
  LOGINFO("No of OnCqConnected events = %d ", myStatusCq2->getCqsConnectedCount());  
  ASSERT(myStatusCq2->getNumUpdates() == 5, "incorrect number of CqStatus Updates count.");
  ASSERT(myStatusCq2->getCqsConnectedCount() == 0, "incorrect number of CqStatus Connected count.");

  MyCqListener* myCq2 = dynamic_cast<MyCqListener*>(vl2[1].ptr());
  LOGINFO("No of insert events = %d ", myCq2->getNumInserts());      
  ASSERT(myCq2->getNumUpdates() == 5, "incorrect number of CqStatus Updates count.");
  
  LOG( "ProcessCQ complete." );
}
END_TASK_DEFINITION

void doThinClientCqStatus( bool poolConfig = false, bool poolLocators = false )
{
  if (poolConfig && poolLocators) {
    CALL_TASK(CreateLocator);
    CALL_TASK(CreateServer1_Locator);
  ...
  if (poolConfig) {
    if (poolLocators) {
      ...
      CALL_TASK(initCqStatusClientEP);            
    }
  } 
  CALL_TASK(createCQ);
  //CALL_TASK(executeCQ);
...
void doThinClientCqStatus3( bool poolConfig = false, bool poolLocators = false )
{
  if (poolConfig && poolLocators) {
    CALL_TASK(CreateLocator);
    CALL_TASK(CreateServer1_Locator);
...
DUNIT_MAIN
{
  //doThinClientCq(); // normal case: pool == false, locators == false

  UnsetPortfolioTypeC1();
  UnsetPortfolioTypeC2();
  for(int runIdx = 1; runIdx <=2; ++runIdx)
  {
    doThinClientCq(true); // pool-with-endpoints case: pool == true, locators == false
  
    doThinClientCq(true, true); // pool-with-locator case: pool == true, locators == true
  ...
  {
    doThinClientCqStatus3(true); // pool-with-endpoints case: pool == true, locators == false  
    doThinClientCqStatus3(true, true); // pool-with-locator case: pool == true, locators == true
  }

  {
    doThinClientCqStatus(true); // pool-with-endpoints case: pool == true, locators == false  
    doThinClientCqStatus(true, true); // pool-with-locator case: pool == true, locators == true
  }

  { 
    //supported only with locator, since it involves server group.
    doThinClientCqStatus2(); // pool-with-locator case: pool == true, locators == true
  }
