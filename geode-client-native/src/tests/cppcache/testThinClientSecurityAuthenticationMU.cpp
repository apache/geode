...
{
  printf("hitesh in initclientAuth 0 = %c ", credentialsType);
  ...
  if ( insertAuthInit ) {
    //config->insert( "security-client-auth-factory","createUserPasswordAuthInitInstance" );
    //config->insert( "security-client-auth-library","authinitImpl" );
    credentialGeneratorHandler->getAuthInit(config);
  }

  try {
    LOG("hitesh in initclientAuth");
    initClient(true ,config);
    LOG("hitesh in initclientAuth 2");
  } catch (...) {
      throw ;
  }
}
...
DUNIT_TASK_DEFINITION(LOCATORSERVER, CreateServer1)
{
  ...
  try {
  if ( isLocalServer ) {
    cmdServerAuthenticator = credentialGeneratorHandler->getServerCmdParams("authenticator", getXmlPath());
    printf("Input to server cmd is -->  %s",cmdServerAuthenticator.c_str());
    CacheHelper::initServer( 1,NULL,locHostPort , (char*)cmdServerAuthenticator.c_str());
    LOG("Server1 started");
  }
  } catch (...) {
...
DUNIT_TASK_DEFINITION(LOCATORSERVER, CreateServer2)
{
  ...
  cmdServerAuthenticator2 = credentialGeneratorHandler->getServerCmdParams("authenticator", getXmlPath());
  printf("Input to server cmd is -->  %s",cmdServerAuthenticator2.c_str());
  CacheHelper::initServer( 2,"cacheserver_notify_subscription2.xml",locHostPort , (char*)cmdServerAuthenticator2.c_str());
  LOG("Server2 started");
}
END_TASK_DEFINITION
...
DUNIT_TASK_DEFINITION(CLIENT1, StepTwo)
{
  ...
  try {
    ...
    {      
      ...
      virtualRegion->containsKeyOnServer(gemfire::CacheableKey::create(keys[0])); 
...
DUNIT_TASK_DEFINITION(CLIENT2,StepFive)
{
  ...
  try {
    ...
    LOG("before get");
    CacheableStringPtr checkPtr = dynCast<CacheableStringPtr>( virtualRegion->get( keyPtr) );
...
{  
  ...
  try {
    ...
    CacheableStringPtr checkPtr = dynCast<CacheableStringPtr>( virtualRegion->get( "TxKey") );
    ...
    if(checkPtr != NULLPTR && !strcmp("TxValue", checkPtr->asChar())){
      LOG("checkPtr is not null");
      char buf[1024];
      sprintf( buf, "In net search, get returned %s for key %s", checkPtr->asChar(), "TxKey" );
      LOG( buf );
      }else{
      LOG("checkPtr is NULL");
      }
...
void doThinClientSecurityAuthentication( int testCase)
{
  callSetGlobals(testCase);

  int i;
  //TODO:hitesh PKCS is commented out here
  for(i = 1 ; i <=2 ; i++)
  {
...
DUNIT_MAIN
{
 // doThinClientSecurityAuthentication(1); // normal case: pool == false, locators == false

  doThinClientSecurityAuthentication(2); // pool-with-endpoints case: pool == true, locators == false

  doThinClientSecurityAuthentication(3); // pool-with-locator case: pool == true, locators == true
}
END_MAIN
