...
DUNIT_TASK_DEFINITION(CLIENT1, StepThree)
{
  ...
  QueryPtr qry = qs->newQuery(const_cast<char*>(resultsetQueries[34].query()));
...
DUNIT_TASK_DEFINITION(CLIENT1, StepFive)
{
  ...
  QueryPtr qry = qs->newQuery(const_cast<char*>(structsetQueries[17].query()));
...
DUNIT_TASK_DEFINITION(CLIENT1, StepSeven)
{
  ...
  QueryPtr qry = qs->newQuery(const_cast<char*>(structsetParamQueries[5].query()));
...
void runRemoteQueryTimeoutTest(bool poolConfig = false, bool withLocators = false)
{
  if(!poolConfig){
...
DUNIT_MAIN
{
  ...
  for(int runIdx = 1; runIdx <=2; ++runIdx)
  {
    ...
    runRemoteQueryTimeoutTest(true);

    //New Test with Pool + Locators
    runRemoteQueryTimeoutTest(true,true);
    setPortfolioPdxType();
  }
}
END_MAIN
