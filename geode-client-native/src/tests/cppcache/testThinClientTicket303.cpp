...
void createAuthzRegion()
{
  ...
  initClientAuth('A');
  RegionPtr regPtr = createOverflowRegion( regionNamesAuth[0], false, endPoints, 1);
...
DUNIT_TASK_DEFINITION(CLIENT1,PutAndVerification)
{
  ...
  ASSERT( rptr->containsKey( "invalidkey-1") == false, "Key should not be found in region." );  
  ASSERT( rptr->containsKey( "key-1") == true, "Key key-1 should be found in region." );  
  ASSERT( rptr->containsKey( "key-2") == true, "Key key-2 should be found in region." );  
  ASSERT( rptr->containsKey( "key-3") == true, "Key key-3 should be found in region." );  
