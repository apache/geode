...
int testXmlCacheCreationWithRefid(const char* fileName) {
  ...
  cout
      << "Create cache with the configurations provided in valid_cache_refid.xml"
      << endl;
  ...
  if (atts->getScope() != ScopeType::LOCAL) {
    LOGINFO("Scope of SubRegion11 is not local");
    return -1;
  }
  ...
  if (atts->getScope() != ScopeType::LOCAL) {
    LOGINFO("Scope of SubRegion2 is not local");
    return -1;
  }
  ...
  if (atts->getScope() != ScopeType::LOCAL) {
    LOGINFO("Scope of SubRegion21 is not local");
    return -1;
  }
  ...
  if (atts->getScope() != ScopeType::LOCAL) {
    LOGINFO("Scope of Root2 is not local");
    return -1;
  }
