...
void createAuthzRegion()
{
  ...
  initClientAuth('A');
  RegionPtr rptr;
  createRegion( regionNamesAuth[0], false, endPoints, true);
...
void verifyEntry(const char * value)
{
  ...
  LOG(buf1);   
  ASSERT( strcmp( valuePtr->asChar(), value ) == 0 , "Updated value not found in region." );
}
DUNIT_TASK_DEFINITION(SERVER, StartServer1)
{
  ...
  if ( isLocalServer ) {
    cmdServerAuthenticator = credentialGeneratorHandler->getServerCmdParams("authenticator:dummy", getXmlPath());
