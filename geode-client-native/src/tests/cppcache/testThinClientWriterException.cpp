...
/*
   ...
   2. Check that writer is invoked and listener is not invoked on the client and no data in local region after the failure.
   3. Check if writer fails for any key the cache writer exception is beimg thrown and no data in local region.
...
opCodeList::value_type tmpRArr[] = { OP_GET, OP_REGISTER_INTEREST, OP_UNREGISTER_INTEREST,OP_KEY_SET, OP_CONTAINS_KEY };
...
void initClientAuth()
{
  ...
  printf("User is %s Pass is %s ",config->find("security-username")->asChar(),
    (config->find("security-password")!=NULLPTR?config->find("security-password")->asChar():" not set"));
  try {
...
void startClient(bool poolConfig, bool withLocator)
{
  // Set Global flags
  setGlobals(poolConfig, withLocator);
  ...
  char buf[100];
  int i = 102;
  LOG( "Creating region in READER_CLIENT , no-ack, no-cache, with-listener and writer" );
  regListener = new TallyListener();
  createRegionForSecurity( regionNamesAuth[0], false, endPoints, true,regListener);
...
DUNIT_TASK_DEFINITION(READER_CLIENT, StartClient)
{
  startClient(false, false);
}
END_TASK_DEFINITION

DUNIT_TASK_DEFINITION(READER_CLIENT, StartClientPoolEP)
{
...
void doThinClientWriterException( bool poolConfig = false, bool poolLocators = false )
{
  for (int i = 1; i <= 2; ++i) {
    CALL_TASK(StartLocator);
    CALL_TASK(StartServer1);
    if(!poolConfig){
      CALL_TASK(StartClient);
    }else if(poolLocators){
      CALL_TASK(StartClientPoolLocator);
    }else{
      CALL_TASK(StartClientPoolEP);
    }
...
{
  //TODO:hitesh
  //doThinClientWriterException(); // normal case: pool == false, locators == false

  doThinClientWriterException(true); // pool-with-endpoints case: pool == true, locators == false

  doThinClientWriterException(true, true); // pool-with-locator case: pool == true, locators == true
}
END_MAIN
