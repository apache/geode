...
opCodeList::value_type tmpRArr[] = { OP_GET, OP_GETALL, OP_REGISTER_INTEREST, OP_UNREGISTER_INTEREST,OP_KEY_SET, OP_CONTAINS_KEY, OP_QUERY, OP_REGISTER_CQ };
...
// This putThread class is used in testThinClientTracking,testThinClientTicket304, testThinClientTicket317
...
class putThread : public ACE_Task_Base
{
  public:
    ...
    void setParams(int opcode, int numofops, int numthreads,bool isCallBack =  false,bool sameKey =false, int waitTime = 0) {//
      m_opcode = opcode;
      m_numops = numofops;
      ...
      while (ops++ < m_numops) {
        ...
        if (m_opcode==0) {
          ...
          try {
            ...
            printf("%d: %d exception got and exception message = %s\n", pid, thr_id, ex.getMessage());
          }
        }
      }
      return 0;
