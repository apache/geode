...
// qRegionNames[0],qRegionNames[1],qRegionNames[2] talk to Pool1 and qRegionNames[3] to Pool2
DUNIT_TASK_DEFINITION(CLIENT1, StepOnePoolServer)
{
  try {
    Serializable::registerType(Position::createDeserializable);
    Serializable::registerType(Portfolio::createDeserializable);
  ...
  createPool(poolNames[1], NULL, NULL,endPoints, 0, true );
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOnePoolLocator)
{
  ...
  createPool(poolNames[1], locHostPort, NULL,NULL, 0, true );
  createRegionAndAttachPool( qRegionNames[3], USE_ACK, poolNames[1]);
...
DUNIT_TASK_DEFINITION(CLIENT1, StepTwo)
{
  ...
  if(!m_isPdx){
    ...
    qh->populatePositionData(subregPtr0, qh->getPositionSetSize()  , qh->getPositionNumSets());
    qh->populatePositionData(regPtr1   , qh->getPositionSetSize()  , qh->getPositionNumSets());

    qh->populatePortfolioData(regPtr2  , qh->getPortfolioSetSize(), qh->getPortfolioNumSets());
    ...
    qh->populatePositionPdxData(subregPtr0, qh->getPositionSetSize()  , qh->getPositionNumSets());
    qh->populatePositionPdxData(regPtr1   , qh->getPositionSetSize()  , qh->getPositionNumSets());

    qh->populatePortfolioPdxData(regPtr2  , qh->getPortfolioSetSize(), qh->getPortfolioNumSets());
...
DUNIT_TASK_DEFINITION(CLIENT1, StepThree)
{
  ...
  for ( int i = 0; i < QueryStrings::RQsize(); i++)
  {
    if (m_isPdx) {
      if (i == 18) {
        LOGINFO("Skipping query index %d because it is unsupported for pdx type.", i);
        continue;
      }
    }
    ...
    SelectResultsPtr results = region->query(const_cast<char*>(regionQueries[i].query()));

    if( results->size() != regionQueryRowCounts[i] )
    {
      char failmsg[100] = {0};
      ACE_OS::sprintf(failmsg, "FAIL: Query # %d expected result size is %d, actual is %d", i, regionQueryRowCounts[i], results->size());
  ...
  try
    ...
    {
      LOG("got expected IllegalArgumentException exception for empty predicate:");
      LOG(ex.getMessage());
    }
  ...
  try
  {
    SelectResultsPtr results = region->query(const_cast<char*>(regionQueries[0].query()), 2200000);
    FAIL("Expected IllegalArgumentException exception for invalid timeout");
  }
  catch(gemfire::IllegalArgumentException ex)
  {
    LOG("got expected IllegalArgumentException exception for invalid timeout:");
    LOG(ex.getMessage());
  }

  try
  {
    SelectResultsPtr results = region->query(const_cast<char*>(regionQueries[0].query()), -1);
    FAIL("Expected IllegalArgumentException exception for invalid timeout");
  }
  catch(gemfire::IllegalArgumentException ex)
  {
...
DUNIT_TASK_DEFINITION(CLIENT1, StepFour)
{
  ...
  for ( int i = 0; i < QueryStrings::RQsize(); i++)
  {
    ...
    bool existsValue = region->existsValue(const_cast<char*>(regionQueries[i].query()));
    ...
    {
      char failmsg[100] = {0};
      ACE_OS::sprintf(failmsg, "FAIL: Query # %d existsValue expected is %s, actual is %s", i, expectedResult?"true":"false", existsValue?"true":"false");
      doAnyErrorOccured = true;
  ...
  try
  {
    bool existsValue = region->existsValue(const_cast<char*>(regionQueries[0].query()), 2200000);
    FAIL("Expected IllegalArgumentException exception for invalid timeout");
  }
  catch(gemfire::IllegalArgumentException ex)
  {
  ...
  try
  {
    bool existsValue = region->existsValue(const_cast<char*>(regionQueries[0].query()), -1);
    FAIL("Expected IllegalArgumentException exception for invalid timeout");
  }
  catch(gemfire::IllegalArgumentException ex)
  {
...
DUNIT_TASK_DEFINITION(CLIENT1, StepFive)
{
  ...
  for ( int i = 0; i < QueryStrings::RQsize(); i++)
  {
    ...
    try
    {
      SerializablePtr result = region->selectValue(const_cast<char*>(regionQueries[i].query()));

/*
      if (result == NULLPTR)
      {
        char logmsg[100] = {0};
        ACE_OS::sprintf(logmsg, "Query # %d query selectValue result is NULL", i);
        LOG(logmsg);
      }
      else
      {
        char logmsg[100] = {0};
        ACE_OS::sprintf(logmsg, "Query # %d query selectValue result size is not NULL", i);
        LOG(logmsg);
      }
*/
      ...
      {
        char logmsg[100] = {0};
        ACE_OS::sprintf(logmsg, "FAIL: Query # %d expected query exception did not occur", i);
        LOG(logmsg);
        doAnyErrorOccured = true;
      }
    }
    catch (const QueryException &)
    {
      ...
      {
        char logmsg[100] = {0};
        ACE_OS::sprintf(logmsg, "FAIL: Query # %d unexpected query exception occured", i);
        LOG(logmsg);
        doAnyErrorOccured = true;
      }
    }
    ...
    {
      char logmsg[100] = {0};
      ACE_OS::sprintf(logmsg, "FAIL: Query # %d unexpected exception occurred", i);
      LOG(logmsg);
      FAIL(logmsg);
    }
  }
  ...
  try
  {
    SelectResultsPtr results = region->selectValue(const_cast<char*>(regionQueries[0].query()), 2200000);
    FAIL("Expected IllegalArgumentException exception for invalid timeout");
  }
  catch(gemfire::IllegalArgumentException ex)
  {
  ...
  try
  {
    SelectResultsPtr results = region->selectValue(const_cast<char*>(regionQueries[0].query()), -1);
    FAIL("Expected IllegalArgumentException exception for invalid timeout");
  }
  catch(gemfire::IllegalArgumentException ex)
  {
...
DUNIT_TASK_DEFINITION(CLIENT1,QueryError)
{
  ...
  for ( int i = 0; i < QueryStrings::RQsize(); i++)
  {
   if ((regionQueries[i].category != unsupported) || (i == 3)) //UNDEFINED case
    {
      continue;
    }
...
void runRemoteRegionQueryTest(bool poolConfig = false, bool withLocators = false)
{
  if(!poolConfig){
...
DUNIT_MAIN
{
  ...
  {
    //New Test with pool + endpoints
    runRemoteRegionQueryTest(true);

    //New Test with pool + locators
    runRemoteRegionQueryTest(true,true);
    setPortfolioPdxType();
  }
}
END_MAIN
