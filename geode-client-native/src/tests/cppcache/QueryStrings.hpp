...
namespace testData {
  ...
  const int RSOPL_ARRAY_SIZE = 5; // result set sizes for queries returned via ObjectPartList
  const int SS_ARRAY_SIZE = 36;
  const int SSP_ARRAY_SIZE = 17;
  const int SSOPL_ARRAY_SIZE = 4; // struct set sizes for queries returned via ObjectPartList
  const int RQ_ARRAY_SIZE = 21; // for region.query() API queries
  const int CQRS_ARRAY_SIZE = 24;

  const int MAX_QRY_LENGTH = 512;
  enum queryCategory { singleRegion = 0, multiRegion, operators, constants, functions, collectionOps, keywords, 
                       regionInterface, nestedQueries, importAndSelect, canonicalization, unsupported, 
                       queryAndIndexing, misc, regionQuery, qcEnd/*to mark the end of enum*/ } qcType;
  
  class QueryStrings {

    public:
      QueryStrings(queryCategory pcategory, const char * pquery, bool pisLargeResultset = false) {
        int querylen = static_cast<int> (strlen(pquery));
        if( querylen < MAX_QRY_LENGTH ) 
          memcpy( _query, pquery, querylen );
...
# define QRY(category,query) \
  QueryStrings(category, query)

//for large resultset
# define LQRY(category,query) \
  QueryStrings(category, query, true)
...
, QRY(singleRegion, "select distinct pkid from /Portfolios")
  ...
  // 2 idx
,LQRY(multiRegion , "select distinct port from /Portfolios port, port.Positions.values")
, QRY(multiRegion , "select distinct pf from /Portfolios pf , pf.positions.values pos where pos.getSecId = 'IBM' and NOT pf.isActive")
,LQRY(multiRegion , "Select distinct pf from /Portfolios pf, /Portfolios2, /Portfolios3 where pf.isActive = TRUE")
,LQRY(multiRegion , "select distinct pos from /Portfolios, /Portfolios/Positions pos")

  // 6 idx
, QRY(operators   , "select distinct * from /Portfolios where ID =  2")
, QRY(operators   , "select distinct * from /Portfolios where ID != 2")
,LQRY(multiRegion   , "select distinct port from /Portfolios port , /Positions pos where pos.secId in (select distinct pos.secId from port.positions.values pos)")
...
, QRY(constants   , "select distinct port from /Portfolios port, port.Positions.values where FALSE")
, QRY(constants   , "select distinct port from /Portfolios port, port.Positions.values where UNDEFINED = 1")
, QRY(constants   , "select distinct port from /Portfolios port where IS_UNDEFINED(UNDEFINED)")

  // 19 idx
, QRY(functions   , "select distinct * from /Portfolios port where IS_DEFINED(TRUE)")
, QRY(functions   , "select distinct * from /Portfolios port where IS_UNDEFINED(ID)")
,LQRY(functions   , "select distinct this.value from /Portfolios.entrySet this where element(select distinct * from /Portfolios.keySet p_k where p_k = this.key ) = 'port1-3' and this.value.status = 'inactive'")
, QRY(functions   , "select distinct nvl(NULL, 'foundNull') from /Portfolios")
//, QRY(functions   , "select distinct nvl('notNull', 'foundNull')  = 'notNull' from /Portfolios" )
, QRY(functions   , "select distinct nvl('notNull', 'foundNull') from /Portfolios" )
, QRY(functions   , "select distinct * from /Portfolios pf where nvl(pf.position2,'foundNull') = 'foundNull'")
, QRY(functions   , "select distinct nvl(pf.position2, 'inProjection') from /Portfolios pf where nvl(pf.position2,'foundNull') = 'foundNull'") //constant

  // 26 idx
, QRY(regionInterface, "select distinct * from /Portfolios.keys where toString = 'port1-1'") //constant
, QRY(regionInterface, "select distinct * from /Portfolios.values where ID = 1")
, QRY(regionInterface, "select distinct value from /Portfolios.entries where value.ID = 1 and key = 'port1-1'") //constant
,LQRY(regionInterface, "select distinct * from /Portfolios.keySet where toString = 'port1-1'") //constant
,LQRY(regionInterface, "select distinct key from /Portfolios.entrySet , value.positions.values   where value.ID = 1 and key = 'port1-1'") //constant

  // 31 idx
, QRY(nestedQueries  , "select distinct * from /Portfolios WHERE NOT(SELECT DISTINCT * FROM positions.values p WHERE p.secId = 'IBM').isEmpty")
,LQRY(nestedQueries  , "select distinct x.value from /Portfolios.entrySet x where x.value.status = ELEMENT(select distinct p.value from /Portfolios.entrySet p where x.key = p.key).status")
,LQRY(nestedQueries  , "select distinct * from /Portfolios x where status = ELEMENT(select distinct p_k.value from /Portfolios.entrySet p_k where p_k.key = 'port1-1').status")
,LQRY(nestedQueries  , "select distinct p from /Portfolios p, (select distinct * from /Portfolios x, x.positions as myPos) where myPos.value.secId = 'YHOO'")
...
" select distinct secId FROM /Portfolios,  positions.values pos TYPE Position WHERE ID > 0 ")
  ...
  // 36 idx
, QRY(unsupported , "element(select distinct * from /Portfolios where ID <=3).status")
, QRY(unsupported , "select distinct * from /Portfolios.subregions(false) where remove('5')!= null")
//, QRY(unsupported, "element(select distinct * from /Portfolios where ID =1).status")
//, QRY(unsupported, "select distinct * from NULL")
//, QRY(unsupported, "select distinct * from UNDEFINED")

,LQRY(misc        , "select distinct * from /Portfolios WHERE NOT (select distinct * from positions.values p WHERE p.secId = 'IBM').isEmpty")
, QRY(misc        , "select distinct to_date('21/10/07','dd/mm/yy') from /Portfolios")
...
//, QRY(constants, "(select * from /Portfolios).size")
 ...
 const int resultsetRowCounts[RS_ARRAY_SIZE] = {  20,20,20,1 ,10,20,1,19,20,0 ,
                                                  0 ,0 ,0 ,20,20,0 ,0,0 ,20,20,
                                                  0 ,1 ,1 ,1 ,10,1 ,1,1 ,1 ,1 ,
                                                  1 ,2 ,20,10,20,10,0,0 ,
                                                  2 ,1
                                               };

 const int constantExpectedRowsRS[12] = { 1, 21, 22, 23, 25, 26, 28, 29, 30, 35, 39, 42 };
...
// 5 idx
,LQRY(functions   , "select distinct this.value from /Portfolios.entrySet this where element(select distinct * from /Portfolios.keySet p_k where p_k = this.key ) = $1 and this.value.status = $2")
, QRY(functions   , "select distinct * from /Portfolios pf where nvl(pf.position2,$1) = $2")
, QRY(functions   , "select distinct nvl(pf.position2, $1) from /Portfolios pf where nvl(pf.position2,$2) = $3") 

// 8 idx
, QRY(regionInterface, "select distinct * from /Portfolios.keys where toString = $1") 
, QRY(regionInterface, "select distinct * from /Portfolios.values where ID = $1") 
, QRY(regionInterface, "select distinct value from /Portfolios.entries where value.ID = $1 and key = $2") 
,LQRY(regionInterface, "select distinct * from /Portfolios.keySet where toString = $1") 
,LQRY(regionInterface, "select distinct key from /Portfolios.entrySet , value.positions.values   where value.ID = $1 and key = $2") 
}; 
...
const int resultsetRowCountsPQ[RSP_ARRAY_SIZE] = {  1, 19, 20, 0, 0, 1, 10, 1, 1, 1, 1, 1, 1 }; 

const int constantExpectedRowsPQRS[6] = { 5, 7, 8, 10, 11, 12};

const QueryStrings resultsetQueriesOPL [RSOPL_ARRAY_SIZE] = { // queries returning result sets via ObjectPartList
  // 0 idx
...
const QueryStrings structsetQueries[SS_ARRAY_SIZE] = { 
  //0 idx
  QRY(singleRegion, "select distinct ID, pkid, position1, position2, getType, status from /Portfolios")
                     
  //1 idx
, QRY(multiRegion , "select distinct ptf,pos from (select distinct * from /Portfolios ptf, positions.values pos) outp where outp.pos.secId = 'IBM'")
, QRY(multiRegion , "select distinct ptf,pos from (select distinct * from ptf IN /Portfolios, pos IN positions.values) where pos.secId = 'IBM'")              
//to be enabled, QRY(multiRegion , "select distinct * from (select distinct ptf.ID, pos AS myPos from /Portfolios ptf, positions.values pos) ")
/*to be removed */, QRY(multiRegion , "select distinct myPos, ID from (select distinct pos AS myPos, ID: ptf.ID from /Portfolios ptf, positions.values pos) ")
// need to see later , QRY(multiRegion , "select distinct * from (select distinct * from /Portfolios ptf, positions pos) p where p.get('pos').value.secId = 'IBM'")
, QRY(multiRegion , "select distinct secId, getSharesOutstanding from /Positions") 

  //5 idx
, QRY(constants   , "select distinct id, positions from /Portfolios where ID = NULL")
, QRY(constants   , "select distinct pkid, getPk() from /Portfolios where ID = UNDEFINED")
, QRY(constants   , "select distinct getPositions, getPositions from /Portfolios where TRUE")
, QRY(constants   , "select distinct id, status from /Portfolios where UNDEFINED")
...
, QRY(misc        , "select distinct to_date('01/19/2038 03:848 AM','mm/dd/yyyy hh:sss') as twoeighty, to_date('01/19/2038','mm/dd/yyyy'), to_date('12/14/1901 00:2753 AM','mm/dd/yyyy hh:ssss') from /Portfolios")
#endif
}; //end of structsetQueries

 const int structsetRowCounts[SS_ARRAY_SIZE] = {    20,2 ,2 ,20,20,0 ,0 ,20,0 ,20,
                                                    20,0 ,20,0 ,20,20,0 ,10,1 ,1 ,
                                                    10,1 ,20,9 ,1 ,1 ,20,20,2 ,20,
                                                    20,20,0 ,1, 1, 1
                                               };

 const int structsetFieldCounts[SS_ARRAY_SIZE] = {  6 ,2 ,2 ,2, 2 ,0 ,0 ,2 ,0 ,4 ,
                                                    3 ,0 ,3 ,0 ,3 ,2 ,0 ,3 ,2 ,2 ,
                                                    3 ,2 ,4 ,4 ,3 ,5 ,3 ,5 ,3 ,2 ,
                                                    2 ,2 ,0 ,3, 3 ,3
                                                 };

 const int constantExpectedRowsSS[17] = { 4, 10, 12, 13, 14, 15, 17, 18, 19, 20, 21, 23, 24, 25, 27, 30, 35}; 

const QueryStrings structsetParamQueries[SSP_ARRAY_SIZE] = {
//0 idx
QRY(multiRegion , "select distinct ptf,pos from (select distinct * from /Portfolios ptf, positions.values pos) outp where outp.pos.secId = $1") 

//1 idx
, QRY(multiRegion , "select distinct ptf,pos from (select distinct * from ptf IN /Portfolios, pos IN positions.values) where pos.secId = $1")
, QRY(constants   , "select distinct ID, pkid, getType from /Portfolios where $1 = $2")
, QRY(constants   , "select distinct ID, pkid, names from /Portfolios where $1 <> $2")

//4 idx
, QRY(constants   , "select distinct port.ID, port.pkid, port.names from /Portfolios port, port.Positions.values where UNDEFINED = $1")
,LQRY(functions   , "select distinct oP.ID, oP.status, oP.getType from /Portfolios oP where element(select distinct p.ID, p.status, p.getType from /Portfolios p where p.ID = oP.ID).status = $1")
, QRY(functions   , "select distinct nvl(NULL, $1)=$2, 'dummy Col' from /Portfolios" )
, QRY(functions   , "select distinct nvl($1, $2) as NtNul , $3 as Dumbo from /Portfolios" )
, QRY(functions   , "select distinct pf.ID, pf.status, pf.getType from /Portfolios pf where nvl(pf.position2,$1) = $2")
, QRY(functions   , "select distinct nvl(pf.position2, $1), $2 from /Portfolios pf where nvl(pf.position2, $3) = $4")

//10 idx
,LQRY(regionInterface, "select distinct ID, pkid, names, position2.secId from /Portfolios.values where nvl(position2, $1) != $1 and ID > $2")
, QRY(regionInterface, "select distinct key, value.ID, value.pkid from /Portfolios.entries where value.ID = $1 and key in Set($2)")
,LQRY(regionInterface, "select distinct key, secType, secId, underlyer, cnt from /Portfolios.entrySet , value.positions.values   where value.ID = $1 and key = $2")
,LQRY(regionInterface, "select distinct k, value.ID, value from /Portfolios.keySet k, (select distinct value from /Portfolios.entrySet) value where k.toString = $1")

//14 idx
,LQRY(nestedQueries, "select distinct derivedProjAttrbts, key: p.key, id: p.value.ID  from /Portfolios.entries p, (select distinct x.ID, x.status, x.getType, myPos.secId from /Portfolios x, x.positions.values as myPos) derivedProjAttrbts where p.value.ID = derivedProjAttrbts.ID and derivedProjAttrbts.secId = $1")
, QRY(unsupported , "select element(select distinct * from /Portfolios where ID <= $1).status = $2")
, LQRY(misc        , "select distinct key: key, iD: entry.value.ID, secId: posnVal.secId  from /Portfolios.entries entry, entry.value.positions.values posnVal WHERE entry.value.getType = $1 AND posnVal.secId = $2")
}; 

const int numSSQueryParam[] = {1, 1, 2, 2, 1, 1, 2, 3, 2, 4, 2, 2, 2, 1, 1, 2, 2};
...
const int structsetRowCountsPQ[SSP_ARRAY_SIZE] = { 2, 2, 20, 0, 0, 10, 1, 1, 10, 1, 9 ,1 ,1 ,20, 2, 0, 1 };

const int structsetFieldCountsPQ[SSP_ARRAY_SIZE] = { 2, 2, 3, 0, 0, 3, 2, 2, 3, 2, 4, 3, 5, 3, 3, 0, 3};

const int constantExpectedRowsSSPQ[10] = { 2, 4, 5, 6, 7, 8, 9, 10, 11, 12};


const QueryStrings regionQueries[RQ_ARRAY_SIZE] = { // intended to be run on the /Portfolios region.query API
// 0 idx
  QRY(regionQuery, "ID = 2")
, QRY(regionQuery, "ID != 2")
, QRY(regionQuery, "ID = NULL")
