...
{
  if(!update){
    createEntry( poolRegNames[0], keys[keyIndex], newValue? nvals[keyIndex]: vals[keyIndex] );
    createEntry( poolRegNames[1], keys[keyIndex], newValue? nvals[keyIndex]: vals[keyIndex] );
    createEntry( poolRegNames[2], keys[keyIndex], newValue? nvals[keyIndex]: vals[keyIndex] );
  }else {
    updateEntry( poolRegNames[0], keys[keyIndex], newValue? nvals[keyIndex]: vals[keyIndex] );
    updateEntry( poolRegNames[1], keys[keyIndex], newValue? nvals[keyIndex]: vals[keyIndex] );
    updateEntry( poolRegNames[2], keys[keyIndex], newValue? nvals[keyIndex]: vals[keyIndex] );
  }
}

void verifyEntries(int keyIndex, bool netSearch = false, bool newValue = false)
{
  if(!netSearch){
    verifyEntry( poolRegNames[0], keys[keyIndex], newValue? nvals[keyIndex]: vals[keyIndex] );
    verifyEntry( poolRegNames[1], keys[keyIndex], newValue? nvals[keyIndex]: vals[keyIndex] );
    verifyEntry( poolRegNames[2], keys[keyIndex], newValue? nvals[keyIndex]: vals[keyIndex] );
  }else {
    doNetsearch( poolRegNames[0], keys[keyIndex], newValue? nvals[keyIndex]: vals[keyIndex] );
    doNetsearch( poolRegNames[1], keys[keyIndex], newValue? nvals[keyIndex]: vals[keyIndex] );
    doNetsearch( poolRegNames[2], keys[keyIndex], newValue? nvals[keyIndex]: vals[keyIndex] );
  }
}
...
DUNIT_TASK_DEFINITION(CLIENT1, CreateClient1_1)
{
  ...
  createPool(poolNames[0], locHostPort,sGNames[0], NULL, 2, true );
  createRegionAndAttachPool( poolRegNames[0], USE_ACK, poolNames[0], true );

  createPool(poolNames[1], locHostPort,sGNames[0], NULL, 1, true );
  createRegionAndAttachPool( poolRegNames[1], USE_ACK, poolNames[1], true );

  createPool(poolNames[2], locHostPort,sGNames[0], NULL, 0, true );
...
DUNIT_TASK_DEFINITION(CLIENT2, CreateClient2_1)
{
  ...
  // create three regions with three pools ( each having diff redun )
  createPool(poolNames[0], locHostPort, NULL, NULL, 2 , true);
  createRegionAndAttachPool( poolRegNames[0], USE_ACK, poolNames[0], true );

  createPool(poolNames[1], locHostPort, NULL, NULL, 1 , true);
  createRegionAndAttachPool( poolRegNames[1], USE_ACK, poolNames[1], true );

  createPool(poolNames[2], locHostPort, NULL, NULL, 0 , true);
  createRegionAndAttachPool( poolRegNames[2], USE_ACK, poolNames[2], true );
...
DUNIT_TASK_DEFINITION(CLIENT1, CreateClient1_2)
{
  ...
  createPool(poolNames[1], locHostPort,sGNames[1], NULL, 1 , true);
  createRegionAndAttachPool( poolRegNames[1], USE_ACK, poolNames[1], true );

  createPool(poolNames[2], locHostPort,sGNames[2], NULL, 0 , true);
  createRegionAndAttachPool( poolRegNames[2], USE_ACK, poolNames[2], true );
