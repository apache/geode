...
public:
   ...
   void close( const RegionPtr& region )
   {
     ...
     if(region != NULLPTR){
       LOGINFO(" Region %s is Destroyed = %d ", region->getName(), region->isDestroyed());
       ASSERT(region->isDestroyed() == true, "region.isDestroyed should return true");
       /*
       if(region.ptr() != NULL && region.ptr()->getCache() != NULLPTR){
         LOGINFO(" Cache Name is Closed = %d ",  region.ptr()->getCache()->isClosed());
       }else{
...
DUNIT_TASK_DEFINITION(CLIENT1, SetupClient)
{
  ...
  dSysPtr = DistributedSystem::connect("ExampleDistributedSystem");
  ...
  cachePtr = CacheFactory::create("ExampleCache", dSysPtr, clientXml.c_str());
...
DUNIT_TASK_DEFINITION(CLIENT1, testLoader)
{
  ...
  ASSERT( !regionPtr->containsKey( keyPtr ), "Key should not have been found in region." );
...
DUNIT_TASK_DEFINITION(CLIENT1, testInvalidateKey)
{
  ...
  //Verify the sequence put()->invalidate()->get()->invalidate()->get() :- CacheLoader to be invoked twice
  //once after each get.
...
DUNIT_TASK_DEFINITION(CLIENT1, testInvalidateRegion)
{
  ...
  //Verify the sequence invalidateRegion()->get() :- CacheLoader to be invoked.
  ...
  //Verify the sequence put()->invalidateRegion()->get()->invalidateRegion()->get() :- CacheLoader to be invoked twice.
  //once after each get.
  CacheableKeyPtr keyPtr4 = CacheableKey::create("Key4");
  regionPtr->put(keyPtr4, "Value4");
  regionPtr->invalidateRegion();
