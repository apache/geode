...
const char * regionNames[] = { "DistRegionAck", "DistRegionNoAck", "ExampleRegion", "SubRegion1", "SubRegion2"  };
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOne_Pool_Locator)
{
  ...
  createPooledRegion( regionNames[2], NO_ACK, NULL, locatorsG, "__TESTPOOL1_", true );
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOne_Pool_EndPoint)
{
  ...
  LOGINFO("NIL: CLIENT1 StepOne_Pool_EndPoint subregions created successfully");
...
DUNIT_TASK_DEFINITION(CLIENT2, StepTwo_Pool_Locator)
{
  ...
  LOGINFO("NIL: CLIENT2 StepTwo_Pool_Locator:: subregions created successfully");
...
DUNIT_TASK_DEFINITION(CLIENT2, StepTwo_Pool_EndPoint)
{
  ...
  LOGINFO("NIL: CLIENT2 StepTwo_Pool_EndPoint:: subregions created successfully");
  LOG( "StepTwo_Pool complete." );
}
END_TASK_DEFINITION
...
{
  LOGINFO("NIL:LINE_537 eventListener1->m_totalEvents = %d " , eventListener1->m_totalEvents);
  ASSERT( eventListener1->m_totalEvents == 0, "Region Event count must be zero" );
}
END_TASK_DEFINITION

DUNIT_TASK_DEFINITION(CLIENT1, verifyEventsForDestroyedSubregion)
{
  LOGINFO("NIL:LINE_543 eventListener2->m_totalEvents = %d " , eventListener2->m_totalEvents);
  ASSERT( eventListener2->m_totalEvents == 0, "Subregion Event count must be zero" );
}
END_TASK_DEFINITION
...
DUNIT_TASK_DEFINITION(CLIENT1, VerifySubRegionOps)
{
  ...
  for (int index=0; index < 5; index++){
    ...
    //Need to check in cliet/server logs that client-1 do not receive any notification for subregPtr0
  }
}
END_TASK_DEFINITION
...
void testRegisterKeyForLocalRegionDestroy(bool poolConfig, bool isLocator = true)
{
...
void testRegisterAllKeysForLocalRegionDestroy(bool poolConfig, bool isLocator = true)
{
  if( poolConfig && isLocator )
  {
    CALL_TASK(CreateLocator1);
...
void testRegisterRegexForLocalRegionDestroy(bool poolConfig, bool isLocator = true)
{
  if( poolConfig && isLocator )
  {
    CALL_TASK(CreateLocator1);
...
void testSubregionForLocalRegionDestroy(bool poolConfig, bool isLocator = true)
{
  if( poolConfig && isLocator )
  {
    CALL_TASK(CreateLocator1);
