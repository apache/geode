...
void initClientAuth( char credentialsType)
{
  ...
  switch(credentialsType) {
    case 'C' :
     ...
     printf("Username is %s and Password is %s ",config->find("security-username")->asChar(),config->find("security-password")->asChar());
      break;
    case 'I' :
...
DUNIT_TASK_DEFINITION(CLIENT2, StepSeven)
{
  ...
  SLEEP(500);
  try {
    createRegionForSecurity( regionNamesAuth[0], USE_ACK, endPoints, true, NULLPTR, true, 0);
    FAIL( "Should have thrown AuthenticationFailedException." );
  } catch ( const gemfire::AuthenticationFailedException& other ) {
    other.printStackTrace();
    LOG( other.getMessage() );
...
void doThinClientSecurityAuthentication( int testCase)
{
  ...
  for(i = 1 ; i <=3 ; i++)
  {
    ...
    {
      CALL_TASK(StepEight); //Tx is only supported with pool.
    }
    CALL_TASK(CloseCache1);
    CALL_TASK(CloseCache2);
