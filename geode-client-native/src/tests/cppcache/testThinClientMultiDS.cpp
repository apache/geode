...
void startServer(int instance, const char *xmlfile, const char* lochostport)
{
  ...
  CacheHelper::initServer( instance,xmlfile,lochostport, (char*)cmdServerAuthenticator.c_str(),false,true,true);
  LOG("Server started");
}

DUNIT_TASK_DEFINITION(SERVER1, CreateServer1)
{
  CacheHelper::initServer( 1, "cacheserver_notify_subscription.xml",NULL,NULL,false,true,true );
  CacheHelper::initServer( 2, "cacheserver_notify_subscription2.xml" ,NULL,NULL,false,true,true);
...
{
  CacheHelper::initServer( 3, "cacheserver_notify_subscription3.xml",NULL,NULL,false,true,true );
  CacheHelper::initServer( 4, "cacheserver_notify_subscription4.xml",NULL,NULL,false,true,true );
  LOG("SERVER2 started");
}
END_TASK_DEFINITION
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOne_Pool_EndPoint)
{
  try {
    ...
    //getHelper()->createPoolWithEPs("__TESTPOOL1_", "localhost:24680,localhost:24681");
    //getHelper()->createPoolWithEPs("__TESTPOOL2_", "localhost:24682,localhost:24683");
    char tmp[128];
    sprintf(tmp, "localhost:%d,localhost:%d", CacheHelper::staticHostPort1, CacheHelper::staticHostPort2);
    getHelper()->createPoolWithEPs("__TESTPOOL1_", tmp);
    char tmp2[128];
    sprintf(tmp2, "localhost:%d,localhost:%d", CacheHelper::staticHostPort3, CacheHelper::staticHostPort4);
    getHelper()->createPoolWithEPs("__TESTPOOL2_", tmp2);

    getHelper()->createRegionAndAttachPool(regionNames[0], USE_ACK, "__TESTPOOL1_", false);
    getHelper()->createRegionAndAttachPool(regionNames[1], NO_ACK, "__TESTPOOL2_", false);
...
{
  try {
    //getHelper()->createPoolWithEPs("__TESTPOOL1_", "localhost:24680,localhost:24681",false);
    //getHelper()->createPoolWithEPs("__TESTPOOL2_", "localhost:24682,localhost:24683",false);
    char tmp[128];
    sprintf(tmp, "localhost:%d,localhost:%d", CacheHelper::staticHostPort1, CacheHelper::staticHostPort2);
    getHelper()->createPoolWithEPs("__TESTPOOL1_", tmp);
    char tmp2[128];
...
try {
  RegionPtr regPtr0 = getHelper()->getRegion(regionNames[0]);
  RegionPtr regPtr1 = getHelper()->getRegion(regionNames[1]);
  CacheableStringPtr checkPtr = dynCast<CacheableStringPtr>( regPtr0->get( keys[2] ) );
  ASSERT(checkPtr == NULLPTR, "checkPtr should be null");
  CacheableStringPtr checkPtr1 = dynCast<CacheableStringPtr>( regPtr0->get( keys[0] ) );
