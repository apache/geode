...
class DisconnectCacheListioner : public CacheListener
{
  ...
  DisconnectCacheListioner(int index) { m_index = index; };
...
void createPooledRegionMine(bool callReadyForEventsAPI = false)
{
  ...
  if((PoolManager::find( "__TEST_POOL1__" ))== NULLPTR ) {// Pool does not exist with the same name.
    PoolPtr pptr = poolFacPtr->create("__TEST_POOL1__");
  }
  SLEEP(10000);
  AttributesFactory af;
  af.setScope( ScopeType::DISTRIBUTED_NO_ACK );
  ...
  RegionPtr region1 = getHelper()->cachePtr->createRegion( regionNames[0], rattrsPtr1 );
  RegionPtr region2 = getHelper()->cachePtr->createRegion( regionNames[1], rattrsPtr2 );
...
{
  SLEEP(10000);
  ASSERT(!isRegionLive[0], "Client should not have gotten region1 live event");
  ASSERT(!isRegionLive[1], "Client should not have gotten region2 live event");
  ASSERT(!isRegionLive[2], "Client should not have gotten subregion1 live event");
  ASSERT(!isRegionLive[3], "Client should not have gotten subregion2 live event");
}
END_TASK_DEFINITION
...
{
  SLEEP(10000);
  ASSERT(isRegionDead[0], "Client should have gotten region1 disconnected event");
  isRegionDead[0] = false;
  ASSERT(isRegionDead[1], "Client should have gotten region2 disconnected event");
  isRegionDead[1] = false;
  ASSERT(isRegionDead[2], "Client should have gotten subregion1 disconnected event");
  isRegionDead[2] = false;
  ASSERT(isRegionDead[3], "Client should have gotten subregion2 disconnected event");
...
{
  SLEEP(10000);
  ASSERT(!isRegionDead[0], "Client should not have gotten region1 disconnected event");
  ASSERT(!isRegionDead[1], "Client should not have gotten region2 disconnected event");
  ASSERT(!isRegionDead[2], "Client should not have gotten subregion1 disconnected event");
  ASSERT(!isRegionDead[3], "Client should not have gotten subregion2 disconnected event");
 }
END_TASK_DEFINITION
