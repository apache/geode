...
void initGridClient( const bool isthinClient, const PropertiesPtr& configPtr = NULLPTR )
{
...
void initClient( const bool isthinClient, const PropertiesPtr& configPtr = NULLPTR )
{
  if (cacheHelper == NULL) {
...
void initClientWithPool( const bool isthinClient, const char* poolName, const char* locators, const char* serverGroup,
    const char* servers = NULL, const PropertiesPtr& configPtr = NULLPTR, int redundancy = 0, bool clientNotification = false, int subscriptionAckInterval = -1,
                      int connections = -1, int loadConditioningInterval = - 1, bool prSingleHop = false, bool threadLocal = false) {
  if (cacheHelper == NULL) {
      cacheHelper = new CacheHelper(isthinClient, poolName, locators, serverGroup, servers, configPtr, redundancy, clientNotification, subscriptionAckInterval, connections, loadConditioningInterval, false, prSingleHop, threadLocal);
    }
  ASSERT(cacheHelper, "Failed to create a CacheHelper client instance.");
...
/* For HA Clients */
void initClient(const char* endPoints, int redundancyLevel,
    const PropertiesPtr& configPtr = NULLPTR)
{
  if (cacheHelper == NULL) {
    ...
    cacheHelper = new CacheHelper(endPoints, redundancyLevel, config);
  }
  ASSERT(cacheHelper, "Failed to create a CacheHelper client instance.");
}

void initGridClient(const char* endPoints, int redundancyLevel,
    const PropertiesPtr& configPtr = NULLPTR)
{
  static bool s_isGridClient = true;
...
const char * testregex[] = { "Key-*1", "Key-*2", "Key-*3", "Key-*4" , "Key-*5" , "Key-*6"};
const char * keys[] = { "Key-1", "Key-2", "Key-3", "Key-4" ,"Key-5" , "Key-6"};
const char * vals[] = { "Value-1", "Value-2", "Value-3", "Value-4" , "Value-5" , "Value-6"};
const char * nvals[] = { "New Value-1", "New Value-2", "New Value-3", "New Value-4" , "New Value-5" , "New Value-6"};
...
void _verifyEntry( const char * name, const char * key, const char * val, bool noKey ,bool checkVal = true)
{
...
void verifyEntry( const char * name, const char * key, const char * val, bool checkVal=true )
{
  char logmsg[1024];
...
void createRegion( const char * name, bool ackMode, const char * endpoints,
    ...
    const CacheListenerPtr& listener = NULLPTR, bool caching = true)
{
  ...
  RegionPtr regPtr = getHelper()->createRegion(name, ackMode, caching,
      listener, endpoints, clientNotificationEnabled);
  ASSERT(regPtr != NULLPTR, "Failed to create region.");
  LOG( "Region created." );
}
 RegionPtr createOverflowRegion(const char * name, bool ackMode, const char * endpoints,int lel = 0,
     bool clientNotificationEnabled = true,bool caching = true)
 {
   std::string bdb_dir = "BDB";
   std::string bdb_dirEnv = "BDBEnv";
   AttributesFactory af;
   af.setScope( ackMode ? ScopeType::DISTRIBUTED_ACK : ScopeType::DISTRIBUTED_NO_ACK );
   ...
   bdbProperties->insert("PersistenceDirectory",bdb_dir.c_str());
   bdbProperties->insert("EnvironmentDirectory",bdb_dirEnv.c_str());
   ...
   return cache->createRegion( name, rattrsPtr );
 }
RegionPtr createPooledRegion( const char * name, bool ackMode, const char * endpoints, const char * locators, const char* poolname,
    bool clientNotificationEnabled = false,
    const CacheListenerPtr& listener = NULLPTR, bool caching = true)
{
  ...
  // ack, caching
  RegionPtr regPtr = getHelper()->createPooledRegion(name, ackMode, endpoints, locators,poolname,caching, clientNotificationEnabled,  0, 0,  0, 0, 0, listener);
...
PoolPtr createPool( const char* poolName, const char* locators, const char* serverGroup,
    const char* servers = NULL, int redundancy = 0, bool clientNotification = false, int subscriptionAckInterval = -1,
                      int connections = -1, int loadConditioningInterval = - 1)
{
  LOG( "createPool() entered." );

  PoolPtr poolPtr = getHelper()->createPool(poolName,locators,serverGroup,servers,redundancy, clientNotification, subscriptionAckInterval, connections, loadConditioningInterval);
  ASSERT(poolPtr != NULLPTR, "Failed to create pool.");
  LOG( "Pool created." );
  return poolPtr;
}

PoolPtr createPoolAndDestroy( const char* poolName, const char* locators, const char* serverGroup,
    const char* servers = NULL, int redundancy = 0, bool clientNotification = false, int subscriptionAckInterval = -1,
    int connections = -1)
{
  LOG( "createPoolAndDestroy() entered." );

  PoolPtr poolPtr = getHelper()->createPool(poolName,locators,serverGroup,servers,redundancy, clientNotification,subscriptionAckInterval,connections);
  ASSERT(poolPtr != NULLPTR, "Failed to create pool.");
  poolPtr->destroy();
...
PoolPtr createPool2( const char* poolName, const char* locators, const char* serverGroup,
    const char* servers = NULL, int redundancy = 0, bool clientNotification = false)
{
  LOG( "createPool2() entered." );

  PoolPtr poolPtr = getHelper()->createPool2(poolName,locators,serverGroup,servers,redundancy, clientNotification);
  ASSERT(poolPtr != NULLPTR, "Failed to create pool.");
  LOG( "Pool created." );
  return poolPtr;
}

RegionPtr createRegionAndAttachPool(const char* name, bool ack, const char* poolName, bool caching = true,
                                    int ettl = 0, int eit = 0, int rttl = 0, int rit = 0,int lel = 0,
                                    ExpirationAction::Action action = ExpirationAction::DESTROY       )
{
  LOG( "createRegion() entered." );
  RegionPtr regPtr = getHelper()->createRegionAndAttachPool(name,ack,poolName,caching,
...
void doNetsearch( const char * name, const char * key, const char * value, bool checkVal = true )
{
...
void createIntEntry( const char * name, const char * key, const int value, bool onlyCreate = false )
{
  LOG( "createEntry() entered." );
...
class RegionOperations
{
  public:
    RegionOperations( const char* name )
       ...
       void invalidateOp(int keys = 1, const UserDataPtr& aCallbackArgument = NULLPTR)
       {
         char keybuf[100];
         char valbuf[100];
         for(int i = 1; i <= keys ; i++) {
