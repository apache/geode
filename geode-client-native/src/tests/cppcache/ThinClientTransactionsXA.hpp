...
void createRegion( const char * name, bool ackMode, const char * endpoints ,bool clientNotificationEnabled = false, bool cachingEnable = true)
{
  LOG( "createRegion() entered." );
  fprintf( stdout, "Creating region --  %s  ackMode is %d\n", name, ackMode );
  fflush( stdout );
  RegionPtr regPtr = getHelper()->createRegion( name, ackMode, cachingEnable, NULLPTR, endpoints, clientNotificationEnabled );
...
void createEntryTwice(const char * name, const char * key, const char * value)
{
  ...
  ASSERT(false, "Creating key twice is not allowed and while doing that exception was not thrown");
...
const char * keys[] = { "Key-1", "Key-2", "Key-3", "Key-4", "Key-5","Key-6","Key-7"};
const char * vals[] = { "Value-1", "Value-2", "Value-3", "Value-4", "Value-5", "Value-6", "Value-7"};
const char * nvals[] = { "New Value-1", "New Value-2", "New Value-3", "New Value-4" , "New Value-5" , "New Value-6" , "New Value-7"};
...
#define THREADERRORCHECK( x, y)  if (!(x)){  \
      m_isFailed = true; \
      sprintf(m_error, y);  \
       return -1; \
    }
...
public:
  SuspendTransactionThread(bool sleep, ACE_Auto_Event* txEvent) :
    m_sleep (sleep), m_suspendedTransaction (NULLPTR),m_txEvent(txEvent){ }
  ...
  int svc(void)
  {
    ...
    InternalCacheTransactionManager2PCPtr txManager = static_cast<InternalCacheTransactionManager2PCPtr>(getHelper()->getCache()->getCacheTransactionManager());
  ...
  TransactionIdPtr getSuspendedTx() 
  {
...
public:
  ResumeTransactionThread(TransactionIdPtr suspendedTransaction, bool commit, bool tryResumeWithSleep, 
    ACE_Auto_Event* txEvent):
    m_suspendedTransaction(suspendedTransaction), m_commit(commit), m_tryResumeWithSleep(tryResumeWithSleep),
    m_isFailed(false), m_txEvent(txEvent){ }
  ...
  int svc(void)
  {
    ...
    THREADERRORCHECK( regPtr0 != NULLPTR, "In ResumeTransactionThread - Region not found." );
    ...
    THREADERRORCHECK( regPtr1 != NULLPTR, "In ResumeTransactionThread - Region not found." );
    
    THREADERRORCHECK( !regPtr0->containsKeyOnServer( keyPtr4 ), "In ResumeTransactionThread - Key should not have been found in region." );
    
    THREADERRORCHECK( !regPtr1->containsKeyOnServer( keyPtr5 ), "In ResumeTransactionThread - Key should not have been found in region." );
    
    InternalCacheTransactionManager2PCPtr txManager = static_cast<InternalCacheTransactionManager2PCPtr>(getHelper()->getCache()->getCacheTransactionManager());
    if (m_tryResumeWithSleep)
    {
      THREADERRORCHECK(!txManager->isSuspended(m_suspendedTransaction), "In ResumeTransactionThread - the transaction should NOT be in suspended state");
    }
    else
    {
      THREADERRORCHECK(txManager->isSuspended(m_suspendedTransaction), "In ResumeTransactionThread - the transaction should be in suspended state");
    }

    THREADERRORCHECK(txManager->exists(m_suspendedTransaction), "In ResumeTransactionThread - the transaction should exist");
    ...
    THREADERRORCHECK( regPtr0->containsKeyOnServer( keyPtr4 ), "In ResumeTransactionThread - Key should have been found in region." );
    THREADERRORCHECK( regPtr1->containsKeyOnServer( keyPtr5 ), "In ResumeTransactionThread - Key should have been found in region." );
    ...
    if (m_commit){
      ...
      THREADERRORCHECK( regPtr0->containsKeyOnServer( keyPtr4 ), "In ResumeTransactionThread - Key should have been found in region." );
      THREADERRORCHECK( regPtr1->containsKeyOnServer( keyPtr5 ), "In ResumeTransactionThread - Key should have been found in region." );
      THREADERRORCHECK( regPtr1->containsKeyOnServer( keyPtr6 ), "In ResumeTransactionThread - Key should have been found in region." );
      ...
      THREADERRORCHECK( !regPtr0->containsKeyOnServer( keyPtr4 ), "In ResumeTransactionThread - Key should not have been found in region." );
      THREADERRORCHECK( !regPtr1->containsKeyOnServer( keyPtr5 ), "In ResumeTransactionThread - Key should not have been found in region." );
      THREADERRORCHECK( !regPtr1->containsKeyOnServer( keyPtr6 ), "In ResumeTransactionThread - Key should not have been found in region." );
    }
...
DUNIT_TASK_DEFINITION(CLIENT1,SuspendResumeCommit)
{
  ...
  ASSERT( !regPtr0->containsKeyOnServer( keyPtr4 ), "In SuspendResumeCommit - Key should not have been found in region." );
  ASSERT( !regPtr1->containsKeyOnServer( keyPtr5 ), "In SuspendResumeCommit - Key should not have been found in region." );
  
  ASSERT(txManager->isSuspended(m_suspendedTransaction), "In SuspendResumeCommit - the transaction should be in suspended state");
  ASSERT(txManager->exists(m_suspendedTransaction), "In SuspendResumeCommit - the transaction should exist");
  
  
  txManager->resume(m_suspendedTransaction);
  
  ASSERT(!txManager->tryResume(m_suspendedTransaction), "SuspendResumeRollback: Transaction shouldnt have been resumed again");
  ASSERT( regPtr0->containsKeyOnServer( keyPtr4 ), "In SuspendResumeCommit - Key should have been found in region." );
  ASSERT( regPtr1->containsKeyOnServer( keyPtr5 ), "In SuspendResumeCommit - Key should have been found in region." );
  ...
  ASSERT( regPtr0->containsKeyOnServer( keyPtr4), "In SuspendResumeCommit - Key should have been found in region." );
  ASSERT( regPtr1->containsKeyOnServer( keyPtr5 ), "In SuspendResumeCommit - Key should have been found in region." );
  ASSERT( regPtr1->containsKeyOnServer( keyPtr6 ), "In SuspendResumeCommit - Key should have been found in region." );
  
  regPtr1->destroy(keyPtr6);
  regPtr1->destroy(keyPtr5);
  regPtr0->destroy(keyPtr4);

  ASSERT(!txManager->isSuspended(m_suspendedTransaction), "In SuspendResumeCommit the transaction should NOT present");
  ASSERT(!txManager->exists(m_suspendedTransaction), "In SuspendResumeCommit - the transaction should NOT exist");
  ASSERT(!txManager->tryResume(m_suspendedTransaction), "In SuspendResumeCommit - the transaction should NOT have been resumed");
  ...
  ASSERT(resumeExc, "SuspendResumeCommit: Transaction shouldnt have been resumed");
  
  ASSERT(txManager->suspend() == NULLPTR, "SuspendResumeCommit: Transaction shouldnt have been suspended");
}
END_TASK_DEFINITION
...
DUNIT_TASK_DEFINITION(CLIENT1,SuspendTimeOut) 
{
  ...
  ASSERT(txManager->isSuspended(tid2), "In SuspendTimeOut the transaction should be present");
  ASSERT(txManager->exists(tid2), "In SuspendTimeOut - the transaction should exist");
  
  ACE_OS::sleep(65);
  ASSERT(!txManager->tryResume(tid2), "In SuspendTimeOut - the transaction should NOT have been resumed");
  ASSERT(!txManager->isSuspended(tid2), "In SuspendTimeOut the transaction should NOT present");
  ASSERT(!txManager->exists(tid2), "In SuspendTimeOut - the transaction should NOT exist");
  ASSERT( regPtr0->containsKeyOnServer( keyPtr4 ), "In SuspendTimeOut - Key should have been found in region." );
  ASSERT( !regPtr0->containsKeyOnServer( keyPtr5 ), "In SuspendTimeOut - Key should not have been found in region." );
...
DUNIT_TASK_DEFINITION(CLIENT1,SuspendResumeRollback)
{
  ...
  ASSERT( !regPtr0->containsKeyOnServer( keyPtr4 ), "In SuspendResumeRollback - Key should not have been found in region." );
  ASSERT( !regPtr1->containsKeyOnServer( keyPtr5 ), "In SuspendResumeRollback - Key should not have been found in region." );

  ASSERT(txManager->isSuspended(m_suspendedTransaction), "In SuspendResumeRollback the transaction should be in suspended state");
  ASSERT(txManager->exists(m_suspendedTransaction), "In SuspendResumeRollback - the transaction should exist");

  txManager->resume(m_suspendedTransaction);

  ASSERT( regPtr0->containsKeyOnServer( keyPtr4 ), "In SuspendResumeRollback - Key should have been found in region." );
  ASSERT( regPtr1->containsKeyOnServer( keyPtr5 ), "In SuspendResumeRollback - Key should have been found in region." );
  ...
  ASSERT( !regPtr0->containsKeyOnServer( keyPtr4 ), "In SuspendResumeRollback - Key should not have been found in region." );
  ASSERT( !regPtr1->containsKeyOnServer( keyPtr5 ), "In SuspendResumeRollback - Key should not have been found in region." );
  ASSERT( !regPtr1->containsKeyOnServer( keyPtr6 ), "In SuspendResumeRollback - Key should not have been found in region." );
  ...
  ASSERT(!txManager->isSuspended(m_suspendedTransaction), "In SuspendResumeRollback the transaction should NOT present");
  ASSERT(!txManager->exists(m_suspendedTransaction), "In SuspendResumeRollback - the transaction should NOT exist");
  ASSERT(!txManager->tryResume(m_suspendedTransaction), "In SuspendResumeRollback - the transaction should NOT have been resumed");
  bool resumeExc = false;
  try {
    txManager->resume(m_suspendedTransaction);
  }
  ...
  ASSERT(resumeExc, "SuspendResumeRollback: Transaction shouldnt have been resumed");
}
END_TASK_DEFINITION
DUNIT_TASK_DEFINITION(CLIENT1,SuspendResumeInThread)
{
  ...
  char buf[1024];
  sprintf( buf, "start suspend thread  and resume thread and rollback immedidately");
  LOG( buf );
  ACE_Auto_Event txEvent;
    
  SuspendTransactionThread *suspendTh = new SuspendTransactionThread(false, &txEvent);
  suspendTh->activate();
  ACE_OS::sleep(2);
  ResumeTransactionThread *resumeTh = new ResumeTransactionThread(suspendTh->getSuspendedTx(), false, false, &txEvent);
  resumeTh->activate();
  ...
  sprintf( buf, "start suspend thread  and resume thread and commit immedidately");
  LOG( buf );
  suspendTh = new SuspendTransactionThread(false, &txEvent);
  suspendTh->activate();
  ACE_OS::sleep(2);
  resumeTh = new ResumeTransactionThread(suspendTh->getSuspendedTx(), true, false, &txEvent);
  resumeTh->activate();
  ...
  // start suspend thread  and tryresume thread with rollback. make tryResume to sleep
  sprintf( buf, "start suspend thread  and tryresume thread with rollback. make tryResume to sleep");
  LOG( buf );
  suspendTh = new SuspendTransactionThread(true, &txEvent);
  suspendTh->activate();
  ACE_OS::sleep(2);
  resumeTh = new ResumeTransactionThread(suspendTh->getSuspendedTx(), false, true, &txEvent);
  resumeTh->activate();
  ...
  // start suspend thread  and tryresume thread with commit. make tryResume to sleep
  sprintf( buf, "start suspend thread  and tryresume thread with commit. make tryResume to sleep");
  LOG( buf );
  suspendTh = new SuspendTransactionThread(true, &txEvent);
  suspendTh->activate();
  ACE_OS::sleep(2);
  ...
  resumeTh = new ResumeTransactionThread(suspendTh->getSuspendedTx(), true, true, &txEvent);
  resumeTh->activate();
...
END_TASK_DEFINITION
DUNIT_TASK_DEFINITION(CLIENT1, CreateNonexistentServerRegion)
{
  initClient( true );
  createRegion( "non-region", USE_ACK, endPoints );
  try {
    createEntry( "non-region", keys[0], vals[0] );
    FAIL( "Expected exception when doing operations on a non-existent region." );
  } catch ( const CacheServerException& ex ) {
    printf( "Got expected CacheServerException when performing operation "
...
{
  initClient( true );
  createPooledRegion( "non-region", USE_ACK, NULL, locatorsG, "__TESTPOOL1_" );
  try {
    createEntry( "non-region", keys[0], vals[0] );
    FAIL( "Expected exception when doing operations on a non-existent region." );
  } catch ( const CacheServerException& ex ) {
...
DUNIT_TASK_DEFINITION(CLIENT1, CreateNonexistentServerRegion_Pooled_Locator_Sticky)
{
  initClient( true );
  createPooledRegionSticky( "non-region", USE_ACK, NULL, locatorsG, "__TESTPOOL1_" );
  try {
    createEntry( "non-region", keys[0], vals[0] );
    FAIL( "Expected exception when doing operations on a non-existent region." );
  } catch ( const CacheServerException& ex ) {
...
DUNIT_TASK_DEFINITION(CLIENT1, CreateNonexistentServerRegion_Pooled_EndPoint)
{
  initClient( true );
  createPooledRegion( "non-region", USE_ACK, endPoints, NULL, "__TESTPOOL1_" );
  try {
    createEntry( "non-region", keys[0], vals[0] );
    FAIL( "Expected exception when doing operations on a non-existent region." );
  } catch ( const CacheServerException& ex ) {
...
DUNIT_TASK_DEFINITION(CLIENT1, CreateNonexistentServerRegion_Pooled_EndPoint_Sticky)
{
  initClient( true );
  createPooledRegionSticky( "non-region", USE_ACK, endPoints, NULL, "__TESTPOOL1_" );
  try {
    createEntry( "non-region", keys[0], vals[0] );
    FAIL( "Expected exception when doing operations on a non-existent region." );
  } catch ( const CacheServerException& ex ) {
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOne_Pooled_Locator)
{
  ...
  createPooledRegion( regionNames[1], NO_ACK, NULL, locatorsG, "__TESTPOOL1_" );
...
{
  createPooledRegionSticky( regionNames[0], USE_ACK, NULL, locatorsG, "__TESTPOOL1_" );
  createPooledRegionSticky( regionNames[1], NO_ACK, NULL, locatorsG, "__TESTPOOL1_" );
...
{
  createPooledRegion( regionNames[0], USE_ACK, endPoints, NULL, "__TESTPOOL1_" );
  createPooledRegion( regionNames[1], NO_ACK, endPoints, NULL, "__TESTPOOL1_" );
  LOG( "StepOne_Pooled complete." );
}
END_TASK_DEFINITION
...
{
  createPooledRegionSticky( regionNames[0], USE_ACK, endPoints, NULL, "__TESTPOOL1_" );
  createPooledRegionSticky( regionNames[1], NO_ACK, endPoints, NULL, "__TESTPOOL1_" );
  LOG( "StepOne_Pooled complete." );
}
END_TASK_DEFINITION
...
{
  createRegion( regionNames[2], NO_ACK, endPoints, false, false);
  ...
  {
    LOG(gemfireExcp.getMessage( ));
    ASSERT(false, "Creating KEY Twice on a caching-enabled false region should be allowed.");
  }
}
END_TASK_DEFINITION
...
{
  createPooledRegion( regionNames[2], NO_ACK, NULL, locatorsG, "__TESTPOOL1_", false, false);
  RegionPtr reg = getHelper( )->getRegion( regionNames[2] );
  LOG("REGION Created with Caching Enabled false");
  CacheableKeyPtr keyPtr = createKey( CREATE_TWICE_KEY );
  CacheableStringPtr valPtr = CacheableString::create( CREATE_TWICE_VALUE );
...
{
  createPooledRegionSticky( regionNames[2], NO_ACK, NULL, locatorsG, "__TESTPOOL1_", false, false);
  RegionPtr reg = getHelper( )->getRegion( regionNames[2] );
  LOG("REGION Created with Caching Enabled false");
  CacheableKeyPtr keyPtr = createKey( CREATE_TWICE_KEY );
  CacheableStringPtr valPtr = CacheableString::create( CREATE_TWICE_VALUE );
...
void runTransactionOps( bool poolConfig = true, bool isLocator = true, bool isSticky = false) {
  ...
  if( !poolConfig )
  {
    CALL_TASK( CreateNonexistentServerRegion );
    CALL_TASK( StepOne );
    CALL_TASK( StepTwo );
  }
  else if( isLocator )
  ...
  else
  {
    if( isSticky )
    {
      CALL_TASK( CreateNonexistentServerRegion_Pooled_EndPoint_Sticky );
      CALL_TASK( StepOne_Pooled_EndPoint_Sticky );
      CALL_TASK( StepTwo_Pooled_EndPoint_Sticky );
    }
    else
    {
      CALL_TASK( CreateNonexistentServerRegion_Pooled_EndPoint );
