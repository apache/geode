...
typedef gemfire::SharedPtr< TallyListener >
   TallyListenerPtr;
...
class TallyListener : public CacheListener {
  ...
  void ignoreTimeouts( bool ignore )
  {
    m_ignoreTimeout = ignore;
  }
 ...
 int expectInvalidates( int expected )
 {
  ...
  {
    return isListnerInvoked;
  }
  void setCallBackArg(const CacheableKeyPtr& callbackArg)
  {
    m_callbackArg = callbackArg;
  }
  CacheableKeyPtr getLastKey( )
  {
    return m_lastKey;
  }

  CacheablePtr getLastValue( )
  {
    return m_lastValue;
  }
  bool isCallBackArgCalled()
  {
  ...
  {
    if (!isListnerInvoked)
      isListnerInvoked = true;
    if (m_callbackArg != NULLPTR) {
      CacheableKeyPtr callbkArg = dynCast<CacheableKeyPtr>(event.getCallbackArgument());
      if (strcmp(m_callbackArg->toString()->asChar(), callbkArg->toString()->asChar())==0)
  ...
  {
    char buf[1024];
    sprintf( buf, "TallyListener state: (updates = %d, creates = %d , invalidates = %d destroys = %d Regionclears = %d)",
  getUpdates(), getCreates(), getInvalidates(), getDestroys(), getClears() );
    LOG( buf );
  }
