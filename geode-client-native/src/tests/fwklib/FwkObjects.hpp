...
// ----------------------------------------------------------------------------
    ...
    class ActionPair {
      ...
      void setLibraryFunctionName( std::string name ) { m_libraryFunctionName = name; }
...
// ----------------------------------------------------------------------------
    ...
    class ExpiryAttributes {
      ...
      void setTimeout( std::string str ) {
        m_timeout = FwkStrCvt::toInt32( str );
      }
...
// ----------------------------------------------------------------------------
    ...
    class Attributes {
      ...
      void setScope( std::string val ) {
        if ( val == "local" ) {
          m_isLocal = true;
          m_factory.setScope( ScopeType::LOCAL );
        }
        else if ( val == "distributed-ack" ) {
          m_factory.setScope( ScopeType::DISTRIBUTED_ACK );
        }
        else if ( val == "distributed-no-ack" ) {
          m_factory.setScope( ScopeType::DISTRIBUTED_NO_ACK );
        }
      }
      ...
      void setClientNotificationEnabled( std::string val ) {
        m_factory.setClientNotificationEnabled( FwkStrCvt::toBool( val ) );
      }
      ...
      void setCacheLoader( ActionPair * val ) {
        m_factory.setCacheLoader( val->getLibraryName(), val->getLibraryFunctionName() );
      }

      void setCacheListener( ActionPair * val ) {
        m_factory.setCacheListener( val->getLibraryName(), val->getLibraryFunctionName() );
      }

      void setCacheWriter( ActionPair * val ) {
        m_factory.setCacheWriter( val->getLibraryName(), val->getLibraryFunctionName() );
      }
      ...
      void setPersistenceManager( PersistManager * val ) {
         m_factory.setPersistenceManager( val->getLibraryName(),
            val->getLibraryFunctionName(), val->getProperties() );
      }
      ...
      void setEndpoints( std::string val ) {
        m_factory.setEndpoints(val.c_str());
      }
...
// ----------------------------------------------------------------------------
    ...
    class FwkRegion {
      ...
      const RegionAttributesPtr getAttributesPtr() const {return m_attributes->getAttributes();}
      void print() const {
        FWKINFO( "FwkRegion " << m_name );
      }
    };
    ...
    class FwkPool {
      ...
      void setSubscriptionMessageTrackingTimeout(std::string val) {
        m_poolFactory->setSubscriptionMessageTrackingTimeout(FwkStrCvt::toInt32( val ));
      }
      ...
      void setLocatorsFlag(std::string val) {
        m_locators = FwkStrCvt::toBool( val );
      }

      void setServersFlag(std::string val) {
        m_servers = FwkStrCvt::toBool( val );
      }
    public:
...
//        ACE_thread_t maxIdx = static_cast<ACE_thread_t> (m_values.size()) - 1;
//        if ( maxIdx < 0 ) {
//          return;
//        }
...
// ----------------------------------------------------------------------------
    ...
    class FwkClient : public FwkObject
    {
      ...
      FwkClient( std::string name )
        : m_program( NULL ), m_arguments( NULL ), m_remaining( false ) { setName( name ); }
        ...
        void print() const {
          FWKINFO( "Client: " << getName() );
        }

        void setProgram( std::string str ) {
          if ( !str.empty() )
            m_program = strdup( str.c_str() );
        }

        void setArguments( std::string str ) {
          if ( !str.empty() )
            m_arguments = strdup( str.c_str() );
        }

        void setHost( std::string str ) {
          if ( ( m_host.empty() ) && ( !str.empty() ) )
            m_host = str;
        }
    ...
    /**
      ...
      */
      class FwkClientSet : public TFwkSet<FwkClient> {
        ...
        void setCount( std::string str ) {
          m_count = FwkStrCvt::toInt32( str );
        }

        void setBegin( std::string str ) {
          m_begin = FwkStrCvt::toInt32( str );
        }
        ...
        void setExclude( std::string str ) {
          m_exclude = FwkStrCvt::toBool( str );
        }

        void setHostGroup( const std::string str ) {
          m_hostGroup = str;
        }
        ...
        void setRemaining( bool remaining ) { m_remaining = remaining; }
        void setRemaining( std::string str ) {
          m_remaining = FwkStrCvt::toBool( str );
        }
        bool getRemaining() const { return m_remaining; }

        void print() const {
          FWKINFO( "ClientSet exclude: " << getExclude() << "  count: "
            << getCount() << "  begin: " << getBegin() );
...
class FwkTest;
    ...
    class FwkTask : public FwkObject
    {
      public:
        ...
        /** @brief Get number of times to run task */
        void setTimesToRun( uint32_t cnt ) {
          m_timesToRun = cnt;
        }
        ...
        void setParallel( std::string str ) {
          m_parallel = FwkStrCvt::toBool( str );
        }
...
class TestDriver;
    ...
    class FwkTest : public FwkObject
    {
      ...
      FwkTest( const DOMNode * node );
        ...
        const FwkTask * getFirst()
        {
          return m_taskSet.getFirst();
        }
        ...
        const FwkTask * find( const char * name )
        {
          return m_taskSet.find( name );
        }
        ...
        void print() const {
          FWKINFO( "Test: " << m_id << "  " << m_description << "  WaitTime: "
            << m_waitTime );
          FWKINFO( "Tasks: " );
          m_taskSet.print();
...
// ----------------------------------------------------------------------------
    ...
    class LocalFile : public FwkObject {
      ...
      void print() const {
        FWKINFO( "LocalFile: " << getName() << " append: " << getAppend() << " has content: " << getContent() );
      }
    };
...
// ----------------------------------------------------------------------------
    ...
    class TestDriver {
      ...
      void addLocalFile( LocalFile * file ) {
        m_localFiles.add( file );
      }
      ...
      void writeLocalFile( const LocalFile * fil ) {
        ...
        FWKDEBUG( "Opened local file: " << fil->getName()
          << " to " << what << " content::>" << fil->getContent() << "<::" );
...
//        fclose( out );
      ...
      const FwkTest * firstTest() const {
        return m_tests.getFirst();
      }
      ...
      const LocalFile * firstLocalFile() const {
        return m_localFiles.getFirst();
      }
