...
void gemfire::testframework::plog( const char * l, const char * s, const char* filename, int32_t lineno )
{
	//ACE_TCHAR tstamp[64];
	  ...
	  pbuf += ACE_OS::snprintf(pbuf , 15 , ".%06ld ", clock.usec());
	  ...
	  fprintf(stdout, "[%s %s %s:P%d:T%u]::%s::%d  %s  %s\n", buf,
	    u.sysname, u.nodename, ACE_OS::getpid(), ( ACE_Thread::self() ),
	    fil, lineno, l, s );
	  fflush(stdout);
}
