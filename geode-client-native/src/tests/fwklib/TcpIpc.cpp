...
int32_t TcpIpc::getSize( int32_t sock, int32_t flag ) {
  ...
  if ( 0 != getsockopt( sock, SOL_SOCKET, flag, param, &plen ) ) {
    FWKSEVERE( "Failed to get buff size for flag " << flag << " on socket.  Errno: " << errno );
  }
#ifdef _LINUX
  val /= 2;
#endif
...
int32_t TcpIpc::readBuffer( char ** buffer, int32_t waitSecs ) {
  ...
  buffs.iov_base = NULL;
  buffs.iov_len = 0;
  int32_t red = (int32_t)m_io->recvv( &buffs, &wtime );
  if ( ( red == -1 ) && ( ( errno == ECONNRESET ) || ( errno == EPIPE ) ) ) {
    FWKEXCEPTION( "During attempt to read: Connection failure errno: " << errno );
  }
  if ( red == -1 ) {
  }
  *buffer = ( char * )( buffs.iov_base );
  return buffs.iov_len;
}

int32_t TcpIpc::sendBuffers( int32_t cnt, char * buffers[], int32_t lengths[], int32_t waitSecs ) {
  ACE_Time_Value wtime( waitSecs );
  int32_t tot = 0;
  if ( cnt > 2 ) {
...
#else
    buffs[idx].iov_base = ( char * )buffers[idx];
#endif
    buffs[idx].iov_len = lengths[idx];
    tot += lengths[idx];
  }
  int32_t wrote = (int32_t)m_io->sendv( buffs, cnt, &wtime );
  if ( ( wrote == -1 ) && ( ( errno == ECONNRESET ) || ( errno == EPIPE ) ) ) {
    FWKEXCEPTION( "During attempt to write: Connection failure errno: " << errno );
  }
  if ( tot != wrote ) {
    FWKSEVERE( "Failed to write all bytes attempted, wrote " << wrote << ", attempted " << tot );
  }
  return wrote;
}
