...
class UDPMessage : public IPCMessage {
  ...
  bool needToAck() {
    return ( m_hdr.cmd == ACK_REQUEST );
  }
  
  const char * cmdString( uint32_t cmd ) {
    const char * UdpStrings[] = {
      "Null",
      "ACK",
      "ACK_REQUEST",
      "ADDR_REQUEST",
...
class UDPMessageQueues : public SharedTaskObject {
  ...
  ~UDPMessageQueues() {
    FWKINFO( m_label << "MessageQueues::Inbound   count: " << m_cntInbound.value() );
    FWKINFO( m_label << "MessageQueues::Processed count: " << m_cntProcessed.value() );
    FWKINFO( m_label << "MessageQueues::Outbound  count: " << m_cntOutbound.value() );
    FWKINFO( m_label << "MessageQueues::Inbound  still queued: " << m_inbound.size() );
    FWKINFO( m_label << "MessageQueues::Outbound still queued: " << m_outbound.size() );
  }
  ...
  UDPMessage * getInbound() { 
    return m_inbound.dequeue();
  }
...
class MigrateVMs : public ServiceTask {
  private:
  UDPMessageQueues * m_queues;
  char *m_username;
  char *m_passwd;
  ...
  public:
  MigrateVMs( UDPMessageQueues * shared, char *username,char *passwd,char *vmhosts,char *vmnames,char *tlimit="10")
    : ServiceTask( shared ),m_username(username),m_passwd(passwd),m_vmhosts(vmhosts),m_vmnames(vmnames),m_tlimit(tlimit){
    m_queues = dynamic_cast< UDPMessageQueues * >( m_shared );
  }
