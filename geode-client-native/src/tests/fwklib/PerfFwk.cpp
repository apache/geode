...
void perf::logSize( const char * tag ) {
  ...
  while ( fgets( rbuff, 1023, fil ) != 0 ) {
    if ( ( ACE_OS::strncasecmp( rbuff, "VmSize:", 7 ) == 0 ) &&
      ...
      if ( ( ACE_OS::strncasecmp( rbuff, "VmRSS:", 6 ) == 0 ) &&
        ( sscanf( rbuff, "%*s %u", &val ) == 1 ) )
          rs = ( double )val;
  }
  fclose( fil );
...
void PerfSuite::addRecord( uint32_t ops, uint32_t micros )
{
  setOperations( ops );
  setMicros( micros );
  m_summary << m_current.asTag() << ", " << m_current.perSec() << ", " 
            << ops << ", " << micros << "\n";
...
//  if ( of == 0 ) {
//    throw new FwkException( "Failed to open result file handle for PerfSuite." );
//  }
//  uint32_t len = 0;
