...
class HRTimer {
  ...
  inline HRTimer() { QueryPerformanceFrequency( &m_freq ); start(); }
  inline void start() { QueryPerformanceCounter( &m_start ); }
  inline int64_t elapsedNanos() {
    QueryPerformanceCounter( &m_stop );
    return ( int64_t )( ( ( ( double )( m_stop.QuadPart - m_start.QuadPart ) * NPS ) / ( double )m_freq.QuadPart ) + .5 );
  }
  inline int64_t elapsedMicros() {
    QueryPerformanceCounter( &m_stop );
    return ( int64_t )( ( ( ( double )( m_stop.QuadPart - m_start.QuadPart ) * MPS ) / ( double )m_freq.QuadPart ) + .5 );
  }
...
// TODO refactor - why not use the ACE_High_Res_Timer for all these platforms?
  ...
  inline void start() { m_timer.reset(); m_timer.start(); }
