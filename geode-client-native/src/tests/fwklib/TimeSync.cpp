...
static const char * byt = ( const char * )&wrd;
...
void TimeSync::sendTimeSync() {
  ...
  if ( sock < 0 ) {
    FWKSEVERE( "Failed to create socket for sending TimeSync messages.  Errno: " << errno );
    return;
  }
  ...
  do {
    ...
    if ( retVal == -1 ) {
      ...
      if ( !m_logged ) {
        m_logged = true;
        FWKINFO( "Sending time sync messages at " << TIME_SYNC_ADDR << ":" << TIME_SYNC_PORT );
      }
    }
...
void TimeSync::recvTimeSync() {
  int32_t sock = (int32_t)socket( AF_INET, SOCK_DGRAM, 0 );
  if ( sock < 0 ) {
    FWKSEVERE( "Failed to create socket for receiving TimeSync messages.  Errno: " << errno );
    return;
  }
  ...
  if ( retVal != 0 ) {
    FWKSEVERE( "Failed to bind to socket for receiving TimeSync messages.  Errno: " << errno );
    return;
  }
  ...
  while ( !m_done ) {
    tval = 0;
#ifdef WIN32  
    retVal = recvfrom( sock, ( char * )&tval, tvLen, 0, ( struct sockaddr * )&sender, &senderLen );
#else
    retVal = recvfrom( sock, ( void * )&tval, tvLen, 0, ( struct sockaddr * )&sender, &senderLen );
#endif    
    switch ( retVal ) {
      case -1: // error!
        if ( ( errno > 0 ) && ( errno != EAGAIN ) ) {
          FWKSEVERE( "Failed while receiving TimeSync message.  Errno: " << errno );
          return;
        }
      break;
      case 8: // process the sync message
      {
        ...
        if ( !m_logged ) {
          m_logged = true;
          FWKINFO( "Receiving time sync messages at " << TIME_SYNC_ADDR << ":" << TIME_SYNC_PORT );
        }
      }
      break;
