...
void FwkBBServer::onSet( FwkBBMessage & message )
{
  ...
  if ( sParameter1.size() && sParameter2.size() && 
    sParameter3.size() && sParameter4.size() ) {
    if ( sParameter1 == "key" ) {
...
void FwkBBServer::clearBB( const std::string & BBName ) {
  ...
  while ( nameKeyIt != m_nameKeyMap.end() ) {
//    FWKINFO( "Looking at: " << nameKeyIt->first.getName() << "  Key: " << nameKeyIt->first.getKey() );
    if ( nameKeyIt->first.getName() == BBName ) {
//    FWKINFO( "Calling erase." );
  ...
  while ( nameCounterIt != m_nameCounterMap.end() ) {
//    FWKINFO( "Looking at: " << nameCounterIt->first.getName() << "  Counter: " << nameCounterIt->first.getKey() );
    if ( nameCounterIt->first.getName() == BBName ) {
//    FWKINFO( "Calling erase." );
...
std::string FwkBBServer::getString( const std::string & BBName, const std::string & Key ) {
  std::string result;
...
void FwkBBServer::set( const std::string & BBName, const std::string & Key, const std::string & Value ) {
  NameKeyPair nameKeyPair( BBName, Key );
...
void FwkBBServer::set( const std::string & BBName, const std::string & Key, const int64_t Value ) {
  NameKeyPair nameKeyPair( BBName, Key );
...
int64_t FwkBBServer::add( const std::string & BBName, const std::string & Key, const int64_t Value ) {
...
int64_t FwkBBServer::subtract( const std::string & BBName, const std::string & Key, const int64_t Value ) {
  return add( BBName, Key, Value * -1 );
}

// ----------------------------------------------------------------------------

int64_t FwkBBServer::increment( const std::string & BBName, const std::string & Key ) {
  return add( BBName, Key, 1 );
}

// ----------------------------------------------------------------------------

int64_t FwkBBServer::decrement( const std::string & BBName, const std::string & Key ) {
  return add( BBName, Key, -1 );
}
...
int64_t FwkBBServer::setIfGreater( const std::string & BBName, const std::string & Key, const int64_t Value ) {
  int64_t result = Value;
  NameKeyPair nameKeyPair( BBName, Key );
...
int64_t FwkBBServer::setIfLess( const std::string & BBName, const std::string & Key, const int64_t Value ) {
  int64_t result = Value;
