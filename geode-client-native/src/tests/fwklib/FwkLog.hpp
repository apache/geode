...
#define FWKDEBUG(x) {std::ostringstream os; os <<  x; \
  plog( "Debug", os.str().c_str(), __FILE__, __LINE__ );}
  
#define FWKSLEEP(x) { ACE_OS::sleep( ACE_Time_Value ( x, 0 ) ); }
...
#define FWKINFO( x ) { std::ostringstream os; os << x; \
  plog( "Info", os.str().c_str(), __FILE__, __LINE__ ); }
#define FWKWARN( x ) { std::ostringstream os; os << x; \
  plog( "Warn", os.str().c_str(), __FILE__, __LINE__ ); }
#define FWKERROR( x ) { std::ostringstream os; os << x; \
  plog( "Error", os.str().c_str(), __FILE__, __LINE__ ); }
#define FWKSEVERE( x ) { std::ostringstream os; os << x; \
...
#define DUMPSTACK( x ) { FWKSEVERE( x ); dumpStack(); }
