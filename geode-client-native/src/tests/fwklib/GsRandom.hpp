...
public:
  ...
  /**
    ...
    *         value from this random number generator's sequence.
    */
  inline uint16_t nextInt16() { return ( uint16_t )singleton->gen.randInt( 0xffff ); }
...
//  }
  ...
  *
  ...
  /**
    * Like randomString(), but returns only alphanumeric, underscore, or space characters.
  *
    * @param max the maximum length of the random string to generate.
    * @return a bounded random string with a length between 0 and
    * max length inclusive.
    */
  char * randomAlphanumericString(int32_t max, int32_t min, const char * prefix = 0);
};
