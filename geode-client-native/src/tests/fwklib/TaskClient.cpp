...
void TaskClient::start() {
  m_busy = false;
  FWKINFO( "Starting client " << m_id << " name: " << m_name << " on " << m_host );
  char buf[1024];
  if ( m_runsTasks ) {
    sprintf( buf, "goHost %s startClient %d %s %s %d\n", 
      m_host, m_id, m_testFile, m_logDirectory, m_port );
    writePipe( buf );
    ...
    if ( m_program == NULL ) {
      ...
      sprintf( buf, "goHost %s startClient %d %s %s %d \"%s\" \"%s\"\n", 
        m_host, m_id, "none", m_logDirectory, 0, m_program, args );
      writePipe( buf );
    }
  }
}
...
void TaskClient::kill() {
  FWKINFO( "Stopping client " << m_id << " name: " << m_name << " on " << m_host );
  char buf[1024];
