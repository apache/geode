...
//
    ...
    inline  std::string perSec() {
      ...
      return oss.str();
//      return (int32_t) ((((double) 1000000 * m_operations) / (double) m_micros ) + 0.5); 
    }
    ...
    inline std::string asCSV( )
    {
      ...
      result << m_testName << "," << m_action << "," << m_numClients <<
        "," << m_numKeys << "," << m_valueSize << "," << 
        m_numThreads << "," << perSec() << "," << m_operations <<
        "," << m_micros << "," << m_runDate;
      return result.str();
    }
};
...
class PerfSuite
{
  ...
  public:
    ...
    std::string asString() {
      return m_current.asTag();
    }
    std::string summary () {
      return m_summary.str();
    }
