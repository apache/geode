...
int32_t IpcHandler::readInt( int32_t waitSeconds ) {
  ...
  int32_t redInt = -1;
  int32_t red = (int32_t)m_io->recv_n( (void *)&redInt, 4, 0, wtime );
  delete wtime;
  if ( red == 4 ) {
    result = ntohl( redInt );
...
std::string IpcHandler::readString( int32_t waitSeconds ) {
  ...
  if ( length == -1 ) {
    FWKEXCEPTION( "Failed to read string, length not available, errno: " << errno );
  }
  ...
  int32_t red = (int32_t)m_io->recv( (void *)buffer, length, 0, wtime );
  delete wtime;
  if ( red <= 0 ) {
    if ( red < 0 ) {
#ifdef WIN32      
...
//  FWKDEBUG( "Sending " << ( int32_t )msg );
  ...
  int32_t wrote = (int32_t)m_io->send( (const void *)&writeInt, 4, &tv );
  if ( wrote == -1 ) {
#ifdef WIN32      
    errno = WSAGetLastError();
#endif      
...
bool IpcHandler::sendBuffer( IpcMsg msg, const char * str ) {
  ...
  char * buffer = checkBuffer( length );
  *( IpcMsg * )buffer = ( IpcMsg )htonl( msg );
  *( int32_t * )( buffer + 4 ) = htonl( length );
  strcpy( ( buffer + 8 ), str );
  
//  FWKDEBUG( "Sending " << ( int32_t )msg << "  and string: " << str );
  length += 8;
  int32_t wrote = (int32_t)m_io->send( (const void *)buffer, length );
  if ( wrote == -1 ) {
#ifdef WIN32      
    errno = WSAGetLastError();
#endif      
