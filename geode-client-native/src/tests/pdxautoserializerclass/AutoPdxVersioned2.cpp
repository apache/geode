...
template <typename T1, typename T2>
bool AutoPdxVersioned2::genericCompare(T1* value1, T2* value2, int length) const
{
  int i = 0;
  while (i < length)
  {
...
template <typename T1, typename T2>
bool AutoPdxVersioned2::generic2DCompare(T1** value1, T2** value2, int length, int* arrLengths) const
{
...
AutoPdxVersioned2::AutoPdxVersioned2(const char* key){
	init(key);
}
...
CacheableStringPtr AutoPdxVersioned2::toString() const {
  ...
  //sprintf(idbuf,"PdxObject testPdxObject:[m_int32=%d] string = %s", m_int32,m_string);
  sprintf(idbuf,"PdxVersioned 2 : %s",m_string);
  return CacheableString::create( idbuf );
}


bool AutoPdxVersioned2::equals(AutoPdxVersioned2& other, bool isPdxReadSerialized)const {
