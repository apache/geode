...
void BatchObject::toData( gemfire::DataOutput& output ) const {
  output.writeInt((int32_t)index);
  output.writeInt((int64_t)timestamp);
  output.writeInt((int32_t)batch);
  output.writeObject(byteArray);
}

gemfire::Serializable* BatchObject::fromData( gemfire::DataInput& input ){
  input.readInt((int32_t*)&index);
  input.readInt((int64_t*)&timestamp);
  input.readInt((int32_t*)&batch);
  input.readObject(byteArray);
  return this;
}
CacheableStringPtr BatchObject::toString() const {
  char buf[102500];
  sprintf(buf, "BatchObject:[index = %d timestamp = %lld batch = %d byteArray=%d ]", index,timestamp,batch,byteArray->length());
