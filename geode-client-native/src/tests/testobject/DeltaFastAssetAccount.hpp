...
public:
  ...
  DeltaFastAssetAccount():encodeTimestamp(0), acctId(0), customerName(NULLPTR), netWorth(0.0),
  assets(NULLPTR), timestamp(0), getBeforeUpdate(false) {
  }
  DeltaFastAssetAccount(int index, bool encodeTimestp,
      int maxVal, int asstSize = 0, bool getbfrUpdate = false);
  ...
  CacheableStringPtr toString() const {
    ...
    sprintf(
        buf,
        "DeltaFastAssetAccount:[acctId = %d customerName = %s netWorth = %f timestamp = %lld]",
  ...
  virtual DeltaPtr clone() {
    ...
    for (HashMapOfCacheable::Iterator item = this->assets->begin(); item
        != this->assets->end(); item++) {
