...
template <typename T1, typename T2>
bool PdxTests::NonPdxType::genericCompare(T1* value1, T2* value2, int length) const
{
  int i = 0;
  while (i < length)
  {
...
template <typename T1, typename T2>
bool PdxTests::NonPdxType::generic2DCompare(T1** value1, T2** value2, int length, int* arrLengths) const
{
...
bool PdxTests::NonPdxType::selfCheck()
{
  return false;
}

bool PdxTests::NonPdxType::equals(PdxTests::NonPdxType& other, bool isPdxReadSerialized)const {
  ...
  if (!isPdxReadSerialized) {
    for(int i = 0; i< m_objectArray->size(); i++) {
      ...
      NonPdxAddress* otherAddr1 = reinterpret_cast<NonPdxAddress*>(wrapper1->getObject());
      PdxWrapperPtr wrapper2 = dynCast<PdxWrapperPtr>(m_objectArray->at(i));
      NonPdxAddress* myAddr1 = reinterpret_cast<NonPdxAddress*>(wrapper2->getObject());
