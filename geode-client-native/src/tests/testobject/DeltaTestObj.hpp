...
namespace testobject {
  class TESTOBJECT_EXPORT DeltaTestObj : public DeltaTestImpl
  {
    private:
      ...
      bool enableFailure;
    public:
      DeltaTestObj(int intVal, CacheableStringPtr str, bool enableFail = false):DeltaTestImpl(intVal, str),enableFailure(enableFail) {
        //DeltaTestImpl::DeltaTestImpl(intVal, str);
      }
      /*
      DeltaTestObj(int intVal, std::string str, int64_t toDeltaCnt, bool enableFail = false):
        toDeltaCounter(toDeltaCnt),enableFailure(enableFail){
        //DeltaTestImpl::DeltaTestImpl(intVal, str);
      }*/
      DeltaTestObj(DeltaTestObj &obj):DeltaTestImpl(obj) {
      }
      DeltaTestObj():DeltaTestImpl(),toDeltaCounter(0),fromDeltaCounter(0),enableFailure(false){
      ...
      void setFromDeltaCounter(int counter){
        fromDeltaCounter=counter;
      }
      void setToDeltaCounter(int counter){
        toDeltaCounter=counter;
      }
      CacheableStringPtr toString() const {
        char buf[102500];
        sprintf(buf, "DeltaTestObj: toDeltaCounter = %lld fromDeltaCounter = %lld\n",toDeltaCounter,fromDeltaCounter);
        //DeltaTestImpl::toString();
        return CacheableString::create( buf );
      }
      static Serializable* create( )
      {
        return new DeltaTestObj( );
      }
    ...
    protected:
      void checkInvalidInt2(int intVal) {
        if (enableFailure) {
          if (intVal%30 == 27) {
            throw new InvalidDeltaException("Delta could not be applied DeltaTestObj. ");
          }
        }
      }
  };
