...
class TESTOBJECT_EXPORT CharTypes : public PdxSerializable
{
  ...
  public:
    ...
    CacheableStringPtr toString() const {
      char idbuf[1024];
      sprintf(idbuf,"%c %lc %c %c %lc %lc", m_ch,  m_widechar, m_chArray[0], m_chArray[1], m_widecharArray[0], m_widecharArray[1]);
      return CacheableString::create( idbuf );
    }
  ...
  bool equals(CharTypes& other)const {
    ...
    LOGINFO("CharTypes::equals ot->m_ch = %c m_ch = %c AND ot->m_widechar = %lc m_widechar = %lc", ot->m_ch, m_ch, ot->m_widechar, m_widechar);
    ...
    while (i < 2)
    {
      LOGINFO("CharTypes::equals Normal char array values ot->m_chArray[%d] = %c m_chArray[%d] = %c", i, ot->m_chArray[i], i, m_chArray[i]);
      LOGINFO("CharTypes::equals Wide char array values ot->m_widecharArray[%d] = %lc m_widecharArray[%d] = %lc", i, ot->m_widecharArray[i], i, m_widecharArray[i]);
      if (ot->m_chArray[i] != m_chArray[i] || ot->m_widecharArray[i] != m_widecharArray[i])
        return false;
      else
        i++;
    }
...
public:
  ...
  inline void init(){
    ...
    m_hashtable->insert(CacheableInt32::create(1), CacheableString::create("1111111111111111"));
    m_hashtable->insert(CacheableInt32::create(2), CacheableString::create("2222222222221111111111111111"));
    ...
    //CacheablePtr cacheableObject = CacheableObject::create()create(ptrArr,length);
    /*char key[2048];
    ...
    m_objectArrayEmptyPdxFieldName->push_back(AddressPtr(new Address(1, "street0", "city0")));
    m_objectArrayEmptyPdxFieldName->push_back(AddressPtr(new Address(2, "street1", "city1")));
    m_objectArrayEmptyPdxFieldName->push_back(AddressPtr(new Address(3, "street2", "city2")));
    m_objectArrayEmptyPdxFieldName->push_back(AddressPtr(new Address(4, "street3", "city3")));
    m_objectArrayEmptyPdxFieldName->push_back(AddressPtr(new Address(5, "street4", "city4")));
    m_objectArrayEmptyPdxFieldName->push_back(AddressPtr(new Address(6, "street5", "city5")));
    m_objectArrayEmptyPdxFieldName->push_back(AddressPtr(new Address(7, "street6", "city6")));
    m_objectArrayEmptyPdxFieldName->push_back(AddressPtr(new Address(8, "street7", "city7")));
    m_objectArrayEmptyPdxFieldName->push_back(AddressPtr(new Address(9, "street8", "city8")));
    m_objectArrayEmptyPdxFieldName->push_back(AddressPtr(new Address(10, "street9", "city9")));
  ...
  CacheableHashSetPtr getHashSet() {
    return m_chs;
  }

  CacheableLinkedHashSetPtr getLinkedHashSet() {
    return m_clhs;
  }
  ...
  CacheableLinkedListPtr getLinkedList() {
	  return m_linkedlist;
  }

  CacheableHashTablePtr getHashTable() {
    return m_hashtable;
  }

  CacheableVectorPtr getVector() {
      return m_vector;
    }
  ...
  CacheableObjectArrayPtr getCacheableObjectArray(){
