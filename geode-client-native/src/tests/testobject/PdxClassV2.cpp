...
void PdxTypes1V2::fromData(PdxReaderPtr pr) {
	...
	if (isIdentity)
		throw new Exception("i2 is not identity field");
	//
	if (!m_useWeakHashMap)
...
CacheableStringPtr PdxTypes1V2::toString() const {
	...
	sprintf(
			idbuf,
			"PdxType1V1:[ m_i1=%d ] [ m_i2=%d ] [ m_i3=%d ] [ m_i4=%d ] [ m_i5=%d ] [ m_i6=%d ] [ m_diffInExtraFields=%d ]",
...
CacheableStringPtr PdxTypes2V2::toString() const {
	char idbuf[4096];
	sprintf(idbuf, "PdxTypes2V2:[ m_i1=%d ] [ m_i2=%d ] [ m_i3=%d ] [ m_i4=%d ] [ m_i5=%d ] [ m_i6=%d ]",
...
void PdxTypes3V2::fromData( PdxReaderPtr pr )
 ...
 char idbuf[4096];
 sprintf(idbuf,"PdxTypes3V2:[ m_i1=%d ] [ m_i2=%d ] [m_str1=%s] [ m_i3=%d ] [ m_i4=%d ] [ m_i6=%d ] [m_str3=%s]", m_i1, m_i2, m_str1, m_i3, m_i4, m_i6, m_str3 );
 return CacheableString::create( idbuf );
 }
...
bool PdxTypesR1V2::equals(PdxSerializablePtr obj) {
  ...
  if (m_i1 + m_diffInSameFields <= pap->m_i1
			...
			&& m_i3 + m_diffInSameFields <= pap->m_i3
			&& m_i4 + m_diffInSameFields <= pap->m_i4) {
		    if ((m_i5 + PdxTypesR1V2::m_diffInExtraFields == pap->m_i5) && (m_i6 + PdxTypesR1V2::m_diffInExtraFields == pap->m_i6) ||
		    	(PdxTypesR1V2::m_diffInExtraFields == pap->m_i5) && (PdxTypesR1V2::m_diffInExtraFields == pap->m_i6))
			    return true;
	}
  return false;
}
...
CacheableStringPtr PdxTypesR1V2::toString() const {
  char idbuf[4096];
  sprintf(idbuf,"PdxTypesR1V2:[ m_i1=%d ] [ m_i2=%d ] [ m_i3=%d ] [ m_i4=%d ] [ m_i5=%d ] [ m_i6=%d ]", m_i1, m_i2, m_i3, m_i4, m_i5, m_i6 );
 return CacheableString::create( idbuf );
 }
...
void PdxTypesR2V2::toData( PdxWriterPtr pw )  {
 ...
 char idbuf[4096];
 sprintf(idbuf,"PdxTypesR2V2:[ m_i1=%d ] [m_i2=%d] [ m_i3=%d ] [ m_i4=%d ] [m_i5=%d] [m_i6=%d] [m_str1=%s]", m_i1, m_i2, m_i3, m_i4, m_i5, m_i6, m_str1 );
 return CacheableString::create( idbuf );
 }
...
bool PdxTypesIgnoreUnreadFieldsV2::equals(PdxSerializablePtr obj) {
  ...
  PdxTypesIgnoreUnreadFieldsV2Ptr pap = dynCast<PdxTypesIgnoreUnreadFieldsV2Ptr>(obj);
  if (pap == NULLPTR)
    return false;
  ...
  LOGINFO("m_i1 =%d m_diffInSameFields=%d pap->m_i1=%d", m_i1, m_diffInSameFields, pap->m_i1);
  LOGINFO("m_i2 =%d m_diffInSameFields=%d pap->m_i2=%d", m_i2, m_diffInSameFields, pap->m_i2);
  LOGINFO("m_i3 =%d m_diffInSameFields=%d pap->m_i3=%d", m_i3, m_diffInSameFields, pap->m_i3);
  LOGINFO("m_i4 =%d m_diffInSameFields=%d pap->m_i4=%d", m_i4, m_diffInSameFields, pap->m_i4);
...
void PdxTypesIgnoreUnreadFieldsV2::fromData( PdxReaderPtr pr )
{
 ...
 char idbuf[4096];
 sprintf(idbuf,"PdxTypesV1R1:[m_i1=%d] [m_i2=%d] [m_i3=%d] [m_i4=%d] [m_i5=%d] [m_i6=%d]", m_i1, m_i2, m_i3, m_i4, m_i5, m_i6 );
 return CacheableString::create( idbuf );
 }
 ...
 * *********************************************************/
 ...
 char idbuf[4096];
 //sprintf(idbuf,"PdxTypesV1R1:[ m_i1=%d ] [ m_i2=%d ] [ m_i3=%d ] [ m_i4=%d ]", m_i1, m_i2, m_i3, m_i4 );
 return CacheableString::create( idbuf );
 }
 ...
 * *********************************************************/
 ...
 bool TestDiffTypePdxSV2::equals(TestDiffTypePdxSV2* obj){
	 if (obj == NULL)
      ...
      LOGINFO("TestDiffTypePdxSV2 other->_coun = %d and _count = %d ", other->_count, _count);
      LOGINFO("TestDiffTypePdxSV2 other->_id = %s and _id = %s ", other->_id, _id);
      LOGINFO("TestDiffTypePdxSV2 other->_name = %s and _name = %s ", other->_name, _name);

	   if (other->_count == _count && strcmp(other->_id, _id) == 0 && strcmp(other->_name, _name) == 0)
	     return true;
