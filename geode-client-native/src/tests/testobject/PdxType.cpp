...
template <typename T1, typename T2>
bool PdxTests::PdxType::genericCompare(T1* value1, T2* value2, int length) const
{
  int i = 0;
  while (i < length)
  {
...
template <typename T1, typename T2>
bool PdxTests::PdxType::generic2DCompare(T1** value1, T2** value2, int length, int* arrLengths) const
{
...
void  PdxTests::PdxType::fromData( PdxReaderPtr pr )
{
  ...
  m_linkedlist = dynCast<CacheableLinkedListPtr>(pr->readObject("m_linkedlist"));
...
bool PdxTests::PdxType::equals(PdxTests::PdxType& other, bool isPdxReadSerialized)const {
  ...
  if (!isPdxReadSerialized) {
    for(int i = 0; i< m_objectArray->size(); i++) {
      Address* otherAddr1 = dynamic_cast<Address*>(ot->m_objectArray->at(i).ptr());
  ...
  if (!isPdxReadSerialized) {
    ...
    LOGINFO("PdxObject::equals Empty Field Name isPdxReadSerialized = %d", isPdxReadSerialized);
  }
