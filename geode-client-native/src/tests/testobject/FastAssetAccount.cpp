...
FastAssetAccount::FastAssetAccount(int idx, bool encodeTimestp, int maxVal, int asstSize):
...
CacheableStringPtr FastAssetAccount::toString() const {
  char buf[102500];
  sprintf(buf, "FastAssetAccount:[acctId = %d customerName = %s netWorth = %f timestamp = %ld]", acctId,customerName->toString(),netWorth,timestamp);
