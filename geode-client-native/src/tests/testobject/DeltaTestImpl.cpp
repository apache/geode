...
DeltaTestImpl::DeltaTestImpl(int intValue, CacheableStringPtr strptr):intVar(intValue),
str(strptr),doubleVar(0),toDeltaCounter(0),fromDeltaCounter(0){
...
DeltaTestImpl::DeltaTestImpl(DeltaTestImplPtr rhs )
{
  ...
  byteArr = (rhs->byteArr == NULLPTR ? NULLPTR :CacheableBytes::create( rhs->byteArr->value( ), rhs->byteArr->length( ) ));
  testObj = ( rhs->testObj == NULLPTR ? NULLPTR : TestObject1Ptr( new TestObject1( *( rhs->testObj.ptr( ) ) ) ) );
...
{
	{
		ACE_Recursive_Thread_Mutex *lock = const_cast<ACE_Recursive_Thread_Mutex *>(&m_lock);
