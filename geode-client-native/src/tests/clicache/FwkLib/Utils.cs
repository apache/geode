  ...
  public class Utils : FwkTest
  {
    ...
    private const string JavaServerJavaArgs =
      //" -J-Xmx1280m -J-Xms512m -J-DCacheClientProxy.MESSAGE_QUEUE_SIZE=100000";
           " -J-Xmx2048m -J-Xms1024m -J-DCacheClientProxy.MESSAGE_QUEUE_SIZE=100000";
    ...
    private string GetSslProperty(string hostType, bool forServer, string startDir)
    {
      ...
      //{
        ...
        string keyStorePath = Util.GetFwkLogDir(hostType) + "/data/keystore/server_keystore.jks";
    ...
    private void SetupJavaServers(string argStr)
    {
      ...
      else
      {
        ...
        Util.BBSet(JavaServerBB, FwkTest.JavaServerEPCountKey, m_numServers);
        ...
        for (int serverNum = 1; serverNum <= m_numServers; serverNum++)
        {
          ...
          Util.BBSet(JavaServerBB, FwkTest.EndPoints + "_" + serverNum.ToString(), targetHost + ':' + javaServerPort);
        }
      ...
      Util.BBSet(JavaServerBB, FwkTest.EndPointTag, globalEndPoints);
      Util.BBSet(JavaServerBB, FwkTest.EndPointTag + hostGroup, endpoints);
      lock (((IDictionary)m_javaServerHostMap).SyncRoot)
      {
