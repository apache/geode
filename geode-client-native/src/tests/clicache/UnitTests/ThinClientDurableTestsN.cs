  ...
  public class ThinClientDurableTests : ThinClientRegionSteps
  {
    ...
    public void InitFeeder(string endpoints, string locators, int redundancyLevel, bool pool, bool locator)
    {
      if (pool)
      {
        if (locator)
        {
          CacheHelper.CreatePool<object, object>("__TESTPOOL1_", (string)null, locators, (string)null, redundancyLevel, false);
          CacheHelper.CreateTCRegion_Pool<object, object>(RegionNames[0], false, true, null, (string)null,
            locators, "__TESTPOOL1_", false);
        }
        else
        {
          CacheHelper.CreatePool<object, object>("__TESTPOOL1_", endpoints, (string)null, (string)null, redundancyLevel, false);
          CacheHelper.CreateTCRegion_Pool<object, object>(RegionNames[0], false, true, null, endpoints,
    ...
    public void InitFeeder2(string endpoints, string locators, int redundancyLevel, bool pool, bool locator)
    {
      if (pool)
      {
        if (locator)
        {
          ...
          CacheHelper.CreatePool<object, object>("__TESTPOOL2_", (string)null, locators, (string)null, redundancyLevel, false);
          CacheHelper.CreateTCRegion_Pool<object, object>(RegionNames[1], false, true, null, (string)null,
        ...
        else
        {
          ...
          CacheHelper.CreatePool<object, object>("__TESTPOOL2_", endpoints, (string)null, (string)null, redundancyLevel, false);
          CacheHelper.CreateTCRegion_Pool<object, object>(RegionNames[1], false, true, null, endpoints,
            (string)null, "__TESTPOOL2_", false);
        }
      }
    }

    public void InitDurableClientWithTwoPools(string endpoints, string locators,
    int redundancyLevel, string durableClientId, int durableTimeout, bool pool, bool locator, int expectedQ0, int expectedQ1)
    {
      DurableListener<object, object> checker = null;
      if (locator)
      {
        CacheHelper.InitConfigForDurable_Pool2((string)null, locators, redundancyLevel,
        ...
        CacheHelper.CreateTCRegion_Pool(RegionNames[1], false, true, checker,
            (string)null, CacheHelper.Locators, "__TESTPOOL2_", true);
      }
      else
      {
        CacheHelper.InitConfigForDurable_Pool2(endpoints, (string)null, redundancyLevel,
             durableClientId, durableTimeout, 35000, "__TESTPOOL1_");
        CacheHelper.CreateTCRegion_Pool(RegionNames[0], false, true, checker,
        ...
        CacheHelper.InitConfigForDurable_Pool2(endpoints, (string)null, redundancyLevel,
            durableClientId, durableTimeout, 35000, "__TESTPOOL2_");
        CacheHelper.CreateTCRegion_Pool(RegionNames[1], false, true, checker,
           CacheHelper.Endpoints, (string)null, "__TESTPOOL2_", true);
      }
      ...
      string durableClientId, int durableTimeout, bool pool, bool locator)
    {
      ...
      {
        CacheHelper.InitConfigForDurable<object, object>(endpoints, redundancyLevel, durableClientId, durableTimeout);
        CacheHelper.CreateTCRegion<object, object>(RegionNames[0], false, true, checker, CacheHelper.Endpoints, true);
      }
      */ 
      CacheHelper.DCache.ReadyForEvents();
    ...
    void runDurableClientWithTwoPools(bool pool, bool locator)
    {
      if (pool && locator)
      {
        CacheHelper.SetupJavaServers(true, "cacheserver_notify_subscription.xml");
      ...
      m_feeder.Call(InitFeeder2, CacheHelper.Endpoints, CacheHelper.Locators, 0, pool, locator);
      Util.Log("Feeder started.");
        
      m_client1.Call(InitDurableClientWithTwoPools, CacheHelper.Endpoints, CacheHelper.Locators, 0, DurableClientId1, 30, pool, locator, -2, -2);
      ...
      m_client1.Call(InitDurableClientWithTwoPools, CacheHelper.Endpoints, CacheHelper.Locators, 0, DurableClientId1, 30, pool, locator, 6, 11); //+1 for marker, so 5+1, 10+1 etc
      ...
      m_client1.Call(InitDurableClientWithTwoPools, CacheHelper.Endpoints, CacheHelper.Locators, 0, DurableClientId1, 30, pool, locator, 16, 16);
      Util.Log("DurableClient with Two Pools after second update");

      Thread.Sleep(45000); //45 > 30 secs.
      m_client1.Call(InitDurableClientWithTwoPools, CacheHelper.Endpoints, CacheHelper.Locators, 0, DurableClientId1, 30, pool, locator, -1, -1);
    ...
    public void CQDurable()
    {
      ...
      runDurableClientWithTwoPools(true, true);
      runDurableClientWithTwoPools(true, false);
    }
    #endregion
  }
}
