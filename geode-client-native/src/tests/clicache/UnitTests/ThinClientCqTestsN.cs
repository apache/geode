...
namespace GemStone.GemFire.Cache.UnitTests.NewAPI
{
  ...
  public class ThinClientCqTests : ThinClientRegionSteps
  {
    ...
    public void StepOnePdxQE(string endpoints, string locators, bool pool, bool locator)
    {
      if (pool)
      {
        if (locator)
    ...
    */
    ...
    public void ProcessCQ(string endpoints, string locators, bool pool, bool locator)
    {
      if (pool) {
        if (locator) {
          CacheHelper.CreateTCRegion_Pool<object, object>(QERegionName, true, true,
    ...
    public void CreateRegion(string endpoints, string locators, bool pool, bool locator, string servergroup, string regionName, string poolName)
    {
      if (pool) {
        if (locator) {
          ...
          CacheHelper.CreateTCRegion_Pool<object, object>(regionName, true, true,
            null, endpoints, (string)null, poolName, true);
        }
      }
    }
    ...
    void runCqQueryTest(bool pool, bool locator)
    {
      ...
      if (!m_usePdxObjects)
        ...
        m_client1.Call(StepOnePdxQE, CacheHelper.Endpoints, CacheHelper.Locators, pool, locator);
      Util.Log("StepOne complete.");
    ...
    void runCqQueryStatusTest(bool pool, bool locator)
    {
      if (pool && locator) {
    ...
    void runCqQueryStatusTest2(bool pool, bool locator)
    {
      if (pool && locator) {
        ...
        CacheHelper.SetupJavaServers(false, "cacheserver_servergroup.xml", "cacheserver_servergroup2.xml");
        CacheHelper.StartJavaServer(1, "GFECS1");
        Util.Log("start server 1 complete.");
        CacheHelper.StartJavaServer(2, "GFECS2");
        Util.Log("start server 2 complete.");
      ...
      m_client1.Call(CreateRegion, CacheHelper.Endpoints, CacheHelper.Locators, pool, locator, "group1", "DistRegionAck", "__TESTPOOL1_");
      Util.Log("CreateRegion DistRegionAck complete.");

      m_client1.Call(CreateRegion, CacheHelper.Endpoints, CacheHelper.Locators, pool, locator, "group2", "DistRegionAck1", "__TESTPOOL2_");
    ...
    void runCqQueryStatusTest3(bool pool, bool locator)
    {
      if (pool && locator) {
        CacheHelper.SetupJavaServers(true, "remotequeryN.xml");
        CacheHelper.StartJavaLocator(1, "GFELOC");
      ...
      m_client1.Call(ProcessCQ, CacheHelper.Endpoints, CacheHelper.Locators, pool, locator);
    ...
    {
      runCqQueryStatusTest(true, false); // pool with server endpoints
      runCqQueryStatusTest(true, true); // pool with locators
    }

    [Test]
    public void CqQueryStatusTest2()
    {
      //This will work only with locator, as it involves servergroup property.      
      runCqQueryStatusTest2(true, true); // pool with locators
    }

    [Test]
    public void CqQueryStatusTest3()
    {
      runCqQueryStatusTest3(true, false); // pool with server endpoints
      runCqQueryStatusTest3(true, true); // pool with locators
    }
