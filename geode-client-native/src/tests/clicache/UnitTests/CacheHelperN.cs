...
namespace GemStone.GemFire.Cache.UnitTests.NewAPI
{
  ...
  public class CacheHelperWrapper : MarshalByRefObject
  {
    public void CreateTCRegions_Pool_AD<TKey, TValue>(string[] regionNames,
      string endpoints, string locators, string poolName, bool clientNotification, bool ssl, bool caching, bool pdxReadSerialized)
    {
      try
      {
  ...
  public class CacheHelper
  {
    ...
    public static void InitConfigForEventId(string endpoints, int redundancyLevel, int ackInterval, int dupCheckLife)
    {
      ...
      config.Insert("notify-ack-interval", ackInterval.ToString());
      config.Insert("notify-dupcheck-life", dupCheckLife.ToString());
      InitConfig(DefaultDSName, DefaultCacheName, config,
        cFact.CreateCacheAttributes(), null);
    }
      ...
      string durableClientId, int durableTimeout, int ackInterval)
    {
      ...
      InitConfig(DefaultDSName, DefaultCacheName, config,
        cFact.CreateCacheAttributes(), null);
      CreatePool<object, object>("__TESTPOOL1_", endpoints, locators, (string)null, redundancyLevel, true,
        ackInterval, 300);
    }

    public static void InitConfigForDurable_Pool2(string endpoints, string locators, int redundancyLevel,
      string durableClientId, int durableTimeout, int ackInterval, string poolName)
    {
        ...
        InitConfig(DefaultDSName, DefaultCacheName, config,
          cFact.CreateCacheAttributes(), null);
        CreatePool<object, object>(poolName, endpoints, locators, (string)null, redundancyLevel, true,
          ackInterval, 300);
    }
      ...
      string durableClientId, string conflation)
    {
      ...
      InitConfig(DefaultDSName, DefaultCacheName, config,
        cFact.CreateCacheAttributes(), null);
      CreatePool<object, object>("__TESTPOOL1_", endpoints, locators, (string)null, 0, true);
    }
    ...
    public static void InitConfig(string dsName, string cacheName,
      ...
      string cacheXml)
    {
      //Console.WriteLine("Hitesh in InitConfig1 " + System.AppDomain.CurrentDomain.Id);
      if (cacheXml != null)
      {
      ...
      if (m_cache == null || m_cache.IsClosed)
      {
        ...
        try
        {
          ...
          else
          {
            ...
            {
              //Console.WriteLine("Hitesh in InitConfig " + System.AppDomain.CurrentDomain.Id);
              if (m_heapLimit != -1)
    ...
    public static IRegion<TKey, TValue> CreateRegion<TKey, TValue>(string name, RegionAttributes<TKey, TValue> attrs)
    {
      Init();
    ...
    public static void CreateScopeRegion<TKey, TValue>(string name, ScopeType scope, bool caching)
    {
      Init();
      IRegion<TKey, TValue> region = GetRegion<TKey, TValue>(name);
      if ((region != null) && !region.IsDestroyed)
    ...
    public static IRegion<TKey, TValue> CreateDistRegion<TKey, TValue>(string rootName, ScopeType rootScope,
      string name, int size)
    {
      Init();
      CreateScopeRegion<TKey, TValue>(rootName, rootScope, true);
      ...
      RegionAttributes<TKey, TValue> rattrs = af.CreateRegionAttributes();
    ...
    {
      return CreateLRURegion<TKey, TValue>(name, size, ScopeType.Local);
    }

    public static IRegion<TKey, TValue> CreateLRURegion<TKey, TValue>(string name, uint size, ScopeType scope)
    {
      Init();
      IRegion<TKey, TValue> region = GetRegion<TKey, TValue>(name);
      if ((region != null) && !region.IsDestroyed)
      ...
      region = m_cache.CreateRegionFactory(RegionShortcut.LOCAL_ENTRY_LRU)
        ...
        /*.SetScope(scope)*/ .Create<TKey, TValue>(name);
    ...
    public static IRegion<TKey, TVal> CreateTCRegion<TKey, TVal>(string name, bool ack, bool caching,
      ICacheListener<TKey, TVal> listener, string endpoints, bool clientNotification)
    {
      Init();
      IRegion<TKey, TVal> region = GetRegion<TKey, TVal>(name);
      if ((region != null) && !region.IsDestroyed)
      {
        region.GetLocalView().DestroyRegion();
        Assert.IsTrue(region.IsDestroyed, "IRegion<object, object> {0} was not destroyed.", name);
      ...
      if (endpoints != null)
      {
        //throw new IllegalArgumentException("Should not specify region level endpoints!");
      }
      PoolFactory/*<TKey, TVal>*/ poolFactory = PoolManager/*<TKey, TVal>*/.CreateFactory();
      string[] location = endpoints.Split(new char[] { ':' });
      poolFactory.AddServer(location[0], int.Parse(location[1]));
      poolFactory.SetSubscriptionEnabled(clientNotification);
      poolFactory.Create("__TESTPOOL__");
      region = regionFactory.SetPoolName("__TESTPOOL__").Create<TKey, TVal>(name);

      Assert.IsNotNull(region, "IRegion<TKey, TValue> {0} was not created.", name);
      //m_currRegion = (IRegion<object, object>)region;
      Util.Log("IRegion<TKey, TValue> {0} has been created with attributes:{1}",
    ...
    public static IRegion<TradeKey, Object> CreateTCRegion2<TradeKey, Object>(string name, bool ack, bool caching,
      IPartitionResolver<TradeKey, Object> resolver, string endpoints, string locators, bool clientNotification)
    {
      Init();
      ...
      if (endpoints != null)
      {
        ...
        foreach (string item in list)
        {
          ...
          poolFactory.AddServer(parts[0], int.Parse(parts[1]));
          Util.Log("AddServer parts[0] = {0} int.Parse(parts[1]) = {1} ", parts[0], int.Parse(parts[1]));
        }
      }
    ...
    public static Pool/*<TKey, TValue>*/ CreatePool<TKey, TValue>(string name, string endpoints, string locators, string serverGroup,
      int redundancy, bool subscription)
    {
      return CreatePool<TKey, TValue>(name, endpoints, locators, serverGroup, redundancy, subscription, 5, 1, 300);
    }

    public static Pool/*<TKey, TValue>*/ CreatePool<TKey, TValue>(string name, string endpoints, string locators, string serverGroup,
      int redundancy, bool subscription, bool prSingleHop, bool threadLocal = false)
    {
      return CreatePool<TKey, TValue>(name, endpoints, locators, serverGroup, redundancy, subscription, -1, 1, 300, false, prSingleHop, threadLocal);
    }

    public static Pool/*<TKey, TValue>*/ CreatePool<TKey, TValue>(string name, string endpoints, string locators, string serverGroup,
      int redundancy, bool subscription, int numConnections, bool isMultiuserMode)
    {
      return CreatePool<TKey, TValue>(name, endpoints, locators, serverGroup, redundancy, subscription, numConnections, 1, 300, isMultiuserMode);
    }

    public static Pool/*<TKey, TValue>*/ CreatePool<TKey, TValue>(string name, string endpoints, string locators, string serverGroup,
      int redundancy, bool subscription, int numConnections)
    {
      return CreatePool<TKey, TValue>(name, endpoints, locators, serverGroup, redundancy, subscription, numConnections, 1, 300);
    }

    public static Pool/*<TKey, TValue>*/ CreatePool<TKey, TValue>(string name, string endpoints, string locators, string serverGroup,
      int redundancy, bool subscription, int ackInterval, int dupCheckLife)
    {
      return CreatePool<TKey, TValue>(name, endpoints, locators, serverGroup, redundancy, subscription,
    ...
    {
      return CreatePool<TKey, TValue>(name, endpoints, locators, serverGroup, redundancy, subscription, numConnections, ackInterval, 300, false);
    }
    ...
    public static IRegion<TKey, TValue> CreateTCRegion_Pool<TKey, TValue>(string name, bool ack, bool caching,
      ICacheListener<TKey, TValue> listener, string endpoints, string locators, string poolName, bool clientNotification)
    {
    ...
    public static void CreateTCRegion_Pool_AD1(string name, bool ack, bool caching,
       string endpoints, string locators, string poolName, bool clientNotification, bool cloningEnable)
    {
      CreateTCRegion_Pool_AD<object, object>(name, ack, caching, null, endpoints, locators, poolName, clientNotification, false, cloningEnable);
    }

    public static IRegion<TKey, TValue> CreateTCRegion_Pool_AD<TKey, TValue>(string name, bool ack, bool caching,
      ICacheListener<TKey, TValue> listener, string endpoints, string locators, string poolName, bool clientNotification, bool ssl,
      bool cloningEnabled)
    {
      if (ssl)
      {
      ...
      bool cloningEnabled)
    {
      ...
      Util.Log("hitesh caching enable " + caching);
      RegionFactory regionFactory = m_cache.CreateRegionFactory(RegionShortcut.PROXY)
        .SetInitialCapacity(100000).SetPoolName(poolName).SetCloningEnabled(cloningEnabled)
        //.SetScope(ack ? ScopeType.DistributedAck : ScopeType.DistributedNoAck)
        .SetCachingEnabled(caching);
    ...
    public static IRegion<TKey, TValue> CreateLRUTCRegion_Pool<TKey, TValue>(string name, bool ack, bool caching,
     ICacheListener<TKey, TValue> listener, string endpoints, string locators, string poolName, bool clientNotification, uint lru)
    {
      Init();
      IRegion<TKey, TValue> region = GetRegion<TKey, TValue>(name);
      if ((region != null) && !region.IsDestroyed)
      ...
      Properties<string, string> bdbProperties = Properties<string, string>.Create<string, string>();
    ...
    public static void SetupJavaServers(bool locators, params string[] cacheXmls)
    {
      ...
      else if (cacheXmls != null)
      {
        ...
        for (int i = 0; i < cacheXmls.Length; i++)
        {
          ...
          if (serverNode == null)
          {
            serverNode = xmlDoc.SelectSingleNode("/cache/cache-server");
          }
          if (m_endpoints == null)
          {
            m_endpoints = System.Net.Dns.GetHostEntry("localhost").HostName + ":" + serverNode.Attributes["port"].Value;
          }
          else
          {
            m_endpoints += "," + System.Net.Dns.GetHostEntry("localhost").HostName + ":" + serverNode.Attributes["port"].Value;
    ...
    public static string RegionAttributesToString<TKey, TVal>(RegionAttributes<TKey, TVal> attrs)
    {
