...
namespace GemStone.GemFire.Cache.UnitTests.NewAPI
{
  ...
  public class ThinClientCallbackArg : ThinClientRegionSteps
  {
      ...
      bool caching, bool listener, bool writer, bool pool, bool locator)
    {
      ...
      {
        throw new IllegalArgumentException("Should not run the new API tests with non pool config");
      }
      if (listener)
    ...
    void runCallbackArgTest2(int callbackArgChange, bool pool, bool locator)
    {
      ...
      m_callbackListener = new CallbackListener();
      m_callbackListener.SetCallbackArg(m_callbackarg);
      CacheHelper.SetupJavaServers(pool && locator, "cacheserver_notify_subscription5N.xml");
      if (pool && locator)
      {
        CacheHelper.StartJavaLocator(1, "GFELOC");
        Util.Log("Locator started");
    ...
    void runPutAllCallbackArgTest(bool pool, bool locator)
    {
      m_callbackListener = new CallbackListener();
      m_callbackListener.SetCallbackArg(m_callbackarg);
    ...
    void runRemoveAllCallbackArgTest(bool pool, bool locator)
    {
    ...
    {
      runRemoveAllCallbackArgTest(true, false); // region config
      runRemoveAllCallbackArgTest(true, true); // region config
    }

    [Test]
    public void PutAllCallbackArgTest()
    {
      runPutAllCallbackArgTest(true, false); // region config
      runPutAllCallbackArgTest(true, true); // region config
    }
  }
}