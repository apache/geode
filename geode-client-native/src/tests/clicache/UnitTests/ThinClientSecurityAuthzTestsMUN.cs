...
namespace GemStone.GemFire.Cache.UnitTests.NewAPI
{
  ...
  public class ThinClientSecurityAuthzTestsMU : ThinClientSecurityAuthzTestBase
  {
    ...
    {
      Util.Log("hitesh in CreateRegion " + listener + " : " + writer + " ; " + pool + " : " + locator);
      if (listener)
      {
      ...
      Util.Log("region created hitesh ");
    ...
    {
      CacheHelper.SetupJavaServers(pool && locator, "remotequeryN.xml");
      ...
      string serverArgs = SecurityTestUtil.GetServerArgs(authenticator,
        ...
        m_client1.Call(SecurityTestUtil.CreateClientMU2, RegionName_CQ,
          CacheHelper.Endpoints, CacheHelper.Locators, authInit, (Properties<string, string>)null, pool, locator, true, true);

        m_client2.Call(SecurityTestUtil.CreateClientMU, RegionName_CQ,
          CacheHelper.Endpoints, CacheHelper.Locators, authInit, (Properties<string, string>)null, pool, locator, true);
    ...
    void runInvalidAccessor(bool pool, bool locator)
    {
      ...
      foreach (AuthzCredentialGenerator authzGen in GetAllGeneratorCombos(true))
      {
        ...
        Util.Log("InvalidAccessor: For first client PUT credentials: " +
          createCredentials);
        m_client1.Call(SecurityTestUtil.CreateClientMU, RegionName,
          CacheHelper.Endpoints, CacheHelper.Locators, authInit, (Properties<string, string>)null,
           pool, locator, true);
    ...
    public void TestDurableCQ()
    {
      ...
      runDurableCQTest(true, false, false, true, true);//no usercache close as it will close user's durable cq
