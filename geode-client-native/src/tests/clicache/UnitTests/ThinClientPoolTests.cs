...
namespace GemStone.GemFire.Cache.UnitTests
{
  ...
  public class ThinClientPoolTests : ThinClientRegionSteps
  {
    ...
    public void runPoolXmlCreation()
    {
      ...
      Region[] subRegions = rootRegions[0].SubRegions(true);
      Assert.AreEqual(1, subRegions.Length);

      Region[] subRegions2 = rootRegions[1].SubRegions(true);
      ...
      bool check1 = checkPoolAttributes(poolOfRegion1, locators, null, 12345, 23456, 3, 7, 3, 5555, 12345,
        "test_pool_1", 23456, "ServerGroup1", 32768, true, 900123, 567, 0, 10123, 5, true);

      bool check2 = checkPoolAttributes(poolOfRegion2, null, servers, 23456, 34567, 2, 8, 5, 6666, 23456,
        "test_pool_2", 34567, "ServerGroup2", 65536, false, 800222, 678, 1, 20345, 3, false);
    ...
    {
      CacheHelper.CreateTCRegion_Pool(regionName, true, true, null, endpoints, locators, poolName, true);
    }
    ...
    {
      Region region = CacheHelper.GetVerifyRegion("PoolRegion3");
      CacheableString cKey = new CacheableString(m_keys[1]);

      CacheableString cVal = region.Get(cKey) as CacheableString;

      Assert.IsNotNull(cVal, "Value should not be null.");
      if (cVal.Value != m_nvals[1])
    ...
    {
      Region region1 = CacheHelper.GetVerifyRegion("PoolRegion1");
      region1.RegisterAllKeys(false, null, true);
      Region region2 = CacheHelper.GetVerifyRegion("PoolRegion2");
      region2.RegisterAllKeys(false, null, true);
      Region region3 = CacheHelper.GetVerifyRegion("PoolRegion3");
      region3.RegisterAllKeys(false, null, true);
    }
