...
namespace GemStone.GemFire.Cache.UnitTests.NewAPI
{
  ...
  public class ThinClientDurableCqTests : ThinClientRegionSteps
  {
    ...
    public void InitDurableClient(string endpoints, string locators, int redundancyLevel,
     string durableClientId, int durableTimeout, bool pool, bool locator)
    {
      if (pool)
      {
        if (locator)
        {
          CacheHelper.InitConfigForDurable_Pool((string)null, locators, redundancyLevel, durableClientId, durableTimeout);
          CacheHelper.CreateTCRegion_Pool(QueryRegionNames[0], true, true, (ICacheListener<object, object>)null, (string)null, CacheHelper.Locators, "__TESTPOOL1_", true);
        }
        else
        {
          CacheHelper.InitConfigForDurable_Pool(endpoints, (string)null, redundancyLevel, durableClientId, durableTimeout);
          CacheHelper.CreateTCRegion_Pool(QueryRegionNames[0], true, true, (ICacheListener<object, object>)null, CacheHelper.Endpoints, (string)null, "__TESTPOOL1_", true);
        }
      }
      else
      {
        CacheHelper.InitConfigForDurable(endpoints, redundancyLevel, durableClientId, durableTimeout);
        CacheHelper.CreateTCRegion(QueryRegionNames[0], false, true, (ICacheListener<object, object>)null, CacheHelper.Endpoints, true);
      }
    }
    ...
    private void RunTestGetDurableCqsFromServer(bool bLocator)
    {
      try
      {
        if (bLocator)
        ...
        {
          CacheHelper.SetupJavaServers("cacheserverDurableCqs.xml");
          CacheHelper.StartJavaServer(1, "GFECS1");
          Util.Log("Cache server 1 started");
        }

        m_client1.Call(InitDurableClient, CacheHelper.Endpoints, CacheHelper.Locators, 0, "DurableClient1", 300, true, bLocator);
        m_client2.Call(InitDurableClient, CacheHelper.Endpoints, CacheHelper.Locators, 0, "DurableClient2", 300, true, bLocator);
    ...
    private void RunTestGetDurableCqsFromServerEmptyList()
    {
      try
      {
        CacheHelper.SetupJavaServers("cacheserverDurableCqs.xml");
        ...
        m_client1.Call(InitDurableClient, CacheHelper.Endpoints, CacheHelper.Locators, 0, "DurableClient1", 300, true, false);
        m_client1.Call(VerifyEmptyDurableCqListClient1);
      }
      finally
      {
        m_client1.Call(CacheHelper.Close);
    ...
    private void RunTestGetDurableCqsFromServerCyclicClients(bool bLocator)
    {
      try
      {
        if (bLocator)
    ...
    private void RunTestGetDurableCqsFromServerMultipleChunks()
    {
      try
      {
        CacheHelper.SetupJavaServers("cacheserverDurableCqs.xml");
        ...
        m_client1.Call(RegisterCqsClient1MultipleChunks);
        Util.Log("Registered DurableCQs.");

        m_client1.Call(VerifyDurableCqListClient1MultipleChunks);
    ...
    {
      RunTestGetDurableCqsFromServer(false); //without locator
    }

    [Test]
    public void TestGetDurableCqsFromServerMultipleChunks()
    {
      RunTestGetDurableCqsFromServerMultipleChunks(); //without locator
    }

    [Test]
    public void TestGetDurableCqsFromServerWithLocator()
    {
      RunTestGetDurableCqsFromServer(true); //with locator
    }

    [Test]
    public void TestGetDurableCqsFromServerEmptyList()
    {
      RunTestGetDurableCqsFromServerEmptyList();
    }


    [Test]
    public void TestGetDurableCqsFromServerCyclicClients()
    {
      RunTestGetDurableCqsFromServerCyclicClients(false); //without locator
    }

    [Test]
    public void TestGetDurableCqsFromServerCyclicClientsWithLocator()
    {
      RunTestGetDurableCqsFromServerCyclicClients(true); //with locator
    }
