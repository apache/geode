...
namespace GemStone.GemFire.Cache.UnitTests.NewAPI
{
 ...
 [Serializable]
  public class CustomPartitionResolver1<TValue> : IFixedPartitionResolver<int, TValue>
  {
    ...
    public string GetPartitionName(EntryEvent<int, TValue> entryEvent, ICollection<string> targetPartitions)
    {
  ...
  public class ThinClientRegionTests : ThinClientRegionSteps
  {
    ...
    public void ClearRegionListenersStep3()
    {
      ...
      RegionAttributes<object, object> attr = region0.Attributes;
    ...
    public void RegexInterestAllStep3(string endpoints, bool pool, bool locator)
    {
      IRegion<object, object> region0 = CacheHelper.GetVerifyRegion<object, object>(m_regionNames[0]);
      IRegion<object, object> region1 = CacheHelper.GetVerifyRegion<object, object>(m_regionNames[1]);
    ...
    void runCheckPutGet(bool pool, bool locator)
    {
      ...
      {
        CacheHelper.SetupJavaServers(false, "cacheServer_pdxreadserialized.xml");
        CacheHelper.StartJavaServer(1, "GFECS1");
      }
      ...
      putGetTest.TestAllKeyValuePairs(m_client1, m_client2,
        RegionNames[0], true, pool,dtTicks);
    ...
    void runCheckPutGetWithAppDomain(bool pool, bool locator)
    {
      ...
      if (pool)
      {
        if (locator)
        {
          m_client1.Call(CreateTCRegions_Pool_AD, RegionNames,
           (string)null, CacheHelper.Locators, "__TESTPOOL1_", false, false, dtTime);
          Util.Log("Client 1 (pool locators) regions created");
        }
        else
        {
          m_client1.Call(CreateTCRegions_Pool_AD, RegionNames,
            CacheHelper.Endpoints, (string)null, "__TESTPOOL1_", false, false, dtTime);
          Util.Log("Client 1 (pool endpoints) regions created");
        }
      }
      ...
      m_client1.Call(TestAllKeyValuePairsAD, RegionNames[0], true, pool, dtTime);
    ...
    void runPdxAppDomainTest(bool pool, bool locator, bool caching, bool readPdxSerialized)
    {
      if (pool && locator)
      {
        CacheHelper.SetupJavaServers(true, "cacheServer_pdxreadserialized.xml");
      ...
      if (pool)
      {
        if (locator)
        {
          m_client1.Call(CreateTCRegions_Pool_AD2, RegionNames,
           (string)null, CacheHelper.Locators, "__TESTPOOL1_", false, false, caching, readPdxSerialized);
          Util.Log("Client 1 (pool locators) regions created");
        }
        else
        {
          m_client1.Call(CreateTCRegions_Pool_AD2, RegionNames,
            CacheHelper.Endpoints, (string)null, "__TESTPOOL1_", false, false, caching, readPdxSerialized);
          Util.Log("Client 1 (pool endpoints) regions created");
        }
      }
      ...
      if (pool)
      {
        if (locator)
        {
          m_client1.Call(CreateTCRegions_Pool2_WithPartitionResolver, RegionNames,
           (string)null, CacheHelper.Locators, "__TESTPOOL1_", false, true);
          Util.Log("Client 1 (pool locators) regions created");
        ...
        {
          m_client1.Call(CreateTCRegions_Pool2_WithPartitionResolver, RegionNames,
            CacheHelper.Endpoints, (string)null, "__TESTPOOL1_", false, true);
          Util.Log("Client 1 (pool endpoints) regions created");
          m_client2.Call(CreateTCRegions_Pool2_WithPartitionResolver, RegionNames,
            CacheHelper.Endpoints, (string)null, "__TESTPOOL1_", false, true);
          Util.Log("Client 2 (pool endpoints) regions created");
    ...
    void runTradeKeyResolver(bool pool, bool locator)
    {
      if (pool && locator)
      {
      ...
      {
        CacheHelper.SetupJavaServers(false, "cacheserver1_TradeKey.xml",
          "cacheserver2_TradeKey.xml", "cacheserver3_TradeKey.xml");
        CacheHelper.StartJavaServer(1, "GFECS1");
        Util.Log("Cacheserver 1 started.");
        CacheHelper.StartJavaServer(2, "GFECS2");
      ...
      if (pool)
      {
        if (locator)
        {
          m_client1.Call(CreateTCRegion2, TradeKeyRegion, true, true, TradeKeyResolver.Create(),
            (string)null,CacheHelper.Locators, true);
          Util.Log("Client 1 (pool locators) region created");          
        }
        else
        {
          m_client1.Call(CreateTCRegion2, TradeKeyRegion, true, true, TradeKeyResolver.Create(),
          CacheHelper.Endpoints, (string)null, true);
          Util.Log("Client 1 (pool endpoints) region created");          
        }
      }
    ...
    void runRemoveAll(bool pool, bool locator)
    {
      Util.Log("Starting iteration for pool " + pool + ", locator " + locator);
    ...
    void runRemoveAllWithSingleHop(bool pool, bool locator)
    {
      if (pool && locator)
      {
        CacheHelper.SetupJavaServers(true, "cacheserver1_pr.xml",
    ...
    void runIdictionaryOps(bool pool, bool locator)
    {
      Util.Log("Starting iteration for pool " + pool + ", locator " + locator);
    ...
    {
      runPdxAppDomainTest(true, true, false, true); // pool with locators
      runPdxAppDomainTest(true, true, true, true); // pool with locators
      runPdxAppDomainTest(true, true, false, false); // pool with locators
      runPdxAppDomainTest(true, true, true, false); // pool with locators
    }
    ...
    {
      runTradeKeyResolver(true, false); // pool with server endpoints
      runTradeKeyResolver(true, true); // pool with locators
    }
    ...
    {
      runRemoveAll(true, false); // pool with server endpoints
      runRemoveAll(true, true); // pool with locators
    }

    [Test]
    public void RemoveAllWithSingleHop()
    {
      runRemoveAllWithSingleHop(true, false); // pool with server endpoints
      runRemoveAllWithSingleHop(true, true); // pool with locators
    }
    ...
    {
      runIdictionaryOps(true, false); // pool with server endpoints
      runIdictionaryOps(true, true); // pool with locators
    }
