...
namespace GemStone.GemFire.Cache.UnitTests
{
  ...
  public class SerializationTests : ThinClientRegionSteps
  {
    ...
    public void CreateRegionForOT(string endpoints)
    {
      CacheHelper.CreateTCRegion(RegionNames[0], true, false,
        null, endpoints, false);
      Serializable.RegisterType(OtherType.CreateDeserializable);
      Serializable.RegisterType(OtherType2.CreateDeserializable);
      Serializable.RegisterType(OtherType22.CreateDeserializable);
      Serializable.RegisterType(OtherType4.CreateDeserializable);
      Serializable.RegisterType(OtherType42.CreateDeserializable);
      Serializable.RegisterType(OtherType43.CreateDeserializable);
    }
    ...
    public void DoNPuts(int n)
    {
      ...
      Region region = CacheHelper.GetVerifyRegion(RegionNames[0]);
      for (int i = 0; i < n; i++)
      {
    ...
    public void DoValidates(int n)
    {
      ...
      for (int i = 0; i < n; i++)
      {
        ...
        Assert.AreEqual(i, val.Value, "Found unexpected value");
      }
    }
    ...
    public void DoValidateNPutsOtherType(int n, int otherType)
    {
      ...
      for (int i = 0; i < n; i++)
      {
        IGFSerializable val = region.Get(i + 10);
        IGFSerializable ot = CreateOtherType(i, otherType);
    ...
    public void CustomTypes()
    {
      ...
      sender.Call(CreateRegionForOT, CacheHelper.Endpoints);
      Util.Log("StepOne complete.");

      receiver.Call(CreateRegionForOT, CacheHelper.Endpoints);
  ...
  public class OtherType : IGFSerializable
  {
    ...
    public static IGFSerializable Duplicate(IGFSerializable orig)
    {
      ...
      IGFSerializable dup = din.ReadObject();
      return dup;
    }
