...
namespace GemStone.GemFire.Cache.UnitTests.NewAPI
{
  ...
  public class PutGetTests : UnitTests
  {
    ...
    public void DoPRSHTradeResolverTasks(string rname)
    {
      ...
      Generic.IResultCollector<object> rc = exc.WithFilter<TradeKey>(filter).Execute(FEOnRegionPrSHOP_OptimizeForWrite, getResult, 15, true, true);
      ...
      rc = exc.WithFilter<TradeKey>(filter).Execute(FEOnRegionPrSHOP, getResult, 15, true, true);
      ...
      rc = exc.WithFilter<TradeKey>(filter).Execute(getFuncName, getResult);
      executeFunctionResult = rc.GetResult();
    ...
    public void DoRunQuery(bool pool)
    {
      ...
      {
        qs = m_region.Cache.GetQueryService<object, object>();
      }
         * */
    ...
    public void TestAllKeyValuePairs(ClientBase client1, ClientBase client2,
      string regionName, bool runQuery, bool pool, long dtTicks)
    {
