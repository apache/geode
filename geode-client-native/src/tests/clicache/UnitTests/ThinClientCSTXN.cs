...
namespace GemStone.GemFire.Cache.UnitTests.NewAPI
{
  ...
  */
  ...
  public class ThinClientCSTX : ThinClientRegionSteps
  {
    ...
    public void CreateRegion(string regionName, string endpoints, string locators,
      bool pool, bool locator, bool listener)
    {
      if (listener)
      ...
      if (pool)
      {
        region = CacheHelper.CreateTCRegion_Pool<object, object>(regionName, false, false,
          m_listener, endpoints, locators, "__TESTPOOL1_", false);
      }
    ...
    */
    ...
    void runThinClientCSTXTest(bool pool, bool locator)
    {
      CacheHelper.SetupJavaServers(pool && locator, "client_server_transactions.xml");
      if (pool && locator)
      {
      ...
      m_client1.Call(CreateRegion, cstxRegions[0], CacheHelper.Endpoints, CacheHelper.Locators,
              pool, locator, false);
      m_client1.Call(CreateRegion, cstxRegions[1], CacheHelper.Endpoints, CacheHelper.Locators,
        pool, locator, false);
      m_client1.Call(CreateRegion, cstxRegions[2], CacheHelper.Endpoints, CacheHelper.Locators,
        pool, locator, true);
    ...
    void runThinClientPersistentTXTest(bool pool, bool locator)
    {
      CacheHelper.SetupJavaServers(pool && locator, "client_server_persistent_transactions.xml");
      if (pool && locator)
      {
        CacheHelper.StartJavaLocator(1, "GFELOC");
        Util.Log("Locator started");
      ...
      {
        CacheHelper.StartJavaServer(1, "GFECS1", "-J-Dgemfire.ALLOW_PERSISTENT_TRANSACTIONS=true");
      }
    ...
    {
      runThinClientCSTXTest(true, false); //pool with server endpoints
      runThinClientCSTXTest(true, true); //pool with locator
    }

    [Test]
    public void ThinClientPersistentTXTest() // Bug ID #930 (https://svn.gemstone.com/trac/gfcpp/ticket/930)
    {
      runThinClientPersistentTXTest(true, false); //pool with server endpoints
      runThinClientPersistentTXTest(true, true); //pool with locator
    }
