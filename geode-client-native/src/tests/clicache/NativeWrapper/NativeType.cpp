...
{
#ifdef _MANAGED
  GemStone::GemFire::Cache::Log::Info("Invoked destructor of NativeType");
#endif
...
bool NativeType::doOp(int size, int numOps, int numGCOps)
{
  ...
  for (int i = 1; i <= numOps; ++i) {
#ifdef _MANAGED
    GemStone::GemFire::Cache::Log::Info("Allocating string number {0} with "
      "size {1}", i, size);
#endif
    ...
    if ((i % numGCOps) == 0) {
      GemStone::GemFire::Cache::Log::Info("Started GC collection.");
      System::GC::Collect();
      System::GC::WaitForPendingFinalizers();
      GemStone::GemFire::Cache::Log::Info("Completed GC collection.");
    }
