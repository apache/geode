...
namespace GemStone.GemFire.Cache.Tests
{
  ...
  public abstract class AuthzCredentialGenerator
  {
        ...
        string[] regionNames, int index)
    {
      ...
      {
        Properties principal = GetAllowedPrincipal(opCodes, regionNames,
        ...
        string[] regionNames, int index)
    {
      ...
      for (int tries = 0; tries < numTries; tries++)
      {
        Properties principal = GetDisallowedPrincipal(opCodes, regionNames,
            (index + tries) % numTries);
        try
        {
          return this.m_cGen.GetValidCredentials(principal);
    ...
    /// the given region
    /// </returns>
    protected abstract Properties GetAllowedPrincipal(OperationCode[] opCodes,
        string[] regionNames, int index);
    ...
    /// in the given region
    /// </returns>
    protected abstract Properties GetDisallowedPrincipal(OperationCode[] opCodes,
        string[] regionNames, int index);
  }
  ...
  public static class Utility
  {
    ...
    public static string GetServerArgs(string authenticator,
      string accessor, string accessorPP, Properties extraProps,
      Properties javaProps)
    {
      ...
      if (extraProps != null)
      {
        PropertyVisitor visitor =
          delegate(ICacheableKey key, IGFSerializable value)
          {
    ...
    public static void GetClientProperties(string authInit,
      Properties credentials, ref Properties props)
    {
