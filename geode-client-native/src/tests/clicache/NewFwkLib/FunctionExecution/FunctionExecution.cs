  ...
  using GemStone.GemFire.Cache.Generic;
    ...
    public class DoFETask<TKey, TVal> : ClientTask
    {
        ...
        bool addNewKeyFunction()
        {
            ...
            if (!isReplicate){
                executeFunctionResult = exc.WithArgs<ArrayList>(args).WithFilter<object>(filterObj).Execute(m_funcName, true).GetResult(); 
            }else
            {
                executeFunctionResult = exc.WithArgs<ArrayList>(args).Execute(m_funcName, true).GetResult();      
            }
...

  {
        ...
        if(!isReplicate){
          if(getresult == true){
            if(executionMode == "onRegion"){
              executeFunctionResult = exc.WithArgs<string>(ops).WithFilter<object>(filterObj).Execute(funcName, getresult, 15, true, true).GetResult();
            }else{
              args.Add(ops);
              executeFunctionResult = exc.WithArgs<ArrayList>(args).Execute(funcName, getresult, 15, true, true).GetResult();
            }
          }else {
            if(executionMode == "onRegion"){
              exc.WithArgs<string>(ops).WithFilter<object>(filterObj).Execute(funcName, getresult, 15, false, true);
            } else {
              args.Add(ops);
              exc.WithArgs<ArrayList>(args).Execute(funcName, getresult, 15, false, true);
            }
          }
        } else {
          args.Add(ops);
          ...
          {
            executeFunctionResult = exc.WithArgs<ArrayList>(args).Execute(funcName, getresult, 15, false, true).GetResult();
          }       
        }
        Thread.Sleep(30000);
        if (ops == "addKey")
    ...
    public void DoParitionedRegionFunctionExecution()
    {
      ...
      try
      {
        exc = Generic.FunctionService<object>.OnRegion<TKey, TVal>(region);
         ...
         // execute function
         Generic.IResultCollector<object> rc = exc.Execute("ExceptionHandlingFunction", true, 30, true, true);
    ...
    public void DoFireAndForgetFunctionExecution()
    {
      ...
      try
      {
        ...
        // execute function
        Generic.IResultCollector<object> rc = exc.Execute("FireNForget", false, 30, false);
      }
      catch (GemFire.Cache.Generic.FunctionExecutionException)
      {
        //expected exception
    ...
    public void DoExecuteFunctionsHA()
    {
      ...
      exc = exc.WithCollector(myRC);
      Generic.IResultCollector<object>  rc = exc.Execute(funcName, getresult, 120, true, true);
      ...
      public void DoGetServerKeys()
      {
          try
          {
              ...
              if (!isReplicate)
              {
                  executeFunctionResult = exc.WithArgs<ArrayList>(args).WithFilter<TKey>(keys).Execute(funcName, true).GetResult();
              }
              else
              {
                  executeFunctionResult = exc.WithArgs<ArrayList>(args).Execute(funcName, true).GetResult();
              }
              if (executeFunctionResult != null)
              {
