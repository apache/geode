...
namespace GemStone.GemFire.Cache.FwkLib
{
  ...
  public class CacheHelper<TKey, TVal>
  {
    ...
    {
      InitConfig(dsName, cacheName, null, null, null,false);
    }
    ...
    public static void InitConfig(Properties<string,string> dsProps, string endpoints,
      int redundancyLevel)
    {
    ...
    {
        InitConfig(DefaultDSName, DefaultCacheName, null, null, cacheXml, false);
    }

    public static void InitConfig(string dsName, string cacheName,
      Properties<string,string> config, CacheAttributes cAttrs,
      string cacheXml, bool PdxReadSerialized)
    {
      ...
      if (m_cache == null || m_cache.IsClosed)
      {
        ...
        try
        {
          ...
          {
            CacheHelper<TKey, TVal>.m_doDisconnect = true;
            ConnectConfig(dsName, config);
          ...
          else
          {
            ...
            else if (PdxReadSerialized)
            {
                ...
                m_cache = CacheFactory.CreateCacheFactory(config)
    ...
    public static string RegionTag(RegionAttributes<TKey, TVal> attrs)
    {
    ...
    public static string RegionAttributesToString(RegionAttributes<TKey, TVal> attrs)
    {
