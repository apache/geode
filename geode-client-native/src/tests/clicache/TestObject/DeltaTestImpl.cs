...
namespace GemStone.GemFire.Cache.Tests
{
  ...
  {
    private static byte INT_MASK = 0x1;
    private static byte STR_MASK = 0X2;
    private static byte DOUBLE_MASK = 0x4;
    private static byte BYTE_ARR_MASK = 0x8;
    private static byte TEST_OBJ_MASK = 0x10;
    //private static byte COMPLETE_MASK = 0x1F;
    ...
    private CacheableString str;
    private double doubleVar;
    //private byte[] byteArr = new byte[1];
    private CacheableBytes byteArr;
    ...
    public DeltaTestImpl()
    {
      ...
      byteArr = CacheableBytes.Create(arr2);
      testobj = null;
      hasDelta = false;
      deltaBits = 0;
      toDeltaCounter = 0;
    ...
    public DeltaTestImpl(Int32 intValue, CacheableString strValue)
    {
      this.intVar = intValue;
      this.str = strValue;
    }

    public DeltaTestImpl(Int32 intValue, CacheableString strValue, double doubleVal, CacheableBytes bytes, TestObject1 testObject)
    {
      this.intVar = intValue;
      this.str = strValue;
    ...
    public void SetStr(CacheableString str1)
    {
      str = str1;
    }

    public CacheableString GetStr()
    {
      return str;
    }
    ...
    public void SetByteArr(CacheableBytes value)
    {
      byteArr = value;
    ...
    public IGFSerializable FromData(DataInput input)
    {
      ...
      str = (CacheableString)input.ReadObject(); //.ReadUTF();
      doubleVar = input.ReadDouble();
      byteArr = (CacheableBytes)input.ReadObject();
    ...
    public void ToData(DataOutput output)
    {
      ...
      output.WriteObject(str);// WriteUTF(str);
    ...
    public void FromDelta(DataInput input)
    {
      ...
      if ((deltaBits & STR_MASK) == STR_MASK)
      {
        str = (CacheableString)input.ReadObject();
      }
      if ((deltaBits & DOUBLE_MASK) == DOUBLE_MASK)
      {
        doubleVar = input.ReadDouble();
