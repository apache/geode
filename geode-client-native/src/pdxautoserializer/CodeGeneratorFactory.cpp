...
namespace gemfire
{
  namespace pdx_auto_serializer
  {
    ...
    CodeGenerator* CodeGeneratorFactory::getInstance(const std::string&
                                                     generatorName) const
    {
    ...
    StringVector CodeGeneratorFactory::getGenerators() const
    {
      ...
      for (std::map<std::string, ASCodeGeneratorFn>::const_iterator
          generatorIterator = m_generatorMap.begin(); generatorIterator
          != m_generatorMap.end(); ++generatorIterator) {
