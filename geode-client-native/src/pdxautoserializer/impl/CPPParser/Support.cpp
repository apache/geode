...
CPPParser::qualifiedItemIs( int lookahead_offset )
{
  ...
  // DW 11/02/05 Note that LT(k)->getType() is not a "type" but a type of token, eg. ID
  //printf("support.cpp qualifiedItemIs while reached k %d %s type %d isType %d, isClass %d, guessing %d\n",
 ...
 * The symbol is added to external scope or whatever the current scope is, in the symbol table.
...
declaratorID( const char *id,QualifiedItem qi )
{
  ...
  CPPSymbol *c;
  //printf("support.cpp declaratorID line %d %s found, _ts = %d, _td = %d, qi = %d\n",
...
void CPPParser::
classForwardDeclaration( TypeSpecifier ts, FunctionSpecifier fs, const char *tag )
