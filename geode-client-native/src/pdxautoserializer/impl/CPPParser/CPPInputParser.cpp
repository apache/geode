...
namespace gemfire
{
  namespace pdx_auto_serializer
  {
    ...
    std::string CPPClassInfo::getName() const
    {
      return m_className;
    }

	std::string CPPClassInfo::getMethodPrefix() const {
		return m_method_prefix;
	}
    ...
    void CPPClassInfo::getReferences(ReferenceVector& references) const
    {
      ...
      /*ReferenceInfo builtinHeaderWriter = { "gfcpp/PdxWriter.hpp",  Reference::HEADER };
      references.push_back(builtinHeaderWriter);

      ReferenceInfo builtinHeaderReader = { "gfcpp/PdxWriter.hpp",  Reference::HEADER };
    ...
    void CPPHeaderParser::beginClassDefinition(TypeSpecifier ts,
                                               const char* tag)
    {
	  ...
	  if ( m_currentClassesVector.size() > 1 ) {
		  for ( std::vector<CPPClassInfo*>::iterator iter = m_currentClassesVector.begin(); iter != m_currentClassesVector.end() - 1; ++iter) {
	...
	void CPPHeaderParser::exitNamespaceScope() {
		m_namespaceVector.pop_back();
	}
    ...
    void CPPHeaderParser::endClassDefinition()
    {
      ...
      if (m_arraySizeRefMap.size() > 0) {
        for (VariableVector::iterator memberIterator =
            ...
            memberIterator != m_currentClassesVector.back()-> m_members.end(); memberIterator++) {
          StringMap::const_iterator findMember = m_arraySizeRefMap.find(
          ...
          StringMap::const_iterator findMemberElem = m_arrayElemSizeRefMap.find(
              memberIterator->m_name);
      ...
      m_classes[m_currentClassesVector.back()->getName()] = std::make_pair(static_cast<ClassInfo*>(m_currentClassesVector.back()),
          m_selectAll);
    ...
    {      
      ...
      if (m_currentClassesVector.size() != 0 && m_currentClassScope >= 0
        ...
        if (item == qiVar) { // check if this is a member field declaration
            if (m_currentInclude && 
                m_currentArraySizeRef.length() == 0 
          ...
          /*else if ( item == qiType ) {
			  ...
			  if ( NULL == strchr(m_currentArraySizeRef.c_str(),',')) {
				...
				char *p = strtok((char*)m_currentArraySizeRef.c_str(), ",");
				while (p) {
              ...
              //std::cout << "Map Entries --> " << "m_arrayElemSizeRefMap[" << m_currentArrayElemSizeRef <<"] = " << id << std::endl;
    ...
    void CPPHeaderParser::declarationSpecifier(bool td, bool fd,
                                               StorageClass sc,
                                               TypeQualifier tq,
                                               TypeSpecifier ts,
    ...
    CPPHeaderParser::CPPHeaderParser(CPPLexer& lexer, ASClassFlagMap& classes,
      ...
      CPPParser(lexer), m_classes(classes), m_selectAll(selectAll),
          m_currentClassesVector(std::vector<CPPClassInfo*>()), m_currentClassScope(-1),
          m_currentInclude(true), m_markIdentityField(false), m_markPdxUnreadField(false)
    {
    }
    ...
    InputParser* CPPInputParser::create()
    {
      return new CPPInputParser();
    }
