    ...
    namespace pdx_auto_serializer
    {
        ...
        */
        ...
        void CPPCodeGenerator::getOptions(OptionMap& options) const
        {
            ...
            optionPair.second = "\tThe output directory of the generated files "
                "(SINGLE,OPTIONAL)";
            options[s_OutDirOption] = optionPair;

            optionPair.second = "\tThe suffix of the generated filenames "
                "-- default is '" + defaultGenSuffix() + "' (SINGLE,OPTIONAL)";
        ...
        void CPPCodeGenerator::init(PropertyMap& properties)
        {
            ...
            bool classIdFound = Helper::getSingleProperty(properties,
                s_ClassIdOption, classIdStr);
            if (!classIdFound) {
        ...
        void CPPCodeGenerator::initClass(const TypeInfo& classInfo)
        {
			...
			for ( StringVector::iterator iter = nameSpaceList.begin(); iter != nameSpaceList.end(); ++iter) {
			...
			{
				outFile = m_outDir + namespacePrefixString + "_" + classInfo.m_nameOrSize + m_genSuffix + ".cpp";
			}
        ...
        void CPPCodeGenerator::addFileHeader(int argc, char** argv) {
          *m_cppFormatter << "// This is auto generated file using \"pdxautoserializer\"" << "\n";
                ...
                referenceIterator != references.end(); ++referenceIterator) {
                    if (referenceIterator->m_kind == Reference::HEADER) {
                        ...
                        if (lastSlash != std::string::npos) {
                            headerPath = m_headerDir + referenceIterator->m_path.substr(
                                lastSlash + 1);
                        }
                        else {
            ...
            const std::string& varName, const std::string& methodPrefix)
        {
            ...
            default: {
                throw std::invalid_argument("unexpected execution");
                     }
            }
        }

        void CPPCodeGenerator::genMethod(const Method::Type type,const std::string& varName, const VariableInfo& var)
        {
            switch (type) 
            {
              case Method::TODATA: 
              {
                ...
                {
                  *m_cppFormatter << s_GFSerializerNamespace 
                      << "::writePdxObject(" 
                      << varName 
                      << ", " 
                      << "\"" << var.m_name << "\""
              ...
              case Method::FROMDATA: 
              {
                if (var.m_markPdxUnreadField == true) 
                {
                  *m_cppFormatter << var.m_name << " = " << varName << "->readUnreadFields(";
                } 
                else 
                {
                  *m_cppFormatter   << s_GFSerializerNamespace 
        ...
        void CPPCodeGenerator::genClassNameMethod(std::map<std::string, std::string> &classNameStringMap, const std::string& methodPrefix)
        {
            StringVector varVec;
            std::string className = getTypeString(m_classInfo);
            std::map<std::string, std::string>::iterator found = classNameStringMap.find(className);
            genFunctionHeader("getClassName", className, "const char*", varVec, true, true,m_cppFormatter, methodPrefix);

            if ( found != classNameStringMap.end() ) {
                *m_cppFormatter << "return " << "\"" << found->second << "\"" << ";\n";
            } else {
        ...
        void CPPCodeGenerator::genCreateDeserializable(const std::string& methodPrefix) {
            StringVector varVec;
            std::string className = getTypeString(m_classInfo);
            genFunctionHeader("createDeserializable", className, "gemfire::PdxSerializable*", varVec, true, false,
                m_cppFormatter, methodPrefix);
        ...
        std::string CPPCodeGenerator::getNamespacePrefix(const StringVector&
            namespaces) const
        {
            ...
            StringSet* templateArgs) const
        {
            ...
            if (type.m_kind & TypeKind::VALUE) {
                ...
                typeString += getTypeString(*(type.m_children), prependNS, postVarStr,
                    templateArgs);
                ...
                if (type.m_numChildren > 0) {
                    typeString += getTypeString(*type.m_children, prependNS, postVarStr,
                        templateArgs);
                    for (int childIndex = 1; childIndex < type.m_numChildren;
                        ++childIndex) {
                            typeString += ", ";
                            typeString += getTypeString(type.m_children[childIndex],
                                prependNS, postVarStr, templateArgs);
                    }
                }
                typeString += " >";
            }
        ...
        CodeGenerator* CPPCodeGenerator::create()
        {
            return new CPPCodeGenerator();
        }

        CPPCodeGenerator::CPPCodeGenerator() :
        m_cppFormatter(new OutputFormatter()), m_outDir("."), m_moduleName(
            "CPPCodeGenerator")
        {
        }
