...
namespace gemfire
{
  namespace pdx_auto_serializer
  {
                                      ...
                                      const std::string& replaceStr)
    {
      ...
      while (srcIterator != source.end()) {
        ...
        while (findIterator != findStr.end()) {
          if (matchIterator == source.end() || *matchIterator++
              != *findIterator++) {
            matchFound = false;
            break;
          }
    ...
    bool Helper::getSingleProperty(PropertyMap& properties,
    ...
    bool Helper::getMultiProperty(PropertyMap& properties,
                                  const std::string& name, StringVector& value)
    {
      PropertyMap::iterator propertyFind = properties.find(name);
      if (propertyFind != properties.end()) {
