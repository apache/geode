...
public class VersionDescription {
  ...
  public VersionDescription(String name, ModuleService moduleService) {
    ServiceResult<List<InputStream>> resourceAsStream = moduleService.findResourceAsStream(name);

    if (resourceAsStream.isFailure()) {
      error = Optional
    ...
    InputStream is = resourceAsStream.getMessage().get(0);
