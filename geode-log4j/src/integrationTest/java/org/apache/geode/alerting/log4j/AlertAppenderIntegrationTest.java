...
public class AlertAppenderIntegrationTest {
  ...
  public void setUp() {
    ...
    alertingService.useAlertMessaging(new ClusterAlertMessaging(internalDistributedSystem));
  ...
  public void getLogEventsReturnsLoggedEvents() {
    ...
    assertThat(theLogEvent.getLevel()).isEqualTo(Level.WARN);
  }
  ...
  public void sendsAlertIfListenerExists() {
    ...
    assertThat(alertAppender.getLogEvents()).hasSize(1);
    verify(alertingService).sendAlerts(eq(AlertLevel.WARNING), any(Date.class), anyString(),
        anyString(), isNull());
  }
}
