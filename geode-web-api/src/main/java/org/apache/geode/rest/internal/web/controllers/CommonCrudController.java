...
public abstract class CommonCrudController extends AbstractBaseController {
  ...
  @ApiOperation(value = "list all resources (Regions)", notes = "List all available resources (Regions) in the Geode cluster", response = void.class)
  @ApiResponses({
   ...
   * @return JSON document containing result
   */
  @RequestMapping(method = RequestMethod.DELETE, value = "/{region}/{keys}", produces = { MediaType.APPLICATION_JSON_VALUE })
  ...
  public ResponseEntity<?> delete(@PathVariable("region") String region, @PathVariable("keys") final String[] keys) {
