...
public class InstallerJUnitTest {
  ...
  private void testTransformation(final String name) throws Exception {
    ...
    String expected = IOUtils.toString(getClass().getResource(name + ".expected")).replaceAll(IOUtils.LINE_SEPARATOR_WINDOWS, "").replaceAll(IOUtils.LINE_SEPARATOR_UNIX, "");
    String actual = output.toString().replaceAll(IOUtils.LINE_SEPARATOR_WINDOWS, "").replaceAll(IOUtils.LINE_SEPARATOR_UNIX, "");
