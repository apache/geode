 ...
 * <p>
 * The class maintains a list of distinct, unassigned test contexts. The list is created just
 * before the first processor launches a JVM, and initially includes enough contexts to supply
 * the maximum possible number of concurrent test JVMs. Before it launches a JVM, each processor
 * removes an unassigned context from the list and uses it to configure the Java command that
 * will launch the JVM. After a processor stops a JVM, it adds the JVM's assigned test context to
 * the unassigned contexts list, making the context available for assignment to future JVMs.
 */
public class IsolatingForkingTestClassProcessor implements TestClassProcessor {
  ...
  private static void initializeTestContexts(int numberOfContexts) {
    FOO_AVAILABLE_TEST_CONTEXTS.updateAndGet(currentValue -> {
      ...
      return new ConcurrentLinkedDeque<>(ParallelTestContext.create(numberOfContexts));
    });
  }
}
