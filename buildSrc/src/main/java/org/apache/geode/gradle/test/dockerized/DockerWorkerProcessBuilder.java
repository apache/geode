...
import static org.apache.geode.gradle.test.dockerized.Reflection.getField;
import static org.apache.geode.gradle.test.dockerized.Reflection.getFieldValue;
import static org.apache.geode.gradle.test.dockerized.Reflection.setFieldValue;
...
/**
 * Wraps a worker process builder to make it build a dockerized worker process.
 */
public class DockerWorkerProcessBuilder implements WorkerProcessBuilder {
  ...
  public DockerWorkerProcessBuilder(WorkerProcessBuilder delegate, DockerProcessLauncher processLauncher) {
  ...
  private synchronized Object assignableProcessLauncher(Class<?> requiredType) {
    if(processLauncherProxy == null) {
      // Assume that only start() will be called, and simply delegate it to our processLauncher
      InvocationHandler handler = (proxy, method, args) -> processLauncher.start((ProcessBuilder) args[0]);
