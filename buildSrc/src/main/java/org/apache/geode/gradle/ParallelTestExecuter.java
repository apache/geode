...
public class ParallelTestExecuter implements TestExecuter<JvmTestExecutionSpec> {
  ...
  public void execute(final JvmTestExecutionSpec testExecutionSpec,
    ...
    System.out.printf("DHE: %s executing tests %s with maxProcessors %d and maxWorkerCount %d%n",
        getClass().getSimpleName(), testExecutionSpec.getPath(), maxProcessors, maxWorkerCount);
    ...
    TestClassProcessor runPreviousFailedTestsFirst =
        new RunPreviousFailedFirstTestClassProcessor(
            testExecutionSpec.getPreviousFailedTestClasses(), runNTestsInParallel);

    processor = new PatternMatchTestClassProcessor(testFilter, runPreviousFailedTestsFirst);
