...
public class GenerateTestModuleDescriptorsTask extends DefaultTask {
  ...
  @TaskAction
  public void run() {
    JBossModuleDescriptorGenerator
        jBossModuleDescriptorGenerator =
        new JBossModuleDescriptorGenerator();

    Collection<String> resources = generateResourceRoots(getProject());
    resources.addAll(getProject().getConfigurations().getByName("distributedTestRuntimeClasspath")
        .getResolvedConfiguration().getResolvedArtifacts().stream().map(resolvedArtifact -> {
    ...
    modules.add(new ModuleDependency(DEFAULT_APPLICATION, false, false));
    ...
    jBossModuleDescriptorGenerator.generate(getRootPath(), DEFAULT_APPLICATION, getProject().getVersion().toString(),
        Collections.emptyList(), Collections.emptyList(), null, Collections.emptyList(),
        Collections.emptyList(), Collections.emptyList());

    jBossModuleDescriptorGenerator.generateAlias(getRootPath(), DEFAULT_APPLICATION, getProject().getVersion().toString());
  }
  ...
  private Collection<String> generateResourceRoots(Project project) {
    ...
    for (String facet : facets) {
      ...
      validateAndAddResourceRoot(resourceRoots,
          project.getBuildDir().toPath().resolve("generated-resources").resolve(facet).toString());
    }
