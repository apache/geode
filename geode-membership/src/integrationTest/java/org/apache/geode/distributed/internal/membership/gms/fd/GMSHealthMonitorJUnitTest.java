...
public class GMSHealthMonitorJUnitTest {
  ...
  public void heartbeatOnTimeWakeupCausesNoWarning() {
    testHeartbeatSleepScenario(period -> 2 * period,
        null);
  }
  ...
  private void testHeartbeatSleepScenario(final LongUnaryOperator actualSleepPeriod,
    ...
    new Runnable() {
      ...
      // the first two values returned by our faked-out timer
      final long[] times = {0L, actualSleepPeriod.applyAsLong(heart.sleepPeriodNanos)};
      ...
      public void run() {
        heart.sendPeriodicHeartbeats(sleepMillis -> {
            ...
            () -> {
              if (periodNumber < times.length) {
                return times[periodNumber++];
              } else {
                gmsHealthMonitor.stop();
