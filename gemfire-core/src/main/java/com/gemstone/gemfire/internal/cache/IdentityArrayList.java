 ...
 */
public class IdentityArrayList  <K> extends AbstractObjectList <K> implements RandomAccess, Cloneable, java.io.Serializable {
 ...
 public int indexOf( final Object k ) {
  for( int i = 0; i < size; i++ ) if ( ( (k) == null ? (a[ i ]) == null : (k) == (a[ i ]) ) ) return i;
  return -1;
 }
 public int lastIndexOf( final Object k ) {
  for( int i = size; i-- != 0; ) if ( ( (k) == null ? (a[ i ]) == null : (k) == (a[ i ]) ) ) return i;
  return -1;
 }
 ...
 /**
  * For highest performance we sometimes must iterate over
  * the actual Object array.  This should only be used in
